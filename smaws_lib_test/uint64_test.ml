...
let compare_unsigned () =
  ...
  Alcotest.(check int) "2^63 < 2^63+1" (-1) (U.compare (of_i64 two_63) (of_i64 Int64.(add two_63 1L)));
...
let add_wraps () =
  ...
  Alcotest.(check string) "2^63 + 2^63 = 0" "0" (U.to_string (U.add (of_i64 two_63) (of_i64 two_63)))

let mul_wraps () =
  (* (2^32) * (2^32) = 2^64 = 0 mod 2^64 *)
  Alcotest.(check bool) "2^32 * 2^32 = 0" true
    (U.equal (U.mul (U.of_int 4294967296) (U.of_int 4294967296)) U.zero)

let div_rem () =
  Alcotest.(check string) "max / 10" "1844674407370955161"
  ...
  Alcotest.(check string) "max % 2^63" "9223372036854775807"
...
let div_by_zero_raises () =
  (try ignore (U.div U.one U.zero); Alcotest.fail "expected Division_by_zero"
   with Division_by_zero -> ());
  (try ignore (U.rem U.one U.zero); Alcotest.fail "expected Division_by_zero"
   with Division_by_zero -> ())
...
let get_set_byte () =
  ...
  Alcotest.(check bool) "set_byte bits" true
    (Int64.equal (to_i64 x) 0xAB00000000000000L)

let shifts_and_logic () =
  Alcotest.(check string) "1 << 63" "9223372036854775808"
    (U.to_string (U.shift_left U.one 63));
  ...
  Alcotest.(check bool) "logand" true
...
let to_int_overflow () =
  ...
  (try ignore (U.of_int (-1)); Alcotest.fail "expected Invalid_argument on negative"
   with Invalid_argument _ -> ())
...
let () =
  Alcotest.run "UInt64"
    [
      ...
      ( "bytes",
        [ ("to_bytes/of_bytes roundtrip", `Quick, bytes_roundtrip); ("get/set byte", `Quick, get_set_byte) ] );
