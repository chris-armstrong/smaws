...
let test_compress_gzip_basic () =
  ...
  Alcotest.(check bool) "compression produces different output" false (String.equal input compressed);
  (* Check that result contains gzip magic bytes *)
  Alcotest.(check bool) "contains gzip magic bytes" true
...
let test_compress_gzip_empty () =
  ...
  Alcotest.(check bool) "empty string compression has gzip headers" true
    (String.length compressed >= 2 &&
     Char.code (String.get compressed 0) = 0x1f &&
     Char.code (String.get compressed 1) = 0x8b)

let test_compress_gzip_roundtrip () =
  let input = "This is a test string for compression. It should compress well because it has repeating patterns. Repeating patterns. Repeating patterns." in
  let compressed = compress_gzip input in
  (* Verify it's actually compressed (should be smaller for repetitive text) *)
  Alcotest.(check bool) "compression reduces size" true (String.length compressed < String.length input);
  (* Verify gzip headers are present *)
  Alcotest.(check bool) "has gzip magic bytes" true
    (String.length compressed >= 2 &&
     Char.code (String.get compressed 0) = 0x1f &&
     Char.code (String.get compressed 1) = 0x8b);
