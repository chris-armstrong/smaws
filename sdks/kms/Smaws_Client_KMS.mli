...
[@@ocaml.doc
  "Cancels the deletion of a KMS key. When this operation succeeds, the key state of the KMS key \
   is [Disabled]. To enable the KMS key, use [EnableKey]. \n\n\
...
[@@ocaml.doc
  "Connects or reconnects a \
   ...
   store} to its backing key store. For an CloudHSM key store, [ConnectCustomKeyStore] connects \
   the key store to its associated CloudHSM cluster. For an external key store, \
   [ConnectCustomKeyStore] connects the key store to the external key store proxy that \
  ...
  \    This operation is part of the custom key stores feature in KMS, which combines the \
   convenience and extensive integration of KMS with the isolation and control of a key store that \
   you own and manage.\n\
  \   \n\
  \    The [ConnectCustomKeyStore] operation might fail for various reasons. To find the reason, \
   use the [DescribeCustomKeyStores] operation and see the [ConnectionErrorCode] in the response. \
   For help interpreting the [ConnectionErrorCode], see [CustomKeyStoresListEntry].\n\
  \    \n\
  \     To fix the failure, use the [DisconnectCustomKeyStore] operation to disconnect the custom \
   key store, correct the error, use the [UpdateCustomKeyStore] operation if necessary, and then \
   use [ConnectCustomKeyStore] again.\n\
  \     \n\
  \       {b CloudHSM key store} \n\
  \      \n\
  \       During the connection process for an CloudHSM key store, KMS finds the CloudHSM cluster \
   that is associated with the custom key store, creates the connection infrastructure, connects \
   to the cluster, logs into the CloudHSM client as the [kmsuser] CU, and rotates its password.\n\
  \       \n\
  \        To connect an CloudHSM key store, its associated CloudHSM cluster must have at least \
   one active HSM. To get the number of active HSMs in a cluster, use the \
   ...
   [kmsuser] crypto user} (CU) must not be logged into the cluster. This prevents KMS from using \
   this account to log in.\n\
  \        \n\
  \         If you are having trouble connecting or disconnecting a CloudHSM key store, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html}Troubleshooting an \
   CloudHSM key store} in the {i Key Management Service Developer Guide}.\n\
  \         \n\
  \           {b External key store} \n\
  \          \n\
  \           When you connect an external key store that uses public endpoint connectivity, KMS \
   tests its ability to communicate with your external key manager by sending a request via the \
   external key store proxy.\n\
  \           \n\
  \            When you connect to an external key store that uses VPC endpoint service \
   connectivity, KMS establishes the networking elements that it needs to communicate with your \
   external key manager via the external key store proxy. This includes creating an interface \
   endpoint to the VPC endpoint service and a private hosted zone for traffic between KMS and the \
   VPC endpoint service.\n\
  \            \n\
  \             To connect an external key store, KMS must be able to connect to the external key \
   store proxy, the external key store proxy must be able to communicate with your external key \
  ...
  \              If you are having trouble connecting or disconnecting an external key store, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/xks-troubleshooting.html}Troubleshooting \
   an external key store} in the {i Key Management Service Developer Guide}.\n\
  \              \n\
  \                {b Cross-account use}: No. You cannot perform this operation on a custom key \
...
[@@ocaml.doc
  ...
  \  Adding, deleting, or updating an alias can allow or deny permission to the KMS key. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/abac.html}ABAC for KMS} \
   in the {i Key Management Service Developer Guide}.\n\
  \  \n\
  \    You can use an alias to identify a KMS key in the KMS console, in the [DescribeKey] \
   ...
   associated with the alias ([UpdateAlias]) or delete the alias ([DeleteAlias]) at any time. \
  ...
  \     You can associate the alias with any customer managed key in the same Amazon Web Services \
   Region. Each alias is associated with only one KMS key at a time, but a KMS key can have \
   multiple aliases. A valid KMS key is required. You can't create an alias without a KMS key.\n\
  \     \n\
  \      The alias must be unique in the account and Region, but you can have aliases with the \
   same name in different Regions. For detailed information about aliases, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-alias.html}Aliases in KMS} in the \
   {i Key Management Service Developer Guide}.\n\
  \      \n\
  ...
  \        \n\
  \          {b Cross-account use}: No. You cannot perform this operation on an alias in a \
  ...
  \                 }\n\
  \   For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/alias-access.html}Controlling access \
   to aliases} in the {i Key Management Service Developer Guide}.\n\
  \   \n\
  \     {b Related operations:} \n\
  \    \n\
  \     {ul\n\
...
[@@ocaml.doc
  "Creates a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   store} backed by a key store that you own and manage. When you use a KMS key in a custom key \
   store for a cryptographic operation, the cryptographic operation is actually performed in your \
   key store using your keys. KMS supports \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html}CloudHSM key \
   stores} backed by an \
   {{:https://docs.aws.amazon.com/cloudhsm/latest/userguide/clusters.html}CloudHSM cluster} and \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-external.html}external key \
   stores} backed by an external key store proxy and external key manager outside of Amazon Web \
   Services.\n\n\
  \  This operation is part of the custom key stores feature in KMS, which combines the \
   convenience and extensive integration of KMS with the isolation and control of a key store that \
   you own and manage.\n\
  ...
  \  Before you create the custom key store, the required elements must be in place and \
   ...
   verification tests, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/create-keystore.html#before-keystore}Assemble \
   the prerequisites (for CloudHSM key stores)} or \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/create-xks-keystore.html#xks-requirements}Assemble \
   the prerequisites (for external key stores)} in the {i Key Management Service Developer Guide}.\n\
  \  \n\
  \   To create a custom key store, use the following parameters.\n\
  \   \n\
  \    {ul\n\
  \          {-  To create an CloudHSM key store, specify the [CustomKeyStoreName], \
   [CloudHsmClusterId], [KeyStorePassword], and [TrustAnchorCertificate]. The [CustomKeyStoreType] \
   parameter is optional for CloudHSM key stores. If you include it, set it to the default value, \
   [AWS_CLOUDHSM]. For help with failures, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html}Troubleshooting an \
   CloudHSM key store} in the {i Key Management Service Developer Guide}.\n\
  \              \n\
  \               }\n\
  \          {-  To create an external key store, specify the [CustomKeyStoreName] and a \
   [CustomKeyStoreType] of [EXTERNAL_KEY_STORE]. Also, specify values for [XksProxyConnectivity], \
   [XksProxyAuthenticationCredential], [XksProxyUriEndpoint], and [XksProxyUriPath]. If your \
   [XksProxyConnectivity] value is [VPC_ENDPOINT_SERVICE], specify the \
  ...
  \      When creating an external key store in the KMS console, you can upload a JSON-based proxy \
   ...
   determine the correct values for the [CreateCustomKeyStore] parameters.\n\
  \      \n\
  \        When the operation completes successfully, it returns the ID of the new custom key \
   store. Before you can use your new custom key store, you need to use the \
   [ConnectCustomKeyStore] operation to connect a new CloudHSM key store to its CloudHSM cluster, \
   or to connect a new external key store to the external key store proxy for your external key \
   manager. Even if you are not going to use your custom key store immediately, you might want to \
   connect it to verify that all settings are correct and then disconnect it until you are ready \
   to use it.\n\
  \        \n\
  \          {b Cross-account use}: No. You cannot perform this operation on a custom key store in \
...
[@@ocaml.doc
  ...
  \ A {i grant} is a policy instrument that allows Amazon Web Services principals to use KMS keys \
   in cryptographic operations. It also can allow them to view a KMS key ([DescribeKey]) and \
   create and manage grants. When authorizing access to a KMS key, grants are considered along \
   with key policies and IAM policies. Grants are often used for temporary permissions because you \
...
[@@ocaml.doc
  "Creates a unique customer managed \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms-keys}KMS key} in \
   your Amazon Web Services account and Region. You can use a KMS key in cryptographic operations, \
   such as encryption and signing. Some Amazon Web Services services let you use KMS keys that you \
   create and manage to protect your service resources.\n\n\
  \ A KMS key is a logical representation of a cryptographic key. In addition to the key material \
   used in cryptographic operations, a KMS key includes metadata, such as the key ID, key policy, \
  ...
  \      \n\
  \        Symmetric encryption KMS key  By default, [CreateKey] creates a symmetric encryption \
   KMS key with key material that KMS generates. This is the basic and most widely used type of \
   KMS key, and provides the best performance.\n\
  \                                      \n\
  \                                       To create a symmetric encryption KMS key, you don't need \
   to specify any parameters. The default value for [KeySpec], [SYMMETRIC_DEFAULT], the default \
   value for [KeyUsage], [ENCRYPT_DECRYPT], and the default value for [Origin], [AWS_KMS], create \
   a symmetric encryption KMS key with KMS key material.\n\
  \                                       \n\
  \                                        If you need a key for basic encryption and decryption \
   or you are creating a KMS key to protect your resources in an Amazon Web Services service, \
   create a symmetric encryption KMS key. The key material in a symmetric encryption key never \
   leaves KMS unencrypted. You can use a symmetric encryption KMS key to encrypt and decrypt data \
   up to 4,096 bytes, but they are typically used to generate data keys and data keys pairs. For \
   details, see [GenerateDataKey] and [GenerateDataKeyPair].\n\
  \                                        \n\
  \                                          \n\
  \                                         \n\
  \                                           Asymmetric KMS keys  To create an asymmetric KMS \
   key, use the [KeySpec] parameter to specify the type of key material in the KMS key. Then, use \
   the [KeyUsage] parameter to determine whether the KMS key will be used to encrypt and decrypt \
   or sign and verify. You can't change these properties after the KMS key is created.\n\
  \                                                                \n\
  \                                                                 Asymmetric KMS keys contain an \
   RSA key pair, Elliptic Curve (ECC) key pair, ML-DSA key pair or an SM2 key pair (China Regions \
   ...
   only) can be used to either encrypt and decrypt data, sign and verify messages, or derive \
   shared secrets (you must choose one key usage type). For information about asymmetric KMS keys, \
   see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}Asymmetric \
   KMS keys} in the {i Key Management Service Developer Guide}.\n\
  \                                                                 \n\
  \                                                                   \n\
  \                                                                  \n\
  \                                                                    HMAC KMS key  \n\
   To create an HMAC KMS key, set the [KeySpec] parameter to a key spec value for HMAC KMS keys. \
   Then set the [KeyUsage] parameter to [GENERATE_VERIFY_MAC]. You must set the key usage even \
   though [GENERATE_VERIFY_MAC] is the only valid key usage value for HMAC KMS keys. You can't \
   change these properties after the KMS key is created.\n\n\
  \ HMAC KMS keys are symmetric keys that never leave KMS unencrypted. You can use HMAC keys to \
   generate ([GenerateMac]) and verify ([VerifyMac]) HMAC codes for messages up to 4096 bytes.\n\
  \ \n\
  \   \n\
  \  \n\
  ...
  \                                                           To import your own key material into \
   ...
   material. Then, use [ImportKeyMaterial] with your import token to import the key material. For \
   step-by-step instructions, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html}Importing Key \
   Material} in the {i  {i Key Management Service Developer Guide} }.\n\
  \                                                           \n\
  \                                                            You can import key material into \
   KMS keys of all supported KMS key types: symmetric encryption KMS keys, HMAC KMS keys, \
   asymmetric encryption KMS keys, and asymmetric signing KMS keys. You can also create \
   multi-Region keys with imported key material. However, you can't import key material into a KMS \
   key in a custom key store.\n\
  \                                                            \n\
  \                                                             To create a multi-Region primary \
   key with imported key material, use the [Origin] parameter of [CreateKey] with a value of \
   [EXTERNAL] and the [MultiRegion] parameter with a value of [True]. To create replicas of the \
   multi-Region primary key, use the [ReplicateKey] operation. For instructions, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-create-cmk.html \
   }Importing key material step 1}. For more information about multi-Region keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html}Multi-Region \
   keys in KMS} in the {i Key Management Service Developer Guide}.\n\
  \                                                             \n\
  \                                                               \n\
  \                                                              \n\
  \                                                                Custom key store  \n\
   A {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   store} lets you protect your Amazon Web Services resources using keys in a backing key store \
   that you own and manage. When you request a cryptographic operation with a KMS key in a custom \
   key store, the operation is performed in the backing key store using its cryptographic keys.\n\n\
  \ KMS supports \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html}CloudHSM key \
   stores} backed by an CloudHSM cluster and \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-external.html}external key \
   stores} backed by an external key manager outside of Amazon Web Services. When you create a KMS \
   key in an CloudHSM key store, KMS generates an encryption key in the CloudHSM cluster and \
   associates it with the KMS key. When you create a KMS key in an external key store, you specify \
  ...
  \      To create a KMS key in a custom key store, use the [CustomKeyStoreId]. Use the default \
   [KeySpec] value, [SYMMETRIC_DEFAULT], and the default [KeyUsage] value, [ENCRYPT_DECRYPT] to \
   create a symmetric encryption key. No other key type is supported in a custom key store.\n\
  \      \n\
  \       To create a KMS key in an \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/create-cmk-keystore.html}CloudHSM key \
   store}, use the [Origin] parameter with a value of [AWS_CLOUDHSM]. The CloudHSM cluster that is \
  ...
  \       \n\
  \        To create a KMS key in an \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/create-xks-keys.html}external key \
   store}, use the [Origin] parameter with a value of [EXTERNAL_KEY_STORE] and an [XksKeyId] \
  ...
  \          \n\
  \               {b Cross-account use}: No. You cannot use this operation to create a KMS key in \
   a different Amazon Web Services account.\n\
  \              \n\
  \                {b Required permissions}: \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/customer-managed-policies.html#iam-policy-example-create-key}Allow \
   a user to create KMS keys} in the {i Key Management Service Developer Guide}.\n\
  \               \n\
  \                 {b Related operations:} \n\
  \                \n\
  \                 {ul\n\
...
[@@ocaml.doc
  ...
  \   You can use this operation to decrypt ciphertext that was encrypted under a symmetric \
   ...
   For information about asymmetric KMS keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}Asymmetric \
   KMS keys} in the {i Key Management Service Developer Guide}.\n\
  \   \n\
  \    The [Decrypt] operation also decrypts ciphertext that was encrypted outside of KMS by the \
   public key in an KMS asymmetric KMS key. However, it cannot decrypt symmetric ciphertext \
   produced by other libraries, such as the \
   {{:https://docs.aws.amazon.com/encryption-sdk/latest/developer-guide/}Amazon Web Services \
   Encryption SDK} or \
   {{:https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingClientSideEncryption.html}Amazon S3 \
   client-side encryption}. These libraries return a ciphertext format that is incompatible with \
   KMS.\n\
  \    \n\
  \     If the ciphertext was encrypted under a symmetric encryption KMS key, the [KeyId] \
   parameter is optional. KMS can get this information from metadata that it adds to the symmetric \
   ...
   When you use the [KeyId] parameter to specify a KMS key, KMS only uses the KMS key you specify. \
   If the ciphertext was encrypted under a different KMS key, the [Decrypt] operation fails. This \
  ...
  \      Whenever possible, use key policies to give users permission to call the [Decrypt] \
   operation on a particular KMS key, instead of using IAM policies. Otherwise, you might create \
   an IAM policy that gives the user [Decrypt] permission on all KMS keys. This user could decrypt \
   ciphertext that was encrypted by KMS keys in other accounts if the key policy for the \
   cross-account KMS key permits it. If you must use an IAM policy for [Decrypt] permissions, \
   limit the user to particular KMS keys or particular trusted accounts. For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/iam-policies.html#iam-policies-best-practices}Best \
   practices for IAM policies} in the {i Key Management Service Developer Guide}.\n\
  \      \n\
...
[@@ocaml.doc
  ...
  \  Adding, deleting, or updating an alias can allow or deny permission to the KMS key. For \
   ...
   in the {i Key Management Service Developer Guide}.\n\
  \  \n\
  \    Because an alias is not a property of a KMS key, you can delete and change the aliases of a \
   KMS key without affecting the KMS key. Also, aliases do not appear in the response from the \
  ...
  \     Each KMS key can have multiple aliases. To change the alias of a KMS key, use \
   [DeleteAlias] to delete the current alias and [CreateAlias] to create a new alias. To associate \
   an existing alias with a different KMS key, call [UpdateAlias].\n\
  \     \n\
  \       {b Cross-account use}: No. You cannot perform this operation on an alias in a different \
   Amazon Web Services account.\n\
  \      \n\
  \        {b Required permissions} \n\
  \       \n\
  ...
  \              }\n\
  \   For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-alias.html#alias-access}Controlling \
   access to aliases} in the {i Key Management Service Developer Guide}.\n\
  \   \n\
  \     {b Related operations:} \n\
  \    \n\
  \     {ul\n\
...
[@@ocaml.doc
  "Deletes a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   store}. This operation does not affect any backing elements of the custom key store. It does \
   not delete the CloudHSM cluster that is associated with an CloudHSM key store, or affect any \
   users or keys in the cluster. For an external key store, it does not affect the external key \
  ...
  \  The custom key store that you delete cannot contain any \
   ...
   Before deleting the key store, verify that you will never need to use any of the KMS keys in \
   the key store for any \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations}cryptographic \
   ...
   [DisconnectCustomKeyStore] to disconnect the key store from KMS. Then, you can delete the \
   custom key store.\n\
  \  \n\
  \   For keys in an CloudHSM key store, the [ScheduleKeyDeletion] operation makes a best effort \
   to delete the key material from the associated cluster. However, you might need to manually \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key}delete \
   the orphaned key material} from the cluster and its backups. KMS never creates, manages, or \
   deletes cryptographic keys in the external key manager associated with an external key store. \
   You must manage them using your external key manager tools.\n\
  \   \n\
  \    Instead of deleting the custom key store, consider using the [DisconnectCustomKeyStore] \
   operation to disconnect the custom key store from its backing key store. While the key store is \
  ...
  \     \n\
  \       {b Cross-account use}: No. You cannot perform this operation on a custom key store in a \
   different Amazon Web Services account.\n\
  \      \n\
  \        {b Required permissions}: \
...
[@@ocaml.doc
  "Deletes key material that was previously imported. This operation makes the specified KMS key \
   temporarily unusable. To restore the usability of the KMS key, reimport the same key material. \
   For more information about importing key material into KMS, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html}Importing Key \
   Material} in the {i Key Management Service Developer Guide}.\n\n\
...
[@@ocaml.doc
  ...
  \    DeriveSharedSecret uses the \
   ...
   Curve Cryptography Cofactor Diffie-Hellman Primitive} (ECDH) to establish a key agreement \
   between two peers by deriving a shared secret from their elliptic curve public-private key \
   pairs. You can use the raw shared secret that DeriveSharedSecret returns to derive a symmetric \
   key that can encrypt and decrypt data that is sent between the two peers, or that can generate \
   and verify HMACs. KMS recommends that you follow \
   {{:https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Cr2.pdf}NIST \
  ...
  \                \n\
  \                 KMS uses the private key from the KMS key pair generated in {b Step 1}, Bob's \
   public key, and the Elliptic Curve Cryptography Cofactor Diffie-Hellman Primitive to derive the \
   shared secret. The private key in your KMS key pair never leaves KMS unencrypted. \
   DeriveSharedSecret returns the raw shared secret.\n\
  \                 \n\
  \                  }\n\
  \            {-   {b Bob} uses the Elliptic Curve Cryptography Cofactor Diffie-Hellman Primitive \
  ...
  \    The KMS key that you use for this operation must be in a compatible key state. For details, \
   ...
   keys} in the {i Key Management Service Developer Guide}.\n\
  \    \n\
  \      {b Cross-account use}: Yes. To perform this operation with a KMS key in a different \
   Amazon Web Services account, specify the key ARN or alias ARN in the value of the [KeyId] \
   parameter.\n\
  \     \n\
  \       {b Required permissions}: \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html}kms:DeriveSharedSecret} \
   (key policy)\n\
  \      \n\
  \        {b Related operations:} \n\
  \       \n\
...
[@@ocaml.doc
  "Gets information about \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   stores} in the account and Region.\n\n\
  \  This operation is part of the custom key stores feature in KMS, which combines the \
   convenience and extensive integration of KMS with the isolation and control of a key store that \
   you own and manage.\n\
  \ \n\
  ...
  \    Custom key stores have a [DISCONNECTED] connection state if the key store has never been \
   connected or you used the [DisconnectCustomKeyStore] operation to disconnect it. Otherwise, the \
   connection state is CONNECTED. If your custom key store connection state is [CONNECTED] but you \
   are having trouble using it, verify that the backing store is active and available. For an \
   CloudHSM key store, verify that the associated CloudHSM cluster is active and contains the \
   minimum number of HSMs required for the operation, if any. For an external key store, verify \
   that the external key store proxy and its associated external key manager are reachable and \
   enabled.\n\
  \    \n\
  \      For help repairing your CloudHSM key store, see the \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/xks-troubleshooting.html}Troubleshooting \
   external key stores}. Both topics are in the {i Key Management Service Developer Guide}.\n\
  \     \n\
  \       {b Cross-account use}: No. You cannot perform this operation on a custom key store in a \
   different Amazon Web Services account.\n\
  \      \n\
...
[@@ocaml.doc
  ...
  \ This detailed information includes the key ARN, creation date (and deletion date, if \
   ...
   includes fields, like [KeySpec], that help you distinguish different types of KMS keys. It also \
   displays the key usage (encryption, signing, or generating and verifying MACs) and the \
   algorithms that the KMS key supports. \n\
  \ \n\
  \  For \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html}multi-Region \
   keys}, [DescribeKey] displays the primary key and all related replica keys. For KMS keys in \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-cloudhsm.html}CloudHSM key \
   stores}, it includes information about the key store, such as the key store ID and the CloudHSM \
   cluster ID. For KMS keys in \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/keystore-external.html}external key \
   stores}, it includes the custom key store ID and the ID of the external key.\n\
  \  \n\
  ...
  \          {-  Whether automatic key rotation is enabled on the KMS key. To get this \
   ...
   automatically rotated. For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotate-keys-how-it-works}How \
   key rotation works} in the {i Key Management Service Developer Guide}.\n\
  \              \n\
  \               }\n\
  ...
  \     {b Cross-account use}: Yes. To perform this operation with a KMS key in a different Amazon \
   Web Services account, specify the key ARN or alias ARN in the value of the [KeyId] parameter.\n\
  \    \n\
  \      {b Required permissions}: \
...
[@@ocaml.doc
  ...
  \ The KMS key that you use for this operation must be in a compatible key state. For more \
   information about how key state affects the use of a KMS key, see \
...
[@@ocaml.doc
  ...
  \    In May 2022, KMS changed the rotation schedule for Amazon Web Services managed keys from \
   every three years to every year. For details, see [EnableKeyRotation].\n\
  \    \n\
  \      The KMS key that you use for this operation must be in a compatible key state. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states \
   of KMS keys} in the {i Key Management Service Developer Guide}.\n\
...
[@@ocaml.doc
  "Disconnects the \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   store} from its backing key store. This operation disconnects an CloudHSM key store from its \
   associated CloudHSM cluster or disconnects an external key store from the external key store \
  ...
  \  While a custom key store is disconnected, you can manage the custom key store and its KMS \
   ...
   time.\n\
  \  \n\
  \    While a custom key store is disconnected, all attempts to create KMS keys in the custom key \
   store or to use existing KMS keys in \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations}cryptographic \
...
[@@ocaml.doc
  ...
  \  You can monitor rotation of the key material for your KMS keys in CloudTrail and Amazon \
   ...
   [DisableKeyRotation] operation. You can use the [GetKeyRotationStatus] operation to identify \
   any in progress rotations. You can use the [ListKeyRotations] operation to view the details of \
   completed rotations.\n\
  \  \n\
  \   Automatic key rotation is supported only on symmetric encryption KMS keys. You cannot enable \
   automatic rotation of \
  ...
  \      \n\
  \       New Amazon Web Services managed keys are automatically rotated one year after they are \
   created, and approximately every year thereafter. \n\
  \       \n\
...
[@@ocaml.doc
  ...
  \ You can use this operation to encrypt small amounts of arbitrary data, such as a personal \
   ...
   [Encrypt] operation to encrypt a data key. The [GenerateDataKey] and [GenerateDataKeyPair] \
  ...
  \  If you use a symmetric encryption KMS key, you can use an encryption context to add \
   additional security to your encryption operation. If you specify an [EncryptionContext] when \
  ...
  \   \n\
  \     When you use an asymmetric KMS key to encrypt or reencrypt data, be sure to record the KMS \
   key and encryption algorithm that you choose. You will be required to provide the same KMS key \
   and encryption algorithm when you decrypt the data. If the KMS key and algorithm do not match \
   the values used to encrypt the data, the decrypt operation fails.\n\
  \     \n\
  \      You are not required to supply the key ID and encryption algorithm when you decrypt with \
   symmetric encryption KMS keys because KMS stores this information in the ciphertext blob. KMS \
   cannot store metadata in ciphertext generated with asymmetric keys. The standard format for \
   asymmetric key ciphertext does not include configurable fields.\n\
  \      \n\
  \        The maximum size of the data that you can encrypt varies with the type of KMS key and \
...
[@@ocaml.doc
  "Returns a unique symmetric data key for use outside of KMS. This operation returns a plaintext \
   copy of the data key and a copy that is encrypted under a symmetric encryption KMS key that you \
   specify. The bytes in the plaintext key are random; they are not related to the caller or the \
   KMS key. You can use the plaintext key to encrypt your data outside of KMS and store the \
   encrypted data key with the encrypted data.\n\n\
  \ To generate a data key, specify the symmetric encryption KMS key that will be used to encrypt \
   the data key. You cannot use an asymmetric KMS key to encrypt data keys. To get the type of \
  ...
  \   To generate a 128-bit SM4 data key (China Regions only), specify a [KeySpec] value of \
   [AES_128] or a [NumberOfBytes] value of [16]. The symmetric encryption key used in China \
   Regions to encrypt your data key is an SM4 encryption key.\n\
  \   \n\
  ...
  \     You can use an optional encryption context to add additional security to the encryption \
   ...
   to decrypt fails with an [InvalidCiphertextException]. For more information, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/encrypt_context.html}Encryption \
   Context} in the {i Key Management Service Developer Guide}.\n\
  \     \n\
  ...
  \         We recommend that you use the following pattern to encrypt data locally in your \
   ...
   {{:https://docs.aws.amazon.com/encryption-sdk/latest/developer-guide/}Amazon Web Services \
   Encryption SDK}, the \
   {{:https://docs.aws.amazon.com/dynamodb-encryption-client/latest/devguide/}Amazon DynamoDB \
   Encryption Client}, or \
   {{:https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingClientSideEncryption.html}Amazon S3 \
...
[@@ocaml.doc
  "Returns a unique asymmetric data key pair for use outside of KMS. This operation returns a \
   plaintext public key, a plaintext private key, and a copy of the private key that is encrypted \
   under the symmetric encryption KMS key you specify. You can use the data key pair to perform \
   asymmetric cryptography and implement digital signatures outside of KMS. The bytes in the keys \
   are random; they are not related to the caller or to the KMS key that is used to encrypt the \
   private key. \n\n\
  \ You can use the public key that [GenerateDataKeyPair] returns to encrypt data or verify a \
   signature outside of KMS. Then, store the encrypted private key with the data. When you are \
   ready to decrypt data or sign a message, you can use the [Decrypt] operation to decrypt the \
   encrypted private key.\n\
  \ \n\
  \  To generate a data key pair, you must specify a symmetric encryption KMS key to encrypt the \
   private key in a data key pair. You cannot use an asymmetric KMS key or a KMS key in a custom \
   key store. To get the type and origin of your KMS key, use the [DescribeKey] operation. \n\
  \  \n\
  \   Use the [KeyPairSpec] parameter to choose an RSA or Elliptic Curve (ECC) data key pair. In \
   China Regions, you can also choose an SM2 data key pair. KMS recommends that you use ECC key \
   pairs for signing, and use RSA and SM2 key pairs for either encryption or signing, but not \
  ...
  \    If you are using the data key pair to encrypt data, or for any operation where you don't \
   ...
   the [Decrypt] operation to decrypt the encrypted private key in the data key pair.\n\
  \    \n\
  \      [GenerateDataKeyPair] returns a unique data key pair for each request. The bytes in the \
   keys are random; they are not related to the caller or the KMS key that is used to encrypt the \
   private key. The public key is a DER-encoded X.509 SubjectPublicKeyInfo, as specified in \
   {{:https://tools.ietf.org/html/rfc5280}RFC 5280}. The private key is a DER-encoded PKCS8 \
   PrivateKeyInfo, as specified in {{:https://tools.ietf.org/html/rfc5958}RFC 5958}.\n\
  \     \n\
...
[@@ocaml.doc
  "Returns a unique asymmetric data key pair for use outside of KMS. This operation returns a \
   ...
   encryption KMS key you specify. Unlike [GenerateDataKeyPair], this operation does not return a \
   plaintext private key. The bytes in the keys are random; they are not related to the caller or \
   to the KMS key that is used to encrypt the private key. \n\n\
  \ You can use the public key that [GenerateDataKeyPairWithoutPlaintext] returns to encrypt data \
   or verify a signature outside of KMS. Then, store the encrypted private key with the data. When \
   you are ready to decrypt data or sign a message, you can use the [Decrypt] operation to decrypt \
  ...
  \     [GenerateDataKeyPairWithoutPlaintext] returns a unique data key pair for each request. The \
   bytes in the key are not related to the caller or KMS key that is used to encrypt the private \
...
[@@ocaml.doc
  "Returns a unique symmetric data key for use outside of KMS. This operation returns a data key \
   that is encrypted under a symmetric encryption KMS key that you specify. The bytes in the key \
  ...
  \  This operation is useful for systems that need to encrypt data at some point, but not \
   immediately. When you need to encrypt the data, you call the [Decrypt] operation on the \
  ...
  \   It's also useful in distributed systems with different levels of trust. For example, you \
   ...
   and stores an encrypted data key with each container. Then, a different component puts the data \
   into the containers. That component first decrypts the data key, uses the plaintext data key to \
   encrypt data, puts the encrypted data into the container, and then destroys the plaintext data \
  ...
  \     To generate a data key, you must specify the symmetric encryption KMS key that is used to \
   ...
   generate a data key. To get the type of your KMS key, use the [DescribeKey] operation.\n\
  \     \n\
  \      You must also specify the length of the data key. Use either the [KeySpec] or \
   [NumberOfBytes] parameters (but not both). For 128-bit and 256-bit data keys, use the [KeySpec] \
   parameter.\n\
  \      \n\
  \       To generate an SM4 data key (China Regions only), specify a [KeySpec] value of [AES_128] \
   or [NumberOfBytes] value of [16]. The symmetric encryption key used in China Regions to encrypt \
...
[@@ocaml.doc
  "Generates a hash-based message authentication code (HMAC) for a message using an HMAC KMS key \
   and a MAC algorithm that the key supports. HMAC KMS keys and the HMAC algorithms that KMS uses \
   conform to industry standards defined in {{:https://datatracker.ietf.org/doc/html/rfc2104}RFC \
   2104}.\n\n\
  \ You can use value that GenerateMac returns in the [VerifyMac] operation to demonstrate that \
   the original message has not changed. Also, because a secret key is used to create the hash, \
   you can verify that the party that generated the hash has the required secret key. You can also \
   use the raw result to implement HMAC-based algorithms such as key derivation functions. This \
   operation is part of KMS support for HMAC KMS keys. For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html}HMAC keys in KMS} in the {i  \
   {i Key Management Service Developer Guide} }.\n\
  \ \n\
  \   Best practices recommend that you limit the time during which any signing mechanism, \
   including an HMAC, is effective. This deters an attack where the actor uses a signed message to \
   establish validity repeatedly or long after the message is superseded. HMAC tags do not include \
   a timestamp, but you can include a timestamp in the token or message to help you detect when \
   its time to refresh the HMAC. \n\
  \   \n\
  \     The KMS key that you use for this operation must be in a compatible key state. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states \
   of KMS keys} in the {i Key Management Service Developer Guide}.\n\
...
[@@ocaml.doc
  ...
  \ \n\
  \  By default, the random byte string is generated in KMS. To generate the byte string in the \
   CloudHSM cluster associated with an CloudHSM key store, use the [CustomKeyStoreId] parameter.\n\
  \  \n\
  ...
  \    For more information about entropy and random number generation, see \
   ...
   and random number generation} in the {i Key Management Service Developer Guide}.\n\
  \    \n\
  \      {b Cross-account use}: Not applicable. [GenerateRandom] does not use any account-specific \
...
[@@ocaml.doc
  "Provides detailed information about the rotation status for a KMS key, including whether \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-enable-disable.html}automatic \
   rotation of the key material} is enabled for the specified KMS key, the \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html#rotation-period}rotation \
  ...
  \              {-  Disabled: The key rotation status does not change when you disable a KMS key. \
   However, while the KMS key is disabled, KMS does not rotate the key material. When you \
   re-enable the KMS key, rotation resumes. If the key material in the re-enabled KMS key hasn't \
   been rotated in one year, KMS rotates it immediately, and every year thereafter. If it's been \
  ...
  \              {-  Pending deletion: While a KMS key is pending deletion, its key rotation \
   status is [false] and KMS does not rotate the key material. If you cancel the deletion, the \
   original key rotation status returns to [true].\n\
  \                  \n\
  \                   }\n\
  \              }\n\
  \    {b Cross-account use}: Yes. To perform this operation on a KMS key in a different Amazon \
...
[@@ocaml.doc
  "Returns the public key and an import token you need to import or reimport key material for a \
   KMS key. \n\n\
  \ By default, KMS keys are created with key material that KMS generates. This operation supports \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html}Importing key \
   material}, an advanced feature that lets you generate and import the cryptographic key material \
   for a KMS key.\n\
  \ \n\
  \  Before calling [GetParametersForImport], use the [CreateKey] operation with an [Origin] value \
   of [EXTERNAL] to create a KMS key with no key material. You can import key material for a \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html}multi-Region \
   key} of any supported type. However, you can't import key material into a KMS key in a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
   store}. You can also use [GetParametersForImport] to get a public key and import token to \
  ...
  \   The public key and its import token are permanently linked and must be used together. Each \
   ...
   the [ParametersValidTo] field in the [GetParametersForImport] response. You cannot use an \
   expired public key or import token in an [ImportKeyMaterial] request. If your key and token \
...
[@@ocaml.doc
  "Returns the public key of an asymmetric KMS key. Unlike the private key of a asymmetric KMS \
   ...
   to encrypt messages and verify signatures outside of KMS. For information about asymmetric KMS \
   keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}Asymmetric \
   KMS keys} in the {i Key Management Service Developer Guide}.\n\n\
  \ You do not need to download the public key. Instead, you can use the public key within KMS by \
   calling the [Encrypt], [ReEncrypt], or [Verify] operations with the identifier of an asymmetric \
   KMS key. When you use the public key within KMS, you benefit from the authentication, \
   authorization, and logging that are part of every KMS operation. You also reduce of risk of \
   encrypting data that cannot be decrypted. These features are not effective outside of KMS.\n\
  \ \n\
  \  To help you use the public key safely outside of KMS, [GetPublicKey] returns important \
  ...
  \         {-   \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/APIReference/API_GetPublicKey.html#KMS-GetPublicKey-response-KeyAgreementAlgorithms}KeyAgreementAlgorithms}, \
   or \
   ...
   A list of the encryption algorithms, key agreement algorithms, or signing algorithms for the \
   key.\n\
  \             \n\
  \              }\n\
  \         }\n\
  \   Although KMS cannot enforce these restrictions on external operations, it is crucial that \
   you use this information to prevent the public key from being used improperly. For example, you \
   can prevent a public signing key from being used encrypt data, or prevent a public key from \
   being used with an encryption algorithm that is not supported by KMS. You can also avoid \
  ...
  \    To verify a signature outside of KMS with an SM2 public key (China Regions only), you must \
   specify the distinguishing ID. By default, KMS uses [1234567812345678] as the distinguishing \
...
[@@ocaml.doc
  ...
  \ By default, KMS creates KMS keys with key material that it generates. You can also generate \
  ...
  \   After you import key material, you can \
   ...
   key material. You can use the [ImportType] parameter to indicate whether you are importing new \
   key material or re-importing previously imported key material. You might reimport key material \
   to replace key material that expired or key material that you deleted. You might also reimport \
   key material to change the expiration model or expiration date of the key material.\n\
  \   \n\
  \    Each time you import key material into KMS, you can determine whether ([ExpirationModel]) \
   and when ([ValidTo]) the key material expires. To change the expiration of your key material, \
   you must import it again, either by calling [ImportKeyMaterial] or using the \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html#importing-keys-import-key-material-console}import \
  ...
  \                 To create a new KMS key for imported key material, call the [CreateKey] \
   ...
   HMAC KMS key, asymmetric encryption KMS key, asymmetric key agreement key, or asymmetric \
   signing KMS key. You can also import key material into a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html}multi-Region \
   key} of any supported type. However, you can't import key material into a KMS key in a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-store-overview.html}custom key \
  ...
  \          {-  Whether the key material expires ([ExpirationModel]) and, if so, when \
   ([ValidTo]). For help with this choice, see \
  ...
  \               If you set an expiration date, KMS deletes the key material from the KMS key on \
   the specified date, making the KMS key unusable. To use the KMS key in cryptographic operations \
   again, you must reimport the same key material. However, you can delete and reimport the key \
   material at any time, including before the key material expires. Each time you reimport, you \
   can eliminate or reset the expiration time.\n\
  \               \n\
  \                }\n\
  \          }\n\
  ...
  \    If this operation fails, use the exception to help determine the problem. If the error is \
   ...
   get a new public key and import token for the KMS key and repeat the import procedure. For \
   help, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-conceptual.html}Create \
   a KMS key with imported key material} in the {i Key Management Service Developer Guide}.\n\
  \    \n\
  \     The KMS key that you use for this operation must be in a compatible key state. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states \
   of KMS keys} in the {i Key Management Service Developer Guide}.\n\
...
[@@ocaml.doc
  ...
  \ By default, the [ListAliases] operation returns all aliases in the account and region. To get \
   only the aliases associated with a particular KMS key, use the [KeyId] parameter.\n\
  \ \n\
  \  The [ListAliases] response can include aliases that you created and associated with your \
   customer managed keys, and aliases that Amazon Web Services created and associated with Amazon \
   Web Services managed keys in your account. You can recognize Amazon Web Services aliases \
   because their names have the format \n\
  \  {[\n\
  \  aws/\n\
  \  ]}\n\
  ...
  \   The response might also include aliases that have no [TargetKeyId] field. These are \
   predefined aliases that Amazon Web Services has created but has not yet associated with a KMS \
   key. Aliases that Amazon Web Services creates in your account, including predefined aliases, do \
   not count against your \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/resource-limits.html#aliases-per-key}KMS \
   aliases quota}.\n\
  \   \n\
  \     {b Cross-account use}: No. [ListAliases] does not return aliases in other Amazon Web \
   Services accounts.\n\
  \    \n\
  \      {b Required permissions}: \
...
[@@ocaml.doc
  "Gets the names of the key policies that are attached to a KMS key. This operation is designed \
   to get policy names that you can use in a [GetKeyPolicy] operation. However, the only valid \
...
[@@ocaml.doc
  "Returns information about the key materials associated with the specified KMS key. You can use \
  ...
  \  For detailed information about automatic and on-demand key rotations, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html}Rotate KMS keys} in \
  ...
  \   \n\
  \     {b Required permissions}: \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html}kms:ListKeyRotations} \
   (key policy)\n\
  \    \n\
  \      {b Related operations:} \n\
  \     \n\
...
[@@ocaml.doc
  ...
  \ For general information about tags, including the format and syntax, see \
   {{:https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html}Tagging Amazon Web Services \
   resources} in the {i Amazon Web Services General Reference}. For information about using tags \
   in KMS, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html}Tags in \
   KMS}.\n\
  \ \n\
  \   {b Cross-account use}: No. You cannot perform this operation on a KMS key in a different \
   Amazon Web Services account.\n\
...
[@@ocaml.doc
  ...
  \  For detailed information about grants, including grant terminology, see \
   ...
   CreateGrant with an Amazon Web Services SDK or CLI}. \n\
  \  \n\
  \    {b Cross-account use}: You must specify a principal in your Amazon Web Services account. \
   This operation returns a list of grants where the retiring principal specified in the \
   [ListRetirableGrants] request is the same retiring principal on the grant. This can include \
   grants on KMS keys owned by other Amazon Web Services accounts, but you do not need \
   [kms:ListRetirableGrants] permission (or any other additional permission) in any Amazon Web \
...
[@@ocaml.doc
  ...
  \ For more information about key policies, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html}Key Policies} in the \
   {i Key Management Service Developer Guide}. For help writing and formatting a JSON policy \
   document, see the \
   {{:https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies.html}IAM JSON Policy \
   Reference} in the {i  {i Identity and Access Management User Guide} }. For examples of adding a \
   key policy in multiple programming languages, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/example_kms_PutKeyPolicy_section.html}Use \
   PutKeyPolicy with an Amazon Web Services SDK or CLI} in the {i Key Management Service Developer \
   Guide}.\n\
  \ \n\
  \   {b Cross-account use}: No. You cannot perform this operation on a KMS key in a different \
   Amazon Web Services account.\n\
...
[@@ocaml.doc
  "Decrypts ciphertext and then reencrypts it entirely within KMS. You can use this operation to \
   change the KMS key under which data is encrypted, such as when you \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys-manually.html}manually \
   rotate} a KMS key or change the KMS key that protects a ciphertext. You can also use it to \
  ...
  \ The [ReEncrypt] operation can decrypt ciphertext that was encrypted by using a KMS key in an \
   ...
   encrypted by using the public key of an \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}asymmetric \
   KMS key} outside of KMS. However, it cannot decrypt ciphertext produced by other libraries, \
   such as the {{:https://docs.aws.amazon.com/encryption-sdk/latest/developer-guide/}Amazon Web \
   Services Encryption SDK} or \
   {{:https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingClientSideEncryption.html}Amazon S3 \
   client-side encryption}. These libraries return a ciphertext format that is incompatible with \
   KMS.\n\
  \ \n\
  ...
  \         {-  If your ciphertext was encrypted under an asymmetric KMS key, you must use the \
   ...
   supply the encryption algorithm that was used. This information is required to decrypt the data.\n\
  \             \n\
  \              }\n\
  \         {-  If your ciphertext was encrypted under a symmetric encryption KMS key, the \
   [SourceKeyId] parameter is optional. KMS can get this information from metadata that it adds to \
   the symmetric ciphertext blob. This feature adds durability to your implementation by ensuring \
   that authorized users can decrypt ciphertext decades after it was encrypted, even if they've \
   lost track of the key ID. However, specifying the source KMS key is always recommended as a \
   best practice. When you use the [SourceKeyId] parameter to specify a KMS key, KMS uses only the \
  ...
  \             \n\
  \              }\n\
  \         {-  To reencrypt the data, you must use the [DestinationKeyId] parameter to specify \
   the KMS key that re-encrypts the data after it is decrypted. If the destination KMS key is an \
   asymmetric KMS key, you must also provide the encryption algorithm. The algorithm that you \
   choose must be compatible with the KMS key.\n\
  \             \n\
  \               When you use an asymmetric KMS key to encrypt or reencrypt data, be sure to \
   record the KMS key and encryption algorithm that you choose. You will be required to provide \
   the same KMS key and encryption algorithm when you decrypt the data. If the KMS key and \
  ...
  \                You are not required to supply the key ID and encryption algorithm when you \
   decrypt with symmetric encryption KMS keys because KMS stores this information in the \
   ciphertext blob. KMS cannot store metadata in ciphertext generated with asymmetric keys. The \
  ...
  \   To permit reencryption from or to a KMS key, include the [\"kms:ReEncrypt*\"] permission in \
   ...
   a KMS key. But you must include it manually when you create a KMS key programmatically or when \
   you use the [PutKeyPolicy] operation to set a key policy.\n\
  \   \n\
  \     {b Related operations:} \n\
  \    \n\
  \     {ul\n\
...
[@@ocaml.doc
  "Replicates a multi-Region key into the specified Region. This operation creates a multi-Region \
   replica key based on a multi-Region primary key in a different Region of the same Amazon Web \
   Services partition. You can create multiple replicas of a primary key, but each must be in a \
   different Region. To create a multi-Region primary key, use the [CreateKey] operation.\n\n\
  \ This operation supports {i multi-Region keys}, an KMS feature that lets you create multiple \
   interoperable KMS keys in different Amazon Web Services Regions. Because these KMS keys have \
   the same key ID, key material, and other metadata, you can use them interchangeably to encrypt \
   data in one Amazon Web Services Region and decrypt it in a different Amazon Web Services Region \
   without re-encrypting the data or making a cross-Region call. For more information about \
   multi-Region keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html}Multi-Region \
   keys in KMS} in the {i Key Management Service Developer Guide}.\n\
  \ \n\
  \  A {i replica key} is a fully-functional KMS key that can be used independently of its primary \
   and peer replica keys. A primary key and its replica keys share properties that make them \
   interoperable. They have the same \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-alias.html}aliases}, and \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}key state}. KMS pricing \
  ...
  \   When this operation completes, the new replica key has a transient key state of [Creating]. \
   This key state changes to [Enabled] (or [PendingImport]) after a few seconds when the process \
   of creating the new replica key is complete. While the key state is [Creating], you can manage \
   key, but you cannot yet use it in cryptographic operations. If you are creating and using the \
   replica key programmatically, retry on [KMSInvalidStateException] or call [DescribeKey] to \
   check its [KeyState] value before using it. For details about the [Creating] key state, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states of KMS keys} \
   in the {i Key Management Service Developer Guide}.\n\
  \   \n\
  \    You cannot create more than one replica of a primary key in any Region. If the Region \
   ...
   you can cancel the scheduled key deletion ([CancelKeyDeletion]) or wait for the key to be \
   deleted. The new replica key you create will have the same \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html#mrk-sync-properties}shared \
  ...
  \     \n\
  \      If you replicate a multi-Region primary key with imported key material, the replica key \
   is created with no key material. You must import the same key material that you imported into \
   the primary key.\n\
  \      \n\
  ...
  \         \n\
  \            {b Cross-account use}: No. You cannot use this operation to create a replica key in \
   a different Amazon Web Services account. \n\
  \           \n\
  \             {b Required permissions}: \n\
  \            \n\
  ...
  \                       \n\
  \                        }\n\
  \                   {-  To use the [Tags] parameter, [kms:TagResource] in an IAM policy in the \
   replica Region.\n\
  \                       \n\
  \                        }\n\
  \                   }\n\
...
[@@ocaml.doc
  "Deletes a grant. Typically, you retire a grant when you no longer need its permissions. To \
   identify the grant to retire, use a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/grants.html#grant_token}grant token}, \
   or both the grant ID and a key identifier (key ID or key ARN) of the KMS key. The [CreateGrant] \
   operation returns both values.\n\n\
  \ This operation can be called by the {i retiring principal} for a grant, by the {i grantee \
   principal} if the grant allows the [RetireGrant] operation, and by the Amazon Web Services \
   account in which the grant is created. It can also be called by principals to whom permission \
   for retiring a grant is delegated.\n\
  \ \n\
  \  For detailed information about grants, including grant terminology, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/grants.html}Grants in KMS} in the {i  \
   {i Key Management Service Developer Guide} }. For examples of creating grants in several \
   ...
   CreateGrant with an Amazon Web Services SDK or CLI}. \n\
  \  \n\
  \    {b Cross-account use}: Yes. You can retire a grant on a KMS key in a different Amazon Web \
   Services account.\n\
  \   \n\
  \     {b Required permissions}: Permission to retire a grant is determined primarily by the \
   grant. For details, see \
...
[@@ocaml.doc
  "Deletes the specified grant. You revoke a grant to terminate the permissions that the grant \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/grant-delete.html}Retiring and \
   revoking grants} in the {i  {i Key Management Service Developer Guide} }.\n\n\
  \ When you create, retire, or revoke a grant, there might be a brief delay, usually less than \
   five minutes, until the grant is available throughout KMS. This state is known as {i eventual \
   ...
   consistency} in the {i  {i Key Management Service Developer Guide} }. \n\
  \ \n\
  \  For detailed information about grants, including grant terminology, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/grants.html}Grants in KMS} in the {i  \
   {i Key Management Service Developer Guide} }. For examples of creating grants in several \
...
[@@ocaml.doc
  "Immediately initiates rotation of the key material of the specified symmetric encryption KMS \
   key.\n\n\
  \ You can perform \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-on-demand.html}on-demand \
   rotation} of the key material in customer managed KMS keys, regardless of whether or not \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotating-keys-enable-disable.html}automatic \
   key rotation} is enabled. On-demand rotations do not change existing automatic rotation \
   schedules. For example, consider a KMS key that has automatic key rotation enabled with a \
   rotation period of 730 days. If the key is scheduled to automatically rotate on April 14, 2024, \
  ...
  \     You can use [GetKeyRotationStatus] to identify any in progress on-demand rotations. You \
   can use [ListKeyRotations] to identify the date that completed on-demand rotations were \
   performed. You can monitor rotation of the key material for your KMS keys in CloudTrail and \
   Amazon CloudWatch.\n\
  \     \n\
  \      On-demand key rotation is supported only on symmetric encryption KMS keys. You cannot \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys-import-key-material.html}new \
   key material} and that key material's state should be [PENDING_ROTATION]. Use the \
  ...
  \         \n\
  \           {b Required permissions}: \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/kms-api-permissions-reference.html}kms:RotateKeyOnDemand} \
   (key policy)\n\
  \          \n\
  \            {b Related operations:} \n\
  \           \n\
...
[@@ocaml.doc
  "Schedules the deletion of a KMS key. By default, KMS applies a waiting period of 30 days, but \
   you can specify a waiting period of 7-30 days. When this operation is successful, the key state \
   of the KMS key changes to [PendingDeletion] and the key can't be used in any cryptographic \
   operations. It remains in this state for the duration of the waiting period. Before the waiting \
   period ends, you can use [CancelKeyDeletion] to cancel the deletion of the KMS key. After the \
   waiting period ends, KMS deletes the KMS key, its key material, and all KMS data associated \
  ...
  \  Deleting a KMS key is a destructive and potentially dangerous operation. When a KMS key is \
   ...
   a \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-delete.html}multi-Region \
   replica key}, or an \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#import-delete-key}asymmetric \
   or HMAC KMS key with imported key material}.) To prevent the use of a KMS key without deleting \
   it, use [DisableKey]. \n\
  \  \n\
  \    You can schedule the deletion of a multi-Region primary key and its replica keys at any \
   time. However, KMS will not delete a multi-Region primary key with existing replica keys. If \
   you schedule the deletion of a primary key with replicas, its key state changes to \
   [PendingReplicaDeletion] and it cannot be replicated or used in cryptographic operations. This \
   status can continue indefinitely. When the last of its replicas keys is deleted (not just \
   scheduled), the key state of the primary key changes to [PendingDeletion] and its waiting \
   period ([PendingWindowInDays]) begins. For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#deleting-mrks}Deleting \
   multi-Region keys} in the {i Key Management Service Developer Guide}.\n\
  \    \n\
  \     When KMS \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#delete-cmk-keystore}deletes \
   a KMS key from an CloudHSM key store}, it makes a best effort to delete the associated key \
   material from the associated CloudHSM cluster. However, you might need to manually \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/fix-keystore.html#fix-keystore-orphaned-key}delete \
   the orphaned key material} from the cluster and its backups. \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html#delete-xks-key}Deleting \
   a KMS key from an external key store} has no effect on the associated external key. However, \
   for both types of custom key stores, deleting a KMS key is destructive and irreversible. You \
   cannot decrypt ciphertext encrypted under the KMS key by using only its associated external key \
   or CloudHSM key. Also, you cannot recreate a KMS key in an external key store by creating a new \
...
[@@ocaml.doc
  "Creates a {{:https://en.wikipedia.org/wiki/Digital_signature}digital signature} for a message \
   or message digest by using the private key in an asymmetric signing KMS key. To verify the \
   signature, use the [Verify] operation, or use the public key in the same asymmetric KMS key \
   outside of KMS. For information about asymmetric KMS keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}Asymmetric \
   KMS keys} in the {i Key Management Service Developer Guide}.\n\n\
  \ Digital signatures are generated and verified by using asymmetric key pair, such as an RSA, \
   ECC, or ML-DSA pair that is represented by an asymmetric KMS key. The key owner (or an \
   authorized user) uses their private key to sign a message. Anyone with the public key can \
   verify that the message was signed with that particular private key and that the message hasn't \
  ...
  \             \n\
  \              }\n\
  \         {-  Use the [Message] parameter to specify the message or message digest to sign. You \
   can submit messages of up to 4096 bytes. To sign a larger message, generate a hash digest of \
   the message, and then provide the hash digest in the [Message] parameter. To indicate whether \
   the message is a full message, a digest, or an ML-DSA EXTERNAL_MU, use the [MessageType] \
   parameter.\n\
  \             \n\
  \              }\n\
  ...
  \       Best practices recommend that you limit the time during which any signature is \
   ...
   repeatedly or long after the message is superseded. Signatures do not include a timestamp, but \
   you can include a timestamp in the signed message to help you detect when its time to refresh \
   the signature. \n\
  \       \n\
  \         To verify the signature that this operation generates, use the [Verify] operation. Or \
   use the [GetPublicKey] operation to download the public key and then use the public key to \
  ...
  \             \n\
  \               {b Eventual consistency}: The KMS API follows an eventual consistency model. For \
...
[@@ocaml.doc
  ...
  \     \n\
  \      You can also add tags to a KMS key while creating it ([CreateKey]) or replicating it \
   ([ReplicateKey]).\n\
  \      \n\
  \       For information about using tags in KMS, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html}Tagging keys}. For \
   general information about tags, including the format and syntax, see \
   {{:https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html}Tagging Amazon Web Services \
   resources} in the {i Amazon Web Services General Reference}. \n\
  \       \n\
  \        The KMS key that you use for this operation must be in a compatible key state. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states \
   of KMS keys} in the {i Key Management Service Developer Guide}.\n\
...
[@@ocaml.doc
  ...
  \  Tagging or untagging a KMS key can allow or deny permission to the KMS key. For details, see \
   ...
   Management Service Developer Guide}.\n\
  \  \n\
  \    When it succeeds, the [UntagResource] operation doesn't return any output. Also, if the \
   specified tag key isn't found on the KMS key, it doesn't throw an exception or return a \
   response. To confirm that the operation worked, use the [ListResourceTags] operation.\n\
  \    \n\
  \     For information about using tags in KMS, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html}Tagging keys}. For \
   general information about tags, including the format and syntax, see \
...
[@@ocaml.doc
  "Associates an existing KMS alias with a different KMS key. Each alias is associated with only \
   one KMS key at a time, although a KMS key can have multiple aliases. The alias and the KMS key \
   must be in the same Amazon Web Services account and Region.\n\n\
  \  Adding, deleting, or updating an alias can allow or deny permission to the KMS key. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/abac.html}ABAC for KMS} \
   in the {i Key Management Service Developer Guide}.\n\
  \  \n\
  \    The current and new KMS key must be the same type (both symmetric or both asymmetric or \
   ...
   that uses aliases. If you must assign an alias to a different type of KMS key, use \
  ...
  \      Because an alias is not a property of a KMS key, you can create, update, and delete the \
   aliases of a KMS key without affecting the KMS key. Also, aliases do not appear in the response \
   from the [DescribeKey] operation. To get the aliases of all KMS keys in the account, use the \
   [ListAliases] operation. \n\
  \      \n\
  \       The KMS key that you use for this operation must be in a compatible key state. For \
   details, see {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states \
...
[@@ocaml.doc
  ...
  \    When updating the properties of an external key store, verify that the updated settings \
   connect your key store, via the external key store proxy, to the same external key manager as \
   the previous settings, or to a backup or snapshot of the external key manager with the same \
   cryptographic keys. If the updated connection settings fail, you can fix them and retry, \
   although an extended delay might disrupt Amazon Web Services services. However, if KMS \
  ...
  \         When updating an external key store in the KMS console, you can upload a JSON-based \
   ...
   file to the [UpdateCustomKeyStore] operation. However, you can use the file to help you \
   determine the correct values for the [UpdateCustomKeyStore] parameters.\n\
  \         \n\
  ...
  \            For an external key store, you can use this operation to change the custom key \
   ...
   key store proxy authentication credentials ([XksProxyAuthenticationCredential]), connection \
   method ([XksProxyConnectivity]), external proxy endpoint ([XksProxyUriEndpoint]) and path \
  ...
  \             If your update requires a [DISCONNECTED] state, before using \
   [UpdateCustomKeyStore], use the [DisconnectCustomKeyStore] operation to disconnect the custom \
   key store. After the [UpdateCustomKeyStore] operation completes, use the \
   [ConnectCustomKeyStore] to reconnect the custom key store. To find the [ConnectionState] of the \
   custom key store, use the [DescribeCustomKeyStores] operation. \n\
  \             \n\
  \               \n\
  \              \n\
  \               Before updating the custom key store, verify that the new values allow KMS to \
   connect the custom key store to its backing key store. For example, before you change the \
   [XksProxyUriPath] value, verify that the external key store proxy is reachable at the new path.\n\
  \               \n\
  \                If the operation succeeds, it returns a JSON object with no properties.\n\
  \                \n\
  \                  {b Cross-account use}: No. You cannot perform this operation on a custom key \
...
[@@ocaml.doc
  ...
  \ This operation changes the replica key in the specified Region to a primary key and changes \
   the former primary key to a replica key. For example, suppose you have a primary key in \
   [us-east-1] and a replica key in [eu-west-2]. If you run [UpdatePrimaryRegion] with a \
   [PrimaryRegion] value of [eu-west-2], the primary key is now the key in [eu-west-2], and the \
   key in [us-east-1] becomes a replica key. For details, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-update.html}Change the \
   primary key in a set of multi-Region keys} in the {i Key Management Service Developer Guide}.\n\
  \ \n\
  \  This operation supports {i multi-Region keys}, an KMS feature that lets you create multiple \
   interoperable KMS keys in different Amazon Web Services Regions. Because these KMS keys have \
   the same key ID, key material, and other metadata, you can use them interchangeably to encrypt \
   ...
   keys in KMS} in the {i Key Management Service Developer Guide}.\n\
  \  \n\
  \   The {i primary key} of a multi-Region key is the source for properties that are always \
   shared by primary and replica keys, including the key material, \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-id-key-id}key ID}, \
   ...
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#key-origin}key material \
   origin}, and \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html}automatic key \
   rotation}. It's the only key that can be replicated. You cannot \
   {{:https://docs.aws.amazon.com/kms/latest/APIReference/API_ScheduleKeyDeletion.html}delete the \
  ...
  \    The key ID and primary Region that you specify uniquely identify the replica key that will \
   become the primary key. The primary Region must already have a replica key. This operation does \
  ...
  \      Even after this operation completes, the process of updating the primary Region might \
   still be in progress for a few more seconds. Operations such as [DescribeKey] might display \
   both the old and new primary keys as replicas. The old and new primary keys have a transient \
   key state of [Updating]. The original key state is restored when the update is complete. While \
   the key state is [Updating], you can use the keys in cryptographic operations, but you cannot \
   replicate the new primary key or perform certain management operations, such as enabling or \
   disabling these keys. For details about the [Updating] key state, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html}Key states of KMS keys} \
   in the {i Key Management Service Developer Guide}.\n\
  \      \n\
  \       This operation does not return any output. To verify that primary key is changed, use \
   the [DescribeKey] operation.\n\
  \       \n\
  \         {b Cross-account use}: No. You cannot use this operation in a different Amazon Web \
   Services account. \n\
  \        \n\
  \          {b Required permissions}: \n\
  \         \n\
...
[@@ocaml.doc
  ...
  \   A digital signature is generated by using the private key in an asymmetric KMS key. The \
   signature is verified by using the public key in the same asymmetric KMS key. For information \
   about asymmetric KMS keys, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html}Asymmetric \
   KMS keys} in the {i Key Management Service Developer Guide}.\n\
  \   \n\
  \    To use the [Verify] operation, specify the same asymmetric KMS key, message, and signing \
   algorithm that were used to produce the signature. The message type does not need to be the \
   same as the one used for signing, but it must indicate whether the value of the [Message] \
  ...
  \     You can also verify the digital signature by using the public key of the KMS key outside \
   of KMS. Use the [GetPublicKey] operation to download the public key in the asymmetric KMS key \
   and then use the public key to verify the signature outside of KMS. The advantage of using the \
   [Verify] operation is that it is performed within KMS. As a result, it's easy to call, the \
   operation is performed within the FIPS boundary, it is logged in CloudTrail, and you can use \
   key policy and IAM policy to determine who is authorized to use the KMS key to verify \
   signatures.\n\
  \     \n\
  \      To verify a signature outside of KMS with an SM2 public key (China Regions only), you \
   must specify the distinguishing ID. By default, KMS uses [1234567812345678] as the \
   distinguishing ID. For more information, see \
   {{:https://docs.aws.amazon.com/kms/latest/developerguide/offline-operations.html#key-spec-sm-offline-verification}Offline \
   verification with SM2 key pairs}.\n\
  \      \n\
  \       The KMS key that you use for this operation must be in a compatible key state. For \
...
[@@ocaml.doc
  "Verifies the hash-based message authentication code (HMAC) for a specified message, HMAC KMS \
   key, and MAC algorithm. To verify the HMAC, [VerifyMac] computes an HMAC using the message, \
   HMAC KMS key, and MAC algorithm that you specify, and compares the computed HMAC to the HMAC \
   that you specify. If the HMACs are identical, the verification succeeds; otherwise, it fails. \
   Verification indicates that the message hasn't changed since the HMAC was calculated, and the \
   specified key was used to generate and verify the HMAC.\n\n\
  \ HMAC KMS keys and the HMAC algorithms that KMS uses conform to industry standards defined in \
