...
let make_xks_proxy_authentication_credential_type
  ...
  ({ raw_secret_access_key = raw_secret_access_key_; access_key_id = access_key_id_ }
...
let make_verify_response ?signing_algorithm:(signing_algorithm_ : signing_algorithm_spec option)
    ?signature_valid:(signature_valid_ : boolean_type option) ?key_id:(key_id_ : key_id_type option)
    () =
  ({ signing_algorithm = signing_algorithm_; signature_valid = signature_valid_; key_id = key_id_ }
    : verify_response)
...
let make_verify_request ?dry_run:(dry_run_ : nullable_boolean_type option)
    ...
    ~signature:(signature_ : ciphertext_type) ~message:(message_ : plaintext_type)
...
let make_verify_mac_response ?mac_algorithm:(mac_algorithm_ : mac_algorithm_spec option)
    ?mac_valid:(mac_valid_ : boolean_type option) ?key_id:(key_id_ : key_id_type option) () =
  ({ mac_algorithm = mac_algorithm_; mac_valid = mac_valid_; key_id = key_id_ }
    : verify_mac_response)

let make_verify_mac_request ?dry_run:(dry_run_ : nullable_boolean_type option)
    ?grant_tokens:(grant_tokens_ : grant_token_list option) ~mac:(mac_ : ciphertext_type)
    ~mac_algorithm:(mac_algorithm_ : mac_algorithm_spec) ~key_id:(key_id_ : key_id_type)
...
let make_update_primary_region_request ~primary_region:(primary_region_ : region_type)
    ~key_id:(key_id_ : key_id_type) () =
  ({ primary_region = primary_region_; key_id = key_id_ } : update_primary_region_request)

let make_update_key_description_request ~description:(description_ : description_type)
    ~key_id:(key_id_ : key_id_type) () =
...
let make_untag_resource_request ~tag_keys:(tag_keys_ : tag_key_list) ~key_id:(key_id_ : key_id_type)
    () =
  ({ tag_keys = tag_keys_; key_id = key_id_ } : untag_resource_request)

let make_tag ~tag_value:(tag_value_ : tag_value_type) ~tag_key:(tag_key_ : tag_key_type) () =
  ({ tag_value = tag_value_; tag_key = tag_key_ } : tag)

let make_tag_resource_request ~tags:(tags_ : tag_list) ~key_id:(key_id_ : key_id_type) () =
  ({ tags = tags_; key_id = key_id_ } : tag_resource_request)

let make_sign_response ?signing_algorithm:(signing_algorithm_ : signing_algorithm_spec option)
    ?signature:(signature_ : ciphertext_type option) ?key_id:(key_id_ : key_id_type option) () =
  ({ signing_algorithm = signing_algorithm_; signature = signature_; key_id = key_id_ }
    : sign_response)
...
let make_schedule_key_deletion_response
    ...
    ?key_state:(key_state_ : key_state option) ?deletion_date:(deletion_date_ : date_type option)
...
let make_retire_grant_request ?dry_run:(dry_run_ : nullable_boolean_type option)
  ...
  ({ dry_run = dry_run_; grant_id = grant_id_; key_id = key_id_; grant_token = grant_token_ }
    : retire_grant_request)

let make_multi_region_key ?region:(region_ : region_type option) ?arn:(arn_ : arn_type option) () =
  ({ region = region_; arn = arn_ } : multi_region_key)

let make_multi_region_configuration ?replica_keys:(replica_keys_ : multi_region_key_list option)
...
let make_key_metadata
    ...
    ?origin:(origin_ : origin_type option) ?valid_to:(valid_to_ : date_type option)
    ...
    ?description:(description_ : description_type option) ?enabled:(enabled_ : boolean_type option)
    ?creation_date:(creation_date_ : date_type option) ?arn:(arn_ : arn_type option)
    ?aws_account_id:(aws_account_id_ : aws_account_id_type option) ~key_id:(key_id_ : key_id_type)
    () =
  ({
...
let make_replicate_key_response ?replica_tags:(replica_tags_ : tag_list option)
...
let make_replicate_key_request ?tags:(tags_ : tag_list option)
    ...
    ?policy:(policy_ : policy_type option) ~replica_region:(replica_region_ : region_type)
    ~key_id:(key_id_ : key_id_type) () =
  ({
     tags = tags_;
     description = description_;
...
let make_re_encrypt_response
    ...
    ?key_id:(key_id_ : key_id_type option) ?source_key_id:(source_key_id_ : key_id_type option)
...
let make_put_key_policy_request
    ?bypass_policy_lockout_safety_check:(bypass_policy_lockout_safety_check_ : boolean_type option)
    ?policy_name:(policy_name_ : policy_name_type option) ~policy:(policy_ : policy_type)
    ~key_id:(key_id_ : key_id_type) () =
  ({
     bypass_policy_lockout_safety_check = bypass_policy_lockout_safety_check_;
     policy = policy_;
...
let make_list_resource_tags_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option) ?tags:(tags_ : tag_list option) () =
...
let make_key_list_entry ?key_arn:(key_arn_ : arn_type option) ?key_id:(key_id_ : key_id_type option)
    () =
  ({ key_arn = key_arn_; key_id = key_id_ } : key_list_entry)

let make_list_keys_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option) ?keys:(keys_ : key_list option) () =
  ({ truncated = truncated_; next_marker = next_marker_; keys = keys_ } : list_keys_response)

let make_list_keys_request ?marker:(marker_ : marker_type option)
    ?limit:(limit_ : limit_type option) () =
  ({ marker = marker_; limit = limit_ } : list_keys_request)

let make_rotations_list_entry ?rotation_type:(rotation_type_ : rotation_type option)
    ?rotation_date:(rotation_date_ : date_type option) ?valid_to:(valid_to_ : date_type option)
    ?expiration_model:(expiration_model_ : expiration_model_type option)
...
let make_list_key_rotations_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option) ?rotations:(rotations_ : rotations_list option)
    () =
  ({ truncated = truncated_; next_marker = next_marker_; rotations = rotations_ }
...
let make_list_key_policies_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option)
    ?policy_names:(policy_names_ : policy_name_list option) () =
  ({ truncated = truncated_; next_marker = next_marker_; policy_names = policy_names_ }
...
let make_alias_list_entry ?last_updated_date:(last_updated_date_ : date_type option)
    ?creation_date:(creation_date_ : date_type option)
    ?target_key_id:(target_key_id_ : key_id_type option) ?alias_arn:(alias_arn_ : arn_type option)
...
let make_list_aliases_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option) ?aliases:(aliases_ : alias_list option) () =
...
let make_import_key_material_request
    ...
    ~import_token:(import_token_ : ciphertext_type) ~key_id:(key_id_ : key_id_type) () =
  ({
     key_material_id = key_material_id_;
     key_material_description = key_material_description_;
...
let make_get_public_key_response
    ...
    ?signing_algorithms:(signing_algorithms_ : signing_algorithm_spec_list option)
    ?encryption_algorithms:(encryption_algorithms_ : encryption_algorithm_spec_list option)
    ?key_usage:(key_usage_ : key_usage_type option) ?key_spec:(key_spec_ : key_spec option)
    ?customer_master_key_spec:(customer_master_key_spec_ : customer_master_key_spec option)
...
let make_get_parameters_for_import_response
    ...
    ?import_token:(import_token_ : ciphertext_type option) ?key_id:(key_id_ : key_id_type option) ()
    =
  ({
...
let make_get_parameters_for_import_request
    ...
    ~wrapping_algorithm:(wrapping_algorithm_ : algorithm_spec) ~key_id:(key_id_ : key_id_type) () =
  ({
...
let make_get_key_policy_response ?policy_name:(policy_name_ : policy_name_type option)
...
let make_generate_random_response
  ...
  ({ ciphertext_for_recipient = ciphertext_for_recipient_; plaintext = plaintext_ }
...
let make_generate_random_request ?recipient:(recipient_ : recipient_info option)
    ?custom_key_store_id:(custom_key_store_id_ : custom_key_store_id_type option)
...
let make_generate_mac_response ?key_id:(key_id_ : key_id_type option)
    ?mac_algorithm:(mac_algorithm_ : mac_algorithm_spec option) ?mac:(mac_ : ciphertext_type option)
    () =
  ({ key_id = key_id_; mac_algorithm = mac_algorithm_; mac = mac_ } : generate_mac_response)

let make_generate_mac_request ?dry_run:(dry_run_ : nullable_boolean_type option)
    ?grant_tokens:(grant_tokens_ : grant_token_list option)
    ~mac_algorithm:(mac_algorithm_ : mac_algorithm_spec) ~key_id:(key_id_ : key_id_type)
    ~message:(message_ : plaintext_type) () =
  ({
...
let make_generate_data_key_without_plaintext_response
    ...
    ?key_id:(key_id_ : key_id_type option)
    ?ciphertext_blob:(ciphertext_blob_ : ciphertext_type option) () =
  ({ key_material_id = key_material_id_; key_id = key_id_; ciphertext_blob = ciphertext_blob_ }
...
let make_generate_data_key_pair_without_plaintext_response
    ...
    ?key_id:(key_id_ : key_id_type option) ?public_key:(public_key_ : public_key_type option)
...
let make_generate_data_key_pair_without_plaintext_request
    ...
    ?grant_tokens:(grant_tokens_ : grant_token_list option)
    ?encryption_context:(encryption_context_ : encryption_context_type option)
    ~key_pair_spec:(key_pair_spec_ : data_key_pair_spec) ~key_id:(key_id_ : key_id_type) () =
  ({
     dry_run = dry_run_;
     grant_tokens = grant_tokens_;
     key_pair_spec = key_pair_spec_;
...
let make_generate_data_key_pair_request ?dry_run:(dry_run_ : nullable_boolean_type option)
...
let make_generate_data_key_response ?key_material_id:(key_material_id_ : backing_key_id_type option)
    ?ciphertext_for_recipient:(ciphertext_for_recipient_ : ciphertext_type option)
    ?key_id:(key_id_ : key_id_type option) ?plaintext:(plaintext_ : plaintext_type option)
    ?ciphertext_blob:(ciphertext_blob_ : ciphertext_type option) () =
  ({
     key_material_id = key_material_id_;
     ciphertext_for_recipient = ciphertext_for_recipient_;
...
let make_generate_data_key_request ?dry_run:(dry_run_ : nullable_boolean_type option)
    ?recipient:(recipient_ : recipient_info option)
    ?grant_tokens:(grant_tokens_ : grant_token_list option)
    ?key_spec:(key_spec_ : data_key_spec option)
    ?number_of_bytes:(number_of_bytes_ : number_of_bytes_type option)
...
let make_describe_custom_key_stores_response ?truncated:(truncated_ : boolean_type option)
    ?next_marker:(next_marker_ : marker_type option)
    ?custom_key_stores:(custom_key_stores_ : custom_key_stores_list option) () =
  ({ truncated = truncated_; next_marker = next_marker_; custom_key_stores = custom_key_stores_ }
    : describe_custom_key_stores_response)

let make_describe_custom_key_stores_request ?marker:(marker_ : marker_type option)
    ?limit:(limit_ : limit_type option)
    ?custom_key_store_name:(custom_key_store_name_ : custom_key_store_name_type option)
    ?custom_key_store_id:(custom_key_store_id_ : custom_key_store_id_type option) () =
  ({
...
let make_derive_shared_secret_response ?key_origin:(key_origin_ : origin_type option)
    ?key_agreement_algorithm:(key_agreement_algorithm_ : key_agreement_algorithm_spec option)
    ?ciphertext_for_recipient:(ciphertext_for_recipient_ : ciphertext_type option)
    ?shared_secret:(shared_secret_ : plaintext_type option) ?key_id:(key_id_ : key_id_type option)
    () =
  ({
...
let make_derive_shared_secret_request ?recipient:(recipient_ : recipient_info option)
    ?dry_run:(dry_run_ : nullable_boolean_type option)
    ?grant_tokens:(grant_tokens_ : grant_token_list option)
...
let make_decrypt_response ?key_material_id:(key_material_id_ : backing_key_id_type option)
    ?ciphertext_for_recipient:(ciphertext_for_recipient_ : ciphertext_type option)
    ?encryption_algorithm:(encryption_algorithm_ : encryption_algorithm_spec option)
...
let make_create_key_request ?xks_key_id:(xks_key_id_ : xks_key_id_type option)
    ?multi_region:(multi_region_ : nullable_boolean_type option) ?tags:(tags_ : tag_list option)
    ?bypass_policy_lockout_safety_check:(bypass_policy_lockout_safety_check_ : boolean_type option)
    ?custom_key_store_id:(custom_key_store_id_ : custom_key_store_id_type option)
    ?origin:(origin_ : origin_type option) ?key_spec:(key_spec_ : key_spec option)
    ?customer_master_key_spec:(customer_master_key_spec_ : customer_master_key_spec option)
    ?key_usage:(key_usage_ : key_usage_type option)
    ?description:(description_ : description_type option) ?policy:(policy_ : policy_type option) ()
    =
  ({
