...
let tag_resource_request_to_yojson (x : tag_resource_request) =
  assoc_to_yojson
    [ ("Tags", Some (tag_list_to_yojson x.tags)); ("KeyId", Some (key_id_type_to_yojson x.key_id)) ]
...
let import_state_to_yojson (x : import_state) =
  match x with PENDING_IMPORT -> `String "PENDING_IMPORT" | IMPORTED -> `String "IMPORTED"
...
let rotation_type_to_yojson (x : rotation_type) =
  match x with ON_DEMAND -> `String "ON_DEMAND" | AUTOMATIC -> `String "AUTOMATIC"
