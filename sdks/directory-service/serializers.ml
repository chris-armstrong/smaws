...
let os_version_to_yojson (x : os_version) =
  match x with VERSION_2019 -> `String "SERVER_2019" | VERSION_2012 -> `String "SERVER_2012"
...
let unshare_target_to_yojson (x : unshare_target) =
  assoc_to_yojson
    [ ("Type", Some (target_type_to_yojson x.type_)); ("Id", Some (target_id_to_yojson x.id)) ]
...
let share_method_to_yojson (x : share_method) =
  match x with HANDSHAKE -> `String "HANDSHAKE" | ORGANIZATIONS -> `String "ORGANIZATIONS"
...
let region_type_to_yojson (x : region_type) =
  match x with ADDITIONAL -> `String "Additional" | PRIMARY -> `String "Primary"
