...
let make_update_info_entry
  ...
  ?region:(region_ : region_name option) () =
  ({
     last_updated_date_time = last_updated_date_time_;
...
let make_update_directory_setup_request
  ?create_snapshot_before_update:(create_snapshot_before_update_ :
                                   create_snapshot_before_update option)
  ?os_update_settings:(os_update_settings_ : os_update_settings option)
...
let make_update_conditional_forwarder_request
  ~dns_ip_addrs:(dns_ip_addrs_ : dns_ip_addrs)
...
let make_trust ?selective_auth:(selective_auth_ : selective_auth option)
  ...
  ?state_last_updated_date_time:(state_last_updated_date_time_ :
                                  state_last_updated_date_time option)
  ?last_updated_date_time:(last_updated_date_time_ :
                            last_updated_date_time option)
...
let make_snapshot ?start_time:(start_time_ : start_time option)
  ?status:(status_ : snapshot_status option)
  ?name:(name_ : snapshot_name option) ?type_:(type__ : snapshot_type option)
...
let make_snapshot_limits
  ?manual_snapshots_limit_reached:(manual_snapshots_limit_reached_ :
                                    manual_snapshots_limit_reached option)
...
let make_shared_directory
  ?last_updated_date_time:(last_updated_date_time_ :
                            last_updated_date_time option)
  ?created_date_time:(created_date_time_ : created_date_time option)
  ?share_notes:(share_notes_ : notes option)
...
let make_setting_entry
  ...
  ?last_requested_date_time:(last_requested_date_time_ :
                              directory_configuration_setting_last_requested_date_time
                                option)
  ?last_updated_date_time:(last_updated_date_time_ :
                            directory_configuration_setting_last_updated_date_time
                              option)
  ?request_status_message:(request_status_message_ :
                            directory_configuration_setting_request_status_message
                              option)
  ...
  ?requested_value:(requested_value_ :
                     directory_configuration_setting_value option)
  ?applied_value:(applied_value_ :
                   directory_configuration_setting_value option)
  ?allowed_values:(allowed_values_ :
                    directory_configuration_setting_allowed_values option)
...
let make_schema_extension_info
  ...
  ?schema_extension_status_reason:(schema_extension_status_reason_ :
                                    schema_extension_status_reason option)
...
let make_reset_user_password_request
  ~new_password:(new_password_ : user_password)
  ~user_name:(user_name_ : customer_user_name)
  ~directory_id:(directory_id_ : directory_id) () =
  ({
...
let make_reject_shared_directory_request
  ~shared_directory_id:(shared_directory_id_ : directory_id) () =
...
let make_directory_vpc_settings ~subnet_ids:(subnet_ids_ : subnet_ids)
...
let make_log_subscription
  ?subscription_created_date_time:(subscription_created_date_time_ :
                                    subscription_created_date_time option)
...
let make_get_snapshot_limits_request
  ~directory_id:(directory_id_ : directory_id) () =
...
let make_event_topic ?status:(status_ : topic_status option)
  ?created_date_time:(created_date_time_ : created_date_time option)
  ?topic_arn:(topic_arn_ : topic_arn option)
...
let make_describe_trusts_request ?limit:(limit_ : limit option)
  ?next_token:(next_token_ : next_token option)
  ?trust_ids:(trust_ids_ : trust_ids option)
  ?directory_id:(directory_id_ : directory_id option) () =
  ({
     limit = limit_;
     next_token = next_token_;
...
let make_describe_ldaps_settings_request ?limit:(limit_ : page_limit option)
  ?next_token:(next_token_ : next_token option)
  ?type_:(type__ : ldaps_type option)
  ~directory_id:(directory_id_ : directory_id) () =
  ({
     limit = limit_;
     next_token = next_token_;
...
let make_directory_description ?os_version:(os_version_ : os_version option)
  ...
  ?dns_ip_addrs:(dns_ip_addrs_ : dns_ip_addrs option)
  ?description:(description_ : description option)
  ?access_url:(access_url_ : access_url option)
  ?alias:(alias_ : alias_name option)
  ...
  ?name:(name_ : directory_name option)
  ?directory_id:(directory_id_ : directory_id option) () =
  ({
...
let make_describe_client_authentication_settings_request
  ?limit:(limit_ : page_limit option)
  ?next_token:(next_token_ : next_token option)
  ?type_:(type__ : client_authentication_type option)
  ~directory_id:(directory_id_ : directory_id) () =
  ({
...
let make_certificate
  ...
  ?registered_date_time:(registered_date_time_ :
                          certificate_registered_date_time option)
  ?common_name:(common_name_ : certificate_c_n option)
...
let make_delete_directory_request
  ~directory_id:(directory_id_ : directory_id) () =
...
let make_create_directory_request ?tags:(tags_ : tags option)
  ...
  ?description:(description_ : description option)
  ?short_name:(short_name_ : directory_short_name option)
  ~size:(size_ : directory_size) ~password:(password_ : password)
...
let make_attribute ?value:(value_ : attribute_value option)
...
let make_computer
  ...
  ?computer_id:(computer_id_ : si_d option) () =
  ({
...
let make_create_computer_request
  ...
  ?organizational_unit_distinguished_name:(organizational_unit_distinguished_name_
                                            : organizational_unit_d_n option)
  ~password:(password_ : computer_password)
  ~computer_name:(computer_name_ : computer_name)
  ~directory_id:(directory_id_ : directory_id) () =
  ({
     computer_attributes = computer_attributes_;
...
let make_directory_connect_settings
  ~customer_user_name:(customer_user_name_ : user_name)
  ~customer_dns_ips:(customer_dns_ips_ : dns_ip_addrs)
...
let make_add_tags_to_resource_request ~tags:(tags_ : tags)
  ~resource_id:(resource_id_ : resource_id) () =
...
let make_add_ip_routes_request
  ?update_security_group_for_directory_controllers:(update_security_group_for_directory_controllers_
                                                     ...
                                                     update_security_group_for_directory_controllers
                                                       option)
  ~ip_routes:(ip_routes_ : ip_routes)
  ~directory_id:(directory_id_ : directory_id) () =
  ({
     update_security_group_for_directory_controllers =
