...
let make_add_tags_to_resource_request ~resource_arn:(resource_arn_ : string_)
    ~tag_list:(tag_list_ : tag_list) () =
  ({ resource_arn = resource_arn_; tag_list = tag_list_ } : add_tags_to_resource_request)

let make_remove_tags_from_resource_response ~status:(status_ : string_) () =
  ({ status = status_ } : remove_tags_from_resource_response)

let make_remove_tags_from_resource_request ~resource_arn:(resource_arn_ : string_)
    ~tag_key_list:(tag_key_list_ : tag_key_list) () =
  ({ resource_arn = resource_arn_; tag_key_list = tag_key_list_ }
...
let make_modify_luna_client_request ~client_arn:(client_arn_ : client_arn)
    ~certificate:(certificate_ : certificate) () =
  ({ client_arn = client_arn_; certificate = certificate_ } : modify_luna_client_request)

let make_modify_hsm_response ?hsm_arn:(hsm_arn_ : hsm_arn option) () =
  ({ hsm_arn = hsm_arn_ } : modify_hsm_response)

let make_modify_hsm_request ?subnet_id:(subnet_id_ : subnet_id option)
    ?eni_ip:(eni_ip_ : ip_address option) ?iam_role_arn:(iam_role_arn_ : iam_role_arn option)
    ?external_id:(external_id_ : external_id option) ?syslog_ip:(syslog_ip_ : ip_address option)
    ~hsm_arn:(hsm_arn_ : hsm_arn) () =
  ({
     hsm_arn = hsm_arn_;
...
let make_modify_hapg_request ?label:(label_ : label option)
    ?partition_serial_list:(partition_serial_list_ : partition_serial_list option)
    ~hapg_arn:(hapg_arn_ : hapg_arn) () =
  ({ hapg_arn = hapg_arn_; label = label_; partition_serial_list = partition_serial_list_ }
    : modify_hapg_request)
...
let make_list_luna_clients_response ?next_token:(next_token_ : pagination_token option)
  ...
  ({ client_list = client_list_; next_token = next_token_ } : list_luna_clients_response)
...
let make_list_hsms_response ?hsm_list:(hsm_list_ : hsm_list option)
    ?next_token:(next_token_ : pagination_token option) () =
  ({ hsm_list = hsm_list_; next_token = next_token_ } : list_hsms_response)
...
let make_list_hapgs_response ?next_token:(next_token_ : pagination_token option)
  ...
  ({ hapg_list = hapg_list_; next_token = next_token_ } : list_hapgs_response)
...
let make_get_config_response ?config_type:(config_type_ : string_ option)
    ?config_file:(config_file_ : string_ option) ?config_cred:(config_cred_ : string_ option) () =
  ({ config_type = config_type_; config_file = config_file_; config_cred = config_cred_ }
    : get_config_response)

let make_get_config_request ~client_arn:(client_arn_ : client_arn)
    ~client_version:(client_version_ : client_version) ~hapg_list:(hapg_list_ : hapg_list) () =
  ({ client_arn = client_arn_; client_version = client_version_; hapg_list = hapg_list_ }
    : get_config_request)

let make_describe_luna_client_response ?client_arn:(client_arn_ : client_arn option)
    ?certificate:(certificate_ : certificate option)
...
let make_describe_luna_client_request ?client_arn:(client_arn_ : client_arn option)
    ?certificate_fingerprint:(certificate_fingerprint_ : certificate_fingerprint option) () =
  ({ client_arn = client_arn_; certificate_fingerprint = certificate_fingerprint_ }
    : describe_luna_client_request)

let make_describe_hsm_response ?hsm_arn:(hsm_arn_ : hsm_arn option)
    ?status:(status_ : hsm_status option) ?status_details:(status_details_ : string_ option)
    ?availability_zone:(availability_zone_ : a_z option) ?eni_id:(eni_id_ : eni_id option)
    ?eni_ip:(eni_ip_ : ip_address option)
    ...
    ?vendor_name:(vendor_name_ : string_ option) ?hsm_type:(hsm_type_ : string_ option)
    ?software_version:(software_version_ : string_ option)
...
let make_describe_hsm_request ?hsm_arn:(hsm_arn_ : hsm_arn option)
    ?hsm_serial_number:(hsm_serial_number_ : hsm_serial_number option) () =
  ({ hsm_arn = hsm_arn_; hsm_serial_number = hsm_serial_number_ } : describe_hsm_request)

let make_describe_hapg_response ?hapg_arn:(hapg_arn_ : hapg_arn option)
    ?hapg_serial:(hapg_serial_ : string_ option)
    ...
    ?last_modified_timestamp:(last_modified_timestamp_ : timestamp option)
    ?partition_serial_list:(partition_serial_list_ : partition_serial_list option)
    ?state:(state_ : cloud_hsm_object_state option) () =
  ({
     hapg_arn = hapg_arn_;
...
let make_create_luna_client_request ?label:(label_ : client_label option)
    ~certificate:(certificate_ : certificate) () =
  ({ label = label_; certificate = certificate_ } : create_luna_client_request)

let make_create_hsm_response ?hsm_arn:(hsm_arn_ : hsm_arn option) () =
  ({ hsm_arn = hsm_arn_ } : create_hsm_response)

let make_create_hsm_request ?eni_ip:(eni_ip_ : ip_address option)
    ?external_id:(external_id_ : external_id option)
    ?client_token:(client_token_ : client_token option) ?syslog_ip:(syslog_ip_ : ip_address option)
    ~subnet_id:(subnet_id_ : subnet_id) ~ssh_key:(ssh_key_ : ssh_key)
