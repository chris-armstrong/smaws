...
let identity_providers_to_yojson tree =
  map_to_yojson identity_provider_name_to_yojson
    identity_provider_id_to_yojson tree
let developer_provider_name_to_yojson = string_to_yojson
let arn_string_to_yojson = string_to_yojson
let oidc_provider_list_to_yojson tree =
  list_to_yojson arn_string_to_yojson tree
...
let cognito_identity_provider_to_yojson (x : cognito_identity_provider) =
  assoc_to_yojson
    [("ServerSideTokenCheck",
       (option_to_yojson cognito_identity_provider_token_check_to_yojson
          x.server_side_token_check));
    ("ClientId",
      (option_to_yojson cognito_identity_provider_client_id_to_yojson
         x.client_id));
    ("ProviderName",
      (option_to_yojson cognito_identity_provider_name_to_yojson
         x.provider_name))]
let cognito_identity_provider_list_to_yojson tree =
  list_to_yojson cognito_identity_provider_to_yojson tree
let saml_provider_list_to_yojson tree =
  list_to_yojson arn_string_to_yojson tree
...
let identity_pool_to_yojson (x : identity_pool) =
  assoc_to_yojson
    [("IdentityPoolTags",
       (option_to_yojson identity_pool_tags_type_to_yojson
          x.identity_pool_tags));
    ("SamlProviderARNs",
      (option_to_yojson saml_provider_list_to_yojson x.saml_provider_ar_ns));
    ("CognitoIdentityProviders",
      (option_to_yojson cognito_identity_provider_list_to_yojson
         x.cognito_identity_providers));
    ("OpenIdConnectProviderARNs",
      (option_to_yojson oidc_provider_list_to_yojson
         x.open_id_connect_provider_ar_ns));
    ("DeveloperProviderName",
      (option_to_yojson developer_provider_name_to_yojson
         x.developer_provider_name));
    ("SupportedLoginProviders",
      (option_to_yojson identity_providers_to_yojson
         x.supported_login_providers));
    ("AllowClassicFlow",
      (option_to_yojson classic_flow_to_yojson x.allow_classic_flow));
    ("AllowUnauthenticatedIdentities",
      (Some
         (identity_pool_unauthenticated_to_yojson
            x.allow_unauthenticated_identities)));
    ("IdentityPoolName",
...
let logins_map_to_yojson tree =
  map_to_yojson identity_provider_name_to_yojson
    identity_provider_token_to_yojson tree
let logins_list_to_yojson tree =
  list_to_yojson identity_provider_name_to_yojson tree
...
let unlink_developer_identity_input_to_yojson
  (x : unlink_developer_identity_input) =
  assoc_to_yojson
    [("DeveloperUserIdentifier",
       (Some
          (developer_user_identifier_to_yojson x.developer_user_identifier)));
    ("DeveloperProviderName",
...
let principal_tags_to_yojson tree =
  map_to_yojson principal_tag_i_d_to_yojson principal_tag_value_to_yojson
    tree
...
let roles_map_to_yojson tree =
  map_to_yojson role_type_to_yojson arn_string_to_yojson tree
let role_mapping_type_to_yojson (x : role_mapping_type) =
  match x with | RULES -> `String "Rules" | TOKEN -> `String "Token"
let ambiguous_role_resolution_type_to_yojson
  (x : ambiguous_role_resolution_type) =
  match x with
  | DENY -> `String "Deny"
...
let mapping_rules_list_to_yojson tree =
  list_to_yojson mapping_rule_to_yojson tree
...
let role_mapping_to_yojson (x : role_mapping) =
  assoc_to_yojson
    [("RulesConfiguration",
       (option_to_yojson rules_configuration_type_to_yojson
          x.rules_configuration));
    ("AmbiguousRoleResolution",
      (option_to_yojson ambiguous_role_resolution_type_to_yojson
         x.ambiguous_role_resolution));
...
let set_identity_pool_roles_input_to_yojson
  (x : set_identity_pool_roles_input) =
  assoc_to_yojson
    [("RoleMappings",
...
let merge_developer_identities_response_to_yojson
  (x : merge_developer_identities_response) =
  assoc_to_yojson
    [("IdentityId", (option_to_yojson identity_id_to_yojson x.identity_id))]
let merge_developer_identities_input_to_yojson
  (x : merge_developer_identities_input) =
  assoc_to_yojson
    [("IdentityPoolId",
       (Some (identity_pool_id_to_yojson x.identity_pool_id)));
    ("DeveloperProviderName",
    ...
    ("DestinationUserIdentifier",
      (Some
         (developer_user_identifier_to_yojson x.destination_user_identifier)));
...
let lookup_developer_identity_response_to_yojson
  (x : lookup_developer_identity_response) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson pagination_key_to_yojson x.next_token));
    ("DeveloperUserIdentifierList",
      (option_to_yojson developer_user_identifier_list_to_yojson
         x.developer_user_identifier_list));
    ("IdentityId", (option_to_yojson identity_id_to_yojson x.identity_id))]
let lookup_developer_identity_input_to_yojson
  (x : lookup_developer_identity_input) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson pagination_key_to_yojson x.next_token));
    ("MaxResults", (option_to_yojson query_limit_to_yojson x.max_results));
    ("DeveloperUserIdentifier",
      (option_to_yojson developer_user_identifier_to_yojson
         x.developer_user_identifier));
    ("IdentityId", (option_to_yojson identity_id_to_yojson x.identity_id));
    ("IdentityPoolId",
      (Some (identity_pool_id_to_yojson x.identity_pool_id)))]
...
let identity_description_to_yojson (x : identity_description) =
  assoc_to_yojson
    ...
    ("IdentityId", (option_to_yojson identity_id_to_yojson x.identity_id))]
let identities_list_to_yojson tree =
  list_to_yojson identity_description_to_yojson tree
...
let invalid_identity_pool_configuration_exception_to_yojson
  ...
  assoc_to_yojson
    [("message", (option_to_yojson string__to_yojson x.message))]
let identity_id_list_to_yojson tree =
  list_to_yojson identity_id_to_yojson tree
...
let get_principal_tag_attribute_map_input_to_yojson
  (x : get_principal_tag_attribute_map_input) =
  assoc_to_yojson
    [("IdentityProviderName",
       (Some (identity_provider_name_to_yojson x.identity_provider_name)));
    ("IdentityPoolId",
...
let get_identity_pool_roles_response_to_yojson
  (x : get_identity_pool_roles_response) =
  assoc_to_yojson
    [("RoleMappings",
       (option_to_yojson role_mapping_map_to_yojson x.role_mappings));
...
let get_identity_pool_roles_input_to_yojson
  (x : get_identity_pool_roles_input) =
  assoc_to_yojson
    [("IdentityPoolId",
       (Some (identity_pool_id_to_yojson x.identity_pool_id)))]
...
let delete_identities_response_to_yojson (x : delete_identities_response) =
  assoc_to_yojson
    [("UnprocessedIdentityIds",
       (option_to_yojson unprocessed_identity_id_list_to_yojson
          x.unprocessed_identity_ids))]
