...
val make_untag_resource_input :
  tag_keys:identity_pool_tags_list_type -> resource_arn:arn_string -> unit -> untag_resource_input

val make_unprocessed_identity_id :
  ?error_code:error_code -> ?identity_id:identity_id -> unit -> unprocessed_identity_id
...
val make_tag_resource_input :
  tags:identity_pool_tags_type -> resource_arn:arn_string -> unit -> tag_resource_input
...
val make_get_open_id_token_response :
  ?token:oidc_token -> ?identity_id:identity_id -> unit -> get_open_id_token_response
...
[@@ocaml.doc
  "Creates a new identity pool. The identity pool is a store of user identity information that is \
   specific to your Amazon Web Services account. The keys for [SupportedLoginProviders] are as \
   follows:\n\n\
  \ {ul\n\
...
[@@ocaml.doc
  "Deletes an identity pool. Once a pool is deleted, users will not be able to authenticate with \
   the pool.\n\n\
  \ You must use Amazon Web Services developer credentials to call this operation.\n\
  \ "]
...
[@@ocaml.doc
  "Returns credentials for the provided identity ID. Any provided logins will be validated against \
   supported login providers. If the token is for [cognito-identity.amazonaws.com], it will be \
   passed through to Security Token Service with the appropriate role for the token.\n\n\
...
[@@ocaml.doc
  "Gets an OpenID token, using a known Cognito ID. This known Cognito ID is returned by [GetId]. \
   You can optionally add additional logins for the identity. Supplying multiple logins creates an \
   implicit link.\n\n\
...
[@@ocaml.doc
  "Registers (or retrieves) a Cognito [IdentityId] and an OpenID Connect token for a user \
   authenticated by your backend authentication process. Supplying multiple logins will create an \
   implicit linked account. You can only specify one developer provider as part of the [Logins] \
   map, which is linked to the identity pool. The developer provider is the \"domain\" by which \
   Cognito will refer to your users.\n\n\
  \ You can use [GetOpenIdTokenForDeveloperIdentity] to create a new identity and to link new \
   logins (that is, user credentials issued by a public provider or developer provider) to an \
   existing identity. When you want to create a new identity, the [IdentityId] should be null. \
   When you want to associate a new login with an existing authenticated/unauthenticated identity, \
   you can do so by providing the existing [IdentityId]. This API will create the identity in the \
...
[@@ocaml.doc
  "Retrieves the [IdentityID] associated with a [DeveloperUserIdentifier] or the list of \
   ...
   response. If you supply both, [DeveloperUserIdentifier] will be matched against [IdentityID]. \
   If the values are verified against the database, the response returns both values and is the \
  ...
  \  [LookupDeveloperIdentity] is intended for low-throughput control plane operations: for \
   example, to enable customer service to locate an identity ID by username. If you are using it \
   for higher-volume operations such as user authentication, your requests are likely to be \
   throttled. [GetOpenIdTokenForDeveloperIdentity] is a better option for higher-volume operations \
...
[@@ocaml.doc
  "Merges two users having different [IdentityId]s, existing in the same identity pool, and \
   ...
   users be merged and identified as a single user in the Cognito environment. Cognito associates \
   the given source user ([SourceUserIdentifier]) with the [IdentityId] of the \
   [DestinationUserIdentifier]. Only developer-authenticated users can be merged. If the users to \
   be merged are associated with the same public provider, but as two different users, an \
   exception will be thrown.\n\n\
  \ The number of linked logins is limited to 20. So, the number of linked logins for the source \
   user, [SourceUserIdentifier], and the destination user, [DestinationUserIdentifier], together \
...
[@@ocaml.doc
  "Assigns a set of tags to the specified Amazon Cognito identity pool. A tag is a label that you \
   can use to categorize and manage identity pools in different ways, such as by purpose, owner, \
   environment, or other criteria.\n\n\
  \ Each tag consists of a key and value, both of which you define. A key is a general category \
   for more specific values. For example, if you have two versions of an identity pool, one for \
   testing and another for production, you might assign an [Environment] tag key to both identity \
   pools. The value of this key might be [Test] for one identity pool and [Production] for the \
   other.\n\
  \ \n\
  \  Tags are useful for cost tracking and access control. You can activate your tags so that they \
   ...
   with your identity pools. In an IAM policy, you can constrain permissions for identity pools \
   based on specific tags or tag values.\n\
  \  \n\
  \   You can use this action up to 5 times per second, per account. An identity pool can have as \
   many as 50 tags.\n\
  \   "]
...
[@@ocaml.doc
  "Unlinks a [DeveloperUserIdentifier] from an existing identity. Unlinked developer users will be \
   considered new identities next time they are seen. If, for a given Cognito identity, you remove \
   all federated identities as well as the developer user identifier, the Cognito identity becomes \
   inaccessible.\n\n\
  \ You must use Amazon Web Services developer credentials to call this operation.\n\
  \ "]
