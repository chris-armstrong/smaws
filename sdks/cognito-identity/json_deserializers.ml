...
let identity_providers_of_yojson tree path =
  map_of_yojson identity_provider_name_of_yojson identity_provider_id_of_yojson tree path

let developer_provider_name_of_yojson = string_of_yojson
let arn_string_of_yojson = string_of_yojson
let oidc_provider_list_of_yojson tree path = list_of_yojson arn_string_of_yojson tree path
...
let cognito_identity_provider_of_yojson tree path =
  ...
  ({
     server_side_token_check =
       option_of_yojson
         (value_for_key cognito_identity_provider_token_check_of_yojson "ServerSideTokenCheck")
         _list path;
     client_id =
       option_of_yojson
         (value_for_key cognito_identity_provider_client_id_of_yojson "ClientId")
         _list path;
     provider_name =
       option_of_yojson
         (value_for_key cognito_identity_provider_name_of_yojson "ProviderName")
         _list path;
   }
...
let saml_provider_list_of_yojson tree path = list_of_yojson arn_string_of_yojson tree path
  ...
  ({
     identity_pool_tags =
       option_of_yojson
         ...
         _list path;
     saml_provider_ar_ns =
       option_of_yojson (value_for_key saml_provider_list_of_yojson "SamlProviderARNs") _list path;
     cognito_identity_providers =
       option_of_yojson
         (value_for_key cognito_identity_provider_list_of_yojson "CognitoIdentityProviders")
         _list path;
     open_id_connect_provider_ar_ns =
       option_of_yojson
         (value_for_key oidc_provider_list_of_yojson "OpenIdConnectProviderARNs")
         _list path;
     developer_provider_name =
       option_of_yojson
         (value_for_key developer_provider_name_of_yojson "DeveloperProviderName")
         _list path;
     supported_login_providers =
       option_of_yojson
         (value_for_key identity_providers_of_yojson "SupportedLoginProviders")
         _list path;
     allow_classic_flow =
       option_of_yojson (value_for_key classic_flow_of_yojson "AllowClassicFlow") _list path;
     allow_unauthenticated_identities =
       value_for_key identity_pool_unauthenticated_of_yojson "AllowUnauthenticatedIdentities" _list
         path;
...
let logins_map_of_yojson tree path =
  map_of_yojson identity_provider_name_of_yojson identity_provider_token_of_yojson tree path

let logins_list_of_yojson tree path = list_of_yojson identity_provider_name_of_yojson tree path
...
let unlink_developer_identity_input_of_yojson tree path =
  ...
  ({
     developer_user_identifier =
       value_for_key developer_user_identifier_of_yojson "DeveloperUserIdentifier" _list path;
     developer_provider_name =
       value_for_key developer_provider_name_of_yojson "DeveloperProviderName" _list path;
     identity_pool_id = value_for_key identity_pool_id_of_yojson "IdentityPoolId" _list path;
     identity_id = value_for_key identity_id_of_yojson "IdentityId" _list path;
   }
...
let principal_tags_of_yojson tree path =
  map_of_yojson principal_tag_i_d_of_yojson principal_tag_value_of_yojson tree path

let set_principal_tag_attribute_map_response_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     principal_tags =
       option_of_yojson (value_for_key principal_tags_of_yojson "PrincipalTags") _list path;
     ...
     identity_provider_name =
       option_of_yojson
         ...
         _list path;
     identity_pool_id =
       option_of_yojson (value_for_key identity_pool_id_of_yojson "IdentityPoolId") _list path;
   }
...
let set_principal_tag_attribute_map_input_of_yojson tree path =
  ...
  ({
     ...
     use_defaults = option_of_yojson (value_for_key use_defaults_of_yojson "UseDefaults") _list path;
     identity_provider_name =
       value_for_key identity_provider_name_of_yojson "IdentityProviderName" _list path;
     identity_pool_id = value_for_key identity_pool_id_of_yojson "IdentityPoolId" _list path;
   }
...
let ambiguous_role_resolution_type_of_yojson (tree : t) path =
  ((match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "AmbiguousRoleResolutionType" value)
...
let mapping_rule_match_type_of_yojson (tree : t) path =
  ((match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "MappingRuleMatchType" value)
...
let mapping_rules_list_of_yojson tree path = list_of_yojson mapping_rule_of_yojson tree path
...
let role_mapping_of_yojson tree path =
  ...
  ({
     rules_configuration =
       option_of_yojson
         (value_for_key rules_configuration_type_of_yojson "RulesConfiguration")
         _list path;
     ambiguous_role_resolution =
       option_of_yojson
         (value_for_key ambiguous_role_resolution_type_of_yojson "AmbiguousRoleResolution")
         _list path;
...
let role_mapping_map_of_yojson tree path =
  map_of_yojson identity_provider_name_of_yojson role_mapping_of_yojson tree path

let set_identity_pool_roles_input_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     role_mappings =
       option_of_yojson (value_for_key role_mapping_map_of_yojson "RoleMappings") _list path;
...
let merge_developer_identities_input_of_yojson tree path =
  ...
  ({
     ...
     destination_user_identifier =
       value_for_key developer_user_identifier_of_yojson "DestinationUserIdentifier" _list path;
     source_user_identifier =
       value_for_key developer_user_identifier_of_yojson "SourceUserIdentifier" _list path;
   }
...
let lookup_developer_identity_response_of_yojson tree path =
  ...
  ({
     ...
     developer_user_identifier_list =
       option_of_yojson
         (value_for_key developer_user_identifier_list_of_yojson "DeveloperUserIdentifierList")
         _list path;
     identity_id = option_of_yojson (value_for_key identity_id_of_yojson "IdentityId") _list path;
   }
...
let lookup_developer_identity_input_of_yojson tree path =
  ...
  ({
     ...
     developer_user_identifier =
       option_of_yojson
         (value_for_key developer_user_identifier_of_yojson "DeveloperUserIdentifier")
         _list path;
     identity_id = option_of_yojson (value_for_key identity_id_of_yojson "IdentityId") _list path;
     identity_pool_id = value_for_key identity_pool_id_of_yojson "IdentityPoolId" _list path;
   }
...
let identity_pool_short_description_of_yojson tree path =
  ...
  ({
     identity_pool_name =
       option_of_yojson (value_for_key identity_pool_name_of_yojson "IdentityPoolName") _list path;
     identity_pool_id =
       option_of_yojson (value_for_key identity_pool_id_of_yojson "IdentityPoolId") _list path;
   }
...
let list_identity_pools_response_of_yojson tree path =
  ...
  ({
     next_token = option_of_yojson (value_for_key pagination_key_of_yojson "NextToken") _list path;
     identity_pools =
       option_of_yojson (value_for_key identity_pools_list_of_yojson "IdentityPools") _list path;
   }
...
let identities_list_of_yojson tree path = list_of_yojson identity_description_of_yojson tree path
...
let list_identities_input_of_yojson tree path =
  ...
  ({
     hide_disabled =
       option_of_yojson (value_for_key hide_disabled_of_yojson "HideDisabled") _list path;
     next_token = option_of_yojson (value_for_key pagination_key_of_yojson "NextToken") _list path;
     max_results = value_for_key query_limit_of_yojson "MaxResults" _list path;
     identity_pool_id = value_for_key identity_pool_id_of_yojson "IdentityPoolId" _list path;
   }
...
let identity_id_list_of_yojson tree path = list_of_yojson identity_id_of_yojson tree path
...
let get_open_id_token_for_developer_identity_input_of_yojson tree path =
  ...
  ({
     token_duration =
       option_of_yojson (value_for_key token_duration_of_yojson "TokenDuration") _list path;
     principal_tags =
       option_of_yojson (value_for_key principal_tags_of_yojson "PrincipalTags") _list path;
     logins = value_for_key logins_map_of_yojson "Logins" _list path;
     identity_id = option_of_yojson (value_for_key identity_id_of_yojson "IdentityId") _list path;
...
let credentials_of_yojson tree path =
  ...
  ({
     ...
     session_token =
       option_of_yojson (value_for_key session_token_string_of_yojson "SessionToken") _list path;
     secret_key =
       option_of_yojson (value_for_key secret_key_string_of_yojson "SecretKey") _list path;
     access_key_id =
       option_of_yojson (value_for_key access_key_string_of_yojson "AccessKeyId") _list path;
   }
    : credentials)
...
let get_credentials_for_identity_input_of_yojson tree path =
  ...
  ({
     custom_role_arn =
       option_of_yojson (value_for_key arn_string_of_yojson "CustomRoleArn") _list path;
     logins = option_of_yojson (value_for_key logins_map_of_yojson "Logins") _list path;
     identity_id = value_for_key identity_id_of_yojson "IdentityId" _list path;
   }
...
let delete_identities_response_of_yojson tree path =
  ...
  ({
     unprocessed_identity_ids =
       option_of_yojson
         (value_for_key unprocessed_identity_id_list_of_yojson "UnprocessedIdentityIds")
         _list path;
   }
...
let delete_identities_input_of_yojson tree path =
  ...
  ({
     identity_ids_to_delete =
       value_for_key identity_id_list_of_yojson "IdentityIdsToDelete" _list path;
   }
