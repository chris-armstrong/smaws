...
let type__to_yojson (x : type_) =
  match x with RequestContext -> `String "RequestContext" | TagContext -> `String "TagContext"
...
let query_statistics_for_describe_query_to_yojson (x : query_statistics_for_describe_query) =
  assoc_to_yojson
    [
      ...
      ("ExecutionTimeInMillis", option_to_yojson integer__to_yojson x.execution_time_in_millis);
...
let query_statistics_to_yojson (x : query_statistics) =
  assoc_to_yojson
    [
      ("BytesScanned", option_to_yojson long_to_yojson x.bytes_scanned);
      ("TotalResultsCount", option_to_yojson integer__to_yojson x.total_results_count);
...
let insights_metric_data_type_to_yojson (x : insights_metric_data_type) =
  match x with NON_ZERO_DATA -> `String "NonZeroData" | FILL_WITH_ZEROS -> `String "FillWithZeros"
