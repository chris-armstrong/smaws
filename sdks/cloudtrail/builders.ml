...
let make_advanced_field_selector
  ?not_ends_with:(not_ends_with_ : operator option)
  ?not_starts_with:(not_starts_with_ : operator option)
  ?not_equals:(not_equals_ : operator option)
  ?ends_with:(ends_with_ : operator option)
  ?starts_with:(starts_with_ : operator option)
  ?equals:(equals_ : operator option) ~field:(field_ : selector_field) () =
  ({
...
let make_update_event_data_store_response
  ...
  ?termination_protection_enabled:(termination_protection_enabled_ :
                                    termination_protection_enabled option)
  ...
  ?advanced_event_selectors:(advanced_event_selectors_ :
                              advanced_event_selectors option)
...
let make_refresh_schedule_frequency
...
let make_update_dashboard_response
  ...
  ?name:(name_ : dashboard_name option)
...
let make_update_dashboard_request
  ?termination_protection_enabled:(termination_protection_enabled_ :
                                    termination_protection_enabled option)
  ?refresh_schedule:(refresh_schedule_ : refresh_schedule option)
  ?widgets:(widgets_ : request_widget_list option)
...
let make_destination ~location:(location_ : location)
...
let make_update_channel_request ?name:(name_ : channel_name option)
  ?destinations:(destinations_ : destinations option)
  ~channel:(channel_ : channel_arn) () =
  ({ name = name_; destinations = destinations_; channel = channel_ } : 
  update_channel_request)
let make_trail_info ?home_region:(home_region_ : string_ option)
  ?name:(name_ : string_ option) ?trail_ar_n:(trail_ar_n_ : string_ option)
  () =
...
let make_stop_logging_request ~name:(name_ : string_) () =
...
let make_s3_import_source
  ...
  ~s3_location_uri:(s3_location_uri_ : string_) () =
  ({
...
let make_search_sample_queries_response
  ...
  ?search_results:(search_results_ :
                    search_sample_queries_search_results option)
  () =
...
let make_query_statistics_for_describe_query
  ...
  ?events_scanned:(events_scanned_ : long option)
...
let make_event_selector
  ?exclude_management_event_sources:(exclude_management_event_sources_ :
                                      exclude_management_event_sources option)
...
let make_context_key_selector ~equals:(equals_ : operator_target_list)
  ~type_:(type__ : type_) () =
...
let make_public_key ?fingerprint:(fingerprint_ : string_ option)
  ...
  ?value:(value_ : byte_buffer option) () =
  ({
...
let make_partition_key ~type_:(type__ : partition_key_type)
  ~name:(name_ : partition_key_name) () =
  ({ type_ = type__; name = name_ } : partition_key)
let make_event ?cloud_trail_event:(cloud_trail_event_ : string_ option)
  ?resources:(resources_ : resource_list option)
  ?username:(username_ : string_ option)
  ?event_source:(event_source_ : string_ option)
  ?event_time:(event_time_ : date option)
  ?access_key_id:(access_key_id_ : string_ option)
  ?read_only:(read_only_ : string_ option)
  ?event_name:(event_name_ : string_ option)
...
let make_list_insights_metric_data_request
  ...
  ?period:(period_ : insights_metric_period option)
  ?end_time:(end_time_ : date option) ?start_time:(start_time_ : date option)
  ?error_code:(error_code_ : error_code option)
  ~insight_type:(insight_type_ : insight_type)
  ~event_name:(event_name_ : event_name)
...
let make_list_event_data_stores_request
  ?max_results:(max_results_ :
                 list_event_data_stores_max_results_count option)
  ?next_token:(next_token_ : pagination_token option) () =
...
let make_list_dashboards_response
  ?next_token:(next_token_ : pagination_token option)
...
let make_get_trail_request ~name:(name_ : string_) () =
...
let make_describe_trails_response
  ?trail_list:(trail_list_ : trail_list option) () =
...
let make_delete_trail_request ~name:(name_ : string_) () =
...
let make_delete_dashboard_request
  ~dashboard_id:(dashboard_id_ : dashboard_arn) () =
...
let make_create_trail_request ?tags_list:(tags_list_ : tags_list option)
  ...
  ?sns_topic_name:(sns_topic_name_ : string_ option)
  ?s3_key_prefix:(s3_key_prefix_ : string_ option)
  ~s3_bucket_name:(s3_bucket_name_ : string_) ~name:(name_ : string_) () =
  ({
...
let make_add_tags_request ~tags_list:(tags_list_ : tags_list)
  ~resource_id:(resource_id_ : string_) () =
