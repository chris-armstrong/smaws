...
let make_xss_match_tuple ~text_transformation:(text_transformation_ : text_transformation)
    ~field_to_match:(field_to_match_ : field_to_match) () =
  ({ text_transformation = text_transformation_; field_to_match = field_to_match_ }
    : xss_match_tuple)

let make_xss_match_set_update ~xss_match_tuple:(xss_match_tuple_ : xss_match_tuple)
...
let make_xss_match_set ?name:(name_ : resource_name option)
    ...
    ~xss_match_set_id:(xss_match_set_id_ : resource_id) () =
  ({ xss_match_tuples = xss_match_tuples_; name = name_; xss_match_set_id = xss_match_set_id_ }
    : xss_match_set)
...
let make_activated_rule ?excluded_rules:(excluded_rules_ : excluded_rules option)
...
let make_web_acl_summary ~name:(name_ : resource_name) ~web_acl_id:(web_acl_id_ : resource_id) () =
  ({ name = name_; web_acl_id = web_acl_id_ } : web_acl_summary)

let make_web_ac_l ?web_acl_arn:(web_acl_arn_ : resource_arn option)
    ?metric_name:(metric_name_ : metric_name option) ?name:(name_ : resource_name option)
    ~rules:(rules_ : activated_rules) ~default_action:(default_action_ : waf_action)
...
let make_update_xss_match_set_request ~updates:(updates_ : xss_match_set_updates)
    ~change_token:(change_token_ : change_token) ~xss_match_set_id:(xss_match_set_id_ : resource_id)
    () =
  ({ updates = updates_; change_token = change_token_; xss_match_set_id = xss_match_set_id_ }
...
let make_update_web_acl_request ?default_action:(default_action_ : waf_action option)
    ?updates:(updates_ : web_acl_updates option) ~change_token:(change_token_ : change_token)
    ~web_acl_id:(web_acl_id_ : resource_id) () =
  ({
     default_action = default_action_;
     updates = updates_;
...
let make_sql_injection_match_tuple ~text_transformation:(text_transformation_ : text_transformation)
    ~field_to_match:(field_to_match_ : field_to_match) () =
  ({ text_transformation = text_transformation_; field_to_match = field_to_match_ }
...
let make_size_constraint_set_update ~size_constraint:(size_constraint_ : size_constraint)
    ~action:(action_ : change_action) () =
  ({ size_constraint = size_constraint_; action = action_ } : size_constraint_set_update)

let make_update_size_constraint_set_request ~updates:(updates_ : size_constraint_set_updates)
    ~change_token:(change_token_ : change_token)
...
let make_predicate ~data_id:(data_id_ : resource_id) ~type_:(type__ : predicate_type)
    ~negated:(negated_ : negated) () =
  ({ data_id = data_id_; type_ = type__; negated = negated_ } : predicate)

let make_rule_update ~predicate:(predicate_ : predicate) ~action:(action_ : change_action) () =
  ({ predicate = predicate_; action = action_ } : rule_update)

let make_update_rule_request ~updates:(updates_ : rule_updates)
    ~change_token:(change_token_ : change_token) ~rule_id:(rule_id_ : resource_id) () =
  ({ updates = updates_; change_token = change_token_; rule_id = rule_id_ } : update_rule_request)
...
let make_update_rule_group_request ~change_token:(change_token_ : change_token)
    ~updates:(updates_ : rule_group_updates) ~rule_group_id:(rule_group_id_ : resource_id) () =
  ({ change_token = change_token_; updates = updates_; rule_group_id = rule_group_id_ }
...
let make_regex_pattern_set_update
    ...
    ~action:(action_ : change_action) () =
  ({ regex_pattern_string = regex_pattern_string_; action = action_ } : regex_pattern_set_update)

let make_update_regex_pattern_set_request ~change_token:(change_token_ : change_token)
...
let make_regex_match_tuple ~regex_pattern_set_id:(regex_pattern_set_id_ : resource_id)
    ~text_transformation:(text_transformation_ : text_transformation)
    ~field_to_match:(field_to_match_ : field_to_match) () =
  ({
     regex_pattern_set_id = regex_pattern_set_id_;
...
let make_regex_match_set_update ~regex_match_tuple:(regex_match_tuple_ : regex_match_tuple)
    ~action:(action_ : change_action) () =
  ({ regex_match_tuple = regex_match_tuple_; action = action_ } : regex_match_set_update)

let make_update_regex_match_set_request ~change_token:(change_token_ : change_token)
    ~updates:(updates_ : regex_match_set_updates)
    ~regex_match_set_id:(regex_match_set_id_ : resource_id) () =
  ({ change_token = change_token_; updates = updates_; regex_match_set_id = regex_match_set_id_ }
...
let make_update_rate_based_rule_request ~rate_limit:(rate_limit_ : rate_limit)
    ~updates:(updates_ : rule_updates) ~change_token:(change_token_ : change_token)
...
let make_ip_set_update ~ip_set_descriptor:(ip_set_descriptor_ : ip_set_descriptor)
    ~action:(action_ : change_action) () =
...
let make_update_ip_set_request ~updates:(updates_ : ip_set_updates)
    ~change_token:(change_token_ : change_token) ~ip_set_id:(ip_set_id_ : resource_id) () =
...
let make_geo_match_set_update ~geo_match_constraint:(geo_match_constraint_ : geo_match_constraint)
    ~action:(action_ : change_action) () =
  ({ geo_match_constraint = geo_match_constraint_; action = action_ } : geo_match_set_update)

let make_update_geo_match_set_request ~updates:(updates_ : geo_match_set_updates)
    ~change_token:(change_token_ : change_token) ~geo_match_set_id:(geo_match_set_id_ : resource_id)
    () =
  ({ updates = updates_; change_token = change_token_; geo_match_set_id = geo_match_set_id_ }
...
let make_byte_match_tuple ~positional_constraint:(positional_constraint_ : positional_constraint)
    ~text_transformation:(text_transformation_ : text_transformation)
...
let make_byte_match_set_update ~byte_match_tuple:(byte_match_tuple_ : byte_match_tuple)
    ~action:(action_ : change_action) () =
  ({ byte_match_tuple = byte_match_tuple_; action = action_ } : byte_match_set_update)

let make_update_byte_match_set_request ~updates:(updates_ : byte_match_set_updates)
    ~change_token:(change_token_ : change_token)
    ~byte_match_set_id:(byte_match_set_id_ : resource_id) () =
  ({ updates = updates_; change_token = change_token_; byte_match_set_id = byte_match_set_id_ }
...
let make_subscribed_rule_group_summary ~metric_name:(metric_name_ : metric_name)
    ~name:(name_ : resource_name) ~rule_group_id:(rule_group_id_ : resource_id) () =
...
let make_size_constraint_set_summary ~name:(name_ : resource_name)
    ~size_constraint_set_id:(size_constraint_set_id_ : resource_id) () =
  ({ name = name_; size_constraint_set_id = size_constraint_set_id_ } : size_constraint_set_summary)
...
let make_sampled_http_request ?rule_within_rule_group:(rule_within_rule_group_ : resource_id option)
...
let make_rule_summary ~name:(name_ : resource_name) ~rule_id:(rule_id_ : resource_id) () =
...
let make_rule_group ?metric_name:(metric_name_ : metric_name option)
    ?name:(name_ : resource_name option) ~rule_group_id:(rule_group_id_ : resource_id) () =
  ({ metric_name = metric_name_; name = name_; rule_group_id = rule_group_id_ } : rule_group)

let make_rule ?metric_name:(metric_name_ : metric_name option) ?name:(name_ : resource_name option)
    ~predicates:(predicates_ : predicates) ~rule_id:(rule_id_ : resource_id) () =
  ({ predicates = predicates_; metric_name = metric_name_; name = name_; rule_id = rule_id_ }
    : rule)
...
let make_regex_match_set ?regex_match_tuples:(regex_match_tuples_ : regex_match_tuples option)
...
let make_rate_based_rule ?metric_name:(metric_name_ : metric_name option)
    ...
    ~rate_key:(rate_key_ : rate_key) ~match_predicates:(match_predicates_ : predicates)
    ~rule_id:(rule_id_ : resource_id) () =
  ({
     rate_limit = rate_limit_;
     rate_key = rate_key_;
...
let make_list_xss_match_sets_response
  ...
  ({ xss_match_sets = xss_match_sets_; next_marker = next_marker_ } : list_xss_match_sets_response)

let make_list_xss_match_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_subscribed_rule_groups_response
    ...
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ rule_groups = rule_groups_; next_marker = next_marker_ }
    : list_subscribed_rule_groups_response)

let make_list_subscribed_rule_groups_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_sql_injection_match_sets_response
    ...
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ sql_injection_match_sets = sql_injection_match_sets_; next_marker = next_marker_ }
    : list_sql_injection_match_sets_response)

let make_list_sql_injection_match_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_size_constraint_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_resources_for_web_acl_request ?resource_type:(resource_type_ : resource_type option)
    ~web_acl_id:(web_acl_id_ : resource_id) () =
  ({ resource_type = resource_type_; web_acl_id = web_acl_id_ }
...
let make_list_regex_match_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ limit = limit_; next_marker = next_marker_ } : list_regex_match_sets_request)

let make_list_rate_based_rules_response ?rules:(rules_ : rule_summaries option)
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ rules = rules_; next_marker = next_marker_ } : list_rate_based_rules_response)

let make_list_rate_based_rules_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_logging_configurations_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ limit = limit_; next_marker = next_marker_ } : list_logging_configurations_request)

let make_ip_set_summary ~name:(name_ : resource_name) ~ip_set_id:(ip_set_id_ : resource_id) () =
...
let make_list_geo_match_sets_response
    ...
    ?next_marker:(next_marker_ : next_marker option) () =
  ({ geo_match_sets = geo_match_sets_; next_marker = next_marker_ } : list_geo_match_sets_response)

let make_list_geo_match_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_byte_match_sets_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option) () =
...
let make_list_activated_rules_in_rule_group_request ?limit:(limit_ : pagination_limit option)
    ?next_marker:(next_marker_ : next_marker option)
    ?rule_group_id:(rule_group_id_ : resource_id option) () =
  ({ limit = limit_; next_marker = next_marker_; rule_group_id = rule_group_id_ }
...
let make_ip_set ?name:(name_ : resource_name option)
  ...
  ({ ip_set_descriptors = ip_set_descriptors_; name = name_; ip_set_id = ip_set_id_ } : ip_set)
...
let make_get_web_acl_for_resource_response
    ?web_acl_summary:(web_acl_summary_ : web_acl_summary option) () =
  ({ web_acl_summary = web_acl_summary_ } : get_web_acl_for_resource_response)
...
let make_get_sampled_requests_request ~max_items:(max_items_ : get_sampled_requests_max_items)
...
let make_get_rate_based_rule_managed_keys_response ?next_marker:(next_marker_ : next_marker option)
    ?managed_keys:(managed_keys_ : managed_keys option) () =
  ({ next_marker = next_marker_; managed_keys = managed_keys_ }
...
let make_byte_match_set ?name:(name_ : resource_name option)
    ...
    ~byte_match_set_id:(byte_match_set_id_ : resource_id) () =
  ({ byte_match_tuples = byte_match_tuples_; name = name_; byte_match_set_id = byte_match_set_id_ }
    : byte_match_set)
...
let make_delete_xss_match_set_request ~change_token:(change_token_ : change_token)
    ~xss_match_set_id:(xss_match_set_id_ : resource_id) () =
...
let make_delete_sql_injection_match_set_request ~change_token:(change_token_ : change_token)
    ~sql_injection_match_set_id:(sql_injection_match_set_id_ : resource_id) () =
  ({ change_token = change_token_; sql_injection_match_set_id = sql_injection_match_set_id_ }
...
let make_delete_size_constraint_set_request ~change_token:(change_token_ : change_token)
    ~size_constraint_set_id:(size_constraint_set_id_ : resource_id) () =
  ({ change_token = change_token_; size_constraint_set_id = size_constraint_set_id_ }
...
let make_delete_rule_group_request ~change_token:(change_token_ : change_token)
    ~rule_group_id:(rule_group_id_ : resource_id) () =
  ({ change_token = change_token_; rule_group_id = rule_group_id_ } : delete_rule_group_request)
...
let make_delete_regex_pattern_set_request ~change_token:(change_token_ : change_token)
    ~regex_pattern_set_id:(regex_pattern_set_id_ : resource_id) () =
  ({ change_token = change_token_; regex_pattern_set_id = regex_pattern_set_id_ }
...
let make_delete_regex_match_set_request ~change_token:(change_token_ : change_token)
    ~regex_match_set_id:(regex_match_set_id_ : resource_id) () =
...
let make_delete_rate_based_rule_request ~change_token:(change_token_ : change_token)
    ~rule_id:(rule_id_ : resource_id) () =
...
let make_delete_geo_match_set_request ~change_token:(change_token_ : change_token)
    ~geo_match_set_id:(geo_match_set_id_ : resource_id) () =
...
let make_delete_byte_match_set_request ~change_token:(change_token_ : change_token)
    ~byte_match_set_id:(byte_match_set_id_ : resource_id) () =
  ({ change_token = change_token_; byte_match_set_id = byte_match_set_id_ }
    : delete_byte_match_set_request)

let make_create_xss_match_set_response ?change_token:(change_token_ : change_token option)
    ?xss_match_set:(xss_match_set_ : xss_match_set option) () =
  ({ change_token = change_token_; xss_match_set = xss_match_set_ } : create_xss_match_set_response)

let make_create_xss_match_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_xss_match_set_request)

let make_create_web_acl_response ?change_token:(change_token_ : change_token option)
...
let make_create_web_acl_request ?tags:(tags_ : tag_list option)
    ~change_token:(change_token_ : change_token) ~default_action:(default_action_ : waf_action)
...
let make_create_web_acl_migration_stack_request
    ...
    ~s3_bucket_name:(s3_bucket_name_ : s3_bucket_name) ~web_acl_id:(web_acl_id_ : resource_id) () =
  ({
...
let make_create_sql_injection_match_set_response ?change_token:(change_token_ : change_token option)
    ?sql_injection_match_set:(sql_injection_match_set_ : sql_injection_match_set option) () =
  ({ change_token = change_token_; sql_injection_match_set = sql_injection_match_set_ }
    : create_sql_injection_match_set_response)

let make_create_sql_injection_match_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_sql_injection_match_set_request)

let make_create_size_constraint_set_response ?change_token:(change_token_ : change_token option)
    ?size_constraint_set:(size_constraint_set_ : size_constraint_set option) () =
  ({ change_token = change_token_; size_constraint_set = size_constraint_set_ }
    : create_size_constraint_set_response)

let make_create_size_constraint_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_size_constraint_set_request)

let make_create_rule_response ?change_token:(change_token_ : change_token option)
...
let make_create_rule_request ?tags:(tags_ : tag_list option)
    ~change_token:(change_token_ : change_token) ~metric_name:(metric_name_ : metric_name)
    ~name:(name_ : resource_name) () =
  ({ tags = tags_; change_token = change_token_; metric_name = metric_name_; name = name_ }
    : create_rule_request)

let make_create_rule_group_response ?change_token:(change_token_ : change_token option)
...
let make_create_regex_pattern_set_response ?change_token:(change_token_ : change_token option)
    ?regex_pattern_set:(regex_pattern_set_ : regex_pattern_set option) () =
  ({ change_token = change_token_; regex_pattern_set = regex_pattern_set_ }
    : create_regex_pattern_set_response)

let make_create_regex_pattern_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_regex_pattern_set_request)

let make_create_regex_match_set_response ?change_token:(change_token_ : change_token option)
...
let make_create_regex_match_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_regex_match_set_request)

let make_create_rate_based_rule_response ?change_token:(change_token_ : change_token option)
...
let make_create_rate_based_rule_request ?tags:(tags_ : tag_list option)
    ~change_token:(change_token_ : change_token) ~rate_limit:(rate_limit_ : rate_limit)
    ~rate_key:(rate_key_ : rate_key) ~metric_name:(metric_name_ : metric_name)
    ~name:(name_ : resource_name) () =
  ({
     tags = tags_;
     change_token = change_token_;
...
let make_create_ip_set_response ?change_token:(change_token_ : change_token option)
...
let make_create_geo_match_set_response ?change_token:(change_token_ : change_token option)
    ?geo_match_set:(geo_match_set_ : geo_match_set option) () =
  ({ change_token = change_token_; geo_match_set = geo_match_set_ } : create_geo_match_set_response)

let make_create_geo_match_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
  ({ change_token = change_token_; name = name_ } : create_geo_match_set_request)

let make_create_byte_match_set_response ?change_token:(change_token_ : change_token option)
...
let make_create_byte_match_set_request ~change_token:(change_token_ : change_token)
    ~name:(name_ : resource_name) () =
...
let make_associate_web_acl_request ~resource_arn:(resource_arn_ : resource_arn)
    ~web_acl_id:(web_acl_id_ : resource_id) () =
  ({ resource_arn = resource_arn_; web_acl_id = web_acl_id_ } : associate_web_acl_request)
