...
let rec set_attribute_of_yojson tree path = list_of_yojson attribute_value_of_yojson tree path
...
let rec cedar_tag_value_of_yojson (tree : t) path =
  let _list = assoc_of_yojson tree path in
  let key, value_ =
    match _list with
    | (key, value_) :: _ -> (key, value_)
...
and cedar_tag_record_attribute_of_yojson tree path =
