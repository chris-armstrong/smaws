...
let evaluation_mode_to_yojson (x : evaluation_mode) =
  match x with PROACTIVE -> `String "PROACTIVE" | DETECTIVE -> `String "DETECTIVE"
...
let recording_frequency_to_yojson (x : recording_frequency) =
  match x with DAILY -> `String "DAILY" | CONTINUOUS -> `String "CONTINUOUS"
...
let aggregate_conformance_pack_compliance_summary_group_key_to_yojson
  ...
  match x with AWS_REGION -> `String "AWS_REGION" | ACCOUNT_ID -> `String "ACCOUNT_ID"
...
let aggregated_source_type_to_yojson (x : aggregated_source_type) =
  match x with ORGANIZATION -> `String "ORGANIZATION" | ACCOUNT -> `String "ACCOUNT"
...
let aggregate_conformance_pack_compliance_to_yojson (x : aggregate_conformance_pack_compliance) =
  assoc_to_yojson
    [
      ("TotalRuleCount", option_to_yojson integer__to_yojson x.total_rule_count);
      ("NonCompliantRuleCount", option_to_yojson integer__to_yojson x.non_compliant_rule_count);
      ("CompliantRuleCount", option_to_yojson integer__to_yojson x.compliant_rule_count);
      ( "ComplianceType",
        option_to_yojson conformance_pack_compliance_type_to_yojson x.compliance_type );
    ]
