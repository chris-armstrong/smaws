...
let make_time_window ?end_time:(end_time_ : date option) ?start_time:(start_time_ : date option) ()
    =
...
let make_stored_query ?expression:(expression_ : query_expression option)
    ...
    ?query_arn:(query_arn_ : query_arn option) ?query_id:(query_id_ : query_id option)
...
let make_status_detail_filters
  ...
  ({ member_account_rule_status = member_account_rule_status_; account_id = account_id_ }
...
let make_start_resource_evaluation_request ?client_token:(client_token_ : client_token option)
...
let make_start_remediation_execution_response ?failed_items:(failed_items_ : resource_keys option)
...
let make_start_remediation_execution_request ~resource_keys:(resource_keys_ : resource_keys)
...
let make_select_resource_config_response ?next_token:(next_token_ : next_token option)
    ?query_info:(query_info_ : query_info option) ?results:(results_ : results option) () =
  ({ next_token = next_token_; query_info = query_info_; results = results_ }
    : select_resource_config_response)

let make_select_resource_config_request ?next_token:(next_token_ : next_token option)
...
let make_select_aggregate_resource_config_response ?next_token:(next_token_ : next_token option)
    ?query_info:(query_info_ : query_info option) ?results:(results_ : results option) () =
  ({ next_token = next_token_; query_info = query_info_; results = results_ }
    : select_aggregate_resource_config_response)

let make_select_aggregate_resource_config_request ?next_token:(next_token_ : next_token option)
...
let make_put_service_linked_configuration_recorder_response ?name:(name_ : recorder_name option)
...
let make_put_service_linked_configuration_recorder_request ?tags:(tags_ : tags_list option)
  ...
  ({ tags = tags_; service_principal = service_principal_ }
...
let make_retention_configuration
  ...
  ({ retention_period_in_days = retention_period_in_days_; name = name_ } : retention_configuration)
...
let make_put_resource_config_request ?tags:(tags_ : tags option)
    ...
    ~configuration:(configuration_ : configuration) ~resource_id:(resource_id_ : resource_id)
...
let make_remediation_exception ?expiration_time:(expiration_time_ : date option)
...
let make_remediation_exception_resource_key
  ...
  ({ resource_id = resource_id_; resource_type = resource_type_ }
    : remediation_exception_resource_key)

let make_put_remediation_exceptions_request ?expiration_time:(expiration_time_ : date option)
    ?message:(message_ : string_with_char_limit1024 option)
...
let make_remediation_parameter_value ?static_value:(static_value_ : static_value option)
    ?resource_value:(resource_value_ : resource_value option) () =
  ({ static_value = static_value_; resource_value = resource_value_ } : remediation_parameter_value)

let make_ssm_controls ?error_percentage:(error_percentage_ : percentage option)
...
let make_remediation_configuration
    ...
    ?automatic:(automatic_ : boolean_ option) ?resource_type:(resource_type_ : string_ option)
...
let make_failed_remediation_batch ?failed_items:(failed_items_ : remediation_configurations option)
    ?failure_message:(failure_message_ : string_ option) () =
  ({ failed_items = failed_items_; failure_message = failure_message_ } : failed_remediation_batch)
...
let make_conformance_pack_input_parameter ~parameter_value:(parameter_value_ : parameter_value)
...
let make_organization_custom_policy_rule_metadata
    ...
    ?description:(description_ : string_with_char_limit256_min0 option)
    ~policy_text:(policy_text_ : policy_text) ~policy_runtime:(policy_runtime_ : policy_runtime) ()
    =
  ({
...
let make_put_external_evaluation_request
    ...
    ~config_rule_name:(config_rule_name_ : config_rule_name) () =
  ({ external_evaluation = external_evaluation_; config_rule_name = config_rule_name_ }
...
let make_put_evaluations_request ?test_mode:(test_mode_ : boolean_ option)
    ?evaluations:(evaluations_ : evaluations option) ~result_token:(result_token_ : string_) () =
  ({ test_mode = test_mode_; result_token = result_token_; evaluations = evaluations_ }
...
let make_delivery_channel
    ...
    ?s3_bucket_name:(s3_bucket_name_ : string_ option) ?name:(name_ : channel_name option) () =
  ({
...
let make_recording_group ?recording_strategy:(recording_strategy_ : recording_strategy option)
...
let make_configuration_recorder ?service_principal:(service_principal_ : service_principal option)
    ...
    ?role_ar_n:(role_ar_n_ : string_ option) ?name:(name_ : recorder_name option)
    ?arn:(arn_ : amazon_resource_name option) () =
  ({
...
let make_account_aggregation_source ?aws_regions:(aws_regions_ : aggregator_region_list option)
    ?all_aws_regions:(all_aws_regions_ : boolean_ option)
    ~account_ids:(account_ids_ : account_aggregation_source_account_list) () =
  ({ aws_regions = aws_regions_; all_aws_regions = all_aws_regions_; account_ids = account_ids_ }
    : account_aggregation_source)

let make_organization_aggregation_source ?all_aws_regions:(all_aws_regions_ : boolean_ option)
    ?aws_regions:(aws_regions_ : aggregator_region_list option) ~role_arn:(role_arn_ : string_) () =
  ({ all_aws_regions = all_aws_regions_; aws_regions = aws_regions_; role_arn = role_arn_ }
    : organization_aggregation_source)

let make_aggregator_filter_resource_type ?value:(value_ : resource_type_value_list option)
    ?type_:(type__ : aggregator_filter_type option) () =
  ({ value = value_; type_ = type__ } : aggregator_filter_resource_type)

let make_aggregator_filter_service_principal ?value:(value_ : service_principal_value_list option)
    ?type_:(type__ : aggregator_filter_type option) () =
...
let make_aggregator_filters
  ...
  ({ service_principal = service_principal_; resource_type = resource_type_ } : aggregator_filters)
...
let make_source ?custom_policy_details:(custom_policy_details_ : custom_policy_details option)
...
let make_aggregation_authorization ?creation_time:(creation_time_ : date option)
...
let make_list_tags_for_resource_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit option) ~resource_arn:(resource_arn_ : amazon_resource_name) () =
  ({ next_token = next_token_; limit = limit_; resource_arn = resource_arn_ }
    : list_tags_for_resource_request)

let make_list_stored_queries_response ?next_token:(next_token_ : string_ option)
...
let make_list_stored_queries_request ?max_results:(max_results_ : limit option)
    ?next_token:(next_token_ : string_ option) () =
  ({ max_results = max_results_; next_token = next_token_ } : list_stored_queries_request)

let make_resource_evaluation ?evaluation_start_timestamp:(evaluation_start_timestamp_ : date option)
...
let make_list_resource_evaluations_response ?next_token:(next_token_ : string_ option)
...
let make_list_resource_evaluations_request ?next_token:(next_token_ : string_ option)
...
let make_list_discovered_resources_response ?next_token:(next_token_ : next_token option)
...
let make_list_discovered_resources_request ?next_token:(next_token_ : next_token option)
    ?include_deleted_resources:(include_deleted_resources_ : boolean_ option)
    ?limit:(limit_ : limit option) ?resource_name:(resource_name_ : resource_name option)
...
let make_list_conformance_pack_compliance_scores_request
    ?next_token:(next_token_ : next_token option) ?limit:(limit_ : page_size_limit option)
...
let make_configuration_recorder_summary
    ...
    ~recording_scope:(recording_scope_ : recording_scope) ~name:(name_ : recorder_name)
...
let make_list_configuration_recorders_request ?next_token:(next_token_ : next_token option)
...
let make_list_aggregate_discovered_resources_response ?next_token:(next_token_ : next_token option)
...
let make_list_aggregate_discovered_resources_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit option) ?filters:(filters_ : resource_filters option)
    ~resource_type:(resource_type_ : resource_type)
    ~configuration_aggregator_name:(configuration_aggregator_name_ : configuration_aggregator_name)
    () =
  ({
...
let make_relationship ?relationship_name:(relationship_name_ : relationship_name option)
    ?resource_name:(resource_name_ : resource_name option)
    ?resource_id:(resource_id_ : resource_id option)
    ?resource_type:(resource_type_ : resource_type option) () =
  ({
...
let make_configuration_item
    ...
    ?related_events:(related_events_ : related_event_list option) ?tags:(tags_ : tags option)
    ...
    ?resource_name:(resource_name_ : resource_name option)
    ?resource_id:(resource_id_ : resource_id option)
    ?resource_type:(resource_type_ : resource_type option) ?arn:(arn_ : ar_n option)
    ...
    ?account_id:(account_id_ : account_id option) ?version:(version_ : version option) () =
  ({
...
let make_get_resource_config_history_response ?next_token:(next_token_ : next_token option)
...
let make_get_resource_config_history_request ?next_token:(next_token_ : next_token option)
    ...
    ?later_time:(later_time_ : later_time option) ~resource_id:(resource_id_ : resource_id)
    ~resource_type:(resource_type_ : resource_type) () =
  ({
     next_token = next_token_;
     limit = limit_;
...
let make_organization_conformance_pack_detailed_status
    ...
    ?error_message:(error_message_ : string_ option) ?error_code:(error_code_ : string_ option)
...
let make_get_organization_conformance_pack_detailed_status_request
    ?next_token:(next_token_ : string_ option) ?limit:(limit_ : cosmos_page_limit option)
...
let make_member_account_status ?last_update_time:(last_update_time_ : date option)
    ?error_message:(error_message_ : string_ option) ?error_code:(error_code_ : string_ option)
...
let make_get_discovered_resource_counts_response ?next_token:(next_token_ : next_token option)
...
let make_get_discovered_resource_counts_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit option) ?resource_types:(resource_types_ : resource_types option) () =
  ({ next_token = next_token_; limit = limit_; resource_types = resource_types_ }
...
let make_get_conformance_pack_compliance_summary_request
    ...
    =
  ({ next_token = next_token_; limit = limit_; conformance_pack_names = conformance_pack_names_ }
    : get_conformance_pack_compliance_summary_request)

let make_evaluation_result_qualifier ?evaluation_mode:(evaluation_mode_ : evaluation_mode option)
...
let make_compliance_contributor_count ?cap_exceeded:(cap_exceeded_ : boolean_ option)
...
let make_get_compliance_details_by_resource_response ?next_token:(next_token_ : string_ option)
...
let make_get_compliance_details_by_config_rule_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit option) ?compliance_types:(compliance_types_ : compliance_types option)
    ~config_rule_name:(config_rule_name_ : string_with_char_limit64) () =
  ({
     next_token = next_token_;
     limit = limit_;
...
let make_resource_count_filters ?region:(region_ : aws_region option)
    ?account_id:(account_id_ : account_id option)
    ?resource_type:(resource_type_ : resource_type option) () =
  ({ region = region_; account_id = account_id_; resource_type = resource_type_ }
    : resource_count_filters)

let make_get_aggregate_discovered_resource_counts_request
    ?next_token:(next_token_ : next_token option) ?limit:(limit_ : group_by_api_limit option)
...
let make_aggregate_conformance_pack_compliance_summary_filters
    ?aws_region:(aws_region_ : aws_region option) ?account_id:(account_id_ : account_id option) () =
  ({ aws_region = aws_region_; account_id = account_id_ }
...
let make_config_rule_compliance_summary_filters ?aws_region:(aws_region_ : aws_region option)
    ?account_id:(account_id_ : account_id option) () =
...
let make_get_aggregate_compliance_details_by_config_rule_response
  ...
  ({ next_token = next_token_; aggregate_evaluation_results = aggregate_evaluation_results_ }
...
let make_get_aggregate_compliance_details_by_config_rule_request
    ...
    ~aws_region:(aws_region_ : aws_region) ~account_id:(account_id_ : account_id)
    ~config_rule_name:(config_rule_name_ : config_rule_name)
    ~configuration_aggregator_name:(configuration_aggregator_name_ : configuration_aggregator_name)
    () =
  ({
...
let make_disassociate_resource_types_request ~resource_types:(resource_types_ : resource_type_list)
    ~configuration_recorder_arn:(configuration_recorder_arn_ : amazon_resource_name) () =
  ({ resource_types = resource_types_; configuration_recorder_arn = configuration_recorder_arn_ }
    : disassociate_resource_types_request)

let make_describe_retention_configurations_response ?next_token:(next_token_ : next_token option)
    ?retention_configurations:(retention_configurations_ : retention_configuration_list option) () =
  ({ next_token = next_token_; retention_configurations = retention_configurations_ }
    : describe_retention_configurations_response)

let make_describe_retention_configurations_request ?next_token:(next_token_ : next_token option)
    ?retention_configuration_names:
      (retention_configuration_names_ : retention_configuration_name_list option) () =
  ({ next_token = next_token_; retention_configuration_names = retention_configuration_names_ }
    : describe_retention_configurations_request)

let make_remediation_execution_step ?stop_time:(stop_time_ : date option)
    ?start_time:(start_time_ : date option) ?error_message:(error_message_ : string_ option)
    ?state:(state_ : remediation_execution_step_state option) ?name:(name_ : string_ option) () =
  ({
...
let make_remediation_execution_status ?last_updated_time:(last_updated_time_ : date option)
...
let make_describe_remediation_execution_status_response ?next_token:(next_token_ : string_ option)
    ?remediation_execution_statuses:
      (remediation_execution_statuses_ : remediation_execution_statuses option) () =
  ({ next_token = next_token_; remediation_execution_statuses = remediation_execution_statuses_ }
    : describe_remediation_execution_status_response)

let make_describe_remediation_execution_status_request ?next_token:(next_token_ : string_ option)
    ?limit:(limit_ : limit option) ?resource_keys:(resource_keys_ : resource_keys option)
    ~config_rule_name:(config_rule_name_ : config_rule_name) () =
  ({
     next_token = next_token_;
     limit = limit_;
...
let make_describe_remediation_exceptions_response ?next_token:(next_token_ : string_ option)
    ?remediation_exceptions:(remediation_exceptions_ : remediation_exceptions option) () =
  ({ next_token = next_token_; remediation_exceptions = remediation_exceptions_ }
    : describe_remediation_exceptions_response)

let make_describe_remediation_exceptions_request ?next_token:(next_token_ : string_ option)
    ?limit:(limit_ : limit option)
...
let make_pending_aggregation_request
  ...
  ({ requester_aws_region = requester_aws_region_; requester_account_id = requester_account_id_ }
    : pending_aggregation_request)

let make_describe_pending_aggregation_requests_response ?next_token:(next_token_ : string_ option)
    ?pending_aggregation_requests:
      (pending_aggregation_requests_ : pending_aggregation_request_list option) () =
  ({ next_token = next_token_; pending_aggregation_requests = pending_aggregation_requests_ }
    : describe_pending_aggregation_requests_response)

let make_describe_pending_aggregation_requests_request ?next_token:(next_token_ : string_ option)
...
let make_organization_conformance_pack_status ?last_update_time:(last_update_time_ : date option)
    ?error_message:(error_message_ : string_ option) ?error_code:(error_code_ : string_ option)
...
let make_describe_organization_conformance_packs_response ?next_token:(next_token_ : string_ option)
    ?organization_conformance_packs:
      (organization_conformance_packs_ : organization_conformance_packs option) () =
  ({ next_token = next_token_; organization_conformance_packs = organization_conformance_packs_ }
    : describe_organization_conformance_packs_response)

let make_describe_organization_conformance_packs_request ?next_token:(next_token_ : string_ option)
...
let make_organization_config_rule_status ?last_update_time:(last_update_time_ : date option)
    ?error_message:(error_message_ : string_ option) ?error_code:(error_code_ : string_ option)
...
let make_describe_organization_config_rules_response ?next_token:(next_token_ : string_ option)
    ?organization_config_rules:(organization_config_rules_ : organization_config_rules option) () =
  ({ next_token = next_token_; organization_config_rules = organization_config_rules_ }
    : describe_organization_config_rules_response)

let make_describe_organization_config_rules_request ?next_token:(next_token_ : string_ option)
    ?limit:(limit_ : cosmos_page_limit option)
    ?organization_config_rule_names:
      (organization_config_rule_names_ : organization_config_rule_names option) () =
  ({
...
let make_config_export_delivery_info ?next_delivery_time:(next_delivery_time_ : date option)
...
let make_describe_conformance_pack_status_response ?next_token:(next_token_ : next_token option)
    ?conformance_pack_status_details:
      (conformance_pack_status_details_ : conformance_pack_status_details_list option) () =
  ({ next_token = next_token_; conformance_pack_status_details = conformance_pack_status_details_ }
    : describe_conformance_pack_status_response)

let make_describe_conformance_pack_status_request ?next_token:(next_token_ : next_token option)
...
let make_describe_conformance_packs_response ?next_token:(next_token_ : next_token option)
    ?conformance_pack_details:(conformance_pack_details_ : conformance_pack_detail_list option) () =
  ({ next_token = next_token_; conformance_pack_details = conformance_pack_details_ }
    : describe_conformance_packs_response)

let make_describe_conformance_packs_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : page_size_limit option)
    ?conformance_pack_names:(conformance_pack_names_ : conformance_pack_names_list option) () =
  ({ next_token = next_token_; limit = limit_; conformance_pack_names = conformance_pack_names_ }
    : describe_conformance_packs_request)

let make_conformance_pack_rule_compliance ?controls:(controls_ : controls_list option)
    ?compliance_type:(compliance_type_ : conformance_pack_compliance_type option)
    ?config_rule_name:(config_rule_name_ : config_rule_name option) () =
  ({
     controls = controls_;
...
let make_conformance_pack_compliance_filters
    ?compliance_type:(compliance_type_ : conformance_pack_compliance_type option)
    ?config_rule_names:(config_rule_names_ : conformance_pack_config_rule_names option) () =
  ({ compliance_type = compliance_type_; config_rule_names = config_rule_names_ }
    : conformance_pack_compliance_filters)

let make_describe_conformance_pack_compliance_request ?next_token:(next_token_ : next_token option)
...
let make_configuration_recorder_status
    ...
    ?last_error_message:(last_error_message_ : string_ option)
    ?last_error_code:(last_error_code_ : string_ option)
    ?last_status:(last_status_ : recorder_status option) ?recording:(recording_ : boolean_ option)
    ?last_stop_time:(last_stop_time_ : date option)
    ?last_start_time:(last_start_time_ : date option) ?name:(name_ : string_ option)
    ?arn:(arn_ : amazon_resource_name option) () =
  ({
     service_principal = service_principal_;
     last_status_change_time = last_status_change_time_;
...
let make_describe_configuration_recorder_status_request ?arn:(arn_ : amazon_resource_name option)
    ?service_principal:(service_principal_ : service_principal option)
...
let make_describe_configuration_recorders_request ?arn:(arn_ : amazon_resource_name option)
    ?service_principal:(service_principal_ : service_principal option)
    ?configuration_recorder_names:
      (configuration_recorder_names_ : configuration_recorder_name_list option) () =
  ({
...
let make_aggregated_source_status ?last_error_message:(last_error_message_ : string_ option)
    ?last_error_code:(last_error_code_ : string_ option)
    ?last_update_time:(last_update_time_ : date option)
...
let make_describe_configuration_aggregator_sources_status_response
  ...
  ({ next_token = next_token_; aggregated_source_status_list = aggregated_source_status_list_ }
    : describe_configuration_aggregator_sources_status_response)

let make_describe_configuration_aggregator_sources_status_request ?limit:(limit_ : limit option)
    ?next_token:(next_token_ : string_ option)
...
let make_describe_configuration_aggregators_response ?next_token:(next_token_ : string_ option)
    ?configuration_aggregators:(configuration_aggregators_ : configuration_aggregator_list option)
    () =
  ({ next_token = next_token_; configuration_aggregators = configuration_aggregators_ }
    : describe_configuration_aggregators_response)

let make_describe_configuration_aggregators_request ?limit:(limit_ : limit option)
    ?next_token:(next_token_ : string_ option)
...
let make_describe_config_rules_response ?next_token:(next_token_ : string_ option)
...
let make_describe_config_rules_request ?filters:(filters_ : describe_config_rules_filters option)
  ...
  ({ filters = filters_; next_token = next_token_; config_rule_names = config_rule_names_ }
...
let make_describe_config_rule_evaluation_status_response ?next_token:(next_token_ : string_ option)
    ?config_rules_evaluation_status:
      (config_rules_evaluation_status_ : config_rule_evaluation_status_list option) () =
  ({ next_token = next_token_; config_rules_evaluation_status = config_rules_evaluation_status_ }
    : describe_config_rule_evaluation_status_response)

let make_describe_config_rule_evaluation_status_request ?limit:(limit_ : rule_limit option)
    ?next_token:(next_token_ : string_ option)
    ?config_rule_names:(config_rule_names_ : config_rule_names option) () =
  ({ limit = limit_; next_token = next_token_; config_rule_names = config_rule_names_ }
...
let make_compliance_by_resource ?compliance:(compliance_ : compliance option)
    ?resource_id:(resource_id_ : base_resource_id option)
    ?resource_type:(resource_type_ : string_with_char_limit256 option) () =
  ({ compliance = compliance_; resource_id = resource_id_; resource_type = resource_type_ }
    : compliance_by_resource)

let make_describe_compliance_by_resource_response ?next_token:(next_token_ : next_token option)
    ?compliance_by_resources:(compliance_by_resources_ : compliance_by_resources option) () =
  ({ next_token = next_token_; compliance_by_resources = compliance_by_resources_ }
    : describe_compliance_by_resource_response)

let make_describe_compliance_by_resource_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit option) ?compliance_types:(compliance_types_ : compliance_types option)
    ?resource_id:(resource_id_ : base_resource_id option)
    ?resource_type:(resource_type_ : string_with_char_limit256 option) () =
  ({
...
let make_compliance_by_config_rule ?compliance:(compliance_ : compliance option)
    ?config_rule_name:(config_rule_name_ : string_with_char_limit64 option) () =
  ({ compliance = compliance_; config_rule_name = config_rule_name_ } : compliance_by_config_rule)

let make_describe_compliance_by_config_rule_response ?next_token:(next_token_ : string_ option)
    ?compliance_by_config_rules:(compliance_by_config_rules_ : compliance_by_config_rules option) ()
    =
  ({ next_token = next_token_; compliance_by_config_rules = compliance_by_config_rules_ }
    : describe_compliance_by_config_rule_response)

let make_describe_compliance_by_config_rule_request ?next_token:(next_token_ : string_ option)
    ?compliance_types:(compliance_types_ : compliance_types option)
    ?config_rule_names:(config_rule_names_ : config_rule_names option) () =
  ({
     next_token = next_token_;
...
let make_describe_aggregation_authorizations_response ?next_token:(next_token_ : string_ option)
    ?aggregation_authorizations:
      (aggregation_authorizations_ : aggregation_authorization_list option) () =
  ({ next_token = next_token_; aggregation_authorizations = aggregation_authorizations_ }
    : describe_aggregation_authorizations_response)

let make_describe_aggregation_authorizations_request ?next_token:(next_token_ : string_ option)
    ?limit:(limit_ : limit option) () =
...
let make_aggregate_compliance_by_conformance_pack ?aws_region:(aws_region_ : aws_region option)
    ?account_id:(account_id_ : account_id option)
...
let make_aggregate_conformance_pack_compliance_filters ?aws_region:(aws_region_ : aws_region option)
    ?account_id:(account_id_ : account_id option)
    ?compliance_type:(compliance_type_ : conformance_pack_compliance_type option)
    ?conformance_pack_name:(conformance_pack_name_ : conformance_pack_name option) () =
  ({
...
let make_aggregate_compliance_by_config_rule ?aws_region:(aws_region_ : aws_region option)
    ?account_id:(account_id_ : account_id option) ?compliance:(compliance_ : compliance option)
    ?config_rule_name:(config_rule_name_ : config_rule_name option) () =
  ({
     aws_region = aws_region_;
     account_id = account_id_;
...
let make_config_rule_compliance_filters ?aws_region:(aws_region_ : aws_region option)
    ?account_id:(account_id_ : account_id option)
    ?compliance_type:(compliance_type_ : compliance_type option)
    ?config_rule_name:(config_rule_name_ : config_rule_name option) () =
  ({
...
let make_delete_service_linked_configuration_recorder_response ~name:(name_ : recorder_name)
    ~arn:(arn_ : amazon_resource_name) () =
...
let make_delete_resource_config_request ~resource_id:(resource_id_ : resource_id)
    ~resource_type:(resource_type_ : resource_type_string) () =
...
let make_associate_resource_types_request ~resource_types:(resource_types_ : resource_type_list)
    ~configuration_recorder_arn:(configuration_recorder_arn_ : amazon_resource_name) () =
  ({ resource_types = resource_types_; configuration_recorder_arn = configuration_recorder_arn_ }
