...
let approximate_creation_date_time_precision_to_yojson
  ...
  match x with MICROSECOND -> `String "MICROSECOND" | MILLISECOND -> `String "MILLISECOND"
...
let attribute_action_to_yojson (x : attribute_action) =
  match x with DELETE -> `String "DELETE" | PUT -> `String "PUT" | ADD -> `String "ADD"
...
let return_consumed_capacity_to_yojson (x : return_consumed_capacity) =
  match x with NONE -> `String "NONE" | TOTAL -> `String "TOTAL" | INDEXES -> `String "INDEXES"
...
let export_format_to_yojson (x : export_format) =
  match x with ION -> `String "ION" | DYNAMODB_JSON -> `String "DYNAMODB_JSON"
