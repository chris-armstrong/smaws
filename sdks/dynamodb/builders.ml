...
let make_write_request ?delete_request:(delete_request_ : delete_request option)
    ?put_request:(put_request_ : put_request option) () =
  ({ delete_request = delete_request_; put_request = put_request_ } : write_request)

let make_warm_throughput ?write_units_per_second:(write_units_per_second_ : long_object option)
...
let make_time_to_live_specification ~attribute_name:(attribute_name_ : time_to_live_attribute_name)
...
let make_update_time_to_live_input
  ...
  ({ time_to_live_specification = time_to_live_specification_; table_name = table_name_ }
...
let make_replica_auto_scaling_description ?replica_status:(replica_status_ : replica_status option)
...
let make_table_auto_scaling_description
  ...
  ({ replicas = replicas_; table_status = table_status_; table_name = table_name_ }
...
let make_attribute_definition ~attribute_type:(attribute_type_ : scalar_attribute_type)
    ~attribute_name:(attribute_name_ : key_schema_attribute_name) () =
  ({ attribute_type = attribute_type_; attribute_name = attribute_name_ } : attribute_definition)
...
let make_projection ?non_key_attributes:(non_key_attributes_ : non_key_attribute_name_list option)
    ?projection_type:(projection_type_ : projection_type option) () =
  ({ non_key_attributes = non_key_attributes_; projection_type = projection_type_ } : projection)

let make_local_secondary_index_description ?index_arn:(index_arn_ : string_ option)
    ?item_count:(item_count_ : long_object option)
    ?index_size_bytes:(index_size_bytes_ : long_object option)
    ?projection:(projection_ : projection option) ?key_schema:(key_schema_ : key_schema option)
    ?index_name:(index_name_ : index_name option) () =
  ({
...
let make_global_secondary_index_warm_throughput_description ?status:(status_ : index_status option)
...
let make_global_secondary_index_description
    ...
    ?index_arn:(index_arn_ : string_ option) ?item_count:(item_count_ : long_object option)
    ?index_size_bytes:(index_size_bytes_ : long_object option)
...
let make_table_warm_throughput_description ?status:(status_ : table_status option)
    ?write_units_per_second:(write_units_per_second_ : positive_long_object option)
    ?read_units_per_second:(read_units_per_second_ : positive_long_object option) () =
  ({
     status = status_;
...
let make_table_class_summary ?last_update_date_time:(last_update_date_time_ : date option)
    ?table_class:(table_class_ : table_class option) () =
  ({ last_update_date_time = last_update_date_time_; table_class = table_class_ }
    : table_class_summary)
...
let make_global_table_witness_description ?witness_status:(witness_status_ : witness_status option)
    ?region_name:(region_name_ : region_name option) () =
  ({ witness_status = witness_status_; region_name = region_name_ }
    : global_table_witness_description)

let make_restore_summary ?source_table_arn:(source_table_arn_ : table_arn option)
...
let make_sse_description
    ...
    ?sse_type:(sse_type_ : sse_type option) ?status:(status_ : sse_status option) () =
  ({
...
let make_archival_summary ?archival_backup_arn:(archival_backup_arn_ : backup_arn option)
...
let make_table_description
    ...
    ?table_id:(table_id_ : table_id option) ?table_arn:(table_arn_ : string_ option)
    ?item_count:(item_count_ : long_object option)
    ?table_size_bytes:(table_size_bytes_ : long_object option)
    ?provisioned_throughput:(provisioned_throughput_ : provisioned_throughput_description option)
    ?creation_date_time:(creation_date_time_ : date option)
    ?table_status:(table_status_ : table_status option)
    ?key_schema:(key_schema_ : key_schema option) ?table_name:(table_name_ : table_name option)
...
let make_provisioned_throughput ~write_capacity_units:(write_capacity_units_ : positive_long_object)
    ~read_capacity_units:(read_capacity_units_ : positive_long_object) () =
  ({ write_capacity_units = write_capacity_units_; read_capacity_units = read_capacity_units_ }
...
let make_create_global_secondary_index_action
    ...
    ?on_demand_throughput:(on_demand_throughput_ : on_demand_throughput option)
    ?provisioned_throughput:(provisioned_throughput_ : provisioned_throughput option)
    ~projection:(projection_ : projection) ~key_schema:(key_schema_ : key_schema)
    ~index_name:(index_name_ : index_name) () =
  ({
     warm_throughput = warm_throughput_;
     on_demand_throughput = on_demand_throughput_;
...
let make_sse_specification ?kms_master_key_id:(kms_master_key_id_ : kms_master_key_id option)
    ?sse_type:(sse_type_ : sse_type option) ?enabled:(enabled_ : sse_enabled option) () =
  ({ kms_master_key_id = kms_master_key_id_; sse_type = sse_type_; enabled = enabled_ }
    : sse_specification)
...
let make_replication_group_update ?delete:(delete_ : delete_replication_group_member_action option)
...
let make_update_kinesis_streaming_destination_output
    ...
    ?stream_arn:(stream_arn_ : stream_arn option) ?table_name:(table_name_ : table_name option) () =
  ({
...
let make_update_kinesis_streaming_destination_input
    ?update_kinesis_streaming_configuration:
      (update_kinesis_streaming_configuration_ : update_kinesis_streaming_configuration option)
    ~stream_arn:(stream_arn_ : stream_arn) ~table_name:(table_name_ : table_arn) () =
  ({
     update_kinesis_streaming_configuration = update_kinesis_streaming_configuration_;
     stream_arn = stream_arn_;
     table_name = table_name_;
...
let make_capacity ?capacity_units:(capacity_units_ : consumed_capacity_units option)
...
let make_expected_attribute_value
    ...
    ?exists:(exists_ : boolean_object option) ?value:(value_ : attribute_value option) () =
  ({
...
let make_update_item_input
    ...
    ?attribute_updates:(attribute_updates_ : attribute_updates option) ~key:(key_ : key)
    ~table_name:(table_name_ : table_arn) () =
  ({
...
let make_replica_global_secondary_index_settings_description
    ...
    ?index_status:(index_status_ : index_status option) ~index_name:(index_name_ : index_name) () =
  ({
     provisioned_write_capacity_auto_scaling_settings =
       provisioned_write_capacity_auto_scaling_settings_;
...
let make_update_global_table_settings_output
  ...
  ({ replica_settings = replica_settings_; global_table_name = global_table_name_ }
...
let make_replica_settings_update ?replica_table_class:(replica_table_class_ : table_class option)
    ?replica_global_secondary_index_settings_update:
...
let make_global_table_description ?global_table_name:(global_table_name_ : table_name option)
...
let make_update_global_table_input ~replica_updates:(replica_updates_ : replica_update_list)
    ~global_table_name:(global_table_name_ : table_name) () =
  ({ replica_updates = replica_updates_; global_table_name = global_table_name_ }
...
let make_update
    ...
    ?condition_expression:(condition_expression_ : condition_expression option)
    ~table_name:(table_name_ : table_arn)
    ~update_expression:(update_expression_ : update_expression) ~key:(key_ : key) () =
  ({
     return_values_on_condition_check_failure = return_values_on_condition_check_failure_;
     expression_attribute_values = expression_attribute_values_;
     expression_attribute_names = expression_attribute_names_;
...
let make_cancellation_reason ?message:(message_ : error_message option) ?code:(code_ : code option)
...
let make_transact_write_items_output
  ...
  ({ item_collection_metrics = item_collection_metrics_; consumed_capacity = consumed_capacity_ }
...
let make_transact_write_item ?update:(update_ : update option) ?delete:(delete_ : delete option)
    ?put:(put_ : put option) ?condition_check:(condition_check_ : condition_check option) () =
  ({ update = update_; delete = delete_; put = put_; condition_check = condition_check_ }
    : transact_write_item)
...
let make_transact_get_items_output ?responses:(responses_ : item_response_list option)
    ?consumed_capacity:(consumed_capacity_ : consumed_capacity_multiple option) () =
  ({ responses = responses_; consumed_capacity = consumed_capacity_ } : transact_get_items_output)
...
let make_transact_get_items_input
  ...
  ({ return_consumed_capacity = return_consumed_capacity_; transact_items = transact_items_ }
...
let make_time_to_live_description
  ...
  ({ attribute_name = attribute_name_; time_to_live_status = time_to_live_status_ }
...
let make_global_secondary_index ?warm_throughput:(warm_throughput_ : warm_throughput option)
    ?on_demand_throughput:(on_demand_throughput_ : on_demand_throughput option)
    ?provisioned_throughput:(provisioned_throughput_ : provisioned_throughput option)
    ~projection:(projection_ : projection) ~key_schema:(key_schema_ : key_schema)
    ~index_name:(index_name_ : index_name) () =
...
let make_table_creation_parameters
    ...
    ?on_demand_throughput:(on_demand_throughput_ : on_demand_throughput option)
    ?provisioned_throughput:(provisioned_throughput_ : provisioned_throughput option)
    ?billing_mode:(billing_mode_ : billing_mode option) ~key_schema:(key_schema_ : key_schema)
...
let make_local_secondary_index_info ?projection:(projection_ : projection option)
    ?key_schema:(key_schema_ : key_schema option) ?index_name:(index_name_ : index_name option) () =
  ({ projection = projection_; key_schema = key_schema_; index_name = index_name_ }
...
let make_source_table_feature_details ?sse_description:(sse_description_ : sse_description option)
...
let make_source_table_details ?billing_mode:(billing_mode_ : billing_mode option)
...
let make_scan_output ?consumed_capacity:(consumed_capacity_ : consumed_capacity option)
    ?last_evaluated_key:(last_evaluated_key_ : key option)
    ?scanned_count:(scanned_count_ : integer option) ?count:(count_ : integer option)
...
let make_condition ?attribute_value_list:(attribute_value_list_ : attribute_value_list option)
  ...
  ({ comparison_operator = comparison_operator_; attribute_value_list = attribute_value_list_ }
    : condition)

let make_scan_input ?consistent_read:(consistent_read_ : consistent_read option)
    ?expression_attribute_values:
      (expression_attribute_values_ : expression_attribute_value_map option)
    ?expression_attribute_names:(expression_attribute_names_ : expression_attribute_name_map option)
    ...
    ?conditional_operator:(conditional_operator_ : conditional_operator option)
    ?scan_filter:(scan_filter_ : filter_condition_map option) ?select:(select_ : select option)
    ?limit:(limit_ : positive_integer_object option)
    ?attributes_to_get:(attributes_to_get_ : attribute_name_list option)
    ?index_name:(index_name_ : index_name option) ~table_name:(table_name_ : table_arn) () =
  ({
...
let make_s3_bucket_source ?s3_key_prefix:(s3_key_prefix_ : s3_prefix option)
    ?s3_bucket_owner:(s3_bucket_owner_ : s3_bucket_owner option) ~s3_bucket:(s3_bucket_ : s3_bucket)
    () =
  ({ s3_key_prefix = s3_key_prefix_; s3_bucket = s3_bucket_; s3_bucket_owner = s3_bucket_owner_ }
    : s3_bucket_source)
...
let make_local_secondary_index ~projection:(projection_ : projection)
    ~key_schema:(key_schema_ : key_schema) ~index_name:(index_name_ : index_name) () =
  ({ projection = projection_; key_schema = key_schema_; index_name = index_name_ }
...
let make_restore_table_from_backup_input
    ...
    ?global_secondary_index_override:
      (global_secondary_index_override_ : global_secondary_index_list option)
    ?billing_mode_override:(billing_mode_override_ : billing_mode option)
    ~backup_arn:(backup_arn_ : backup_arn) ~target_table_name:(target_table_name_ : table_name) () =
  ({
     sse_specification_override = sse_specification_override_;
     on_demand_throughput_override = on_demand_throughput_override_;
     provisioned_throughput_override = provisioned_throughput_override_;
...
let make_query_output ?consumed_capacity:(consumed_capacity_ : consumed_capacity option)
    ?last_evaluated_key:(last_evaluated_key_ : key option)
    ?scanned_count:(scanned_count_ : integer option) ?count:(count_ : integer option)
    ?items:(items_ : item_list option) () =
  ({
...
let make_query_input
    ...
    ?limit:(limit_ : positive_integer_object option)
    ?attributes_to_get:(attributes_to_get_ : attribute_name_list option)
    ?select:(select_ : select option) ?index_name:(index_name_ : index_name option)
    ~table_name:(table_name_ : table_arn) () =
  ({
     expression_attribute_values = expression_attribute_values_;
     expression_attribute_names = expression_attribute_names_;
...
let make_put_resource_policy_input
    ...
    ~policy:(policy_ : resource_policy) ~resource_arn:(resource_arn_ : resource_arn_string) () =
  ({
...
let make_batch_statement_error ?item:(item_ : attribute_map option)
    ?message:(message_ : string_ option) ?code:(code_ : batch_statement_error_code_enum option) () =
...
let make_batch_statement_response ?item:(item_ : attribute_map option)
    ?table_name:(table_name_ : table_name option) ?error:(error_ : batch_statement_error option) ()
    =
...
let make_list_tags_of_resource_output ?next_token:(next_token_ : next_token_string option)
...
let make_list_tables_output
  ...
  ({ last_evaluated_table_name = last_evaluated_table_name_; table_names = table_names_ }
    : list_tables_output)
...
let make_list_tables_input ?limit:(limit_ : list_tables_input_limit option)
  ...
  ({ limit = limit_; exclusive_start_table_name = exclusive_start_table_name_ } : list_tables_input)
...
let make_list_imports_output ?next_token:(next_token_ : import_next_token option)
    ?import_summary_list:(import_summary_list_ : import_summary_list option) () =
  ({ next_token = next_token_; import_summary_list = import_summary_list_ } : list_imports_output)

let make_list_imports_input ?next_token:(next_token_ : import_next_token option)
    ?page_size:(page_size_ : list_imports_max_limit option)
    ?table_arn:(table_arn_ : table_arn option) () =
  ({ next_token = next_token_; page_size = page_size_; table_arn = table_arn_ }
    : list_imports_input)

let make_global_table ?replication_group:(replication_group_ : replica_list option)
    ?global_table_name:(global_table_name_ : table_name option) () =
  ({ replication_group = replication_group_; global_table_name = global_table_name_ }
    : global_table)
...
let make_list_global_tables_input ?region_name:(region_name_ : region_name option)
    ?limit:(limit_ : positive_integer_object option)
...
let make_export_summary ?export_type:(export_type_ : export_type option)
  ...
  ({ export_type = export_type_; export_status = export_status_; export_arn = export_arn_ }
    : export_summary)

let make_list_exports_output ?next_token:(next_token_ : export_next_token option)
    ?export_summaries:(export_summaries_ : export_summaries option) () =
  ({ next_token = next_token_; export_summaries = export_summaries_ } : list_exports_output)

let make_list_exports_input ?next_token:(next_token_ : export_next_token option)
    ?max_results:(max_results_ : list_exports_max_limit option)
    ?table_arn:(table_arn_ : table_arn option) () =
  ({ next_token = next_token_; max_results = max_results_; table_arn = table_arn_ }
    : list_exports_input)
...
let make_list_contributor_insights_output ?next_token:(next_token_ : next_token_string option)
    ?contributor_insights_summaries:
      (contributor_insights_summaries_ : contributor_insights_summaries option) () =
  ({ next_token = next_token_; contributor_insights_summaries = contributor_insights_summaries_ }
...
let make_list_contributor_insights_input
    ...
    ?table_name:(table_name_ : table_arn option) () =
  ({ max_results = max_results_; next_token = next_token_; table_name = table_name_ }
    : list_contributor_insights_input)

let make_backup_summary ?backup_size_bytes:(backup_size_bytes_ : backup_size_bytes option)
    ...
    ?backup_name:(backup_name_ : backup_name option) ?backup_arn:(backup_arn_ : backup_arn option)
...
let make_list_backups_output
  ...
  ({ last_evaluated_backup_arn = last_evaluated_backup_arn_; backup_summaries = backup_summaries_ }
    : list_backups_output)

let make_list_backups_input ?backup_type:(backup_type_ : backup_type_filter option)
    ...
    ?limit:(limit_ : backups_input_limit option) ?table_name:(table_name_ : table_arn option) () =
  ({
     backup_type = backup_type_;
...
let make_keys_and_attributes
    ...
    ?projection_expression:(projection_expression_ : projection_expression option)
    ?consistent_read:(consistent_read_ : consistent_read option)
    ?attributes_to_get:(attributes_to_get_ : attribute_name_list option) ~keys:(keys_ : key_list) ()
    =
  ({
     expression_attribute_names = expression_attribute_names_;
     projection_expression = projection_expression_;
...
let make_import_table_description ?failure_message:(failure_message_ : failure_message option)
    ...
    ?s3_bucket_source:(s3_bucket_source_ : s3_bucket_source option)
    ?client_token:(client_token_ : client_token option) ?table_id:(table_id_ : table_id option)
    ?table_arn:(table_arn_ : table_arn option)
    ?import_status:(import_status_ : import_status option)
    ?import_arn:(import_arn_ : import_arn option) () =
  ({
...
let make_get_resource_policy_output ?revision_id:(revision_id_ : policy_revision_id option)
...
let make_get_item_output ?consumed_capacity:(consumed_capacity_ : consumed_capacity option)
    ?item:(item_ : attribute_map option) () =
...
let make_get_item_input
    ...
    ?return_consumed_capacity:(return_consumed_capacity_ : return_consumed_capacity option)
    ?consistent_read:(consistent_read_ : consistent_read option)
    ?attributes_to_get:(attributes_to_get_ : attribute_name_list option) ~key:(key_ : key)
    ~table_name:(table_name_ : table_arn) () =
  ({
     expression_attribute_names = expression_attribute_names_;
     projection_expression = projection_expression_;
...
let make_failure_exception
  ...
  ({ exception_description = exception_description_; exception_name = exception_name_ }
    : failure_exception)

let make_export_description
    ?incremental_export_specification:
      (incremental_export_specification_ : incremental_export_specification option)
    ?export_type:(export_type_ : export_type option) ?item_count:(item_count_ : item_count option)
    ...
    ?s3_bucket:(s3_bucket_ : s3_bucket option) ?client_token:(client_token_ : client_token option)
    ?export_time:(export_time_ : export_time option) ?table_id:(table_id_ : table_id option)
    ?table_arn:(table_arn_ : table_arn option)
...
let make_export_table_to_point_in_time_input
    ...
    ?s3_bucket_owner:(s3_bucket_owner_ : s3_bucket_owner option)
    ?client_token:(client_token_ : client_token option)
    ?export_time:(export_time_ : export_time option) ~s3_bucket:(s3_bucket_ : s3_bucket)
    ~table_arn:(table_arn_ : table_arn) () =
  ({
     incremental_export_specification = incremental_export_specification_;
     export_type = export_type_;
     export_format = export_format_;
...
let make_execute_transaction_output
  ...
  ({ consumed_capacity = consumed_capacity_; responses = responses_ } : execute_transaction_output)
...
let make_execute_statement_output ?last_evaluated_key:(last_evaluated_key_ : key option)
    ?consumed_capacity:(consumed_capacity_ : consumed_capacity option)
    ?next_token:(next_token_ : parti_ql_next_token option) ?items:(items_ : item_list option) () =
  ({
     last_evaluated_key = last_evaluated_key_;
     consumed_capacity = consumed_capacity_;
...
let make_backup_details ?backup_expiry_date_time:(backup_expiry_date_time_ : date option)
    ...
    ~backup_type:(backup_type_ : backup_type) ~backup_status:(backup_status_ : backup_status)
    ~backup_name:(backup_name_ : backup_name) ~backup_arn:(backup_arn_ : backup_arn) () =
  ({
     backup_expiry_date_time = backup_expiry_date_time_;
     backup_creation_date_time = backup_creation_date_time_;
     backup_type = backup_type_;
...
let make_delete_resource_policy_input
    ?expected_revision_id:(expected_revision_id_ : policy_revision_id option)
    ~resource_arn:(resource_arn_ : resource_arn_string) () =
  ({ expected_revision_id = expected_revision_id_; resource_arn = resource_arn_ }
...
let make_create_table_input
    ...
    ?warm_throughput:(warm_throughput_ : warm_throughput option)
    ?deletion_protection_enabled:(deletion_protection_enabled_ : deletion_protection_enabled option)
    ?table_class:(table_class_ : table_class option) ?tags:(tags_ : tag_list option)
    ?sse_specification:(sse_specification_ : sse_specification option)
    ?stream_specification:(stream_specification_ : stream_specification option)
    ?provisioned_throughput:(provisioned_throughput_ : provisioned_throughput option)
    ?billing_mode:(billing_mode_ : billing_mode option)
...
let make_create_global_table_input ~replication_group:(replication_group_ : replica_list)
    ~global_table_name:(global_table_name_ : table_name) () =
  ({ replication_group = replication_group_; global_table_name = global_table_name_ }
...
let make_batch_get_item_input
  ...
  ({ return_consumed_capacity = return_consumed_capacity_; request_items = request_items_ }
...
let make_batch_execute_statement_input
  ...
  ({ return_consumed_capacity = return_consumed_capacity_; statements = statements_ }
