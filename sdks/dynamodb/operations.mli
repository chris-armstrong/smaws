...
[@@ocaml.doc
  ...
  \  A single [SELECT] statement response can return up to the maximum number of items (if using \
   the Limit parameter) or a maximum of 1 MB of data (and then apply any filtering to the results \
   using [WHERE] clause). If [LastEvaluatedKey] is present in the response, you need to paginate \
   the result set. If [NextToken] is present, you need to paginate the result set and include \
   [NextToken].\n\
  \  "]
...
[@@ocaml.doc
  ...
  \  [GetResourcePolicy] follows an \
   ...
   {i eventually consistent} } model. The following list describes the outcomes when you issue the \
  ...
  \             }\n\
  \        }\n\
  \   Because [GetResourcePolicy] uses an {i eventually consistent} query, the metadata for your \
   policy or table might not be available at that moment. Wait for a few seconds, and then retry \
   the [GetResourcePolicy] request.\n\
  \   \n\
...
[@@ocaml.doc
  ...
  \  \n\
  \   If you want to retrieve the complete list of backups made with Amazon Web Services Backup, \
   use the \
   {{:https://docs.aws.amazon.com/aws-backup/latest/devguide/API_ListBackupJobs.html}Amazon Web \
   Services Backup list API.} \n\
  \   "]
...
[@@ocaml.doc
  ...
  \  Empty String and Binary attribute values are allowed. Attribute values of type String and \
   Binary must have a length greater than zero if the attribute is used as a key attribute for a \
   table or index. Set type attributes cannot be empty. \n\
  \  \n\
...
[@@ocaml.doc
  "Attaches a resource-based policy document to the resource, which can be a table or stream. When \
   you attach a resource-based policy using this API, the policy application is \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.ReadConsistency.html} \
   {i eventually consistent} }.\n\n\
  \  [PutResourcePolicy] is an idempotent operation; running it multiple times on the same \
   resource using the same policy document will return the same revision ID. If you specify an \
  ...
  \    [PutResourcePolicy] is an asynchronous operation. If you issue a [GetResourcePolicy] \
   request immediately after a [PutResourcePolicy] request, DynamoDB might return your previous \
   policy, if there was one, or return the [PolicyNotFoundException]. This is because \
   [GetResourcePolicy] uses an eventually consistent query, and the metadata for your policy or \
...
[@@ocaml.doc
  ...
  \     DynamoDB calculates the number of read capacity units consumed based on item size, not on \
   ...
   will be the same whether you request all of the attributes (the default behavior) or just some \
   of them (using a projection expression). The number will also be the same whether or not you \
   use a [FilterExpression]. \n\
  \    \n\
  \       [Query] results are always sorted by the sort key value. If the data type of the sort \
   key is Number, the results are returned in numeric order; otherwise, the results are returned \
   in order of UTF-8 bytes. By default, the sort order is ascending. To reverse the order, set the \
   [ScanIndexForward] parameter to false. \n\
  \      \n\
  \        A single [Query] operation will read up to the maximum number of items set (if using \
   the [Limit] parameter) or a maximum of 1 MB of data and then apply any filtering to the results \
   using [FilterExpression]. If [LastEvaluatedKey] is present in the response, you will need to \
   paginate the result set. For more information, see \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Query.html#Query.Pagination}Paginating \
   the Results} in the {i Amazon DynamoDB Developer Guide}. \n\
  \       \n\
  ...
  \            You can query a table, a local secondary index, or a global secondary index. For a \
   query on a table or on a local secondary index, you can set the [ConsistentRead] parameter to \
   [true] and obtain a strongly consistent result. Global secondary indexes support eventually \
   consistent reads only, so do not specify [ConsistentRead] when querying a global secondary \
   index.\n\
  \            "]
...
[@@ocaml.doc
  ...
  \ \n\
  \  Along with data, the following are also included on the new restored table using point in \
   time recovery: \n\
  \  \n\
  \   {ul\n\
  \         {-  Global secondary indexes (GSIs)\n\
...
[@@ocaml.doc
  ...
  \ If the total size of scanned items exceeds the maximum dataset size limit of 1 MB, the scan \
   ...
   and the requestor can use the [LastEvaluatedKey] to continue the scan in a subsequent \
   operation. Each scan response also includes number of items that were scanned (ScannedCount) as \
   part of the request. If using a [FilterExpression], a scan result can result in no items \
   meeting the criteria and the [Count] will result in zero. If you did not use a \
   [FilterExpression] in the scan request, then [Count] is the same as [ScannedCount].\n\
  \ \n\
  ...
  \     A single [Scan] operation first reads up to the maximum number of items set (if using the \
   [Limit] parameter) or a maximum of 1 MB of data and then applies any filtering to the results \
   if a [FilterExpression] is provided. If [LastEvaluatedKey] is present in the response, \
   pagination is required to complete the full table scan. For more information, see \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.Pagination}Paginating \
   the Results} in the {i Amazon DynamoDB Developer Guide}.\n\
  \     \n\
  \       [Scan] operations proceed sequentially; however, for faster performance on a large table \
   or secondary index, applications can request a parallel [Scan] operation by providing the \
   [Segment] and [TotalSegments] parameters. For more information, see \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.ParallelScan}Parallel \
   Scan} in the {i Amazon DynamoDB Developer Guide}.\n\
  \      \n\
  \       By default, a [Scan] uses eventually consistent reads when accessing the items in a \
   table. Therefore, the results from an eventually consistent [Scan] may not include the latest \
   item changes at the time the scan iterates through each item in the table. If you require a \
   strongly consistent read of each item as the scan iterates through the items in the table, you \
   can set the [ConsistentRead] parameter to true. Strong consistency only relates to the \
   consistency of the read at the item level.\n\
  \       \n\
  \          DynamoDB does not provide snapshot isolation for a scan operation when the \
   [ConsistentRead] parameter is set to true. Thus, a DynamoDB scan operation does not guarantee \
   that all reads in a scan see a consistent snapshot of the table when the scan operation was \
   requested. \n\
  \         \n\
  \          "]
...
[@@ocaml.doc
  ...
  \       {-   [TagResource] is an asynchronous operation. If you issue a [ListTagsOfResource] \
   request immediately after a [TagResource] request, DynamoDB might return your previous tag set, \
   if there was one, or an empty tag set. This is because [ListTagsOfResource] uses an eventually \
...
[@@ocaml.doc
  " [TransactGetItems] is a synchronous operation that atomically retrieves multiple items from \
   one or more tables (but not from indexes) in a single account and Region. A [TransactGetItems] \
   call can contain up to 100 [TransactGetItem] objects, each of which contains a [Get] structure \
   that specifies an item to retrieve from a table in the account and Region. A call to \
...
[@@ocaml.doc
  ...
  \ \n\
  \  {ul\n\
  \        {-   [Put] \194\160\226\128\148 \194\160 Initiates a [PutItem] operation to write a new \
   item. This structure specifies the primary key of the item to be written, the name of the table \
   to write it in, an optional condition expression that must be satisfied for the write to \
   succeed, a list of the item's attributes, and a field indicating whether to retrieve the item's \
   attributes if the condition is not met.\n\
  \            \n\
  \             }\n\
  \        {-   [Update] \194\160\226\128\148 \194\160 Initiates an [UpdateItem] operation to \
   update an existing item. This structure specifies the primary key of the item to be updated, \
   the name of the table where it resides, an optional condition expression that must be satisfied \
   for the update to succeed, an expression that defines one or more attributes to be updated, and \
   a field indicating whether to retrieve the item's attributes if the condition is not met.\n\
  \            \n\
  \             }\n\
  \        {-   [Delete] \194\160\226\128\148 \194\160 Initiates a [DeleteItem] operation to \
   delete an existing item. This structure specifies the primary key of the item to be deleted, \
   the name of the table where it resides, an optional condition expression that must be satisfied \
   for the deletion to succeed, and a field indicating whether to retrieve the item's attributes \
   if the condition is not met.\n\
  \            \n\
  \             }\n\
  \        {-   [ConditionCheck] \194\160\226\128\148 \194\160 Applies a condition to an item that \
   is not being modified by the transaction. This structure specifies the primary key of the item \
   to be checked, the name of the table where it resides, a condition expression that must be \
   satisfied for the transaction to succeed, and a field indicating whether to retrieve the item's \
   attributes if the condition is not met.\n\
  \            \n\
  \             }\n\
  \        }\n\
...
[@@ocaml.doc
  ...
  \ {ul\n\
  \       {-   [UntagResource] is an asynchronous operation. If you issue a [ListTagsOfResource] \
   request immediately after an [UntagResource] request, DynamoDB might return your previous tag \
...
[@@ocaml.doc
  ...
  \ \n\
  \   [LatestRestorableDateTime] is typically 5 minutes before the current time. You can restore \
   your table to any point in time in the last 35 days. You can set the [RecoveryPeriodInDays] to \
   any value between 1 and 35 days.\n\
  \  "]
...
[@@ocaml.doc
  "Updates the status for contributor insights for a specific table or index. CloudWatch \
   ...
   of frequently accessed items and frequently throttled items in plaintext. If you require the \
   use of Amazon Web Services Key Management Service (KMS) to encrypt this table\226\128\153s \
...
[@@ocaml.doc
  ...
  \   To determine which version you're using, see \
   ...
   global tables}.\n\
  \   \n\
  \       If you are using global tables \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/GlobalTables.html}Version \
   2019.11.21} (Current) you can use \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_UpdateTable.html}UpdateTable} \
   instead. \n\
  \      \n\
...
[@@ocaml.doc
  ...
  \             }\n\
  \        }\n\
  \    [UpdateTable] is an asynchronous operation; while it's executing, the table status changes \
   from [ACTIVE] to [UPDATING]. While it's [UPDATING], you can't issue another [UpdateTable] \
   request. When the table returns to the [ACTIVE] state, the [UpdateTable] operation is complete.\n\
  \   "]
...
[@@ocaml.doc
  ...
  \ TTL compares the current time in epoch time format to the time stored in the TTL attribute of \
   an item. If the epoch time value stored in the attribute is less than the current time, the \
   item is marked as expired and subsequently deleted.\n\
  \ \n\
  ...
  \       DynamoDB typically deletes expired items within two days of expiration. The exact \
   duration within which an item gets deleted after expiration is specific to the nature of the \
   workload. Items that have expired and not been deleted will still show up in reads, queries, \
   and scans.\n\
  \       \n\
