...
type nonrec archival_reason = string [@@ocaml.doc ""]
...
type nonrec archival_summary = {
  archival_date_time : date option;
      [@ocaml.doc
  ...
  archival_reason : archival_reason option;
      [@ocaml.doc
        ...
        \       {-   [INACCESSIBLE_ENCRYPTION_CREDENTIALS] - The table was archived due to the \
         table's KMS key being inaccessible for more than seven days. An On-Demand backup was \
...
type nonrec attribute_action = ADD [@ocaml.doc ""] | PUT [@ocaml.doc ""] | DELETE [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec scalar_attribute_type = S [@ocaml.doc ""] | N [@ocaml.doc ""] | B [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec key_schema_attribute_name = string [@@ocaml.doc ""]

type nonrec attribute_definition = {
  attribute_name : key_schema_attribute_name; [@ocaml.doc "A name for the attribute.\n"]
...
[@@ocaml.doc "Represents an attribute for describing the schema for the table and indexes.\n"]

type nonrec attribute_definitions = attribute_definition list [@@ocaml.doc ""]
...
type map_attribute_value = (attribute_name * attribute_value) list [@@ocaml.doc ""]
...
type nonrec attribute_name_list = attribute_name list [@@ocaml.doc ""]
  ...
  action : attribute_action option;
      [@ocaml.doc
        ...
        \        {-   [DELETE] - If no value is specified, the attribute and its value are removed \
         ...
         type.\n\
        \            \n\
        \             If a {i set} of values is specified, then those values are subtracted from \
         the old set. For example, if the attribute value was the set [\\[a,b,c\\]] and the \
        ...
        \        {-   [ADD] - If the attribute does not already exist, then the attribute and its \
         values are added to the item. If the attribute does exist, then the behavior of [ADD] \
         depends on the data type of the attribute:\n\
        \            \n\
        \             {ul\n\
        \                   {-  If the existing attribute is a number, and if [Value] is also a \
         number, then the [Value] is mathematically added to the existing attribute. If [Value] is \
        ...
        \                          If you use [ADD] to increment or decrement a number value for \
         an item that doesn't exist before the update, DynamoDB uses 0 as the initial value.\n\
        \                         \n\
        \                          In addition, if you use [ADD] to update an existing item, and \
         intend to increment or decrement an attribute value which does not yet exist, DynamoDB \
         uses [0] as the initial value. For example, suppose that the item you want to update does \
         not yet have an attribute named {i itemcount}, but you decide to [ADD] the number [3] to \
         this attribute anyway, even though it currently does not exist. DynamoDB will create the \
         {i itemcount} attribute, set its initial value to [0], and finally add [3] to it. The \
         result will be a new {i itemcount} attribute in the item, with a value of [3].\n\
        \                          \n\
        \                            }\n\
        \                   {-  If the existing data type is a set, and if the [Value] is also a \
         set, then the [Value] is added to the existing set. (This is a {i set} operation, not \
         mathematical addition.) For example, if the attribute value was the set [\\[1,2\\]], and \
         the [ADD] action specified [\\[3\\]], then the final attribute value would be \
         [\\[1,2,3\\]]. An error occurs if an Add action is specified for a set attribute and the \
        ...
        \                        Both sets must have the same primitive data type. For example, if \
         the existing data type is a set of strings, the [Value] must also be a set of strings. \
...
[@@ocaml.doc
  ...
  \    Attribute values cannot be null; string and binary type attributes must have lengths \
   greater than zero; and set type attributes must not be empty. Requests with empty values will \
   be rejected with a [ValidationException] exception.\n\
  \    "]

type nonrec attribute_updates = (attribute_name * attribute_value_update) list [@@ocaml.doc ""]

type nonrec attribute_value_list = attribute_value list [@@ocaml.doc ""]

type nonrec double_object = float [@@ocaml.doc ""]

type nonrec integer_object = int [@@ocaml.doc ""]
  ...
  scale_out_cooldown : integer_object option;
      [@ocaml.doc
        "The amount of time, in seconds, after a scale out activity completes before another scale \
         out activity can start. While the cooldown period is in effect, the capacity that has \
         been added by the previous scale out event that initiated the cooldown is calculated as \
         part of the desired capacity for the next scale out. You should continuously (but not \
...
[@@ocaml.doc "Represents the properties of a target tracking scaling policy.\n"]

type nonrec auto_scaling_policy_name = string [@@ocaml.doc ""]

type nonrec auto_scaling_policy_description = {
  policy_name : auto_scaling_policy_name option; [@ocaml.doc "The name of the scaling policy.\n"]
  target_tracking_scaling_policy_configuration :
    auto_scaling_target_tracking_scaling_policy_configuration_description option;
      [@ocaml.doc "Represents a target tracking scaling policy configuration.\n"]
}
[@@ocaml.doc "Represents the properties of the scaling policy.\n"]

type nonrec auto_scaling_policy_description_list = auto_scaling_policy_description list
[@@ocaml.doc ""]
...
[@@ocaml.doc "Represents the settings of a target tracking scaling policy that will be modified.\n"]
...
type nonrec auto_scaling_role_arn = string [@@ocaml.doc ""]
...
type nonrec auto_scaling_settings_description = {
  ...
  maximum_units : positive_long_object option;
      [@ocaml.doc
        "The maximum capacity units that a global table or global secondary index should be scaled \
         up to.\n"]
  auto_scaling_disabled : boolean_object option;
      [@ocaml.doc "Disabled auto scaling for this global table or global secondary index.\n"]
...
[@@ocaml.doc "Represents the auto scaling settings for a global table or global secondary index.\n"]
...
type nonrec backfilling = bool [@@ocaml.doc ""]

type nonrec backup_creation_date_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec kms_master_key_arn = string [@@ocaml.doc ""]

type nonrec sse_type = AES256 [@ocaml.doc ""] | KMS [@ocaml.doc ""] [@@ocaml.doc ""]
...
type nonrec time_to_live_attribute_name = string [@@ocaml.doc ""]
...
type nonrec time_to_live_description = {
  time_to_live_status : time_to_live_status option; [@ocaml.doc " The TTL status for the table.\n"]
  attribute_name : time_to_live_attribute_name option;
      [@ocaml.doc " The name of the TTL attribute for items in the table.\n"]
}
[@@ocaml.doc "The description of the Time to Live (TTL) status on the specified table. \n"]
...
type nonrec stream_enabled = bool [@@ocaml.doc ""]
...
[@@ocaml.doc "Represents the DynamoDB Streams configuration for a table in DynamoDB.\n"]
...
[@@ocaml.doc
  "Represents the provisioned throughput settings for the specified global secondary index. You \
   must use [ProvisionedThroughput] or [OnDemandThroughput] based on your table\226\128\153s \
   capacity mode.\n\n\
...
type nonrec non_key_attribute_name = string [@@ocaml.doc ""]

type nonrec non_key_attribute_name_list = non_key_attribute_name list [@@ocaml.doc ""]
  ...
  non_key_attributes : non_key_attribute_name_list option;
      [@ocaml.doc
        ...
        \ For global and local secondary indexes, the total count of [NonKeyAttributes] summed \
         ...
         [INCLUDE]. You still can specify the ProjectionType of [ALL] to project all attributes \
...
type nonrec key_type = HASH [@ocaml.doc ""] | RANGE [@ocaml.doc ""] [@@ocaml.doc ""]
...
type nonrec global_secondary_index_info = {
  index_name : index_name option; [@ocaml.doc "The name of the global secondary index.\n"]
...
type nonrec global_secondary_indexes = global_secondary_index_info list [@@ocaml.doc ""]

type nonrec local_secondary_index_info = {
  index_name : index_name option; [@ocaml.doc "Represents the name of the local secondary index.\n"]
  key_schema : key_schema option;
      [@ocaml.doc
...
type nonrec local_secondary_indexes = local_secondary_index_info list [@@ocaml.doc ""]
...
type nonrec source_table_feature_details = {
  ...
  stream_description : stream_specification option;
      [@ocaml.doc "Stream settings on the table when the backup was created.\n"]
  time_to_live_description : time_to_live_description option;
      [@ocaml.doc "Time to Live settings on the table when the backup was created.\n"]
...
type nonrec billing_mode = PROVISIONED [@ocaml.doc ""] | PAY_PER_REQUEST [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec item_count = Smaws_Lib.CoreTypes.Int64.t [@@ocaml.doc ""]

type nonrec table_creation_date_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec table_arn = string [@@ocaml.doc ""]

type nonrec table_id = string [@@ocaml.doc ""]

type nonrec table_name = string [@@ocaml.doc ""]

type nonrec source_table_details = {
  table_name : table_name; [@ocaml.doc "The name of the table for which the backup was created. \n"]
  table_id : table_id;
      [@ocaml.doc "Unique identifier for the table for which the backup was created. \n"]
  table_arn : table_arn option; [@ocaml.doc "ARN of the table for which backup was created. \n"]
  table_size_bytes : long_object option;
      [@ocaml.doc "Size of the table in bytes. Note that this is an approximate value.\n"]
  key_schema : key_schema; [@ocaml.doc "Schema of the table. \n"]
  table_creation_date_time : table_creation_date_time;
      [@ocaml.doc "Time when the source table was created. \n"]
  provisioned_throughput : provisioned_throughput;
      [@ocaml.doc "Read IOPs and Write IOPS on the table when the backup was created.\n"]
  on_demand_throughput : on_demand_throughput option; [@ocaml.doc ""]
  item_count : item_count option;
      [@ocaml.doc "Number of items in the table. Note that this is an approximate value. \n"]
...
[@@ocaml.doc "Contains the details of the table when the backup was created. \n"]
...
type nonrec backup_name = string [@@ocaml.doc ""]

type nonrec backup_details = {
  backup_arn : backup_arn; [@ocaml.doc "ARN associated with the backup.\n"]
  backup_name : backup_name; [@ocaml.doc "Name of the requested backup.\n"]
  ...
  backup_status : backup_status;
      [@ocaml.doc "Backup can be in one of the following states: CREATING, ACTIVE, DELETED. \n"]
  backup_type : backup_type;
      [@ocaml.doc
        ...
        \       {-   [SYSTEM] - If you delete a table with point-in-time recovery enabled, a \
         [SYSTEM] backup is automatically created and is retained for 35 days (at no additional \
         cost). System backups allow you to restore the deleted table to the state it was in just \
...
[@@ocaml.doc "Contains the description of the backup created for the table.\n"]

type nonrec error_message = string [@@ocaml.doc ""]

type nonrec backup_in_use_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec backup_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "Backup not found for the given BackupARN. \n"]

type nonrec backup_summary = {
  table_name : table_name option; [@ocaml.doc "Name of the table.\n"]
  table_id : table_id option; [@ocaml.doc "Unique identifier for the table.\n"]
  table_arn : table_arn option; [@ocaml.doc "ARN associated with the table.\n"]
  backup_arn : backup_arn option; [@ocaml.doc "ARN associated with the backup.\n"]
  backup_name : backup_name option; [@ocaml.doc "Name of the specified backup.\n"]
...
[@@ocaml.doc "Contains details for the backup.\n"]

type nonrec backup_summaries = backup_summary list [@@ocaml.doc ""]
...
type nonrec backups_input_limit = int [@@ocaml.doc ""]
...
type nonrec consumed_capacity_units = float [@@ocaml.doc ""]

type nonrec capacity = {
  read_capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of read capacity units consumed on a table or an index.\n"]
  write_capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of write capacity units consumed on a table or an index.\n"]
  capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of capacity units consumed on a table or an index.\n"]
}
[@@ocaml.doc
  "Represents the amount of provisioned throughput capacity consumed on a table or an index.\n"]

type nonrec secondary_indexes_capacity_map = (index_name * capacity) list [@@ocaml.doc ""]
...
type nonrec consumed_capacity = {
  ...
  capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of capacity units consumed by the operation.\n"]
  read_capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of read capacity units consumed by the operation.\n"]
  write_capacity_units : consumed_capacity_units option;
      [@ocaml.doc "The total number of write capacity units consumed by the operation.\n"]
  table : capacity option;
      [@ocaml.doc "The amount of throughput consumed on the table affected by the operation.\n"]
...
type nonrec consumed_capacity_multiple = consumed_capacity list [@@ocaml.doc ""]
...
type nonrec batch_statement_error = {
  code : batch_statement_error_code_enum option;
      [@ocaml.doc " The error code associated with the failed PartiQL batch statement. \n"]
  message : string_ option;
      [@ocaml.doc " The error message associated with the PartiQL batch response. \n"]
...
[@@ocaml.doc " An error associated with a statement in a PartiQL batch that was run. \n"]

type nonrec batch_statement_response = {
  error : batch_statement_error option;
      [@ocaml.doc " The error associated with a failed PartiQL batch statement. \n"]
  table_name : table_name option;
      [@ocaml.doc " The table name associated with a failed PartiQL batch statement. \n"]
  item : attribute_map option;
      [@ocaml.doc " A DynamoDB item associated with a BatchStatementResponse \n"]
}
[@@ocaml.doc " A PartiQL batch statement response.. \n"]

type nonrec parti_ql_batch_response = batch_statement_response list [@@ocaml.doc ""]
...
[@@ocaml.doc
  ...
  \       {-   [INDEXES] - The response includes the aggregate [ConsumedCapacity] for the \
   operation, together with [ConsumedCapacity] for each table and secondary index that was \
   accessed.\n\
  \           \n\
  \            Note that some operations, such as [GetItem] and [BatchGetItem], do not access any \
   indexes at all. In these cases, specifying [INDEXES] will only return [ConsumedCapacity] \
...
type nonrec consistent_read = bool [@@ocaml.doc ""]

type nonrec prepared_statement_parameters = attribute_value list [@@ocaml.doc ""]

type nonrec parti_ql_statement = string [@@ocaml.doc ""]

type nonrec batch_statement_request = {
  statement : parti_ql_statement; [@ocaml.doc " A valid PartiQL statement. \n"]
  parameters : prepared_statement_parameters option;
      [@ocaml.doc " The parameters associated with a PartiQL statement in the batch request. \n"]
...
[@@ocaml.doc " A PartiQL batch statement request. \n"]

type nonrec parti_ql_batch_request = batch_statement_request list [@@ocaml.doc ""]

type nonrec batch_execute_statement_input = {
  statements : parti_ql_batch_request;
      [@ocaml.doc "The list of PartiQL statements representing the batch to run.\n"]
...
[@@ocaml.doc
  "The request was denied due to request throttling. For detailed information about why the \
   ...
   Retries and Exponential Backoff} in the {i Amazon DynamoDB Developer Guide}.\n"]

type nonrec invalid_endpoint_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc ""]

type nonrec expression_attribute_name_variable = string [@@ocaml.doc ""]

type nonrec expression_attribute_name_map =
  (expression_attribute_name_variable * attribute_name) list
[@@ocaml.doc ""]

type nonrec projection_expression = string [@@ocaml.doc ""]

type nonrec key = (attribute_name * attribute_value) list [@@ocaml.doc ""]

type nonrec key_list = key list [@@ocaml.doc ""]
...
type nonrec keys_and_attributes = {
  ...
  expression_attribute_names : expression_attribute_name_map option;
      [@ocaml.doc
        ...
        \            }\n\
        \       }\n\
        \   Use the {b #} character in an expression to dereference an attribute name. For \
        ...
        \   The name of this attribute conflicts with a reserved word, so it cannot be used \
         ...
         Words} in the {i Amazon DynamoDB Developer Guide}). To work around this, you could \
        ...
        \               }\n\
        \          }\n\
        \    Tokens that begin with the {b :} character are {i expression attribute values}, which \
...
[@@ocaml.doc
  ...
  \ For each primary key, you must provide {i all} of the key attributes. For example, with a \
   simple primary key, you only need to provide the partition key. For a composite primary key, \
   you must provide {i both} the partition key and the sort key.\n\
  \ "]

type nonrec batch_get_request_map = (table_arn * keys_and_attributes) list [@@ocaml.doc ""]

type nonrec item_list = attribute_map list [@@ocaml.doc ""]

type nonrec batch_get_response_map = (table_arn * item_list) list [@@ocaml.doc ""]
  ...
  responses : batch_get_response_map option;
      [@ocaml.doc
        "A map of table name or table ARN to a list of items. Each object in [Responses] consists \
         of a table name or ARN, along with a map of attribute data consisting of the data type \
         and attribute value.\n"]
  unprocessed_keys : batch_get_request_map option;
      [@ocaml.doc
        "A map of tables and their respective keys that were not processed with the current \
         response. The [UnprocessedKeys] value is in the same form as [RequestItems], so the value \
         can be provided directly to a subsequent [BatchGetItem] operation. For more information, \
        ...
        \        {-   [ProjectionExpression] - One or more attributes to be retrieved from the \
         table or index. By default, all attributes are returned. If a requested attribute is not \
  ...
  request_items : batch_get_request_map;
      [@ocaml.doc
        "A map of one or more table names or table ARNs and, for each table, a map that describes \
         one or more items to retrieve from that table. Each table name or ARN can be used only \
        ...
        \        {-   [ExpressionAttributeNames] - One or more substitution tokens for attribute \
         names in the [ProjectionExpression] parameter. The following are some use cases for using \
        ...
        \              }\n\
        \        {-   [Keys] - An array of primary key attribute values that define specific items \
         in the table. For each primary key, you must provide {i all} of the key attributes. For \
         example, with a simple primary key, you only need to provide the partition key value. For \
         a composite key, you must provide {i both} the partition key value and the sort key value.\n\
        \            \n\
        \             }\n\
        ...
        \             If no attribute names are specified, then all attributes are returned. If \
         any of the requested attributes are not found, they do not appear in the result.\n\
        \             \n\
        \              For more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.AccessingItemAttributes.html}Accessing \
         Item Attributes} in the {i Amazon DynamoDB Developer Guide}.\n\
...
[@@ocaml.doc
  "The request was rejected because one or more items in the request are being modified by a \
...
type nonrec item_collection_size_estimate_bound = float [@@ocaml.doc ""]

type nonrec item_collection_size_estimate_range = item_collection_size_estimate_bound list
[@@ocaml.doc ""]

type nonrec item_collection_key_attribute_map = (attribute_name * attribute_value) list
[@@ocaml.doc ""]
...
type nonrec item_collection_metrics_multiple = item_collection_metrics list [@@ocaml.doc ""]

type nonrec item_collection_metrics_per_table = (table_arn * item_collection_metrics_multiple) list
[@@ocaml.doc ""]
...
[@@ocaml.doc "Represents a request to perform a [DeleteItem] operation on an item.\n"]

type nonrec put_item_input_attribute_map = (attribute_name * attribute_value) list [@@ocaml.doc ""]
  ...
  item : put_item_input_attribute_map;
      [@ocaml.doc
        "A map of attribute name to attribute values, representing the primary key of an item to \
         be processed by [PutItem]. All of the table's primary key attributes must be specified, \
         and their data types must match those of the table's key schema. If any attributes are \
         present in the item that are part of an index key schema for the table, their types must \
         match the index key schema.\n"]
}
[@@ocaml.doc "Represents a request to perform a [PutItem] operation on an item.\n"]

type nonrec write_request = {
  put_request : put_request option; [@ocaml.doc "A request to perform a [PutItem] operation.\n"]
...
type nonrec write_requests = write_request list [@@ocaml.doc ""]

type nonrec batch_write_item_request_map = (table_arn * write_requests) list [@@ocaml.doc ""]
  ...
  unprocessed_items : batch_write_item_request_map option;
      [@ocaml.doc
        ...
        \                   {-   [Key] - A map of primary key attribute values that uniquely \
         identify the item. Each entry in this map consists of an attribute name and an attribute \
         value.\n\
        \                       \n\
        \                        }\n\
        \                   \n\
        ...
        \                   {-   [Item] - A map of attributes and their values. Each entry in this \
         map consists of an attribute name and an attribute value. Attribute values must not be \
         null; string and binary type attributes must have lengths greater than zero; and set type \
         attributes must not be empty. Requests that contain empty values will be rejected with a \
         [ValidationException] exception.\n\
        \                       \n\
        \                        If you specify any attributes that are part of an index key, then \
         the data types for those attributes must match those of the schema in the table's \
  ...
  item_collection_metrics : item_collection_metrics_per_table option;
      [@ocaml.doc
        ...
        \        {-   [SizeEstimateRangeGB] - An estimate of item collection size, expressed in \
         GB. This is a two-element array containing a lower bound and an upper bound for the \
         estimate. The estimate includes the size of all the items in the table, plus the size of \
         all attributes projected into all of the local secondary indexes on the table. Use this \
...
type nonrec return_item_collection_metrics = SIZE [@ocaml.doc ""] | NONE [@ocaml.doc ""]
[@@ocaml.doc ""]
  ...
  request_items : batch_write_item_request_map;
      [@ocaml.doc
        "A map of one or more table names or table ARNs and, for each table, a list of operations \
         to be performed ([DeleteRequest] or [PutRequest]). Each element in the map consists of \
         the following:\n\n\
        \ {ul\n\
        ...
        \                  {-   [Key] - A map of primary key attribute values that uniquely \
         identify the item. Each entry in this map consists of an attribute name and an attribute \
         value. For each primary key, you must provide {i all} of the key attributes. For example, \
         with a simple primary key, you only need to provide a value for the partition key. For a \
         composite primary key, you must provide values for {i both} the partition key and the \
         sort key.\n\
        \                      \n\
        \                       }\n\
        \                  \n\
        ...
        \                  {-   [Item] - A map of attributes and their values. Each entry in this \
         map consists of an attribute name and an attribute value. Attribute values must not be \
         null; string and binary type attributes must have lengths greater than zero; and set type \
         attributes must not be empty. Requests that contain empty values are rejected with a \
         [ValidationException] exception.\n\
        \                      \n\
        \                       If you specify any attributes that are part of an index key, then \
         the data types for those attributes must match those of the schema in the table's \
  ...
  return_item_collection_metrics : return_item_collection_metrics option;
      [@ocaml.doc
        "Determines whether item collection metrics are returned. If set to [SIZE], the response \
         ...
         operation are returned in the response. If set to [NONE] (the default), no statistics are \
         returned.\n"]
}
...
[@@ocaml.doc
  "Contains the details for the read/write capacity mode. This page talks about [PROVISIONED] and \
   [PAY_PER_REQUEST] billing modes. For more information about these modes, see \
...
type nonrec cancellation_reason = {
  item : attribute_map option;
      [@ocaml.doc "Item in the request which caused the transaction to get cancelled.\n"]
  code : code option; [@ocaml.doc "Status code for the result of the cancelled transaction.\n"]
  message : error_message option; [@ocaml.doc "Cancellation reason message description.\n"]
}
[@@ocaml.doc
...
type nonrec cancellation_reason_list = cancellation_reason list [@@ocaml.doc ""]

type nonrec client_request_token = string [@@ocaml.doc ""]

type nonrec client_token = string [@@ocaml.doc ""]

type nonrec cloud_watch_log_group_arn = string [@@ocaml.doc ""]
  ...
  attribute_value_list : attribute_value_list option;
      [@ocaml.doc
        ...
        \  String value comparisons for greater than, equals, or less than are based on ASCII \
         character code values. For example, [a] is greater than [A], and [a] is greater than [B]. \
  ...
  comparison_operator : comparison_operator;
      [@ocaml.doc
        ...
        \                [AttributeValueList] can contain only one [AttributeValue] element of \
         type String, Number, Binary, String Set, Number Set, or Binary Set. If an item contains \
         an [AttributeValue] element of a different type than the one provided in the request, the \
        ...
        \                This operator tests for the existence of an attribute, not its data type. \
         If the data type of attribute \"[a]\" is null, and you evaluate it using [NOT_NULL], the \
         result is a Boolean [true]. This result is because the attribute \"[a]\" exists; its data \
        ...
        \                This operator tests for the nonexistence of an attribute, not its data \
         type. If the data type of attribute \"[a]\" is null, and you evaluate it using [NULL], \
         the result is a Boolean [false]. This is because the attribute \"[a]\" exists; its data \
        ...
        \                [AttributeValueList] can contain only one [AttributeValue] element of \
         ...
         comparison is of type String, then the operator checks for a substring match. If the \
         target attribute of the comparison is of type Binary, then the operator looks for a \
        ...
        \                [AttributeValueList] can contain only one [AttributeValue] element of \
         type String, Number, or Binary (not a set type). If the target attribute of the \
         comparison is a String, then the operator checks for the absence of a substring match. If \
         the target attribute of the comparison is Binary, then the operator checks for the \
         absence of a subsequence of the target that matches the input. If the target attribute of \
         the comparison is a set (\"[SS]\", \"[NS]\", or \"[BS]\"), then the operator evaluates to \
         true if it {i does not} find an exact match with any member of the set.\n\
        \               \n\
        ...
        \                [AttributeValueList] can contain only one [AttributeValue] of type String \
         or Binary (not a Number or a set type). The target attribute of the comparison must be of \
        ...
        \                [AttributeValueList] can contain one or more [AttributeValue] elements of \
         type String, Number, or Binary. These attributes are compared against an existing \
         attribute of an item. If any elements of the input are equal to the item attribute, the \
        ...
        \                [AttributeValueList] must contain two [AttributeValue] elements of the \
         ...
         if the target value is greater than, or equal to, the first element and less than, or \
         equal to, the second element. If an item contains an [AttributeValue] element of a \
         different type than the one provided in the request, the value does not match. For \
         example, [{\"S\":\"6\"}] does not compare to [{\"N\":\"6\"}]. Also, [{\"N\":\"6\"}] does \
         not compare to [{\"NS\":\\[\"6\", \"2\",\n\
        \                        \"1\"\\]}] \n\
        \               \n\
        \                }\n\
        \          }\n\
...
[@@ocaml.doc
  ...
  \       {-  For a [Query] operation, [Condition] is used for specifying the [KeyConditions] to \
   use when querying a table or an index. For [KeyConditions], only the following comparison \
...
type nonrec expression_attribute_value_variable = string [@@ocaml.doc ""]

type nonrec expression_attribute_value_map =
  (expression_attribute_value_variable * attribute_value) list
[@@ocaml.doc ""]

type nonrec condition_expression = string [@@ocaml.doc ""]
...
type nonrec condition_check = {
  ...
  table_name : table_arn;
      [@ocaml.doc
        "Name of the table for the check item request. You can also provide the Amazon Resource \
...
type nonrec conditional_check_failed_exception = {
  message : error_message option; [@ocaml.doc "The conditional request failed.\n"]
  item : attribute_map option;
      [@ocaml.doc "Item which caused the [ConditionalCheckFailedException].\n"]
}
[@@ocaml.doc "A condition specified in the operation failed to be evaluated.\n"]

type nonrec conditional_operator = AND [@ocaml.doc ""] | OR [@ocaml.doc ""] [@@ocaml.doc ""]

type nonrec confirm_remove_self_resource_access = bool [@@ocaml.doc ""]

type nonrec recovery_period_in_days = int [@@ocaml.doc ""]

type nonrec point_in_time_recovery_status = ENABLED [@ocaml.doc ""] | DISABLED [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec point_in_time_recovery_description = {
  ...
  recovery_period_in_days : recovery_period_in_days option;
      [@ocaml.doc
        "The number of preceding days for which continuous backups are taken and maintained. Your \
         table data is only recoverable to any point-in-time from within the configured recovery \
         period. This parameter is optional.\n"]
  ...
  latest_restorable_date_time : date option;
      [@ocaml.doc " [LatestRestorableDateTime] is typically 5 minutes before the current time. \n"]
}
[@@ocaml.doc "The description of the point in time settings applied to the table.\n"]

type nonrec continuous_backups_status = ENABLED [@ocaml.doc ""] | DISABLED [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec continuous_backups_description = {
  ...
  point_in_time_recovery_description : point_in_time_recovery_description option;
      [@ocaml.doc "The description of the point in time recovery settings applied to the table.\n"]
}
[@@ocaml.doc
...
[@@ocaml.doc "Backups have not yet been enabled for this table.\n"]

type nonrec contributor_insights_action = ENABLE [@ocaml.doc ""] | DISABLE [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec contributor_insights_rule = string [@@ocaml.doc ""]
...
type nonrec contributor_insights_summary = {
  table_name : table_name option; [@ocaml.doc "Name of the table associated with the summary.\n"]
...
[@@ocaml.doc "Represents a Contributor Insights summary entry.\n"]

type nonrec contributor_insights_summaries = contributor_insights_summary list [@@ocaml.doc ""]

type nonrec table_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec table_in_use_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "A target table with the specified name is either being created or deleted. \n"]
...
type nonrec create_backup_input = {
  table_name : table_arn;
      [@ocaml.doc
        "The name of the table. You can also provide the Amazon Resource Name (ARN) of the table \
         in this parameter.\n"]
  backup_name : backup_name; [@ocaml.doc "Specified name for the backup.\n"]
}
[@@ocaml.doc ""]
...
[@@ocaml.doc "The specified global table already exists.\n"]
...
type nonrec global_table_arn_string = string [@@ocaml.doc ""]
...
type nonrec table_class = STANDARD [@ocaml.doc ""] | STANDARD_INFREQUENT_ACCESS [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec table_class_summary = {
  ...
  last_update_date_time : date option;
      [@ocaml.doc "The date and time at which the table class was last updated.\n"]
}
...
[@@ocaml.doc "The description of the warm throughput value on a global secondary index.\n"]

type nonrec on_demand_throughput_override = {
  max_read_request_units : long_object option;
      [@ocaml.doc "Maximum number of read request units for the specified replica table.\n"]
}
[@@ocaml.doc
  "Overrides the on-demand throughput settings for this replica table. If you don't specify a \
   value for this parameter, it uses the source table's on-demand throughput settings.\n"]
...
type nonrec replica_global_secondary_index_description = {
  ...
  provisioned_throughput_override : provisioned_throughput_override option;
      [@ocaml.doc "If not described, uses the source table GSI's read capacity settings.\n"]
...
[@@ocaml.doc "Represents the properties of a replica global secondary index.\n"]
...
type nonrec table_warm_throughput_description = {
  read_units_per_second : positive_long_object option;
      [@ocaml.doc "Represents the base table's warm throughput value in read units per second.\n"]
  write_units_per_second : positive_long_object option;
      [@ocaml.doc "Represents the base table's warm throughput value in write units per second.\n"]
  status : table_status option; [@ocaml.doc "Represents warm throughput value of the base table.\n"]
}
[@@ocaml.doc
...
type nonrec kms_master_key_id = string [@@ocaml.doc ""]

type nonrec replica_status_percent_progress = string [@@ocaml.doc ""]

type nonrec replica_status_description = string [@@ocaml.doc ""]
...
type nonrec replica_description = {
  ...
  replica_status : replica_status option;
      [@ocaml.doc
        ...
        \             If the Amazon Web Services Region remains inaccessible for more than 20 \
         hours, DynamoDB will remove this replica from the replication group. The replica will not \
         be deleted and replication will stop from and to this region.\n\
        \             \n\
        \               }\n\
  ...
  kms_master_key_id : kms_master_key_id option;
      [@ocaml.doc "The KMS key of the replica that will be used for KMS encryption.\n"]
  provisioned_throughput_override : provisioned_throughput_override option;
      [@ocaml.doc
  ...
  global_secondary_indexes : replica_global_secondary_index_description_list option;
...
type nonrec replica_description_list = replica_description list [@@ocaml.doc ""]
...
type nonrec global_table_description = {
  ...
  creation_date_time : date option; [@ocaml.doc "The creation time of the global table.\n"]
  global_table_status : global_table_status option;
      [@ocaml.doc
        ...
        \       }\n\
        \  "]
  global_table_name : table_name option; [@ocaml.doc "The global table name.\n"]
}
...
type nonrec replica_list = replica list [@@ocaml.doc ""]

type nonrec create_global_table_input = {
  global_table_name : table_name; [@ocaml.doc "The global table name.\n"]
...
[@@ocaml.doc
  "Specifies the action to add a new witness Region to a MRSC global table. A MRSC global table \
   can be configured with either three replicas, or with two replicas and one witness.\n"]

type nonrec create_replica_action = {
  region_name : region_name; [@ocaml.doc "The Region of the replica to be added.\n"]
}
[@@ocaml.doc "Represents a replica to be added.\n"]

type nonrec replica_global_secondary_index = {
  index_name : index_name; [@ocaml.doc "The name of the global secondary index.\n"]
  provisioned_throughput_override : provisioned_throughput_override option;
      [@ocaml.doc
...
type nonrec replica_global_secondary_index_list = replica_global_secondary_index list
[@@ocaml.doc ""]

type nonrec create_replication_group_member_action = {
  region_name : region_name; [@ocaml.doc "The Region where the new replica will be created.\n"]
  kms_master_key_id : kms_master_key_id option;
      [@ocaml.doc
        "The KMS key that should be used for KMS encryption in the new replica. To specify a key, \
         use its key ID, Amazon Resource Name (ARN), alias name, or alias ARN. Note that you \
         should only provide this parameter if the key is different from the default DynamoDB KMS \
  ...
  on_demand_throughput_override : on_demand_throughput_override option;
      [@ocaml.doc
        "The maximum on-demand throughput settings for the specified replica table being created. \
         You can only modify [MaxReadRequestUnits], because you can't modify \
...
type nonrec resource_in_use_exception = {
  message : error_message option;
      [@ocaml.doc "The resource which is being attempted to be changed is in use.\n"]
}
[@@ocaml.doc
...
type nonrec multi_region_consistency = EVENTUAL [@ocaml.doc ""] | STRONG [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec deletion_protection_enabled = bool [@@ocaml.doc ""]

type nonrec restore_in_progress = bool [@@ocaml.doc ""]
...
type nonrec restore_summary = {
  ...
  source_table_arn : table_arn option;
      [@ocaml.doc "The ARN of the source table of the backup that is being restored.\n"]
  restore_date_time : date; [@ocaml.doc "Point in time or source backup time.\n"]
...
type nonrec global_table_witness_description = {
  ...
  witness_status : witness_status option;
      [@ocaml.doc "The current status of the witness Region in the MRSC global table.\n"]
}
[@@ocaml.doc "Represents the properties of a witness Region in a MRSC global table. \n"]

type nonrec global_table_witness_description_list = global_table_witness_description list
[@@ocaml.doc ""]
...
type nonrec provisioned_throughput_description = {
  last_increase_date_time : date option;
      [@ocaml.doc "The date and time of the last provisioned throughput increase for this table.\n"]
  last_decrease_date_time : date option;
      [@ocaml.doc "The date and time of the last provisioned throughput decrease for this table.\n"]
  number_of_decreases_today : positive_long_object option;
      [@ocaml.doc
        "The number of provisioned throughput decreases for this table during this UTC calendar \
         day. For current maximums on provisioned throughput decreases, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Limits.html}Service, \
         Account, and Table Quotas} in the {i Amazon DynamoDB Developer Guide}.\n"]
...
type nonrec global_secondary_index_description = {
  ...
  backfilling : backfilling option;
      [@ocaml.doc
        "Indicates whether the index is currently backfilling. {i Backfilling} is the process of \
         reading items from the table and determining whether they can be added to the index. (Not \
         all items will qualify: For example, a partition key cannot have any duplicate values.) \
  ...
  index_arn : string_ option;
      [@ocaml.doc "The Amazon Resource Name (ARN) that uniquely identifies the index.\n"]
  on_demand_throughput : on_demand_throughput option;
      [@ocaml.doc
        "The maximum number of read and write units for the specified global secondary index. If \
...
type nonrec global_secondary_index_description_list = global_secondary_index_description list
[@@ocaml.doc ""]
...
type nonrec local_secondary_index_description_list = local_secondary_index_description list
[@@ocaml.doc ""]
...
type nonrec table_description = {
  ...
  key_schema : key_schema option;
      [@ocaml.doc
        ...
        \                  \n\
        \        }\n\
        \          The partition key of an item is also known as its {i hash attribute}. The term \
         \"hash attribute\" derives from DynamoDB's usage of an internal hash function to evenly \
         distribute data items across partitions, based on their partition key values.\n\
        \          \n\
        \           The sort key of an item is also known as its {i range attribute}. The term \
  ...
  table_status : table_status option;
      [@ocaml.doc
        ...
        \       {-   [INACCESSIBLE_ENCRYPTION_CREDENTIALS] - The KMS key used to encrypt the table \
         ...
         will initiate the table archival process when a table's KMS key remains inaccessible for \
  ...
  creation_date_time : date option;
      [@ocaml.doc
        "The date and time when the table was created, in {{:http://www.epochconverter.com/}UNIX \
         epoch time} format.\n"]
  provisioned_throughput : provisioned_throughput_description option;
      [@ocaml.doc
  ...
  table_arn : string_ option;
      [@ocaml.doc "The Amazon Resource Name (ARN) that uniquely identifies the table.\n"]
  ...
  local_secondary_indexes : local_secondary_index_description_list option;
      [@ocaml.doc
        ...
        \       {-   [Projection] - Specifies attributes that are copied (projected) from the \
         table into the index. These are in addition to the primary key attributes and index key \
        ...
        \                  {-   [NonKeyAttributes] - A list of one or more non-key attribute names \
         that are projected into the secondary index. The total count of attributes provided in \
         [NonKeyAttributes], summed across all of the secondary indexes, must not exceed 100. If \
         you project the same attribute into two different indexes, this counts as two distinct \
         attributes when determining the total. This limit only applies when you specify the \
         ProjectionType of [INCLUDE]. You still can specify the ProjectionType of [ALL] to project \
  ...
  global_secondary_indexes : global_secondary_index_description_list option;
      [@ocaml.doc
        ...
        \             You can delete an index that is being created during the [Backfilling] phase \
         when [IndexStatus] is set to CREATING and [Backfilling] is true. You can't delete the \
         index that is being created when [IndexStatus] is set to CREATING and [Backfilling] is \
        ...
        \       {-   [IndexSizeBytes] - The total size of the global secondary index, in bytes. \
         DynamoDB updates this value approximately every six hours. Recent changes might not be \
  ...
  stream_specification : stream_specification option;
      [@ocaml.doc "The current DynamoDB Streams configuration for the table.\n"]
  ...
  replicas : replica_description_list option; [@ocaml.doc "Represents replicas of the table.\n"]
  ...
  on_demand_throughput : on_demand_throughput option;
      [@ocaml.doc
        "The maximum number of read and write units for the specified on-demand table. If you use \
         this parameter, you must specify [MaxReadRequestUnits], [MaxWriteRequestUnits], or both.\n"]
  warm_throughput : table_warm_throughput_description option;
  ...
  multi_region_consistency : multi_region_consistency option;
      [@ocaml.doc
        ...
        \   If you don't specify this field, the global table consistency mode defaults to \
         ...
         {{:https://docs.aws.amazon.com/V2globaltables_HowItWorks.html#V2globaltables_HowItWorks.consistency-modes} \
...
type nonrec tag_value_string = string [@@ocaml.doc ""]

type nonrec tag_key_string = string [@@ocaml.doc ""]
...
[@@ocaml.doc
  ...
  \ Amazon Web Services-assigned tag names and values are automatically assigned the [aws:] \
   ...
   towards the tag limit of 50. User-assigned tag names have the prefix [user:] in the Cost \
   Allocation Report. You cannot backdate the application of a tag.\n\
  \ \n\
  \  For an overview on tagging DynamoDB resources, see \
   {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Tagging.html}Tagging for \
...
type nonrec sse_specification = {
  enabled : sse_enabled option;
      [@ocaml.doc
        "Indicates whether server-side encryption is done using an Amazon Web Services managed key \
         or an Amazon Web Services owned key. If enabled (true), server-side encryption type is \
         set to [KMS] and an Amazon Web Services managed key is used (KMS charges apply). If \
         disabled (false) or not specified, server-side encryption is set to Amazon Web Services \
         owned key.\n"]
  sse_type : sse_type option;
      [@ocaml.doc
        "Server-side encryption type. The only supported value is:\n\n\
  ...
  kms_master_key_id : kms_master_key_id option;
      [@ocaml.doc
        "The KMS key that should be used for the KMS encryption. To specify a key, use its key ID, \
         Amazon Resource Name (ARN), alias name, or alias ARN. Note that you should only provide \
         this parameter if the key is different from the default DynamoDB key [alias/aws/dynamodb].\n"]
}
[@@ocaml.doc "Represents the settings used to enable server-side encryption.\n"]
...
type nonrec global_secondary_index = {
  ...
  on_demand_throughput : on_demand_throughput option;
      [@ocaml.doc
        "The maximum number of read and write units for the specified global secondary index. If \
         you use this parameter, you must specify [MaxReadRequestUnits], [MaxWriteRequestUnits], \
         or both. You must use either [OnDemandThroughput] or [ProvisionedThroughput] based on \
...
type nonrec global_secondary_index_list = global_secondary_index list [@@ocaml.doc ""]
...
type nonrec local_secondary_index_list = local_secondary_index list [@@ocaml.doc ""]
  ...
  key_schema : key_schema option;
      [@ocaml.doc
        ...
        \        For a composite primary key (partition key and sort key), you must provide \
         exactly two elements, in this order: The first element must have a [KeyType] of [HASH], \
        ...
        \         "]
  local_secondary_indexes : local_secondary_index_list option;
      [@ocaml.doc
        "One or more local secondary indexes (the maximum is 5) to be created on the table. Each \
         index is scoped to a given partition key value. There is a 10 GB size limit per partition \
         key value; otherwise, the size of a local secondary index is unconstrained.\n\n\
        ...
        \                   {-   [NonKeyAttributes] - A list of one or more non-key attribute \
         ...
         in [NonKeyAttributes], summed across all of the secondary indexes, must not exceed 100. \
  ...
  billing_mode : billing_mode option;
      [@ocaml.doc
        ...
        \       {-   [PAY_PER_REQUEST] - We recommend using [PAY_PER_REQUEST] for most DynamoDB \
         workloads. [PAY_PER_REQUEST] sets the billing mode to \
        ...
        \       {-   [PROVISIONED] - We recommend using [PROVISIONED] for steady workloads with \
         predictable growth where capacity requirements can be reliably forecasted. [PROVISIONED] \
...
type nonrec csv_delimiter = string [@@ocaml.doc ""]

type nonrec csv_header = string [@@ocaml.doc ""]

type nonrec csv_header_list = csv_header list [@@ocaml.doc ""]

type nonrec csv_options = {
  delimiter : csv_delimiter option;
      [@ocaml.doc " The delimiter used for separating items in the CSV file being imported. \n"]
...
type nonrec delete = {
  key : key;
      [@ocaml.doc
        "The primary key of the item to be deleted. Each element consists of an attribute name and \
         a value for that attribute.\n"]
  table_name : table_arn;
      [@ocaml.doc
  ...
  expression_attribute_names : expression_attribute_name_map option;
      [@ocaml.doc "One or more substitution tokens for attribute names in an expression.\n"]
  expression_attribute_values : expression_attribute_value_map option;
      [@ocaml.doc "One or more values that can be substituted in an expression.\n"]
  return_values_on_condition_check_failure : return_values_on_condition_check_failure option;
      [@ocaml.doc
...
type nonrec delete_backup_input = {
  backup_arn : backup_arn; [@ocaml.doc "The ARN associated with the backup.\n"]
}
[@@ocaml.doc ""]

type nonrec delete_global_secondary_index_action = {
  index_name : index_name; [@ocaml.doc "The name of the global secondary index to be deleted.\n"]
}
[@@ocaml.doc "Represents a global secondary index to be deleted from an existing table.\n"]

type nonrec delete_global_table_witness_group_member_action = {
  region_name : region_name;
      [@ocaml.doc "The witness Region name to be removed from the MRSC global table.\n"]
}
[@@ocaml.doc
  ...
  consumed_capacity : consumed_capacity option;
      [@ocaml.doc
        "The capacity units consumed by the [DeleteItem] operation. The data returned includes the \
         ...
         indexes involved in the operation. [ConsumedCapacity] is only returned if the \
         [ReturnConsumedCapacity] parameter was specified. For more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/provisioned-capacity-mode.html}Provisioned \
         capacity mode} in the {i Amazon DynamoDB Developer Guide}.\n"]
  item_collection_metrics : item_collection_metrics option;
      [@ocaml.doc
        ...
        \        {-   [SizeEstimateRangeGB] - An estimate of item collection size, in gigabytes. \
         This value is a two-element array containing a lower bound and an upper bound for the \
         estimate. The estimate includes the size of all the items in the table, plus the size of \
         all attributes projected into all of the local secondary indexes on that table. Use this \
         estimate to measure whether a local secondary index is approaching its size limit.\n\
        \            \n\
        \             The estimate is subject to change over time; therefore, do not rely on the \
         precision or accuracy of the estimate.\n\
  ...
  exists : boolean_object option;
      [@ocaml.doc
        ...
        \       {-  If [Exists] is [false], DynamoDB assumes that the attribute value does not \
         exist in the table. If in fact the value does not exist, then the assumption is valid and \
         the operation succeeds. If the value is found, despite the assumption that it does not \
         exist, the operation fails with a [ConditionCheckFailedException].\n\
        \           \n\
        \            }\n\
        \       }\n\
...
[@@ocaml.doc
  ...
  \       {-  Use [AttributeValueList] to specify one or more values to compare against an \
   attribute. Use [ComparisonOperator] to specify how you want to perform the comparison. If the \
   comparison evaluates to true, then the conditional operation succeeds.\n\
  \           \n\
  \            }\n\
  \       {-  Use [Value] to specify a value that DynamoDB will compare against an attribute. If \
   the values match, then [ExpectedAttributeValue] evaluates to true and the conditional operation \
   succeeds. Optionally, you can also set [Exists] to false, indicating that you {i do not} expect \
...
type nonrec expected_attribute_map = (attribute_name * expected_attribute_value) list
[@@ocaml.doc ""]
...
type nonrec delete_item_input = {
  ...
  expression_attribute_values : expression_attribute_value_map option;
      [@ocaml.doc
        ...
        \ Use the {b :} (colon) character in an expression to dereference an attribute value. For \
         example, suppose that you wanted to check whether the value of the {i ProductStatus} \
  ...
  return_values_on_condition_check_failure : return_values_on_condition_check_failure option;
      [@ocaml.doc
        "An optional parameter that returns the item attributes for a [DeleteItem] operation that \
...
type nonrec delete_replica_action = {
  region_name : region_name; [@ocaml.doc "The Region of the replica to be removed.\n"]
}
[@@ocaml.doc "Represents a replica to be removed.\n"]

type nonrec delete_replication_group_member_action = {
  region_name : region_name; [@ocaml.doc "The Region where the replica exists.\n"]
}
...
[@@ocaml.doc
  ...
  \ If you specified an [ExpectedRevisionId], it's possible that a policy is present for the \
  ...
  resource_arn : resource_arn_string;
      [@ocaml.doc
        "The Amazon Resource Name (ARN) of the DynamoDB resource from which the policy will be \
         ...
         policy of a table, it will also remove the permissions for the table's indexes defined in \
         that policy document. This is because index permissions are defined in the table's policy.\n"]
  expected_revision_id : policy_revision_id option;
      [@ocaml.doc
        "A string value that you can use to conditionally delete your policy. When you provide an \
         expected revision ID, if the revision ID of the existing policy on the resource doesn't \
         match or if there's no policy attached to the resource, the request will fail and return \
...
[@@ocaml.doc "Represents the output of a [DeleteTable] operation.\n"]
...
type nonrec describe_backup_input = {
  backup_arn : backup_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) associated with the backup.\n"]
}
[@@ocaml.doc ""]
...
type nonrec describe_continuous_backups_input = {
  table_name : table_arn;
      [@ocaml.doc
        "Name of the table for which the customer wants to check the continuous backups and point \
         in time recovery settings.\n\n\
        \ You can also provide the Amazon Resource Name (ARN) of the table in this parameter.\n\
        \ "]
}
[@@ocaml.doc ""]

type nonrec exception_description = string [@@ocaml.doc ""]

type nonrec exception_name = string [@@ocaml.doc ""]

type nonrec failure_exception = {
  exception_name : exception_name option; [@ocaml.doc "Exception name.\n"]
  exception_description : exception_description option; [@ocaml.doc "Description of the failure.\n"]
}
[@@ocaml.doc "Represents a failure a contributor insights operation.\n"]
...
type nonrec describe_contributor_insights_output = {
  table_name : table_name option; [@ocaml.doc "The name of the table being described.\n"]
  index_name : index_name option;
  ...
  contributor_insights_rule_list : contributor_insights_rule_list option;
      [@ocaml.doc "List of names of the associated contributor insights rules.\n"]
  contributor_insights_status : contributor_insights_status option;
  ...
  failure_exception : failure_exception option;
      [@ocaml.doc
        ...
        \        {-  LimitExceededException - Per-account Amazon CloudWatch Contributor Insights \
         rule limit reached. Please disable Contributor Insights for other tables/indexes OR \
         disable Contributor Insights rules before retrying.\n\
        \            \n\
        \             }\n\
  ...
  table_name : table_arn;
      [@ocaml.doc
        "The name of the table to describe. You can also provide the Amazon Resource Name (ARN) of \
         the table in this parameter.\n"]
  index_name : index_name option;
      [@ocaml.doc "The name of the global secondary index to describe, if applicable.\n"]
}
[@@ocaml.doc ""]
...
type nonrec endpoint = {
  address : string_; [@ocaml.doc "IP address of the endpoint.\n"]
  cache_period_in_minutes : long; [@ocaml.doc "Endpoint cache time to live (TTL) value.\n"]
}
[@@ocaml.doc "An endpoint information details.\n"]

type nonrec endpoints = endpoint list [@@ocaml.doc ""]

type nonrec describe_endpoints_response = {
  endpoints : endpoints; [@ocaml.doc "List of endpoints.\n"]
}
[@@ocaml.doc ""]

type nonrec describe_endpoints_request = unit [@@ocaml.doc ""]

type nonrec export_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The specified export was not found.\n"]

type nonrec export_view_type = NEW_IMAGE [@ocaml.doc ""] | NEW_AND_OLD_IMAGES [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec export_to_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec export_from_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]
...
type nonrec export_type = FULL_EXPORT [@ocaml.doc ""] | INCREMENTAL_EXPORT [@ocaml.doc ""]
[@@ocaml.doc ""]

type nonrec export_format = DYNAMODB_JSON [@ocaml.doc ""] | ION [@ocaml.doc ""] [@@ocaml.doc ""]

type nonrec failure_message = string [@@ocaml.doc ""]

type nonrec failure_code = string [@@ocaml.doc ""]

type nonrec s3_sse_kms_key_id = string [@@ocaml.doc ""]

type nonrec s3_sse_algorithm = AES256 [@ocaml.doc ""] | KMS [@ocaml.doc ""] [@@ocaml.doc ""]

type nonrec s3_prefix = string [@@ocaml.doc ""]

type nonrec s3_bucket_owner = string [@@ocaml.doc ""]

type nonrec s3_bucket = string [@@ocaml.doc ""]

type nonrec export_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec export_manifest = string [@@ocaml.doc ""]

type nonrec export_end_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec export_start_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]
...
type nonrec export_arn = string [@@ocaml.doc ""]
...
type nonrec export_description = {
  ...
  start_time : export_start_time option; [@ocaml.doc "The time at which the export task began.\n"]
  end_time : export_end_time option; [@ocaml.doc "The time at which the export task completed.\n"]
  ...
  table_arn : table_arn option;
      [@ocaml.doc "The Amazon Resource Name (ARN) of the table that was exported.\n"]
  table_id : table_id option; [@ocaml.doc "Unique ID of the table that was exported.\n"]
  ...
  failure_message : failure_message option; [@ocaml.doc "Export failure reason description.\n"]
  export_format : export_format option;
      [@ocaml.doc
        "The format of the exported data. Valid values for [ExportFormat] are [DYNAMODB_JSON] or \
         [ION].\n"]
  billed_size_bytes : billed_size_bytes option;
      [@ocaml.doc "The billable size of the table export.\n"]
  item_count : item_count option; [@ocaml.doc "The number of items exported.\n"]
...
type nonrec describe_export_input = {
  export_arn : export_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) associated with the export.\n"]
}
[@@ocaml.doc ""]

type nonrec global_table_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The specified global table does not exist.\n"]
...
type nonrec describe_global_table_input = {
  global_table_name : table_name; [@ocaml.doc "The name of the global table.\n"]
}
[@@ocaml.doc ""]
...
type nonrec replica_settings_description = {
  region_name : region_name; [@ocaml.doc "The Region name of the replica.\n"]
  replica_status : replica_status option;
      [@ocaml.doc
  ...
  replica_provisioned_read_capacity_auto_scaling_settings :
    auto_scaling_settings_description option;
      [@ocaml.doc "Auto scaling settings for a global table replica's read capacity units.\n"]
  ...
  replica_provisioned_write_capacity_auto_scaling_settings :
    auto_scaling_settings_description option;
      [@ocaml.doc "Auto scaling settings for a global table replica's write capacity units.\n"]
  replica_global_secondary_index_settings :
    replica_global_secondary_index_settings_description_list option;
      [@ocaml.doc "Replica global secondary index settings for the global table.\n"]
  replica_table_class_summary : table_class_summary option; [@ocaml.doc ""]
}
[@@ocaml.doc "Represents the properties of a replica.\n"]

type nonrec replica_settings_description_list = replica_settings_description list [@@ocaml.doc ""]

type nonrec describe_global_table_settings_output = {
  global_table_name : table_name option; [@ocaml.doc "The name of the global table.\n"]
...
type nonrec describe_global_table_settings_input = {
  global_table_name : table_name; [@ocaml.doc "The name of the global table to describe.\n"]
}
[@@ocaml.doc ""]

type nonrec import_not_found_exception = { message : error_message option [@ocaml.doc ""] }
...
type nonrec import_end_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec import_start_time = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec table_creation_parameters = {
  table_name : table_name;
      [@ocaml.doc " The name of the table created as part of the import operation. \n"]
  attribute_definitions : attribute_definitions;
      [@ocaml.doc " The attributes of the table created as part of the import operation. \n"]
  key_schema : key_schema;
      [@ocaml.doc
...
[@@ocaml.doc " The parameters for the table created as part of the import operation. \n"]
...
type nonrec s3_bucket_source = {
  s3_bucket_owner : s3_bucket_owner option;
      [@ocaml.doc
        " The account number of the S3 bucket that is being imported from. If the bucket is owned \
         by the requester this is optional. \n"]
  s3_bucket : s3_bucket; [@ocaml.doc " The S3 bucket that is being imported from. \n"]
  s3_key_prefix : s3_prefix option;
      [@ocaml.doc " The key prefix shared by all S3 Objects that are being imported. \n"]
}
...
type nonrec import_arn = string [@@ocaml.doc ""]

type nonrec import_table_description = {
  import_arn : import_arn option;
      [@ocaml.doc " The Amazon Resource Number (ARN) corresponding to the import request. \n"]
  import_status : import_status option; [@ocaml.doc " The status of the import. \n"]
  table_arn : table_arn option;
      [@ocaml.doc " The Amazon Resource Number (ARN) of the table being imported into. \n"]
  table_id : table_id option;
      [@ocaml.doc " The table id corresponding to the table created by import table process. \n"]
  client_token : client_token option;
      [@ocaml.doc
  ...
  input_format : input_format option;
      [@ocaml.doc " The format of the source data going into the target table. \n"]
  ...
  input_compression_type : input_compression_type option;
      [@ocaml.doc
        " The compression options for the data that has been imported into the target table. The \
         values are NONE, GZIP, or ZSTD. \n"]
  table_creation_parameters : table_creation_parameters option;
      [@ocaml.doc " The parameters for the new table that is being imported into. \n"]
  start_time : import_start_time option; [@ocaml.doc " The time when this import task started. \n"]
  end_time : import_end_time option;
      [@ocaml.doc
        " The time at which the creation of the table associated with this import task completed. \n"]
  processed_size_bytes : long_object option;
      [@ocaml.doc " The total size of data processed from the source file, in Bytes. \n"]
  processed_item_count : processed_item_count option;
      [@ocaml.doc " The total number of items processed from the source file. \n"]
  imported_item_count : imported_item_count option;
      [@ocaml.doc " The number of items successfully imported into the new table. \n"]
  failure_code : failure_code option;
      [@ocaml.doc
...
[@@ocaml.doc " Represents the properties of the table being imported into. \n"]
...
type nonrec kinesis_data_stream_destination = {
  stream_arn : stream_arn option; [@ocaml.doc "The ARN for a specific Kinesis data stream.\n"]
  ...
  destination_status_description : string_ option;
      [@ocaml.doc "The human-readable string that corresponds to the replica status.\n"]
...
[@@ocaml.doc "Describes a Kinesis data stream destination.\n"]

type nonrec kinesis_data_stream_destinations = kinesis_data_stream_destination list [@@ocaml.doc ""]
...
type nonrec describe_kinesis_streaming_destination_output = {
  ...
  kinesis_data_stream_destinations : kinesis_data_stream_destinations option;
      [@ocaml.doc "The list of replica structures for the table being described.\n"]
}
[@@ocaml.doc ""]
...
type nonrec describe_kinesis_streaming_destination_input = {
  table_name : table_arn;
      [@ocaml.doc
        "The name of the table being described. You can also provide the Amazon Resource Name \
  ...
  table_max_read_capacity_units : positive_long_object option;
      [@ocaml.doc
        "The maximum read capacity units that your account allows you to provision for a new table \
         that you are creating in this Region, including the read capacity units provisioned for \
...
type nonrec describe_table_output = {
  table : table_description option; [@ocaml.doc "The properties of the table.\n"]
}
[@@ocaml.doc "Represents the output of a [DescribeTable] operation.\n"]
...
[@@ocaml.doc "Represents the auto scaling configuration for a replica global secondary index.\n"]
...
type nonrec replica_auto_scaling_description = {
  region_name : region_name option; [@ocaml.doc "The Region where the replica exists.\n"]
  global_secondary_indexes : replica_global_secondary_index_auto_scaling_description_list option;
      [@ocaml.doc "Replica-specific global secondary index auto scaling settings.\n"]
  replica_provisioned_read_capacity_auto_scaling_settings :
    auto_scaling_settings_description option;
      [@ocaml.doc ""]
  replica_provisioned_write_capacity_auto_scaling_settings :
...
[@@ocaml.doc "Represents the auto scaling settings of the replica.\n"]

type nonrec replica_auto_scaling_description_list = replica_auto_scaling_description list
[@@ocaml.doc ""]
...
[@@ocaml.doc "Represents the auto scaling configuration for a global table.\n"]
...
type nonrec kinesis_streaming_destination_output = {
  table_name : table_name option; [@ocaml.doc "The name of the table being modified.\n"]
  stream_arn : stream_arn option; [@ocaml.doc "The ARN for the specific Kinesis data stream.\n"]
  destination_status : destination_status option;
  ...
  enable_kinesis_streaming_configuration : enable_kinesis_streaming_configuration option;
      [@ocaml.doc "The destination for the Kinesis streaming information that is being enabled.\n"]
}
[@@ocaml.doc ""]
  ...
  table_name : table_arn;
      [@ocaml.doc
        "The name of the DynamoDB table. You can also provide the Amazon Resource Name (ARN) of \
         the table in this parameter.\n"]
  stream_arn : stream_arn; [@ocaml.doc "The ARN for a Kinesis data stream.\n"]
  enable_kinesis_streaming_configuration : enable_kinesis_streaming_configuration option;
      [@ocaml.doc "The source for the Kinesis streaming information that is being enabled.\n"]
}
[@@ocaml.doc ""]

type nonrec duplicate_item_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  " There was an attempt to insert an item with the same primary key as an item that already \
   exists in the DynamoDB table.\n"]

type nonrec time_to_live_enabled = bool [@@ocaml.doc ""]
...
type nonrec update_time_to_live_output = {
  time_to_live_specification : time_to_live_specification option;
      [@ocaml.doc "Represents the output of an [UpdateTimeToLive] operation.\n"]
}
[@@ocaml.doc ""]
...
type nonrec update_table_replica_auto_scaling_output = {
  table_auto_scaling_description : table_auto_scaling_description option;
      [@ocaml.doc "Returns information about the auto scaling settings of a table with replicas.\n"]
}
[@@ocaml.doc ""]
...
[@@ocaml.doc "Represents the auto scaling settings of a replica that will be modified.\n"]

type nonrec replica_auto_scaling_update_list = replica_auto_scaling_update list [@@ocaml.doc ""]
...
type nonrec update_table_replica_auto_scaling_input = {
  global_secondary_index_updates : global_secondary_index_auto_scaling_update_list option;
      [@ocaml.doc
...
[@@ocaml.doc "Represents the output of an [UpdateTable] operation.\n"]
  ...
  delete : delete_global_table_witness_group_member_action option;
      [@ocaml.doc
        "Specifies a witness Region to be removed from an existing global table. Must be done in \
         conjunction with removing a replica. The deletion of both a witness and replica converts \
...
type nonrec global_table_witness_group_update_list = global_table_witness_group_update list
[@@ocaml.doc ""]
...
type nonrec update_replication_group_member_action = {
  ...
  kms_master_key_id : kms_master_key_id option;
      [@ocaml.doc
        "The KMS key of the replica that should be used for KMS encryption. To specify a key, use \
         its key ID, Amazon Resource Name (ARN), alias name, or alias ARN. Note that you should \
         only provide this parameter if the key is different from the default DynamoDB KMS key \
         [alias/aws/dynamodb].\n"]
  provisioned_throughput_override : provisioned_throughput_override option;
      [@ocaml.doc
        "Replica-specific provisioned throughput. If not specified, uses the source table's \
         provisioned throughput settings.\n"]
  on_demand_throughput_override : on_demand_throughput_override option;
      [@ocaml.doc "Overrides the maximum on-demand throughput for the replica table.\n"]
  global_secondary_indexes : replica_global_secondary_index_list option;
      [@ocaml.doc "Replica-specific global secondary index settings.\n"]
  table_class_override : table_class option;
      [@ocaml.doc
...
type nonrec replication_group_update = {
  create : create_replication_group_member_action option;
      [@ocaml.doc "The parameters required for creating a replica for the table.\n"]
  update : update_replication_group_member_action option;
      [@ocaml.doc "The parameters required for updating a replica for the table.\n"]
  delete : delete_replication_group_member_action option;
      [@ocaml.doc "The parameters required for deleting a replica for the table.\n"]
}
[@@ocaml.doc
  "Represents one of the following:\n\n\
  \ {ul\n\
  \       {-  A new replica to be added to an existing regional table or global table. This \
   request invokes the [CreateTableReplica] action in the destination Region.\n\
  \           \n\
  \            }\n\
  ...
  \       {-  An existing replica to be deleted. The request invokes the [DeleteTableReplica] \
   action in the destination Region, deleting the replica and all if its items in the destination \
   Region.\n\
  \           \n\
  \            }\n\
  \       }\n\
  \    When you manually remove a table or global table replica, you do not automatically remove \
   any associated scalable targets, scaling policies, or CloudWatch alarms.\n\
  \    \n\
  \     "]

type nonrec replication_group_update_list = replication_group_update list [@@ocaml.doc ""]

type nonrec update_global_secondary_index_action = {
  index_name : index_name; [@ocaml.doc "The name of the global secondary index to be updated.\n"]
  provisioned_throughput : provisioned_throughput option;
      [@ocaml.doc
        "Represents the provisioned throughput settings for the specified global secondary index.\n\n\
        \ For current minimum and maximum provisioned throughput values, see \
...
type nonrec global_secondary_index_update = {
  ...
  delete : delete_global_secondary_index_action option;
      [@ocaml.doc "The name of an existing global secondary index to be removed.\n"]
}
[@@ocaml.doc
  "Represents one of the following:\n\n\
  \ {ul\n\
...
type nonrec global_secondary_index_update_list = global_secondary_index_update list [@@ocaml.doc ""]

type nonrec update_table_input = {
  attribute_definitions : attribute_definitions option;
      [@ocaml.doc
        "An array of attributes that describe the key schema for the table and indexes. If you are \
         adding a new global secondary index to the table, [AttributeDefinitions] must include the \
         key element(s) of the new index.\n"]
  table_name : table_arn;
      [@ocaml.doc
  ...
  billing_mode : billing_mode option;
      [@ocaml.doc
        "Controls how you are charged for read and write throughput and how you manage capacity. \
         ...
         consumed read and write capacity of your table and global secondary indexes over the past \
         30 minutes.\n\n\
        \ {ul\n\
        \       {-   [PAY_PER_REQUEST] - We recommend using [PAY_PER_REQUEST] for most DynamoDB \
         workloads. [PAY_PER_REQUEST] sets the billing mode to \
        ...
        \  "]
  provisioned_throughput : provisioned_throughput option;
      [@ocaml.doc "The new provisioned throughput settings for the specified table or index.\n"]
  ...
  stream_specification : stream_specification option;
      [@ocaml.doc
        ...
        \   "]
  sse_specification : sse_specification option;
      [@ocaml.doc "The new server-side encryption settings for the specified table.\n"]
  replica_updates : replication_group_update_list option;
      [@ocaml.doc "A list of replica update actions (create, delete, or update) for the table.\n"]
  table_class : table_class option;
      [@ocaml.doc
  ...
  multi_region_consistency : multi_region_consistency option;
      [@ocaml.doc
        "Specifies the consistency mode for a new global table. This parameter is only valid when \
         you create a global table by specifying one or more \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_ReplicationGroupUpdate.html#DDB-Type-ReplicationGroupUpdate-Create}Create} \
         actions in the \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_UpdateTable.html#DDB-UpdateTable-request-ReplicaUpdates}ReplicaUpdates} \
         action list.\n\n\
  ...
  global_table_witness_updates : global_table_witness_group_update_list option;
      [@ocaml.doc
        ...
        \   \n\
        \    For more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/V2globaltables_HowItWorks.html#V2globaltables_HowItWorks.consistency-modes}Multi-Region \
         strong consistency (MRSC)} in the Amazon DynamoDB Developer Guide\n\
        \    "]
  on_demand_throughput : on_demand_throughput option;
      [@ocaml.doc
...
type nonrec update_kinesis_streaming_configuration = {
  approximate_creation_date_time_precision : approximate_creation_date_time_precision option;
      [@ocaml.doc "Enables updating the precision of Kinesis data stream timestamp. \n"]
}
...
type nonrec update_kinesis_streaming_destination_output = {
  table_name : table_name option;
      [@ocaml.doc "The table name for the Kinesis streaming destination output.\n"]
  stream_arn : stream_arn option; [@ocaml.doc "The ARN for the Kinesis stream input.\n"]
  destination_status : destination_status option;
      [@ocaml.doc "The status of the attempt to update the Kinesis streaming destination output.\n"]
  update_kinesis_streaming_configuration : update_kinesis_streaming_configuration option;
      [@ocaml.doc "The command to update the Kinesis streaming destination configuration.\n"]
}
[@@ocaml.doc ""]
...
type nonrec update_kinesis_streaming_destination_input = {
  table_name : table_arn;
      [@ocaml.doc
        "The table name for the Kinesis streaming destination input. You can also provide the ARN \
         of the table in this parameter.\n"]
  stream_arn : stream_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) for the Kinesis stream input.\n"]
  update_kinesis_streaming_configuration : update_kinesis_streaming_configuration option;
  ...
  attributes : attribute_map option;
      [@ocaml.doc
        ...
        \ The [Attributes] map is only present if the update was successful and [ReturnValues] was \
         specified as something other than [NONE] in the request. Each element represents one \
         attribute.\n\
        \ "]
  consumed_capacity : consumed_capacity option;
      [@ocaml.doc
        "The capacity units consumed by the [UpdateItem] operation. The data returned includes the \
         ...
         unity consumption for write operations} in the {i Amazon DynamoDB Developer Guide}.\n"]
  item_collection_metrics : item_collection_metrics option;
      [@ocaml.doc
...
type nonrec update_expression = string [@@ocaml.doc ""]
...
type nonrec update_item_input = {
  ...
  key : key;
      [@ocaml.doc
        "The primary key of the item to be updated. Each element consists of an attribute name and \
         a value for that attribute.\n\n\
  ...
  return_values : return_value option;
      [@ocaml.doc
        "Use [ReturnValues] if you want to get the item attributes as they appear before or after \
         they are successfully updated. For [UpdateItem], the valid values are:\n\n\
        \ {ul\n\
        \       {-   [NONE] - If [ReturnValues] is not specified, or if its value is [NONE], then \
         nothing is returned. (This setting is the default for [ReturnValues].)\n\
        \           \n\
  ...
  update_expression : update_expression option;
      [@ocaml.doc
        ...
        \        {-   [SET] - Adds one or more attributes and values to an item. If any of these \
         attributes already exist, they are replaced by the new values. You can also use [SET] to \
         add or subtract from an attribute that is of type Number. For example: [SET myNum = myNum \
         + :val] \n\
        \            \n\
        ...
        \                    {-   [if_not_exists (path, operand)] - if the item does not contain \
         an attribute at the specified path, then [if_not_exists] evaluates to operand; otherwise, \
         it evaluates to path. You can use this function to avoid overwriting an attribute that \
         may already be present in the item.\n\
        \                        \n\
        \                         }\n\
        \                    {-   [list_append (operand, operand)] - evaluates to a list with a \
         new element added to it. You can append the new element to the start or the end of the \
        ...
        \        {-   [ADD] - Adds the specified value to the item, if the attribute does not \
         already exist. If the attribute does exist, then the behavior of [ADD] depends on the \
         data type of the attribute:\n\
        \            \n\
        \             {ul\n\
        \                   {-  If the existing attribute is a number, and if [Value] is also a \
         number, then [Value] is mathematically added to the existing attribute. If [Value] is a \
         negative number, then it is subtracted from the existing attribute.\n\
        \                       \n\
        \                         If you use [ADD] to increment or decrement a number value for an \
         item that doesn't exist before the update, DynamoDB uses [0] as the initial value.\n\
        \                         \n\
        \                          Similarly, if you use [ADD] for an existing item to increment \
         or decrement an attribute value that doesn't exist before the update, DynamoDB uses [0] \
         as the initial value. For example, suppose that the item you want to update doesn't have \
         an attribute named [itemcount], but you decide to [ADD] the number [3] to this attribute \
        ...
        \                   {-  If the existing data type is a set and if [Value] is also a set, \
         then [Value] is added to the existing set. For example, if the attribute value is the set \
        ...
        \             If a set of values is specified, then those values are subtracted from the \
         old set. For example, if the attribute value was the set [\\[a,b,c\\]] and the [DELETE] \
...
type nonrec replica_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The specified replica is no longer part of the global table.\n"]

type nonrec index_not_found_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The operation tried to access a nonexistent index.\n"]
...
[@@ocaml.doc "Represents the settings for a global table in a Region that will be modified.\n"]

type nonrec replica_settings_update_list = replica_settings_update list [@@ocaml.doc ""]
...
type nonrec update_global_table_settings_input = {
  ...
  global_table_billing_mode : billing_mode option;
      [@ocaml.doc
        "The billing mode of the global table. If [GlobalTableBillingMode] is not specified, the \
         global table defaults to [PROVISIONED] capacity billing mode.\n\n\
        \ {ul\n\
        \       {-   [PROVISIONED] - We recommend using [PROVISIONED] for predictable workloads. \
         [PROVISIONED] sets the billing mode to \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/provisioned-capacity-mode.html}Provisioned \
         capacity mode}.\n\
        \           \n\
        \            }\n\
...
type nonrec replica_already_exists_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The specified replica is already part of the global table.\n"]
...
type nonrec replica_update = {
  create : create_replica_action option;
      [@ocaml.doc "The parameters required for creating a replica on an existing global table.\n"]
...
type nonrec replica_update_list = replica_update list [@@ocaml.doc ""]

type nonrec update_global_table_input = {
  global_table_name : table_name; [@ocaml.doc "The global table name.\n"]
  replica_updates : replica_update_list;
      [@ocaml.doc "A list of Regions that should be added or removed from the global table.\n"]
}
[@@ocaml.doc ""]
...
[@@ocaml.doc "Represents the settings used to enable point in time recovery.\n"]
...
type nonrec tag_key_list = tag_key_string list [@@ocaml.doc ""]
...
[@@ocaml.doc
  ...
  \ \n\
  \    This is a general recommendation for handling the [TransactionInProgressException]. These \
  ...
  \           {-   [requestTimeout] should be set based on the time taken for the individual \
   retries of a single HTTP request for your use case, but setting it to 1 second or higher should \
   work well to reduce chances of retries and [TransactionInProgressException] errors. \n\
  \               \n\
  \                }\n\
  ...
  \    Assuming \
   ...
   retry policy}, example timeout settings based on the guidelines above are as follows: \n\
  \   \n\
  \    Example timeline:\n\
  \    \n\
  \     {ul\n\
...
[@@ocaml.doc
  ...
  \        {-  An item size becomes too large (larger than 400 KB), or a local secondary index \
   (LSI) becomes too large, or a similar validation error occurs because of changes made by the \
   transaction.\n\
  \            \n\
  \             }\n\
  ...
  \        {-   There is an ongoing [TransactWriteItems] operation that conflicts with a \
   concurrent [TransactWriteItems] request. In this case the [TransactWriteItems] operation fails \
  ...
  \                                   {-  The level of configured provisioned throughput for the \
   table was exceeded. Consider increasing your provisioning level with the UpdateTable API.\n\
  \                                       \n\
  \                                         This Message is received when provisioned throughput \
   is exceeded is on a provisioned DynamoDB table.\n\
  \                                         \n\
  \                                           }\n\
  \                                   {-  The level of configured provisioned throughput for one \
   or more global secondary indexes of the table was exceeded. Consider increasing your \
   provisioning level for the under-provisioned global secondary indexes with the UpdateTable API.\n\
  \                                       \n\
  \                                         This message is returned when provisioned throughput \
   is exceeded is on a provisioned GSI.\n\
  \                                         \n\
  \                                           }\n\
  \                                   \n\
  \                         }\n\
  ...
  \                                   {-  Throughput exceeds the current capacity of your table or \
   index. DynamoDB is automatically scaling your table or index so please try again shortly. If \
   exceptions persist, check if you have a hot key: \
   https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-partition-key-design.html.\n\
  \                                       \n\
  \                                         This message is returned when writes get throttled on \
   an On-Demand table as DynamoDB is automatically scaling the table.\n\
  \                                         \n\
  \                                           }\n\
  \                                   {-  Throughput exceeds the current capacity for one or more \
   global secondary indexes. DynamoDB is automatically scaling your index so please try again \
   shortly.\n\
  \                                       \n\
  \                                         This message is returned when writes get throttled on \
   an On-Demand GSI as DynamoDB is automatically scaling the GSI.\n\
  \                                         \n\
  \                                           }\n\
  \                                   \n\
  \                         }\n\
...
type nonrec put = {
  item : put_item_input_attribute_map;
      [@ocaml.doc
        "A map of attribute name to attribute values, representing the primary key of the item to \
         be written by [PutItem]. All of the table's primary key attributes must be specified, and \
         their data types must match those of the table's key schema. If any attributes are \
         present in the item that are part of an index key schema for the table, their types must \
         match the index key schema. \n"]
  table_name : table_arn;
      [@ocaml.doc
...
type nonrec transact_write_item = {
  ...
  put : put option; [@ocaml.doc "A request to perform a [PutItem] operation.\n"]
  delete : delete option; [@ocaml.doc "A request to perform a [DeleteItem] operation.\n"]
  update : update option; [@ocaml.doc "A request to perform an [UpdateItem] operation.\n"]
}
[@@ocaml.doc
...
type nonrec transact_write_item_list = transact_write_item list [@@ocaml.doc ""]
...
type nonrec transact_write_items_input = {
  ...
  return_item_collection_metrics : return_item_collection_metrics option;
      [@ocaml.doc
        "Determines whether item collection metrics are returned. If set to [SIZE], the response \
         includes statistics about item collections (if any), that were modified during the \
         operation and are returned in the response. If set to [NONE] (the default), no statistics \
         are returned. \n"]
  client_request_token : client_request_token option;
      [@ocaml.doc
        ...
        \ Although multiple identical calls using the same client request token produce the same \
         ...
         call returns the amount of write capacity units consumed in making the changes. \
         Subsequent [TransactWriteItems] calls with the same client token return the number of \
...
type nonrec item_response = {
  item : attribute_map option;
      [@ocaml.doc "Map of attribute data consisting of the data type and attribute value.\n"]
}
[@@ocaml.doc "Details for the requested item.\n"]

type nonrec item_response_list = item_response list [@@ocaml.doc ""]
  ...
  consumed_capacity : consumed_capacity_multiple option;
      [@ocaml.doc
        "If the {i ReturnConsumedCapacity} value was [TOTAL], this is an array of \
         ...
         the {i TransactItems} parameter. These [ConsumedCapacity] objects report the \
         read-capacity units consumed by the [TransactGetItems] call in that table.\n"]
  responses : item_response_list option;
      [@ocaml.doc
        "An ordered array of up to 100 [ItemResponse] objects, each of which corresponds to the \
         [TransactGetItem] object in the same position in the {i TransactItems} array. Each \
...
type nonrec get = {
  ...
  table_name : table_arn;
      [@ocaml.doc
        "The name of the table from which to retrieve the specified item. You can also provide the \
         Amazon Resource Name (ARN) of the table in this parameter.\n"]
  projection_expression : projection_expression option;
      [@ocaml.doc
...
[@@ocaml.doc "Specifies an item to be retrieved as part of the transaction.\n"]

type nonrec transact_get_item_list = transact_get_item list [@@ocaml.doc ""]
...
type nonrec tag_resource_input = {
  ...
  tags : tag_list; [@ocaml.doc "The tags to be assigned to the Amazon DynamoDB resource.\n"]
}
[@@ocaml.doc ""]
...
type nonrec scan_output = {
  ...
  count : integer option;
      [@ocaml.doc
        ...
        \ If you set [ScanFilter] in the request, then [Count] is the number of items returned \
         after the filter was applied, and [ScannedCount] is the number of matching items before \
  ...
  scanned_count : integer option;
      [@ocaml.doc
        "The number of items evaluated, before any [ScanFilter] is applied. A high [ScannedCount] \
         value with few, or no, [Count] results indicates an inefficient [Scan] operation. For \
         more information, see \
  ...
  last_evaluated_key : key option;
      [@ocaml.doc
        ...
        \ \n\
        \  If [LastEvaluatedKey] is not empty, it does not necessarily mean that there is more \
         data in the result set. The only way to know when you have reached the end of the result \
         set is when [LastEvaluatedKey] is empty.\n\
        \  "]
  consumed_capacity : consumed_capacity option;
      [@ocaml.doc
        "The capacity units consumed by the [Scan] operation. The data returned includes the total \
         ...
         unit consumption for read operations} in the {i Amazon DynamoDB Developer Guide}.\n"]
}
[@@ocaml.doc "Represents the output of a [Scan] operation.\n"]

type nonrec scan_segment = int [@@ocaml.doc ""]

type nonrec scan_total_segments = int [@@ocaml.doc ""]

type nonrec filter_condition_map = (attribute_name * condition) list [@@ocaml.doc ""]
...
type nonrec scan_input = {
  table_name : table_arn;
      [@ocaml.doc
        "The name of the table containing the requested items or if you provide [IndexName], the \
  ...
  index_name : index_name option;
      [@ocaml.doc
        "The name of a secondary index to scan. This index can be any local secondary index or \
         global secondary index. Note that if you use the [IndexName] parameter, you must also \
         provide [TableName].\n"]
  attributes_to_get : attribute_name_list option;
      [@ocaml.doc
        "This is a legacy parameter. Use [ProjectionExpression] instead. For more information, see \
  ...
  limit : positive_integer_object option;
      [@ocaml.doc
        "The maximum number of items to evaluate (not necessarily the number of matching items). \
         ...
         [LastEvaluatedKey] to apply in a subsequent operation, so that you can pick up where you \
         left off. Also, if the processed dataset size exceeds 1 MB before DynamoDB reaches this \
         limit, it stops the operation and returns the matching values up to the limit, and a key \
         in [LastEvaluatedKey] to apply in a subsequent operation to continue the operation. For \
         more information, see \
  ...
  select : select option;
      [@ocaml.doc
        ...
        \       {-   [ALL_ATTRIBUTES] - Returns all of the item attributes from the specified \
         table or index. If you query a local secondary index, then for each matching item in the \
        ...
        \       {-   [ALL_PROJECTED_ATTRIBUTES] - Allowed only when querying an index. Retrieves \
         all attributes that have been projected into the index. If the index is configured to \
        ...
        \       {-   [COUNT] - Returns the number of matching items, rather than the matching \
         items themselves. Note that this uses the same quantity of read capacity units as getting \
        ...
        \            If you query or scan a local secondary index and request only attributes that \
         are projected into that index, the operation reads only the index and not the table. If \
        ...
        \             If you query or scan a global secondary index, you can only request \
         attributes that are projected into the index. Global secondary index queries cannot fetch \
  ...
  exclusive_start_key : key option;
      [@ocaml.doc
        "The primary key of the first item that this operation will evaluate. Use the value that \
  ...
  total_segments : scan_total_segments option;
      [@ocaml.doc
        "For a parallel [Scan] request, [TotalSegments] represents the total number of segments \
         into which the [Scan] operation will be divided. The value of [TotalSegments] corresponds \
         to the number of application workers that will perform the parallel scan. For example, if \
         you want to use four application threads to scan a table or an index, specify a \
  ...
  projection_expression : projection_expression option;
      [@ocaml.doc
        "A string that identifies one or more attributes to retrieve from the specified table or \
         index. These attributes can include scalars, sets, or elements of a JSON document. The \
...
type nonrec table_already_exists_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "A target table with the specified name already exists. \n"]

type nonrec point_in_time_recovery_unavailable_exception = {
  message : error_message option; [@ocaml.doc ""]
}
[@@ocaml.doc "Point in time recovery has not yet been enabled for this source table.\n"]

type nonrec invalid_restore_time_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec restore_table_to_point_in_time_input = {
  ...
  target_table_name : table_name;
      [@ocaml.doc "The name of the new table to which it must be restored to.\n"]
  ...
  restore_date_time : date option; [@ocaml.doc "Time in the past to restore the table to.\n"]
  ...
  global_secondary_index_override : global_secondary_index_list option;
      [@ocaml.doc
        "List of global secondary indexes for the restored table. The indexes provided should \
         match existing secondary indexes. You can choose to exclude some or all of the indexes at \
         the time of restore.\n"]
  local_secondary_index_override : local_secondary_index_list option;
      [@ocaml.doc
        "List of local secondary indexes for the restored table. The indexes provided should match \
         existing secondary indexes. You can choose to exclude some or all of the indexes at the \
         time of restore.\n"]
  ...
  sse_specification_override : sse_specification option;
      [@ocaml.doc "The new server-side encryption settings for the restored table.\n"]
}
[@@ocaml.doc ""]

type nonrec restore_table_from_backup_output = {
  table_description : table_description option;
      [@ocaml.doc "The description of the table created from an existing backup.\n"]
}
[@@ocaml.doc ""]

type nonrec restore_table_from_backup_input = {
  target_table_name : table_name;
      [@ocaml.doc "The name of the new table to which the backup must be restored.\n"]
  backup_arn : backup_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) associated with the backup.\n"]
  billing_mode_override : billing_mode option;
      [@ocaml.doc "The billing mode of the restored table.\n"]
...
type nonrec query_output = {
  ...
  scanned_count : integer option;
      [@ocaml.doc
        "The number of items evaluated, before any [QueryFilter] is applied. A high [ScannedCount] \
         ...
         more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Scan.html#Scan.Count}Count \
         and ScannedCount} in the {i Amazon DynamoDB Developer Guide}.\n\n\
        \ If you did not use a filter in the request, then [ScannedCount] is the same as [Count].\n\
        \ "]
  last_evaluated_key : key option;
  ...
  consumed_capacity : consumed_capacity option;
      [@ocaml.doc
        "The capacity units consumed by the [Query] operation. The data returned includes the \
         total provisioned throughput consumed, along with statistics for the table and any \
         indexes involved in the operation. [ConsumedCapacity] is only returned if the \
         [ReturnConsumedCapacity] parameter was specified. For more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/read-write-operations.html#read-operation-consumption}Capacity \
...
type nonrec key_expression = string [@@ocaml.doc ""]

type nonrec key_conditions = (attribute_name * condition) list [@@ocaml.doc ""]
...
type nonrec query_input = {
  ...
  index_name : index_name option;
      [@ocaml.doc
        "The name of an index to query. This index can be any local secondary index or global \
         secondary index on the table. Note that if you use the [IndexName] parameter, you must \
  ...
  key_conditions : key_conditions option;
      [@ocaml.doc
        "This is a legacy parameter. Use [KeyConditionExpression] instead. For more information, \
         see \
  ...
  scan_index_forward : boolean_object option;
      [@ocaml.doc
        "Specifies the order for index traversal: If [true] (default), the traversal is performed \
         in ascending order; if [false], the traversal is performed in descending order. \n\n\
        ...
        \  If [ScanIndexForward] is [true], DynamoDB returns the results in the order in which \
         ...
         is [false], DynamoDB reads the results in reverse order by sort key value, and then \
  ...
  key_condition_expression : key_expression option;
      [@ocaml.doc
        ...
        \  The condition can optionally perform one of several comparison tests on a single sort \
         key value. This allows [Query] to retrieve one item with a given partition key value and \
         sort key value, or several items that have the same partition key value but different \
         sort key values.\n\
        \  \n\
        ...
        \     If you also want to provide a condition for the sort key, it must be combined using \
         [AND] with the condition for the sort key. Following is an example, using the {b =} \
        ...
        \              {-   [sortKeyName] [BETWEEN] [:sortkeyval1] [AND] [:sortkeyval2] - true if \
         the sort key value is greater than or equal to [:sortkeyval1], and less than or equal to \
         [:sortkeyval2].\n\
        \                  \n\
        \                   }\n\
        \              {-   [begins_with (] [sortKeyName], [:sortkeyval] [)] - true if the sort \
         key value begins with a particular operand. (You cannot use this function with a sort key \
        ...
        \    You can optionally use the [ExpressionAttributeNames] parameter to replace the names \
         ...
         [KeyConditionExpression] parameter causes an error because {i Size} is a reserved word:\n\
        \    \n\
        \     {ul\n\
        \           {-   [Size = :myval] \n\
        \               \n\
        ...
        \   To work around this, define a placeholder (such a [#S]) to represent the attribute \
         name {i Size}. [KeyConditionExpression] then is as follows:\n\
        \   \n\
        \    {ul\n\
        \          {-   [#S = :myval] \n\
        \              \n\
        ...
        \    For more information on [ExpressionAttributeNames] and [ExpressionAttributeValues], \
         ...
         Placeholders for Attribute Names and Values} in the {i Amazon DynamoDB Developer Guide}.\n\
        \    "]
  expression_attribute_names : expression_attribute_name_map option;
      [@ocaml.doc
        "One or more substitution tokens for attribute names in an expression. The following are \
  ...
  policy : resource_policy;
      [@ocaml.doc
        ...
        \       {-  Within a resource-based policy, if the action for a DynamoDB service-linked \
         role (SLR) to replicate data for a global table is denied, adding or deleting a replica \
  ...
  expected_revision_id : policy_revision_id option;
      [@ocaml.doc
        ...
        \  When you provide an expected revision ID, if the revision ID of the existing policy on \
         the resource doesn't match or if there's no policy attached to the resource, your request \
         will be rejected with a [PolicyNotFoundException].\n\
        \  \n\
        \    To conditionally attach a policy when no policy exists for the resource, specify \
         [NO_POLICY] for the revision ID.\n\
        \    "]
  confirm_remove_self_resource_access : confirm_remove_self_resource_access option;
      [@ocaml.doc
        "Set this parameter to [true] to confirm that you want to remove your permissions to \
         change the policy of this resource in the future.\n"]
}
[@@ocaml.doc ""]
...
type nonrec put_item_output = {
  ...
  consumed_capacity : consumed_capacity option;
      [@ocaml.doc
        "The capacity units consumed by the [PutItem] operation. The data returned includes the \
         total provisioned throughput consumed, along with statistics for the table and any \
         indexes involved in the operation. [ConsumedCapacity] is only returned if the \
         [ReturnConsumedCapacity] parameter was specified. For more information, see \
         {{:https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/read-write-operations.html#write-operation-consumption}Capacity \
...
type nonrec put_item_input = {
  ...
  return_values_on_condition_check_failure : return_values_on_condition_check_failure option;
      [@ocaml.doc
        "An optional parameter that returns the item attributes for a [PutItem] operation that \
         failed a condition check.\n\n\
        \ There is no additional cost associated with requesting a return value aside from the \
         small network and processing overhead of receiving a larger response. No read capacity \
         units are consumed.\n\
...
type nonrec list_tags_of_resource_output = {
  tags : tag_list option;
      [@ocaml.doc "The tags currently associated with the Amazon DynamoDB resource.\n"]
...
type nonrec list_tags_of_resource_input = {
  ...
  next_token : next_token_string option;
      [@ocaml.doc
        "An optional string that, if supplied, must be copied from the output of a previous call \
         to ListTagOfResource. When provided in this manner, this API fetches the next page of \
         results.\n"]
}
[@@ocaml.doc ""]

type nonrec table_name_list = table_name list [@@ocaml.doc ""]
...
type nonrec list_tables_input_limit = int [@@ocaml.doc ""]
  ...
  exclusive_start_table_name : table_name option;
      [@ocaml.doc
        "The first table name that this operation will evaluate. Use the value that was returned \
         for [LastEvaluatedTableName] in a previous operation, so that you can obtain the next \
         page of results.\n"]
...
type nonrec import_next_token = string [@@ocaml.doc ""]

type nonrec import_summary = {
  import_arn : import_arn option;
      [@ocaml.doc " The Amazon Resource Number (ARN) corresponding to the import request. \n"]
  import_status : import_status option; [@ocaml.doc " The status of the import operation. \n"]
  table_arn : table_arn option;
      [@ocaml.doc " The Amazon Resource Number (ARN) of the table being imported into. \n"]
  s3_bucket_source : s3_bucket_source option;
      [@ocaml.doc
...
type nonrec import_summary_list = import_summary list [@@ocaml.doc ""]
...
type nonrec list_imports_output = {
  ...
  next_token : import_next_token option;
      [@ocaml.doc
        " If this value is returned, there are additional results to be displayed. To retrieve \
         them, call [ListImports] again, with [NextToken] set to this value. \n"]
}
[@@ocaml.doc ""]

type nonrec list_imports_max_limit = int [@@ocaml.doc ""]
...
type nonrec list_imports_input = {
  ...
  page_size : list_imports_max_limit option;
      [@ocaml.doc " The number of [ImportSummary ]objects returned in a single page. \n"]
  next_token : import_next_token option;
      [@ocaml.doc
        " An optional string that, if supplied, must be copied from the output of a previous call \
         to [ListImports]. When provided in this manner, the API fetches the next page of results. \n"]
}
[@@ocaml.doc ""]
...
type nonrec global_table_list = global_table list [@@ocaml.doc ""]

type nonrec list_global_tables_output = {
  global_tables : global_table_list option; [@ocaml.doc "List of global table names.\n"]
...
type nonrec list_global_tables_input = {
  exclusive_start_global_table_name : table_name option;
      [@ocaml.doc "The first global table name that this operation will evaluate.\n"]
  limit : positive_integer_object option;
      [@ocaml.doc
        ...
        \ "]
  region_name : region_name option; [@ocaml.doc "Lists the global tables in a specific Region.\n"]
}
[@@ocaml.doc ""]

type nonrec export_next_token = string [@@ocaml.doc ""]

type nonrec export_summary = {
  export_arn : export_arn option; [@ocaml.doc "The Amazon Resource Name (ARN) of the export.\n"]
  export_status : export_status option;
      [@ocaml.doc
        "Export can be in one of the following states: IN_PROGRESS, COMPLETED, or FAILED.\n"]
  export_type : export_type option;
...
[@@ocaml.doc "Summary information about an export task.\n"]

type nonrec export_summaries = export_summary list [@@ocaml.doc ""]

type nonrec list_exports_output = {
  export_summaries : export_summaries option; [@ocaml.doc "A list of [ExportSummary] objects.\n"]
  next_token : export_next_token option;
      [@ocaml.doc
        "If this value is returned, there are additional results to be displayed. To retrieve \
         them, call [ListExports] again, with [NextToken] set to this value.\n"]
}
[@@ocaml.doc ""]

type nonrec list_exports_max_limit = int [@@ocaml.doc ""]

type nonrec list_exports_input = {
  table_arn : table_arn option;
      [@ocaml.doc "The Amazon Resource Name (ARN) associated with the exported table.\n"]
  max_results : list_exports_max_limit option;
      [@ocaml.doc "Maximum number of results to return per page.\n"]
  next_token : export_next_token option;
      [@ocaml.doc
        "An optional string that, if supplied, must be copied from the output of a previous call \
         to [ListExports]. When provided in this manner, the API fetches the next page of results.\n"]
}
[@@ocaml.doc ""]
...
type nonrec list_contributor_insights_limit = int [@@ocaml.doc ""]
...
type nonrec list_backups_output = {
  backup_summaries : backup_summaries option; [@ocaml.doc "List of [BackupSummary] objects.\n"]
  last_evaluated_backup_arn : backup_arn option;
      [@ocaml.doc
        " The ARN of the backup last evaluated when the current page of results was returned, \
         ...
         page of results. \n\n\
        \  If [LastEvaluatedBackupArn] is empty, then the last page of results has been processed \
         and there are no more results to be retrieved. \n\
        \ \n\
        \   If [LastEvaluatedBackupArn] is not empty, this may or may not indicate that there is \
         more data to be returned. All results are guaranteed to have been returned if and only if \
         no value for [LastEvaluatedBackupArn] is returned. \n\
        \  "]
}
[@@ocaml.doc ""]

type nonrec time_range_upper_bound = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec time_range_lower_bound = Smaws_Lib.CoreTypes.Timestamp.t [@@ocaml.doc ""]

type nonrec list_backups_input = {
  table_name : table_arn option;
      [@ocaml.doc
        "Lists the backups from the table specified in [TableName]. You can also provide the \
         Amazon Resource Name (ARN) of the table in this parameter.\n"]
  limit : backups_input_limit option; [@ocaml.doc "Maximum number of backups to return at once.\n"]
  ...
  exclusive_start_backup_arn : backup_arn option;
      [@ocaml.doc
        " [LastEvaluatedBackupArn] is the Amazon Resource Name (ARN) of the backup last evaluated \
         when the current page of results was returned, inclusive of the current page of results. \
         This value may be specified as the [ExclusiveStartBackupArn] of a new [ListBackups] \
...
type nonrec import_conflict_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec import_table_input = {
  ...
  input_format_options : input_format_options option;
      [@ocaml.doc " Additional properties that specify how the input is formatted, \n"]
  input_compression_type : input_compression_type option;
      [@ocaml.doc " Type of compression to be used on the input coming from the imported table. \n"]
...
type nonrec invalid_export_time_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The specified [ExportTime] is outside of the point in time recovery window.\n"]

type nonrec export_conflict_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "There was a conflict when writing to the specified S3 bucket.\n"]
...
type nonrec export_table_to_point_in_time_input = {
  table_arn : table_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) associated with the table to export.\n"]
  export_time : export_time option;
      [@ocaml.doc
        "Time in the past from which to export table data, counted in seconds from the start of \
         the Unix epoch. The table export will be a snapshot of the table's state at this point in \
         time.\n"]
  client_token : client_token option;
      [@ocaml.doc
        ...
        \  "]
  s3_bucket : s3_bucket;
      [@ocaml.doc "The name of the Amazon S3 bucket to export the snapshot to.\n"]
  s3_bucket_owner : s3_bucket_owner option;
      [@ocaml.doc
        "The ID of the Amazon Web Services account that owns the bucket the export will be stored \
         in.\n\n\
  ...
  s3_sse_kms_key_id : s3_sse_kms_key_id option;
      [@ocaml.doc
        "The ID of the KMS managed key used to encrypt the S3 bucket where export data will be \
...
type nonrec execute_transaction_output = {
  responses : item_response_list option; [@ocaml.doc "The response to a PartiQL transaction.\n"]
  consumed_capacity : consumed_capacity_multiple option;
      [@ocaml.doc
        "The capacity units consumed by the entire operation. The values of the list are ordered \
         according to the ordering of the statements.\n"]
...
type nonrec parameterized_statement = {
  statement : parti_ql_statement; [@ocaml.doc " A PartiQL statement that uses parameters. \n"]
  parameters : prepared_statement_parameters option; [@ocaml.doc " The parameter values. \n"]
  return_values_on_condition_check_failure : return_values_on_condition_check_failure option;
      [@ocaml.doc
...
[@@ocaml.doc " Represents a PartiQL statement that uses parameters. \n"]

type nonrec parameterized_statements = parameterized_statement list [@@ocaml.doc ""]

type nonrec execute_transaction_input = {
  transact_statements : parameterized_statements;
      [@ocaml.doc "The list of PartiQL statements representing the transaction to run.\n"]
  client_request_token : client_request_token option;
      [@ocaml.doc
        "Set this value to get remaining results, if [NextToken] was returned in the statement \
         response.\n"]
  return_consumed_capacity : return_consumed_capacity option;
      [@ocaml.doc
...
type nonrec parti_ql_next_token = string [@@ocaml.doc ""]

type nonrec execute_statement_output = {
  items : item_list option;
      [@ocaml.doc
        "If a read operation was used, this property will contain the result of the read \
         operation; a map of attribute names and their values. For the write operations this value \
         will be empty.\n"]
  next_token : parti_ql_next_token option;
      [@ocaml.doc
        "If the response of a read request exceeds the response payload limit DynamoDB will set \
         this value in the response. If set, you can use that this value in the subsequent request \
  ...
  last_evaluated_key : key option;
      [@ocaml.doc
        "The primary key of the item where the operation stopped, inclusive of the previous result \
         ...
         [LastEvaluatedKey] is empty, then the \"last page\" of results has been processed and \
         there is no more data to be retrieved. If [LastEvaluatedKey] is not empty, it does not \
         necessarily mean that there is more data in the result set. The only way to know when you \
  ...
  limit : positive_integer_object option;
      [@ocaml.doc
        "The maximum number of items to evaluate (not necessarily the number of matching items). \
         ...
         left off. Also, if the processed dataset size exceeds 1 MB before DynamoDB reaches this \
         limit, it stops the operation and returns the matching values up to the limit, and a key \
         in [LastEvaluatedKey] to apply in a subsequent operation to continue the operation. \n"]
  return_values_on_condition_check_failure : return_values_on_condition_check_failure option;
      [@ocaml.doc
