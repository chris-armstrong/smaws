...
let start_message_move_task_request_of_yojson tree path =
  ...
  ({
     max_number_of_messages_per_second =
       option_of_yojson
         (value_for_key nullable_integer_of_yojson "MaxNumberOfMessagesPerSecond")
         _list path;
     destination_arn =
       option_of_yojson (value_for_key string__of_yojson "DestinationArn") _list path;
...
let send_message_result_of_yojson tree path =
  ...
  ({
     sequence_number =
       option_of_yojson (value_for_key string__of_yojson "SequenceNumber") _list path;
     message_id = option_of_yojson (value_for_key string__of_yojson "MessageId") _list path;
     md5_of_message_system_attributes =
       option_of_yojson (value_for_key string__of_yojson "MD5OfMessageSystemAttributes") _list path;
     md5_of_message_attributes =
       option_of_yojson (value_for_key string__of_yojson "MD5OfMessageAttributes") _list path;
     md5_of_message_body =
       option_of_yojson (value_for_key string__of_yojson "MD5OfMessageBody") _list path;
   }
    : send_message_result)

let binary_of_yojson = blob_of_yojson
let binary_list_of_yojson tree path = list_of_yojson binary_of_yojson tree path
...
let message_attribute_value_of_yojson tree path =
  ...
  ({
     ...
     binary_list_values =
       option_of_yojson (value_for_key binary_list_of_yojson "BinaryListValues") _list path;
     string_list_values =
       option_of_yojson (value_for_key string_list_of_yojson "StringListValues") _list path;
...
let send_message_request_of_yojson tree path =
  ...
  ({
     message_group_id =
       option_of_yojson (value_for_key string__of_yojson "MessageGroupId") _list path;
     message_deduplication_id =
       option_of_yojson (value_for_key string__of_yojson "MessageDeduplicationId") _list path;
     message_system_attributes =
       option_of_yojson
         (value_for_key message_body_system_attribute_map_of_yojson "MessageSystemAttributes")
         _list path;
     message_attributes =
       option_of_yojson
         (value_for_key message_body_attribute_map_of_yojson "MessageAttributes")
         _list path;
     delay_seconds =
       option_of_yojson (value_for_key nullable_integer_of_yojson "DelaySeconds") _list path;
...
let send_message_batch_result_of_yojson tree path =
  ...
  ({
     ...
     successful =
       value_for_key send_message_batch_result_entry_list_of_yojson "Successful" _list path;
   }
...
let message_system_attribute_name_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String "All" -> All
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "MessageSystemAttributeName" value)
...
let message_system_attribute_map_of_yojson tree path =
  map_of_yojson message_system_attribute_name_of_yojson string__of_yojson tree path
...
let message_list_of_yojson tree path = list_of_yojson message_of_yojson tree path
...
let receive_message_request_of_yojson tree path =
  ...
  ({
     receive_request_attempt_id =
       option_of_yojson (value_for_key string__of_yojson "ReceiveRequestAttemptId") _list path;
     wait_time_seconds =
       option_of_yojson (value_for_key nullable_integer_of_yojson "WaitTimeSeconds") _list path;
     visibility_timeout =
       option_of_yojson (value_for_key nullable_integer_of_yojson "VisibilityTimeout") _list path;
     max_number_of_messages =
       option_of_yojson (value_for_key nullable_integer_of_yojson "MaxNumberOfMessages") _list path;
     message_attribute_names =
       option_of_yojson
         (value_for_key message_attribute_name_list_of_yojson "MessageAttributeNames")
         _list path;
     message_system_attribute_names =
       option_of_yojson
         (value_for_key message_system_attribute_list_of_yojson "MessageSystemAttributeNames")
         _list path;
     attribute_names =
...
let queue_url_list_of_yojson tree path = list_of_yojson string__of_yojson tree path
...
let purge_queue_request_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ queue_url = value_for_key string__of_yojson "QueueUrl" _list path } : purge_queue_request)
...
let list_queues_request_of_yojson tree path =
  ...
  ({
     ...
     next_token = option_of_yojson (value_for_key token_of_yojson "NextToken") _list path;
     queue_name_prefix =
       option_of_yojson (value_for_key string__of_yojson "QueueNamePrefix") _list path;
   }
    : list_queues_request)
...
let list_queue_tags_request_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ queue_url = value_for_key string__of_yojson "QueueUrl" _list path } : list_queue_tags_request)

let list_message_move_tasks_result_entry_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     started_timestamp =
       option_of_yojson (value_for_key long_of_yojson "StartedTimestamp") _list path;
     ...
     approximate_number_of_messages_to_move =
       option_of_yojson
         (value_for_key nullable_long_of_yojson "ApproximateNumberOfMessagesToMove")
         _list path;
...
let list_message_move_tasks_result_of_yojson tree path =
  ...
  ({
     results =
       option_of_yojson
         (value_for_key list_message_move_tasks_result_entry_list_of_yojson "Results")
         _list path;
   }
...
let list_message_move_tasks_request_of_yojson tree path =
  ...
  ({
     max_results =
       option_of_yojson (value_for_key nullable_integer_of_yojson "MaxResults") _list path;
     source_arn = value_for_key string__of_yojson "SourceArn" _list path;
   }
...
let get_queue_url_request_of_yojson tree path =
  ...
  ({
     queue_owner_aws_account_id =
       option_of_yojson (value_for_key string__of_yojson "QueueOwnerAWSAccountId") _list path;
...
let get_queue_attributes_result_of_yojson tree path =
  ...
  ({
     attributes =
       option_of_yojson (value_for_key queue_attribute_map_of_yojson "Attributes") _list path;
   }
...
let delete_queue_request_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ queue_url = value_for_key string__of_yojson "QueueUrl" _list path } : delete_queue_request)
...
let delete_message_batch_result_of_yojson tree path =
  ...
  ({
     failed = value_for_key batch_result_error_entry_list_of_yojson "Failed" _list path;
     successful =
       value_for_key delete_message_batch_result_entry_list_of_yojson "Successful" _list path;
   }
...
let change_message_visibility_batch_result_entry_list_of_yojson tree path =
  list_of_yojson change_message_visibility_batch_result_entry_of_yojson tree path
...
let change_message_visibility_batch_result_of_yojson tree path =
  ...
  ({
     failed = value_for_key batch_result_error_entry_list_of_yojson "Failed" _list path;
     successful =
       value_for_key change_message_visibility_batch_result_entry_list_of_yojson "Successful" _list
         path;
   }
...
let change_message_visibility_batch_request_entry_list_of_yojson tree path =
  list_of_yojson change_message_visibility_batch_request_entry_of_yojson tree path

let change_message_visibility_batch_request_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     entries =
       value_for_key change_message_visibility_batch_request_entry_list_of_yojson "Entries" _list
         path;
     queue_url = value_for_key string__of_yojson "QueueUrl" _list path;
   }
...
let aws_account_id_list_of_yojson tree path = list_of_yojson string__of_yojson tree path
let action_name_list_of_yojson tree path = list_of_yojson string__of_yojson tree path
