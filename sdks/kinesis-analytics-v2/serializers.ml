...
let artifact_type_to_yojson (x : artifact_type) =
  match x with DEPENDENCY_JAR -> `String "DEPENDENCY_JAR" | UDF -> `String "UDF"
...
let code_content_type_to_yojson (x : code_content_type) =
  match x with ZIPFILE -> `String "ZIPFILE" | PLAINTEXT -> `String "PLAINTEXT"
...
let application_mode_to_yojson (x : application_mode) =
  match x with INTERACTIVE -> `String "INTERACTIVE" | STREAMING -> `String "STREAMING"
  ...
  assoc_to_yojson
    [
      ...
      ( "ApplicationVersionCreateTimestamp",
        option_to_yojson timestamp__to_yojson x.application_version_create_timestamp );
      ...
      ("CreateTimestamp", option_to_yojson timestamp__to_yojson x.create_timestamp);
...
let delete_application_snapshot_request_to_yojson (x : delete_application_snapshot_request) =
  assoc_to_yojson
    [
      ("SnapshotCreationTimestamp", Some (timestamp__to_yojson x.snapshot_creation_timestamp));
      ("SnapshotName", Some (snapshot_name_to_yojson x.snapshot_name));
      ("ApplicationName", Some (application_name_to_yojson x.application_name));
    ]
