...
let make_s3_content_base_location_update ?base_path_update:(base_path_update_ : base_path option)
    ?bucket_arn_update:(bucket_arn_update_ : bucket_ar_n option) () =
  ({ base_path_update = base_path_update_; bucket_arn_update = bucket_arn_update_ }
...
let make_s3_content_location ?object_version:(object_version_ : object_version option)
    ~file_key:(file_key_ : file_key) ~bucket_ar_n:(bucket_ar_n_ : bucket_ar_n) () =
  ({ object_version = object_version_; file_key = file_key_; bucket_ar_n = bucket_ar_n_ }
    : s3_content_location)

let make_maven_reference ~version:(version_ : maven_version)
    ~artifact_id:(artifact_id_ : maven_artifact_id) ~group_id:(group_id_ : maven_group_id) () =
  ({ version = version_; artifact_id = artifact_id_; group_id = group_id_ } : maven_reference)

let make_custom_artifact_configuration ?maven_reference:(maven_reference_ : maven_reference option)
...
let make_vpc_configuration ~security_group_ids:(security_group_ids_ : security_group_ids)
    ~subnet_ids:(subnet_ids_ : subnet_ids) () =
  ({ security_group_ids = security_group_ids_; subnet_ids = subnet_ids_ } : vpc_configuration)

let make_vpc_configuration_update
    ?security_group_id_updates:(security_group_id_updates_ : security_group_ids option)
    ?subnet_id_updates:(subnet_id_updates_ : subnet_ids option)
...
let make_csv_mapping_parameters
    ...
    ~record_row_delimiter:(record_row_delimiter_ : record_row_delimiter) () =
  ({
...
let make_record_format ?mapping_parameters:(mapping_parameters_ : mapping_parameters option)
  ...
  ({ mapping_parameters = mapping_parameters_; record_format_type = record_format_type_ }
    : record_format)

let make_record_column ?mapping:(mapping_ : record_column_mapping option)
    ~sql_type:(sql_type_ : record_column_sql_type) ~name:(name_ : record_column_name) () =
...
let make_input_description
    ...
    ?name_prefix:(name_prefix_ : in_app_stream_name option) ?input_id:(input_id_ : id option) () =
  ({
...
let make_output_description ?destination_schema:(destination_schema_ : destination_schema option)
    ...
    ?name:(name_ : in_app_stream_name option) ?output_id:(output_id_ : id option) () =
  ({
...
let make_s3_reference_data_source_description
    ?reference_role_ar_n:(reference_role_ar_n_ : role_ar_n option) ~file_key:(file_key_ : file_key)
    ~bucket_ar_n:(bucket_ar_n_ : bucket_ar_n) () =
  ({ reference_role_ar_n = reference_role_ar_n_; file_key = file_key_; bucket_ar_n = bucket_ar_n_ }
...
let make_reference_data_source_description
    ...
    ~table_name:(table_name_ : in_app_table_name) ~reference_id:(reference_id_ : id) () =
  ({
...
let make_s3_application_code_location_description
    ?object_version:(object_version_ : object_version option) ~file_key:(file_key_ : file_key)
    ~bucket_ar_n:(bucket_ar_n_ : bucket_ar_n) () =
  ({ object_version = object_version_; file_key = file_key_; bucket_ar_n = bucket_ar_n_ }
...
let make_code_content_description
    ...
    ?code_size:(code_size_ : code_size option) ?code_m_d5:(code_m_d5_ : code_m_d5 option)
    ?text_content:(text_content_ : text_content option) () =
  ({
...
let make_application_code_configuration_description
  ...
  ({ code_content_description = code_content_description_; code_content_type = code_content_type_ }
...
let make_application_restore_configuration ?snapshot_name:(snapshot_name_ : snapshot_name option)
  ...
  ({ snapshot_name = snapshot_name_; application_restore_type = application_restore_type_ }
...
let make_monitoring_configuration_description ?log_level:(log_level_ : log_level option)
...
let make_property_group ~property_map:(property_map_ : property_map)
    ~property_group_id:(property_group_id_ : id) () =
  ({ property_map = property_map_; property_group_id = property_group_id_ } : property_group)
...
let make_cloud_watch_logging_option_description ?role_ar_n:(role_ar_n_ : role_ar_n option)
    ?cloud_watch_logging_option_id:(cloud_watch_logging_option_id_ : id option)
    ~log_stream_ar_n:(log_stream_ar_n_ : log_stream_ar_n) () =
  ({
...
let make_application_detail ?application_mode:(application_mode_ : application_mode option)
...
let make_input_schema_update ?record_column_updates:(record_column_updates_ : record_columns option)
...
let make_input_update
    ...
    ?name_prefix_update:(name_prefix_update_ : in_app_stream_name option) ~input_id:(input_id_ : id)
    () =
  ({
...
let make_output_update
    ...
    ?name_update:(name_update_ : in_app_stream_name option) ~output_id:(output_id_ : id) () =
  ({
...
let make_s3_reference_data_source_update ?file_key_update:(file_key_update_ : file_key option)
    ?bucket_arn_update:(bucket_arn_update_ : bucket_ar_n option) () =
  ({ file_key_update = file_key_update_; bucket_arn_update = bucket_arn_update_ }
...
let make_sql_application_configuration_update
    ...
    ?input_updates:(input_updates_ : input_updates option) () =
  ({
...
let make_code_content_update
    ...
    ?text_content_update:(text_content_update_ : text_content option) () =
  ({
     s3_content_location_update = s3_content_location_update_;
...
let make_monitoring_configuration_update ?log_level_update:(log_level_update_ : log_level option)
...
let make_untag_resource_request ~tag_keys:(tag_keys_ : tag_keys)
    ~resource_ar_n:(resource_ar_n_ : kinesis_analytics_ar_n) () =
...
let make_start_application_request
    ...
    ~application_name:(application_name_ : application_name) () =
  ({ run_configuration = run_configuration_; application_name = application_name_ }
...
let make_input ?input_parallelism:(input_parallelism_ : input_parallelism option)
    ...
    ~input_schema:(input_schema_ : source_schema) ~name_prefix:(name_prefix_ : in_app_stream_name)
    () =
  ({
     input_schema = input_schema_;
     input_parallelism = input_parallelism_;
...
let make_output ?lambda_output:(lambda_output_ : lambda_output option)
    ...
    ?kinesis_streams_output:(kinesis_streams_output_ : kinesis_streams_output option)
    ~destination_schema:(destination_schema_ : destination_schema)
    ~name:(name_ : in_app_stream_name) () =
  ({
     destination_schema = destination_schema_;
...
let make_s3_reference_data_source ?file_key:(file_key_ : file_key option)
    ?bucket_ar_n:(bucket_ar_n_ : bucket_ar_n option) () =
...
let make_sql_application_configuration
    ...
    ?outputs:(outputs_ : outputs option) ?inputs:(inputs_ : inputs option) () =
...
let make_s3_configuration ~file_key:(file_key_ : file_key) ~bucket_ar_n:(bucket_ar_n_ : bucket_ar_n)
    () =
...
let make_operation_failure_details ?error_info:(error_info_ : error_info option)
...
let make_list_applications_request ?next_token:(next_token_ : application_name option)
    ?limit:(limit_ : list_applications_input_limit option) () =
  ({ next_token = next_token_; limit = limit_ } : list_applications_request)

let make_application_version_summary ~application_status:(application_status_ : application_status)
    ~application_version_id:(application_version_id_ : application_version_id) () =
  ({ application_status = application_status_; application_version_id = application_version_id_ }
    : application_version_summary)

let make_list_application_versions_response ?next_token:(next_token_ : next_token option)
    ?application_version_summaries:
      (application_version_summaries_ : application_version_summaries option) () =
  ({ next_token = next_token_; application_version_summaries = application_version_summaries_ }
    : list_application_versions_response)

let make_list_application_versions_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : list_application_versions_input_limit option)
    ~application_name:(application_name_ : application_name) () =
  ({ next_token = next_token_; limit = limit_; application_name = application_name_ }
    : list_application_versions_request)

let make_list_application_snapshots_response ?next_token:(next_token_ : next_token option)
...
let make_list_application_snapshots_request ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : list_snapshots_input_limit option)
    ~application_name:(application_name_ : application_name) () =
  ({ next_token = next_token_; limit = limit_; application_name = application_name_ }
    : list_application_snapshots_request)

let make_application_operation_info ?operation_status:(operation_status_ : operation_status option)
    ?end_time:(end_time_ : timestamp option) ?start_time:(start_time_ : timestamp option)
    ?operation_id:(operation_id_ : operation_id option) ?operation:(operation_ : operation option)
    () =
  ({
...
let make_list_application_operations_response ?next_token:(next_token_ : next_token option)
    ?application_operation_info_list:
      (application_operation_info_list_ : application_operation_info_list option) () =
  ({ next_token = next_token_; application_operation_info_list = application_operation_info_list_ }
...
let make_list_application_operations_request
    ...
    ?operation:(operation_ : operation option) ?next_token:(next_token_ : next_token option)
...
let make_describe_application_version_request
    ~application_version_id:(application_version_id_ : application_version_id)
    ~application_name:(application_name_ : application_name) () =
  ({ application_version_id = application_version_id_; application_name = application_name_ }
...
let make_describe_application_snapshot_request ~snapshot_name:(snapshot_name_ : snapshot_name)
    ~application_name:(application_name_ : application_name) () =
...
let make_application_operation_info_details
    ...
    ~operation_status:(operation_status_ : operation_status) ~end_time:(end_time_ : timestamp)
    ~start_time:(start_time_ : timestamp) ~operation:(operation_ : operation) () =
  ({
...
let make_describe_application_operation_request ~operation_id:(operation_id_ : operation_id)
    ~application_name:(application_name_ : application_name) () =
...
let make_delete_application_reference_data_source_response
    ?application_version_id:(application_version_id_ : application_version_id option)
    ?application_ar_n:(application_ar_n_ : resource_ar_n option) () =
  ({ application_version_id = application_version_id_; application_ar_n = application_ar_n_ }
    : delete_application_reference_data_source_response)

let make_delete_application_reference_data_source_request ~reference_id:(reference_id_ : id)
    ~current_application_version_id:(current_application_version_id_ : application_version_id)
    ~application_name:(application_name_ : application_name) () =
  ({
     reference_id = reference_id_;
...
let make_delete_application_output_request ~output_id:(output_id_ : id)
    ~current_application_version_id:(current_application_version_id_ : application_version_id)
    ~application_name:(application_name_ : application_name) () =
  ({
     output_id = output_id_;
...
let make_delete_application_input_processing_configuration_request ~input_id:(input_id_ : id)
    ~current_application_version_id:(current_application_version_id_ : application_version_id)
    ~application_name:(application_name_ : application_name) () =
  ({
     input_id = input_id_;
...
let make_delete_application_request ~create_timestamp:(create_timestamp_ : timestamp)
    ~application_name:(application_name_ : application_name) () =
  ({ create_timestamp = create_timestamp_; application_name = application_name_ }
    : delete_application_request)

let make_create_application_snapshot_response () = (() : unit)

let make_create_application_snapshot_request ~snapshot_name:(snapshot_name_ : snapshot_name)
    ~application_name:(application_name_ : application_name) () =
  ({ snapshot_name = snapshot_name_; application_name = application_name_ }
...
let make_create_application_presigned_url_request
    ...
    ~url_type:(url_type_ : url_type) ~application_name:(application_name_ : application_name) () =
  ({
...
let make_code_content ?s3_content_location:(s3_content_location_ : s3_content_location option)
...
let make_create_application_request ?application_mode:(application_mode_ : application_mode option)
...
let make_add_application_input_processing_configuration_response
    ?input_processing_configuration_description:
      (input_processing_configuration_description_ :
         input_processing_configuration_description option) ?input_id:(input_id_ : id option)
    ?application_version_id:(application_version_id_ : application_version_id option)
    ?application_ar_n:(application_ar_n_ : resource_ar_n option) () =
  ({
     input_processing_configuration_description = input_processing_configuration_description_;
...
let make_add_application_input_processing_configuration_request
    ~input_processing_configuration:
      (input_processing_configuration_ : input_processing_configuration) ~input_id:(input_id_ : id)
    ~current_application_version_id:(current_application_version_id_ : application_version_id)
    ~application_name:(application_name_ : application_name) () =
  ({
     input_processing_configuration = input_processing_configuration_;
