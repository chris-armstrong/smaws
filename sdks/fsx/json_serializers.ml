...
let open_zfs_data_compression_type_to_yojson (x : open_zfs_data_compression_type) =
  match x with NONE -> `String "NONE" | ZSTD -> `String "ZSTD" | LZ4 -> `String "LZ4"
...
let disk_iops_configuration_mode_to_yojson (x : disk_iops_configuration_mode) =
  match x with AUTOMATIC -> `String "AUTOMATIC" | USER_PROVISIONED -> `String "USER_PROVISIONED"
...
let data_compression_type_to_yojson (x : data_compression_type) =
  match x with NONE -> `String "NONE" | LZ4 -> `String "LZ4"

let drive_cache_type_to_yojson (x : drive_cache_type) =
  match x with NONE -> `String "NONE" | READ -> `String "READ"
...
let file_system_maintenance_operation_to_yojson (x : file_system_maintenance_operation) =
  match x with PATCHING -> `String "PATCHING" | BACKING_UP -> `String "BACKING_UP"
...
let volume_style_to_yojson (x : volume_style) =
  match x with FLEXVOL -> `String "FLEXVOL" | FLEXGROUP -> `String "FLEXGROUP"

let snaplock_type_to_yojson (x : snaplock_type) =
  match x with COMPLIANCE -> `String "COMPLIANCE" | ENTERPRISE -> `String "ENTERPRISE"
...
let ontap_volume_type_to_yojson (x : ontap_volume_type) =
  match x with RW -> `String "RW" | DP -> `String "DP" | LS -> `String "LS"
...
let security_style_to_yojson (x : security_style) =
  match x with UNIX -> `String "UNIX" | NTFS -> `String "NTFS" | MIXED -> `String "MIXED"

let junction_path_to_yojson = string_to_yojson

let flex_cache_endpoint_type_to_yojson (x : flex_cache_endpoint_type) =
  match x with NONE -> `String "NONE" | ORIGIN -> `String "ORIGIN" | CACHE -> `String "CACHE"
...
let rec volume_to_yojson (x : volume) =
  assoc_to_yojson
    [
...
and administrative_action_to_yojson (x : administrative_action) =
  assoc_to_yojson
    [
...
let event_type_to_yojson (x : event_type) =
  match x with NEW -> `String "NEW" | CHANGED -> `String "CHANGED" | DELETED -> `String "DELETED"
...
let snapshot_filter_name_to_yojson (x : snapshot_filter_name) =
  match x with FILE_SYSTEM_ID -> `String "file-system-id" | VOLUME_ID -> `String "volume-id"
...
let ontap_file_system_user_type_to_yojson (x : ontap_file_system_user_type) =
  match x with UNIX -> `String "UNIX" | WINDOWS -> `String "WINDOWS"
...
let resource_type_to_yojson (x : resource_type) =
  match x with FILE_SYSTEM -> `String "FILE_SYSTEM" | VOLUME -> `String "VOLUME"
...
let input_ontap_volume_type_to_yojson (x : input_ontap_volume_type) =
  match x with RW -> `String "RW" | DP -> `String "DP"
