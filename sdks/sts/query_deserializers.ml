...
let provided_context_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "ContextAssertion"; "ProviderArn" ] (fun tag _ ->
      match tag with
      | "ContextAssertion" ->
          r_context_assertion :=
            Some
      ...
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ context_assertion = ( ! ) r_context_assertion; provider_arn = ( ! ) r_provider_arn }
    : provided_context)
...
let credentials_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "Expiration"; "SessionToken"; "SecretAccessKey"; "AccessKeyId" ] (fun tag _ ->
      match tag with
      | "Expiration" ->
          r_expiration :=
            Some
...
let get_session_token_request_of_xml i =
  ...
  let r_duration_seconds = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "TokenCode"; "SerialNumber"; "DurationSeconds" ]
    (fun tag _ ->
      match tag with
      | "TokenCode" ->
          r_token_code :=
...
let federated_user_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Arn"; "FederatedUserId" ] (fun tag _ ->
      match tag with
      | "Arn" ->
          r_arn :=
            Some (Smaws_Lib.Xml.Parse.Read.sequence i "Arn" (fun i _ -> arn_type_of_xml i) ())
...
let get_federation_token_response_of_xml i =
  ...
  let r_credentials = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "PackedPolicySize"; "FederatedUser"; "Credentials" ] (fun tag _ ->
      match tag with
      | "PackedPolicySize" ->
...
let get_federation_token_request_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "Tags"; "DurationSeconds"; "PolicyArns"; "Policy"; "Name" ] (fun tag _ ->
      match tag with
      | "Tags" ->
          r_tags :=
            Some
...
let get_caller_identity_response_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Arn"; "Account"; "UserId" ] (fun tag _ ->
      match tag with
      | "Arn" ->
          r_arn :=
            Some (Smaws_Lib.Xml.Parse.Read.sequence i "Arn" (fun i _ -> arn_type_of_xml i) ())
      ...
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ arn = ( ! ) r_arn; account = ( ! ) r_account; user_id = ( ! ) r_user_id }
...
let assumed_role_user_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Arn"; "AssumedRoleId" ] (fun tag _ ->
      match tag with
      | "Arn" ->
          r_arn :=
            Some (Smaws_Lib.Xml.Parse.Read.sequence i "Arn" (fun i _ -> arn_type_of_xml i) ())
...
let assume_root_response_of_xml i =
  ...
  let r_credentials = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "SourceIdentity"; "Credentials" ] (fun tag _ ->
      match tag with
      | "SourceIdentity" ->
          r_source_identity :=
            Some
          ...
          r_credentials :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "Credentials"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ source_identity = ( ! ) r_source_identity; credentials = ( ! ) r_credentials }
...
let assume_root_request_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "DurationSeconds"; "TaskPolicyArn"; "TargetPrincipal" ] (fun tag _ ->
      match tag with
      | "DurationSeconds" ->
          r_duration_seconds :=
            Some
...
let assume_role_with_saml_request_of_xml i =
  ...
  let r_role_arn = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "DurationSeconds"; "Policy"; "PolicyArns"; "SAMLAssertion"; "PrincipalArn"; "RoleArn" ]
    (fun tag _ ->
      match tag with
      | "DurationSeconds" ->
          r_duration_seconds :=
...
let assume_role_response_of_xml i =
  ...
  let r_credentials = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "SourceIdentity"; "PackedPolicySize"; "AssumedRoleUser"; "Credentials" ] (fun tag _ ->
      match tag with
      | "SourceIdentity" ->
          r_source_identity :=
            Some
