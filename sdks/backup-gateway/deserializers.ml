  ...
  let _res : vmware_tag =
    {
      vmware_tag_description =
        (option_of_yojson
           (value_for_key string__of_yojson "VmwareTagDescription") _list
           path);
      vmware_tag_name =
        (option_of_yojson
  ...
  let _res : virtual_machine =
    {
      ...
      hypervisor_id =
        (option_of_yojson (value_for_key string__of_yojson "HypervisorId")
           _list path);
      host_name =
        (option_of_yojson (value_for_key name_of_yojson "HostName") _list
           path)
    } in
  _res
  ...
  let _res : bandwidth_rate_limit_interval =
    {
      ...
      start_minute_of_hour =
        (value_for_key minute_of_hour_of_yojson "StartMinuteOfHour" _list
           path);
      end_hour_of_day =
  ...
  let _res : gateway_details =
    {
      vpc_endpoint =
        (option_of_yojson
           (value_for_key vpc_endpoint_of_yojson "VpcEndpoint") _list path);
...
let associate_gateway_to_server_input_of_yojson tree path =
  ...
  let _res : associate_gateway_to_server_input =
    {
      server_arn =
        (value_for_key server_arn_of_yojson "ServerArn" _list path);
      gateway_arn =
        (value_for_key gateway_arn_of_yojson "GatewayArn" _list path)
    } in
  _res
