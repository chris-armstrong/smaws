...
let make_vmware_to_aws_tag_mapping
  ...
  ~aws_tag_key:(aws_tag_key_ : tag_key)
  ~vmware_tag_name:(vmware_tag_name_ : vmware_tag_name)
...
let make_virtual_machine ?last_backup_date:(last_backup_date_ : time option)
  ...
  ?path:(path_ : path option) ?name:(name_ : name option)
  ?hypervisor_id:(hypervisor_id_ : string_ option)
...
let make_tag_resource_output
  ?resource_ar_n:(resource_ar_n_ : resource_arn option) () =
  ({ resource_ar_n = resource_ar_n_ } : tag_resource_output)
let make_tag_resource_input ~tags:(tags_ : tags)
  ~resource_ar_n:(resource_ar_n_ : resource_arn) () =
...
let make_get_virtual_machine_input
  ~resource_arn:(resource_arn_ : resource_arn) () =
...
let make_get_hypervisor_property_mappings_output
  ...
  ?vmware_to_aws_tag_mappings:(vmware_to_aws_tag_mappings_ :
                                vmware_to_aws_tag_mappings option)
  ?hypervisor_arn:(hypervisor_arn_ : server_arn option) () =
  ({
     iam_role_arn = iam_role_arn_;
     vmware_to_aws_tag_mappings = vmware_to_aws_tag_mappings_;
...
let make_get_bandwidth_rate_limit_schedule_output
  ?bandwidth_rate_limit_intervals:(bandwidth_rate_limit_intervals_ :
                                    bandwidth_rate_limit_intervals option)
  ?gateway_arn:(gateway_arn_ : gateway_arn option) () =
  ({
     bandwidth_rate_limit_intervals = bandwidth_rate_limit_intervals_;
     gateway_arn = gateway_arn_
...
let make_delete_hypervisor_input
  ~hypervisor_arn:(hypervisor_arn_ : server_arn) () =
  ({ hypervisor_arn = hypervisor_arn_ } : delete_hypervisor_input)
let make_delete_gateway_output
  ?gateway_arn:(gateway_arn_ : gateway_arn option) () =
...
let make_create_gateway_output
  ?gateway_arn:(gateway_arn_ : gateway_arn option) () =
