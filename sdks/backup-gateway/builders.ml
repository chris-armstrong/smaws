...
let make_virtual_machine
  ...
  ?resource_arn:(resource_arn_ : string option) ?path:(path_ : string option)
...
let make_test_hypervisor_configuration_input
  ...
  ~host:(host_ : string) ~gateway_arn:(gateway_arn_ : string) () =
  ({
     password = password_;
     username = username_;
     host = host_;
...
let make_import_hypervisor_configuration_input
  ?tags:(tags_ : tag list option) ?kms_key_arn:(kms_key_arn_ : string option)
  ?password:(password_ : string option) ?username:(username_ : string option)
  ~host:(host_ : string) ~name:(name_ : string) () =
  ({
     tags = tags_;
     kms_key_arn = kms_key_arn_;
     password = password_;
