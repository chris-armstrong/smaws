...
let make_upload_ssh_public_key_request ~user_name:(user_name_ : user_name_type)
    ~ssh_public_key_body:(ssh_public_key_body_ : public_key_material_type) () =
  ({ user_name = user_name_; ssh_public_key_body = ssh_public_key_body_ }
    : upload_ssh_public_key_request)

let make_signing_certificate ?upload_date:(upload_date_ : date_type option)
    ~user_name:(user_name_ : user_name_type) ~certificate_id:(certificate_id_ : certificate_id_type)
    ~certificate_body:(certificate_body_ : certificate_body_type) ~status:(status_ : status_type) ()
    =
  ({
     user_name = user_name_;
...
let make_upload_signing_certificate_request ?user_name:(user_name_ : existing_user_name_type option)
  ...
  ({ user_name = user_name_; certificate_body = certificate_body_ }
    : upload_signing_certificate_request)

let make_tag ~key:(key_ : tag_key_type) ~value:(value_ : tag_value_type) () =
  ({ key = key_; value = value_ } : tag)

let make_server_certificate_metadata ?upload_date:(upload_date_ : date_type option)
    ?expiration:(expiration_ : date_type option) ~path:(path_ : path_type)
    ~server_certificate_name:(server_certificate_name_ : server_certificate_name_type)
    ~server_certificate_id:(server_certificate_id_ : id_type) ~arn:(arn_ : arn_type) () =
  ({
     path = path_;
...
let make_upload_server_certificate_response
  ...
  ({ server_certificate_metadata = server_certificate_metadata_; tags = tags_ }
    : upload_server_certificate_response)

let make_upload_server_certificate_request ?path:(path_ : path_type option)
    ?certificate_chain:(certificate_chain_ : certificate_chain_type option)
    ?tags:(tags_ : tag_list_type option)
    ~server_certificate_name:(server_certificate_name_ : server_certificate_name_type)
    ~certificate_body:(certificate_body_ : certificate_body_type)
    ~private_key:(private_key_ : private_key_type) () =
  ({
     path = path_;
     server_certificate_name = server_certificate_name_;
     certificate_body = certificate_body_;
...
let make_update_user_request ?new_path:(new_path_ : path_type option)
    ?new_user_name:(new_user_name_ : user_name_type option)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ user_name = user_name_; new_path = new_path_; new_user_name = new_user_name_ }
    : update_user_request)

let make_update_ssh_public_key_request ~user_name:(user_name_ : user_name_type)
    ~ssh_public_key_id:(ssh_public_key_id_ : public_key_id_type) ~status:(status_ : status_type) ()
    =
  ({ user_name = user_name_; ssh_public_key_id = ssh_public_key_id_; status = status_ }
...
let make_update_signing_certificate_request ?user_name:(user_name_ : existing_user_name_type option)
    ~certificate_id:(certificate_id_ : certificate_id_type) ~status:(status_ : status_type) () =
  ({ user_name = user_name_; certificate_id = certificate_id_; status = status_ }
...
let make_update_server_certificate_request ?new_path:(new_path_ : path_type option)
...
let make_role_last_used ?last_used_date:(last_used_date_ : date_type option)
    ?region:(region_ : string_type option) () =
  ({ last_used_date = last_used_date_; region = region_ } : role_last_used)
...
let make_role
    ...
    ?tags:(tags_ : tag_list_type option) ?role_last_used:(role_last_used_ : role_last_used option)
    ~path:(path_ : path_type) ~role_name:(role_name_ : role_name_type) ~role_id:(role_id_ : id_type)
    ~arn:(arn_ : arn_type) ~create_date:(create_date_ : date_type) () =
  ({
     path = path_;
...
let make_update_role_description_request ~role_name:(role_name_ : role_name_type)
    ~description:(description_ : role_description_type) () =
  ({ role_name = role_name_; description = description_ } : update_role_description_request)

let make_update_role_response () = (() : unit)

let make_update_role_request ?description:(description_ : role_description_type option)
    ?max_session_duration:(max_session_duration_ : role_max_session_duration_type option)
...
let make_update_login_profile_request ?password:(password_ : password_type option)
...
let make_update_group_request ?new_path:(new_path_ : path_type option)
    ?new_group_name:(new_group_name_ : group_name_type option)
    ~group_name:(group_name_ : group_name_type) () =
  ({ group_name = group_name_; new_path = new_path_; new_group_name = new_group_name_ }
...
let make_update_delegation_request_request ?notes:(notes_ : notes_type option)
  ...
  ({ delegation_request_id = delegation_request_id_; notes = notes_ }
    : update_delegation_request_request)

let make_update_assume_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ role_name = role_name_; policy_document = policy_document_ }
...
let make_update_access_key_request ?user_name:(user_name_ : existing_user_name_type option)
    ~access_key_id:(access_key_id_ : access_key_id_type) ~status:(status_ : status_type) () =
  ({ user_name = user_name_; access_key_id = access_key_id_; status = status_ }
    : update_access_key_request)

let make_untag_user_request ~user_name:(user_name_ : existing_user_name_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ user_name = user_name_; tag_keys = tag_keys_ } : untag_user_request)

let make_untag_server_certificate_request
    ~server_certificate_name:(server_certificate_name_ : server_certificate_name_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ server_certificate_name = server_certificate_name_; tag_keys = tag_keys_ }
    : untag_server_certificate_request)

let make_untag_saml_provider_request ~saml_provider_arn:(saml_provider_arn_ : arn_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ saml_provider_arn = saml_provider_arn_; tag_keys = tag_keys_ } : untag_saml_provider_request)

let make_untag_role_request ~role_name:(role_name_ : role_name_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ role_name = role_name_; tag_keys = tag_keys_ } : untag_role_request)

let make_untag_policy_request ~policy_arn:(policy_arn_ : arn_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ policy_arn = policy_arn_; tag_keys = tag_keys_ } : untag_policy_request)

let make_untag_open_id_connect_provider_request
    ~open_id_connect_provider_arn:(open_id_connect_provider_arn_ : arn_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ open_id_connect_provider_arn = open_id_connect_provider_arn_; tag_keys = tag_keys_ }
    : untag_open_id_connect_provider_request)

let make_untag_mfa_device_request ~serial_number:(serial_number_ : serial_number_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ serial_number = serial_number_; tag_keys = tag_keys_ } : untag_mfa_device_request)

let make_untag_instance_profile_request
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type)
    ~tag_keys:(tag_keys_ : tag_key_list_type) () =
  ({ instance_profile_name = instance_profile_name_; tag_keys = tag_keys_ }
    : untag_instance_profile_request)

let make_tag_user_request ~user_name:(user_name_ : existing_user_name_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ user_name = user_name_; tags = tags_ } : tag_user_request)

let make_tag_server_certificate_request
    ~server_certificate_name:(server_certificate_name_ : server_certificate_name_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ server_certificate_name = server_certificate_name_; tags = tags_ }
    : tag_server_certificate_request)

let make_tag_saml_provider_request ~saml_provider_arn:(saml_provider_arn_ : arn_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ saml_provider_arn = saml_provider_arn_; tags = tags_ } : tag_saml_provider_request)

let make_tag_role_request ~role_name:(role_name_ : role_name_type) ~tags:(tags_ : tag_list_type) ()
    =
  ({ role_name = role_name_; tags = tags_ } : tag_role_request)

let make_tag_policy_request ~policy_arn:(policy_arn_ : arn_type) ~tags:(tags_ : tag_list_type) () =
  ({ policy_arn = policy_arn_; tags = tags_ } : tag_policy_request)

let make_tag_open_id_connect_provider_request
    ~open_id_connect_provider_arn:(open_id_connect_provider_arn_ : arn_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ open_id_connect_provider_arn = open_id_connect_provider_arn_; tags = tags_ }
    : tag_open_id_connect_provider_request)

let make_tag_mfa_device_request ~serial_number:(serial_number_ : serial_number_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ serial_number = serial_number_; tags = tags_ } : tag_mfa_device_request)

let make_tag_instance_profile_request
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type)
    ~tags:(tags_ : tag_list_type) () =
  ({ instance_profile_name = instance_profile_name_; tags = tags_ } : tag_instance_profile_request)
...
let make_position ?line:(line_ : line_number option) ?column:(column_ : column_number option) () =
  ({ line = line_; column = column_ } : position)

let make_statement ?source_policy_id:(source_policy_id_ : policy_identifier_type option)
    ?source_policy_type:(source_policy_type_ : policy_source_type option)
    ?start_position:(start_position_ : position option)
    ?end_position:(end_position_ : position option) () =
  ({
...
let make_simulate_policy_response
  ...
  ({ evaluation_results = evaluation_results_; is_truncated = is_truncated_; marker = marker_ }
    : simulate_policy_response)

let make_context_entry ?context_key_name:(context_key_name_ : context_key_name_type option)
    ?context_key_values:(context_key_values_ : context_key_value_list_type option)
    ?context_key_type:(context_key_type_ : context_key_type_enum option) () =
  ({
...
let make_simulate_principal_policy_request
    ...
    ?max_items:(max_items_ : max_items_type option) ?marker:(marker_ : marker_type option)
...
let make_simulate_custom_policy_request
    ?permissions_boundary_policy_input_list:
      (permissions_boundary_policy_input_list_ : simulation_policy_list_type option)
    ?resource_arns:(resource_arns_ : resource_name_list_type option)
    ?resource_policy:(resource_policy_ : policy_document_type option)
...
let make_set_default_policy_version_request ~policy_arn:(policy_arn_ : arn_type)
    ~version_id:(version_id_ : policy_version_id_type) () =
  ({ policy_arn = policy_arn_; version_id = version_id_ } : set_default_policy_version_request)

let make_send_delegation_token_request
    ~delegation_request_id:(delegation_request_id_ : delegation_request_id_type) () =
  ({ delegation_request_id = delegation_request_id_ } : send_delegation_token_request)

let make_resync_mfa_device_request ~user_name:(user_name_ : existing_user_name_type)
...
let make_service_specific_credential ?expiration_date:(expiration_date_ : date_type option)
    ...
    ~create_date:(create_date_ : date_type) ~service_name:(service_name_ : service_name)
    ~service_specific_credential_id:
      (service_specific_credential_id_ : service_specific_credential_id)
    ~user_name:(user_name_ : user_name_type) ~status:(status_ : status_type) () =
  ({
     create_date = create_date_;
...
let make_reset_service_specific_credential_request ?user_name:(user_name_ : user_name_type option)
    ~service_specific_credential_id:
      (service_specific_credential_id_ : service_specific_credential_id) () =
  ({ user_name = user_name_; service_specific_credential_id = service_specific_credential_id_ }
    : reset_service_specific_credential_request)

let make_remove_user_from_group_request ~group_name:(group_name_ : group_name_type)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ group_name = group_name_; user_name = user_name_ } : remove_user_from_group_request)

let make_remove_role_from_instance_profile_request
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type)
    ~role_name:(role_name_ : role_name_type) () =
  ({ instance_profile_name = instance_profile_name_; role_name = role_name_ }
...
let make_remove_client_id_from_open_id_connect_provider_request
  ...
  ({ open_id_connect_provider_arn = open_id_connect_provider_arn_; client_i_d = client_i_d_ }
...
let make_put_user_policy_request ~user_name:(user_name_ : existing_user_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ user_name = user_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : put_user_policy_request)

let make_put_user_permissions_boundary_request ~user_name:(user_name_ : user_name_type)
    ~permissions_boundary:(permissions_boundary_ : arn_type) () =
  ({ user_name = user_name_; permissions_boundary = permissions_boundary_ }
    : put_user_permissions_boundary_request)

let make_put_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ role_name = role_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : put_role_policy_request)

let make_put_role_permissions_boundary_request ~role_name:(role_name_ : role_name_type)
    ~permissions_boundary:(permissions_boundary_ : arn_type) () =
  ({ role_name = role_name_; permissions_boundary = permissions_boundary_ }
    : put_role_permissions_boundary_request)

let make_put_group_policy_request ~group_name:(group_name_ : group_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ group_name = group_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : put_group_policy_request)

let make_user ?password_last_used:(password_last_used_ : date_type option)
    ?permissions_boundary:(permissions_boundary_ : attached_permissions_boundary option)
    ?tags:(tags_ : tag_list_type option) ~path:(path_ : path_type)
    ~user_name:(user_name_ : user_name_type) ~user_id:(user_id_ : id_type) ~arn:(arn_ : arn_type)
    ~create_date:(create_date_ : date_type) () =
  ({
     path = path_;
     user_name = user_name_;
     user_id = user_id_;
...
let make_virtual_mfa_device ?base32_string_seed:(base32_string_seed_ : bootstrap_datum option)
    ?qr_code_pn_g:(qr_code_pn_g_ : bootstrap_datum option) ?user:(user_ : user option)
    ?enable_date:(enable_date_ : date_type option) ?tags:(tags_ : tag_list_type option)
    ~serial_number:(serial_number_ : serial_number_type) () =
  ({
     serial_number = serial_number_;
...
let make_list_virtual_mfa_devices_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~virtual_mfa_devices:(virtual_mfa_devices_ : virtual_mfa_device_list_type) () =
  ({ virtual_mfa_devices = virtual_mfa_devices_; is_truncated = is_truncated_; marker = marker_ }
    : list_virtual_mfa_devices_response)

let make_list_virtual_mfa_devices_request
  ...
  ({ assignment_status = assignment_status_; marker = marker_; max_items = max_items_ }
    : list_virtual_mfa_devices_request)

let make_list_user_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ } : list_user_tags_response)

let make_list_user_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ } : list_user_tags_request)

let make_list_users_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~users:(users_ : user_list_type) () =
  ({ users = users_; is_truncated = is_truncated_; marker = marker_ } : list_users_response)

let make_list_users_request ?path_prefix:(path_prefix_ : path_prefix_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ path_prefix = path_prefix_; marker = marker_; max_items = max_items_ } : list_users_request)

let make_list_user_policies_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~policy_names:(policy_names_ : policy_name_list_type) () =
  ({ policy_names = policy_names_; is_truncated = is_truncated_; marker = marker_ }
    : list_user_policies_response)

let make_list_user_policies_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ }
    : list_user_policies_request)

let make_ssh_public_key_metadata ~user_name:(user_name_ : user_name_type)
    ~ssh_public_key_id:(ssh_public_key_id_ : public_key_id_type) ~status:(status_ : status_type)
    ~upload_date:(upload_date_ : date_type) () =
  ({
     user_name = user_name_;
     ssh_public_key_id = ssh_public_key_id_;
     status = status_;
...
let make_list_ssh_public_keys_response
    ?ssh_public_keys:(ssh_public_keys_ : ssh_public_key_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ ssh_public_keys = ssh_public_keys_; is_truncated = is_truncated_; marker = marker_ }
    : list_ssh_public_keys_response)

let make_list_ssh_public_keys_request ?user_name:(user_name_ : user_name_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ }
    : list_ssh_public_keys_request)

let make_list_signing_certificates_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~certificates:(certificates_ : certificate_list_type) () =
  ({ certificates = certificates_; is_truncated = is_truncated_; marker = marker_ }
    : list_signing_certificates_response)

let make_list_signing_certificates_request ?user_name:(user_name_ : existing_user_name_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ }
    : list_signing_certificates_request)

let make_service_specific_credential_metadata
    ?service_user_name:(service_user_name_ : service_user_name option)
    ?service_credential_alias:(service_credential_alias_ : service_credential_alias option)
    ?expiration_date:(expiration_date_ : date_type option) ~user_name:(user_name_ : user_name_type)
    ~status:(status_ : status_type) ~create_date:(create_date_ : date_type)
    ~service_specific_credential_id:
      (service_specific_credential_id_ : service_specific_credential_id)
...
let make_list_service_specific_credentials_request ?user_name:(user_name_ : user_name_type option)
    ?service_name:(service_name_ : service_name option) ?all_users:(all_users_ : all_users option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({
     user_name = user_name_;
     service_name = service_name_;
...
let make_list_server_certificate_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ }
    : list_server_certificate_tags_response)

let make_list_server_certificate_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~server_certificate_name:(server_certificate_name_ : server_certificate_name_type) () =
  ({ server_certificate_name = server_certificate_name_; marker = marker_; max_items = max_items_ }
    : list_server_certificate_tags_request)

let make_list_server_certificates_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
...
let make_list_server_certificates_request ?path_prefix:(path_prefix_ : path_prefix_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ path_prefix = path_prefix_; marker = marker_; max_items = max_items_ }
    : list_server_certificates_request)

let make_list_saml_provider_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ }
    : list_saml_provider_tags_response)

let make_list_saml_provider_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~saml_provider_arn:(saml_provider_arn_ : arn_type) () =
  ({ saml_provider_arn = saml_provider_arn_; marker = marker_; max_items = max_items_ }
    : list_saml_provider_tags_request)

let make_saml_provider_list_entry ?arn:(arn_ : arn_type option)
    ?valid_until:(valid_until_ : date_type option) ?create_date:(create_date_ : date_type option) ()
    =
  ({ arn = arn_; valid_until = valid_until_; create_date = create_date_ }
...
let make_list_role_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ } : list_role_tags_response)

let make_list_role_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~role_name:(role_name_ : role_name_type) () =
  ({ role_name = role_name_; marker = marker_; max_items = max_items_ } : list_role_tags_request)

let make_list_roles_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~roles:(roles_ : role_list_type) () =
  ({ roles = roles_; is_truncated = is_truncated_; marker = marker_ } : list_roles_response)

let make_list_roles_request ?path_prefix:(path_prefix_ : path_prefix_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ path_prefix = path_prefix_; marker = marker_; max_items = max_items_ } : list_roles_request)

let make_list_role_policies_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~policy_names:(policy_names_ : policy_name_list_type) () =
  ({ policy_names = policy_names_; is_truncated = is_truncated_; marker = marker_ }
    : list_role_policies_response)

let make_list_role_policies_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~role_name:(role_name_ : role_name_type) () =
  ({ role_name = role_name_; marker = marker_; max_items = max_items_ }
    : list_role_policies_request)

let make_policy_version ?document:(document_ : policy_document_type option)
...
let make_list_policy_versions_response
    ?versions:(versions_ : policy_document_version_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ versions = versions_; is_truncated = is_truncated_; marker = marker_ }
    : list_policy_versions_response)

let make_list_policy_versions_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~policy_arn:(policy_arn_ : arn_type) () =
  ({ policy_arn = policy_arn_; marker = marker_; max_items = max_items_ }
    : list_policy_versions_request)

let make_list_policy_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ } : list_policy_tags_response)

let make_list_policy_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~policy_arn:(policy_arn_ : arn_type) () =
  ({ policy_arn = policy_arn_; marker = marker_; max_items = max_items_ }
    : list_policy_tags_request)

let make_policy_granting_service_access ?policy_arn:(policy_arn_ : arn_type option)
    ?entity_type:(entity_type_ : policy_owner_entity_type option)
    ?entity_name:(entity_name_ : entity_name_type option)
    ~policy_name:(policy_name_ : policy_name_type) ~policy_type:(policy_type_ : policy_type) () =
  ({
...
let make_list_policies_granting_service_access_entry
  ...
  ({ service_namespace = service_namespace_; policies = policies_ }
...
let make_list_policies_granting_service_access_request ?marker:(marker_ : marker_type option)
    ~arn:(arn_ : arn_type) ~service_namespaces:(service_namespaces_ : service_namespace_list_type)
    () =
  ({ marker = marker_; arn = arn_; service_namespaces = service_namespaces_ }
    : list_policies_granting_service_access_request)

let make_policy ?policy_name:(policy_name_ : policy_name_type option)
    ?policy_id:(policy_id_ : id_type option) ?arn:(arn_ : arn_type option)
    ...
    ?create_date:(create_date_ : date_type option) ?update_date:(update_date_ : date_type option)
    ?tags:(tags_ : tag_list_type option) () =
  ({
     policy_name = policy_name_;
...
let make_list_policies_response ?policies:(policies_ : policy_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ policies = policies_; is_truncated = is_truncated_; marker = marker_ }
    : list_policies_response)

let make_list_policies_request ?scope:(scope_ : policy_scope_type option)
    ?only_attached:(only_attached_ : boolean_type option)
...
let make_list_organizations_features_response
  ...
  ({ organization_id = organization_id_; enabled_features = enabled_features_ }
    : list_organizations_features_response)

let make_list_organizations_features_request () = (() : unit)

let make_list_open_id_connect_provider_tags_response
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ }
    : list_open_id_connect_provider_tags_response)

let make_list_open_id_connect_provider_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~open_id_connect_provider_arn:(open_id_connect_provider_arn_ : arn_type) () =
  ({
     open_id_connect_provider_arn = open_id_connect_provider_arn_;
...
let make_list_mfa_device_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ } : list_mfa_device_tags_response)

let make_list_mfa_device_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~serial_number:(serial_number_ : serial_number_type) () =
  ({ serial_number = serial_number_; marker = marker_; max_items = max_items_ }
    : list_mfa_device_tags_request)

let make_mfa_device ~user_name:(user_name_ : user_name_type)
    ~serial_number:(serial_number_ : serial_number_type) ~enable_date:(enable_date_ : date_type) ()
    =
  ({ user_name = user_name_; serial_number = serial_number_; enable_date = enable_date_ }
    : mfa_device)

let make_list_mfa_devices_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~mfa_devices:(mfa_devices_ : mfa_device_list_type) () =
  ({ mfa_devices = mfa_devices_; is_truncated = is_truncated_; marker = marker_ }
    : list_mfa_devices_response)

let make_list_mfa_devices_request ?user_name:(user_name_ : existing_user_name_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ } : list_mfa_devices_request)

let make_list_instance_profile_tags_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~tags:(tags_ : tag_list_type) () =
  ({ tags = tags_; is_truncated = is_truncated_; marker = marker_ }
    : list_instance_profile_tags_response)

let make_list_instance_profile_tags_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type) () =
  ({ instance_profile_name = instance_profile_name_; marker = marker_; max_items = max_items_ }
    : list_instance_profile_tags_request)

let make_instance_profile ?tags:(tags_ : tag_list_type option) ~path:(path_ : path_type)
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type)
    ~instance_profile_id:(instance_profile_id_ : id_type) ~arn:(arn_ : arn_type)
    ~create_date:(create_date_ : date_type) ~roles:(roles_ : role_list_type) () =
  ({
     path = path_;
...
let make_list_instance_profiles_for_role_response
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~instance_profiles:(instance_profiles_ : instance_profile_list_type) () =
  ({ instance_profiles = instance_profiles_; is_truncated = is_truncated_; marker = marker_ }
    : list_instance_profiles_for_role_response)

let make_list_instance_profiles_for_role_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~role_name:(role_name_ : role_name_type) () =
  ({ role_name = role_name_; marker = marker_; max_items = max_items_ }
    : list_instance_profiles_for_role_request)

let make_list_instance_profiles_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~instance_profiles:(instance_profiles_ : instance_profile_list_type) () =
  ({ instance_profiles = instance_profiles_; is_truncated = is_truncated_; marker = marker_ }
    : list_instance_profiles_response)

let make_list_instance_profiles_request ?path_prefix:(path_prefix_ : path_prefix_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ path_prefix = path_prefix_; marker = marker_; max_items = max_items_ }
    : list_instance_profiles_request)

let make_group ~path:(path_ : path_type) ~group_name:(group_name_ : group_name_type)
    ~group_id:(group_id_ : id_type) ~arn:(arn_ : arn_type) ~create_date:(create_date_ : date_type)
    () =
  ({
     path = path_;
...
let make_list_groups_for_user_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~groups:(groups_ : group_list_type) () =
  ({ groups = groups_; is_truncated = is_truncated_; marker = marker_ }
    : list_groups_for_user_response)

let make_list_groups_for_user_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ }
    : list_groups_for_user_request)

let make_list_groups_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~groups:(groups_ : group_list_type) () =
  ({ groups = groups_; is_truncated = is_truncated_; marker = marker_ } : list_groups_response)

let make_list_groups_request ?path_prefix:(path_prefix_ : path_prefix_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ path_prefix = path_prefix_; marker = marker_; max_items = max_items_ } : list_groups_request)

let make_list_group_policies_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~policy_names:(policy_names_ : policy_name_list_type) () =
  ({ policy_names = policy_names_; is_truncated = is_truncated_; marker = marker_ }
    : list_group_policies_response)

let make_list_group_policies_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~group_name:(group_name_ : group_name_type) () =
  ({ group_name = group_name_; marker = marker_; max_items = max_items_ }
    : list_group_policies_request)

let make_policy_role ?role_name:(role_name_ : role_name_type option)
    ?role_id:(role_id_ : id_type option) () =
  ({ role_name = role_name_; role_id = role_id_ } : policy_role)

let make_policy_user ?user_name:(user_name_ : user_name_type option)
    ?user_id:(user_id_ : id_type option) () =
  ({ user_name = user_name_; user_id = user_id_ } : policy_user)

let make_policy_group ?group_name:(group_name_ : group_name_type option)
    ?group_id:(group_id_ : id_type option) () =
  ({ group_name = group_name_; group_id = group_id_ } : policy_group)

let make_list_entities_for_policy_response
...
let make_list_entities_for_policy_request ?entity_filter:(entity_filter_ : entity_type option)
...
let make_policy_parameter ?name:(name_ : policy_parameter_name_type option)
    ?values:(values_ : policy_parameter_values_list_type option)
    ?type_:(type__ : policy_parameter_type_enum option) () =
  ({ name = name_; values = values_; type_ = type__ } : policy_parameter)

let make_delegation_permission ?policy_template_arn:(policy_template_arn_ : arn_type option)
    ?parameters:(parameters_ : policy_parameter_list_type option) () =
  ({ policy_template_arn = policy_template_arn_; parameters = parameters_ } : delegation_permission)

let make_delegation_request
    ...
    ?owner_id:(owner_id_ : owner_id_type option) ?approver_id:(approver_id_ : arn_type option)
    ?state:(state_ : state_type option) ?expiration_time:(expiration_time_ : date_type option)
    ...
    ?redirect_url:(redirect_url_ : redirect_url_type option) ?notes:(notes_ : notes_type option)
    ?rejection_reason:(rejection_reason_ : notes_type option)
    ?only_send_by_owner:(only_send_by_owner_ : boolean_type option)
    ?updated_time:(updated_time_ : date_type option) () =
  ({
...
let make_list_delegation_requests_response
    ?delegation_requests:(delegation_requests_ : delegation_requests_list_type option)
    ?marker:(marker_ : marker_type option) ?is_truncated:(is_truncated_ : boolean_type option) () =
  ({ delegation_requests = delegation_requests_; marker = marker_; is_truncated = is_truncated_ }
    : list_delegation_requests_response)

let make_list_delegation_requests_request ?owner_id:(owner_id_ : owner_id_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ owner_id = owner_id_; marker = marker_; max_items = max_items_ }
    : list_delegation_requests_request)

let make_attached_policy ?policy_name:(policy_name_ : policy_name_type option)
    ?policy_arn:(policy_arn_ : arn_type option) () =
  ({ policy_name = policy_name_; policy_arn = policy_arn_ } : attached_policy)

let make_list_attached_user_policies_response
    ?attached_policies:(attached_policies_ : attached_policies_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ attached_policies = attached_policies_; is_truncated = is_truncated_; marker = marker_ }
    : list_attached_user_policies_response)

let make_list_attached_user_policies_request ?path_prefix:(path_prefix_ : policy_path_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option)
    ~user_name:(user_name_ : user_name_type) () =
  ({ user_name = user_name_; path_prefix = path_prefix_; marker = marker_; max_items = max_items_ }
    : list_attached_user_policies_request)

let make_list_attached_role_policies_response
    ?attached_policies:(attached_policies_ : attached_policies_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ attached_policies = attached_policies_; is_truncated = is_truncated_; marker = marker_ }
...
let make_list_attached_role_policies_request ?path_prefix:(path_prefix_ : policy_path_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option)
    ~role_name:(role_name_ : role_name_type) () =
  ({ role_name = role_name_; path_prefix = path_prefix_; marker = marker_; max_items = max_items_ }
    : list_attached_role_policies_request)

let make_list_attached_group_policies_response
    ?attached_policies:(attached_policies_ : attached_policies_list_type option)
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) () =
  ({ attached_policies = attached_policies_; is_truncated = is_truncated_; marker = marker_ }
...
let make_list_attached_group_policies_request ?path_prefix:(path_prefix_ : policy_path_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option)
    ~group_name:(group_name_ : group_name_type) () =
  ({
     group_name = group_name_;
...
let make_list_account_aliases_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~account_aliases:(account_aliases_ : account_alias_list_type) () =
  ({ account_aliases = account_aliases_; is_truncated = is_truncated_; marker = marker_ }
    : list_account_aliases_response)

let make_list_account_aliases_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) () =
  ({ marker = marker_; max_items = max_items_ } : list_account_aliases_request)

let make_access_key_metadata ?user_name:(user_name_ : user_name_type option)
    ?access_key_id:(access_key_id_ : access_key_id_type option)
    ?status:(status_ : status_type option) ?create_date:(create_date_ : date_type option) () =
  ({
     user_name = user_name_;
...
let make_list_access_keys_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option)
    ~access_key_metadata:(access_key_metadata_ : access_key_metadata_list_type) () =
  ({ access_key_metadata = access_key_metadata_; is_truncated = is_truncated_; marker = marker_ }
    : list_access_keys_response)

let make_list_access_keys_request ?user_name:(user_name_ : existing_user_name_type option)
    ?marker:(marker_ : marker_type option) ?max_items:(max_items_ : max_items_type option) () =
  ({ user_name = user_name_; marker = marker_; max_items = max_items_ } : list_access_keys_request)

let make_get_user_policy_response ~user_name:(user_name_ : existing_user_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ user_name = user_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : get_user_policy_response)

let make_get_user_policy_request ~user_name:(user_name_ : existing_user_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ user_name = user_name_; policy_name = policy_name_ } : get_user_policy_request)
...
let make_get_ssh_public_key_request ~user_name:(user_name_ : user_name_type)
    ~ssh_public_key_id:(ssh_public_key_id_ : public_key_id_type)
    ~encoding:(encoding_ : encoding_type) () =
  ({ user_name = user_name_; ssh_public_key_id = ssh_public_key_id_; encoding = encoding_ }
    : get_ssh_public_key_request)

let make_role_usage_type ?region:(region_ : region_name_type option)
    ?resources:(resources_ : arn_list_type option) () =
  ({ region = region_; resources = resources_ } : role_usage_type)

let make_deletion_task_failure_reason_type ?reason:(reason_ : reason_type option)
    ?role_usage_list:(role_usage_list_ : role_usage_list_type option) () =
  ({ reason = reason_; role_usage_list = role_usage_list_ } : deletion_task_failure_reason_type)
...
let make_get_service_linked_role_deletion_status_response
  ...
  ({ status = status_; reason = reason_ } : get_service_linked_role_deletion_status_response)
...
let make_error_details ~message:(message_ : string_type) ~code:(code_ : string_type) () =
  ({ message = message_; code = code_ } : error_details)

let make_entity_info ?path:(path_ : path_type option) ~arn:(arn_ : arn_type)
    ~name:(name_ : user_name_type) ~type_:(type__ : policy_owner_entity_type) ~id:(id_ : id_type) ()
    =
  ({ arn = arn_; name = name_; type_ = type__; id = id_; path = path_ } : entity_info)

let make_entity_details ?last_authenticated:(last_authenticated_ : date_type option)
    ~entity_info:(entity_info_ : entity_info) () =
  ({ entity_info = entity_info_; last_authenticated = last_authenticated_ } : entity_details)

let make_get_service_last_accessed_details_with_entities_response
    ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ?error:(error_ : error_details option)
    ~job_status:(job_status_ : job_status_type) ~job_creation_date:(job_creation_date_ : date_type)
...
let make_get_service_last_accessed_details_with_entities_request
    ?max_items:(max_items_ : max_items_type option) ?marker:(marker_ : marker_type option)
    ~job_id:(job_id_ : job_id_type) ~service_namespace:(service_namespace_ : service_namespace_type)
    () =
  ({
     job_id = job_id_;
...
let make_tracked_action_last_accessed ?action_name:(action_name_ : string_type option)
...
let make_service_last_accessed ?last_authenticated:(last_authenticated_ : date_type option)
...
let make_get_service_last_accessed_details_response
...
let make_get_service_last_accessed_details_request ?max_items:(max_items_ : max_items_type option)
    ?marker:(marker_ : marker_type option) ~job_id:(job_id_ : job_id_type) () =
  ({ job_id = job_id_; max_items = max_items_; marker = marker_ }
    : get_service_last_accessed_details_request)

let make_server_certificate ?certificate_chain:(certificate_chain_ : certificate_chain_type option)
    ?tags:(tags_ : tag_list_type option)
...
let make_saml_private_key ?key_id:(key_id_ : private_key_id_type option)
    ?timestamp:(timestamp_ : date_type option) () =
  ({ key_id = key_id_; timestamp = timestamp_ } : saml_private_key)

let make_get_saml_provider_response
    ?saml_provider_uui_d:(saml_provider_uui_d_ : private_key_id_type option)
    ?saml_metadata_document:(saml_metadata_document_ : saml_metadata_document_type option)
    ?create_date:(create_date_ : date_type option) ?valid_until:(valid_until_ : date_type option)
    ?tags:(tags_ : tag_list_type option)
    ?assertion_encryption_mode:(assertion_encryption_mode_ : assertion_encryption_mode_type option)
    ?private_key_list:(private_key_list_ : private_key_list option) () =
  ({
...
let make_get_role_policy_response ~role_name:(role_name_ : role_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ role_name = role_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : get_role_policy_response)

let make_get_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ role_name = role_name_; policy_name = policy_name_ } : get_role_policy_request)
...
let make_get_policy_version_request ~policy_arn:(policy_arn_ : arn_type)
    ~version_id:(version_id_ : policy_version_id_type) () =
  ({ policy_arn = policy_arn_; version_id = version_id_ } : get_policy_version_request)
...
let make_get_outbound_web_identity_federation_info_response
  ...
  ({ issuer_identifier = issuer_identifier_; jwt_vending_enabled = jwt_vending_enabled_ }
    : get_outbound_web_identity_federation_info_response)

let make_access_detail ?region:(region_ : string_type option)
...
let make_get_organizations_access_report_response
    ...
    ?is_truncated:(is_truncated_ : boolean_type option) ?marker:(marker_ : marker_type option)
...
let make_get_organizations_access_report_request ?max_items:(max_items_ : max_items_type option)
    ?marker:(marker_ : marker_type option) ?sort_key:(sort_key_ : sort_key_type option)
    ~job_id:(job_id_ : job_id_type) () =
  ({ job_id = job_id_; max_items = max_items_; marker = marker_; sort_key = sort_key_ }
    : get_organizations_access_report_request)

let make_get_open_id_connect_provider_response
    ...
    ?create_date:(create_date_ : date_type option) ?tags:(tags_ : tag_list_type option) () =
  ({
     url = url_;
...
let make_get_mfa_device_response ?user_name:(user_name_ : user_name_type option)
    ?enable_date:(enable_date_ : date_type option)
    ?certifications:(certifications_ : certification_map_type option)
    ~serial_number:(serial_number_ : serial_number_type) () =
  ({
     user_name = user_name_;
     serial_number = serial_number_;
...
let make_get_mfa_device_request ?user_name:(user_name_ : user_name_type option)
    ~serial_number:(serial_number_ : serial_number_type) () =
  ({ serial_number = serial_number_; user_name = user_name_ } : get_mfa_device_request)

let make_login_profile ?password_reset_required:(password_reset_required_ : boolean_type option)
    ~user_name:(user_name_ : user_name_type) ~create_date:(create_date_ : date_type) () =
  ({
     user_name = user_name_;
     create_date = create_date_;
     password_reset_required = password_reset_required_;
...
let make_get_human_readable_summary_response
  ...
  ({ summary_content = summary_content_; locale = locale_; summary_state = summary_state_ }
...
let make_get_human_readable_summary_request ?locale:(locale_ : locale_type option)
  ...
  ({ entity_arn = entity_arn_; locale = locale_ } : get_human_readable_summary_request)

let make_get_group_policy_response ~group_name:(group_name_ : group_name_type)
    ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({ group_name = group_name_; policy_name = policy_name_; policy_document = policy_document_ }
    : get_group_policy_response)

let make_get_group_policy_request ~group_name:(group_name_ : group_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ group_name = group_name_; policy_name = policy_name_ } : get_group_policy_request)

let make_get_group_response ?is_truncated:(is_truncated_ : boolean_type option)
    ?marker:(marker_ : response_marker_type option) ~group:(group_ : group)
    ~users:(users_ : user_list_type) () =
  ({ group = group_; users = users_; is_truncated = is_truncated_; marker = marker_ }
    : get_group_response)

let make_get_group_request ?marker:(marker_ : marker_type option)
    ?max_items:(max_items_ : max_items_type option) ~group_name:(group_name_ : group_name_type) () =
  ({ group_name = group_name_; marker = marker_; max_items = max_items_ } : get_group_request)
...
let make_get_credential_report_response ?content:(content_ : report_content_type option)
    ?report_format:(report_format_ : report_format_type option)
    ?generated_time:(generated_time_ : date_type option) () =
  ({ content = content_; report_format = report_format_; generated_time = generated_time_ }
...
let make_get_context_keys_for_principal_policy_request
    ?policy_input_list:(policy_input_list_ : simulation_policy_list_type option)
    ~policy_source_arn:(policy_source_arn_ : arn_type) () =
  ({ policy_source_arn = policy_source_arn_; policy_input_list = policy_input_list_ }
...
let make_managed_policy_detail ?policy_name:(policy_name_ : policy_name_type option)
    ?policy_id:(policy_id_ : id_type option) ?arn:(arn_ : arn_type option)
    ?path:(path_ : policy_path_type option)
    ?default_version_id:(default_version_id_ : policy_version_id_type option)
    ?attachment_count:(attachment_count_ : attachment_count_type option)
...
let make_policy_detail ?policy_name:(policy_name_ : policy_name_type option)
    ?policy_document:(policy_document_ : policy_document_type option) () =
  ({ policy_name = policy_name_; policy_document = policy_document_ } : policy_detail)

let make_role_detail ?path:(path_ : path_type option)
    ?role_name:(role_name_ : role_name_type option) ?role_id:(role_id_ : id_type option)
    ?arn:(arn_ : arn_type option) ?create_date:(create_date_ : date_type option)
    ?assume_role_policy_document:(assume_role_policy_document_ : policy_document_type option)
...
let make_group_detail ?path:(path_ : path_type option)
    ?group_name:(group_name_ : group_name_type option) ?group_id:(group_id_ : id_type option)
    ?arn:(arn_ : arn_type option) ?create_date:(create_date_ : date_type option)
...
let make_user_detail ?path:(path_ : path_type option)
    ?user_name:(user_name_ : user_name_type option) ?user_id:(user_id_ : id_type option)
    ?arn:(arn_ : arn_type option) ?create_date:(create_date_ : date_type option)
...
let make_get_account_authorization_details_response
...
let make_get_account_authorization_details_request ?filter:(filter_ : entity_list_type option)
    ?max_items:(max_items_ : max_items_type option) ?marker:(marker_ : marker_type option) () =
  ({ filter = filter_; max_items = max_items_; marker = marker_ }
    : get_account_authorization_details_request)

let make_access_key_last_used ?last_used_date:(last_used_date_ : date_type option)
    ~service_name:(service_name_ : string_type) ~region:(region_ : string_type) () =
  ({ last_used_date = last_used_date_; service_name = service_name_; region = region_ }
    : access_key_last_used)

let make_get_access_key_last_used_response ?user_name:(user_name_ : existing_user_name_type option)
    ?access_key_last_used:(access_key_last_used_ : access_key_last_used option) () =
  ({ user_name = user_name_; access_key_last_used = access_key_last_used_ }
...
let make_generate_service_last_accessed_details_request
    ...
    ~arn:(arn_ : arn_type) () =
  ({ arn = arn_; granularity = granularity_ } : generate_service_last_accessed_details_request)
...
let make_generate_organizations_access_report_request
  ...
  ({ entity_path = entity_path_; organizations_policy_id = organizations_policy_id_ }
    : generate_organizations_access_report_request)

let make_generate_credential_report_response ?state:(state_ : report_state_type option)
    ?description:(description_ : report_state_description_type option) () =
  ({ state = state_; description = description_ } : generate_credential_report_response)
...
let make_enable_mfa_device_request ~user_name:(user_name_ : existing_user_name_type)
    ~serial_number:(serial_number_ : serial_number_type)
    ~authentication_code1:(authentication_code1_ : authentication_code_type)
    ~authentication_code2:(authentication_code2_ : authentication_code_type) () =
  ({
...
let make_detach_user_policy_request ~user_name:(user_name_ : user_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ user_name = user_name_; policy_arn = policy_arn_ } : detach_user_policy_request)

let make_detach_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ role_name = role_name_; policy_arn = policy_arn_ } : detach_role_policy_request)

let make_detach_group_policy_request ~group_name:(group_name_ : group_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ group_name = group_name_; policy_arn = policy_arn_ } : detach_group_policy_request)
...
let make_delete_user_policy_request ~user_name:(user_name_ : existing_user_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ user_name = user_name_; policy_name = policy_name_ } : delete_user_policy_request)
...
let make_delete_ssh_public_key_request ~user_name:(user_name_ : user_name_type)
    ~ssh_public_key_id:(ssh_public_key_id_ : public_key_id_type) () =
  ({ user_name = user_name_; ssh_public_key_id = ssh_public_key_id_ }
...
let make_delete_signing_certificate_request ?user_name:(user_name_ : existing_user_name_type option)
  ...
  ({ user_name = user_name_; certificate_id = certificate_id_ }
...
let make_delete_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ role_name = role_name_; policy_name = policy_name_ } : delete_role_policy_request)
...
let make_delete_policy_version_request ~policy_arn:(policy_arn_ : arn_type)
    ~version_id:(version_id_ : policy_version_id_type) () =
  ({ policy_arn = policy_arn_; version_id = version_id_ } : delete_policy_version_request)
...
let make_delete_group_policy_request ~group_name:(group_name_ : group_name_type)
    ~policy_name:(policy_name_ : policy_name_type) () =
  ({ group_name = group_name_; policy_name = policy_name_ } : delete_group_policy_request)
...
let make_delete_access_key_request ?user_name:(user_name_ : existing_user_name_type option)
  ...
  ({ user_name = user_name_; access_key_id = access_key_id_ } : delete_access_key_request)

let make_deactivate_mfa_device_request ?user_name:(user_name_ : existing_user_name_type option)
    ~serial_number:(serial_number_ : serial_number_type) () =
  ({ user_name = user_name_; serial_number = serial_number_ } : deactivate_mfa_device_request)
...
let make_create_virtual_mfa_device_request ?path:(path_ : path_type option)
    ?tags:(tags_ : tag_list_type option)
    ~virtual_mfa_device_name:(virtual_mfa_device_name_ : virtual_mfa_device_name) () =
  ({ path = path_; virtual_mfa_device_name = virtual_mfa_device_name_; tags = tags_ }
    : create_virtual_mfa_device_request)

let make_create_user_response ?user:(user_ : user option) () =
  ({ user = user_ } : create_user_response)

let make_create_user_request ?path:(path_ : path_type option)
    ?permissions_boundary:(permissions_boundary_ : arn_type option)
    ?tags:(tags_ : tag_list_type option) ~user_name:(user_name_ : user_name_type) () =
  ({
     path = path_;
     user_name = user_name_;
     permissions_boundary = permissions_boundary_;
...
let make_create_service_specific_credential_request
    ...
    ~user_name:(user_name_ : user_name_type) ~service_name:(service_name_ : service_name) () =
  ({
     user_name = user_name_;
     service_name = service_name_;
...
let make_create_saml_provider_response ?saml_provider_arn:(saml_provider_arn_ : arn_type option)
    ?tags:(tags_ : tag_list_type option) () =
  ({ saml_provider_arn = saml_provider_arn_; tags = tags_ } : create_saml_provider_response)

let make_create_saml_provider_request ?tags:(tags_ : tag_list_type option)
    ?assertion_encryption_mode:(assertion_encryption_mode_ : assertion_encryption_mode_type option)
    ?add_private_key:(add_private_key_ : private_key_type option)
    ~saml_metadata_document:(saml_metadata_document_ : saml_metadata_document_type)
    ~name:(name_ : saml_provider_name_type) () =
  ({
     saml_metadata_document = saml_metadata_document_;
     name = name_;
     tags = tags_;
...
let make_create_role_request ?path:(path_ : path_type option)
    ?description:(description_ : role_description_type option)
    ?max_session_duration:(max_session_duration_ : role_max_session_duration_type option)
    ?permissions_boundary:(permissions_boundary_ : arn_type option)
    ?tags:(tags_ : tag_list_type option) ~role_name:(role_name_ : role_name_type)
...
let make_create_policy_version_request ?set_as_default:(set_as_default_ : boolean_type option)
    ~policy_arn:(policy_arn_ : arn_type) ~policy_document:(policy_document_ : policy_document_type)
    () =
  ({
     policy_arn = policy_arn_;
...
let make_create_policy_request ?path:(path_ : policy_path_type option)
    ?description:(description_ : policy_description_type option)
    ?tags:(tags_ : tag_list_type option) ~policy_name:(policy_name_ : policy_name_type)
    ~policy_document:(policy_document_ : policy_document_type) () =
  ({
     policy_name = policy_name_;
     path = path_;
...
let make_create_open_id_connect_provider_response
    ?open_id_connect_provider_arn:(open_id_connect_provider_arn_ : arn_type option)
    ?tags:(tags_ : tag_list_type option) () =
  ({ open_id_connect_provider_arn = open_id_connect_provider_arn_; tags = tags_ }
    : create_open_id_connect_provider_response)

let make_create_open_id_connect_provider_request
    ?client_id_list:(client_id_list_ : client_id_list_type option)
    ?thumbprint_list:(thumbprint_list_ : thumbprint_list_type option)
    ?tags:(tags_ : tag_list_type option) ~url:(url_ : open_id_connect_provider_url_type) () =
  ({
     url = url_;
     client_id_list = client_id_list_;
     thumbprint_list = thumbprint_list_;
...
let make_create_login_profile_request ?user_name:(user_name_ : user_name_type option)
...
let make_create_instance_profile_request ?path:(path_ : path_type option)
    ?tags:(tags_ : tag_list_type option)
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type) () =
  ({ instance_profile_name = instance_profile_name_; path = path_; tags = tags_ }
...
let make_create_group_request ?path:(path_ : path_type option)
    ~group_name:(group_name_ : group_name_type) () =
  ({ path = path_; group_name = group_name_ } : create_group_request)

let make_create_delegation_request_response
    ?console_deep_link:(console_deep_link_ : console_deep_link_type option)
    ?delegation_request_id:(delegation_request_id_ : delegation_request_id_type option) () =
  ({ console_deep_link = console_deep_link_; delegation_request_id = delegation_request_id_ }
...
let make_access_key ?create_date:(create_date_ : date_type option)
    ~user_name:(user_name_ : user_name_type) ~access_key_id:(access_key_id_ : access_key_id_type)
    ~status:(status_ : status_type) ~secret_access_key:(secret_access_key_ : access_key_secret_type)
    () =
  ({
     user_name = user_name_;
     access_key_id = access_key_id_;
...
let make_change_password_request ~old_password:(old_password_ : password_type)
    ~new_password:(new_password_ : password_type) () =
  ({ old_password = old_password_; new_password = new_password_ } : change_password_request)

let make_attach_user_policy_request ~user_name:(user_name_ : user_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ user_name = user_name_; policy_arn = policy_arn_ } : attach_user_policy_request)

let make_attach_role_policy_request ~role_name:(role_name_ : role_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ role_name = role_name_; policy_arn = policy_arn_ } : attach_role_policy_request)

let make_attach_group_policy_request ~group_name:(group_name_ : group_name_type)
    ~policy_arn:(policy_arn_ : arn_type) () =
  ({ group_name = group_name_; policy_arn = policy_arn_ } : attach_group_policy_request)
...
let make_add_user_to_group_request ~group_name:(group_name_ : group_name_type)
    ~user_name:(user_name_ : existing_user_name_type) () =
  ({ group_name = group_name_; user_name = user_name_ } : add_user_to_group_request)

let make_add_role_to_instance_profile_request
    ~instance_profile_name:(instance_profile_name_ : instance_profile_name_type)
    ~role_name:(role_name_ : role_name_type) () =
  ({ instance_profile_name = instance_profile_name_; role_name = role_name_ }
