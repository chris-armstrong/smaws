  ...
  let _res : sync_configuration =
    {
      ...
      sync_type =
        (value_for_key sync_configuration_type_of_yojson "SyncType" _list
           path);
  ...
  let _res : update_sync_configuration_input =
    {
      ...
      config_file =
        (option_of_yojson
           ...
           path);
      branch =
        (option_of_yojson (value_for_key branch_name_of_yojson "Branch")
           _list path)
    } in
  _res
...
let sync_blocker_context_of_yojson tree path =
  ...
  let _res : sync_blocker_context =
    {
      value =
        (value_for_key sync_blocker_context_value_of_yojson "Value" _list
           path);
      key =
  ...
  let _res : repository_link_info =
    {
      ...
      repository_link_id =
        (value_for_key repository_link_id_of_yojson "RepositoryLinkId" _list
           path);
      repository_link_arn =
        (value_for_key repository_link_arn_of_yojson "RepositoryLinkArn"
           _list path);
      provider_type =
        (value_for_key provider_type_of_yojson "ProviderType" _list path);
      owner_id = (value_for_key owner_id_of_yojson "OwnerId" _list path);
      encryption_key_arn =
        (option_of_yojson
           (value_for_key kms_key_arn_of_yojson "EncryptionKeyArn") _list
           path);
      connection_arn =
  ...
  let _res : resource_sync_attempt =
    {
      ...
      started_at =
        (value_for_key timestamp__of_yojson "StartedAt" _list path);
      initial_revision =
  ...
  let _res : host =
    {
      status_message =
        (option_of_yojson
           (value_for_key host_status_message_of_yojson "StatusMessage")
           _list path);
      status =
        (option_of_yojson (value_for_key host_status_of_yojson "Status")
           _list path);
      vpc_configuration =
        (option_of_yojson
           (value_for_key vpc_configuration_of_yojson "VpcConfiguration")
