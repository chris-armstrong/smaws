...
let make_vpc_configuration
  ?tls_certificate:(tls_certificate_ : tls_certificate option)
...
let make_sync_configuration
  ...
  ~owner_id:(owner_id_ : owner_id) ~branch:(branch_ : branch_name) () =
  ({
...
let make_update_sync_configuration_input
  ...
  ?config_file:(config_file_ : deployment_file_path option)
  ?branch:(branch_ : branch_name option)
  ~sync_type:(sync_type_ : sync_configuration_type)
  ~resource_name:(resource_name_ : resource_name) () =
  ({
     pull_request_comment = pull_request_comment_;
...
let make_sync_blocker ?resolved_at:(resolved_at_ : timestamp option)
  ...
  ?contexts:(contexts_ : sync_blocker_context_list option)
  ~created_at:(created_at_ : timestamp)
  ~created_reason:(created_reason_ : created_reason)
...
let make_revision ~sha:(sha_ : sh_a)
  ~provider_type:(provider_type_ : provider_type)
  ~repository_name:(repository_name_ : repository_name)
  ~owner_id:(owner_id_ : owner_id) ~directory:(directory_ : directory)
...
let make_resource_sync_attempt ~target:(target_ : target)
  ...
  ~started_at:(started_at_ : timestamp)
...
let make_repository_sync_event
  ?external_id:(external_id_ : external_id option) ~type_:(type__ : type_)
  ~time:(time_ : timestamp) ~event:(event_ : event) () =
  ({ type_ = type__; time = time_; external_id = external_id_; event = event_
   } : repository_sync_event)
let make_repository_sync_definition ~target:(target_ : target)
  ~parent:(parent_ : parent) ~directory:(directory_ : directory)
  ~branch:(branch_ : branch_name) () =
  ({
     target = target_;
     parent = parent_;
...
let make_repository_sync_attempt
  ~events:(events_ : repository_sync_event_list)
...
let make_list_repository_sync_definitions_output
  ...
  ~repository_sync_definitions:(repository_sync_definitions_ :
                                 repository_sync_definition_list)
  () =
  ({
     next_token = next_token_;
     repository_sync_definitions = repository_sync_definitions_
...
let make_host ?status_message:(status_message_ : host_status_message option)
  ...
  ?host_arn:(host_arn_ : host_arn option) ?name:(name_ : host_name option) ()
  =
  ({
...
let make_get_host_output
  ...
  ?provider_endpoint:(provider_endpoint_ : url option)
  ?provider_type:(provider_type_ : provider_type option)
  ?status:(status_ : host_status option) ?name:(name_ : host_name option) ()
  =
  ({
     vpc_configuration = vpc_configuration_;
     provider_endpoint = provider_endpoint_;
...
let make_delete_connection_input
  ~connection_arn:(connection_arn_ : connection_arn) () =
...
let make_create_sync_configuration_input
  ...
  ~resource_name:(resource_name_ : resource_name)
  ~repository_link_id:(repository_link_id_ : repository_link_id)
  ~config_file:(config_file_ : deployment_file_path)
  ~branch:(branch_ : branch_name) () =
  ({
     pull_request_comment = pull_request_comment_;
     trigger_resource_update_on = trigger_resource_update_on_;
...
let make_create_host_output ?tags:(tags_ : tag_list option)
...
let make_create_host_input ?tags:(tags_ : tag_list option)
  ?vpc_configuration:(vpc_configuration_ : vpc_configuration option)
  ~provider_endpoint:(provider_endpoint_ : url)
  ~provider_type:(provider_type_ : provider_type) ~name:(name_ : host_name)
  () =
  ({
     tags = tags_;
     vpc_configuration = vpc_configuration_;
