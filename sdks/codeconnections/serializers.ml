...
let trigger_resource_update_on_to_yojson (x : trigger_resource_update_on) =
  match x with FILE_CHANGE -> `String "FILE_CHANGE" | ANY_CHANGE -> `String "ANY_CHANGE"
...
let sync_configuration_to_yojson (x : sync_configuration) =
  assoc_to_yojson
    [
      ("PullRequestComment", option_to_yojson pull_request_comment_to_yojson x.pull_request_comment);
