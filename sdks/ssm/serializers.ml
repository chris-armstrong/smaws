...
let patch_compliance_status_to_yojson (x : patch_compliance_status) =
  match x with NonCompliant -> `String "NON_COMPLIANT" | Compliant -> `String "COMPLIANT"
...
let ops_item_data_type_to_yojson (x : ops_item_data_type) =
  match x with STRING -> `String "String" | SEARCHABLE_STRING -> `String "SearchableString"
...
let notification_type_to_yojson (x : notification_type) =
  match x with Invocation -> `String "Invocation" | Command -> `String "Command"
...
let maintenance_window_task_cutoff_behavior_to_yojson (x : maintenance_window_task_cutoff_behavior)
    =
  match x with CancelTask -> `String "CANCEL_TASK" | ContinueTask -> `String "CONTINUE_TASK"
...
let document_parameter_type_to_yojson (x : document_parameter_type) =
  match x with StringList -> `String "StringList" | String -> `String "String"
...
let platform_type_to_yojson (x : platform_type) =
  match x with MACOS -> `String "MacOS" | LINUX -> `String "Linux" | WINDOWS -> `String "Windows"
...
let document_format_to_yojson (x : document_format) =
  match x with TEXT -> `String "TEXT" | JSON -> `String "JSON" | YAML -> `String "YAML"
...
let step_preview_map_to_yojson tree = map_to_yojson impact_type_to_yojson integer__to_yojson tree
...
let execution_mode_to_yojson (x : execution_mode) =
  match x with Interactive -> `String "Interactive" | Auto -> `String "Auto"
...
let access_type_to_yojson (x : access_type) =
  match x with JUSTINTIME -> `String "JustInTime" | STANDARD -> `String "Standard"
...
let maintenance_window_resource_type_to_yojson (x : maintenance_window_resource_type) =
  match x with ResourceGroup -> `String "RESOURCE_GROUP" | Instance -> `String "INSTANCE"
...
let reboot_option_to_yojson (x : reboot_option) =
  match x with NO_REBOOT -> `String "NoReboot" | REBOOT_IF_NEEDED -> `String "RebootIfNeeded"
...
let compliance_upload_type_to_yojson (x : compliance_upload_type) =
  match x with Partial -> `String "PARTIAL" | Complete -> `String "COMPLETE"
...
let progress_counters_to_yojson (x : progress_counters) =
  assoc_to_yojson
    [
      ("TimedOutSteps", option_to_yojson integer__to_yojson x.timed_out_steps);
      ("CancelledSteps", option_to_yojson integer__to_yojson x.cancelled_steps);
...
let parameters_filter_key_to_yojson (x : parameters_filter_key) =
  match x with KEY_ID -> `String "KeyId" | TYPE -> `String "Type" | NAME -> `String "Name"
...
and ops_aggregator_list_to_yojson tree = list_to_yojson ops_aggregator_to_yojson tree
...
and node_aggregator_list_to_yojson tree = list_to_yojson node_aggregator_to_yojson tree
...
let inventory_schema_delete_option_to_yojson (x : inventory_schema_delete_option) =
  match x with DELETE_SCHEMA -> `String "DeleteSchema" | DISABLE_SCHEMA -> `String "DisableSchema"
...
and inventory_aggregator_list_to_yojson tree = list_to_yojson inventory_aggregator_to_yojson tree
...
let automation_execution_preview_to_yojson (x : automation_execution_preview) =
  assoc_to_yojson
    [
      ("TotalAccounts", option_to_yojson integer__to_yojson x.total_accounts);
...
let connection_status_to_yojson (x : connection_status) =
  match x with NOT_CONNECTED -> `String "notconnected" | CONNECTED -> `String "connected"
...
let automation_subtype_to_yojson (x : automation_subtype) =
  match x with AccessRequest -> `String "AccessRequest" | ChangeRequest -> `String "ChangeRequest"
  ...
  assoc_to_yojson
    [
      ( "InstancesWithAvailableSecurityUpdates",
        option_to_yojson integer__to_yojson x.instances_with_available_security_updates );
...
let automation_type_to_yojson (x : automation_type) =
  match x with Local -> `String "Local" | CrossAccount -> `String "CrossAccount"
