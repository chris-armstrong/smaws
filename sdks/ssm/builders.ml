...
let make_update_service_setting_request ~setting_value:(setting_value_ : service_setting_value)
...
let make_update_resource_data_sync_request ~sync_source:(sync_source_ : resource_data_sync_source)
    ~sync_type:(sync_type_ : resource_data_sync_type)
    ~sync_name:(sync_name_ : resource_data_sync_name) () =
  ({ sync_source = sync_source_; sync_type = sync_type_; sync_name = sync_name_ }
    : update_resource_data_sync_request)

let make_patch_filter ~values:(values_ : patch_filter_value_list) ~key:(key_ : patch_filter_key) ()
    =
...
let make_patch_rule ?enable_non_security:(enable_non_security_ : boolean_ option)
...
let make_patch_source ~configuration:(configuration_ : patch_source_configuration)
    ~products:(products_ : patch_source_product_list) ~name:(name_ : patch_source_name) () =
  ({ configuration = configuration_; products = products_; name = name_ } : patch_source)
...
let make_update_patch_baseline_request ?replace:(replace_ : boolean_ option)
    ...
    ?name:(name_ : baseline_name option) ~baseline_id:(baseline_id_ : baseline_id) () =
  ({
     replace = replace_;
...
let make_update_ops_item_request ?ops_item_arn:(ops_item_arn_ : ops_item_arn option)
    ...
    ?category:(category_ : ops_item_category option) ?title:(title_ : ops_item_title option)
...
let make_update_managed_instance_role_request ~iam_role:(iam_role_ : iam_role)
...
let make_target ?values:(values_ : target_values option) ?key:(key_ : target_key option) () =
...
let make_notification_config ?notification_type:(notification_type_ : notification_type option)
...
let make_maintenance_window_lambda_parameters
  ...
  ({ payload = payload_; qualifier = qualifier_; client_context = client_context_ }
...
let make_logging_info ?s3_key_prefix:(s3_key_prefix_ : s3_key_prefix option)
    ~s3_region:(s3_region_ : s3_region) ~s3_bucket_name:(s3_bucket_name_ : s3_bucket_name) () =
  ({ s3_region = s3_region_; s3_key_prefix = s3_key_prefix_; s3_bucket_name = s3_bucket_name_ }
    : logging_info)
...
let make_update_maintenance_window_task_request
    ...
    ?service_role_arn:(service_role_arn_ : service_role option)
    ?task_arn:(task_arn_ : maintenance_window_task_arn option) ?targets:(targets_ : targets option)
...
let make_update_maintenance_window_target_request ?replace:(replace_ : boolean_ option)
    ?description:(description_ : maintenance_window_description option)
    ?name:(name_ : maintenance_window_name option)
...
let make_update_maintenance_window_request ?replace:(replace_ : boolean_ option)
    ...
    ?description:(description_ : maintenance_window_description option)
    ?name:(name_ : maintenance_window_name option) ~window_id:(window_id_ : maintenance_window_id)
    () =
  ({
     replace = replace_;
     enabled = enabled_;
...
let make_document_parameter
    ...
    ?type_:(type__ : document_parameter_type option) ?name:(name_ : document_parameter_name option)
    () =
  ({ default_value = default_value_; description = description_; type_ = type__; name = name_ }
    : document_parameter)
...
let make_document_requires ?version_name:(version_name_ : document_version_name option)
    ?require_type:(require_type_ : require_type option)
    ?version:(version_ : document_version option) ~name:(name_ : document_ar_n) () =
  ({ version_name = version_name_; require_type = require_type_; version = version_; name = name_ }
    : document_requires)

let make_review_information ?reviewer:(reviewer_ : reviewer option)
    ?status:(status_ : review_status option) ?reviewed_time:(reviewed_time_ : date_time option) () =
  ({ reviewer = reviewer_; status = status_; reviewed_time = reviewed_time_ } : review_information)

let make_document_description ?category_enum:(category_enum_ : category_enum_list option)
    ...
    ?author:(author_ : document_author option) ?requires:(requires_ : document_requires_list option)
    ?attachments_information:(attachments_information_ : attachment_information_list option)
    ?tags:(tags_ : tag_list option) ?target_type:(target_type_ : target_type option)
    ...
    ?status:(status_ : document_status option) ?created_date:(created_date_ : date_time option)
    ...
    ?name:(name_ : document_ar_n option) ?hash_type:(hash_type_ : document_hash_type option)
...
let make_attachments_source ?name:(name_ : attachment_identifier option)
    ?values:(values_ : attachments_source_values option) ?key:(key_ : attachments_source_key option)
    () =
  ({ name = name_; values = values_; key = key_ } : attachments_source)

let make_update_document_request ?target_type:(target_type_ : target_type option)
    ?document_format:(document_format_ : document_format option)
    ?document_version:(document_version_ : document_version option)
    ?version_name:(version_name_ : document_version_name option)
    ?display_name:(display_name_ : document_display_name option)
    ?attachments:(attachments_ : attachments_source_list option) ~name:(name_ : document_name)
...
let make_document_review_comment_source ?content:(content_ : document_review_comment option)
...
let make_update_document_metadata_request
    ?document_version:(document_version_ : document_version option)
    ~document_reviews:(document_reviews_ : document_reviews) ~name:(name_ : document_name) () =
  ({ document_reviews = document_reviews_; document_version = document_version_; name = name_ }
...
let make_update_document_default_version_request
    ~document_version:(document_version_ : document_version_number) ~name:(name_ : document_name) ()
    =
...
let make_association_status ?additional_info:(additional_info_ : status_additional_info option)
    ~message:(message_ : status_message) ~name:(name_ : association_status_name)
    ~date:(date_ : date_time) () =
  ({ additional_info = additional_info_; message = message_; name = name_; date = date_ }
    : association_status)
...
let make_association_overview
    ...
    ?detailed_status:(detailed_status_ : status_name option) ?status:(status_ : status_name option)
    () =
  ({
...
let make_s3_output_location ?output_s3_key_prefix:(output_s3_key_prefix_ : s3_key_prefix option)
    ?output_s3_bucket_name:(output_s3_bucket_name_ : s3_bucket_name option)
...
let make_target_location ?targets_max_errors:(targets_max_errors_ : max_errors option)
    ...
    ?regions:(regions_ : regions option) ?accounts:(accounts_ : accounts option) () =
  ({
...
let make_alarm_state_information ~state:(state_ : external_alarm_state) ~name:(name_ : alarm_name)
    () =
...
let make_association_description
    ...
    ?targets:(targets_ : targets option) ?association_id:(association_id_ : association_id option)
    ?parameters:(parameters_ : parameters option)
...
let make_update_association_status_request
  ...
  ({ association_status = association_status_; instance_id = instance_id_; name = name_ }
...
let make_unlabel_parameter_version_request ~labels:(labels_ : parameter_label_list)
    ~parameter_version:(parameter_version_ : ps_parameter_version) ~name:(name_ : ps_parameter_name)
    () =
...
let make_failure_details ?details:(details_ : automation_parameter_map option)
    ?failure_type:(failure_type_ : string_ option) ?failure_stage:(failure_stage_ : string_ option)
    () =
  ({ details = details_; failure_type = failure_type_; failure_stage = failure_stage_ }
    : failure_details)

let make_parent_step_details ?iterator_value:(iterator_value_ : string_ option)
    ?iteration:(iteration_ : integer option) ?action:(action_ : automation_action_name option)
...
let make_step_execution ?parent_step_details:(parent_step_details_ : parent_step_details option)
    ?triggered_alarms:(triggered_alarms_ : alarm_state_information_list option)
    ?target_location:(target_location_ : target_location option)
    ?targets:(targets_ : targets option)
    ?valid_next_steps:(valid_next_steps_ : valid_next_step_list option)
    ?is_critical:(is_critical_ : boolean_ option) ?next_step:(next_step_ : string_ option)
    ...
    ?failure_message:(failure_message_ : string_ option) ?response:(response_ : string_ option)
    ?outputs:(outputs_ : automation_parameter_map option)
    ?inputs:(inputs_ : normal_string_map option) ?response_code:(response_code_ : string_ option)
    ...
    ?max_attempts:(max_attempts_ : integer option) ?on_failure:(on_failure_ : string_ option)
    ?timeout_seconds:(timeout_seconds_ : long option)
    ?action:(action_ : automation_action_name option) ?step_name:(step_name_ : string_ option) () =
  ({
...
let make_step_execution_filter ~values:(values_ : step_execution_filter_value_list)
...
let make_start_session_response ?stream_url:(stream_url_ : stream_url option)
    ?token_value:(token_value_ : token_value option) ?session_id:(session_id_ : session_id option)
    () =
  ({ stream_url = stream_url_; token_value = token_value_; session_id = session_id_ }
    : start_session_response)

let make_start_session_request ?parameters:(parameters_ : session_manager_parameters option)
    ?reason:(reason_ : session_reason option) ?document_name:(document_name_ : document_ar_n option)
    ~target:(target_ : session_target) () =
  ({ parameters = parameters_; reason = reason_; document_name = document_name_; target = target_ }
...
let make_automation_execution_inputs
    ...
    ?target_locations:(target_locations_ : target_locations option)
    ?target_maps:(target_maps_ : target_maps option) ?targets:(targets_ : targets option)
...
let make_runbook ?target_locations:(target_locations_ : target_locations option)
    ?max_errors:(max_errors_ : max_errors option)
    ?max_concurrency:(max_concurrency_ : max_concurrency option)
    ?target_maps:(target_maps_ : target_maps option) ?targets:(targets_ : targets option)
    ?target_parameter_name:(target_parameter_name_ : automation_parameter_key option)
...
let make_start_change_request_execution_request
    ...
    ?scheduled_end_time:(scheduled_end_time_ : date_time option) ?tags:(tags_ : tag_list option)
    ...
    ?parameters:(parameters_ : automation_parameter_map option)
    ?document_version:(document_version_ : document_version option)
    ?scheduled_time:(scheduled_time_ : date_time option) ~runbooks:(runbooks_ : runbooks)
    ~document_name:(document_name_ : document_ar_n) () =
  ({
...
let make_start_automation_execution_request
    ?target_locations_ur_l:(target_locations_ur_l_ : target_locations_ur_l option)
    ?alarm_configuration:(alarm_configuration_ : alarm_configuration option)
    ?tags:(tags_ : tag_list option) ?target_locations:(target_locations_ : target_locations option)
    ?max_errors:(max_errors_ : max_errors option)
    ?max_concurrency:(max_concurrency_ : max_concurrency option)
    ?target_maps:(target_maps_ : target_maps option) ?targets:(targets_ : targets option)
    ?target_parameter_name:(target_parameter_name_ : automation_parameter_key option)
    ?mode:(mode_ : execution_mode option) ?client_token:(client_token_ : idempotency_token option)
    ?parameters:(parameters_ : automation_parameter_map option)
    ?document_version:(document_version_ : document_version option)
    ~document_name:(document_name_ : document_ar_n) () =
  ({
...
let make_start_access_request_request ?tags:(tags_ : tag_list option) ~targets:(targets_ : targets)
    ~reason:(reason_ : string1to256) () =
  ({ tags = tags_; targets = targets_; reason = reason_ } : start_access_request_request)

let make_severity_summary ?unspecified_count:(unspecified_count_ : compliance_summary_count option)
...
let make_session_manager_output_url
  ...
  ({ cloud_watch_output_url = cloud_watch_output_url_; s3_output_url = s3_output_url_ }
...
let make_session ?access_type:(access_type_ : access_type option)
    ...
    ?details:(details_ : session_details option) ?reason:(reason_ : session_reason option)
    ?owner:(owner_ : session_owner option) ?document_name:(document_name_ : document_name option)
    ?end_date:(end_date_ : date_time option) ?start_date:(start_date_ : date_time option)
    ?status:(status_ : session_status option) ?target:(target_ : session_target option)
...
let make_session_filter ~value:(value_ : session_filter_value) ~key:(key_ : session_filter_key) () =
  ({ value = value_; key = key_ } : session_filter)

let make_service_setting ?status:(status_ : string_ option) ?ar_n:(ar_n_ : string_ option)
...
let make_command ?triggered_alarms:(triggered_alarms_ : alarm_state_information_list option)
    ?alarm_configuration:(alarm_configuration_ : alarm_configuration option)
    ?timeout_seconds:(timeout_seconds_ : timeout_seconds option)
    ?cloud_watch_output_config:(cloud_watch_output_config_ : cloud_watch_output_config option)
    ?notification_config:(notification_config_ : notification_config option)
    ...
    ?targets:(targets_ : targets option) ?instance_ids:(instance_ids_ : instance_id_list option)
    ?parameters:(parameters_ : parameters option) ?expires_after:(expires_after_ : date_time option)
    ?comment:(comment_ : comment option)
    ?document_version:(document_version_ : document_version option)
...
let make_send_command_request
    ...
    ?output_s3_bucket_name:(output_s3_bucket_name_ : s3_bucket_name option)
    ?output_s3_region:(output_s3_region_ : s3_region option)
    ?parameters:(parameters_ : parameters option) ?comment:(comment_ : comment option)
    ?timeout_seconds:(timeout_seconds_ : timeout_seconds option)
    ?document_hash_type:(document_hash_type_ : document_hash_type option)
    ?document_hash:(document_hash_ : document_hash option)
    ?document_version:(document_version_ : document_version option)
...
let make_resume_session_response ?stream_url:(stream_url_ : stream_url option)
    ?token_value:(token_value_ : token_value option) ?session_id:(session_id_ : session_id option)
    () =
  ({ stream_url = stream_url_; token_value = token_value_; session_id = session_id_ }
...
let make_compliant_summary ?severity_summary:(severity_summary_ : severity_summary option)
    ?compliant_count:(compliant_count_ : compliance_summary_count option) () =
  ({ severity_summary = severity_summary_; compliant_count = compliant_count_ } : compliant_summary)

let make_non_compliant_summary ?severity_summary:(severity_summary_ : severity_summary option)
    ?non_compliant_count:(non_compliant_count_ : compliance_summary_count option) () =
  ({ severity_summary = severity_summary_; non_compliant_count = non_compliant_count_ }
...
let make_resolved_targets ?truncated:(truncated_ : boolean_ option)
  ...
  ({ truncated = truncated_; parameter_values = parameter_values_ } : resolved_targets)
...
let make_registration_metadata_item ~value:(value_ : registration_metadata_value)
...
let make_register_task_with_maintenance_window_request
    ...
    ?task_invocation_parameters:
      (task_invocation_parameters_ : maintenance_window_task_invocation_parameters option)
    ?task_parameters:(task_parameters_ : maintenance_window_task_parameters option)
    ?service_role_arn:(service_role_arn_ : service_role option) ?targets:(targets_ : targets option)
...
let make_register_target_with_maintenance_window_request
    ...
    ?description:(description_ : maintenance_window_description option)
    ?name:(name_ : maintenance_window_name option)
    ?owner_information:(owner_information_ : owner_information option) ~targets:(targets_ : targets)
...
let make_register_patch_baseline_for_patch_group_request ~patch_group:(patch_group_ : patch_group)
    ~baseline_id:(baseline_id_ : baseline_id) () =
  ({ patch_group = patch_group_; baseline_id = baseline_id_ }
...
let make_put_resource_policy_response ?policy_hash:(policy_hash_ : policy_hash option)
    ?policy_id:(policy_id_ : policy_id option) () =
  ({ policy_hash = policy_hash_; policy_id = policy_id_ } : put_resource_policy_response)

let make_put_resource_policy_request ?policy_hash:(policy_hash_ : policy_hash option)
...
let make_put_parameter_request ?data_type:(data_type_ : parameter_data_type option)
    ?policies:(policies_ : parameter_policies option) ?tier:(tier_ : parameter_tier option)
    ?tags:(tags_ : tag_list option) ?allowed_pattern:(allowed_pattern_ : allowed_pattern option)
    ?overwrite:(overwrite_ : boolean_ option) ?key_id:(key_id_ : parameter_key_id option)
    ?type_:(type__ : parameter_type option)
    ?description:(description_ : parameter_description option) ~value:(value_ : ps_parameter_value)
...
let make_inventory_item ?context:(context_ : inventory_item_content_context option)
...
let make_compliance_item_entry ?details:(details_ : compliance_item_details option)
    ?title:(title_ : compliance_item_title option) ?id:(id_ : compliance_item_id option)
    ~status:(status_ : compliance_status) ~severity:(severity_ : compliance_severity) () =
  ({ details = details_; status = status_; severity = severity_; title = title_; id = id_ }
    : compliance_item_entry)

let make_put_compliance_items_request ?upload_type:(upload_type_ : compliance_upload_type option)
...
let make_progress_counters ?timed_out_steps:(timed_out_steps_ : integer option)
    ?cancelled_steps:(cancelled_steps_ : integer option)
    ?failed_steps:(failed_steps_ : integer option) ?success_steps:(success_steps_ : integer option)
...
let make_patch_orchestrator_filter ?values:(values_ : patch_orchestrator_filter_values option)
...
let make_patch ?repository:(repository_ : patch_repository option)
    ?severity:(severity_ : patch_severity option) ?arch:(arch_ : patch_arch option)
    ?release:(release_ : patch_release option) ?version:(version_ : patch_version option)
    ...
    ?vendor:(vendor_ : patch_vendor option) ?content_url:(content_url_ : patch_content_url option)
    ?description:(description_ : patch_description option) ?title:(title_ : patch_title option)
    ?release_date:(release_date_ : date_time option) ?id:(id_ : patch_id option) () =
  ({
...
let make_patch_baseline_identity ?default_baseline:(default_baseline_ : default_baseline option)
...
let make_patch_compliance_data ?cve_ids:(cve_ids_ : patch_cve_ids option)
    ~installed_time:(installed_time_ : date_time) ~state:(state_ : patch_compliance_data_state)
    ~severity:(severity_ : patch_severity) ~classification:(classification_ : patch_classification)
...
let make_parameter_string_filter ?values:(values_ : parameter_string_filter_value_list option)
...
let make_parameter_inline_policy ?policy_status:(policy_status_ : string_ option)
    ?policy_type:(policy_type_ : string_ option) ?policy_text:(policy_text_ : string_ option) () =
  ({ policy_status = policy_status_; policy_type = policy_type_; policy_text = policy_text_ }
    : parameter_inline_policy)

let make_parameter_metadata ?data_type:(data_type_ : parameter_data_type option)
    ?policies:(policies_ : parameter_policy_list option) ?tier:(tier_ : parameter_tier option)
    ...
    ?last_modified_user:(last_modified_user_ : string_ option)
    ?last_modified_date:(last_modified_date_ : date_time option)
    ?key_id:(key_id_ : parameter_key_id option) ?type_:(type__ : parameter_type option)
    ?ar_n:(ar_n_ : string_ option) ?name:(name_ : ps_parameter_name option) () =
  ({
     data_type = data_type_;
     policies = policies_;
     tier = tier_;
...
let make_parameter ?data_type:(data_type_ : parameter_data_type option)
    ?ar_n:(ar_n_ : string_ option) ?last_modified_date:(last_modified_date_ : date_time option)
    ?source_result:(source_result_ : string_ option)
    ?selector:(selector_ : ps_parameter_selector option)
    ?version:(version_ : ps_parameter_version option) ?value:(value_ : ps_parameter_value option)
    ?type_:(type__ : parameter_type option) ?name:(name_ : ps_parameter_name option) () =
  ({
     data_type = data_type_;
     ar_n = ar_n_;
     last_modified_date = last_modified_date_;
...
let make_parameter_history ?data_type:(data_type_ : parameter_data_type option)
    ?policies:(policies_ : parameter_policy_list option) ?tier:(tier_ : parameter_tier option)
...
let make_output_source ?output_source_type:(output_source_type_ : output_source_type option)
    ?output_source_id:(output_source_id_ : output_source_id option) () =
  ({ output_source_type = output_source_type_; output_source_id = output_source_id_ }
    : output_source)
...
let make_ops_metadata_filter ~values:(values_ : ops_metadata_filter_value_list)
    ~key:(key_ : ops_metadata_filter_key) () =
  ({ values = values_; key = key_ } : ops_metadata_filter)

let make_ops_item_summary ?planned_end_time:(planned_end_time_ : date_time option)
    ?planned_start_time:(planned_start_time_ : date_time option)
    ?actual_end_time:(actual_end_time_ : date_time option)
    ?actual_start_time:(actual_start_time_ : date_time option)
    ...
    ?operational_data:(operational_data_ : ops_item_operational_data option)
    ?title:(title_ : ops_item_title option) ?ops_item_id:(ops_item_id_ : ops_item_id option)
    ?status:(status_ : ops_item_status option) ?source:(source_ : ops_item_source option)
    ?priority:(priority_ : ops_item_priority option)
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?last_modified_by:(last_modified_by_ : string_ option)
    ?created_time:(created_time_ : date_time option) ?created_by:(created_by_ : string_ option) () =
  ({
     planned_end_time = planned_end_time_;
     planned_start_time = planned_start_time_;
     actual_end_time = actual_end_time_;
...
let make_ops_item_related_item_summary ?last_modified_time:(last_modified_time_ : date_time option)
...
let make_ops_item_filter ~operator:(operator_ : ops_item_filter_operator)
    ~values:(values_ : ops_item_filter_values) ~key:(key_ : ops_item_filter_key) () =
  ({ operator = operator_; values = values_; key = key_ } : ops_item_filter)

let make_ops_item_event_summary ?created_time:(created_time_ : date_time option)
    ?created_by:(created_by_ : ops_item_identity option) ?detail:(detail_ : string_ option)
    ?detail_type:(detail_type_ : string_ option) ?source:(source_ : string_ option)
    ?event_id:(event_id_ : string_ option) ?ops_item_id:(ops_item_id_ : string_ option) () =
  ({
     created_time = created_time_;
     created_by = created_by_;
     detail = detail_;
...
let make_ops_item_event_filter ~operator:(operator_ : ops_item_event_filter_operator)
    ~values:(values_ : ops_item_event_filter_values) ~key:(key_ : ops_item_event_filter_key) () =
...
let make_ops_item ?ops_item_arn:(ops_item_arn_ : ops_item_arn option)
    ...
    ?category:(category_ : ops_item_category option)
    ?operational_data:(operational_data_ : ops_item_operational_data option)
    ?source:(source_ : ops_item_source option) ?title:(title_ : ops_item_title option)
    ?version:(version_ : string_ option) ?ops_item_id:(ops_item_id_ : ops_item_id option)
    ?status:(status_ : ops_item_status option)
    ?related_ops_items:(related_ops_items_ : related_ops_items option)
    ?priority:(priority_ : ops_item_priority option)
    ?notifications:(notifications_ : ops_item_notifications option)
...
let make_ops_entity_item ?content:(content_ : ops_entity_item_entry_list option)
...
let make_ops_entity ?data:(data_ : ops_entity_item_map option) ?id:(id_ : ops_entity_id option) () =
  ({ data = data_; id = id_ } : ops_entity)

let make_ops_aggregator ?aggregators:(aggregators_ : ops_aggregator_list option)
    ?filters:(filters_ : ops_filter_list option) ?values:(values_ : ops_aggregator_value_map option)
...
let make_node ?node_type:(node_type_ : node_type option) ?region:(region_ : node_region option)
...
let make_node_aggregator ?aggregators:(aggregators_ : node_aggregator_list option)
    ~attribute_name:(attribute_name_ : node_attribute_name) ~type_name:(type_name_ : node_type_name)
...
let make_modify_document_permission_request
    ...
    ~permission_type:(permission_type_ : document_permission_type) ~name:(name_ : document_name) ()
    =
  ({
...
let make_maintenance_window_identity_for_target ?name:(name_ : maintenance_window_name option)
    ?window_id:(window_id_ : maintenance_window_id option) () =
...
let make_maintenance_window_task
    ...
    ?cutoff_behavior:(cutoff_behavior_ : maintenance_window_task_cutoff_behavior option)
    ?description:(description_ : maintenance_window_description option)
    ?name:(name_ : maintenance_window_name option) ?max_errors:(max_errors_ : max_errors option)
    ?max_concurrency:(max_concurrency_ : max_concurrency option)
    ?service_role_arn:(service_role_arn_ : service_role option)
    ?logging_info:(logging_info_ : logging_info option)
    ?priority:(priority_ : maintenance_window_task_priority option)
    ?task_parameters:(task_parameters_ : maintenance_window_task_parameters option)
    ?targets:(targets_ : targets option) ?type_:(type__ : maintenance_window_task_type option)
...
let make_maintenance_window_filter ?values:(values_ : maintenance_window_filter_values option)
...
let make_maintenance_window_execution ?end_time:(end_time_ : date_time option)
...
let make_list_tags_for_resource_request ~resource_id:(resource_id_ : resource_id)
    ~resource_type:(resource_type_ : resource_type_for_tagging) () =
  ({ resource_id = resource_id_; resource_type = resource_type_ } : list_tags_for_resource_request)

let make_list_resource_data_sync_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
    ?sync_type:(sync_type_ : resource_data_sync_type option) () =
  ({ max_results = max_results_; next_token = next_token_; sync_type = sync_type_ }
    : list_resource_data_sync_request)

let make_compliance_string_filter ?type_:(type__ : compliance_query_operator_type option)
...
let make_list_resource_compliance_summaries_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_list_ops_metadata_request ?next_token:(next_token_ : next_token option)
...
let make_list_ops_item_related_items_request ?next_token:(next_token_ : string_ option)
...
let make_list_ops_item_events_response ?summaries:(summaries_ : ops_item_event_summaries option)
    ?next_token:(next_token_ : string_ option) () =
  ({ summaries = summaries_; next_token = next_token_ } : list_ops_item_events_response)

let make_list_ops_item_events_request ?next_token:(next_token_ : string_ option)
...
let make_list_nodes_summary_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) ?filters:(filters_ : node_filter_list option)
    ?sync_name:(sync_name_ : resource_data_sync_name option)
...
let make_inventory_filter ?type_:(type__ : inventory_query_operator_type option)
    ~values:(values_ : inventory_filter_value_list) ~key:(key_ : inventory_filter_key) () =
  ({ type_ = type__; values = values_; key = key_ } : inventory_filter)

let make_list_inventory_entries_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) ?filters:(filters_ : inventory_filter_list option)
    ~type_name:(type_name_ : inventory_item_type_name) ~instance_id:(instance_id_ : instance_id) ()
    =
  ({
     max_results = max_results_;
     next_token = next_token_;
...
let make_document_identifier ?author:(author_ : document_author option)
    ?review_status:(review_status_ : review_status option)
    ?requires:(requires_ : document_requires_list option) ?tags:(tags_ : tag_list option)
...
let make_document_filter ~value:(value_ : document_filter_value) ~key:(key_ : document_filter_key)
    () =
  ({ value = value_; key = key_ } : document_filter)

let make_document_key_values_filter ?values:(values_ : document_key_values_filter_values option)
...
let make_document_version_info ?review_status:(review_status_ : review_status option)
    ?status_information:(status_information_ : document_status_information option)
...
let make_list_document_versions_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) ~name:(name_ : document_ar_n) () =
  ({ next_token = next_token_; max_results = max_results_; name = name_ }
    : list_document_versions_request)

let make_document_reviewer_response_source ?reviewer:(reviewer_ : reviewer option)
    ?comment:(comment_ : document_review_comment_list option)
    ?review_status:(review_status_ : review_status option)
    ?updated_time:(updated_time_ : date_time option) ?create_time:(create_time_ : date_time option)
    () =
  ({
     reviewer = reviewer_;
     comment = comment_;
...
let make_list_document_metadata_history_response ?next_token:(next_token_ : next_token option)
...
let make_list_document_metadata_history_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
    ?document_version:(document_version_ : document_version option)
    ~metadata:(metadata_ : document_metadata_enum) ~name:(name_ : document_name) () =
  ({
     max_results = max_results_;
     next_token = next_token_;
     metadata = metadata_;
...
let make_list_compliance_summaries_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
    ?filters:(filters_ : compliance_string_filter_list option) () =
  ({ max_results = max_results_; next_token = next_token_; filters = filters_ }
...
let make_compliance_item ?details:(details_ : compliance_item_details option)
    ?execution_summary:(execution_summary_ : compliance_execution_summary option)
    ?severity:(severity_ : compliance_severity option) ?status:(status_ : compliance_status option)
    ?title:(title_ : compliance_item_title option) ?id:(id_ : compliance_item_id option)
    ?resource_id:(resource_id_ : compliance_resource_id option)
    ?resource_type:(resource_type_ : compliance_resource_type option)
    ?compliance_type:(compliance_type_ : compliance_type_name option) () =
...
let make_list_compliance_items_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_command_filter ~value:(value_ : command_filter_value) ~key:(key_ : command_filter_key) () =
  ({ value = value_; key = key_ } : command_filter)

let make_list_commands_request ?filters:(filters_ : command_filter_list option)
    ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : command_max_results option)
    ?instance_id:(instance_id_ : instance_id option) ?command_id:(command_id_ : command_id option)
    () =
  ({
     filters = filters_;
     next_token = next_token_;
...
let make_command_plugin ?output_s3_key_prefix:(output_s3_key_prefix_ : s3_key_prefix option)
    ?output_s3_bucket_name:(output_s3_bucket_name_ : s3_bucket_name option)
    ?output_s3_region:(output_s3_region_ : s3_region option)
    ...
    ?status_details:(status_details_ : status_details option)
    ?status:(status_ : command_plugin_status option) ?name:(name_ : command_plugin_name option) () =
  ({
     output_s3_key_prefix = output_s3_key_prefix_;
     output_s3_bucket_name = output_s3_bucket_name_;
     output_s3_region = output_s3_region_;
...
let make_command_invocation
    ...
    ?requested_date_time:(requested_date_time_ : date_time option)
    ?document_version:(document_version_ : document_version option)
    ?document_name:(document_name_ : document_name option) ?comment:(comment_ : comment option)
...
let make_list_command_invocations_request ?details:(details_ : boolean_ option)
    ?filters:(filters_ : command_filter_list option) ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : command_max_results option)
    ?instance_id:(instance_id_ : instance_id option) ?command_id:(command_id_ : command_id option)
    () =
  ({
...
let make_list_associations_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
...
let make_association_version_info ?target_maps:(target_maps_ : target_maps option)
    ...
    ?output_location:(output_location_ : instance_association_output_location option)
    ?schedule_expression:(schedule_expression_ : schedule_expression option)
    ?targets:(targets_ : targets option) ?parameters:(parameters_ : parameters option)
    ?document_version:(document_version_ : document_version option)
    ?name:(name_ : document_ar_n option) ?created_date:(created_date_ : date_time option)
    ?association_version:(association_version_ : association_version option)
    ?association_id:(association_id_ : association_id option) () =
  ({
     target_maps = target_maps_;
...
let make_list_association_versions_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
    ~association_id:(association_id_ : association_id) () =
  ({ next_token = next_token_; max_results = max_results_; association_id = association_id_ }
...
let make_inventory_result_item ?content_hash:(content_hash_ : inventory_item_content_hash option)
...
let make_inventory_result_entity ?data:(data_ : inventory_result_item_map option)
    ?id:(id_ : inventory_result_entity_id option) () =
  ({ data = data_; id = id_ } : inventory_result_entity)

let make_inventory_item_attribute ~data_type:(data_type_ : inventory_attribute_data_type)
    ~name:(name_ : inventory_item_attribute_name) () =
  ({ data_type = data_type_; name = name_ } : inventory_item_attribute)

let make_inventory_item_schema ?display_name:(display_name_ : inventory_type_display_name option)
...
let make_instance_property_filter ~value_set:(value_set_ : instance_property_filter_value_set)
...
let make_instance_property ?source_type:(source_type_ : source_type option)
    ...
    ?iam_role:(iam_role_ : iam_role option) ?activation_id:(activation_id_ : activation_id option)
    ?platform_version:(platform_version_ : platform_version option)
    ?platform_name:(platform_name_ : platform_name option)
    ?platform_type:(platform_type_ : platform_type option)
    ?agent_version:(agent_version_ : version option)
    ?last_ping_date_time:(last_ping_date_time_ : date_time option)
    ?ping_status:(ping_status_ : ping_status option) ?launch_time:(launch_time_ : date_time option)
    ?ip_address:(ip_address_ : ip_address option)
    ?architecture:(architecture_ : architecture option)
    ?instance_state:(instance_state_ : instance_state option)
    ?key_name:(key_name_ : key_name option) ?instance_role:(instance_role_ : instance_role option)
    ?instance_type:(instance_type_ : instance_type option)
    ?instance_id:(instance_id_ : instance_id option) ?name:(name_ : instance_name option) () =
  ({
     source_type = source_type_;
...
let make_instance_patch_state
    ...
    ?snapshot_id:(snapshot_id_ : snapshot_id option) ~operation:(operation_ : patch_operation_type)
    ~operation_end_time:(operation_end_time_ : date_time)
    ~operation_start_time:(operation_start_time_ : date_time)
    ~baseline_id:(baseline_id_ : baseline_id) ~patch_group:(patch_group_ : patch_group)
    ~instance_id:(instance_id_ : instance_id) () =
  ({
...
let make_instance_patch_state_filter ~type_:(type__ : instance_patch_state_operator_type)
...
let make_instance_information ?source_type:(source_type_ : source_type option)
    ?source_id:(source_id_ : source_id option)
    ?association_overview:(association_overview_ : instance_aggregated_association_overview option)
    ?last_successful_association_execution_date:
      (last_successful_association_execution_date_ : date_time option)
    ...
    ?association_status:(association_status_ : status_name option)
    ?computer_name:(computer_name_ : computer_name option)
    ?ip_address:(ip_address_ : ip_address option) ?name:(name_ : string_ option)
...
let make_instance_information_filter ~value_set:(value_set_ : instance_information_filter_value_set)
...
let make_instance_association_status_info
    ...
    ?association_version:(association_version_ : association_version option)
    ?document_version:(document_version_ : document_version option)
    ?name:(name_ : document_ar_n option) ?association_id:(association_id_ : association_id option)
    () =
  ({
     association_name = association_name_;
     output_url = output_url_;
...
let make_instance_association
    ?association_version:(association_version_ : association_version option)
    ?content:(content_ : document_content option) ?instance_id:(instance_id_ : instance_id option)
    ?association_id:(association_id_ : association_id option) () =
  ({
     association_version = association_version_;
     content = content_;
...
let make_get_resource_policies_response_entry ?policy:(policy_ : policy option)
    ?policy_hash:(policy_hash_ : policy_hash option) ?policy_id:(policy_id_ : policy_id option) () =
...
let make_get_resource_policies_request
    ...
    ?next_token:(next_token_ : string_ option) ~resource_arn:(resource_arn_ : resource_arn_string)
    () =
...
let make_get_parameters_by_path_request ?next_token:(next_token_ : next_token option)
...
let make_get_parameter_history_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
    ?with_decryption:(with_decryption_ : boolean_ option) ~name:(name_ : ps_parameter_name) () =
  ({
     next_token = next_token_;
     max_results = max_results_;
     with_decryption = with_decryption_;
...
let make_get_ops_summary_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_get_ops_metadata_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : get_ops_metadata_max_results option)
    ~ops_metadata_arn:(ops_metadata_arn_ : ops_metadata_arn) () =
  ({ next_token = next_token_; max_results = max_results_; ops_metadata_arn = ops_metadata_arn_ }
...
let make_get_maintenance_window_task_request
    ~window_task_id:(window_task_id_ : maintenance_window_task_id)
    ~window_id:(window_id_ : maintenance_window_id) () =
  ({ window_task_id = window_task_id_; window_id = window_id_ }
...
let make_get_inventory_schema_request ?sub_type:(sub_type_ : is_sub_type_schema option)
...
let make_get_inventory_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_automation_execution_preview ?total_accounts:(total_accounts_ : integer option)
...
let make_get_execution_preview_response
    ...
    ?status:(status_ : execution_preview_status option) ?ended_at:(ended_at_ : date_time option)
    ?execution_preview_id:(execution_preview_id_ : execution_preview_id option) () =
  ({
...
let make_attachment_content ?url:(url_ : attachment_url option)
    ?hash_type:(hash_type_ : attachment_hash_type option) ?hash:(hash_ : attachment_hash option)
    ?size:(size_ : content_length option) ?name:(name_ : attachment_name option) () =
  ({ url = url_; hash_type = hash_type_; hash = hash_; size = size_; name = name_ }
    : attachment_content)

let make_get_document_request ?document_format:(document_format_ : document_format option)
    ?document_version:(document_version_ : document_version option)
...
let make_get_connection_status_response ?status:(status_ : connection_status option)
...
let make_get_command_invocation_request ?plugin_name:(plugin_name_ : command_plugin_name option)
    ~instance_id:(instance_id_ : instance_id) ~command_id:(command_id_ : command_id) () =
  ({ plugin_name = plugin_name_; instance_id = instance_id_; command_id = command_id_ }
...
let make_get_calendar_state_response
    ...
    ?at_time:(at_time_ : iso8601_string option) ?state:(state_ : calendar_state option) () =
  ({ next_transition_time = next_transition_time_; at_time = at_time_; state = state_ }
...
let make_automation_execution ?variables:(variables_ : automation_parameter_map option)
    ?change_request_name:(change_request_name_ : change_request_name option)
    ?association_id:(association_id_ : string_ option) ?ops_item_id:(ops_item_id_ : string_ option)
    ?runbooks:(runbooks_ : runbooks option) ?scheduled_time:(scheduled_time_ : date_time option)
    ...
    ?target_locations:(target_locations_ : target_locations option)
    ?target:(target_ : string_ option) ?max_errors:(max_errors_ : max_errors option)
    ?max_concurrency:(max_concurrency_ : max_concurrency option)
    ...
    ?mode:(mode_ : execution_mode option) ?failure_message:(failure_message_ : string_ option)
    ?outputs:(outputs_ : automation_parameter_map option)
    ?parameters:(parameters_ : automation_parameter_map option)
...
let make_get_access_token_response
  ...
  ({ access_request_status = access_request_status_; credentials = credentials_ }
...
let make_create_association_batch_request_entry
    ...
    ?automation_target_parameter_name:
      (automation_target_parameter_name_ : automation_target_parameter_name option)
    ?parameters:(parameters_ : parameters option) ?instance_id:(instance_id_ : instance_id option)
...
let make_describe_sessions_response ?next_token:(next_token_ : next_token option)
    ?sessions:(sessions_ : session_list option) () =
  ({ next_token = next_token_; sessions = sessions_ } : describe_sessions_response)

let make_describe_sessions_request ?filters:(filters_ : session_filter_list option)
    ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : session_max_results option) ~state:(state_ : session_state) () =
  ({ filters = filters_; next_token = next_token_; max_results = max_results_; state = state_ }
    : describe_sessions_request)

let make_describe_patch_properties_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) ?patch_set:(patch_set_ : patch_set option)
    ~property:(property_ : patch_property) ~operating_system:(operating_system_ : operating_system)
    () =
  ({
     next_token = next_token_;
     max_results = max_results_;
...
let make_describe_patch_groups_request ?next_token:(next_token_ : next_token option)
...
let make_describe_patch_baselines_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : patch_baseline_max_results option)
    ?filters:(filters_ : patch_orchestrator_filter_list option) () =
  ({ next_token = next_token_; max_results = max_results_; filters = filters_ }
...
let make_describe_ops_items_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : ops_item_max_results option)
    ?ops_item_filters:(ops_item_filters_ : ops_item_filters option) () =
  ({ next_token = next_token_; max_results = max_results_; ops_item_filters = ops_item_filters_ }
    : describe_ops_items_request)

let make_describe_maintenance_windows_request ?next_token:(next_token_ : next_token option)
...
let make_describe_maintenance_window_tasks_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : maintenance_window_max_results option)
    ?filters:(filters_ : maintenance_window_filter_list option)
    ~window_id:(window_id_ : maintenance_window_id) () =
  ({
...
let make_describe_maintenance_window_targets_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : maintenance_window_max_results option)
    ?filters:(filters_ : maintenance_window_filter_list option)
    ~window_id:(window_id_ : maintenance_window_id) () =
  ({
...
let make_describe_maintenance_window_schedule_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : maintenance_window_search_max_results option)
    ?filters:(filters_ : patch_orchestrator_filter_list option)
    ?resource_type:(resource_type_ : maintenance_window_resource_type option)
    ?targets:(targets_ : targets option) ?window_id:(window_id_ : maintenance_window_id option) () =
  ({
     next_token = next_token_;
     max_results = max_results_;
     filters = filters_;
...
let make_describe_inventory_deletions_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) ?deletion_id:(deletion_id_ : uui_d option) () =
  ({ max_results = max_results_; next_token = next_token_; deletion_id = deletion_id_ }
    : describe_inventory_deletions_request)

let make_describe_instance_properties_request ?next_token:(next_token_ : next_token option)
...
let make_describe_instance_patch_states_request
    ?max_results:(max_results_ : patch_compliance_max_results option)
    ?next_token:(next_token_ : next_token option) ~instance_ids:(instance_ids_ : instance_id_list)
    () =
  ({ max_results = max_results_; next_token = next_token_; instance_ids = instance_ids_ }
...
let make_describe_instance_information_request ?next_token:(next_token_ : next_token option)
...
let make_describe_instance_associations_status_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) ~instance_id:(instance_id_ : instance_id) () =
  ({ next_token = next_token_; max_results = max_results_; instance_id = instance_id_ }
...
let make_describe_effective_patches_for_patch_baseline_request
    ...
    ?max_results:(max_results_ : patch_baseline_max_results option)
    ~baseline_id:(baseline_id_ : baseline_id) () =
  ({ next_token = next_token_; max_results = max_results_; baseline_id = baseline_id_ }
...
let make_describe_document_request ?version_name:(version_name_ : document_version_name option)
    ?document_version:(document_version_ : document_version option) ~name:(name_ : document_ar_n) ()
    =
  ({ version_name = version_name_; document_version = document_version_; name = name_ }
    : describe_document_request)

let make_account_sharing_info
    ?shared_document_version:(shared_document_version_ : shared_document_version option)
    ?account_id:(account_id_ : account_id option) () =
  ({ shared_document_version = shared_document_version_; account_id = account_id_ }
    : account_sharing_info)

let make_describe_document_permission_response ?next_token:(next_token_ : next_token option)
...
let make_describe_document_permission_request ?next_token:(next_token_ : next_token option)
...
let make_describe_available_patches_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : patch_baseline_max_results option)
    ?filters:(filters_ : patch_orchestrator_filter_list option) () =
  ({ next_token = next_token_; max_results = max_results_; filters = filters_ }
...
let make_automation_execution_metadata
    ...
    ?triggered_alarms:(triggered_alarms_ : alarm_state_information_list option)
    ?alarm_configuration:(alarm_configuration_ : alarm_configuration option)
    ?automation_type:(automation_type_ : automation_type option) ?target:(target_ : string_ option)
    ?max_errors:(max_errors_ : max_errors option)
    ?max_concurrency:(max_concurrency_ : max_concurrency option)
    ?resolved_targets:(resolved_targets_ : resolved_targets option)
    ?target_maps:(target_maps_ : target_maps option) ?targets:(targets_ : targets option)
    ...
    ?parent_automation_execution_id:
      (parent_automation_execution_id_ : automation_execution_id option)
    ?mode:(mode_ : execution_mode option) ?outputs:(outputs_ : automation_parameter_map option)
    ?log_file:(log_file_ : string_ option) ?executed_by:(executed_by_ : string_ option)
    ?execution_end_time:(execution_end_time_ : date_time option)
    ?execution_start_time:(execution_start_time_ : date_time option)
    ?automation_execution_status:(automation_execution_status_ : automation_execution_status option)
    ?document_version:(document_version_ : document_version option)
...
let make_automation_execution_filter ~values:(values_ : automation_execution_filter_value_list)
    ~key:(key_ : automation_execution_filter_key) () =
  ({ values = values_; key = key_ } : automation_execution_filter)

let make_describe_automation_executions_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
...
let make_association_execution_filter ~type_:(type__ : association_filter_operator_type)
...
let make_describe_association_executions_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
...
let make_association_execution_target ?output_source:(output_source_ : output_source option)
    ?last_execution_date:(last_execution_date_ : date_time option)
    ?detailed_status:(detailed_status_ : status_name option) ?status:(status_ : status_name option)
...
let make_activation ?tags:(tags_ : tag_list option)
    ?created_date:(created_date_ : created_date option) ?expired:(expired_ : boolean_ option)
...
let make_describe_activations_filter ?filter_values:(filter_values_ : string_list option)
...
let make_describe_activations_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
...
let make_deregister_target_from_maintenance_window_request ?safe:(safe_ : boolean_ option)
    ~window_target_id:(window_target_id_ : maintenance_window_target_id)
    ~window_id:(window_id_ : maintenance_window_id) () =
  ({ safe = safe_; window_target_id = window_target_id_; window_id = window_id_ }
    : deregister_target_from_maintenance_window_request)

let make_deregister_patch_baseline_for_patch_group_request ~patch_group:(patch_group_ : patch_group)
    ~baseline_id:(baseline_id_ : baseline_id) () =
  ({ patch_group = patch_group_; baseline_id = baseline_id_ }
...
let make_delete_resource_policy_request ~policy_hash:(policy_hash_ : policy_hash)
    ~policy_id:(policy_id_ : policy_id) ~resource_arn:(resource_arn_ : resource_arn_string) () =
  ({ policy_hash = policy_hash_; policy_id = policy_id_; resource_arn = resource_arn_ }
...
let make_delete_inventory_request ?client_token:(client_token_ : uui_d option)
...
let make_delete_association_request ?association_id:(association_id_ : association_id option)
    ?instance_id:(instance_id_ : instance_id option) ?name:(name_ : document_ar_n option) () =
  ({ association_id = association_id_; instance_id = instance_id_; name = name_ }
...
let make_create_patch_baseline_request ?tags:(tags_ : tag_list option)
    ...
    ?approval_rules:(approval_rules_ : patch_rule_group option)
    ?global_filters:(global_filters_ : patch_filter_group option)
    ?operating_system:(operating_system_ : operating_system option) ~name:(name_ : baseline_name) ()
    =
  ({
     tags = tags_;
     client_token = client_token_;
...
let make_create_ops_metadata_request ?tags:(tags_ : tag_list option)
  ...
  ({ tags = tags_; metadata = metadata_; resource_id = resource_id_ } : create_ops_metadata_request)

let make_create_ops_item_response ?ops_item_arn:(ops_item_arn_ : ops_item_arn option)
    ?ops_item_id:(ops_item_id_ : string_ option) () =
  ({ ops_item_arn = ops_item_arn_; ops_item_id = ops_item_id_ } : create_ops_item_response)

let make_create_ops_item_request ?account_id:(account_id_ : ops_item_account_id option)
    ?planned_end_time:(planned_end_time_ : date_time option)
    ?planned_start_time:(planned_start_time_ : date_time option)
    ?actual_end_time:(actual_end_time_ : date_time option)
    ?actual_start_time:(actual_start_time_ : date_time option)
    ?severity:(severity_ : ops_item_severity option)
    ?category:(category_ : ops_item_category option) ?tags:(tags_ : tag_list option)
    ?related_ops_items:(related_ops_items_ : related_ops_items option)
    ?priority:(priority_ : ops_item_priority option)
    ?notifications:(notifications_ : ops_item_notifications option)
    ?operational_data:(operational_data_ : ops_item_operational_data option)
    ?ops_item_type:(ops_item_type_ : ops_item_type option) ~title:(title_ : ops_item_title)
    ~source:(source_ : ops_item_source) ~description:(description_ : ops_item_description) () =
  ({
     account_id = account_id_;
     planned_end_time = planned_end_time_;
     planned_start_time = planned_start_time_;
...
let make_create_maintenance_window_request ?tags:(tags_ : tag_list option)
    ...
    ~schedule:(schedule_ : maintenance_window_schedule) ~name:(name_ : maintenance_window_name) () =
  ({
     tags = tags_;
     client_token = client_token_;
     allow_unassociated_targets = allow_unassociated_targets_;
...
let make_create_document_request ?tags:(tags_ : tag_list option)
    ...
    ?requires:(requires_ : document_requires_list option) ~name:(name_ : document_name)
    ~content:(content_ : document_content) () =
  ({
     tags = tags_;
     target_type = target_type_;
...
let make_create_activation_request
    ...
    ?tags:(tags_ : tag_list option) ?expiration_date:(expiration_date_ : expiration_date option)
    ?registration_limit:(registration_limit_ : registration_limit option)
    ?default_instance_name:(default_instance_name_ : default_instance_name option)
    ?description:(description_ : activation_description option) ~iam_role:(iam_role_ : iam_role) ()
    =
  ({
