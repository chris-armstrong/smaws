...
let make_stream_record ?stream_view_type:(stream_view_type_ : stream_view_type option)
    ?size_bytes:(size_bytes_ : positive_long_object option)
    ?sequence_number:(sequence_number_ : sequence_number option)
    ?old_image:(old_image_ : attribute_map option) ?new_image:(new_image_ : attribute_map option)
...
let make_stream ?stream_label:(stream_label_ : string_ option)
    ?table_name:(table_name_ : table_name option) ?stream_arn:(stream_arn_ : stream_arn option) () =
  ({ stream_label = stream_label_; table_name = table_name_; stream_arn = stream_arn_ } : stream)
...
let make_stream_description ?last_evaluated_shard_id:(last_evaluated_shard_id_ : shard_id option)
    ?shards:(shards_ : shard_description_list option) ?key_schema:(key_schema_ : key_schema option)
    ...
    ?stream_label:(stream_label_ : string_ option) ?stream_arn:(stream_arn_ : stream_arn option) ()
    =
  ({
...
let make_record ?user_identity:(user_identity_ : identity option)
    ?dynamodb:(dynamodb_ : stream_record option) ?aws_region:(aws_region_ : string_ option)
    ?event_source:(event_source_ : string_ option) ?event_version:(event_version_ : string_ option)
    ?event_name:(event_name_ : operation_type option) ?event_i_d:(event_i_d_ : string_ option) () =
  ({
...
let make_list_streams_output
  ...
  ({ last_evaluated_stream_arn = last_evaluated_stream_arn_; streams = streams_ }
    : list_streams_output)

let make_list_streams_input
    ?exclusive_start_stream_arn:(exclusive_start_stream_arn_ : stream_arn option)
    ?limit:(limit_ : positive_integer_object option) ?table_name:(table_name_ : table_name option)
    () =
  ({
...
let make_get_records_output ?next_shard_iterator:(next_shard_iterator_ : shard_iterator option)
  ...
  ({ next_shard_iterator = next_shard_iterator_; records = records_ } : get_records_output)
