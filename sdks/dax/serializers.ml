...
let parameter_type_to_yojson (x : parameter_type) =
  match x with NODE_TYPE_SPECIFIC -> `String "NODE_TYPE_SPECIFIC" | DEFAULT -> `String "DEFAULT"
...
let change_type_to_yojson (x : change_type) =
  match x with REQUIRES_REBOOT -> `String "REQUIRES_REBOOT" | IMMEDIATE -> `String "IMMEDIATE"
...
let increase_replication_factor_request_to_yojson (x : increase_replication_factor_request) =
  assoc_to_yojson
    [
      ...
      ("NewReplicationFactor", Some (integer__to_yojson x.new_replication_factor));
      ("ClusterName", Some (string__to_yojson x.cluster_name));
    ]
