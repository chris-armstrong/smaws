...
let subnet_identifier_list_to_yojson tree =
...
let service_linked_role_not_found_fault_to_yojson
  (x : service_linked_role_not_found_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let update_parameter_group_response_to_yojson
  (x : update_parameter_group_response) =
  assoc_to_yojson
    [("ParameterGroup",
...
let parameter_name_value_list_to_yojson tree =
  list_to_yojson parameter_name_value_to_yojson tree
let update_parameter_group_request_to_yojson
  (x : update_parameter_group_request) =
  assoc_to_yojson
    [("ParameterNameValues",
...
let parameter_group_not_found_fault_to_yojson
  (x : parameter_group_not_found_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let invalid_parameter_group_state_fault_to_yojson
  (x : invalid_parameter_group_state_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let node_identifier_list_to_yojson tree =
  list_to_yojson string__to_yojson tree
...
let cluster_endpoint_encryption_type_to_yojson
  ...
  match x with | TLS -> `String "TLS" | NONE -> `String "NONE"
let cluster_to_yojson (x : cluster) =
  assoc_to_yojson
    ...
    ("SecurityGroups",
      (option_to_yojson security_group_membership_list_to_yojson
         x.security_groups));
    ("SubnetGroup", (option_to_yojson string__to_yojson x.subnet_group));
    ("NotificationConfiguration",
      (option_to_yojson notification_configuration_to_yojson
...
let update_cluster_response_to_yojson (x : update_cluster_response) =
  assoc_to_yojson
    [("Cluster", (option_to_yojson cluster_to_yojson x.cluster))]
let security_group_identifier_list_to_yojson tree =
  list_to_yojson string__to_yojson tree
let update_cluster_request_to_yojson (x : update_cluster_request) =
  assoc_to_yojson
    [("SecurityGroupIds",
       (option_to_yojson security_group_identifier_list_to_yojson
          x.security_group_ids));
    ("ParameterGroupName",
      (option_to_yojson string__to_yojson x.parameter_group_name));
...
let tag_quota_per_resource_exceeded_to_yojson
  (x : tag_quota_per_resource_exceeded) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
let subnet_group_quota_exceeded_fault_to_yojson
  (x : subnet_group_quota_exceeded_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
let subnet_group_name_list_to_yojson tree =
  list_to_yojson string__to_yojson tree
let subnet_group_list_to_yojson tree =
  list_to_yojson subnet_group_to_yojson tree
let subnet_group_in_use_fault_to_yojson (x : subnet_group_in_use_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
let subnet_group_already_exists_fault_to_yojson
  (x : subnet_group_already_exists_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let parameter_to_yojson (x : parameter) =
  assoc_to_yojson
    ...
    ("NodeTypeSpecificValues",
      (option_to_yojson node_type_specific_value_list_to_yojson
         x.node_type_specific_values));
    ("ParameterValue",
      (option_to_yojson string__to_yojson x.parameter_value));
    ("ParameterType",
...
let parameter_group_quota_exceeded_fault_to_yojson
  (x : parameter_group_quota_exceeded_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
let parameter_group_name_list_to_yojson tree =
  list_to_yojson string__to_yojson tree
let parameter_group_list_to_yojson tree =
  list_to_yojson parameter_group_to_yojson tree
let parameter_group_already_exists_fault_to_yojson
  (x : parameter_group_already_exists_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let node_quota_for_cluster_exceeded_fault_to_yojson
  (x : node_quota_for_cluster_exceeded_fault) =
  assoc_to_yojson
    [("message", (option_to_yojson exception_message_to_yojson x.message))]
...
let describe_parameter_groups_response_to_yojson
  (x : describe_parameter_groups_response) =
  assoc_to_yojson
    [("ParameterGroups",
       (option_to_yojson parameter_group_list_to_yojson x.parameter_groups));
    ("NextToken", (option_to_yojson string__to_yojson x.next_token))]
let describe_parameter_groups_request_to_yojson
  (x : describe_parameter_groups_request) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson string__to_yojson x.next_token));
    ("MaxResults",
      (option_to_yojson integer_optional_to_yojson x.max_results));
    ("ParameterGroupNames",
      (option_to_yojson parameter_group_name_list_to_yojson
         x.parameter_group_names))]
...
let describe_default_parameters_response_to_yojson
  (x : describe_default_parameters_response) =
  assoc_to_yojson
    [("Parameters", (option_to_yojson parameter_list_to_yojson x.parameters));
    ("NextToken", (option_to_yojson string__to_yojson x.next_token))]
let describe_default_parameters_request_to_yojson
  (x : describe_default_parameters_request) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson string__to_yojson x.next_token));
    ("MaxResults",
      (option_to_yojson integer_optional_to_yojson x.max_results))]
...
let delete_parameter_group_response_to_yojson
  (x : delete_parameter_group_response) =
  assoc_to_yojson
    [("DeletionMessage",
       (option_to_yojson string__to_yojson x.deletion_message))]
let delete_parameter_group_request_to_yojson
  (x : delete_parameter_group_request) =
  assoc_to_yojson
    [("ParameterGroupName",
...
let decrease_replication_factor_response_to_yojson
  (x : decrease_replication_factor_response) =
  assoc_to_yojson
    [("Cluster", (option_to_yojson cluster_to_yojson x.cluster))]
let decrease_replication_factor_request_to_yojson
  (x : decrease_replication_factor_request) =
  assoc_to_yojson
    [("NodeIdsToRemove",
       (option_to_yojson node_identifier_list_to_yojson x.node_ids_to_remove));
    ("AvailabilityZones",
...
let create_parameter_group_response_to_yojson
  (x : create_parameter_group_response) =
  assoc_to_yojson
    [("ParameterGroup",
       (option_to_yojson parameter_group_to_yojson x.parameter_group))]
let create_parameter_group_request_to_yojson
  (x : create_parameter_group_request) =
  assoc_to_yojson
    [("Description", (option_to_yojson string__to_yojson x.description));
    ("ParameterGroupName", (Some (string__to_yojson x.parameter_group_name)))]
...
let create_cluster_request_to_yojson (x : create_cluster_request) =
  assoc_to_yojson
    ...
    ("AvailabilityZones",
      (option_to_yojson availability_zone_list_to_yojson x.availability_zones));
    ("ReplicationFactor", (Some (integer__to_yojson x.replication_factor)));
    ("Description", (option_to_yojson string__to_yojson x.description));
