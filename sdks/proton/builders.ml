...
let make_template_sync_config ?subdirectory:(subdirectory_ : subdirectory option)
    ~branch:(branch_ : git_branch_name) ~repository_name:(repository_name_ : repository_name)
    ~repository_provider:(repository_provider_ : repository_provider)
    ~template_type:(template_type_ : template_type) ~template_name:(template_name_ : resource_name)
    () =
  ({
...
let make_compatible_environment_template ~major_version:(major_version_ : template_version_part)
...
let make_service_template_version
    ...
    ?schema:(schema_ : template_schema option) ?description:(description_ : description option)
    ...
    ~arn:(arn_ : service_template_version_arn) ~status:(status_ : template_version_status)
...
let make_service_template ?pipeline_provisioning:(pipeline_provisioning_ : provisioning option)
...
let make_update_service_template_input ?description:(description_ : description option)
    ?display_name:(display_name_ : display_name option) ~name:(name_ : resource_name) () =
...
let make_update_service_sync_config_input ~file_path:(file_path_ : ops_file_path)
    ~branch:(branch_ : git_branch_name) ~repository_name:(repository_name_ : repository_name)
    ~repository_provider:(repository_provider_ : repository_provider)
    ~service_name:(service_name_ : resource_name) () =
  ({
...
let make_sync_blocker_context ~value:(value_ : Smaws_Lib.Smithy_api.Types.string_)
    ~key:(key_ : Smaws_Lib.Smithy_api.Types.string_) () =
  ({ value = value_; key = key_ } : sync_blocker_context)

let make_sync_blocker ?resolved_at:(resolved_at_ : Smaws_Lib.Smithy_api.Types.timestamp option)
...
let make_service_pipeline
    ...
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp) ~arn:(arn_ : arn) () =
  ({
...
let make_update_service_pipeline_input
    ...
    ~deployment_type:(deployment_type_ : deployment_update_type) ~spec:(spec_ : spec_contents)
    ~service_name:(service_name_ : resource_name) () =
  ({
     template_minor_version = template_minor_version_;
     template_major_version = template_major_version_;
...
let make_service ?branch_name:(branch_name_ : git_branch_name option)
    ...
    ?status_message:(status_message_ : status_message option)
    ?description:(description_ : description option) ~spec:(spec_ : spec_contents)
...
let make_update_service_instance_input ?client_token:(client_token_ : client_token option)
    ?template_minor_version:(template_minor_version_ : template_version_part option)
    ?template_major_version:(template_major_version_ : template_version_part option)
    ?spec:(spec_ : spec_contents option)
...
let make_environment_template_version ?schema:(schema_ : template_schema option)
    ?description:(description_ : description option)
    ?status_message:(status_message_ : status_message option)
    ?recommended_minor_version:(recommended_minor_version_ : template_version_part option)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~arn:(arn_ : environment_template_version_arn) ~status:(status_ : template_version_status)
    ~minor_version:(minor_version_ : template_version_part)
    ~major_version:(major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) () =
  ({
...
let make_environment_template ?provisioning:(provisioning_ : provisioning option)
    ?encryption_key:(encryption_key_ : arn option)
    ?recommended_version:(recommended_version_ : full_template_version_number option)
    ?description:(description_ : description option)
    ?display_name:(display_name_ : display_name option)
...
let make_update_environment_template_input ?description:(description_ : description option)
    ?display_name:(display_name_ : display_name option) ~name:(name_ : resource_name) () =
  ({ description = description_; display_name = display_name_; name = name_ }
    : update_environment_template_input)

let make_repository_branch ~branch:(branch_ : git_branch_name) ~name:(name_ : repository_name)
    ~provider:(provider_ : repository_provider) ~arn:(arn_ : repository_arn) () =
  ({ branch = branch_; name = name_; provider = provider_; arn = arn_ } : repository_branch)
...
let make_environment
    ...
    ?provisioning:(provisioning_ : provisioning option) ?spec:(spec_ : spec_contents option)
    ...
    ~template_minor_version:(template_minor_version_ : template_version_part)
    ~template_major_version:(template_major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) ~arn:(arn_ : environment_arn)
    ~last_deployment_succeeded_at:
      (last_deployment_succeeded_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~last_deployment_attempted_at:
      (last_deployment_attempted_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp) ~name:(name_ : resource_name)
    () =
  ({
     last_succeeded_deployment_id = last_succeeded_deployment_id_;
     last_attempted_deployment_id = last_attempted_deployment_id_;
...
let make_repository_branch_input ~branch:(branch_ : git_branch_name) ~name:(name_ : repository_name)
    ~provider:(provider_ : repository_provider) () =
  ({ branch = branch_; name = name_; provider = provider_ } : repository_branch_input)

let make_update_environment_input ?codebuild_role_arn:(codebuild_role_arn_ : role_arn option)
    ?component_role_arn:(component_role_arn_ : role_arn option)
    ...
    ?template_minor_version:(template_minor_version_ : template_version_part option)
    ?template_major_version:(template_major_version_ : template_version_part option)
    ?spec:(spec_ : spec_contents option) ?description:(description_ : description option)
    ~deployment_type:(deployment_type_ : deployment_update_type) ~name:(name_ : resource_name) () =
  ({
     codebuild_role_arn = codebuild_role_arn_;
     component_role_arn = component_role_arn_;
     provisioning_repository = provisioning_repository_;
...
let make_environment_account_connection ?codebuild_role_arn:(codebuild_role_arn_ : role_arn option)
    ?component_role_arn:(component_role_arn_ : role_arn option)
    ~status:(status_ : environment_account_connection_status)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~requested_at:(requested_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~environment_name:(environment_name_ : resource_name) ~role_arn:(role_arn_ : arn)
    ~environment_account_id:(environment_account_id_ : aws_account_id)
    ~management_account_id:(management_account_id_ : aws_account_id)
    ~arn:(arn_ : environment_account_connection_arn) ~id:(id_ : environment_account_connection_id)
    () =
  ({
     codebuild_role_arn = codebuild_role_arn_;
     component_role_arn = component_role_arn_;
...
let make_update_environment_account_connection_input
    ?codebuild_role_arn:(codebuild_role_arn_ : role_arn option)
    ?component_role_arn:(component_role_arn_ : role_arn option)
    ?role_arn:(role_arn_ : role_arn option) ~id:(id_ : environment_account_connection_id) () =
  ({
     codebuild_role_arn = codebuild_role_arn_;
     component_role_arn = component_role_arn_;
     role_arn = role_arn_;
...
let make_component
    ...
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~environment_name:(environment_name_ : resource_name) ~arn:(arn_ : component_arn)
    ~name:(name_ : resource_name) () =
  ({
     last_succeeded_deployment_id = last_succeeded_deployment_id_;
     last_attempted_deployment_id = last_attempted_deployment_id_;
...
let make_update_component_input ?client_token:(client_token_ : client_token option)
...
let make_service_template_version_summary ?description:(description_ : description option)
    ?status_message:(status_message_ : status_message option)
    ?recommended_minor_version:(recommended_minor_version_ : template_version_part option)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
...
let make_service_summary ?status_message:(status_message_ : status_message option)
    ?description:(description_ : description option) ~status:(status_ : service_status)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~template_name:(template_name_ : resource_name) ~arn:(arn_ : service_arn)
    ~name:(name_ : resource_name) () =
...
let make_service_instance_state
    ...
    ~template_minor_version:(template_minor_version_ : template_version_part)
    ~template_major_version:(template_major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) ~spec:(spec_ : spec_contents) () =
  ({
     last_successful_service_pipeline_deployment_id =
...
let make_revision ~branch:(branch_ : git_branch_name)
    ~directory:(directory_ : Smaws_Lib.Smithy_api.Types.string_) ~sha:(sha_ : sh_a)
    ~repository_provider:(repository_provider_ : repository_provider)
...
let make_resource_sync_attempt ~events:(events_ : resource_sync_events)
    ...
    ~target_revision:(target_revision_ : revision) ~initial_revision:(initial_revision_ : revision)
    () =
  ({
     events = events_;
     status = status_;
...
let make_repository_sync_definition ~directory:(directory_ : Smaws_Lib.Smithy_api.Types.string_)
    ~branch:(branch_ : git_branch_name) ~parent:(parent_ : Smaws_Lib.Smithy_api.Types.string_)
    ~target:(target_ : Smaws_Lib.Smithy_api.Types.string_) () =
  ({ directory = directory_; branch = branch_; parent = parent_; target = target_ }
...
let make_repository_summary ~connection_arn:(connection_arn_ : arn) ~name:(name_ : repository_name)
    ~provider:(provider_ : repository_provider) ~arn:(arn_ : repository_arn) () =
  ({ connection_arn = connection_arn_; name = name_; provider = provider_; arn = arn_ }
    : repository_summary)
...
let make_provisioned_resource
  ...
  ({ provisioning_engine = provisioning_engine_; identifier = identifier_; name = name_ }
...
let make_notify_resource_deployment_status_change_input
    ?status_message:(status_message_ : status_message option)
    ?deployment_id:(deployment_id_ : deployment_id option) ?outputs:(outputs_ : outputs_list option)
    ?status:(status_ : resource_deployment_status option) ~resource_arn:(resource_arn_ : arn) () =
  ({
     status_message = status_message_;
...
let make_list_tags_for_resource_input ?max_results:(max_results_ : max_page_results option)
...
let make_list_services_input ?max_results:(max_results_ : max_page_results option)
...
let make_list_service_templates_input ?max_results:(max_results_ : max_page_results option)
    ?next_token:(next_token_ : next_token option) () =
...
let make_list_service_template_versions_input
    ...
    ?next_token:(next_token_ : next_token option) ~template_name:(template_name_ : resource_name) ()
    =
  ({
     major_version = major_version_;
     template_name = template_name_;
...
let make_list_service_pipeline_provisioned_resources_input
    ?next_token:(next_token_ : empty_next_token option)
    ~service_name:(service_name_ : resource_name) () =
  ({ next_token = next_token_; service_name = service_name_ }
...
let make_list_service_pipeline_outputs_input ?deployment_id:(deployment_id_ : deployment_id option)
    ?next_token:(next_token_ : empty_next_token option)
    ~service_name:(service_name_ : resource_name) () =
  ({ deployment_id = deployment_id_; next_token = next_token_; service_name = service_name_ }
...
let make_list_service_instances_filter ?value:(value_ : list_service_instances_filter_value option)
    ?key:(key_ : list_service_instances_filter_by option) () =
  ({ value = value_; key = key_ } : list_service_instances_filter)

let make_list_service_instances_input ?sort_order:(sort_order_ : sort_order option)
...
let make_list_service_instance_outputs_input ?deployment_id:(deployment_id_ : deployment_id option)
    ?next_token:(next_token_ : empty_next_token option)
    ~service_name:(service_name_ : resource_name)
    ~service_instance_name:(service_instance_name_ : resource_name) () =
  ({
...
let make_list_repositories_input ?max_results:(max_results_ : max_page_results option)
    ?next_token:(next_token_ : next_token option) () =
...
let make_environment_template_filter ~major_version:(major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) () =
  ({ major_version = major_version_; template_name = template_name_ } : environment_template_filter)
...
let make_environment_template_summary ?provisioning:(provisioning_ : provisioning option)
    ?recommended_version:(recommended_version_ : full_template_version_number option)
    ?description:(description_ : description option)
    ?display_name:(display_name_ : display_name option)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
...
let make_list_environment_templates_input ?max_results:(max_results_ : max_page_results option)
    ?next_token:(next_token_ : next_token option) () =
  ({ max_results = max_results_; next_token = next_token_ } : list_environment_templates_input)

let make_environment_template_version_summary ?description:(description_ : description option)
    ?status_message:(status_message_ : status_message option)
    ?recommended_minor_version:(recommended_minor_version_ : template_version_part option)
    ~last_modified_at:(last_modified_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
    ~created_at:(created_at_ : Smaws_Lib.Smithy_api.Types.timestamp)
...
let make_list_environment_outputs_input ?deployment_id:(deployment_id_ : deployment_id option)
    ?next_token:(next_token_ : empty_next_token option)
    ~environment_name:(environment_name_ : resource_name) () =
  ({
     deployment_id = deployment_id_;
...
let make_list_environment_account_connections_output ?next_token:(next_token_ : next_token option)
  ...
  ({ next_token = next_token_; environment_account_connections = environment_account_connections_ }
...
let make_deployment_summary
    ...
    ~target_arn:(target_arn_ : arn) ~arn:(arn_ : deployment_arn) ~id:(id_ : deployment_id) () =
  ({
     deployment_status = deployment_status_;
     last_succeeded_deployment_id = last_succeeded_deployment_id_;
     last_attempted_deployment_id = last_attempted_deployment_id_;
...
let make_list_deployments_input ?max_results:(max_results_ : max_page_results option)
    ?component_name:(component_name_ : resource_name option)
    ?service_instance_name:(service_instance_name_ : resource_name option)
    ?service_name:(service_name_ : resource_name option)
    ?environment_name:(environment_name_ : resource_name option)
...
let make_list_components_input ?max_results:(max_results_ : max_page_results option)
    ?service_instance_name:(service_instance_name_ : resource_name option)
    ?service_name:(service_name_ : resource_name option)
    ?environment_name:(environment_name_ : resource_name option)
    ?next_token:(next_token_ : next_token option) () =
...
let make_list_component_provisioned_resources_input
  ...
  ({ next_token = next_token_; component_name = component_name_ }
...
let make_list_component_outputs_input ?deployment_id:(deployment_id_ : deployment_id option)
    ?next_token:(next_token_ : empty_next_token option)
    ~component_name:(component_name_ : resource_name) () =
  ({ deployment_id = deployment_id_; next_token = next_token_; component_name = component_name_ }
    : list_component_outputs_input)

let make_get_template_sync_status_output ?desired_state:(desired_state_ : revision option)
...
let make_get_template_sync_config_input ~template_type:(template_type_ : template_type)
    ~template_name:(template_name_ : resource_name) () =
...
let make_get_service_template_version_input ~minor_version:(minor_version_ : template_version_part)
    ~major_version:(major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) () =
  ({
     minor_version = minor_version_;
...
let make_get_service_instance_sync_status_output ?desired_state:(desired_state_ : revision option)
    ?latest_successful_sync:(latest_successful_sync_ : resource_sync_attempt option)
    ?latest_sync:(latest_sync_ : resource_sync_attempt option) () =
  ({
     desired_state = desired_state_;
...
let make_get_service_instance_input ~service_name:(service_name_ : resource_name)
    ~name:(name_ : resource_name) () =
...
let make_counts_summary ?pipelines:(pipelines_ : resource_counts_summary option)
...
let make_component_state ?template_file:(template_file_ : template_file_contents option)
    ?service_spec:(service_spec_ : spec_contents option)
    ?service_instance_name:(service_instance_name_ : resource_name_or_empty option)
    ?service_name:(service_name_ : resource_name_or_empty option) () =
  ({
...
let make_get_deployment_input ?component_name:(component_name_ : resource_name option)
    ?service_instance_name:(service_instance_name_ : resource_name option)
    ?service_name:(service_name_ : resource_name option)
    ?environment_name:(environment_name_ : resource_name option) ~id:(id_ : deployment_id) () =
  ({
     component_name = component_name_;
     service_instance_name = service_instance_name_;
     service_name = service_name_;
...
let make_delete_template_sync_config_input ~template_type:(template_type_ : template_type)
    ~template_name:(template_name_ : resource_name) () =
  ({ template_type = template_type_; template_name = template_name_ }
...
let make_create_service_template_version_input
    ?supported_component_sources:
      (supported_component_sources_ : service_template_supported_component_source_input_list option)
    ?tags:(tags_ : tag_list option) ?major_version:(major_version_ : template_version_part option)
    ?description:(description_ : description option)
...
let make_create_service_template_input ?tags:(tags_ : tag_list option)
    ?pipeline_provisioning:(pipeline_provisioning_ : provisioning option)
    ?encryption_key:(encryption_key_ : arn option) ?description:(description_ : description option)
    ?display_name:(display_name_ : display_name option) ~name:(name_ : resource_name) () =
  ({
     tags = tags_;
     pipeline_provisioning = pipeline_provisioning_;
...
let make_create_service_sync_config_input ~file_path:(file_path_ : ops_file_path)
    ~branch:(branch_ : git_branch_name) ~repository_name:(repository_name_ : repository_name)
    ~repository_provider:(repository_provider_ : repository_provider)
    ~service_name:(service_name_ : resource_name) () =
  ({
...
let make_create_service_instance_input ?client_token:(client_token_ : client_token option)
...
let make_create_service_input ?tags:(tags_ : tag_list option)
    ...
    ?description:(description_ : description option) ~spec:(spec_ : spec_contents)
    ~template_major_version:(template_major_version_ : template_version_part)
    ~template_name:(template_name_ : resource_name) ~name:(name_ : resource_name) () =
  ({
     tags = tags_;
     branch_name = branch_name_;
     repository_id = repository_id_;
...
let make_create_repository_input ?tags:(tags_ : tag_list option)
    ?encryption_key:(encryption_key_ : arn option) ~connection_arn:(connection_arn_ : arn)
...
let make_create_environment_template_version_input ?tags:(tags_ : tag_list option)
    ?major_version:(major_version_ : template_version_part option)
    ?description:(description_ : description option)
    ?client_token:(client_token_ : client_token option)
    ~source:(source_ : template_version_source_input)
...
let make_create_environment_input ?codebuild_role_arn:(codebuild_role_arn_ : role_arn option)
    ?component_role_arn:(component_role_arn_ : role_arn option)
    ?provisioning_repository:(provisioning_repository_ : repository_branch_input option)
    ?tags:(tags_ : tag_list option)
    ?environment_account_connection_id:
...
let make_create_environment_account_connection_input
    ?codebuild_role_arn:(codebuild_role_arn_ : role_arn option)
    ?component_role_arn:(component_role_arn_ : role_arn option) ?tags:(tags_ : tag_list option)
...
let make_create_component_input ?client_token:(client_token_ : client_token option)
    ?tags:(tags_ : tag_list option) ?service_spec:(service_spec_ : spec_contents option)
    ?environment_name:(environment_name_ : resource_name option)
    ?service_instance_name:(service_instance_name_ : resource_name option)
    ?service_name:(service_name_ : resource_name option)
    ?description:(description_ : description option)
    ~manifest:(manifest_ : template_manifest_contents)
    ~template_file:(template_file_ : template_file_contents) ~name:(name_ : resource_name) () =
  ({
     client_token = client_token_;
     tags = tags_;
     service_spec = service_spec_;
...
let make_cancel_service_instance_deployment_input ~service_name:(service_name_ : resource_name)
    ~service_instance_name:(service_instance_name_ : resource_name) () =
