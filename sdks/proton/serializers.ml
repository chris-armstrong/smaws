...
let update_template_sync_config_output_to_yojson
  (x : update_template_sync_config_output) =
  assoc_to_yojson
    [("templateSyncConfig",
       (option_to_yojson template_sync_config_to_yojson
          x.template_sync_config))]
let update_template_sync_config_input_to_yojson
  (x : update_template_sync_config_input) =
  assoc_to_yojson
    [("subdirectory",
       (option_to_yojson subdirectory_to_yojson x.subdirectory));
    ("branch", (Some (git_branch_name_to_yojson x.branch)));
...
let compatible_environment_template_to_yojson
  (x : compatible_environment_template) =
  assoc_to_yojson
    [("majorVersion",
...
let service_template_supported_component_source_input_list_to_yojson tree =
  list_to_yojson service_template_supported_component_source_type_to_yojson
    tree
let service_template_version_to_yojson (x : service_template_version) =
  assoc_to_yojson
    [("supportedComponentSources",
       (option_to_yojson
          service_template_supported_component_source_input_list_to_yojson
          x.supported_component_sources));
    ("schema", (option_to_yojson template_schema_to_yojson x.schema));
    ("compatibleEnvironmentTemplates",
      (Some
         (compatible_environment_template_list_to_yojson
            x.compatible_environment_templates)));
    ("lastModifiedAt", (Some (base_timestamp_to_yojson x.last_modified_at)));
    ("createdAt", (Some (base_timestamp_to_yojson x.created_at)));
    ...
    ("recommendedMinorVersion",
      (option_to_yojson template_version_part_to_yojson
         x.recommended_minor_version));
    ("minorVersion",
...
let update_service_template_version_output_to_yojson
  (x : update_service_template_version_output) =
  assoc_to_yojson
...
let compatible_environment_template_input_to_yojson
  (x : compatible_environment_template_input) =
  assoc_to_yojson
    [("majorVersion",
       (Some (template_version_part_to_yojson x.major_version)));
    ("templateName", (Some (resource_name_to_yojson x.template_name)))]
...
let update_service_template_version_input_to_yojson
  (x : update_service_template_version_input) =
  assoc_to_yojson
    [("supportedComponentSources",
       (option_to_yojson
          service_template_supported_component_source_input_list_to_yojson
...
let service_template_to_yojson (x : service_template) =
  assoc_to_yojson
    ...
    ("recommendedVersion",
      (option_to_yojson full_template_version_number_to_yojson
         x.recommended_version));
    ("description", (option_to_yojson description_to_yojson x.description));
...
let blocker_type_to_yojson (x : blocker_type) =
  match x with | AUTOMATED -> `String "AUTOMATED"
...
let sync_blocker_context_to_yojson (x : sync_blocker_context) =
  assoc_to_yojson
    [("value", (Some (base_string_to_yojson x.value)));
    ("key", (Some (base_string_to_yojson x.key)))]
let sync_blocker_contexts_to_yojson tree =
  list_to_yojson sync_blocker_context_to_yojson tree
...
let update_service_sync_blocker_input_to_yojson
  (x : update_service_sync_blocker_input) =
  assoc_to_yojson
    [("resolvedReason", (Some (base_string_to_yojson x.resolved_reason)));
    ("id", (Some (base_string_to_yojson x.id)))]
...
let service_pipeline_to_yojson (x : service_pipeline) =
  assoc_to_yojson
    [("lastSucceededDeploymentId",
       (option_to_yojson deployment_id_to_yojson
          x.last_succeeded_deployment_id));
    ("lastAttemptedDeploymentId",
      (option_to_yojson deployment_id_to_yojson
         x.last_attempted_deployment_id));
...
let update_service_pipeline_output_to_yojson
  (x : update_service_pipeline_output) =
  assoc_to_yojson
    [("pipeline", (Some (service_pipeline_to_yojson x.pipeline)))]
...
let update_service_pipeline_input_to_yojson
  (x : update_service_pipeline_input) =
  assoc_to_yojson
    [("templateMinorVersion",
       (option_to_yojson template_version_part_to_yojson
          x.template_minor_version));
    ("templateMajorVersion",
      (option_to_yojson template_version_part_to_yojson
         x.template_major_version));
    ("deploymentType",
...
let update_service_instance_output_to_yojson
  (x : update_service_instance_output) =
  assoc_to_yojson
    [("serviceInstance",
...
let update_environment_template_version_output_to_yojson
  ...
  assoc_to_yojson
    [("environmentTemplateVersion",
       (Some
          (environment_template_version_to_yojson
             x.environment_template_version)))]
...
let update_environment_output_to_yojson (x : update_environment_output) =
  assoc_to_yojson
    [("environment", (Some (environment_to_yojson x.environment)))]
...
let update_environment_input_to_yojson (x : update_environment_input) =
  assoc_to_yojson
    ...
    ("componentRoleArn",
      (option_to_yojson role_arn_to_yojson x.component_role_arn));
    ("provisioningRepository",
      (option_to_yojson repository_branch_input_to_yojson
         x.provisioning_repository));
    ("environmentAccountConnectionId",
      (option_to_yojson environment_account_connection_id_to_yojson
         x.environment_account_connection_id));
    ("deploymentType",
...
let environment_account_connection_status_to_yojson
  (x : environment_account_connection_status) =
  match x with
  | PENDING -> `String "PENDING"
  | CONNECTED -> `String "CONNECTED"
  | REJECTED -> `String "REJECTED"
let environment_account_connection_to_yojson
  (x : environment_account_connection) =
  assoc_to_yojson
    [("codebuildRoleArn",
       (option_to_yojson role_arn_to_yojson x.codebuild_role_arn));
    ("componentRoleArn",
    ...
    ("lastModifiedAt", (Some (base_timestamp_to_yojson x.last_modified_at)));
    ("requestedAt", (Some (base_timestamp_to_yojson x.requested_at)));
    ("environmentName", (Some (resource_name_to_yojson x.environment_name)));
...
let update_environment_account_connection_output_to_yojson
  ...
  assoc_to_yojson
    [("environmentAccountConnection",
       (Some
          (environment_account_connection_to_yojson
             x.environment_account_connection)))]
...
let component_to_yojson (x : component) =
  assoc_to_yojson
    ...
    ("deploymentStatus",
      (Some (deployment_status_to_yojson x.deployment_status)));
    ("lastDeploymentSucceededAt",
      (option_to_yojson base_timestamp_to_yojson
...
let component_deployment_update_type_to_yojson
  (x : component_deployment_update_type) =
  match x with
  | NONE -> `String "NONE"
  | CURRENT_VERSION -> `String "CURRENT_VERSION"
...
let update_component_input_to_yojson (x : update_component_input) =
  assoc_to_yojson
    ...
    ("serviceSpec",
      (option_to_yojson spec_contents_to_yojson x.service_spec));
    ("serviceInstanceName",
      (option_to_yojson resource_name_or_empty_to_yojson
  ...
  assoc_to_yojson
    [("pipelineCodebuildRoleArn",
       (option_to_yojson role_arn_or_empty_string_to_yojson
          x.pipeline_codebuild_role_arn));
    ("pipelineProvisioningRepository",
      (option_to_yojson repository_branch_to_yojson
         x.pipeline_provisioning_repository));
    ("pipelineServiceRoleArn",
      (option_to_yojson role_arn_or_empty_string_to_yojson
         x.pipeline_service_role_arn))]
let update_account_settings_output_to_yojson
  (x : update_account_settings_output) =
  assoc_to_yojson
    [("accountSettings",
...
let update_account_settings_input_to_yojson
  (x : update_account_settings_input) =
  assoc_to_yojson
    [("pipelineCodebuildRoleArn",
       (option_to_yojson role_arn_or_empty_string_to_yojson
          x.pipeline_codebuild_role_arn));
    ("deletePipelineProvisioningRepository",
      (option_to_yojson base_boolean_to_yojson
...
let service_template_version_summary_to_yojson
  (x : service_template_version_summary) =
  assoc_to_yojson
    [("lastModifiedAt", (Some (base_timestamp_to_yojson x.last_modified_at)));
    ("createdAt", (Some (base_timestamp_to_yojson x.created_at)));
    ("arn", (Some (service_template_version_arn_to_yojson x.arn)));
...
let latest_sync_blockers_to_yojson tree =
  list_to_yojson sync_blocker_to_yojson tree
let service_sync_blocker_summary_to_yojson (x : service_sync_blocker_summary)
  ...
  assoc_to_yojson
    ...
    ("serviceName", (Some (base_string_to_yojson x.service_name)))]
let service_summary_to_yojson (x : service_summary) =
  assoc_to_yojson
    ...
    ("description", (option_to_yojson description_to_yojson x.description));
    ("name", (Some (resource_name_to_yojson x.name)))]
let service_summary_list_to_yojson tree =
  list_to_yojson service_summary_to_yojson tree
...
let component_deployment_id_list_to_yojson tree =
  list_to_yojson deployment_id_to_yojson tree
let service_instance_state_to_yojson (x : service_instance_state) =
  assoc_to_yojson
    [("lastSuccessfulServicePipelineDeploymentId",
       (option_to_yojson deployment_id_to_yojson
          x.last_successful_service_pipeline_deployment_id));
    ("lastSuccessfulEnvironmentDeploymentId",
      (option_to_yojson deployment_id_to_yojson
         x.last_successful_environment_deployment_id));
...
let revision_to_yojson (x : revision) =
  assoc_to_yojson
    [("branch", (Some (git_branch_name_to_yojson x.branch)));
    ("directory", (Some (base_string_to_yojson x.directory)));
...
let resource_sync_event_to_yojson (x : resource_sync_event) =
  assoc_to_yojson
    [("event", (Some (base_string_to_yojson x.event)));
    ("time", (Some (base_timestamp_to_yojson x.time)));
    ("externalId", (option_to_yojson base_string_to_yojson x.external_id));
    ("type", (Some (base_string_to_yojson x.type_)))]
let resource_sync_events_to_yojson tree =
  list_to_yojson resource_sync_event_to_yojson tree
let resource_sync_attempt_to_yojson (x : resource_sync_attempt) =
  assoc_to_yojson
    ...
    ("startedAt", (Some (base_timestamp_to_yojson x.started_at)));
    ("target", (Some (base_string_to_yojson x.target)));
...
let resource_counts_summary_to_yojson (x : resource_counts_summary) =
  assoc_to_yojson
    ...
    ("behindMajor", (option_to_yojson base_integer_to_yojson x.behind_major));
    ("upToDate", (option_to_yojson base_integer_to_yojson x.up_to_date));
    ("failed", (option_to_yojson base_integer_to_yojson x.failed));
    ("total", (Some (base_integer_to_yojson x.total)))]
...
let repository_sync_event_to_yojson (x : repository_sync_event) =
  assoc_to_yojson
    ...
    ("externalId", (option_to_yojson base_string_to_yojson x.external_id));
    ("type", (Some (base_string_to_yojson x.type_)))]
let repository_sync_events_to_yojson tree =
  list_to_yojson repository_sync_event_to_yojson tree
let repository_sync_definition_to_yojson (x : repository_sync_definition) =
  assoc_to_yojson
    [("directory", (Some (base_string_to_yojson x.directory)));
    ("branch", (Some (git_branch_name_to_yojson x.branch)));
    ("parent", (Some (base_string_to_yojson x.parent)));
    ("target", (Some (base_string_to_yojson x.target)))]
...
let repository_summary_to_yojson (x : repository_summary) =
  assoc_to_yojson
    ...
    ("provider", (Some (repository_provider_to_yojson x.provider)));
    ("arn", (Some (repository_arn_to_yojson x.arn)))]
let repository_summary_list_to_yojson tree =
  list_to_yojson repository_summary_to_yojson tree
...
let provisioned_resource_to_yojson (x : provisioned_resource) =
  assoc_to_yojson
    [("provisioningEngine",
       (option_to_yojson provisioned_resource_engine_to_yojson
          x.provisioning_engine));
    ("identifier",
      (option_to_yojson provisioned_resource_identifier_to_yojson
         x.identifier));
    ("name", (option_to_yojson provisioned_resource_name_to_yojson x.name))]
let provisioned_resource_list_to_yojson tree =
  list_to_yojson provisioned_resource_to_yojson tree
...
let list_tags_for_resource_output_to_yojson
  ...
  assoc_to_yojson
    [("nextToken", (option_to_yojson base_string_to_yojson x.next_token));
    ("tags", (Some (tag_list_to_yojson x.tags)))]
...
let list_service_templates_output_to_yojson
  (x : list_service_templates_output) =
  assoc_to_yojson
    [("templates",
...
let list_service_template_versions_output_to_yojson
  (x : list_service_template_versions_output) =
  assoc_to_yojson
    [("templateVersions",
       (Some
          (service_template_version_summary_list_to_yojson
             x.template_versions)));
    ("nextToken", (option_to_yojson next_token_to_yojson x.next_token))]
let list_service_template_versions_input_to_yojson
  (x : list_service_template_versions_input) =
  assoc_to_yojson
    [("majorVersion",
...
let list_service_instances_sort_by_to_yojson
  (x : list_service_instances_sort_by) =
  match x with
  | NAME -> `String "name"
...
let list_service_instances_filter_to_yojson
  (x : list_service_instances_filter) =
  assoc_to_yojson
    [("value",
       (option_to_yojson list_service_instances_filter_value_to_yojson
          x.value));
    ("key",
...
let list_service_instances_input_to_yojson (x : list_service_instances_input)
  ...
  assoc_to_yojson
    ...
    ("filters",
      (option_to_yojson list_service_instances_filter_list_to_yojson
         x.filters));
    ("maxResults",
      (option_to_yojson max_page_results_to_yojson x.max_results));
    ("nextToken", (option_to_yojson next_token_to_yojson x.next_token));
...
let list_service_instance_outputs_output_to_yojson
  (x : list_service_instance_outputs_output) =
  assoc_to_yojson
    [("outputs", (Some (outputs_list_to_yojson x.outputs)));
    ("nextToken", (option_to_yojson empty_next_token_to_yojson x.next_token))]
let list_service_instance_outputs_input_to_yojson
  (x : list_service_instance_outputs_input) =
  assoc_to_yojson
    [("deploymentId",
       (option_to_yojson deployment_id_to_yojson x.deployment_id));
    ("nextToken", (option_to_yojson empty_next_token_to_yojson x.next_token));
...
let environment_summary_to_yojson (x : environment_summary) =
  assoc_to_yojson
    ...
    ("description", (option_to_yojson description_to_yojson x.description));
    ("name", (Some (resource_name_to_yojson x.name)))]
let environment_summary_list_to_yojson tree =
  list_to_yojson environment_summary_to_yojson tree
...
let list_environments_input_to_yojson (x : list_environments_input) =
  assoc_to_yojson
    [("environmentTemplates",
       (option_to_yojson environment_template_filter_list_to_yojson
          x.environment_templates));
    ("maxResults",
      (option_to_yojson max_page_results_to_yojson x.max_results));
    ("nextToken", (option_to_yojson next_token_to_yojson x.next_token))]
...
let list_environment_template_versions_output_to_yojson
  ...
  assoc_to_yojson
    [("templateVersions",
       (Some
          (environment_template_version_summary_list_to_yojson
             x.template_versions)));
    ("nextToken", (option_to_yojson next_token_to_yojson x.next_token))]
...
let list_environment_outputs_output_to_yojson
  (x : list_environment_outputs_output) =
  assoc_to_yojson
    [("outputs", (Some (outputs_list_to_yojson x.outputs)));
    ("nextToken", (option_to_yojson empty_next_token_to_yojson x.next_token))]
let list_environment_outputs_input_to_yojson
  (x : list_environment_outputs_input) =
  assoc_to_yojson
    [("deploymentId",
       (option_to_yojson deployment_id_to_yojson x.deployment_id));
    ("nextToken", (option_to_yojson empty_next_token_to_yojson x.next_token));
...
let environment_account_connection_summary_to_yojson
  (x : environment_account_connection_summary) =
  assoc_to_yojson
    [("componentRoleArn",
       (option_to_yojson arn_to_yojson x.component_role_arn));
    ("status",
...
let list_environment_account_connections_output_to_yojson
  ...
  assoc_to_yojson
    ...
    ("environmentAccountConnections",
      (Some
         (environment_account_connection_summary_list_to_yojson
            x.environment_account_connections)))]
...
let list_environment_account_connections_input_to_yojson
  ...
  assoc_to_yojson
    ...
    ("nextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("statuses",
      (option_to_yojson environment_account_connection_status_list_to_yojson
         x.statuses));
    ("environmentName",
      (option_to_yojson resource_name_to_yojson x.environment_name));
    ("requestedBy",
      (Some
         (environment_account_connection_requester_account_type_to_yojson
            x.requested_by)))]
let deployment_arn_to_yojson = string_to_yojson
let deployment_target_resource_type_to_yojson
  (x : deployment_target_resource_type) =
  match x with
  | ENVIRONMENT -> `String "ENVIRONMENT"
...
let deployment_summary_to_yojson (x : deployment_summary) =
  assoc_to_yojson
    ...
    ("targetResourceType",
      (Some
         (deployment_target_resource_type_to_yojson x.target_resource_type)));
...
let deployment_summary_list_to_yojson tree =
  list_to_yojson deployment_summary_to_yojson tree
...
let component_summary_to_yojson (x : component_summary) =
  assoc_to_yojson
    ...
    ("arn", (Some (component_arn_to_yojson x.arn)));
    ("name", (Some (resource_name_to_yojson x.name)))]
let component_summary_list_to_yojson tree =
  list_to_yojson component_summary_to_yojson tree
...
let list_component_outputs_output_to_yojson
  (x : list_component_outputs_output) =
  assoc_to_yojson
    [("outputs", (Some (outputs_list_to_yojson x.outputs)));
    ("nextToken", (option_to_yojson empty_next_token_to_yojson x.next_token))]
...
let get_service_sync_config_output_to_yojson
  (x : get_service_sync_config_output) =
  assoc_to_yojson
    [("serviceSyncConfig",
       (option_to_yojson service_sync_config_to_yojson x.service_sync_config))]
let get_service_sync_config_input_to_yojson
  (x : get_service_sync_config_input) =
  assoc_to_yojson
    [("serviceName", (Some (resource_name_to_yojson x.service_name)))]
let get_service_sync_blocker_summary_output_to_yojson
  (x : get_service_sync_blocker_summary_output) =
  assoc_to_yojson
    [("serviceSyncBlockerSummary",
       (option_to_yojson service_sync_blocker_summary_to_yojson
          x.service_sync_blocker_summary))]
let get_service_sync_blocker_summary_input_to_yojson
  (x : get_service_sync_blocker_summary_input) =
  assoc_to_yojson
    [("serviceInstanceName",
       (option_to_yojson resource_name_to_yojson x.service_instance_name));
    ("serviceName", (Some (resource_name_to_yojson x.service_name)))]
...
let get_service_instance_sync_status_output_to_yojson
  (x : get_service_instance_sync_status_output) =
  assoc_to_yojson
    [("desiredState", (option_to_yojson revision_to_yojson x.desired_state));
    ("latestSuccessfulSync",
      (option_to_yojson resource_sync_attempt_to_yojson
...
let get_service_instance_sync_status_input_to_yojson
  (x : get_service_instance_sync_status_input) =
  assoc_to_yojson
    [("serviceInstanceName",
       (Some (resource_name_to_yojson x.service_instance_name)));
    ("serviceName", (Some (resource_name_to_yojson x.service_name)))]
...
let counts_summary_to_yojson (x : counts_summary) =
  assoc_to_yojson
    ...
    ("environmentTemplates",
      (option_to_yojson resource_counts_summary_to_yojson
         x.environment_templates));
    ("environments",
...
let get_repository_sync_status_input_to_yojson
  (x : get_repository_sync_status_input) =
  assoc_to_yojson
    [("syncType", (Some (sync_type_to_yojson x.sync_type)));
    ("branch", (Some (git_branch_name_to_yojson x.branch)));
    ("repositoryProvider",
...
let get_environment_template_version_output_to_yojson
  (x : get_environment_template_version_output) =
  assoc_to_yojson
    [("environmentTemplateVersion",
       (Some
          (environment_template_version_to_yojson
...
let get_environment_template_version_input_to_yojson
  (x : get_environment_template_version_input) =
  assoc_to_yojson
    [("minorVersion",
       (Some (template_version_part_to_yojson x.minor_version)));
    ("majorVersion",
...
let get_environment_template_output_to_yojson
  (x : get_environment_template_output) =
  assoc_to_yojson
    [("environmentTemplate",
       (Some (environment_template_to_yojson x.environment_template)))]
let get_environment_template_input_to_yojson
  (x : get_environment_template_input) =
  assoc_to_yojson [("name", (Some (resource_name_to_yojson x.name)))]
...
let deployment_state_to_yojson (x : deployment_state) =
  match x with
  | Component arg ->
      assoc_to_yojson [("component", (Some (component_state_to_yojson arg)))]
  | ServicePipeline arg ->
      assoc_to_yojson
        [("servicePipeline", (Some (service_pipeline_state_to_yojson arg)))]
  | Environment arg ->
      assoc_to_yojson
        [("environment", (Some (environment_state_to_yojson arg)))]
  | ServiceInstance arg ->
      assoc_to_yojson
        [("serviceInstance", (Some (service_instance_state_to_yojson arg)))]
...
let get_deployment_output_to_yojson (x : get_deployment_output) =
  assoc_to_yojson
    [("deployment", (option_to_yojson deployment_to_yojson x.deployment))]
...
let get_component_output_to_yojson (x : get_component_output) =
  assoc_to_yojson
    [("component", (option_to_yojson component_to_yojson x.component))]
...
let delete_template_sync_config_input_to_yojson
  (x : delete_template_sync_config_input) =
  assoc_to_yojson
    [("templateType", (Some (template_type_to_yojson x.template_type)));
    ("templateName", (Some (resource_name_to_yojson x.template_name)))]
let delete_service_template_version_output_to_yojson
  (x : delete_service_template_version_output) =
  assoc_to_yojson
    [("serviceTemplateVersion",
       (option_to_yojson service_template_version_to_yojson
          x.service_template_version))]
let delete_service_template_version_input_to_yojson
  (x : delete_service_template_version_input) =
  assoc_to_yojson
    [("minorVersion",
       (Some (template_version_part_to_yojson x.minor_version)));
    ("majorVersion",
...
let delete_environment_template_version_output_to_yojson
  ...
  assoc_to_yojson
    [("environmentTemplateVersion",
       (option_to_yojson environment_template_version_to_yojson
          x.environment_template_version))]
...
let delete_environment_template_output_to_yojson
  (x : delete_environment_template_output) =
  assoc_to_yojson
    [("environmentTemplate",
       (option_to_yojson environment_template_to_yojson
          x.environment_template))]
let delete_environment_template_input_to_yojson
  (x : delete_environment_template_input) =
  assoc_to_yojson [("name", (Some (resource_name_to_yojson x.name)))]
...
let delete_environment_account_connection_output_to_yojson
  ...
  assoc_to_yojson
    [("environmentAccountConnection",
       (option_to_yojson environment_account_connection_to_yojson
          x.environment_account_connection))]
...
let create_template_sync_config_output_to_yojson
  (x : create_template_sync_config_output) =
  assoc_to_yojson
    [("templateSyncConfig",
       (option_to_yojson template_sync_config_to_yojson
          x.template_sync_config))]
let create_template_sync_config_input_to_yojson
  (x : create_template_sync_config_input) =
  assoc_to_yojson
    [("subdirectory",
       (option_to_yojson subdirectory_to_yojson x.subdirectory));
    ("branch", (Some (git_branch_name_to_yojson x.branch)));
...
let create_service_template_version_output_to_yojson
  (x : create_service_template_version_output) =
  assoc_to_yojson
    [("serviceTemplateVersion",
       (Some (service_template_version_to_yojson x.service_template_version)))]
let create_service_template_version_input_to_yojson
  (x : create_service_template_version_input) =
  assoc_to_yojson
    [("supportedComponentSources",
       (option_to_yojson
          service_template_supported_component_source_input_list_to_yojson
    ...
    ("compatibleEnvironmentTemplates",
      (Some
         (compatible_environment_template_input_list_to_yojson
            x.compatible_environment_templates)));
...
let create_service_template_output_to_yojson
  (x : create_service_template_output) =
  assoc_to_yojson
    [("serviceTemplate",
       (Some (service_template_to_yojson x.service_template)))]
let create_service_template_input_to_yojson
  (x : create_service_template_input) =
  assoc_to_yojson
    [("tags", (option_to_yojson tag_list_to_yojson x.tags));
    ("pipelineProvisioning",
      (option_to_yojson provisioning_to_yojson x.pipeline_provisioning));
...
let create_service_sync_config_output_to_yojson
  (x : create_service_sync_config_output) =
  assoc_to_yojson
    [("serviceSyncConfig",
       (option_to_yojson service_sync_config_to_yojson x.service_sync_config))]
let create_service_sync_config_input_to_yojson
  (x : create_service_sync_config_input) =
  assoc_to_yojson
    [("filePath", (Some (ops_file_path_to_yojson x.file_path)));
    ("branch", (Some (git_branch_name_to_yojson x.branch)));
    ("repositoryName", (Some (repository_name_to_yojson x.repository_name)));
...
let create_service_instance_output_to_yojson
  (x : create_service_instance_output) =
  assoc_to_yojson
    [("serviceInstance",
       (Some (service_instance_to_yojson x.service_instance)))]
let create_service_instance_input_to_yojson
  (x : create_service_instance_input) =
  assoc_to_yojson
    [("clientToken",
       (option_to_yojson client_token_to_yojson x.client_token));
    ("tags", (option_to_yojson tag_list_to_yojson x.tags));
...
let create_environment_template_output_to_yojson
  (x : create_environment_template_output) =
  assoc_to_yojson
    [("environmentTemplate",
       (Some (environment_template_to_yojson x.environment_template)))]
let create_environment_template_input_to_yojson
  (x : create_environment_template_input) =
  assoc_to_yojson
    [("tags", (option_to_yojson tag_list_to_yojson x.tags));
    ("provisioning",
      (option_to_yojson provisioning_to_yojson x.provisioning));
...
let cancel_environment_deployment_output_to_yojson
  (x : cancel_environment_deployment_output) =
  assoc_to_yojson
    [("environment", (Some (environment_to_yojson x.environment)))]
let cancel_environment_deployment_input_to_yojson
  (x : cancel_environment_deployment_input) =
  assoc_to_yojson
    [("environmentName", (Some (resource_name_to_yojson x.environment_name)))]
let cancel_component_deployment_output_to_yojson
  (x : cancel_component_deployment_output) =
  assoc_to_yojson [("component", (Some (component_to_yojson x.component)))]
let cancel_component_deployment_input_to_yojson
  (x : cancel_component_deployment_input) =
  assoc_to_yojson
    [("componentName", (Some (resource_name_to_yojson x.component_name)))]
