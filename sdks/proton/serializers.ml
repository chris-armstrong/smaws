...
let sync_blocker_context_to_yojson (x : sync_blocker_context) =
  assoc_to_yojson
    [ ("value", Some (base_string_to_yojson x.value)); ("key", Some (base_string_to_yojson x.key)) ]
...
let sync_blocker_to_yojson (x : sync_blocker) =
  assoc_to_yojson
    [
      ("resolvedAt", option_to_yojson base_timestamp_to_yojson x.resolved_at);
      ("resolvedReason", option_to_yojson base_string_to_yojson x.resolved_reason);
...
let service_pipeline_to_yojson (x : service_pipeline) =
  assoc_to_yojson
    [
      ...
      ("templateName", Some (resource_name_to_yojson x.template_name));
      ("lastDeploymentSucceededAt", Some (base_timestamp_to_yojson x.last_deployment_succeeded_at));
      ("lastDeploymentAttemptedAt", Some (base_timestamp_to_yojson x.last_deployment_attempted_at));
      ("createdAt", Some (base_timestamp_to_yojson x.created_at));
...
let service_instance_to_yojson (x : service_instance) =
  assoc_to_yojson
    [
      ...
      ( "lastAttemptedDeploymentId",
        option_to_yojson deployment_id_to_yojson x.last_attempted_deployment_id );
      ("lastClientRequestToken", option_to_yojson base_string_to_yojson x.last_client_request_token);
      ("spec", option_to_yojson spec_contents_to_yojson x.spec);
      ( "deploymentStatusMessage",
        option_to_yojson status_message_to_yojson x.deployment_status_message );
      ("deploymentStatus", Some (deployment_status_to_yojson x.deployment_status));
...
let service_sync_blocker_summary_to_yojson (x : service_sync_blocker_summary) =
  assoc_to_yojson
    [
      ...
      ("serviceInstanceName", option_to_yojson base_string_to_yojson x.service_instance_name);
...
let resource_counts_summary_to_yojson (x : resource_counts_summary) =
  assoc_to_yojson
    [
      ("behindMinor", option_to_yojson base_integer_to_yojson x.behind_minor);
...
let deployment_summary_to_yojson (x : deployment_summary) =
  assoc_to_yojson
    [
      ...
      ("serviceName", option_to_yojson resource_name_to_yojson x.service_name);
      ("environmentName", Some (resource_name_to_yojson x.environment_name));
      ("completedAt", option_to_yojson base_timestamp_to_yojson x.completed_at);
      ("lastModifiedAt", Some (base_timestamp_to_yojson x.last_modified_at));
      ("createdAt", Some (base_timestamp_to_yojson x.created_at));
      ("targetResourceType", Some (deployment_target_resource_type_to_yojson x.target_resource_type));
      ("targetResourceCreatedAt", Some (base_timestamp_to_yojson x.target_resource_created_at));
