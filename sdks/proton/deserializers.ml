...
let template_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "TemplateType" value)
...
let repository_provider_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "RepositoryProvider"
             value)
...
let template_sync_config_of_yojson tree path =
  ...
  ({
     subdirectory =
       (option_of_yojson
          (value_for_key subdirectory_of_yojson "subdirectory") _list path);
     ...
     repository_provider =
       (value_for_key repository_provider_of_yojson "repositoryProvider"
          _list path);
     template_type =
       (value_for_key template_type_of_yojson "templateType" _list path);
     template_name =
       (value_for_key resource_name_of_yojson "templateName" _list path)
...
let template_version_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "TemplateVersionStatus"
             value)
    | _ -> raise (deserialize_wrong_type_error path "TemplateVersionStatus") : 
     template_version_status) : template_version_status)
...
let compatible_environment_template_of_yojson tree path =
  ...
  ({
     major_version =
       (value_for_key template_version_part_of_yojson "majorVersion" _list
          path);
     template_name =
       (value_for_key resource_name_of_yojson "templateName" _list path)
...
let service_template_supported_component_source_type_of_yojson (tree : t)
  path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path
             "ServiceTemplateSupportedComponentSourceType") : service_template_supported_component_source_type) : 
  service_template_supported_component_source_type)
let service_template_supported_component_source_input_list_of_yojson tree
  path =
  list_of_yojson service_template_supported_component_source_type_of_yojson
    tree path
let service_template_version_of_yojson tree path =
  ...
  ({
     supported_component_sources =
       (option_of_yojson
          (value_for_key
             service_template_supported_component_source_input_list_of_yojson
             "supportedComponentSources") _list path);
     schema =
       (option_of_yojson (value_for_key template_schema_of_yojson "schema")
          _list path);
     compatible_environment_templates =
       (value_for_key compatible_environment_template_list_of_yojson
          "compatibleEnvironmentTemplates" _list path);
     last_modified_at =
       (value_for_key base_timestamp_of_yojson "lastModifiedAt" _list path);
     created_at =
       (value_for_key base_timestamp_of_yojson "createdAt" _list path);
     arn =
       (value_for_key service_template_version_arn_of_yojson "arn" _list path);
     description =
       (option_of_yojson (value_for_key description_of_yojson "description")
          _list path);
     status_message =
       (option_of_yojson
          (value_for_key status_message_of_yojson "statusMessage") _list path);
     status =
       (value_for_key template_version_status_of_yojson "status" _list path);
     recommended_minor_version =
       (option_of_yojson
          (value_for_key template_version_part_of_yojson
             "recommendedMinorVersion") _list path);
     minor_version =
       (value_for_key template_version_part_of_yojson "minorVersion" _list
          path);
     major_version =
       (value_for_key template_version_part_of_yojson "majorVersion" _list
          path);
...
let update_service_template_version_output_of_yojson tree path =
  ...
  ({
     service_template_version =
       (value_for_key service_template_version_of_yojson
          "serviceTemplateVersion" _list path)
...
let update_service_template_version_input_of_yojson tree path =
  ...
  ({
     ...
     status =
       (option_of_yojson
          (value_for_key template_version_status_of_yojson "status") _list
          path);
     description =
       (option_of_yojson (value_for_key description_of_yojson "description")
          _list path);
...
let provisioning_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "Provisioning" value)
...
let service_template_of_yojson tree path =
  ...
  ({
     pipeline_provisioning =
       (option_of_yojson
          (value_for_key provisioning_of_yojson "pipelineProvisioning") _list
          path);
     encryption_key =
       (option_of_yojson (value_for_key arn_of_yojson "encryptionKey") _list
          path);
     recommended_version =
       (option_of_yojson
          (value_for_key full_template_version_number_of_yojson
             "recommendedVersion") _list path);
     description =
       (option_of_yojson (value_for_key description_of_yojson "description")
          _list path);
     display_name =
       (option_of_yojson (value_for_key display_name_of_yojson "displayName")
          _list path);
     last_modified_at =
       (value_for_key base_timestamp_of_yojson "lastModifiedAt" _list path);
     created_at =
...
let update_service_template_output_of_yojson tree path =
  ...
  ({
     service_template =
       (value_for_key service_template_of_yojson "serviceTemplate" _list path)
...
let service_sync_config_of_yojson tree path =
  ...
  ({
     file_path =
       (value_for_key ops_file_path_of_yojson "filePath" _list path);
     branch = (value_for_key git_branch_name_of_yojson "branch" _list path);
     repository_name =
       (value_for_key repository_name_of_yojson "repositoryName" _list path);
     repository_provider =
       (value_for_key repository_provider_of_yojson "repositoryProvider"
          _list path);
     service_name =
       (value_for_key resource_name_of_yojson "serviceName" _list path)
   } : service_sync_config)
let update_service_sync_config_output_of_yojson tree path =
  ...
  ({
     service_sync_config =
       (option_of_yojson
          (value_for_key service_sync_config_of_yojson "serviceSyncConfig")
          _list path)
...
let sync_blocker_of_yojson tree path =
  ...
  ({
     resolved_at =
       (option_of_yojson
          (value_for_key base_timestamp_of_yojson "resolvedAt") _list path);
     resolved_reason =
       (option_of_yojson
          (value_for_key base_string_of_yojson "resolvedReason") _list path);
     contexts =
       (option_of_yojson
          (value_for_key sync_blocker_contexts_of_yojson "contexts") _list
          path);
     created_at =
       (value_for_key base_timestamp_of_yojson "createdAt" _list path);
     created_reason =
       (value_for_key base_string_of_yojson "createdReason" _list path);
...
let update_service_sync_blocker_output_of_yojson tree path =
  ...
  ({
     service_sync_blocker =
       (value_for_key sync_blocker_of_yojson "serviceSyncBlocker" _list path);
     service_instance_name =
       (option_of_yojson
          (value_for_key resource_name_of_yojson "serviceInstanceName") _list
          path);
     service_name =
       (value_for_key resource_name_of_yojson "serviceName" _list path)
...
let deployment_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "DeploymentStatus" value)
  ...
  ({
     ...
     last_attempted_deployment_id =
       (option_of_yojson
          ...
          _list path);
     spec =
       (option_of_yojson (value_for_key spec_contents_of_yojson "spec") _list
          path);
     deployment_status_message =
       (option_of_yojson
          ...
          _list path);
     deployment_status =
       (value_for_key deployment_status_of_yojson "deploymentStatus" _list
          path);
...
let update_service_pipeline_output_of_yojson tree path =
  ...
  ({
     pipeline =
       (value_for_key service_pipeline_of_yojson "pipeline" _list path)
...
let deployment_update_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "DeploymentUpdateType"
             value)
    | _ -> raise (deserialize_wrong_type_error path "DeploymentUpdateType") : 
     deployment_update_type) : deployment_update_type)
let update_service_pipeline_input_of_yojson tree path =
  ...
  ({
     template_minor_version =
       (option_of_yojson
          (value_for_key template_version_part_of_yojson
             "templateMinorVersion") _list path);
     template_major_version =
       (option_of_yojson
          (value_for_key template_version_part_of_yojson
             "templateMajorVersion") _list path);
     deployment_type =
       (value_for_key deployment_update_type_of_yojson "deploymentType" _list
          path);
...
let service_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String "CREATE_FAILED_CLEANUP_IN_PROGRESS" ->
        CREATE_FAILED_CLEANUP_IN_PROGRESS
    | `String "CREATE_FAILED_CLEANUP_COMPLETE" ->
    ...
    | `String "UPDATE_FAILED_CLEANUP_IN_PROGRESS" ->
        UPDATE_FAILED_CLEANUP_IN_PROGRESS
    | `String "UPDATE_FAILED_CLEANUP_COMPLETE" ->
    ...
    | `String "UPDATE_COMPLETE_CLEANUP_FAILED" ->
        UPDATE_COMPLETE_CLEANUP_FAILED
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "ServiceStatus" value)
...
let service_of_yojson tree path =
  ...
  ({
     branch_name =
       (option_of_yojson
          (value_for_key git_branch_name_of_yojson "branchName") _list path);
     repository_id =
       (option_of_yojson
          (value_for_key repository_id_of_yojson "repositoryId") _list path);
     repository_connection_arn =
       (option_of_yojson
          (value_for_key arn_of_yojson "repositoryConnectionArn") _list path);
     pipeline =
       (option_of_yojson
          (value_for_key service_pipeline_of_yojson "pipeline") _list path);
     spec = (value_for_key spec_contents_of_yojson "spec" _list path);
...
let service_instance_of_yojson tree path =
  ...
  ({
     ...
     last_client_request_token =
       (option_of_yojson
          (value_for_key base_string_of_yojson "lastClientRequestToken")
          _list path);
     spec =
       (option_of_yojson (value_for_key spec_contents_of_yojson "spec") _list
          path);
     ...
     template_name =
       (value_for_key resource_name_of_yojson "templateName" _list path);
     environment_name =
       (value_for_key resource_name_of_yojson "environmentName" _list path);
     service_name =
       (value_for_key resource_name_of_yojson "serviceName" _list path);
     last_deployment_succeeded_at =
       (value_for_key base_timestamp_of_yojson "lastDeploymentSucceededAt"
...
let update_service_instance_output_of_yojson tree path =
  ...
  ({
     service_instance =
       (value_for_key service_instance_of_yojson "serviceInstance" _list path)
...
let environment_template_version_of_yojson tree path =
  ...
  ({
     ...
     created_at =
       (value_for_key base_timestamp_of_yojson "createdAt" _list path);
     arn =
       (value_for_key environment_template_version_arn_of_yojson "arn" _list
          path);
     description =
       (option_of_yojson (value_for_key description_of_yojson "description")
          _list path);
...
let update_environment_template_version_output_of_yojson tree path =
  ...
  ({
     environment_template_version =
       (value_for_key environment_template_version_of_yojson
          "environmentTemplateVersion" _list path)
...
let environment_template_of_yojson tree path =
  ...
  ({
     provisioning =
       (option_of_yojson
          (value_for_key provisioning_of_yojson "provisioning") _list path);
     encryption_key =
       (option_of_yojson (value_for_key arn_of_yojson "encryptionKey") _list
          path);
     recommended_version =
     ...
     created_at =
       (value_for_key base_timestamp_of_yojson "createdAt" _list path);
     arn =
       (value_for_key environment_template_arn_of_yojson "arn" _list path);
     name = (value_for_key resource_name_of_yojson "name" _list path)
...
let repository_branch_of_yojson tree path =
  ...
  ({
     ...
     provider =
       (value_for_key repository_provider_of_yojson "provider" _list path);
...
let environment_of_yojson tree path =
  ...
  ({
     ...
     codebuild_role_arn =
       (option_of_yojson
          (value_for_key role_arn_of_yojson "codebuildRoleArn") _list path);
     component_role_arn =
       (option_of_yojson
          (value_for_key role_arn_of_yojson "componentRoleArn") _list path);
     ...
     environment_account_id =
       (option_of_yojson
          (value_for_key aws_account_id_of_yojson "environmentAccountId")
          _list path);
     environment_account_connection_id =
       (option_of_yojson
          (value_for_key environment_account_connection_id_of_yojson
             "environmentAccountConnectionId") _list path);
     proton_service_role_arn =
       (option_of_yojson (value_for_key arn_of_yojson "protonServiceRoleArn")
          _list path);
     deployment_status_message =
       (option_of_yojson
          (value_for_key status_message_of_yojson "deploymentStatusMessage")
...
let update_environment_output_of_yojson tree path =
  ...
  ({
     environment =
       (value_for_key environment_of_yojson "environment" _list path)
...
let update_environment_input_of_yojson tree path =
  ...
  ({
     ...
     provisioning_repository =
       (option_of_yojson
          (value_for_key repository_branch_input_of_yojson
             "provisioningRepository") _list path);
     environment_account_connection_id =
       (option_of_yojson
          (value_for_key environment_account_connection_id_of_yojson
             "environmentAccountConnectionId") _list path);
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "EnvironmentAccountConnectionStatus" value)
    | _ ->
        raise
          (deserialize_wrong_type_error path
             "EnvironmentAccountConnectionStatus") : environment_account_connection_status) : 
  ...
  ({
     ...
     component_role_arn =
       (option_of_yojson
          (value_for_key role_arn_of_yojson "componentRoleArn") _list path);
     status =
       (value_for_key environment_account_connection_status_of_yojson
          "status" _list path);
     last_modified_at =
       (value_for_key base_timestamp_of_yojson "lastModifiedAt" _list path);
     requested_at =
       (value_for_key base_timestamp_of_yojson "requestedAt" _list path);
     environment_name =
       (value_for_key resource_name_of_yojson "environmentName" _list path);
     role_arn = (value_for_key arn_of_yojson "roleArn" _list path);
     environment_account_id =
       (value_for_key aws_account_id_of_yojson "environmentAccountId" _list
          path);
     management_account_id =
       (value_for_key aws_account_id_of_yojson "managementAccountId" _list
          path);
     arn =
       (value_for_key environment_account_connection_arn_of_yojson "arn"
          _list path);
     id =
       (value_for_key environment_account_connection_id_of_yojson "id" _list
          path)
...
let update_environment_account_connection_output_of_yojson tree path =
  ...
  ({
     environment_account_connection =
       (value_for_key environment_account_connection_of_yojson
          "environmentAccountConnection" _list path)
...
let component_of_yojson tree path =
  ...
  ({
     ...
     service_spec =
       (option_of_yojson
          (value_for_key spec_contents_of_yojson "serviceSpec") _list path);
     deployment_status_message =
       (option_of_yojson
          (value_for_key status_message_of_yojson "deploymentStatusMessage")
          _list path);
...
let component_deployment_update_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path "ComponentDeploymentUpdateType") : 
     component_deployment_update_type) : component_deployment_update_type)
...
let update_component_input_of_yojson tree path =
  ...
  ({
     ...
     template_file =
       (option_of_yojson
          (value_for_key template_file_contents_of_yojson "templateFile")
          _list path);
     service_spec =
       (option_of_yojson
          (value_for_key spec_contents_of_yojson "serviceSpec") _list path);
     service_instance_name =
       (option_of_yojson
          (value_for_key resource_name_or_empty_of_yojson
             "serviceInstanceName") _list path);
     service_name =
       (option_of_yojson
          (value_for_key resource_name_or_empty_of_yojson "serviceName")
          _list path);
     description =
       (option_of_yojson (value_for_key description_of_yojson "description")
          _list path);
     deployment_type =
       (value_for_key component_deployment_update_type_of_yojson
          "deploymentType" _list path);
     name = (value_for_key resource_name_of_yojson "name" _list path)
...
let account_settings_of_yojson tree path =
  ...
  ({
     pipeline_codebuild_role_arn =
       (option_of_yojson
          (value_for_key role_arn_or_empty_string_of_yojson
             "pipelineCodebuildRoleArn") _list path);
     pipeline_provisioning_repository =
       (option_of_yojson
          (value_for_key repository_branch_of_yojson
             "pipelineProvisioningRepository") _list path);
     pipeline_service_role_arn =
       (option_of_yojson
          (value_for_key role_arn_or_empty_string_of_yojson
             "pipelineServiceRoleArn") _list path)
   } : account_settings)
let update_account_settings_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     account_settings =
       (value_for_key account_settings_of_yojson "accountSettings" _list path)
...
let update_account_settings_input_of_yojson tree path =
  ...
  ({
     ...
     delete_pipeline_provisioning_repository =
       (option_of_yojson
          (value_for_key base_boolean_of_yojson
             "deletePipelineProvisioningRepository") _list path);
     pipeline_provisioning_repository =
       (option_of_yojson
          (value_for_key repository_branch_input_of_yojson
             "pipelineProvisioningRepository") _list path);
...
let template_version_source_input_of_yojson (tree : t) path =
  ...
  (match key with
   ...
   | _ as unknown ->
       raise
         (deserialize_unknown_enum_value_error path
            "TemplateVersionSourceInput" unknown) : template_version_source_input)
...
let sync_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "SyncType" value)
    | _ -> raise (deserialize_wrong_type_error path "SyncType") : sync_type) : 
  sync_type)
...
let latest_sync_blockers_of_yojson tree path =
...
let service_sync_blocker_summary_of_yojson tree path =
  ...
  ({
     latest_blockers =
       (option_of_yojson
          (value_for_key latest_sync_blockers_of_yojson "latestBlockers")
          _list path);
     service_instance_name =
       (option_of_yojson
          (value_for_key base_string_of_yojson "serviceInstanceName") _list
          path);
     service_name =
       (value_for_key base_string_of_yojson "serviceName" _list path)
...
let service_summary_list_of_yojson tree path =
...
let service_instance_state_of_yojson tree path =
  ...
  ({
     last_successful_service_pipeline_deployment_id =
       (option_of_yojson
          (value_for_key deployment_id_of_yojson
             "lastSuccessfulServicePipelineDeploymentId") _list path);
     last_successful_environment_deployment_id =
       (option_of_yojson
          (value_for_key deployment_id_of_yojson
             "lastSuccessfulEnvironmentDeploymentId") _list path);
     last_successful_component_deployment_ids =
       (option_of_yojson
          (value_for_key component_deployment_id_list_of_yojson
             "lastSuccessfulComponentDeploymentIds") _list path);
     template_minor_version =
       (value_for_key template_version_part_of_yojson "templateMinorVersion"
          _list path);
     template_major_version =
...
let resource_sync_event_of_yojson tree path =
  ...
  ({
     ...
     external_id =
       (option_of_yojson (value_for_key base_string_of_yojson "externalId")
          _list path);
...
let resource_sync_attempt_of_yojson tree path =
  ...
  ({
     events =
       (value_for_key resource_sync_events_of_yojson "events" _list path);
     status =
       (value_for_key resource_sync_status_of_yojson "status" _list path);
     started_at =
       (value_for_key base_timestamp_of_yojson "startedAt" _list path);
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "ResourceDeploymentStatus" value)
    | _ ->
        raise (deserialize_wrong_type_error path "ResourceDeploymentStatus") : 
     resource_deployment_status) : resource_deployment_status)
let base_integer_of_yojson = int_of_yojson
let resource_counts_summary_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     behind_minor =
       (option_of_yojson (value_for_key base_integer_of_yojson "behindMinor")
          _list path);
     behind_major =
       (option_of_yojson (value_for_key base_integer_of_yojson "behindMajor")
          _list path);
     up_to_date =
       (option_of_yojson (value_for_key base_integer_of_yojson "upToDate")
          _list path);
     failed =
       (option_of_yojson (value_for_key base_integer_of_yojson "failed")
          _list path);
...
let repository_sync_attempt_of_yojson tree path =
  ...
  ({
     events =
       (value_for_key repository_sync_events_of_yojson "events" _list path);
     status =
...
let repository_summary_of_yojson tree path =
  ...
  ({
     connection_arn =
       (value_for_key arn_of_yojson "connectionArn" _list path);
     name = (value_for_key repository_name_of_yojson "name" _list path);
     provider =
       (value_for_key repository_provider_of_yojson "provider" _list path);
     arn = (value_for_key repository_arn_of_yojson "arn" _list path)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "ProvisionedResourceEngine" value)
    | _ ->
        raise (deserialize_wrong_type_error path "ProvisionedResourceEngine") : 
     provisioned_resource_engine) : provisioned_resource_engine)
let provisioned_resource_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     provisioning_engine =
       (option_of_yojson
          (value_for_key provisioned_resource_engine_of_yojson
             "provisioningEngine") _list path);
     identifier =
       (option_of_yojson
          (value_for_key provisioned_resource_identifier_of_yojson
             "identifier") _list path);
     name =
       (option_of_yojson
          (value_for_key provisioned_resource_name_of_yojson "name") _list
          path)
...
let output_of_yojson tree path =
  ...
  ({
     value_string =
       (option_of_yojson
          (value_for_key output_value_string_of_yojson "valueString") _list
          path);
     key =
       (option_of_yojson (value_for_key output_key_of_yojson "key") _list
          path)
   } : output)
...
let notify_resource_deployment_status_change_input_of_yojson tree path =
  ...
  ({
     ...
     deployment_id =
       (option_of_yojson
          (value_for_key deployment_id_of_yojson "deploymentId") _list path);
     outputs =
       (option_of_yojson (value_for_key outputs_list_of_yojson "outputs")
          _list path);
     status =
       (option_of_yojson
          (value_for_key resource_deployment_status_of_yojson "status") _list
          path);
     resource_arn = (value_for_key arn_of_yojson "resourceArn" _list path)
...
let list_tags_for_resource_output_of_yojson tree path =
  ...
  ({
     next_token =
       (option_of_yojson (value_for_key base_string_of_yojson "nextToken")
          _list path);
     tags = (value_for_key tag_list_of_yojson "tags" _list path)
...
let list_tags_for_resource_input_of_yojson tree path =
  ...
  ({
     max_results =
       (option_of_yojson
          (value_for_key max_page_results_of_yojson "maxResults") _list path);
     next_token =
       (option_of_yojson (value_for_key base_string_of_yojson "nextToken")
          _list path);
     resource_arn = (value_for_key arn_of_yojson "resourceArn" _list path)
...
let list_services_output_of_yojson tree path =
  ...
  ({
     services =
       (value_for_key service_summary_list_of_yojson "services" _list path);
     next_token =
...
let list_service_templates_output_of_yojson tree path =
  ...
  ({
     templates =
       (value_for_key service_template_summary_list_of_yojson "templates"
          _list path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let list_service_template_versions_output_of_yojson tree path =
  ...
  ({
     template_versions =
       (value_for_key service_template_version_summary_list_of_yojson
          "templateVersions" _list path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let list_service_template_versions_input_of_yojson tree path =
  ...
  ({
     major_version =
       (option_of_yojson
          (value_for_key template_version_part_of_yojson "majorVersion")
          _list path);
     template_name =
       (value_for_key resource_name_of_yojson "templateName" _list path);
     max_results =
...
let list_service_pipeline_provisioned_resources_output_of_yojson tree path =
  ...
  ({
     provisioned_resources =
       (value_for_key provisioned_resource_list_of_yojson
          "provisionedResources" _list path);
     next_token =
       (option_of_yojson
          (value_for_key empty_next_token_of_yojson "nextToken") _list path)
...
let list_service_instances_sort_by_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path "ListServiceInstancesSortBy") : 
     list_service_instances_sort_by) : list_service_instances_sort_by)
let list_service_instances_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     service_instances =
       (value_for_key service_instance_summary_list_of_yojson
          "serviceInstances" _list path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "ListServiceInstancesFilterBy" value)
    | _ ->
        raise
          (deserialize_wrong_type_error path "ListServiceInstancesFilterBy") : 
     list_service_instances_filter_by) : list_service_instances_filter_by)
...
let list_service_instances_filter_of_yojson tree path =
  ...
  ({
     value =
       (option_of_yojson
          (value_for_key list_service_instances_filter_value_of_yojson
             "value") _list path);
     key =
       (option_of_yojson
          (value_for_key list_service_instances_filter_by_of_yojson "key")
          _list path)
...
let list_service_instances_input_of_yojson tree path =
  ...
  ({
     ...
     sort_by =
       (option_of_yojson
          (value_for_key list_service_instances_sort_by_of_yojson "sortBy")
          _list path);
     filters =
       (option_of_yojson
          (value_for_key list_service_instances_filter_list_of_yojson
             "filters") _list path);
     max_results =
       (option_of_yojson
          (value_for_key max_page_results_of_yojson "maxResults") _list path);
...
let list_repositories_output_of_yojson tree path =
  ...
  ({
     repositories =
       (value_for_key repository_summary_list_of_yojson "repositories" _list
          path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
  ...
  ({
     ...
     last_attempted_deployment_id =
       (option_of_yojson
          ...
          _list path);
     component_role_arn =
       (option_of_yojson (value_for_key arn_of_yojson "componentRoleArn")
          _list path);
     provisioning =
       (option_of_yojson
          (value_for_key provisioning_of_yojson "provisioning") _list path);
...
let list_environments_output_of_yojson tree path =
  ...
  ({
     environments =
       (value_for_key environment_summary_list_of_yojson "environments" _list
          path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let list_environments_input_of_yojson tree path =
  ...
  ({
     environment_templates =
       (option_of_yojson
          (value_for_key environment_template_filter_list_of_yojson
             "environmentTemplates") _list path);
     max_results =
       (option_of_yojson
          (value_for_key max_page_results_of_yojson "maxResults") _list path);
     next_token =
...
let list_environment_templates_output_of_yojson tree path =
  ...
  ({
     templates =
       (value_for_key environment_template_summary_list_of_yojson "templates"
          _list path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let list_environment_template_versions_output_of_yojson tree path =
  ...
  ({
     template_versions =
       (value_for_key environment_template_version_summary_list_of_yojson
          "templateVersions" _list path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path
             "EnvironmentAccountConnectionRequesterAccountType") : environment_account_connection_requester_account_type) : 
...
let list_environment_account_connections_input_of_yojson tree path =
  ...
  ({
     ...
     statuses =
       (option_of_yojson
          (value_for_key environment_account_connection_status_list_of_yojson
             "statuses") _list path);
     environment_name =
       (option_of_yojson
          (value_for_key resource_name_of_yojson "environmentName") _list
          path);
     requested_by =
       (value_for_key
          environment_account_connection_requester_account_type_of_yojson
          "requestedBy" _list path)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "DeploymentTargetResourceType" value)
    | _ ->
        raise
          (deserialize_wrong_type_error path "DeploymentTargetResourceType") : 
...
let deployment_summary_of_yojson tree path =
  ...
  ({
     ...
     component_name =
       (option_of_yojson
          (value_for_key resource_name_of_yojson "componentName") _list path);
     service_instance_name =
       (option_of_yojson
          (value_for_key resource_name_of_yojson "serviceInstanceName") _list
          path);
     ...
     completed_at =
       (option_of_yojson
          (value_for_key base_timestamp_of_yojson "completedAt") _list path);
     last_modified_at =
       (value_for_key base_timestamp_of_yojson "lastModifiedAt" _list path);
     created_at =
       (value_for_key base_timestamp_of_yojson "createdAt" _list path);
     target_resource_type =
       (value_for_key deployment_target_resource_type_of_yojson
          "targetResourceType" _list path);
     target_resource_created_at =
       (value_for_key base_timestamp_of_yojson "targetResourceCreatedAt"
          _list path);
...
let list_deployments_output_of_yojson tree path =
  ...
  ({
     deployments =
       (value_for_key deployment_summary_list_of_yojson "deployments" _list
          path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let list_components_output_of_yojson tree path =
  ...
  ({
     components =
       (value_for_key component_summary_list_of_yojson "components" _list
          path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "nextToken")
          _list path)
...
let get_template_sync_status_output_of_yojson tree path =
  ...
  ({
     ...
     latest_sync =
       (option_of_yojson
          (value_for_key resource_sync_attempt_of_yojson "latestSync") _list
          path)
...
let get_template_sync_status_input_of_yojson tree path =
  ...
  ({
     template_version =
       (value_for_key template_version_part_of_yojson "templateVersion" _list
          path);
     template_type =
       (value_for_key template_type_of_yojson "templateType" _list path);
     template_name =
...
let get_service_sync_blocker_summary_output_of_yojson tree path =
  ...
  ({
     service_sync_blocker_summary =
       (option_of_yojson
          (value_for_key service_sync_blocker_summary_of_yojson
             "serviceSyncBlockerSummary") _list path)
...
let counts_summary_of_yojson tree path =
  ...
  ({
     pipelines =
       (option_of_yojson
          (value_for_key resource_counts_summary_of_yojson "pipelines") _list
          path);
     service_templates =
       (option_of_yojson
     ...
     services =
       (option_of_yojson
          (value_for_key resource_counts_summary_of_yojson "services") _list
          path);
     service_instances =
       (option_of_yojson
     ...
     environment_templates =
       (option_of_yojson
          (value_for_key resource_counts_summary_of_yojson
             "environmentTemplates") _list path);
     environments =
       (option_of_yojson
          (value_for_key resource_counts_summary_of_yojson "environments")
          _list path);
     components =
       (option_of_yojson
          (value_for_key resource_counts_summary_of_yojson "components")
          _list path)
   } : counts_summary)
...
let get_repository_sync_status_output_of_yojson tree path =
  ...
  ({
     latest_sync =
       (option_of_yojson
          (value_for_key repository_sync_attempt_of_yojson "latestSync")
          _list path)
...
let get_repository_output_of_yojson tree path =
  ...
  ({
     repository =
       (value_for_key repository_of_yojson "repository" _list path)
...
let deployment_state_of_yojson (tree : t) path =
  ...
  (match key with
   ...
   | "servicePipeline" ->
       ServicePipeline (service_pipeline_state_of_yojson value_ path)
   | "environment" -> Environment (environment_state_of_yojson value_ path)
   | "serviceInstance" ->
       ServiceInstance (service_instance_state_of_yojson value_ path)
   | _ as unknown ->
       raise
         (deserialize_unknown_enum_value_error path "DeploymentState" unknown) : 
    deployment_state)
let deployment_of_yojson tree path =
  ...
  ({
     target_state =
       (option_of_yojson
          (value_for_key deployment_state_of_yojson "targetState") _list path);
     initial_state =
       (option_of_yojson
          (value_for_key deployment_state_of_yojson "initialState") _list
          path);
     last_succeeded_deployment_id =
       (option_of_yojson
          (value_for_key deployment_id_of_yojson "lastSucceededDeploymentId")
...
let get_deployment_output_of_yojson tree path =
  ...
  ({
     deployment =
       (option_of_yojson (value_for_key deployment_of_yojson "deployment")
          _list path)
...
let get_component_output_of_yojson tree path =
  ...
  ({
     component =
       (option_of_yojson (value_for_key component_of_yojson "component")
          _list path)
...
let get_account_settings_output_of_yojson tree path =
  ...
  ({
     account_settings =
       (option_of_yojson
          (value_for_key account_settings_of_yojson "accountSettings") _list
          path)
...
let delete_service_template_output_of_yojson tree path =
  ...
  ({
     service_template =
       (option_of_yojson
          (value_for_key service_template_of_yojson "serviceTemplate") _list
          path)
...
let delete_repository_output_of_yojson tree path =
  ...
  ({
     repository =
       (option_of_yojson (value_for_key repository_of_yojson "repository")
          _list path)
  ...
  ({
     ...
     compatible_environment_templates =
       (value_for_key compatible_environment_template_input_list_of_yojson
          "compatibleEnvironmentTemplates" _list path);
     source =
       (value_for_key template_version_source_input_of_yojson "source" _list
          path);
     major_version =
       (option_of_yojson
          (value_for_key template_version_part_of_yojson "majorVersion")
  ...
  ({
     ...
     service_spec =
       (option_of_yojson
          (value_for_key spec_contents_of_yojson "serviceSpec") _list path);
     manifest =
       (value_for_key template_manifest_contents_of_yojson "manifest" _list
          path);
     template_file =
       (value_for_key template_file_contents_of_yojson "templateFile" _list
          path);
     environment_name =
       (option_of_yojson
          (value_for_key resource_name_of_yojson "environmentName") _list
