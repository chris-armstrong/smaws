...
type nonrec optimistic_lock_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec locked_subscription_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "You are trying to update a subscription that has not yet completed the 1-year commitment. You \
...
type nonrec emergency_contact_list = emergency_contact list [@@ocaml.doc ""]

type nonrec update_emergency_contact_settings_request = {
  emergency_contact_list : emergency_contact_list option;
      [@ocaml.doc
        "A list of email addresses and phone numbers that the Shield Response Team (SRT) can use \
         to contact you if you have proactive engagement enabled, for escalations to the SRT and \
...
[@@ocaml.doc
  "Specifies the action setting that Shield Advanced should use in the WAF rules that it creates \
   on behalf of the protected resource in response to DDoS attacks. You specify this as part of \
   the configuration for the automatic application layer DDoS mitigation feature, when you enable \
   or update automatic mitigation. Shield Advanced creates the WAF rules in a Shield \
  ...
  action : response_action;
      [@ocaml.doc
        "Specifies the action setting that Shield Advanced should use in the WAF rules that it \
         creates on behalf of the protected resource in response to DDoS attacks. You specify this \
         as part of the configuration for the automatic application layer DDoS mitigation feature, \
...
type nonrec invalid_resource_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
...
[@@ocaml.doc
  "Narrows the set of protection groups that the call retrieves. You can retrieve a single \
   ...
   with a specific pattern, aggregation, or resource type. You can provide up to one criteria per \
   filter type. Shield Advanced returns the protection groups that exactly match all of the search \
...
type nonrec get_subscription_state_request = unit [@@ocaml.doc ""]

type nonrec enable_proactive_engagement_response = unit [@@ocaml.doc ""]
...
type nonrec disassociate_drt_role_response = unit [@@ocaml.doc ""]
...
[@@ocaml.doc
  "In order to grant the necessary access to the Shield Response Team (SRT) the user submitting \
   the request must have the [iam:PassRole] permission. This error indicates the user did not have \
...
type nonrec disable_proactive_engagement_request = unit [@@ocaml.doc ""]

type nonrec disable_application_layer_automatic_response_response = unit [@@ocaml.doc ""]
  ...
  protection_group : protection_group;
      [@ocaml.doc
        "A grouping of protected resources that you and Shield Advanced can monitor as a \
         collective. This resource grouping improves the accuracy of detection and reduces false \
         positives. \n"]
}
[@@ocaml.doc ""]
...
[@@ocaml.doc
  "Information about the volume of attacks during the time period, included in an \
   [AttackStatisticsDataItem]. If the accompanying [AttackCount] in the statistics object is zero, \
...
[@@ocaml.doc
  "A single attack statistics data record. This is returned by [DescribeAttackStatistics] along \
   with a time range indicating the time period that the attack statistics apply to. \n"]
  ...
  attack_layer : attack_layer option;
      [@ocaml.doc
        "The type of Shield event that was observed. [NETWORK] indicates layer 3 and layer 4 \
         events and [APPLICATION] indicates layer 7 events.\n\n\
        \ For infrastructure layer events (L3 and L4 events), you can view metrics for top \
         contributors in Amazon CloudWatch metrics. For more information, see \
...
type nonrec describe_attack_response = {
  attack : attack_detail option; [@ocaml.doc "The attack that you requested. \n"]
}
[@@ocaml.doc ""]

type nonrec describe_attack_request = {
  attack_id : attack_id; [@ocaml.doc "The unique identifier (ID) for the attack.\n"]
}
[@@ocaml.doc ""]
...
type nonrec create_protection_response = {
  protection_id : protection_id option;
      [@ocaml.doc "The unique identifier (ID) for the [Protection] object that is created.\n"]
}
[@@ocaml.doc ""]
...
type nonrec create_protection_request = {
  ...
  resource_arn : resource_arn;
      [@ocaml.doc
        ...
        \            {[\n\
        \            arn:aws:elasticloadbalancing:{i region}:{i account-id}:loadbalancer/app/{i \
         load-balancer-name}/{i load-balancer-id} \n\
        \            ]}\n\
        \             \n\
        \            \n\
        \             }\n\
        ...
        \            {[\n\
        \            arn:aws:elasticloadbalancing:{i region}:{i account-id}:loadbalancer/{i \
         load-balancer-name} \n\
        \            ]}\n\
        \             \n\
        \            \n\
...
type nonrec associate_proactive_engagement_details_response = unit [@@ocaml.doc ""]
...
type nonrec associate_drt_role_response = unit [@@ocaml.doc ""]

type nonrec associate_drt_role_request = {
  role_arn : role_arn;
      [@ocaml.doc
        "The Amazon Resource Name (ARN) of the role the SRT will use to access your Amazon Web \
         Services account.\n\n\
...
type nonrec associate_drt_log_bucket_response = unit [@@ocaml.doc ""]
