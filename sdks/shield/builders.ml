...
let make_update_protection_group_request ?members:(members_ : protection_group_members option)
...
let make_emergency_contact ?contact_notes:(contact_notes_ : contact_notes option)
    ?phone_number:(phone_number_ : phone_number option)
    ~email_address:(email_address_ : email_address) () =
  ({ contact_notes = contact_notes_; phone_number = phone_number_; email_address = email_address_ }
    : emergency_contact)
...
let make_response_action ?count:(count_ : count_action option) ?block:(block_ : block_action option)
    () =
  ({ count = count_; block = block_ } : response_action)

let make_update_application_layer_automatic_response_request ~action:(action_ : response_action)
    ~resource_arn:(resource_arn_ : resource_arn) () =
  ({ action = action_; resource_arn = resource_arn_ }
...
let make_contributor ?value:(value_ : long option) ?name:(name_ : string_ option) () =
...
let make_time_range ?to_exclusive:(to_exclusive_ : timestamp option)
  ...
  ({ to_exclusive = to_exclusive_; from_inclusive = from_inclusive_ } : time_range)
...
let make_summarized_counter ?unit_:(unit__ : string_ option) ?n:(n_ : integer option)
    ?sum:(sum_ : double option) ?average:(average_ : double option) ?max:(max_ : double option)
    ?name:(name_ : string_ option) () =
  ({ unit_ = unit__; n = n_; sum = sum_; average = average_; max = max_; name = name_ }
    : summarized_counter)
...
let make_summarized_attack_vector
  ...
  ({ vector_counters = vector_counters_; vector_type = vector_type_ } : summarized_attack_vector)
...
let make_protection_group_limits
  ...
  ({ pattern_type_limits = pattern_type_limits_; max_protection_groups = max_protection_groups_ }
...
let make_subscription_limits
  ...
  ({ protection_group_limits = protection_group_limits_; protection_limits = protection_limits_ }
    : subscription_limits)

let make_subscription ?subscription_arn:(subscription_arn_ : resource_arn option)
    ?proactive_engagement_status:(proactive_engagement_status_ : proactive_engagement_status option)
    ?limits:(limits_ : limits option) ?auto_renew:(auto_renew_ : auto_renew option)
...
let make_sub_resource_summary ?counters:(counters_ : summarized_counter_list option)
  ...
  ({ counters = counters_; attack_vectors = attack_vectors_; id = id_; type_ = type__ }
    : sub_resource_summary)

let make_application_layer_automatic_response_configuration ~action:(action_ : response_action)
...
let make_protection
    ...
    ?resource_arn:(resource_arn_ : resource_arn option) ?name:(name_ : protection_name option)
    ?id:(id_ : protection_id option) () =
  ({
     application_layer_automatic_response_configuration =
       application_layer_automatic_response_configuration_;
...
let make_protection_group ?protection_group_arn:(protection_group_arn_ : resource_arn option)
    ?resource_type:(resource_type_ : protected_resource_type option)
    ~members:(members_ : protection_group_members) ~pattern:(pattern_ : protection_group_pattern)
    ~aggregation:(aggregation_ : protection_group_aggregation)
    ~protection_group_id:(protection_group_id_ : protection_group_id) () =
  ({
...
let make_list_resources_in_protection_group_response ?next_token:(next_token_ : token option)
  ...
  ({ next_token = next_token_; resource_arns = resource_arns_ }
    : list_resources_in_protection_group_response)

let make_list_resources_in_protection_group_request ?max_results:(max_results_ : max_results option)
...
let make_list_protections_request
  ...
  ({ inclusion_filters = inclusion_filters_; max_results = max_results_; next_token = next_token_ }
...
let make_attack_summary ?attack_vectors:(attack_vectors_ : attack_vector_description_list option)
    ?end_time:(end_time_ : attack_timestamp option)
    ?start_time:(start_time_ : attack_timestamp option)
    ?resource_arn:(resource_arn_ : string_ option) ?attack_id:(attack_id_ : string_ option) () =
  ({
...
let make_list_attacks_response ?next_token:(next_token_ : token option)
  ...
  ({ next_token = next_token_; attack_summaries = attack_summaries_ } : list_attacks_response)

let make_list_attacks_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : token option) ?end_time:(end_time_ : time_range option)
...
let make_enable_application_layer_automatic_response_request ~action:(action_ : response_action)
    ~resource_arn:(resource_arn_ : resource_arn) () =
  ({ action = action_; resource_arn = resource_arn_ }
...
let make_disassociate_health_check_request ~health_check_arn:(health_check_arn_ : health_check_arn)
...
let make_describe_protection_request ?resource_arn:(resource_arn_ : resource_arn option)
    ?protection_id:(protection_id_ : protection_id option) () =
  ({ resource_arn = resource_arn_; protection_id = protection_id_ } : describe_protection_request)
...
let make_describe_drt_access_response ?log_bucket_list:(log_bucket_list_ : log_bucket_list option)
...
let make_attack_volume ?requests_per_second:(requests_per_second_ : attack_volume_statistics option)
...
let make_attack_statistics_data_item ?attack_volume:(attack_volume_ : attack_volume option)
  ...
  ({ attack_count = attack_count_; attack_volume = attack_volume_ } : attack_statistics_data_item)

let make_describe_attack_statistics_response ~data_items:(data_items_ : attack_statistics_data_list)
...
let make_attack_property ?total:(total_ : long option) ?unit_:(unit__ : unit_ option)
...
let make_attack_detail ?mitigations:(mitigations_ : mitigation_list option)
    ...
    ?resource_arn:(resource_arn_ : resource_arn option) ?attack_id:(attack_id_ : attack_id option)
    () =
  ({
...
let make_create_protection_request ?tags:(tags_ : tag_list option)
    ~resource_arn:(resource_arn_ : resource_arn) ~name:(name_ : protection_name) () =
...
let make_associate_health_check_request ~health_check_arn:(health_check_arn_ : health_check_arn)
    ~protection_id:(protection_id_ : protection_id) () =
  ({ health_check_arn = health_check_arn_; protection_id = protection_id_ }
