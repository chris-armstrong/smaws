...
let protection_group_aggregation_to_yojson (x : protection_group_aggregation) =
  match x with MAX -> `String "MAX" | MEAN -> `String "MEAN" | SUM -> `String "SUM"
...
let time_range_to_yojson (x : time_range) =
  assoc_to_yojson
    [
      ...
      ("FromInclusive", option_to_yojson timestamp__to_yojson x.from_inclusive);
    ]
...
let attack_layer_to_yojson (x : attack_layer) =
  match x with APPLICATION -> `String "APPLICATION" | NETWORK -> `String "NETWORK"
