open Types
let make_single_header ~name:(name_ : field_to_match_data) () =
...
let make_custom_request_handling
  ~insert_headers:(insert_headers_ : custom_http_headers) () =
...
let make_excluded_rule ~name:(name_ : entity_name) () =
...
let make_aws_managed_rules_bot_control_rule_set
  ?enable_machine_learning:(enable_machine_learning_ :
                             enable_machine_learning option)
...
let make_response_inspection_status_code
  ~failure_codes:(failure_codes_ :
                   response_inspection_status_code_failure_codes)
  ~success_codes:(success_codes_ :
                   response_inspection_status_code_success_codes)
  () =
...
let make_response_inspection_header
  ~failure_values:(failure_values_ :
                    response_inspection_header_failure_values)
  ~success_values:(success_values_ :
                    response_inspection_header_success_values)
...
let make_response_inspection_body_contains
  ~failure_strings:(failure_strings_ :
                     response_inspection_body_contains_failure_strings)
  ~success_strings:(success_strings_ :
                     response_inspection_body_contains_success_strings)
  () =
...
let make_client_side_action
  ?exempt_uri_regular_expressions:(exempt_uri_regular_expressions_ :
                                    regular_expression_list option)
...
let make_label_match_statement ~key:(key_ : label_match_key)
...
let make_managed_rule_group_statement
  ...
  ?managed_rule_group_configs:(managed_rule_group_configs_ :
                                managed_rule_group_configs option)
...
let make_immunity_time_property
  ~immunity_time:(immunity_time_ : time_window_second) () =
...
let make_rule ?challenge_config:(challenge_config_ : challenge_config option)
  ...
  ~statement:(statement_ : statement) ~priority:(priority_ : rule_priority)
  ~name:(name_ : entity_name) () =
  ({
...
let make_field_to_protect
  ?field_keys:(field_keys_ : field_to_protect_keys option)
...
let make_data_protection_config
  ~data_protections:(data_protections_ : data_protections) () =
...
let make_application_config
  ?attributes:(attributes_ : application_attributes option) () =
...
let make_web_ac_l
  ...
  ~ar_n:(ar_n_ : resource_arn) ~id:(id_ : entity_id)
  ~name:(name_ : entity_name) () =
  ({
...
let make_update_web_acl_request
  ...
  ~visibility_config:(visibility_config_ : visibility_config)
  ~default_action:(default_action_ : default_action) ~id:(id_ : entity_id)
...
let make_update_managed_rule_set_version_expiry_date_request
  ...
  ~lock_token:(lock_token_ : lock_token) ~id:(id_ : entity_id)
  ~scope:(scope_ : scope) ~name:(name_ : entity_name) () =
  ({
     expiry_timestamp = expiry_timestamp_;
...
let make_update_ip_set_request
  ?description:(description_ : entity_description option)
  ~lock_token:(lock_token_ : lock_token)
  ~addresses:(addresses_ : ip_addresses) ~id:(id_ : entity_id)
  ~scope:(scope_ : scope) ~name:(name_ : entity_name) () =
  ({
     lock_token = lock_token_;
...
let make_sampled_http_request
  ?overridden_action:(overridden_action_ : action option)
...
let make_label_summary ?name:(name_ : label_name option) () =
...
let make_rule_group
  ...
  ?description:(description_ : entity_description option)
  ~visibility_config:(visibility_config_ : visibility_config)
  ~ar_n:(ar_n_ : resource_arn) ~capacity:(capacity_ : capacity_unit)
...
let make_regex_pattern_set
  ?regular_expression_list:(regular_expression_list_ :
                             regular_expression_list option)
  ?description:(description_ : entity_description option)
  ?ar_n:(ar_n_ : resource_arn option) ?id:(id_ : entity_id option)
  ?name:(name_ : entity_name option) () =
  ({
     regular_expression_list = regular_expression_list_;
     description = description_;
...
let make_managed_rule_set_summary
  ?label_namespace:(label_namespace_ : label_name option)
  ?ar_n:(ar_n_ : resource_arn option)
  ?lock_token:(lock_token_ : lock_token option)
  ?description:(description_ : entity_description option)
  ?id:(id_ : entity_id option) ?name:(name_ : entity_name option) () =
  ({
...
let make_list_available_managed_rule_groups_response
  ?managed_rule_groups:(managed_rule_groups_ :
                         managed_rule_group_summaries option)
  ?next_marker:(next_marker_ : next_marker option) () =
...
let make_get_web_acl_request ?ar_n:(ar_n_ : resource_arn option)
  ?id:(id_ : entity_id option) ?scope:(scope_ : scope option)
  ?name:(name_ : entity_name option) () =
...
let make_get_sampled_requests_request
  ~max_items:(max_items_ : list_max_items)
  ~time_window:(time_window_ : time_window) ~scope:(scope_ : scope)
  ~rule_metric_name:(rule_metric_name_ : metric_name)
  ~web_acl_arn:(web_acl_arn_ : resource_arn) () =
  ({
...
let make_get_rule_group_request ?ar_n:(ar_n_ : resource_arn option)
  ?id:(id_ : entity_id option) ?scope:(scope_ : scope option)
  ?name:(name_ : entity_name option) () =
...
let make_get_regex_pattern_set_request ~id:(id_ : entity_id)
  ~scope:(scope_ : scope) ~name:(name_ : entity_name) () =
...
let make_get_managed_rule_set_request ~id:(id_ : entity_id)
  ~scope:(scope_ : scope) ~name:(name_ : entity_name) () =
...
let make_get_decrypted_api_key_request ~api_key:(api_key_ : api_key)
  ~scope:(scope_ : scope) () =
  ({ api_key = api_key_; scope = scope_ } : get_decrypted_api_key_request)
let make_generate_mobile_sdk_release_url_response
  ?url:(url_ : download_url option) () =
...
let make_disassociate_web_acl_request
  ~resource_arn:(resource_arn_ : resource_arn) () =
...
let make_delete_web_acl_request ~lock_token:(lock_token_ : lock_token)
  ~id:(id_ : entity_id) ~scope:(scope_ : scope) ~name:(name_ : entity_name)
  () =
  ({ lock_token = lock_token_; id = id_; scope = scope_; name = name_ } : 
  delete_web_acl_request)
let make_delete_rule_group_response () = (() : unit)
let make_delete_rule_group_request ~lock_token:(lock_token_ : lock_token)
  ~id:(id_ : entity_id) ~scope:(scope_ : scope) ~name:(name_ : entity_name)
  () =
  ({ lock_token = lock_token_; id = id_; scope = scope_; name = name_ } : 
...
let make_delete_ip_set_request ~lock_token:(lock_token_ : lock_token)
  ~id:(id_ : entity_id) ~scope:(scope_ : scope) ~name:(name_ : entity_name)
  () =
  ({ lock_token = lock_token_; id = id_; scope = scope_; name = name_ } : 
...
let make_delete_api_key_request ~api_key:(api_key_ : api_key)
  ~scope:(scope_ : scope) () =
...
let make_create_rule_group_request
  ...
  ?description:(description_ : entity_description option)
  ~visibility_config:(visibility_config_ : visibility_config)
  ~capacity:(capacity_ : capacity_unit) ~scope:(scope_ : scope)
  ~name:(name_ : entity_name) () =
  ({
     custom_response_bodies = custom_response_bodies_;
     tags = tags_;
...
let make_check_capacity_response
  ?capacity:(capacity_ : consumed_capacity option) () =
  ({ capacity = capacity_ } : check_capacity_response)
let make_check_capacity_request ~rules:(rules_ : rules)
  ~scope:(scope_ : scope) () =
