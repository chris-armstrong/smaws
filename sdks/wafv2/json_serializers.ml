...
let body_parsing_fallback_behavior_to_yojson
  (x : body_parsing_fallback_behavior) =
  match x with
...
let json_body_to_yojson (x : json_body) =
  assoc_to_yojson
    ...
    ("InvalidFallbackBehavior",
      (option_to_yojson body_parsing_fallback_behavior_to_yojson
         x.invalid_fallback_behavior));
    ("MatchScope", (Some (json_match_scope_to_yojson x.match_scope)));
    ("MatchPattern", (Some (json_match_pattern_to_yojson x.match_pattern)))]
let header_names_to_yojson tree =
  list_to_yojson field_to_match_data_to_yojson tree
...
let cookie_names_to_yojson tree =
  list_to_yojson single_cookie_name_to_yojson tree
...
let field_to_match_to_yojson (x : field_to_match) =
  assoc_to_yojson
    ...
    ("SingleQueryArgument",
      (option_to_yojson single_query_argument_to_yojson
         x.single_query_argument));
    ("SingleHeader",
...
let text_transformations_to_yojson tree =
  list_to_yojson text_transformation_to_yojson tree
...
let web_acl_summaries_to_yojson tree =
  list_to_yojson web_acl_summary_to_yojson tree
...
let custom_http_headers_to_yojson tree =
  list_to_yojson custom_http_header_to_yojson tree
...
let allow_action_to_yojson (x : allow_action) =
  assoc_to_yojson
    [("CustomRequestHandling",
       (option_to_yojson custom_request_handling_to_yojson
          x.custom_request_handling))]
...
let sensitivity_level_to_yojson (x : sensitivity_level) =
  match x with | HIGH -> `String "HIGH" | LOW -> `String "LOW"
...
let excluded_rules_to_yojson tree =
  list_to_yojson excluded_rule_to_yojson tree
...
let rule_action_overrides_to_yojson tree =
  list_to_yojson rule_action_override_to_yojson tree
let rule_group_reference_statement_to_yojson
  (x : rule_group_reference_statement) =
  assoc_to_yojson
    [("RuleActionOverrides",
       (option_to_yojson rule_action_overrides_to_yojson
          x.rule_action_overrides));
    ("ExcludedRules",
...
let ip_set_reference_statement_to_yojson (x : ip_set_reference_statement) =
  assoc_to_yojson
    [("IPSetForwardedIPConfig",
       (option_to_yojson ip_set_forwarded_ip_config_to_yojson
          x.ip_set_forwarded_ip_config));
    ("ARN", (Some (resource_arn_to_yojson x.ar_n)))]
...
let rate_based_statement_aggregate_key_type_to_yojson
  (x : rate_based_statement_aggregate_key_type) =
  match x with
...
let rate_limit_label_namespace_to_yojson (x : rate_limit_label_namespace) =
  assoc_to_yojson
    [("Namespace", (Some (label_namespace_to_yojson x.namespace)))]
...
let inspection_level_to_yojson (x : inspection_level) =
  match x with | TARGETED -> `String "TARGETED" | COMMON -> `String "COMMON"
...
let aws_managed_rules_bot_control_rule_set_to_yojson
  ...
  assoc_to_yojson
    [("EnableMachineLearning",
       (option_to_yojson enable_machine_learning_to_yojson
          x.enable_machine_learning));
    ("InspectionLevel",
...
let response_inspection_status_code_to_yojson
  (x : response_inspection_status_code) =
  assoc_to_yojson
    [("FailureCodes",
       (Some
          (response_inspection_status_code_failure_codes_to_yojson
             x.failure_codes)));
    ("SuccessCodes",
      (Some
         (response_inspection_status_code_success_codes_to_yojson
            x.success_codes)))]
...
let response_inspection_header_to_yojson (x : response_inspection_header) =
  assoc_to_yojson
    [("FailureValues",
       (Some
          (response_inspection_header_failure_values_to_yojson
             x.failure_values)));
    ("SuccessValues",
      (Some
         (response_inspection_header_success_values_to_yojson
            x.success_values)));
...
let response_inspection_body_contains_to_yojson
  (x : response_inspection_body_contains) =
  assoc_to_yojson
    [("FailureStrings",
       (Some
          (response_inspection_body_contains_failure_strings_to_yojson
             x.failure_strings)));
    ("SuccessStrings",
      (Some
         (response_inspection_body_contains_success_strings_to_yojson
            x.success_strings)))]
...
let response_inspection_json_to_yojson (x : response_inspection_json) =
  assoc_to_yojson
    [("FailureValues",
       (Some
          (response_inspection_json_failure_values_to_yojson x.failure_values)));
    ("SuccessValues",
      (Some
         (response_inspection_json_success_values_to_yojson x.success_values)));
    ("Identifier", (Some (field_identifier_to_yojson x.identifier)))]
let response_inspection_to_yojson (x : response_inspection) =
  assoc_to_yojson
    [("Json", (option_to_yojson response_inspection_json_to_yojson x.json));
    ("BodyContains",
      (option_to_yojson response_inspection_body_contains_to_yojson
         x.body_contains));
    ("Header",
      (option_to_yojson response_inspection_header_to_yojson x.header));
    ("StatusCode",
      (option_to_yojson response_inspection_status_code_to_yojson
         x.status_code))]
...
let phone_number_field_to_yojson (x : phone_number_field) =
  assoc_to_yojson
    [("Identifier", (Some (field_identifier_to_yojson x.identifier)))]
let phone_number_fields_to_yojson tree =
  list_to_yojson phone_number_field_to_yojson tree
let address_field_to_yojson (x : address_field) =
  assoc_to_yojson
    [("Identifier", (Some (field_identifier_to_yojson x.identifier)))]
let address_fields_to_yojson tree =
  list_to_yojson address_field_to_yojson tree
...
let label_match_scope_to_yojson (x : label_match_scope) =
  match x with | NAMESPACE -> `String "NAMESPACE" | LABEL -> `String "LABEL"
...
and managed_rule_group_statement_to_yojson (x : managed_rule_group_statement)
  ...
  assoc_to_yojson
    ...
    ("ManagedRuleGroupConfigs",
      (option_to_yojson managed_rule_group_configs_to_yojson
         x.managed_rule_group_configs));
    ("ScopeDownStatement",
...
and rate_based_statement_to_yojson (x : rate_based_statement) =
  assoc_to_yojson
    [("CustomKeys",
       (option_to_yojson rate_based_statement_custom_keys_to_yojson
          x.custom_keys));
    ("ForwardedIPConfig",
      (option_to_yojson forwarded_ip_config_to_yojson x.forwarded_ip_config));
    ("ScopeDownStatement",
    ...
    ("AggregateKeyType",
      (Some
         (rate_based_statement_aggregate_key_type_to_yojson
            x.aggregate_key_type)));
    ("EvaluationWindowSec",
      (option_to_yojson evaluation_window_sec_to_yojson
         x.evaluation_window_sec));
...
and statement_to_yojson (x : statement) =
  assoc_to_yojson
    ...
    ("ManagedRuleGroupStatement",
      (option_to_yojson managed_rule_group_statement_to_yojson
         x.managed_rule_group_statement));
    ("NotStatement",
    ...
    ("IPSetReferenceStatement",
      (option_to_yojson ip_set_reference_statement_to_yojson
         x.ip_set_reference_statement));
    ("RuleGroupReferenceStatement",
      (option_to_yojson rule_group_reference_statement_to_yojson
         x.rule_group_reference_statement));
    ("GeoMatchStatement",
      (option_to_yojson geo_match_statement_to_yojson x.geo_match_statement));
    ("SizeConstraintStatement",
      (option_to_yojson size_constraint_statement_to_yojson
         x.size_constraint_statement));
    ("XssMatchStatement",
...
let label_to_yojson (x : label) =
  assoc_to_yojson [("Name", (Some (label_name_to_yojson x.name)))]
...
let immunity_time_property_to_yojson (x : immunity_time_property) =
  assoc_to_yojson
    [("ImmunityTime", (Some (time_window_second_to_yojson x.immunity_time)))]
let captcha_config_to_yojson (x : captcha_config) =
  assoc_to_yojson
    [("ImmunityTimeProperty",
       (option_to_yojson immunity_time_property_to_yojson
          x.immunity_time_property))]
...
let firewall_manager_rule_group_to_yojson (x : firewall_manager_rule_group) =
  assoc_to_yojson
    ...
    ("FirewallManagerStatement",
      (Some
         (firewall_manager_statement_to_yojson x.firewall_manager_statement)));
    ("Priority", (Some (rule_priority_to_yojson x.priority)));
    ("Name", (Some (entity_name_to_yojson x.name)))]
...
let request_body_associated_resource_type_config_to_yojson
  ...
  assoc_to_yojson
    [("DefaultSizeInspectionLimit",
       (Some
          (size_inspection_limit_to_yojson x.default_size_inspection_limit)))]
...
let waf_unavailable_entity_exception_to_yojson
  (x : waf_unavailable_entity_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
...
let waf_subscription_not_found_exception_to_yojson
  (x : waf_subscription_not_found_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
let waf_service_linked_role_error_exception_to_yojson
  (x : waf_service_linked_role_error_exception) =
  assoc_to_yojson
    [("message", (option_to_yojson error_message_to_yojson x.message))]
let waf_optimistic_lock_exception_to_yojson
  (x : waf_optimistic_lock_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
let waf_nonexistent_item_exception_to_yojson
  (x : waf_nonexistent_item_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
...
let waf_invalid_parameter_exception_to_yojson
  (x : waf_invalid_parameter_exception) =
  assoc_to_yojson
...
let waf_configuration_warning_exception_to_yojson
  (x : waf_configuration_warning_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
let waf_associated_item_exception_to_yojson
  (x : waf_associated_item_exception) =
  assoc_to_yojson
    [("Message", (option_to_yojson error_message_to_yojson x.message))]
...
let versions_to_publish_to_yojson tree =
  map_to_yojson version_key_string_to_yojson version_to_publish_to_yojson
    tree
...
let update_regex_pattern_set_response_to_yojson
  (x : update_regex_pattern_set_response) =
  assoc_to_yojson
    [("NextLockToken",
       (option_to_yojson lock_token_to_yojson x.next_lock_token))]
...
let update_managed_rule_set_version_expiry_date_response_to_yojson
  ...
  assoc_to_yojson
    [("NextLockToken",
       (option_to_yojson lock_token_to_yojson x.next_lock_token));
    ("ExpiryTimestamp",
      (option_to_yojson timestamp_to_yojson x.expiry_timestamp));
    ("ExpiringVersion",
...
let sampled_http_requests_to_yojson tree =
  list_to_yojson sampled_http_request_to_yojson tree
let rule_summary_to_yojson (x : rule_summary) =
  assoc_to_yojson
    [("Action", (option_to_yojson rule_action_to_yojson x.action));
    ("Name", (option_to_yojson entity_name_to_yojson x.name))]
let rule_summaries_to_yojson tree =
  list_to_yojson rule_summary_to_yojson tree
let rule_group_summary_to_yojson (x : rule_group_summary) =
  assoc_to_yojson
    ...
    ("Id", (option_to_yojson entity_id_to_yojson x.id));
    ("Name", (option_to_yojson entity_name_to_yojson x.name))]
let rule_group_summaries_to_yojson tree =
  list_to_yojson rule_group_summary_to_yojson tree
...
let label_summaries_to_yojson tree =
  list_to_yojson label_summary_to_yojson tree
...
let release_summaries_to_yojson tree =
  list_to_yojson release_summary_to_yojson tree
...
let regex_pattern_set_to_yojson (x : regex_pattern_set) =
  assoc_to_yojson
    [("RegularExpressionList",
       (option_to_yojson regular_expression_list_to_yojson
          x.regular_expression_list));
    ("Description",
      (option_to_yojson entity_description_to_yojson x.description));
    ("ARN", (option_to_yojson resource_arn_to_yojson x.ar_n));
    ("Id", (option_to_yojson entity_id_to_yojson x.id));
    ("Name", (option_to_yojson entity_name_to_yojson x.name))]
let redacted_fields_to_yojson tree =
  list_to_yojson field_to_match_to_yojson tree
...
let put_managed_rule_set_versions_response_to_yojson
  (x : put_managed_rule_set_versions_response) =
  assoc_to_yojson
    [("NextLockToken",
       (option_to_yojson lock_token_to_yojson x.next_lock_token))]
let put_managed_rule_set_versions_request_to_yojson
  (x : put_managed_rule_set_versions_request) =
  assoc_to_yojson
    [("VersionsToPublish",
...
let condition_to_yojson (x : condition) =
  assoc_to_yojson
    [("LabelNameCondition",
       (option_to_yojson label_name_condition_to_yojson
          x.label_name_condition));
    ("ActionCondition",
...
let log_type_to_yojson (x : log_type) =
  match x with | WAF_LOGS -> `String "WAF_LOGS"
...
let put_logging_configuration_response_to_yojson
  (x : put_logging_configuration_response) =
  assoc_to_yojson
    [("LoggingConfiguration",
       (option_to_yojson logging_configuration_to_yojson
          x.logging_configuration))]
let put_logging_configuration_request_to_yojson
  (x : put_logging_configuration_request) =
  assoc_to_yojson
    [("LoggingConfiguration",
...
let managed_rule_set_version_to_yojson (x : managed_rule_set_version) =
  assoc_to_yojson
    ...
    ("PublishTimestamp",
      (option_to_yojson timestamp_to_yojson x.publish_timestamp));
    ("ForecastedLifetime",
      (option_to_yojson time_window_day_to_yojson x.forecasted_lifetime));
...
let published_versions_to_yojson tree =
  map_to_yojson version_key_string_to_yojson
    managed_rule_set_version_to_yojson tree
...
let logging_configurations_to_yojson tree =
  list_to_yojson logging_configuration_to_yojson tree
...
let list_tags_for_resource_response_to_yojson
  ...
  assoc_to_yojson
    [("TagInfoForResource",
       (option_to_yojson tag_info_for_resource_to_yojson
          x.tag_info_for_resource));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker))]
...
let list_regex_pattern_sets_response_to_yojson
  (x : list_regex_pattern_sets_response) =
  assoc_to_yojson
    [("RegexPatternSets",
       (option_to_yojson regex_pattern_set_summaries_to_yojson
          x.regex_pattern_sets));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker))]
let list_regex_pattern_sets_request_to_yojson
  (x : list_regex_pattern_sets_request) =
  assoc_to_yojson
    [("Limit", (option_to_yojson pagination_limit_to_yojson x.limit));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
    ("Scope", (Some (scope_to_yojson x.scope)))]
let list_mobile_sdk_releases_response_to_yojson
  (x : list_mobile_sdk_releases_response) =
  assoc_to_yojson
    [("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
    ("ReleaseSummaries",
      (option_to_yojson release_summaries_to_yojson x.release_summaries))]
let list_mobile_sdk_releases_request_to_yojson
  (x : list_mobile_sdk_releases_request) =
  assoc_to_yojson
    [("Limit", (option_to_yojson pagination_limit_to_yojson x.limit));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
...
let list_managed_rule_sets_response_to_yojson
  (x : list_managed_rule_sets_response) =
  assoc_to_yojson
    [("ManagedRuleSets",
       (option_to_yojson managed_rule_set_summaries_to_yojson
          x.managed_rule_sets));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker))]
let list_managed_rule_sets_request_to_yojson
  (x : list_managed_rule_sets_request) =
  assoc_to_yojson
    [("Limit", (option_to_yojson pagination_limit_to_yojson x.limit));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
    ("Scope", (Some (scope_to_yojson x.scope)))]
let list_logging_configurations_response_to_yojson
  (x : list_logging_configurations_response) =
  assoc_to_yojson
    [("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
    ("LoggingConfigurations",
      (option_to_yojson logging_configurations_to_yojson
         x.logging_configurations))]
let list_logging_configurations_request_to_yojson
  (x : list_logging_configurations_request) =
  assoc_to_yojson
    [("LogScope", (option_to_yojson log_scope_to_yojson x.log_scope));
    ("Limit", (option_to_yojson pagination_limit_to_yojson x.limit));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker));
...
let ip_set_summary_to_yojson (x : ip_set_summary) =
  assoc_to_yojson
    ...
    ("Id", (option_to_yojson entity_id_to_yojson x.id));
    ("Name", (option_to_yojson entity_name_to_yojson x.name))]
let ip_set_summaries_to_yojson tree =
  list_to_yojson ip_set_summary_to_yojson tree
...
let list_available_managed_rule_groups_response_to_yojson
  ...
  assoc_to_yojson
    [("ManagedRuleGroups",
       (option_to_yojson managed_rule_group_summaries_to_yojson
          x.managed_rule_groups));
    ("NextMarker", (option_to_yojson next_marker_to_yojson x.next_marker))]
...
let list_available_managed_rule_group_versions_response_to_yojson
  ...
  assoc_to_yojson
    [("CurrentDefaultVersion",
       (option_to_yojson version_key_string_to_yojson
          x.current_default_version));
    ("Versions",
...
let api_key_summary_to_yojson (x : api_key_summary) =
  assoc_to_yojson
    ...
    ("CreationTimestamp",
      (option_to_yojson timestamp_to_yojson x.creation_timestamp));
    ("APIKey", (option_to_yojson api_key_to_yojson x.api_key));
    ("TokenDomains",
      (option_to_yojson token_domains_to_yojson x.token_domains))]
let api_key_summaries_to_yojson tree =
  list_to_yojson api_key_summary_to_yojson tree
...
let get_web_acl_for_resource_request_to_yojson
  (x : get_web_acl_for_resource_request) =
  assoc_to_yojson
    [("ResourceArn", (Some (resource_arn_to_yojson x.resource_arn)))]
let get_sampled_requests_response_to_yojson
  (x : get_sampled_requests_response) =
  assoc_to_yojson
...
let get_rate_based_statement_managed_keys_response_to_yojson
  ...
  assoc_to_yojson
    [("ManagedKeysIPV6",
       (option_to_yojson rate_based_statement_managed_keys_ip_set_to_yojson
          x.managed_keys_ip_v6));
    ("ManagedKeysIPV4",
      (option_to_yojson rate_based_statement_managed_keys_ip_set_to_yojson
         x.managed_keys_ip_v4))]
...
let get_logging_configuration_response_to_yojson
  (x : get_logging_configuration_response) =
  assoc_to_yojson
    [("LoggingConfiguration",
       (option_to_yojson logging_configuration_to_yojson
          x.logging_configuration))]
let get_logging_configuration_request_to_yojson
  (x : get_logging_configuration_request) =
  assoc_to_yojson
    [("LogScope", (option_to_yojson log_scope_to_yojson x.log_scope));
    ("LogType", (option_to_yojson log_type_to_yojson x.log_type));
    ("ResourceArn", (Some (resource_arn_to_yojson x.resource_arn)))]
...
let get_decrypted_api_key_response_to_yojson
  (x : get_decrypted_api_key_response) =
  assoc_to_yojson
    [("CreationTimestamp",
       (option_to_yojson timestamp_to_yojson x.creation_timestamp));
    ("TokenDomains",
...
let get_decrypted_api_key_request_to_yojson
  (x : get_decrypted_api_key_request) =
  assoc_to_yojson
...
let generate_mobile_sdk_release_url_request_to_yojson
  (x : generate_mobile_sdk_release_url_request) =
  assoc_to_yojson
    [("ReleaseVersion",
       (Some (version_key_string_to_yojson x.release_version)));
    ("Platform", (Some (platform_to_yojson x.platform)))]
...
let describe_managed_rule_group_response_to_yojson
  (x : describe_managed_rule_group_response) =
  assoc_to_yojson
    [("ConsumedLabels",
       (option_to_yojson label_summaries_to_yojson x.consumed_labels));
    ("AvailableLabels",
...
let describe_managed_rule_group_request_to_yojson
  (x : describe_managed_rule_group_request) =
  assoc_to_yojson
    [("VersionName",
       (option_to_yojson version_key_string_to_yojson x.version_name));
    ("Scope", (Some (scope_to_yojson x.scope)));
...
let describe_managed_products_by_vendor_response_to_yojson
  ...
  assoc_to_yojson
    [("ManagedProducts",
       (option_to_yojson managed_product_descriptors_to_yojson
          x.managed_products))]
...
let describe_all_managed_products_response_to_yojson
  (x : describe_all_managed_products_response) =
  assoc_to_yojson
    [("ManagedProducts",
       (option_to_yojson managed_product_descriptors_to_yojson
          x.managed_products))]
...
let create_rule_group_response_to_yojson (x : create_rule_group_response) =
  assoc_to_yojson
    [("Summary", (option_to_yojson rule_group_summary_to_yojson x.summary))]
...
let create_ip_set_response_to_yojson (x : create_ip_set_response) =
  assoc_to_yojson
    [("Summary", (option_to_yojson ip_set_summary_to_yojson x.summary))]
...
let create_api_key_response_to_yojson (x : create_api_key_response) =
  assoc_to_yojson
    [("APIKey", (option_to_yojson api_key_to_yojson x.api_key))]
let api_key_token_domains_to_yojson tree =
  list_to_yojson token_domain_to_yojson tree
