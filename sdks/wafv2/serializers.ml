...
let json_match_scope_to_yojson (x : json_match_scope) =
  match x with VALUE -> `String "VALUE" | KEY -> `String "KEY" | ALL -> `String "ALL"
...
let forwarded_ip_position_to_yojson (x : forwarded_ip_position) =
  match x with ANY -> `String "ANY" | LAST -> `String "LAST" | FIRST -> `String "FIRST"
...
let payload_type_to_yojson (x : payload_type) =
  match x with FORM_ENCODED -> `String "FORM_ENCODED" | JSON -> `String "JSON"
...
let data_protection_action_to_yojson (x : data_protection_action) =
  match x with HASH -> `String "HASH" | SUBSTITUTION -> `String "SUBSTITUTION"
...
let low_reputation_mode_to_yojson (x : low_reputation_mode) =
  match x with ALWAYS_ON -> `String "ALWAYS_ON" | ACTIVE_UNDER_DDOS -> `String "ACTIVE_UNDER_DDOS"
...
let scope_to_yojson (x : scope) =
  match x with REGIONAL -> `String "REGIONAL" | CLOUDFRONT -> `String "CLOUDFRONT"
...
let update_managed_rule_set_version_expiry_date_response_to_yojson
  ...
  assoc_to_yojson
    [
      ...
      ("ExpiryTimestamp", option_to_yojson timestamp__to_yojson x.expiry_timestamp);
...
let filter_requirement_to_yojson (x : filter_requirement) =
  match x with MEETS_ANY -> `String "MEETS_ANY" | MEETS_ALL -> `String "MEETS_ALL"
...
let managed_rule_set_version_to_yojson (x : managed_rule_set_version) =
  assoc_to_yojson
    [
      ("ExpiryTimestamp", option_to_yojson timestamp__to_yojson x.expiry_timestamp);
      ("LastUpdateTimestamp", option_to_yojson timestamp__to_yojson x.last_update_timestamp);
      ("PublishTimestamp", option_to_yojson timestamp__to_yojson x.publish_timestamp);
      ("ForecastedLifetime", option_to_yojson time_window_day_to_yojson x.forecasted_lifetime);
...
let api_key_summary_to_yojson (x : api_key_summary) =
  assoc_to_yojson
    [
      ...
      ("CreationTimestamp", option_to_yojson timestamp__to_yojson x.creation_timestamp);
...
let get_decrypted_api_key_request_to_yojson (x : get_decrypted_api_key_request) =
  assoc_to_yojson
    [ ("APIKey", Some (api_key_to_yojson x.api_key)); ("Scope", Some (scope_to_yojson x.scope)) ]
...
let check_capacity_request_to_yojson (x : check_capacity_request) =
  assoc_to_yojson
    [ ("Rules", Some (rules_to_yojson x.rules)); ("Scope", Some (scope_to_yojson x.scope)) ]
