...
let make_vocabulary_info ?vocabulary_state:(vocabulary_state_ : vocabulary_state option)
...
let make_vocabulary_filter_info ?last_modified_time:(last_modified_time_ : date_time option)
    ?language_code:(language_code_ : language_code option)
...
let make_update_vocabulary_response ?vocabulary_state:(vocabulary_state_ : vocabulary_state option)
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?language_code:(language_code_ : language_code option)
    ?vocabulary_name:(vocabulary_name_ : vocabulary_name option) () =
  ({
...
let make_update_vocabulary_request
    ...
    ?vocabulary_file_uri:(vocabulary_file_uri_ : uri option) ?phrases:(phrases_ : phrases option)
...
let make_update_medical_vocabulary_request ~vocabulary_file_uri:(vocabulary_file_uri_ : uri)
    ~language_code:(language_code_ : language_code)
    ~vocabulary_name:(vocabulary_name_ : vocabulary_name) () =
  ({
     vocabulary_file_uri = vocabulary_file_uri_;
...
let make_absolute_time_range ?last:(last_ : timestamp_milliseconds option)
  ...
  ({ last = last_; first = first_; end_time = end_time_; start_time = start_time_ }
    : absolute_time_range)

let make_relative_time_range ?last:(last_ : percentage option) ?first:(first_ : percentage option)
...
let make_category_properties ?input_type:(input_type_ : input_type option)
    ?tags:(tags_ : tag_list option) ?last_update_time:(last_update_time_ : date_time option)
    ?create_time:(create_time_ : date_time option) ?rules:(rules_ : rule_list option)
...
let make_update_call_analytics_category_request ?input_type:(input_type_ : input_type option)
    ~rules:(rules_ : rule_list) ~category_name:(category_name_ : category_name) () =
...
let make_language_code_item ?duration_in_seconds:(duration_in_seconds_ : duration_in_seconds option)
    ?language_code:(language_code_ : language_code option) () =
  ({ duration_in_seconds = duration_in_seconds_; language_code = language_code_ }
    : language_code_item)
...
let make_media ?redacted_media_file_uri:(redacted_media_file_uri_ : uri option)
    ?media_file_uri:(media_file_uri_ : uri option) () =
  ({ redacted_media_file_uri = redacted_media_file_uri_; media_file_uri = media_file_uri_ } : media)

let make_transcript ?redacted_transcript_file_uri:(redacted_transcript_file_uri_ : uri option)
...
let make_language_id_settings ?language_model_name:(language_model_name_ : model_name option)
    ?vocabulary_filter_name:(vocabulary_filter_name_ : vocabulary_filter_name option)
    ?vocabulary_name:(vocabulary_name_ : vocabulary_name option) () =
  ({
...
let make_transcription_job ?toxicity_detection:(toxicity_detection_ : toxicity_detection option)
    ?language_id_settings:(language_id_settings_ : language_id_settings_map option)
    ?subtitles:(subtitles_ : subtitles_output option) ?tags:(tags_ : tag_list option)
    ?language_codes:(language_codes_ : language_code_list option)
    ?identified_language_score:(identified_language_score_ : identified_language_score option)
    ...
    ?transcript:(transcript_ : transcript option) ?media:(media_ : media option)
...
let make_subtitles ?output_start_index:(output_start_index_ : subtitle_output_start_index option)
    ?formats:(formats_ : subtitle_formats option) () =
  ({ output_start_index = output_start_index_; formats = formats_ } : subtitles)
...
let make_start_transcription_job_request
    ...
    ?media_format:(media_format_ : media_format option)
    ?media_sample_rate_hertz:(media_sample_rate_hertz_ : media_sample_rate_hertz option)
    ?language_code:(language_code_ : language_code option) ~media:(media_ : media)
...
let make_medical_transcription_setting ?vocabulary_name:(vocabulary_name_ : vocabulary_name option)
    ?max_alternatives:(max_alternatives_ : max_alternatives option)
    ?show_alternatives:(show_alternatives_ : boolean_ option)
    ?channel_identification:(channel_identification_ : boolean_ option)
    ?max_speaker_labels:(max_speaker_labels_ : max_speakers option)
...
let make_medical_transcription_job ?tags:(tags_ : tag_list option) ?type_:(type__ : type_ option)
    ...
    ?completion_time:(completion_time_ : date_time option)
    ?creation_time:(creation_time_ : date_time option) ?start_time:(start_time_ : date_time option)
    ?transcript:(transcript_ : medical_transcript option) ?media:(media_ : media option)
    ?media_format:(media_format_ : media_format option)
...
let make_start_medical_transcription_job_request ?tags:(tags_ : tag_list option)
    ?content_identification_type:
      (content_identification_type_ : medical_content_identification_type option)
    ?settings:(settings_ : medical_transcription_setting option)
    ?kms_encryption_context:(kms_encryption_context_ : kms_encryption_context_map option)
...
let make_medical_scribe_output ~clinical_document_uri:(clinical_document_uri_ : uri)
    ~transcript_file_uri:(transcript_file_uri_ : uri) () =
  ({ clinical_document_uri = clinical_document_uri_; transcript_file_uri = transcript_file_uri_ }
...
let make_call_analytics_job_settings ?summarization:(summarization_ : summarization option)
    ?language_id_settings:(language_id_settings_ : language_id_settings_map option)
    ?language_options:(language_options_ : language_options option)
    ?content_redaction:(content_redaction_ : content_redaction option)
...
let make_channel_definition ?participant_role:(participant_role_ : participant_role option)
    ?channel_id:(channel_id_ : channel_id option) () =
  ({ participant_role = participant_role_; channel_id = channel_id_ } : channel_definition)
...
let make_start_call_analytics_job_request
    ?channel_definitions:(channel_definitions_ : channel_definitions option)
    ?tags:(tags_ : tag_list option) ?settings:(settings_ : call_analytics_job_settings option)
    ?data_access_role_arn:(data_access_role_arn_ : data_access_role_arn option)
    ?output_encryption_kms_key_id:(output_encryption_kms_key_id_ : kms_key_id option)
...
let make_list_vocabulary_filters_request
    ...
    () =
  ({ name_contains = name_contains_; max_results = max_results_; next_token = next_token_ }
    : list_vocabulary_filters_request)

let make_list_vocabularies_response ?vocabularies:(vocabularies_ : vocabularies option)
    ?next_token:(next_token_ : next_token option) ?status:(status_ : vocabulary_state option) () =
  ({ vocabularies = vocabularies_; next_token = next_token_; status = status_ }
    : list_vocabularies_response)

let make_list_vocabularies_request ?name_contains:(name_contains_ : vocabulary_name option)
...
let make_list_transcription_jobs_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_list_medical_vocabularies_response ?vocabularies:(vocabularies_ : vocabularies option)
    ?next_token:(next_token_ : next_token option) ?status:(status_ : vocabulary_state option) () =
  ({ vocabularies = vocabularies_; next_token = next_token_; status = status_ }
    : list_medical_vocabularies_response)

let make_list_medical_vocabularies_request ?name_contains:(name_contains_ : vocabulary_name option)
    ?state_equals:(state_equals_ : vocabulary_state option)
    ?max_results:(max_results_ : max_results option) ?next_token:(next_token_ : next_token option)
    () =
  ({
...
let make_list_medical_transcription_jobs_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
    ?job_name_contains:(job_name_contains_ : transcription_job_name option)
    ?status:(status_ : transcription_job_status option) () =
  ({
...
let make_medical_scribe_job_summary ?failure_reason:(failure_reason_ : failure_reason option)
    ?medical_scribe_job_status:(medical_scribe_job_status_ : medical_scribe_job_status option)
    ?language_code:(language_code_ : medical_scribe_language_code option)
    ?completion_time:(completion_time_ : date_time option)
    ?start_time:(start_time_ : date_time option) ?creation_time:(creation_time_ : date_time option)
...
let make_list_medical_scribe_jobs_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
    ?job_name_contains:(job_name_contains_ : transcription_job_name option)
    ?status:(status_ : medical_scribe_job_status option) () =
  ({
...
let make_input_data_config ?tuning_data_s3_uri:(tuning_data_s3_uri_ : uri option)
    ~data_access_role_arn:(data_access_role_arn_ : data_access_role_arn) ~s3_uri:(s3_uri_ : uri) ()
    =
  ({
     data_access_role_arn = data_access_role_arn_;
...
let make_language_model ?input_data_config:(input_data_config_ : input_data_config option)
    ?failure_reason:(failure_reason_ : failure_reason option)
    ...
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?create_time:(create_time_ : date_time option) ?model_name:(model_name_ : model_name option) ()
    =
  ({
...
let make_list_language_models_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_call_analytics_job_summary ?failure_reason:(failure_reason_ : failure_reason option)
    ?call_analytics_job_details:(call_analytics_job_details_ : call_analytics_job_details option)
    ?call_analytics_job_status:(call_analytics_job_status_ : call_analytics_job_status option)
    ?language_code:(language_code_ : language_code option)
    ?completion_time:(completion_time_ : date_time option)
...
let make_list_call_analytics_jobs_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option)
...
let make_list_call_analytics_categories_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) () =
  ({ max_results = max_results_; next_token = next_token_ }
    : list_call_analytics_categories_request)

let make_get_vocabulary_filter_response ?download_uri:(download_uri_ : uri option)
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?language_code:(language_code_ : language_code option)
    ?vocabulary_filter_name:(vocabulary_filter_name_ : vocabulary_filter_name option) () =
  ({
...
let make_get_medical_vocabulary_response ?download_uri:(download_uri_ : uri option)
    ?failure_reason:(failure_reason_ : failure_reason option)
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?vocabulary_state:(vocabulary_state_ : vocabulary_state option)
    ?language_code:(language_code_ : language_code option)
...
let make_create_vocabulary_filter_request
    ...
    ?vocabulary_filter_file_uri:(vocabulary_filter_file_uri_ : uri option)
    ?words:(words_ : words option) ~language_code:(language_code_ : language_code)
    ~vocabulary_filter_name:(vocabulary_filter_name_ : vocabulary_filter_name) () =
  ({
     data_access_role_arn = data_access_role_arn_;
     tags = tags_;
...
let make_create_vocabulary_response ?failure_reason:(failure_reason_ : failure_reason option)
    ?last_modified_time:(last_modified_time_ : date_time option)
    ?vocabulary_state:(vocabulary_state_ : vocabulary_state option)
    ?language_code:(language_code_ : language_code option)
    ?vocabulary_name:(vocabulary_name_ : vocabulary_name option) () =
...
let make_create_vocabulary_request
    ?data_access_role_arn:(data_access_role_arn_ : data_access_role_arn option)
    ?tags:(tags_ : tag_list option) ?vocabulary_file_uri:(vocabulary_file_uri_ : uri option)
    ?phrases:(phrases_ : phrases option) ~language_code:(language_code_ : language_code)
    ~vocabulary_name:(vocabulary_name_ : vocabulary_name) () =
  ({
     data_access_role_arn = data_access_role_arn_;
     tags = tags_;
...
let make_create_language_model_response ?model_status:(model_status_ : model_status option)
...
let make_create_language_model_request ?tags:(tags_ : tag_list option)
    ...
    ~model_name:(model_name_ : model_name) ~base_model_name:(base_model_name_ : base_model_name)
...
let make_create_call_analytics_category_request ?input_type:(input_type_ : input_type option)
    ?tags:(tags_ : tag_list option) ~rules:(rules_ : rule_list)
    ~category_name:(category_name_ : category_name) () =
  ({ input_type = input_type_; tags = tags_; rules = rules_; category_name = category_name_ }
