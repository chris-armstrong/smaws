...
let vocabulary_filter_method_to_yojson (x : vocabulary_filter_method) =
  match x with TAG -> `String "tag" | MASK -> `String "mask" | REMOVE -> `String "remove"
...
let input_type_to_yojson (x : input_type) =
  match x with POST_CALL -> `String "POST_CALL" | REAL_TIME -> `String "REAL_TIME"
...
let type__to_yojson (x : type_) =
  match x with DICTATION -> `String "DICTATION" | CONVERSATION -> `String "CONVERSATION"
...
let medical_scribe_participant_role_to_yojson (x : medical_scribe_participant_role) =
  match x with CLINICIAN -> `String "CLINICIAN" | PATIENT -> `String "PATIENT"
...
let base_model_name_to_yojson (x : base_model_name) =
  match x with WIDE_BAND -> `String "WideBand" | NARROW_BAND -> `String "NarrowBand"
