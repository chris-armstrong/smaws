...
let update_routing_control_state_entry_to_yojson
  (x : update_routing_control_state_entry) =
  assoc_to_yojson
    [("RoutingControlState",
...
let update_routing_control_states_request_to_yojson
  (x : update_routing_control_states_request) =
  assoc_to_yojson
    [("SafetyRulesToOverride",
       (option_to_yojson arns_to_yojson x.safety_rules_to_override));
    ("UpdateRoutingControlStateEntries",
      (Some
         (update_routing_control_state_entries_to_yojson
            x.update_routing_control_state_entries)))]
...
let service_limit_exceeded_exception_to_yojson
  (x : service_limit_exceeded_exception) =
  assoc_to_yojson
...
let routing_control_to_yojson (x : routing_control) =
  assoc_to_yojson
    ...
    ("RoutingControlState",
      (option_to_yojson routing_control_state_to_yojson
         x.routing_control_state));
    ("RoutingControlName",
...
let routing_controls_to_yojson tree =
  list_to_yojson routing_control_to_yojson tree
let page_token_to_yojson = string_to_yojson
let list_routing_controls_response_to_yojson
  (x : list_routing_controls_response) =
  assoc_to_yojson
...
let list_routing_controls_request_to_yojson
  (x : list_routing_controls_request) =
  assoc_to_yojson
    [("MaxResults", (option_to_yojson max_results_to_yojson x.max_results));
    ("NextToken", (option_to_yojson page_token_to_yojson x.next_token));
    ("ControlPanelArn", (option_to_yojson arn_to_yojson x.control_panel_arn))]
let get_routing_control_state_response_to_yojson
  (x : get_routing_control_state_response) =
  assoc_to_yojson
    [("RoutingControlName",
       (option_to_yojson routing_control_name_to_yojson
          x.routing_control_name));
    ("RoutingControlState",
      (Some (routing_control_state_to_yojson x.routing_control_state)));
    ("RoutingControlArn", (Some (arn_to_yojson x.routing_control_arn)))]
let get_routing_control_state_request_to_yojson
  (x : get_routing_control_state_request) =
  assoc_to_yojson
    [("RoutingControlArn", (Some (arn_to_yojson x.routing_control_arn)))]
