  ...
  let _res : update_routing_control_state_entry =
    {
      routing_control_state =
        (value_for_key routing_control_state_of_yojson "RoutingControlState"
           _list path);
      routing_control_arn =
        (value_for_key arn_of_yojson "RoutingControlArn" _list path)
    } in
  _res
  ...
  let _res : update_routing_control_states_request =
    {
      safety_rules_to_override =
        (option_of_yojson
           (value_for_key arns_of_yojson "SafetyRulesToOverride") _list path);
...
let resource_not_found_exception_of_yojson tree path =
  ...
  let _res : resource_not_found_exception =
    {
      resource_type =
        (value_for_key string__of_yojson "resourceType" _list path);
...
let routing_control_of_yojson tree path =
  ...
  let _res : routing_control =
    {
      owner =
        (option_of_yojson (value_for_key owner_of_yojson "Owner") _list path);
      routing_control_state =
        (option_of_yojson
           (value_for_key routing_control_state_of_yojson
              "RoutingControlState") _list path);
      routing_control_name =
        (option_of_yojson
           (value_for_key routing_control_name_of_yojson "RoutingControlName")
           _list path);
      routing_control_arn =
        (option_of_yojson (value_for_key arn_of_yojson "RoutingControlArn")
           _list path);
      control_panel_name =
        (option_of_yojson
           (value_for_key control_panel_name_of_yojson "ControlPanelName")
           _list path);
      control_panel_arn =
        (option_of_yojson (value_for_key arn_of_yojson "ControlPanelArn")
           _list path)
    } in
  _res
let routing_controls_of_yojson tree path =
  list_of_yojson routing_control_of_yojson tree path
  ...
  let _res : list_routing_controls_response =
    {
      next_token =
        (option_of_yojson (value_for_key page_token_of_yojson "NextToken")
           _list path);
      routing_controls =
        (value_for_key routing_controls_of_yojson "RoutingControls" _list
           path)
    } in
  _res
