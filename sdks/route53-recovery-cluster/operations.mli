...
[@@ocaml.doc
  ...
  \ With Route 53 ARC, you can add safety rules for routing controls, which are safeguards for \
   ...
   safeguards that are enforced with safety rules that you've configured. For example, you might \
   want to fail over quickly for disaster recovery, and one or more safety rules might be \
  ...
  \ \n\
  \  The [SafetyRulesToOverride] property enables you override one or more safety rules and update \
   routing control states. For more information, see \
...
[@@ocaml.doc
  "Set multiple routing control states. You can set the value for each state to be ON or OFF. When \
   the state is ON, traffic flows to a cell. When it's OFF, traffic does not flow.\n\n\
  \ With Route 53 ARC, you can add safety rules for routing controls, which are safeguards for \
   routing control state updates that help prevent unexpected outcomes, like fail open traffic \
   routing. However, there are scenarios when you might want to bypass the routing control \
   safeguards that are enforced with safety rules that you've configured. For example, you might \
