...
let make_rule ?event_bus_name:(event_bus_name_ : string option)
  ...
  ?event_pattern:(event_pattern_ : string option) ?arn:(arn_ : string option)
  ?name:(name_ : string option) () =
  ({
...
let make_remove_targets_request ?force:(force_ : bool option)
  ?event_bus_name:(event_bus_name_ : string option) ~ids:(ids_ : string list)
  ~rule:(rule_ : string) () =
  ({
     force = force_;
     ids = ids_;
...
let make_create_connection_o_auth_client_request_parameters
  ~client_secret:(client_secret_ : string) ~client_i_d:(client_i_d_ : string)
  () =
