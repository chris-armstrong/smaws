...
let make_secondary ~route:(route_ : route) () = ({ route = route_ } : secondary)

let make_failover_config ~secondary:(secondary_ : secondary) ~primary:(primary_ : primary) () =
...
let make_endpoint_event_bus ~event_bus_arn:(event_bus_arn_ : non_partner_event_bus_arn) () =
...
let make_update_endpoint_response ?state:(state_ : endpoint_state option)
    ...
    ?endpoint_id:(endpoint_id_ : endpoint_id option) ?role_arn:(role_arn_ : iam_role_arn option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?replication_config:(replication_config_ : replication_config option)
    ?routing_config:(routing_config_ : routing_config option) ?arn:(arn_ : endpoint_arn option)
...
let make_update_endpoint_request ?role_arn:(role_arn_ : iam_role_arn option)
    ...
    ?description:(description_ : endpoint_description option) ~name:(name_ : endpoint_name) () =
  ({
     role_arn = role_arn_;
     event_buses = event_buses_;
     replication_config = replication_config_;
...
let make_update_connection_response ?last_authorized_time:(last_authorized_time_ : timestamp option)
...
let make_update_connection_o_auth_client_request_parameters
  ...
  ({ client_secret = client_secret_; client_i_d = client_i_d_ }
    : update_connection_o_auth_client_request_parameters)

let make_connection_header_parameter ?is_value_secret:(is_value_secret_ : boolean_ option)
    ?value:(value_ : header_value_sensitive option) ?key:(key_ : header_key option) () =
  ({ is_value_secret = is_value_secret_; value = value_; key = key_ } : connection_header_parameter)

let make_connection_query_string_parameter ?is_value_secret:(is_value_secret_ : boolean_ option)
    ?value:(value_ : query_string_value_sensitive option) ?key:(key_ : query_string_key option) () =
  ({ is_value_secret = is_value_secret_; value = value_; key = key_ }
    : connection_query_string_parameter)

let make_connection_body_parameter ?is_value_secret:(is_value_secret_ : boolean_ option)
    ?value:(value_ : sensitive_string option) ?key:(key_ : string_ option) () =
  ({ is_value_secret = is_value_secret_; value = value_; key = key_ } : connection_body_parameter)
...
let make_connectivity_resource_configuration_arn
    ~resource_configuration_arn:(resource_configuration_arn_ : resource_configuration_arn) () =
...
let make_connectivity_resource_parameters
    ~resource_parameters:(resource_parameters_ : connectivity_resource_configuration_arn) () =
...
let make_update_connection_auth_request_parameters
    ?connectivity_parameters:(connectivity_parameters_ : connectivity_resource_parameters option)
...
let make_update_connection_request
    ...
    ?invocation_connectivity_parameters:
      (invocation_connectivity_parameters_ : connectivity_resource_parameters option)
    ...
    ?description:(description_ : connection_description option) ~name:(name_ : connection_name) () =
  ({
     kms_key_identifier = kms_key_identifier_;
...
let make_update_archive_response ?creation_time:(creation_time_ : timestamp option)
    ?state_reason:(state_reason_ : archive_state_reason option)
    ?state:(state_ : archive_state option) ?archive_arn:(archive_arn_ : archive_arn option) () =
  ({
     creation_time = creation_time_;
...
let make_input_transformer ?input_paths_map:(input_paths_map_ : transformer_paths option)
  ...
  ({ input_template = input_template_; input_paths_map = input_paths_map_ } : input_transformer)
...
let make_placement_constraint ?expression:(expression_ : placement_constraint_expression option)
...
let make_ecs_parameters ?tags:(tags_ : tag_list option)
    ...
    ?group:(group_ : string_ option) ?platform_version:(platform_version_ : string_ option)
    ?network_configuration:(network_configuration_ : network_configuration option)
    ?launch_type:(launch_type_ : launch_type option) ?task_count:(task_count_ : limit_min1 option)
...
let make_batch_parameters ?retry_strategy:(retry_strategy_ : batch_retry_strategy option)
...
let make_redshift_data_parameters ?sqls:(sqls_ : sqls option)
    ...
    ?statement_name:(statement_name_ : statement_name option) ?sql:(sql_ : sql option)
    ?db_user:(db_user_ : db_user option)
...
let make_sage_maker_pipeline_parameter ~value:(value_ : sage_maker_pipeline_parameter_value)
...
let make_app_sync_parameters ?graph_ql_operation:(graph_ql_operation_ : graph_ql_operation option)
    () =
  ({ graph_ql_operation = graph_ql_operation_ } : app_sync_parameters)

let make_target ?app_sync_parameters:(app_sync_parameters_ : app_sync_parameters option)
    ...
    ?input_path:(input_path_ : target_input_path option) ?input:(input_ : target_input option)
    ?role_arn:(role_arn_ : role_arn option) ~arn:(arn_ : target_arn) ~id:(id_ : target_id) () =
  ({
...
let make_tag_resource_request ~tags:(tags_ : tag_list) ~resource_ar_n:(resource_ar_n_ : arn) () =
  ({ tags = tags_; resource_ar_n = resource_ar_n_ } : tag_resource_request)

let make_start_replay_response ?replay_start_time:(replay_start_time_ : timestamp option)
    ?state_reason:(state_reason_ : replay_state_reason option) ?state:(state_ : replay_state option)
...
let make_start_replay_request ?description:(description_ : replay_description option)
    ~destination:(destination_ : replay_destination) ~event_end_time:(event_end_time_ : timestamp)
    ~event_start_time:(event_start_time_ : timestamp)
    ~event_source_arn:(event_source_arn_ : archive_arn) ~replay_name:(replay_name_ : replay_name) ()
    =
  ({
...
let make_rule ?event_bus_name:(event_bus_name_ : event_bus_name option)
    ?managed_by:(managed_by_ : managed_by option) ?role_arn:(role_arn_ : role_arn option)
    ?schedule_expression:(schedule_expression_ : schedule_expression option)
    ?description:(description_ : rule_description option) ?state:(state_ : rule_state option)
    ?event_pattern:(event_pattern_ : event_pattern option) ?arn:(arn_ : rule_arn option)
...
let make_remove_targets_result_entry ?error_message:(error_message_ : error_message option)
    ?error_code:(error_code_ : error_code option) ?target_id:(target_id_ : target_id option) () =
  ({ error_message = error_message_; error_code = error_code_; target_id = target_id_ }
...
let make_remove_targets_response
  ...
  ({ failed_entries = failed_entries_; failed_entry_count = failed_entry_count_ }
    : remove_targets_response)

let make_remove_targets_request ?force:(force_ : boolean_ option)
    ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option) ~ids:(ids_ : target_id_list)
    ~rule:(rule_ : rule_name) () =
  ({ force = force_; ids = ids_; event_bus_name = event_bus_name_; rule = rule_ }
...
let make_put_targets_result_entry ?error_message:(error_message_ : error_message option)
    ?error_code:(error_code_ : error_code option) ?target_id:(target_id_ : target_id option) () =
  ({ error_message = error_message_; error_code = error_code_; target_id = target_id_ }
...
let make_put_targets_request ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option)
  ...
  ({ targets = targets_; event_bus_name = event_bus_name_; rule = rule_ } : put_targets_request)

let make_put_rule_response ?rule_arn:(rule_arn_ : rule_arn option) () =
  ({ rule_arn = rule_arn_ } : put_rule_response)

let make_put_rule_request ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option)
...
let make_condition ~value:(value_ : string_) ~key:(key_ : string_) ~type_:(type__ : string_) () =
  ({ value = value_; key = key_; type_ = type__ } : condition)

let make_put_permission_request ?policy:(policy_ : string_ option)
    ?condition:(condition_ : condition option) ?statement_id:(statement_id_ : statement_id option)
    ?principal:(principal_ : principal option) ?action:(action_ : action option)
    ?event_bus_name:(event_bus_name_ : non_partner_event_bus_name option) () =
  ({
     policy = policy_;
...
let make_put_partner_events_result_entry ?error_message:(error_message_ : error_message option)
    ?error_code:(error_code_ : error_code option) ?event_id:(event_id_ : event_id option) () =
  ({ error_message = error_message_; error_code = error_code_; event_id = event_id_ }
...
let make_put_partner_events_response
    ...
    ?failed_entry_count:(failed_entry_count_ : integer option) () =
  ({ entries = entries_; failed_entry_count = failed_entry_count_ } : put_partner_events_response)
...
let make_put_partner_events_request_entry ?detail:(detail_ : string_ option)
    ...
    ?source:(source_ : event_source_name option) ?time:(time_ : event_time option) () =
  ({
     detail = detail_;
...
let make_put_events_result_entry ?error_message:(error_message_ : error_message option)
    ?error_code:(error_code_ : error_code option) ?event_id:(event_id_ : event_id option) () =
  ({ error_message = error_message_; error_code = error_code_; event_id = event_id_ }
    : put_events_result_entry)

let make_put_events_response ?entries:(entries_ : put_events_result_entry_list option)
    ?failed_entry_count:(failed_entry_count_ : integer option) () =
  ({ entries = entries_; failed_entry_count = failed_entry_count_ } : put_events_response)

let make_put_events_request_entry ?trace_header:(trace_header_ : trace_header option)
    ?event_bus_name:(event_bus_name_ : non_partner_event_bus_name_or_arn option)
    ?detail:(detail_ : string_ option) ?detail_type:(detail_type_ : string_ option)
    ?resources:(resources_ : event_resource_list option) ?source:(source_ : string_ option)
...
let make_partner_event_source ?name:(name_ : string_ option) ?arn:(arn_ : string_ option) () =
  ({ name = name_; arn = arn_ } : partner_event_source)

let make_partner_event_source_account ?state:(state_ : event_source_state option)
    ?expiration_time:(expiration_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option) ?account:(account_ : account_id option) () =
  ({
     state = state_;
...
let make_list_targets_by_rule_response ?next_token:(next_token_ : next_token option)
...
let make_list_targets_by_rule_request ?limit:(limit_ : limit_max100 option)
    ...
    ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option) ~rule:(rule_ : rule_name) () =
  ({ limit = limit_; next_token = next_token_; event_bus_name = event_bus_name_; rule = rule_ }
...
let make_list_rule_names_by_target_response ?next_token:(next_token_ : next_token option)
...
let make_list_rule_names_by_target_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option)
    ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option)
...
let make_list_replays_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option)
    ?event_source_arn:(event_source_arn_ : archive_arn option) ?state:(state_ : replay_state option)
...
let make_list_partner_event_sources_response ?next_token:(next_token_ : next_token option)
...
let make_list_partner_event_sources_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option)
...
let make_list_partner_event_source_accounts_response ?next_token:(next_token_ : next_token option)
    ?partner_event_source_accounts:
      (partner_event_source_accounts_ : partner_event_source_account_list option) () =
  ({ next_token = next_token_; partner_event_source_accounts = partner_event_source_accounts_ }
    : list_partner_event_source_accounts_response)

let make_list_partner_event_source_accounts_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option)
    ~event_source_name:(event_source_name_ : event_source_name) () =
  ({ limit = limit_; next_token = next_token_; event_source_name = event_source_name_ }
    : list_partner_event_source_accounts_request)

let make_event_source ?state:(state_ : event_source_state option) ?name:(name_ : string_ option)
    ?expiration_time:(expiration_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option) ?created_by:(created_by_ : string_ option)
...
let make_list_event_sources_response ?next_token:(next_token_ : next_token option)
...
let make_event_bus ?last_modified_time:(last_modified_time_ : timestamp option)
...
let make_list_event_buses_response ?next_token:(next_token_ : next_token option)
...
let make_endpoint ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?state_reason:(state_reason_ : endpoint_state_reason option)
    ?state:(state_ : endpoint_state option) ?endpoint_url:(endpoint_url_ : endpoint_url option)
    ?endpoint_id:(endpoint_id_ : endpoint_id option) ?role_arn:(role_arn_ : iam_role_arn option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?replication_config:(replication_config_ : replication_config option)
    ?routing_config:(routing_config_ : routing_config option) ?arn:(arn_ : endpoint_arn option)
    ?description:(description_ : endpoint_description option) ?name:(name_ : endpoint_name option)
    () =
  ({
     last_modified_time = last_modified_time_;
     creation_time = creation_time_;
...
let make_list_endpoints_response ?next_token:(next_token_ : next_token option)
    ?endpoints:(endpoints_ : endpoint_list option) () =
  ({ next_token = next_token_; endpoints = endpoints_ } : list_endpoints_response)

let make_list_endpoints_request ?max_results:(max_results_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option) ?home_region:(home_region_ : home_region option)
...
let make_connection ?last_authorized_time:(last_authorized_time_ : timestamp option)
    ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?authorization_type:(authorization_type_ : connection_authorization_type option)
    ?state_reason:(state_reason_ : connection_state_reason option)
    ?connection_state:(connection_state_ : connection_state option)
    ?name:(name_ : connection_name option) ?connection_arn:(connection_arn_ : connection_arn option)
    () =
  ({
     last_authorized_time = last_authorized_time_;
     last_modified_time = last_modified_time_;
...
let make_archive ?creation_time:(creation_time_ : timestamp option)
    ?event_count:(event_count_ : long option) ?size_bytes:(size_bytes_ : long option)
    ?retention_days:(retention_days_ : retention_days option)
    ?state_reason:(state_reason_ : archive_state_reason option)
...
let make_list_archives_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option) ?state:(state_ : archive_state option)
    ?event_source_arn:(event_source_arn_ : event_bus_arn option)
...
let make_api_destination ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?invocation_rate_limit_per_second:
      (invocation_rate_limit_per_second_ : api_destination_invocation_rate_limit_per_second option)
    ?http_method:(http_method_ : api_destination_http_method option)
...
let make_list_api_destinations_response ?next_token:(next_token_ : next_token option)
...
let make_describe_replay_response ?replay_end_time:(replay_end_time_ : timestamp option)
    ?replay_start_time:(replay_start_time_ : timestamp option)
    ?event_last_replayed_time:(event_last_replayed_time_ : timestamp option)
    ?event_end_time:(event_end_time_ : timestamp option)
    ?event_start_time:(event_start_time_ : timestamp option)
    ...
    ?replay_arn:(replay_arn_ : replay_arn option) ?replay_name:(replay_name_ : replay_name option)
    () =
  ({
     replay_end_time = replay_end_time_;
     replay_start_time = replay_start_time_;
...
let make_describe_partner_event_source_response ?name:(name_ : string_ option)
    ?arn:(arn_ : string_ option) () =
...
let make_describe_event_source_response ?state:(state_ : event_source_state option)
    ?name:(name_ : string_ option) ?expiration_time:(expiration_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option) ?created_by:(created_by_ : string_ option)
    ?arn:(arn_ : string_ option) () =
  ({
     state = state_;
...
let make_describe_endpoint_response ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?state_reason:(state_reason_ : endpoint_state_reason option)
    ?state:(state_ : endpoint_state option) ?endpoint_url:(endpoint_url_ : endpoint_url option)
    ?endpoint_id:(endpoint_id_ : endpoint_id option) ?role_arn:(role_arn_ : iam_role_arn option)
...
let make_describe_endpoint_request ?home_region:(home_region_ : home_region option)
...
let make_describe_connection_resource_parameters
    ~resource_association_arn:(resource_association_arn_ : resource_association_arn)
    ~resource_configuration_arn:(resource_configuration_arn_ : resource_configuration_arn) () =
  ({
...
let make_describe_connection_connectivity_parameters
    ~resource_parameters:(resource_parameters_ : describe_connection_resource_parameters) () =
...
let make_connection_auth_response_parameters
    ?connectivity_parameters:
      (connectivity_parameters_ : describe_connection_connectivity_parameters option)
    ?invocation_http_parameters:(invocation_http_parameters_ : connection_http_parameters option)
    ?api_key_auth_parameters:
...
let make_describe_connection_response
    ...
    ?connection_state:(connection_state_ : connection_state option)
    ?invocation_connectivity_parameters:
      (invocation_connectivity_parameters_ : describe_connection_connectivity_parameters option)
...
let make_describe_archive_response ?creation_time:(creation_time_ : timestamp option)
    ?event_count:(event_count_ : long option) ?size_bytes:(size_bytes_ : long option)
    ?retention_days:(retention_days_ : retention_days option)
    ?kms_key_identifier:(kms_key_identifier_ : kms_key_identifier option)
    ?state_reason:(state_reason_ : archive_state_reason option)
    ?state:(state_ : archive_state option) ?event_pattern:(event_pattern_ : event_pattern option)
    ?description:(description_ : archive_description option)
    ?event_source_arn:(event_source_arn_ : event_bus_arn option)
    ?archive_name:(archive_name_ : archive_name option)
...
let make_delete_rule_request ?force:(force_ : boolean_ option)
    ?event_bus_name:(event_bus_name_ : event_bus_name_or_arn option) ~name:(name_ : rule_name) () =
  ({ force = force_; event_bus_name = event_bus_name_; name = name_ } : delete_rule_request)
...
let make_delete_connection_response ?last_authorized_time:(last_authorized_time_ : timestamp option)
    ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?connection_state:(connection_state_ : connection_state option)
    ?connection_arn:(connection_arn_ : connection_arn option) () =
...
let make_create_endpoint_request ?role_arn:(role_arn_ : iam_role_arn option)
    ...
    ~routing_config:(routing_config_ : routing_config) ~name:(name_ : endpoint_name) () =
  ({
     role_arn = role_arn_;
     event_buses = event_buses_;
     replication_config = replication_config_;
...
let make_create_connection_response ?last_modified_time:(last_modified_time_ : timestamp option)
    ?creation_time:(creation_time_ : timestamp option)
    ?connection_state:(connection_state_ : connection_state option)
    ?connection_arn:(connection_arn_ : connection_arn option) () =
  ({
...
let make_create_archive_response ?creation_time:(creation_time_ : timestamp option)
    ?state_reason:(state_reason_ : archive_state_reason option)
    ?state:(state_ : archive_state option) ?archive_arn:(archive_arn_ : archive_arn option) () =
  ({
     creation_time = creation_time_;
...
let make_create_api_destination_request
    ...
    ~connection_arn:(connection_arn_ : connection_arn) ~name:(name_ : api_destination_name) () =
  ({
     invocation_rate_limit_per_second = invocation_rate_limit_per_second_;
     http_method = http_method_;
     invocation_endpoint = invocation_endpoint_;
...
let make_cancel_replay_response ?state_reason:(state_reason_ : replay_state_reason option)
    ?state:(state_ : replay_state option) ?replay_arn:(replay_arn_ : replay_arn option) () =
