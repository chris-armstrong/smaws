...
let make_log_config ?include_detail:(include_detail_ : include_detail option)
    ?level:(level_ : level option) () =
  ({ include_detail = include_detail_; level = level_ } : log_config)

let make_dead_letter_config ?arn:(arn_ : resource_arn option) () =
  ({ arn = arn_ } : dead_letter_config)

let make_update_event_bus_response ?arn:(arn_ : string_ option)
...
let make_update_event_bus_request ?name:(name_ : event_bus_name option)
    ?kms_key_identifier:(kms_key_identifier_ : kms_key_identifier option)
    ?description:(description_ : event_bus_description option)
    ?dead_letter_config:(dead_letter_config_ : dead_letter_config option)
    ?log_config:(log_config_ : log_config option) () =
...
let make_failover_config ~primary:(primary_ : primary) ~secondary:(secondary_ : secondary) () =
  ({ primary = primary_; secondary = secondary_ } : failover_config)

let make_routing_config ~failover_config:(failover_config_ : failover_config) () =
  ({ failover_config = failover_config_ } : routing_config)

let make_update_endpoint_response ?name:(name_ : endpoint_name option)
    ?arn:(arn_ : endpoint_arn option) ?routing_config:(routing_config_ : routing_config option)
    ?replication_config:(replication_config_ : replication_config option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?role_arn:(role_arn_ : iam_role_arn option) ?endpoint_id:(endpoint_id_ : endpoint_id option)
    ?endpoint_url:(endpoint_url_ : endpoint_url option) ?state:(state_ : endpoint_state option) () =
  ({
     name = name_;
     arn = arn_;
...
let make_update_endpoint_request ?description:(description_ : endpoint_description option)
    ?routing_config:(routing_config_ : routing_config option)
    ?replication_config:(replication_config_ : replication_config option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?role_arn:(role_arn_ : iam_role_arn option) ~name:(name_ : endpoint_name) () =
  ({
     name = name_;
     description = description_;
     routing_config = routing_config_;
...
let make_start_replay_request ?description:(description_ : replay_description option)
    ~replay_name:(replay_name_ : replay_name) ~event_source_arn:(event_source_arn_ : archive_arn)
...
let make_redshift_data_parameters
    ...
    ?db_user:(db_user_ : db_user option) ?sql:(sql_ : sql option)
    ?statement_name:(statement_name_ : statement_name option)
    ?with_event:(with_event_ : boolean_ option) ?sqls:(sqls_ : sqls option)
...
let make_target ?role_arn:(role_arn_ : role_arn option) ?input:(input_ : target_input option)
    ...
    ?app_sync_parameters:(app_sync_parameters_ : app_sync_parameters option) ~id:(id_ : target_id)
    ~arn:(arn_ : target_arn) () =
  ({
     id = id_;
     arn = arn_;
...
let make_put_events_request ?endpoint_id:(endpoint_id_ : endpoint_id option)
  ...
  ({ entries = entries_; endpoint_id = endpoint_id_ } : put_events_request)
...
let make_list_replays_request ?name_prefix:(name_prefix_ : replay_name option)
    ?state:(state_ : replay_state option) ?event_source_arn:(event_source_arn_ : archive_arn option)
    ?next_token:(next_token_ : next_token option) ?limit:(limit_ : limit_max100 option) () =
  ({
     name_prefix = name_prefix_;
     state = state_;
...
let make_event_bus ?name:(name_ : string_ option) ?arn:(arn_ : string_ option)
    ?description:(description_ : event_bus_description option) ?policy:(policy_ : string_ option)
    ?creation_time:(creation_time_ : timestamp option)
    ?last_modified_time:(last_modified_time_ : timestamp option) () =
  ({
     name = name_;
...
let make_endpoint ?name:(name_ : endpoint_name option)
    ?description:(description_ : endpoint_description option) ?arn:(arn_ : endpoint_arn option)
    ?routing_config:(routing_config_ : routing_config option)
    ?replication_config:(replication_config_ : replication_config option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?role_arn:(role_arn_ : iam_role_arn option) ?endpoint_id:(endpoint_id_ : endpoint_id option)
...
let make_list_endpoints_response ?endpoints:(endpoints_ : endpoint_list option)
    ?next_token:(next_token_ : next_token option) () =
  ({ endpoints = endpoints_; next_token = next_token_ } : list_endpoints_response)

let make_list_endpoints_request ?name_prefix:(name_prefix_ : endpoint_name option)
    ?home_region:(home_region_ : home_region option) ?next_token:(next_token_ : next_token option)
...
let make_list_archives_request ?name_prefix:(name_prefix_ : archive_name option)
    ?event_source_arn:(event_source_arn_ : event_bus_arn option)
    ?state:(state_ : archive_state option) ?next_token:(next_token_ : next_token option)
    ?limit:(limit_ : limit_max100 option) () =
  ({
     name_prefix = name_prefix_;
     event_source_arn = event_source_arn_;
...
let make_describe_event_bus_response ?name:(name_ : string_ option) ?arn:(arn_ : string_ option)
    ...
    ?kms_key_identifier:(kms_key_identifier_ : kms_key_identifier option)
    ?dead_letter_config:(dead_letter_config_ : dead_letter_config option)
    ?policy:(policy_ : string_ option) ?log_config:(log_config_ : log_config option)
    ?creation_time:(creation_time_ : timestamp option)
    ?last_modified_time:(last_modified_time_ : timestamp option) () =
  ({
     name = name_;
...
let make_describe_endpoint_response ?name:(name_ : endpoint_name option)
    ?description:(description_ : endpoint_description option) ?arn:(arn_ : endpoint_arn option)
    ?routing_config:(routing_config_ : routing_config option)
    ?replication_config:(replication_config_ : replication_config option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
...
let make_describe_endpoint_request ?home_region:(home_region_ : home_region option)
  ...
  ({ name = name_; home_region = home_region_ } : describe_endpoint_request)
...
let make_create_event_bus_request ?event_source_name:(event_source_name_ : event_source_name option)
    ...
    ?kms_key_identifier:(kms_key_identifier_ : kms_key_identifier option)
    ?dead_letter_config:(dead_letter_config_ : dead_letter_config option)
    ?log_config:(log_config_ : log_config option) ?tags:(tags_ : tag_list option)
...
let make_create_endpoint_response ?name:(name_ : endpoint_name option)
    ?arn:(arn_ : endpoint_arn option) ?routing_config:(routing_config_ : routing_config option)
    ?replication_config:(replication_config_ : replication_config option)
    ?event_buses:(event_buses_ : endpoint_event_bus_list option)
    ?role_arn:(role_arn_ : iam_role_arn option) ?state:(state_ : endpoint_state option) () =
  ({
     name = name_;
     arn = arn_;
     routing_config = routing_config_;
...
let make_create_endpoint_request ?description:(description_ : endpoint_description option)
    ?replication_config:(replication_config_ : replication_config option)
    ?role_arn:(role_arn_ : iam_role_arn option) ~name:(name_ : endpoint_name)
