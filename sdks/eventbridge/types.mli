...
[@@ocaml.doc
  ...
  \ For more information, see \
   {{:https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-rule-event-delivery.html#eb-rule-dlq}Using \
  ...
  kms_key_identifier : kms_key_identifier option;
      [@ocaml.doc
        "The identifier of the KMS customer managed key for EventBridge to use, if you choose to \
         use a customer managed key to encrypt events on this event bus. The identifier can be the \
        ...
        \   To enable schema discovery on an event bus, choose to use an Amazon Web Services owned \
         ...
         {{:https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-encryption-event-bus-cmkey.html}Encrypting \
         events} in the {i Amazon EventBridge User Guide}.\n\
        \   \n\
        \      If you have specified that EventBridge use a customer managed key for encrypting \
         the source event bus, we strongly recommend you also specify a customer managed key for \
...
type nonrec update_connection_request = {
  kms_key_identifier : kms_key_identifier option;
      [@ocaml.doc
        "The identifier of the KMS customer managed key for EventBridge to use, if you choose to \
         use a customer managed key to encrypt this connection. The identifier can be the key \
...
type nonrec update_archive_request = {
  kms_key_identifier : kms_key_identifier option;
      [@ocaml.doc
        "The identifier of the KMS customer managed key for EventBridge to use, if you choose to \
         use a customer managed key to encrypt this archive. The identifier can be the key Amazon \
  ...
  input_template : transformer_input;
      [@ocaml.doc
        "Input template where you specify placeholders that will be filled with the values of the \
         ...
         [InputPathsMaps] value in brackets: <{i value}> \n\n\
  ...
  http_parameters : http_parameters option;
      [@ocaml.doc
        ...
        \ If you specify an API Gateway API or EventBridge ApiDestination as a target, you can use \
         this parameter to specify headers, path parameters, and query string keys/values as part \
         of your target invoking request. If you're using ApiDestinations, the corresponding \
  ...
  entries : put_partner_events_result_entry_list option;
      [@ocaml.doc
        "The results for each event entry the partner submitted in this request. If the event was \
         successfully submitted, the entry has the event ID in it. Otherwise, you can use the \
...
[@@ocaml.doc
  "An event bus receives events from a source, uses rules to evaluate them, applies any configured \
   input transformation, and routes them to the appropriate target(s). Your account's default \
   event bus receives events from Amazon Web Services services. A custom event bus can receive \
   events from your custom applications and services. A partner event bus receives events from an \
   event source created by an SaaS partner. These events come from the partners services or \
   applications.\n"]
...
[@@ocaml.doc
  "A global endpoint used to improve your application's availability by making it regional-fault \
   tolerant. For more information about global endpoints, see \
  ...
  resource_association_arn : resource_association_arn;
      [@ocaml.doc
        "For connections to private APIs, the Amazon Resource Name (ARN) of the resource \
         association EventBridge created between the connection and the private API's resource \
         configuration.\n\n\
        \ For more information, see \
