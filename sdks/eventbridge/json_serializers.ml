...
let update_event_bus_response_to_yojson (x : update_event_bus_response) =
  assoc_to_yojson
    ...
    ("Name", (option_to_yojson event_bus_name_to_yojson x.name));
...
let primary_to_yojson (x : primary) =
  assoc_to_yojson
    [("HealthCheck", (Some (health_check_to_yojson x.health_check)))]
let route_to_yojson = string_to_yojson
let secondary_to_yojson (x : secondary) =
  assoc_to_yojson [("Route", (Some (route_to_yojson x.route)))]
let failover_config_to_yojson (x : failover_config) =
  assoc_to_yojson
    [("Secondary", (Some (secondary_to_yojson x.secondary)));
    ("Primary", (Some (primary_to_yojson x.primary)))]
let routing_config_to_yojson (x : routing_config) =
  assoc_to_yojson
    [("FailoverConfig", (Some (failover_config_to_yojson x.failover_config)))]
let replication_state_to_yojson (x : replication_state) =
  match x with
  | DISABLED -> `String "DISABLED"
  | ENABLED -> `String "ENABLED"
let replication_config_to_yojson (x : replication_config) =
  assoc_to_yojson
    [("State", (option_to_yojson replication_state_to_yojson x.state))]
let non_partner_event_bus_arn_to_yojson = string_to_yojson
let endpoint_event_bus_to_yojson (x : endpoint_event_bus) =
  assoc_to_yojson
    [("EventBusArn",
       (Some (non_partner_event_bus_arn_to_yojson x.event_bus_arn)))]
let endpoint_event_bus_list_to_yojson tree =
  list_to_yojson endpoint_event_bus_to_yojson tree
...
let endpoint_state_to_yojson (x : endpoint_state) =
  match x with
...
let update_endpoint_response_to_yojson (x : update_endpoint_response) =
  assoc_to_yojson
    [("State", (option_to_yojson endpoint_state_to_yojson x.state));
    ("EndpointUrl", (option_to_yojson endpoint_url_to_yojson x.endpoint_url));
    ...
    ("Arn", (option_to_yojson endpoint_arn_to_yojson x.arn));
    ("Name", (option_to_yojson endpoint_name_to_yojson x.name))]
...
let update_connection_response_to_yojson (x : update_connection_response) =
  assoc_to_yojson
    [("LastAuthorizedTime",
       (option_to_yojson timestamp_to_yojson x.last_authorized_time));
    ("LastModifiedTime",
      (option_to_yojson timestamp_to_yojson x.last_modified_time));
...
let connection_authorization_type_to_yojson
  (x : connection_authorization_type) =
  match x with
...
let update_connection_basic_auth_request_parameters_to_yojson
  ...
  assoc_to_yojson
    [("Password",
       (option_to_yojson auth_header_parameters_sensitive_to_yojson
          x.password));
    ("Username",
...
let connection_o_auth_http_method_to_yojson
  (x : connection_o_auth_http_method) =
  match x with
  | PUT -> `String "PUT"
  | POST -> `String "POST"
  | GET -> `String "GET"
...
let connection_query_string_parameter_to_yojson
  (x : connection_query_string_parameter) =
  assoc_to_yojson
    [("IsValueSecret",
       (option_to_yojson boolean__to_yojson x.is_value_secret));
    ("Value",
...
let connection_http_parameters_to_yojson (x : connection_http_parameters) =
  assoc_to_yojson
    [("BodyParameters",
       (option_to_yojson connection_body_parameters_list_to_yojson
          x.body_parameters));
    ("QueryStringParameters",
      (option_to_yojson connection_query_string_parameters_list_to_yojson
         x.query_string_parameters));
    ("HeaderParameters",
      (option_to_yojson connection_header_parameters_list_to_yojson
         x.header_parameters))]
let update_connection_o_auth_request_parameters_to_yojson
  ...
  assoc_to_yojson
    ...
    ("ClientParameters",
      (option_to_yojson
         update_connection_o_auth_client_request_parameters_to_yojson
         x.client_parameters))]
...
let update_connection_auth_request_parameters_to_yojson
  ...
  assoc_to_yojson
    ...
    ("OAuthParameters",
      (option_to_yojson update_connection_o_auth_request_parameters_to_yojson
         x.o_auth_parameters));
    ("BasicAuthParameters",
      (option_to_yojson
         update_connection_basic_auth_request_parameters_to_yojson
...
let invalid_event_pattern_exception_to_yojson
  (x : invalid_event_pattern_exception) =
  assoc_to_yojson
    [("message", (option_to_yojson error_message_to_yojson x.message))]
...
let update_api_destination_response_to_yojson
  (x : update_api_destination_response) =
  assoc_to_yojson
    [("LastModifiedTime",
       (option_to_yojson timestamp_to_yojson x.last_modified_time));
    ("CreationTime", (option_to_yojson timestamp_to_yojson x.creation_time));
    ("ApiDestinationState",
      (option_to_yojson api_destination_state_to_yojson
         x.api_destination_state));
    ("ApiDestinationArn",
...
let update_api_destination_request_to_yojson
  (x : update_api_destination_request) =
  assoc_to_yojson
    [("InvocationRateLimitPerSecond",
       (option_to_yojson
          api_destination_invocation_rate_limit_per_second_to_yojson
...
let transformer_paths_to_yojson tree =
  map_to_yojson input_transformer_path_key_to_yojson
    target_input_path_to_yojson tree
...
let test_event_pattern_response_to_yojson (x : test_event_pattern_response) =
  assoc_to_yojson
    [("Result", (option_to_yojson boolean__to_yojson x.result))]
...
let run_command_targets_to_yojson tree =
  list_to_yojson run_command_target_to_yojson tree
...
let network_configuration_to_yojson (x : network_configuration) =
  assoc_to_yojson
    [("awsvpcConfiguration",
       (option_to_yojson aws_vpc_configuration_to_yojson
          x.awsvpc_configuration))]
...
let capacity_provider_strategy_item_to_yojson
  (x : capacity_provider_strategy_item) =
  assoc_to_yojson
    [("base",
       (option_to_yojson capacity_provider_strategy_item_base_to_yojson
          x.base));
    ("weight",
      (option_to_yojson capacity_provider_strategy_item_weight_to_yojson
         x.weight));
    ("capacityProvider",
...
let placement_constraint_to_yojson (x : placement_constraint) =
  assoc_to_yojson
    [("expression",
       (option_to_yojson placement_constraint_expression_to_yojson
          x.expression));
    ("type", (option_to_yojson placement_constraint_type_to_yojson x.type_))]
let placement_constraints_to_yojson tree =
  list_to_yojson placement_constraint_to_yojson tree
...
let placement_strategies_to_yojson tree =
  list_to_yojson placement_strategy_to_yojson tree
...
let ecs_parameters_to_yojson (x : ecs_parameters) =
  assoc_to_yojson
    ...
    ("PlacementConstraints",
      (option_to_yojson placement_constraints_to_yojson
         x.placement_constraints));
    ...
    ("CapacityProviderStrategy",
      (option_to_yojson capacity_provider_strategy_to_yojson
         x.capacity_provider_strategy));
    ("Group", (option_to_yojson string__to_yojson x.group));
    ("PlatformVersion",
      (option_to_yojson string__to_yojson x.platform_version));
    ("NetworkConfiguration",
      (option_to_yojson network_configuration_to_yojson
...
let batch_retry_strategy_to_yojson (x : batch_retry_strategy) =
  assoc_to_yojson
    [("Attempts", (option_to_yojson integer_to_yojson x.attempts))]
...
let path_parameter_list_to_yojson tree =
  list_to_yojson path_parameter_to_yojson tree
...
let http_parameters_to_yojson (x : http_parameters) =
  assoc_to_yojson
    [("QueryStringParameters",
       (option_to_yojson query_string_parameters_map_to_yojson
          x.query_string_parameters));
    ("HeaderParameters",
...
let sage_maker_pipeline_parameter_to_yojson
  (x : sage_maker_pipeline_parameter) =
  assoc_to_yojson
    [("Value",
...
let sage_maker_pipeline_parameters_to_yojson
  (x : sage_maker_pipeline_parameters) =
  assoc_to_yojson
    [("PipelineParameterList",
       (option_to_yojson sage_maker_pipeline_parameter_list_to_yojson
          x.pipeline_parameter_list))]
...
let retry_policy_to_yojson (x : retry_policy) =
  assoc_to_yojson
    [("MaximumEventAgeInSeconds",
       (option_to_yojson maximum_event_age_in_seconds_to_yojson
          x.maximum_event_age_in_seconds));
    ("MaximumRetryAttempts",
      (option_to_yojson maximum_retry_attempts_to_yojson
         x.maximum_retry_attempts))]
let graph_ql_operation_to_yojson = string_to_yojson
let app_sync_parameters_to_yojson (x : app_sync_parameters) =
  assoc_to_yojson
    [("GraphQLOperation",
...
let target_to_yojson (x : target) =
  assoc_to_yojson
    ...
    ("SageMakerPipelineParameters",
      (option_to_yojson sage_maker_pipeline_parameters_to_yojson
         x.sage_maker_pipeline_parameters));
    ("RedshiftDataParameters",
      (option_to_yojson redshift_data_parameters_to_yojson
         x.redshift_data_parameters));
    ("HttpParameters",
    ...
    ("RunCommandParameters",
      (option_to_yojson run_command_parameters_to_yojson
         x.run_command_parameters));
    ("KinesisParameters",
...
let start_replay_response_to_yojson (x : start_replay_response) =
  assoc_to_yojson
    [("ReplayStartTime",
       (option_to_yojson timestamp_to_yojson x.replay_start_time));
    ("StateReason",
...
let replay_destination_filters_to_yojson tree =
  list_to_yojson arn_to_yojson tree
...
let resource_already_exists_exception_to_yojson
  (x : resource_already_exists_exception) =
  assoc_to_yojson
    [("message", (option_to_yojson error_message_to_yojson x.message))]
...
let replay_to_yojson (x : replay) =
  assoc_to_yojson
    [("ReplayEndTime",
       (option_to_yojson timestamp_to_yojson x.replay_end_time));
    ("ReplayStartTime",
      (option_to_yojson timestamp_to_yojson x.replay_start_time));
    ("EventLastReplayedTime",
      (option_to_yojson timestamp_to_yojson x.event_last_replayed_time));
    ("EventEndTime", (option_to_yojson timestamp_to_yojson x.event_end_time));
    ("EventStartTime",
      (option_to_yojson timestamp_to_yojson x.event_start_time));
    ("StateReason",
      (option_to_yojson replay_state_reason_to_yojson x.state_reason));
    ("State", (option_to_yojson replay_state_to_yojson x.state));
    ("EventSourceArn",
...
let remove_targets_response_to_yojson (x : remove_targets_response) =
  assoc_to_yojson
    [("FailedEntries",
       (option_to_yojson remove_targets_result_entry_list_to_yojson
          x.failed_entries));
    ("FailedEntryCount",
      (option_to_yojson integer_to_yojson x.failed_entry_count))]
...
let remove_permission_request_to_yojson (x : remove_permission_request) =
  assoc_to_yojson
    [("EventBusName",
       (option_to_yojson non_partner_event_bus_name_to_yojson
          x.event_bus_name));
...
let put_targets_response_to_yojson (x : put_targets_response) =
  assoc_to_yojson
    [("FailedEntries",
       (option_to_yojson put_targets_result_entry_list_to_yojson
          x.failed_entries));
    ("FailedEntryCount",
      (option_to_yojson integer_to_yojson x.failed_entry_count))]
...
let put_partner_events_response_to_yojson (x : put_partner_events_response) =
  assoc_to_yojson
    [("Entries",
       (option_to_yojson put_partner_events_result_entry_list_to_yojson
          x.entries));
    ("FailedEntryCount",
      (option_to_yojson integer_to_yojson x.failed_entry_count))]
...
let event_resource_list_to_yojson tree =
  list_to_yojson event_resource_to_yojson tree
let put_partner_events_request_entry_to_yojson
  (x : put_partner_events_request_entry) =
  assoc_to_yojson
...
let put_events_request_entry_to_yojson (x : put_events_request_entry) =
  assoc_to_yojson
    ...
    ("EventBusName",
      (option_to_yojson non_partner_event_bus_name_or_arn_to_yojson
         x.event_bus_name));
    ("Detail", (option_to_yojson string__to_yojson x.detail));
    ("DetailType", (option_to_yojson string__to_yojson x.detail_type));
    ("Resources",
...
let put_events_request_to_yojson (x : put_events_request) =
  assoc_to_yojson
    [("EndpointId", (option_to_yojson endpoint_id_to_yojson x.endpoint_id));
    ("Entries", (Some (put_events_request_entry_list_to_yojson x.entries)))]
...
let partner_event_source_to_yojson (x : partner_event_source) =
  assoc_to_yojson
    ...
    ("Arn", (option_to_yojson string__to_yojson x.arn))]
let partner_event_source_list_to_yojson tree =
  list_to_yojson partner_event_source_to_yojson tree
...
let partner_event_source_account_to_yojson (x : partner_event_source_account)
  ...
  assoc_to_yojson
    ...
    ("ExpirationTime",
      (option_to_yojson timestamp_to_yojson x.expiration_time));
    ("CreationTime", (option_to_yojson timestamp_to_yojson x.creation_time));
...
let list_targets_by_rule_response_to_yojson
  (x : list_targets_by_rule_response) =
  assoc_to_yojson
...
let list_partner_event_sources_response_to_yojson
  (x : list_partner_event_sources_response) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("PartnerEventSources",
      (option_to_yojson partner_event_source_list_to_yojson
         x.partner_event_sources))]
let list_partner_event_sources_request_to_yojson
  (x : list_partner_event_sources_request) =
  assoc_to_yojson
    [("Limit", (option_to_yojson limit_max100_to_yojson x.limit));
    ("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("NamePrefix",
...
let list_partner_event_source_accounts_response_to_yojson
  ...
  assoc_to_yojson
    ...
    ("PartnerEventSourceAccounts",
      (option_to_yojson partner_event_source_account_list_to_yojson
         x.partner_event_source_accounts))]
...
let event_source_to_yojson (x : event_source) =
  assoc_to_yojson
    ...
    ("Arn", (option_to_yojson string__to_yojson x.arn))]
let event_source_list_to_yojson tree =
  list_to_yojson event_source_to_yojson tree
...
let list_endpoints_request_to_yojson (x : list_endpoints_request) =
  assoc_to_yojson
    [("MaxResults", (option_to_yojson limit_max100_to_yojson x.max_results));
    ("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("HomeRegion", (option_to_yojson home_region_to_yojson x.home_region));
    ("NamePrefix", (option_to_yojson endpoint_name_to_yojson x.name_prefix))]
...
let connection_to_yojson (x : connection) =
  assoc_to_yojson
    ...
    ("ConnectionArn",
      (option_to_yojson connection_arn_to_yojson x.connection_arn))]
let connection_response_list_to_yojson tree =
  list_to_yojson connection_to_yojson tree
...
let api_destination_to_yojson (x : api_destination) =
  assoc_to_yojson
    ...
    ("ApiDestinationArn",
      (option_to_yojson api_destination_arn_to_yojson x.api_destination_arn))]
let api_destination_response_list_to_yojson tree =
  list_to_yojson api_destination_to_yojson tree
let list_api_destinations_response_to_yojson
  (x : list_api_destinations_response) =
  assoc_to_yojson
    [("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("ApiDestinations",
      (option_to_yojson api_destination_response_list_to_yojson
         x.api_destinations))]
let list_api_destinations_request_to_yojson
  (x : list_api_destinations_request) =
  assoc_to_yojson
    [("Limit", (option_to_yojson limit_max100_to_yojson x.limit));
    ("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("ConnectionArn",
...
let describe_event_source_request_to_yojson
  (x : describe_event_source_request) =
  assoc_to_yojson [("Name", (Some (event_source_name_to_yojson x.name)))]
...
let describe_event_bus_request_to_yojson (x : describe_event_bus_request) =
  assoc_to_yojson
    [("Name", (option_to_yojson event_bus_name_or_arn_to_yojson x.name))]
...
let connection_o_auth_response_parameters_to_yojson
  (x : connection_o_auth_response_parameters) =
  assoc_to_yojson
    [("OAuthHttpParameters",
       (option_to_yojson connection_http_parameters_to_yojson
          x.o_auth_http_parameters));
    ...
    ("ClientParameters",
      (option_to_yojson
         connection_o_auth_client_response_parameters_to_yojson
         x.client_parameters))]
  ...
  assoc_to_yojson
    ...
    ("ApiKeyAuthParameters",
      (option_to_yojson connection_api_key_auth_response_parameters_to_yojson
         x.api_key_auth_parameters));
    ("OAuthParameters",
      (option_to_yojson connection_o_auth_response_parameters_to_yojson
         x.o_auth_parameters));
    ("BasicAuthParameters",
      (option_to_yojson connection_basic_auth_response_parameters_to_yojson
         x.basic_auth_parameters))]
...
let describe_api_destination_response_to_yojson
  (x : describe_api_destination_response) =
  assoc_to_yojson
    [("LastModifiedTime",
       (option_to_yojson timestamp_to_yojson x.last_modified_time));
    ("CreationTime", (option_to_yojson timestamp_to_yojson x.creation_time));
...
let describe_api_destination_request_to_yojson
  (x : describe_api_destination_request) =
...
let delete_partner_event_source_request_to_yojson
  (x : delete_partner_event_source_request) =
  assoc_to_yojson
...
let deauthorize_connection_response_to_yojson
  (x : deauthorize_connection_response) =
  assoc_to_yojson
    [("LastAuthorizedTime",
       (option_to_yojson timestamp_to_yojson x.last_authorized_time));
    ("LastModifiedTime",
...
let create_event_bus_response_to_yojson (x : create_event_bus_response) =
  assoc_to_yojson
    ...
    ("Description",
      (option_to_yojson event_bus_description_to_yojson x.description));
    ("EventBusArn", (option_to_yojson string__to_yojson x.event_bus_arn))]
...
let create_endpoint_request_to_yojson (x : create_endpoint_request) =
  assoc_to_yojson
    [("RoleArn", (option_to_yojson iam_role_arn_to_yojson x.role_arn));
    ("EventBuses", (Some (endpoint_event_bus_list_to_yojson x.event_buses)));
    ("ReplicationConfig",
      (option_to_yojson replication_config_to_yojson x.replication_config));
    ("RoutingConfig", (Some (routing_config_to_yojson x.routing_config)));
    ("Description",
      (option_to_yojson endpoint_description_to_yojson x.description));
    ("Name", (Some (endpoint_name_to_yojson x.name)))]
...
let create_connection_o_auth_request_parameters_to_yojson
  ...
  assoc_to_yojson
    ...
    ("ClientParameters",
      (Some
         (create_connection_o_auth_client_request_parameters_to_yojson
            x.client_parameters)))]
  ...
  assoc_to_yojson
    ...
    ("ApiKeyAuthParameters",
      (option_to_yojson
         ...
         x.api_key_auth_parameters));
    ("OAuthParameters",
      (option_to_yojson create_connection_o_auth_request_parameters_to_yojson
         x.o_auth_parameters));
    ("BasicAuthParameters",
      (option_to_yojson
         create_connection_basic_auth_request_parameters_to_yojson
         x.basic_auth_parameters))]
...
let create_api_destination_request_to_yojson
  (x : create_api_destination_request) =
  assoc_to_yojson
    [("InvocationRateLimitPerSecond",
       (option_to_yojson
          api_destination_invocation_rate_limit_per_second_to_yojson
...
let activate_event_source_request_to_yojson
  (x : activate_event_source_request) =
  assoc_to_yojson [("Name", (Some (event_source_name_to_yojson x.name)))]