...
let access_log_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "Enabled"; "S3BucketName"; "EmitInterval"; "S3BucketPrefix" ] (fun tag _ ->
      match tag with
      | "Enabled" ->
          r_enabled :=
            Some
...
let add_availability_zones_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "AvailabilityZones" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
...
let add_tags_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerNames"; "Tags" ] (fun tag _ ->
      match tag with
      | "LoadBalancerNames" ->
          ...
          r_value :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "Value"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ key = ( ! ) r_key; value = ( ! ) r_value } : additional_attribute)
...
let app_cookie_stickiness_policy_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "PolicyName"; "CookieName" ] (fun tag _ ->
      match tag with
      | "PolicyName" ->
          r_policy_name :=
            Some
          ...
          r_cookie_name :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "CookieName"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ policy_name = ( ! ) r_policy_name; cookie_name = ( ! ) r_cookie_name }
...
let apply_security_groups_to_load_balancer_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_security_groups = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "SecurityGroups" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
...
let attach_load_balancer_to_subnets_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_subnets = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "Subnets" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let backend_server_description_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "InstancePort"; "PolicyNames" ] (fun tag _ ->
      match tag with
      | "InstancePort" ->
          r_instance_port :=
            Some
          ...
          r_policy_names :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "PolicyNames"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ instance_port = ( ! ) r_instance_port; policy_names = ( ! ) r_policy_names }
...
let health_check_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "Target"; "Interval"; "Timeout"; "UnhealthyThreshold"; "HealthyThreshold" ] (fun tag _ ->
      match tag with
      | "Target" ->
          r_target :=
            Some
...
let configure_health_check_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_health_check = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "HealthCheck" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let connection_draining_of_xml i =
  let r_enabled = ref None in
  let r_timeout = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Enabled"; "Timeout" ] (fun tag _ ->
      match tag with
      | "Enabled" ->
          r_enabled :=
            Some
...
let listener_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "Protocol"; "LoadBalancerPort"; "InstanceProtocol"; "InstancePort"; "SSLCertificateId" ]
    (fun tag _ ->
      match tag with
      | "Protocol" ->
          r_protocol :=
...
let create_app_cookie_stickiness_policy_input_of_xml i =
  ...
  let r_policy_name = ref None in
  let r_cookie_name = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "PolicyName"; "CookieName" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
...
let create_lb_cookie_stickiness_policy_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "LoadBalancerName"; "PolicyName"; "CookieExpirationPeriod" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let create_load_balancer_listener_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_listeners = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "Listeners" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let policy_attribute_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "AttributeName"; "AttributeValue" ] (fun tag _ ->
      match tag with
      | "AttributeName" ->
          r_attribute_name :=
            Some
          ...
          r_attribute_value :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "AttributeValue"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ attribute_name = ( ! ) r_attribute_name; attribute_value = ( ! ) r_attribute_value }
    : policy_attribute)
...
let create_load_balancer_policy_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "LoadBalancerName"; "PolicyName"; "PolicyTypeName"; "PolicyAttributes" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let delete_load_balancer_listener_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "LoadBalancerPorts" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
...
let delete_load_balancer_policy_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_policy_name = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "PolicyName" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let deregister_end_points_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "Instances" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let describe_access_points_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerNames"; "Marker"; "PageSize" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerNames" ->
          r_load_balancer_names :=
...
let source_security_group_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "OwnerAlias"; "GroupName" ] (fun tag _ ->
      match tag with
      | "OwnerAlias" ->
          r_owner_alias :=
            Some
          ...
          r_group_name :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "GroupName"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ owner_alias = ( ! ) r_owner_alias; group_name = ( ! ) r_group_name } : source_security_group)
...
let lb_cookie_stickiness_policy_of_xml i =
  let r_policy_name = ref None in
  let r_cookie_expiration_period = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "PolicyName"; "CookieExpirationPeriod" ]
    (fun tag _ ->
      match tag with
      | "PolicyName" ->
          r_policy_name :=
...
let policies_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "AppCookieStickinessPolicies"; "LBCookieStickinessPolicies"; "OtherPolicies" ] (fun tag _ ->
      match tag with
...
let listener_description_of_xml i =
  ...
  let r_policy_names = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Listener"; "PolicyNames" ] (fun tag _ ->
      match tag with
      | "Listener" ->
          r_listener :=
          ...
          r_policy_names :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "PolicyNames"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ listener = ( ! ) r_listener; policy_names = ( ! ) r_policy_names } : listener_description)
...
let describe_access_points_output_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerDescriptions"; "NextMarker" ]
    (fun tag _ ->
      match tag with
...
let instance_state_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "InstanceId"; "State"; "ReasonCode"; "Description" ] (fun tag _ ->
      match tag with
      | "InstanceId" ->
          r_instance_id :=
            Some
...
let policy_description_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "PolicyName"; "PolicyTypeName"; "PolicyAttributeDescriptions" ] (fun tag _ ->
      match tag with
      | "PolicyName" ->
          r_policy_name :=
            Some
...
let describe_load_balancer_policies_input_of_xml i =
  let r_load_balancer_name = ref None in
  let r_policy_names = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "PolicyNames" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
          ...
          r_policy_names :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "PolicyNames"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ load_balancer_name = ( ! ) r_load_balancer_name; policy_names = ( ! ) r_policy_names }
...
let policy_attribute_type_description_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "AttributeName"; "AttributeType"; "Description"; "DefaultValue"; "Cardinality" ] (fun tag _ ->
      match tag with
      | "AttributeName" ->
          r_attribute_name :=
            Some
...
let policy_type_description_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "PolicyTypeName"; "Description"; "PolicyAttributeTypeDescriptions" ] (fun tag _ ->
      match tag with
      | "PolicyTypeName" ->
          r_policy_type_name :=
            Some
...
let tag_description_of_xml i =
  let r_load_balancer_name = ref None in
  let r_tags = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "Tags" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
          ...
          r_tags :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "Tags"
                 ...
                 ())
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({ load_balancer_name = ( ! ) r_load_balancer_name; tags = ( ! ) r_tags } : tag_description)
...
let set_load_balancer_policies_of_listener_input_of_xml i =
  ...
  let r_policy_names = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "LoadBalancerName"; "LoadBalancerPort"; "PolicyNames" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let set_load_balancer_policies_for_backend_server_input_of_xml i =
  ...
  let r_instance_port = ref None in
  let r_policy_names = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "InstancePort"; "PolicyNames" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
...
let set_load_balancer_listener_ssl_certificate_input_of_xml i =
  ...
  let r_ssl_certificate_id = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "LoadBalancerName"; "LoadBalancerPort"; "SSLCertificateId" ] (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
            Some
...
let modify_load_balancer_attributes_output_of_xml i =
  let r_load_balancer_name = ref None in
  let r_load_balancer_attributes = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "LoadBalancerName"; "LoadBalancerAttributes" ]
    (fun tag _ ->
      match tag with
      | "LoadBalancerName" ->
          r_load_balancer_name :=
