...
let make_failure_info ?error_message:(error_message_ : error_message option)
  ...
  ?status_code:(status_code_ : status_code option) () =
  ({
...
let make_tag_filter ?values:(values_ : tag_value_list option)
  ?key:(key_ : tag_key option) () =
  ({ values = values_; key = key_ } : tag_filter)
let make_summary
  ?non_compliant_resources:(non_compliant_resources_ :
                             non_compliant_resources option)
  ...
  ?last_updated:(last_updated_ : last_updated option) () =
  ({
     non_compliant_resources = non_compliant_resources_;
...
let make_get_resources_output
  ?resource_tag_mapping_list:(resource_tag_mapping_list_ :
                               resource_tag_mapping_list option)
  ?pagination_token:(pagination_token_ : pagination_token option) () =
  ({
     resource_tag_mapping_list = resource_tag_mapping_list_;
...
let make_get_resources_input
  ...
  ?exclude_compliant_resources:(exclude_compliant_resources_ :
                                 exclude_compliant_resources option)
  ?include_compliance_details:(include_compliance_details_ :
                                include_compliance_details option)
  ?resource_type_filters:(resource_type_filters_ :
                           resource_type_filter_list option)
...
let make_describe_report_creation_output
  ...
  ?start_date:(start_date_ : start_date option)
