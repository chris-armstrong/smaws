...
type nonrec failure_info = {
  error_message : error_message option; [@ocaml.doc "The message of the common error.\n"]
  error_code : error_code option;
      [@ocaml.doc
        "The code of the common error. Valid values include [InternalServiceException], \
         [InvalidParameterException], and any valid error code returned by the Amazon Web Services \
         service that hosts the resource that you want to tag.\n"]
  status_code : status_code option; [@ocaml.doc "The HTTP status code of the common error.\n"]
}
[@@ocaml.doc
  "Information about the errors that are returned for each failed resource. This information can \
   include [InternalServiceException] and [InvalidParameterException] errors. It can also include \
   any valid error code returned by the Amazon Web Services service that hosts the resource that \
   the ARN key represents.\n\n\
  \ The following are common error codes that you might receive from other Amazon Web Services \
   services:\n\
  \ \n\
  \  {ul\n\
  \        {-   {b InternalServiceException} \226\128\147 This can mean that the Resource Groups \
   Tagging API didn't receive a response from another Amazon Web Services service. It can also \
   mean that the resource type in the request is not supported by the Resource Groups Tagging API. \
   In these cases, it's safe to retry the request and then call \
   {{:https://docs.aws.amazon.com/resourcegroupstagging/latest/APIReference/API_GetResources.html}GetResources} \
   to verify the changes.\n\
  \            \n\
  \             }\n\
  \        {-   {b AccessDeniedException} \226\128\147 This can mean that you need permission to \
   call the tagging operations in the Amazon Web Services service that contains the resource. For \
   example, to use the Resource Groups Tagging API to tag a Amazon CloudWatch alarm resource, you \
   need permission to call both \
...
type nonrec failed_resources_map = (resource_ar_n * failure_info) list [@@ocaml.doc ""]

type nonrec untag_resources_output = {
  failed_resources_map : failed_resources_map option;
      [@ocaml.doc
        "A map containing a key-value pair for each failed item that couldn't be untagged. The key \
         is the ARN of the failed resource. The value is a [FailureInfo] object that contains an \
         error code, a status code, and an error message. If there are no errors, the \
...
type nonrec internal_service_exception = { message : exception_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "The request processing failed because of an unknown error, exception, or failure. You can retry \
   the request.\n"]

type nonrec target_id_type = ROOT [@ocaml.doc ""] | OU [@ocaml.doc ""] | ACCOUNT [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec tag_resources_output = {
  failed_resources_map : failed_resources_map option;
      [@ocaml.doc
        "A map containing a key-value pair for each failed item that couldn't be tagged. The key \
         is the ARN of the failed resource. The value is a [FailureInfo] object that contains an \
         error code, a status code, and an error message. If there are no errors, the \
         [FailedResourcesMap] is empty.\n"]
}
...
[@@ocaml.doc
  "The metadata that you apply to Amazon Web Services resources to help you categorize and \
   organize them. Each tag consists of a key and a value, both of which you define. For more \
   information, see {{:https://docs.aws.amazon.com/general/latest/gr/aws_tagging.html}Tagging \
   Amazon Web Services Resources} in the {i Amazon Web Services General Reference}.\n"]
...
type nonrec resource_tag_mapping = {
  ...
  tags : tag_list option;
      [@ocaml.doc "The tags that have been applied to one or more Amazon Web Services resources.\n"]
  resource_ar_n : resource_ar_n option; [@ocaml.doc "The ARN of the resource.\n"]
}
[@@ocaml.doc
...
type nonrec get_tag_values_output = {
  tag_values : tag_values_output_list option;
      [@ocaml.doc
        "A list of all tag values for the specified key currently used in the specified Amazon Web \
         Services Region for the calling account.\n"]
  pagination_token : pagination_token option;
      [@ocaml.doc
        "A string that indicates that there is more data available than this response contains. To \
         receive the next part of the response, specify this response value as the \
         [PaginationToken] value in the request for the next page.\n"]
}
[@@ocaml.doc ""]
...
type nonrec get_tag_values_input = {
  ...
  pagination_token : pagination_token option;
      [@ocaml.doc
        "Specifies a [PaginationToken] response value from a previous request to indicate that you \
...
type nonrec get_tag_keys_output = {
  tag_keys : tag_key_list option;
      [@ocaml.doc "A list of all tag keys in the Amazon Web Services account.\n"]
  pagination_token : pagination_token option;
      [@ocaml.doc
        "A string that indicates that there is more data available than this response contains. To \
         receive the next part of the response, specify this response value as the \
...
type nonrec get_resources_output = {
  resource_tag_mapping_list : resource_tag_mapping_list option;
      [@ocaml.doc "A list of resource ARNs and the tags (keys and values) associated with each.\n"]
  pagination_token : pagination_token option;
      [@ocaml.doc
        "A string that indicates that there is more data available than this response contains. To \
         receive the next part of the response, specify this response value as the \
  ...
  exclude_compliant_resources : exclude_compliant_resources option;
      [@ocaml.doc
        "Specifies whether to exclude resources that are compliant with the tag policy. Set this \
         to [true] if you are interested in retrieving information on noncompliant resources \
         only.\n\n\
  ...
  resources_per_page : resources_per_page option;
      [@ocaml.doc
        "Specifies the maximum number of results to be returned in each page. A query can return \
         ...
         always check the [PaginationToken] response value to see if there are more results. You \
         can specify a minimum of 1 and a maximum value of 100.\n"]
...
type nonrec get_compliance_summary_input = {
  ...
  tag_key_filters : tag_key_filter_list option;
      [@ocaml.doc
        "Specifies that you want the response to include information for only resources that have \
         tags with the specified tag keys. If you use this parameter, the count of returned \
  ...
  resource_type_filters : resource_type_filter_list option;
      [@ocaml.doc
        "Specifies that you want the response to include information for only resources of the \
         specified types. The format of each resource type is [service\\[:resourceType\\]]. For \
         example, specifying a resource type of [ec2] returns all Amazon EC2 resources (which \
         includes EC2 instances). Specifying a resource type of [ec2:instance] returns only EC2 \
         instances.\n\n\
        \ The string for each service name and resource type is the same as that embedded in a \
        ...
        \        {-  For resource type strings, see \
         {{:https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arns-syntax}Example \
         ARNs}.\n\
        \            \n\
        \             }\n\
  ...
  region_filters : region_filter_list option;
      [@ocaml.doc
        "Specifies a list of Amazon Web Services Regions to limit the output to. If you use this \
         parameter, the count of returned noncompliant resources includes only resources in the \
         specified Regions.\n"]
  target_id_filters : target_id_filter_list option;
      [@ocaml.doc
        "Specifies target identifiers (usually, specific account IDs) to limit the output by. If \
...
type nonrec describe_report_creation_output = {
  ...
  start_date : start_date option; [@ocaml.doc "The date and time that the report was started. \n"]
  s3_location : s3_location option;
      [@ocaml.doc "The path to the Amazon S3 bucket where the report was stored on creation.\n"]
