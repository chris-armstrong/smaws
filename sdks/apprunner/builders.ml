...
let make_vpc_ingress_connection_summary ?service_arn:(service_arn_ : app_runner_resource_arn option)
    ?vpc_ingress_connection_arn:(vpc_ingress_connection_arn_ : app_runner_resource_arn option) () =
  ({ service_arn = service_arn_; vpc_ingress_connection_arn = vpc_ingress_connection_arn_ }
    : vpc_ingress_connection_summary)

let make_ingress_vpc_configuration ?vpc_endpoint_id:(vpc_endpoint_id_ : string_ option)
...
let make_vpc_connector ?deleted_at:(deleted_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option) ?status:(status_ : vpc_connector_status option)
...
let make_source_code_version ~value:(value_ : string_) ~type_:(type__ : source_code_version_type) ()
    =
...
let make_code_configuration_values
    ...
    ?port:(port_ : string_ option) ?start_command:(start_command_ : start_command option)
    ?build_command:(build_command_ : build_command option) ~runtime:(runtime_ : runtime) () =
  ({
...
let make_code_repository ?source_directory:(source_directory_ : source_directory option)
...
let make_authentication_configuration ?access_role_arn:(access_role_arn_ : role_arn option)
...
let make_instance_configuration ?instance_role_arn:(instance_role_arn_ : role_arn option)
...
let make_health_check_configuration
    ...
    ?interval:(interval_ : health_check_interval option) ?path:(path_ : health_check_path option)
...
let make_auto_scaling_configuration_summary ?is_default:(is_default_ : is_default option)
...
let make_egress_configuration
  ...
  ({ vpc_connector_arn = vpc_connector_arn_; egress_type = egress_type_ } : egress_configuration)
...
let make_network_configuration ?ip_address_type:(ip_address_type_ : ip_address_type option)
...
let make_service
    ...
    ?deleted_at:(deleted_at_ : timestamp option) ?service_url:(service_url_ : string_ option)
    ...
    ~created_at:(created_at_ : timestamp) ~service_arn:(service_arn_ : app_runner_resource_arn)
    ~service_id:(service_id_ : service_id) ~service_name:(service_name_ : service_name) () =
  ({
...
let make_update_service_response ~operation_id:(operation_id_ : uui_d) ~service:(service_ : service)
    () =
...
let make_auto_scaling_configuration ?is_default:(is_default_ : is_default option)
    ?has_associated_service:(has_associated_service_ : has_associated_service option)
    ?deleted_at:(deleted_at_ : timestamp option) ?created_at:(created_at_ : timestamp option)
    ...
    ?status:(status_ : auto_scaling_configuration_status option) ?latest:(latest_ : latest option)
...
let make_operation_summary ?updated_at:(updated_at_ : timestamp option)
    ?ended_at:(ended_at_ : timestamp option) ?started_at:(started_at_ : timestamp option)
...
let make_observability_configuration ?deleted_at:(deleted_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option)
...
let make_list_vpc_ingress_connections_filter ?vpc_endpoint_id:(vpc_endpoint_id_ : string_ option)
    ?service_arn:(service_arn_ : app_runner_resource_arn option) () =
  ({ vpc_endpoint_id = vpc_endpoint_id_; service_arn = service_arn_ }
    : list_vpc_ingress_connections_filter)

let make_list_vpc_ingress_connections_request ?next_token:(next_token_ : next_token option)
...
let make_list_vpc_connectors_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) () =
...
let make_list_services_request ?max_results:(max_results_ : service_max_results option)
...
let make_list_operations_response ?next_token:(next_token_ : string_ option)
  ...
  ({ next_token = next_token_; operation_summary_list = operation_summary_list_ }
    : list_operations_response)

let make_list_operations_request ?max_results:(max_results_ : list_operations_max_results option)
    ?next_token:(next_token_ : string_ option) ~service_arn:(service_arn_ : app_runner_resource_arn)
    () =
  ({ max_results = max_results_; next_token = next_token_; service_arn = service_arn_ }
...
let make_list_observability_configurations_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) ?latest_only:(latest_only_ : boolean_ option)
    ?observability_configuration_name:
      (observability_configuration_name_ : observability_configuration_name option) () =
  ({
     next_token = next_token_;
...
let make_list_connections_response ?next_token:(next_token_ : next_token option)
  ...
  ({ next_token = next_token_; connection_summary_list = connection_summary_list_ }
    : list_connections_response)

let make_list_connections_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option)
    ?connection_name:(connection_name_ : connection_name option) () =
  ({ next_token = next_token_; max_results = max_results_; connection_name = connection_name_ }
...
let make_list_auto_scaling_configurations_request ?next_token:(next_token_ : next_token option)
    ?max_results:(max_results_ : max_results option) ?latest_only:(latest_only_ : boolean_ option)
    ?auto_scaling_configuration_name:
      (auto_scaling_configuration_name_ : auto_scaling_configuration_name option) () =
  ({
...
let make_certificate_validation_record
    ?status:(status_ : certificate_validation_record_status option) ?value:(value_ : string_ option)
...
let make_disassociate_custom_domain_response
    ...
    ~service_arn:(service_arn_ : app_runner_resource_arn) ~dns_target:(dns_target_ : string_) () =
  ({
...
let make_disassociate_custom_domain_request ~domain_name:(domain_name_ : domain_name)
    ~service_arn:(service_arn_ : app_runner_resource_arn) () =
...
let make_delete_service_response ~operation_id:(operation_id_ : uui_d) ~service:(service_ : service)
    () =
...
let make_create_service_response ~operation_id:(operation_id_ : uui_d) ~service:(service_ : service)
    () =
...
let make_create_observability_configuration_request ?tags:(tags_ : tag_list option)
    ?trace_configuration:(trace_configuration_ : trace_configuration option)
    ~observability_configuration_name:
...
let make_create_connection_request ?tags:(tags_ : tag_list option)
  ...
  ({ tags = tags_; provider_type = provider_type_; connection_name = connection_name_ }
...
let make_create_auto_scaling_configuration_request ?tags:(tags_ : tag_list option)
...
let make_associate_custom_domain_response ~vpc_dns_targets:(vpc_dns_targets_ : vpc_dns_target_list)
    ~custom_domain:(custom_domain_ : custom_domain)
    ~service_arn:(service_arn_ : app_runner_resource_arn) ~dns_target:(dns_target_ : string_) () =
  ({
     vpc_dns_targets = vpc_dns_targets_;
...
let make_associate_custom_domain_request
    ...
    ~domain_name:(domain_name_ : domain_name) ~service_arn:(service_arn_ : app_runner_resource_arn)
    () =
  ({
     enable_www_subdomain = enable_www_subdomain_;
     domain_name = domain_name_;
