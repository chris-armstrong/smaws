...
let impersonation_role_type_to_yojson (x : impersonation_role_type) =
  match x with READ_ONLY -> `String "READ_ONLY" | FULL_ACCESS -> `String "FULL_ACCESS"
...
let delegate_to_yojson (x : delegate) =
  assoc_to_yojson
    [ ("Type", Some (member_type_to_yojson x.type_)); ("Id", Some (string__to_yojson x.id)) ]
...
let personal_access_token_summary_to_yojson (x : personal_access_token_summary) =
  assoc_to_yojson
    [
      ...
      ("DateLastUsed", option_to_yojson timestamp__to_yojson x.date_last_used);
...
let describe_user_response_to_yojson (x : describe_user_response) =
  assoc_to_yojson
    [
      ...
      ("MailboxProvisionedDate", option_to_yojson timestamp__to_yojson x.mailbox_provisioned_date);
      ("DisabledDate", option_to_yojson timestamp__to_yojson x.disabled_date);
      ("EnabledDate", option_to_yojson timestamp__to_yojson x.enabled_date);
      ("UserRole", option_to_yojson user_role_to_yojson x.user_role);
      ("State", option_to_yojson entity_state_to_yojson x.state);
...
let describe_organization_response_to_yojson (x : describe_organization_response) =
  assoc_to_yojson
    [
      ...
      ("ErrorMessage", option_to_yojson string__to_yojson x.error_message);
      ("CompletedDate", option_to_yojson timestamp__to_yojson x.completed_date);
...
let entity_type_to_yojson (x : entity_type) =
  match x with RESOURCE -> `String "RESOURCE" | USER -> `String "USER" | GROUP -> `String "GROUP"
