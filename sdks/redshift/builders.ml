...
let make_network_interface ?ipv6_address:(ipv6_address_ : string_ option)
    ...
    ?network_interface_id:(network_interface_id_ : string_ option) () =
  ({
...
let make_vpc_endpoint ?network_interfaces:(network_interfaces_ : network_interface_list option)
    ?vpc_id:(vpc_id_ : string_ option) ?vpc_endpoint_id:(vpc_endpoint_id_ : string_ option) () =
  ({
...
let make_usage_limit ?tags:(tags_ : tag_list option)
    ?breach_action:(breach_action_ : usage_limit_breach_action option)
    ?period:(period_ : usage_limit_period option) ?amount:(amount_ : long option)
    ?limit_type:(limit_type_ : usage_limit_limit_type option)
    ?feature_type:(feature_type_ : usage_limit_feature_type option)
    ?cluster_identifier:(cluster_identifier_ : string_ option)
    ?usage_limit_id:(usage_limit_id_ : string_ option) () =
  ({
     tags = tags_;
...
let make_usage_limit_list ?marker:(marker_ : string_ option)
    ?usage_limits:(usage_limits_ : usage_limits option) () =
  ({ marker = marker_; usage_limits = usage_limits_ } : usage_limit_list)
...
let make_partner_integration_output_message
  ...
  ({ partner_name = partner_name_; database_name = database_name_ }
    : partner_integration_output_message)

let make_maintenance_track ?update_targets:(update_targets_ : eligible_tracks_to_update_list option)
    ?database_version:(database_version_ : string_ option)
    ?maintenance_track_name:(maintenance_track_name_ : string_ option) () =
  ({
...
let make_track_list_message ?marker:(marker_ : string_ option)
    ?maintenance_tracks:(maintenance_tracks_ : track_list option) () =
  ({ marker = marker_; maintenance_tracks = maintenance_tracks_ } : track_list_message)

let make_tagged_resource ?resource_type:(resource_type_ : string_ option)
    ?resource_name:(resource_name_ : string_ option) ?tag:(tag_ : tag option) () =
  ({ resource_type = resource_type_; resource_name = resource_name_; tag = tag_ } : tagged_resource)

let make_tagged_resource_list_message ?marker:(marker_ : string_ option)
    ?tagged_resources:(tagged_resources_ : tagged_resource_list option) () =
  ({ marker = marker_; tagged_resources = tagged_resources_ } : tagged_resource_list_message)

let make_table_restore_status ?new_table_name:(new_table_name_ : string_ option)
    ...
    ?request_time:(request_time_ : t_stamp option) ?message:(message_ : string_ option)
    ?status:(status_ : table_restore_status_type option)
...
let make_table_restore_status_message ?marker:(marker_ : string_ option)
    ?table_restore_status_details:(table_restore_status_details_ : table_restore_status_list option)
    () =
  ({ marker = marker_; table_restore_status_details = table_restore_status_details_ }
...
let make_availability_zone
  ...
  ({ supported_platforms = supported_platforms_; name = name_ } : availability_zone)
...
let make_snapshot_sorting_entity ?sort_order:(sort_order_ : sort_by_order option)
  ...
  ({ sort_order = sort_order_; attribute = attribute_ } : snapshot_sorting_entity)

let make_cluster_associated_to_schedule
...
let make_snapshot_schedule
    ...
    ?tags:(tags_ : tag_list option) ?schedule_description:(schedule_description_ : string_ option)
...
let make_account_with_restore_access ?account_alias:(account_alias_ : string_ option)
    ?account_id:(account_id_ : string_ option) () =
  ({ account_alias = account_alias_; account_id = account_id_ } : account_with_restore_access)

let make_snapshot ?snapshot_arn:(snapshot_arn_ : string_ option)
    ...
    ?tags:(tags_ : tag_list option) ?source_region:(source_region_ : string_ option)
    ...
    ?kms_key_id:(kms_key_id_ : string_ option) ?encrypted:(encrypted_ : boolean_ option)
    ?vpc_id:(vpc_id_ : string_ option) ?db_name:(db_name_ : string_ option)
    ?number_of_nodes:(number_of_nodes_ : integer option) ?node_type:(node_type_ : string_ option)
    ...
    ?availability_zone:(availability_zone_ : string_ option) ?port:(port_ : integer option)
...
let make_snapshot_message ?snapshots:(snapshots_ : snapshot_list option)
    ?marker:(marker_ : string_ option) () =
  ({ snapshots = snapshots_; marker = marker_ } : snapshot_message)

let make_snapshot_error_message ?failure_reason:(failure_reason_ : string_ option)
...
let make_snapshot_copy_grant ?tags:(tags_ : tag_list option)
    ?kms_key_id:(kms_key_id_ : string_ option)
    ?snapshot_copy_grant_name:(snapshot_copy_grant_name_ : string_ option) () =
  ({ tags = tags_; kms_key_id = kms_key_id_; snapshot_copy_grant_name = snapshot_copy_grant_name_ }
    : snapshot_copy_grant)

let make_snapshot_copy_grant_message
    ?snapshot_copy_grants:(snapshot_copy_grants_ : snapshot_copy_grant_list option)
    ?marker:(marker_ : string_ option) () =
  ({ snapshot_copy_grants = snapshot_copy_grants_; marker = marker_ } : snapshot_copy_grant_message)
...
let make_serverless_identifier ~workgroup_identifier:(workgroup_identifier_ : string_)
    ~namespace_identifier:(namespace_identifier_ : string_) () =
  ({ workgroup_identifier = workgroup_identifier_; namespace_identifier = namespace_identifier_ }
    : serverless_identifier)

let make_cluster_node ?public_ip_address:(public_ip_address_ : string_ option)
    ?private_ip_address:(private_ip_address_ : string_ option)
    ?node_role:(node_role_ : string_ option) () =
  ({
...
let make_secondary_cluster_info ?cluster_nodes:(cluster_nodes_ : cluster_nodes_list option)
    ?availability_zone:(availability_zone_ : string_ option) () =
  ({ cluster_nodes = cluster_nodes_; availability_zone = availability_zone_ }
    : secondary_cluster_info)

let make_resize_cluster_message
    ...
    ?node_type:(node_type_ : string_ option) ?cluster_type:(cluster_type_ : string_ option)
...
let make_scheduled_action_type ?resume_cluster:(resume_cluster_ : resume_cluster_message option)
    ?pause_cluster:(pause_cluster_ : pause_cluster_message option)
    ?resize_cluster:(resize_cluster_ : resize_cluster_message option) () =
  ({
...
let make_scheduled_action ?end_time:(end_time_ : t_stamp option)
    ...
    ?iam_role:(iam_role_ : string_ option) ?schedule:(schedule_ : string_ option)
...
let make_scheduled_actions_message
    ?scheduled_actions:(scheduled_actions_ : scheduled_action_list option)
    ?marker:(marker_ : string_ option) () =
  ({ scheduled_actions = scheduled_actions_; marker = marker_ } : scheduled_actions_message)

let make_scheduled_action_filter ~values:(values_ : value_string_list)
    ~name:(name_ : scheduled_action_filter_name) () =
  ({ values = values_; name = name_ } : scheduled_action_filter)

let make_endpoint ?vpc_endpoints:(vpc_endpoints_ : vpc_endpoints_list option)
    ?port:(port_ : integer option) ?address:(address_ : string_ option) () =
  ({ vpc_endpoints = vpc_endpoints_; port = port_; address = address_ } : endpoint)

let make_cluster_security_group_membership ?status:(status_ : string_ option)
    ?cluster_security_group_name:(cluster_security_group_name_ : string_ option) () =
  ({ status = status_; cluster_security_group_name = cluster_security_group_name_ }
    : cluster_security_group_membership)

let make_cluster_parameter_status
...
let make_pending_modified_values ?encryption_type:(encryption_type_ : string_ option)
    ?maintenance_track_name:(maintenance_track_name_ : string_ option)
...
let make_data_transfer_progress
...
let make_hsm_status ?status:(status_ : string_ option)
...
let make_cluster_snapshot_copy_status
    ?snapshot_copy_grant_name:(snapshot_copy_grant_name_ : string_ option)
    ...
    ?destination_region:(destination_region_ : string_ option) () =
  ({
...
let make_elastic_ip_status ?status:(status_ : string_ option)
    ?elastic_ip:(elastic_ip_ : string_ option) () =
  ({ status = status_; elastic_ip = elastic_ip_ } : elastic_ip_status)

let make_cluster_iam_role ?apply_status:(apply_status_ : string_ option)
    ?iam_role_arn:(iam_role_arn_ : string_ option) () =
  ({ apply_status = apply_status_; iam_role_arn = iam_role_arn_ } : cluster_iam_role)
...
let make_resize_info ?allow_cancel_resize:(allow_cancel_resize_ : boolean_ option)
    ?resize_type:(resize_type_ : string_ option) () =
  ({ allow_cancel_resize = allow_cancel_resize_; resize_type = resize_type_ } : resize_info)

let make_aqua_configuration
    ?aqua_configuration_status:(aqua_configuration_status_ : aqua_configuration_status option)
    ?aqua_status:(aqua_status_ : aqua_status option) () =
  ({ aqua_configuration_status = aqua_configuration_status_; aqua_status = aqua_status_ }
    : aqua_configuration)
...
let make_cluster
    ...
    ?multi_a_z:(multi_a_z_ : string_ option) ?ip_address_type:(ip_address_type_ : string_ option)
    ?master_password_secret_kms_key_id:(master_password_secret_kms_key_id_ : string_ option)
    ?master_password_secret_arn:(master_password_secret_arn_ : string_ option)
    ...
    ?cluster_create_time:(cluster_create_time_ : t_stamp option)
    ?endpoint:(endpoint_ : endpoint option) ?db_name:(db_name_ : string_ option)
    ?master_username:(master_username_ : string_ option)
    ?modify_status:(modify_status_ : string_ option)
    ?cluster_availability_status:(cluster_availability_status_ : string_ option)
    ?cluster_status:(cluster_status_ : string_ option) ?node_type:(node_type_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
...
let make_revoke_snapshot_access_message
    ?snapshot_cluster_identifier:(snapshot_cluster_identifier_ : string_ option)
...
let make_revoke_endpoint_access_message ?force:(force_ : boolean_ option)
    ?vpc_ids:(vpc_ids_ : vpc_identifier_list option) ?account:(account_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
     force = force_;
     vpc_ids = vpc_ids_;
...
let make_endpoint_authorization ?endpoint_count:(endpoint_count_ : integer option)
    ...
    ?cluster_identifier:(cluster_identifier_ : string_ option) ?grantee:(grantee_ : string_ option)
    ?grantor:(grantor_ : string_ option) () =
  ({
...
let make_ec2_security_group ?tags:(tags_ : tag_list option)
...
let make_ip_range ?tags:(tags_ : tag_list option) ?cidri_p:(cidri_p_ : string_ option)
    ?status:(status_ : string_ option) () =
  ({ tags = tags_; cidri_p = cidri_p_; status = status_ } : ip_range)

let make_cluster_security_group ?tags:(tags_ : tag_list option)
...
let make_revoke_cluster_security_group_ingress_message
    ?ec2_security_group_owner_id:(ec2_security_group_owner_id_ : string_ option)
    ?ec2_security_group_name:(ec2_security_group_name_ : string_ option)
    ?cidri_p:(cidri_p_ : string_ option)
...
let make_restore_table_from_cluster_snapshot_message
    ...
    ?target_database_name:(target_database_name_ : string_ option)
    ?source_schema_name:(source_schema_name_ : string_ option)
    ~new_table_name:(new_table_name_ : string_) ~source_table_name:(source_table_name_ : string_)
...
let make_restore_from_cluster_snapshot_message
    ...
    ?enhanced_vpc_routing:(enhanced_vpc_routing_ : boolean_optional option)
    ?node_type:(node_type_ : string_ option) ?kms_key_id:(kms_key_id_ : string_ option)
    ?manual_snapshot_retention_period:(manual_snapshot_retention_period_ : integer_optional option)
    ?automated_snapshot_retention_period:
      (automated_snapshot_retention_period_ : integer_optional option)
    ?preferred_maintenance_window:(preferred_maintenance_window_ : string_ option)
...
let make_resize_progress_message
    ...
    ?message:(message_ : string_ option) ?resize_type:(resize_type_ : string_ option)
    ?estimated_time_to_completion_in_seconds:
      (estimated_time_to_completion_in_seconds_ : long_optional option)
    ?elapsed_time_in_seconds:(elapsed_time_in_seconds_ : long_optional option)
    ?progress_in_mega_bytes:(progress_in_mega_bytes_ : long_optional option)
    ...
    ?status:(status_ : string_ option) ?target_cluster_type:(target_cluster_type_ : string_ option)
    ?target_number_of_nodes:(target_number_of_nodes_ : integer_optional option)
    ?target_node_type:(target_node_type_ : string_ option) () =
  ({
...
let make_reset_cluster_parameter_group_message ?parameters:(parameters_ : parameters_list option)
...
let make_reserved_node
    ...
    ?node_count:(node_count_ : integer option) ?currency_code:(currency_code_ : string_ option)
...
let make_reserved_nodes_message ?reserved_nodes:(reserved_nodes_ : reserved_node_list option)
    ?marker:(marker_ : string_ option) () =
  ({ reserved_nodes = reserved_nodes_; marker = marker_ } : reserved_nodes_message)
...
let make_reserved_node_offerings_message
    ?reserved_node_offerings:(reserved_node_offerings_ : reserved_node_offering_list option)
    ?marker:(marker_ : string_ option) () =
  ({ reserved_node_offerings = reserved_node_offerings_; marker = marker_ }
...
let make_data_share_association
    ...
    ?consumer_identifier:(consumer_identifier_ : string_ option) () =
  ({
...
let make_data_share ?data_share_type:(data_share_type_ : data_share_type option)
    ...
    ?data_share_arn:(data_share_arn_ : string_ option) () =
  ({
...
let make_register_namespace_input_message
  ...
  ({ consumer_identifiers = consumer_identifiers_; namespace_identifier = namespace_identifier_ }
    : register_namespace_input_message)

let make_reference_link ?link:(link_ : string_ option) ?text:(text_ : string_ option) () =
  ({ link = link_; text = text_ } : reference_link)
...
let make_put_resource_policy_message ~policy:(policy_ : string_)
    ~resource_arn:(resource_arn_ : string_) () =
  ({ policy = policy_; resource_arn = resource_arn_ } : put_resource_policy_message)
...
let make_purchase_reserved_node_offering_message ?node_count:(node_count_ : integer_optional option)
  ...
  ({ node_count = node_count_; reserved_node_offering_id = reserved_node_offering_id_ }
    : purchase_reserved_node_offering_message)

let make_modify_usage_limit_message
    ?breach_action:(breach_action_ : usage_limit_breach_action option)
    ?amount:(amount_ : long_optional option) ~usage_limit_id:(usage_limit_id_ : string_) () =
  ({ breach_action = breach_action_; amount = amount_; usage_limit_id = usage_limit_id_ }
    : modify_usage_limit_message)

let make_modify_snapshot_schedule_message
    ~schedule_definitions:(schedule_definitions_ : schedule_definition_list)
    ~schedule_identifier:(schedule_identifier_ : string_) () =
  ({ schedule_definitions = schedule_definitions_; schedule_identifier = schedule_identifier_ }
...
let make_modify_scheduled_action_message ?enable:(enable_ : boolean_optional option)
    ?end_time:(end_time_ : t_stamp option) ?start_time:(start_time_ : t_stamp option)
    ?scheduled_action_description:(scheduled_action_description_ : string_ option)
    ?iam_role:(iam_role_ : string_ option) ?schedule:(schedule_ : string_ option)
    ?target_action:(target_action_ : scheduled_action_type option)
...
let make_redshift_idc_application ?sso_tag_keys:(sso_tag_keys_ : tag_key_list option)
    ?tags:(tags_ : tag_list option) ?application_type:(application_type_ : application_type option)
    ...
    ?idc_instance_arn:(idc_instance_arn_ : string_ option) () =
  ({
...
let make_lakehouse_configuration ?catalog_arn:(catalog_arn_ : string_ option)
    ?lakehouse_registration_status:(lakehouse_registration_status_ : string_ option)
...
let make_modify_lakehouse_configuration_message ?dry_run:(dry_run_ : boolean_optional option)
    ?lakehouse_idc_application_arn:(lakehouse_idc_application_arn_ : string_ option)
...
let make_integration ?tags:(tags_ : tag_list option)
    ?additional_encryption_context:(additional_encryption_context_ : encryption_context_map option)
    ?kms_key_id:(kms_key_id_ : string_ option) ?description:(description_ : description option)
    ...
    ?target_arn:(target_arn_ : target_arn option) ?source_arn:(source_arn_ : source_arn option)
...
let make_event_subscription ?tags:(tags_ : tag_list option) ?enabled:(enabled_ : boolean_ option)
    ?severity:(severity_ : string_ option)
...
let make_modify_event_subscription_message ?enabled:(enabled_ : boolean_optional option)
    ...
    ?source_ids:(source_ids_ : source_ids_list option) ?source_type:(source_type_ : string_ option)
...
let make_endpoint_access ?vpc_endpoint:(vpc_endpoint_ : vpc_endpoint option)
    ?vpc_security_groups:(vpc_security_groups_ : vpc_security_group_membership_list option)
    ?address:(address_ : string_ option) ?port:(port_ : integer option)
    ?endpoint_create_time:(endpoint_create_time_ : t_stamp option)
...
let make_cluster_subnet_group
    ...
    ?tags:(tags_ : tag_list option) ?subnets:(subnets_ : subnet_list option)
...
let make_modify_cluster_snapshot_message ?force:(force_ : boolean_ option)
    ?manual_snapshot_retention_period:(manual_snapshot_retention_period_ : integer_optional option)
    ~snapshot_identifier:(snapshot_identifier_ : string_) () =
  ({
     force = force_;
...
let make_modify_cluster_parameter_group_message ~parameters:(parameters_ : parameters_list)
    ~parameter_group_name:(parameter_group_name_ : string_) () =
  ({ parameters = parameters_; parameter_group_name = parameter_group_name_ }
...
let make_modify_cluster_iam_roles_message
    ?default_iam_role_arn:(default_iam_role_arn_ : string_ option)
    ?remove_iam_roles:(remove_iam_roles_ : iam_role_arn_list option)
    ?add_iam_roles:(add_iam_roles_ : iam_role_arn_list option)
    ~cluster_identifier:(cluster_identifier_ : string_) () =
  ({
     default_iam_role_arn = default_iam_role_arn_;
...
let make_modify_cluster_db_revision_message ~revision_target:(revision_target_ : string_)
    ~cluster_identifier:(cluster_identifier_ : string_) () =
  ({ revision_target = revision_target_; cluster_identifier = cluster_identifier_ }
    : modify_cluster_db_revision_message)

let make_modify_cluster_message
    ?extra_compute_for_automatic_optimization:
    ...
    ?availability_zone_relocation:(availability_zone_relocation_ : boolean_optional option)
    ?kms_key_id:(kms_key_id_ : string_ option) ?encrypted:(encrypted_ : boolean_optional option)
    ?maintenance_track_name:(maintenance_track_name_ : string_ option)
    ?enhanced_vpc_routing:(enhanced_vpc_routing_ : boolean_optional option)
    ?elastic_ip:(elastic_ip_ : string_ option)
    ?publicly_accessible:(publicly_accessible_ : boolean_optional option)
...
let make_recommended_action ?type_:(type__ : recommended_action_type option)
    ?command:(command_ : string_ option) ?database:(database_ : string_ option)
    ?text:(text_ : string_ option) () =
  ({ type_ = type__; command = command_; database = database_; text = text_ } : recommended_action)

let make_recommendation ?reference_links:(reference_links_ : reference_link_list option)
    ...
    ?observation:(observation_ : string_ option) ?description:(description_ : string_ option)
    ?title:(title_ : string_ option) ?recommendation_type:(recommendation_type_ : string_ option)
    ?created_at:(created_at_ : t_stamp option) ?namespace_arn:(namespace_arn_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) ?id:(id_ : string_ option) () =
  ({
...
let make_list_recommendations_message ?marker:(marker_ : string_ option)
...
let make_get_reserved_node_exchange_offerings_output_message
    ?reserved_node_offerings:(reserved_node_offerings_ : reserved_node_offering_list option)
    ?marker:(marker_ : string_ option) () =
  ({ reserved_node_offerings = reserved_node_offerings_; marker = marker_ }
    : get_reserved_node_exchange_offerings_output_message)

let make_get_reserved_node_exchange_offerings_input_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ~reserved_node_id:(reserved_node_id_ : string_) () =
  ({ marker = marker_; max_records = max_records_; reserved_node_id = reserved_node_id_ }
...
let make_get_identity_center_auth_token_response
    ?expiration_time:(expiration_time_ : t_stamp option) ?token:(token_ : sensitive_string option)
    () =
...
let make_cluster_extended_credentials ?next_refresh_time:(next_refresh_time_ : t_stamp option)
    ?expiration:(expiration_ : t_stamp option) ?db_password:(db_password_ : sensitive_string option)
    ?db_user:(db_user_ : string_ option) () =
  ({
...
let make_get_cluster_credentials_with_iam_message
    ?custom_domain_name:(custom_domain_name_ : string_ option)
    ?duration_seconds:(duration_seconds_ : integer_optional option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) ?db_name:(db_name_ : string_ option)
    () =
  ({
     custom_domain_name = custom_domain_name_;
...
let make_cluster_credentials ?expiration:(expiration_ : t_stamp option)
    ?db_password:(db_password_ : sensitive_string option) ?db_user:(db_user_ : string_ option) () =
  ({ expiration = expiration_; db_password = db_password_; db_user = db_user_ }
    : cluster_credentials)

let make_get_cluster_credentials_message ?custom_domain_name:(custom_domain_name_ : string_ option)
    ?db_groups:(db_groups_ : db_group_list option)
    ?auto_create:(auto_create_ : boolean_optional option)
    ?duration_seconds:(duration_seconds_ : integer_optional option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) ?db_name:(db_name_ : string_ option)
    ~db_user:(db_user_ : string_) () =
  ({
     custom_domain_name = custom_domain_name_;
...
let make_logging_status ?log_exports:(log_exports_ : log_type_list option)
    ...
    ?logging_enabled:(logging_enabled_ : boolean_ option) () =
  ({
...
let make_enable_logging_message ?log_exports:(log_exports_ : log_type_list option)
    ?log_destination_type:(log_destination_type_ : log_destination_type option)
    ?s3_key_prefix:(s3_key_prefix_ : s3_key_prefix_value option)
    ?bucket_name:(bucket_name_ : string_ option) ~cluster_identifier:(cluster_identifier_ : string_)
    () =
  ({
     log_exports = log_exports_;
     log_destination_type = log_destination_type_;
...
let make_describe_usage_limits_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?feature_type:(feature_type_ : usage_limit_feature_type option)
    ?cluster_identifier:(cluster_identifier_ : string_ option)
    ?usage_limit_id:(usage_limit_id_ : string_ option) () =
...
let make_describe_tags_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
...
let make_describe_table_restore_status_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?table_restore_request_id:(table_restore_request_id_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
...
let make_describe_snapshot_schedules_output_message ?marker:(marker_ : string_ option)
    ?snapshot_schedules:(snapshot_schedules_ : snapshot_schedule_list option) () =
  ({ marker = marker_; snapshot_schedules = snapshot_schedules_ }
    : describe_snapshot_schedules_output_message)

let make_describe_snapshot_schedules_message ?max_records:(max_records_ : integer_optional option)
    ?marker:(marker_ : string_ option) ?tag_values:(tag_values_ : tag_value_list option)
...
let make_describe_snapshot_copy_grants_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?snapshot_copy_grant_name:(snapshot_copy_grant_name_ : string_ option) () =
  ({
...
let make_describe_scheduled_actions_message ?max_records:(max_records_ : integer_optional option)
    ?marker:(marker_ : string_ option) ?filters:(filters_ : scheduled_action_filter_list option)
    ?active:(active_ : boolean_optional option) ?end_time:(end_time_ : t_stamp option)
    ?start_time:(start_time_ : t_stamp option)
...
let make_describe_reserved_nodes_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?reserved_node_id:(reserved_node_id_ : string_ option) () =
  ({ marker = marker_; max_records = max_records_; reserved_node_id = reserved_node_id_ }
    : describe_reserved_nodes_message)

let make_describe_reserved_node_offerings_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?reserved_node_offering_id:(reserved_node_offering_id_ : string_ option) () =
  ({
     marker = marker_;
...
let make_describe_reserved_node_exchange_status_output_message ?marker:(marker_ : string_ option)
...
let make_describe_reserved_node_exchange_status_input_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?reserved_node_exchange_request_id:(reserved_node_exchange_request_id_ : string_ option)
    ?reserved_node_id:(reserved_node_id_ : string_ option) () =
  ({
...
let make_describe_redshift_idc_applications_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?redshift_idc_application_arn:(redshift_idc_application_arn_ : string_ option) () =
  ({
     marker = marker_;
...
let make_partner_integration_info ?updated_at:(updated_at_ : t_stamp option)
...
let make_orderable_cluster_option
...
let make_orderable_cluster_options_message ?marker:(marker_ : string_ option)
    ?orderable_cluster_options:(orderable_cluster_options_ : orderable_cluster_options_list option)
    () =
  ({ marker = marker_; orderable_cluster_options = orderable_cluster_options_ }
    : orderable_cluster_options_message)

let make_describe_orderable_cluster_options_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option) ?node_type:(node_type_ : string_ option)
    ?cluster_version:(cluster_version_ : string_ option) () =
  ({
     marker = marker_;
     max_records = max_records_;
...
let make_node_configuration_option ?mode:(mode_ : mode option)
    ?estimated_disk_utilization_percent:
      (estimated_disk_utilization_percent_ : double_optional option)
    ?number_of_nodes:(number_of_nodes_ : integer option) ?node_type:(node_type_ : string_ option) ()
    =
  ({
     mode = mode_;
...
let make_node_configuration_options_message ?marker:(marker_ : string_ option)
    ?node_configuration_option_list:
      (node_configuration_option_list_ : node_configuration_option_list option) () =
  ({ marker = marker_; node_configuration_option_list = node_configuration_option_list_ }
    : node_configuration_options_message)

let make_node_configuration_options_filter ?values:(values_ : value_string_list option)
    ?operator:(operator_ : operator_type option)
    ?name:(name_ : node_configuration_options_filter_name option) () =
  ({ values = values_; operator = operator_; name = name_ } : node_configuration_options_filter)
...
let make_describe_node_configuration_options_message
    ...
    ?owner_account:(owner_account_ : string_ option) ?snapshot_arn:(snapshot_arn_ : string_ option)
    ?snapshot_identifier:(snapshot_identifier_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option)
...
let make_integrations_message ?integrations:(integrations_ : integration_list option)
    ?marker:(marker_ : string_ option) () =
  ({ integrations = integrations_; marker = marker_ } : integrations_message)

let make_describe_integrations_filter ~values:(values_ : describe_integrations_filter_value_list)
    ~name:(name_ : describe_integrations_filter_name) () =
  ({ values = values_; name = name_ } : describe_integrations_filter)
...
let make_inbound_integration ?create_time:(create_time_ : t_stamp option)
    ?errors:(errors_ : integration_error_list option)
    ?status:(status_ : zero_etl_integration_status option)
    ?target_arn:(target_arn_ : target_arn option) ?source_arn:(source_arn_ : string_ option)
...
let make_inbound_integrations_message
...
let make_describe_inbound_integrations_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
...
let make_hsm_configuration ?tags:(tags_ : tag_list option)
    ?hsm_partition_name:(hsm_partition_name_ : string_ option)
    ?hsm_ip_address:(hsm_ip_address_ : string_ option) ?description:(description_ : string_ option)
    ?hsm_configuration_identifier:(hsm_configuration_identifier_ : string_ option) () =
  ({
     tags = tags_;
...
let make_hsm_configuration_message
    ?hsm_configurations:(hsm_configurations_ : hsm_configuration_list option)
    ?marker:(marker_ : string_ option) () =
  ({ hsm_configurations = hsm_configurations_; marker = marker_ } : hsm_configuration_message)

let make_describe_hsm_configurations_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?hsm_configuration_identifier:(hsm_configuration_identifier_ : string_ option) () =
  ({
...
let make_hsm_client_certificate ?tags:(tags_ : tag_list option)
...
let make_hsm_client_certificate_message
    ?hsm_client_certificates:(hsm_client_certificates_ : hsm_client_certificate_list option)
    ?marker:(marker_ : string_ option) () =
  ({ hsm_client_certificates = hsm_client_certificates_; marker = marker_ }
    : hsm_client_certificate_message)

let make_describe_hsm_client_certificates_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?hsm_client_certificate_identifier:(hsm_client_certificate_identifier_ : string_ option) () =
  ({
...
let make_describe_event_subscriptions_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
...
let make_event ?event_id:(event_id_ : string_ option) ?date:(date_ : t_stamp option)
    ?severity:(severity_ : string_ option)
    ?event_categories:(event_categories_ : event_categories_list option)
...
let make_event_info_map ?severity:(severity_ : string_ option)
    ?event_description:(event_description_ : string_ option)
    ?event_categories:(event_categories_ : event_categories_list option)
...
let make_event_categories_map ?events:(events_ : event_info_map_list option)
    ?source_type:(source_type_ : string_ option) () =
  ({ events = events_; source_type = source_type_ } : event_categories_map)
...
let make_endpoint_authorization_list ?marker:(marker_ : string_ option)
    ?endpoint_authorization_list:(endpoint_authorization_list_ : endpoint_authorizations option) ()
    =
  ({ marker = marker_; endpoint_authorization_list = endpoint_authorization_list_ }
    : endpoint_authorization_list)

let make_describe_endpoint_authorization_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?grantee:(grantee_ : boolean_optional option) ?account:(account_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
     marker = marker_;
     max_records = max_records_;
...
let make_endpoint_access_list ?marker:(marker_ : string_ option)
    ?endpoint_access_list:(endpoint_access_list_ : endpoint_accesses option) () =
  ({ marker = marker_; endpoint_access_list = endpoint_access_list_ } : endpoint_access_list)

let make_describe_endpoint_access_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option) ?vpc_id:(vpc_id_ : string_ option)
    ?endpoint_name:(endpoint_name_ : string_ option)
    ?resource_owner:(resource_owner_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
...
let make_default_cluster_parameters ?parameters:(parameters_ : parameters_list option)
    ?marker:(marker_ : string_ option)
    ?parameter_group_family:(parameter_group_family_ : string_ option) () =
  ({ parameters = parameters_; marker = marker_; parameter_group_family = parameter_group_family_ }
    : default_cluster_parameters)

let make_describe_default_cluster_parameters_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
...
let make_describe_data_shares_for_producer_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?status:(status_ : data_share_status_for_producer option)
    ?producer_arn:(producer_arn_ : string_ option) () =
  ({ marker = marker_; max_records = max_records_; status = status_; producer_arn = producer_arn_ }
    : describe_data_shares_for_producer_message)

let make_describe_data_shares_for_consumer_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?status:(status_ : data_share_status_for_consumer option)
    ?consumer_arn:(consumer_arn_ : string_ option) () =
  ({ marker = marker_; max_records = max_records_; status = status_; consumer_arn = consumer_arn_ }
    : describe_data_shares_for_consumer_message)

let make_describe_data_shares_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?data_share_arn:(data_share_arn_ : string_ option) () =
  ({ marker = marker_; max_records = max_records_; data_share_arn = data_share_arn_ }
    : describe_data_shares_message)

let make_certificate_association ?cluster_identifier:(cluster_identifier_ : string_ option)
    ?custom_domain_name:(custom_domain_name_ : string_ option) () =
...
let make_custom_domain_associations_message ?associations:(associations_ : association_list option)
    ?marker:(marker_ : string_ option) () =
  ({ associations = associations_; marker = marker_ } : custom_domain_associations_message)

let make_describe_custom_domain_associations_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?custom_domain_certificate_arn:
...
let make_cluster_version ?description:(description_ : string_ option)
...
let make_cluster_versions_message
    ?cluster_versions:(cluster_versions_ : cluster_version_list option)
    ?marker:(marker_ : string_ option) () =
  ({ cluster_versions = cluster_versions_; marker = marker_ } : cluster_versions_message)

let make_describe_cluster_versions_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?cluster_parameter_group_family:(cluster_parameter_group_family_ : string_ option)
    ?cluster_version:(cluster_version_ : string_ option) () =
  ({
...
let make_describe_cluster_tracks_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?maintenance_track_name:(maintenance_track_name_ : string_ option) () =
  ({
     marker = marker_;
...
let make_cluster_subnet_group_message
    ?cluster_subnet_groups:(cluster_subnet_groups_ : cluster_subnet_groups option)
    ?marker:(marker_ : string_ option) () =
  ({ cluster_subnet_groups = cluster_subnet_groups_; marker = marker_ }
    : cluster_subnet_group_message)

let make_describe_cluster_subnet_groups_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?cluster_subnet_group_name:(cluster_subnet_group_name_ : string_ option) () =
  ({
...
let make_describe_cluster_snapshots_message
    ...
    ?tag_values:(tag_values_ : tag_value_list option) ?tag_keys:(tag_keys_ : tag_key_list option)
    ?owner_account:(owner_account_ : string_ option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option) ?end_time:(end_time_ : t_stamp option)
    ?start_time:(start_time_ : t_stamp option) ?snapshot_type:(snapshot_type_ : string_ option)
    ?snapshot_arn:(snapshot_arn_ : string_ option)
    ?snapshot_identifier:(snapshot_identifier_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
...
let make_cluster_security_group_message
    ?cluster_security_groups:(cluster_security_groups_ : cluster_security_groups option)
    ?marker:(marker_ : string_ option) () =
  ({ cluster_security_groups = cluster_security_groups_; marker = marker_ }
    : cluster_security_group_message)

let make_describe_cluster_security_groups_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?cluster_security_group_name:(cluster_security_group_name_ : string_ option) () =
  ({
...
let make_clusters_message ?clusters:(clusters_ : cluster_list option)
    ?marker:(marker_ : string_ option) () =
  ({ clusters = clusters_; marker = marker_ } : clusters_message)

let make_describe_clusters_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({
...
let make_cluster_parameter_group_details ?marker:(marker_ : string_ option)
    ?parameters:(parameters_ : parameters_list option) () =
  ({ marker = marker_; parameters = parameters_ } : cluster_parameter_group_details)

let make_describe_cluster_parameters_message ?marker:(marker_ : string_ option)
...
let make_cluster_parameter_group ?tags:(tags_ : tag_list option)
    ?description:(description_ : string_ option)
    ?parameter_group_family:(parameter_group_family_ : string_ option)
    ?parameter_group_name:(parameter_group_name_ : string_ option) () =
  ({
...
let make_cluster_parameter_groups_message
    ?parameter_groups:(parameter_groups_ : parameter_group_list option)
    ?marker:(marker_ : string_ option) () =
  ({ parameter_groups = parameter_groups_; marker = marker_ } : cluster_parameter_groups_message)

let make_describe_cluster_parameter_groups_message ?tag_values:(tag_values_ : tag_value_list option)
    ?tag_keys:(tag_keys_ : tag_key_list option) ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?parameter_group_name:(parameter_group_name_ : string_ option) () =
  ({
...
let make_cluster_db_revision ?revision_targets:(revision_targets_ : revision_targets_list option)
    ?database_revision_release_date:(database_revision_release_date_ : t_stamp option)
...
let make_cluster_db_revisions_message
    ?cluster_db_revisions:(cluster_db_revisions_ : cluster_db_revisions_list option)
    ?marker:(marker_ : string_ option) () =
  ({ cluster_db_revisions = cluster_db_revisions_; marker = marker_ }
    : cluster_db_revisions_message)

let make_describe_cluster_db_revisions_message ?marker:(marker_ : string_ option)
    ?max_records:(max_records_ : integer_optional option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) () =
  ({ marker = marker_; max_records = max_records_; cluster_identifier = cluster_identifier_ }
...
let make_account_attribute ?attribute_values:(attribute_values_ : attribute_value_list option)
    ?attribute_name:(attribute_name_ : string_ option) () =
  ({ attribute_values = attribute_values_; attribute_name = attribute_name_ } : account_attribute)
...
let make_delete_tags_message ~tag_keys:(tag_keys_ : tag_key_list)
    ~resource_name:(resource_name_ : string_) () =
  ({ tag_keys = tag_keys_; resource_name = resource_name_ } : delete_tags_message)
...
let make_deauthorize_data_share_message ~consumer_identifier:(consumer_identifier_ : string_)
    ~data_share_arn:(data_share_arn_ : string_) () =
  ({ consumer_identifier = consumer_identifier_; data_share_arn = data_share_arn_ }
    : deauthorize_data_share_message)

let make_create_usage_limit_message ?tags:(tags_ : tag_list option)
    ?breach_action:(breach_action_ : usage_limit_breach_action option)
    ?period:(period_ : usage_limit_period option) ~amount:(amount_ : long)
    ~limit_type:(limit_type_ : usage_limit_limit_type)
...
let make_create_tags_message ~tags:(tags_ : tag_list) ~resource_name:(resource_name_ : string_) () =
  ({ tags = tags_; resource_name = resource_name_ } : create_tags_message)

let make_create_snapshot_schedule_message
    ?next_invocations:(next_invocations_ : integer_optional option)
    ?dry_run:(dry_run_ : boolean_optional option) ?tags:(tags_ : tag_list option)
    ?schedule_description:(schedule_description_ : string_ option)
    ?schedule_identifier:(schedule_identifier_ : string_ option)
    ?schedule_definitions:(schedule_definitions_ : schedule_definition_list option) () =
  ({
     next_invocations = next_invocations_;
...
let make_create_snapshot_copy_grant_message ?tags:(tags_ : tag_list option)
    ?kms_key_id:(kms_key_id_ : string_ option)
    ~snapshot_copy_grant_name:(snapshot_copy_grant_name_ : string_) () =
  ({ tags = tags_; kms_key_id = kms_key_id_; snapshot_copy_grant_name = snapshot_copy_grant_name_ }
    : create_snapshot_copy_grant_message)

let make_create_scheduled_action_message ?enable:(enable_ : boolean_optional option)
    ?end_time:(end_time_ : t_stamp option) ?start_time:(start_time_ : t_stamp option)
    ?scheduled_action_description:(scheduled_action_description_ : string_ option)
    ~iam_role:(iam_role_ : string_) ~schedule:(schedule_ : string_)
    ~target_action:(target_action_ : scheduled_action_type)
    ~scheduled_action_name:(scheduled_action_name_ : string_) () =
  ({
     enable = enable_;
     end_time = end_time_;
...
let make_create_redshift_idc_application_message ?sso_tag_keys:(sso_tag_keys_ : tag_key_list option)
    ?tags:(tags_ : tag_list option) ?application_type:(application_type_ : application_type option)
    ?service_integrations:(service_integrations_ : service_integration_list option)
    ?authorized_token_issuer_list:
      (authorized_token_issuer_list_ : authorized_token_issuer_list option)
...
let make_create_integration_message ?description:(description_ : integration_description option)
    ?additional_encryption_context:(additional_encryption_context_ : encryption_context_map option)
    ?tag_list:(tag_list_ : tag_list option) ?kms_key_id:(kms_key_id_ : string_ option)
    ~integration_name:(integration_name_ : integration_name) ~target_arn:(target_arn_ : target_arn)
...
let make_create_hsm_configuration_message ?tags:(tags_ : tag_list option)
    ...
    ~hsm_partition_name:(hsm_partition_name_ : string_) ~hsm_ip_address:(hsm_ip_address_ : string_)
...
let make_create_hsm_client_certificate_message ?tags:(tags_ : tag_list option)
    ~hsm_client_certificate_identifier:(hsm_client_certificate_identifier_ : string_) () =
  ({ tags = tags_; hsm_client_certificate_identifier = hsm_client_certificate_identifier_ }
    : create_hsm_client_certificate_message)

let make_create_event_subscription_message ?tags:(tags_ : tag_list option)
    ?enabled:(enabled_ : boolean_optional option) ?severity:(severity_ : string_ option)
    ?event_categories:(event_categories_ : event_categories_list option)
    ?source_ids:(source_ids_ : source_ids_list option) ?source_type:(source_type_ : string_ option)
    ~sns_topic_arn:(sns_topic_arn_ : string_) ~subscription_name:(subscription_name_ : string_) () =
  ({
     tags = tags_;
     enabled = enabled_;
     severity = severity_;
...
let make_create_endpoint_access_message
    ?vpc_security_group_ids:(vpc_security_group_ids_ : vpc_security_group_id_list option)
    ?resource_owner:(resource_owner_ : string_ option)
    ?cluster_identifier:(cluster_identifier_ : string_ option)
    ~subnet_group_name:(subnet_group_name_ : string_) ~endpoint_name:(endpoint_name_ : string_) () =
  ({
     vpc_security_group_ids = vpc_security_group_ids_;
     subnet_group_name = subnet_group_name_;
     endpoint_name = endpoint_name_;
...
let make_create_cluster_subnet_group_message ?tags:(tags_ : tag_list option)
    ~subnet_ids:(subnet_ids_ : subnet_identifier_list) ~description:(description_ : string_)
    ~cluster_subnet_group_name:(cluster_subnet_group_name_ : string_) () =
  ({
     tags = tags_;
     subnet_ids = subnet_ids_;
...
let make_create_cluster_snapshot_message ?tags:(tags_ : tag_list option)
    ?manual_snapshot_retention_period:(manual_snapshot_retention_period_ : integer_optional option)
    ~cluster_identifier:(cluster_identifier_ : string_)
    ~snapshot_identifier:(snapshot_identifier_ : string_) () =
  ({
...
let make_create_cluster_message
    ?extra_compute_for_automatic_optimization:
      (extra_compute_for_automatic_optimization_ : boolean_optional option)
    ?catalog_name:(catalog_name_ : catalog_name_string option)
    ?redshift_idc_application_arn:(redshift_idc_application_arn_ : string_ option)
    ...
    ?number_of_nodes:(number_of_nodes_ : integer_optional option)
    ?allow_version_upgrade:(allow_version_upgrade_ : boolean_optional option)
    ?cluster_version:(cluster_version_ : string_ option) ?port:(port_ : integer_optional option)
    ?manual_snapshot_retention_period:(manual_snapshot_retention_period_ : integer_optional option)
    ?automated_snapshot_retention_period:
      (automated_snapshot_retention_period_ : integer_optional option)
    ?cluster_parameter_group_name:(cluster_parameter_group_name_ : string_ option)
    ...
    ?cluster_security_groups:(cluster_security_groups_ : cluster_security_group_name_list option)
    ?master_user_password:(master_user_password_ : sensitive_string option)
    ?cluster_type:(cluster_type_ : string_ option) ?db_name:(db_name_ : string_ option)
    ~master_username:(master_username_ : string_) ~node_type:(node_type_ : string_)
    ~cluster_identifier:(cluster_identifier_ : string_) () =
  ({
     extra_compute_for_automatic_optimization = extra_compute_for_automatic_optimization_;
     catalog_name = catalog_name_;
...
let make_batch_modify_cluster_snapshots_output_message
  ...
  ({ errors = errors_; resources = resources_ } : batch_modify_cluster_snapshots_output_message)

let make_batch_modify_cluster_snapshots_message ?force:(force_ : boolean_ option)
    ?manual_snapshot_retention_period:(manual_snapshot_retention_period_ : integer_optional option)
...
let make_authorize_endpoint_access_message ?vpc_ids:(vpc_ids_ : vpc_identifier_list option)
    ?cluster_identifier:(cluster_identifier_ : string_ option) ~account:(account_ : string_) () =
  ({ vpc_ids = vpc_ids_; account = account_; cluster_identifier = cluster_identifier_ }
...
let make_authorize_cluster_security_group_ingress_message
    ?ec2_security_group_owner_id:(ec2_security_group_owner_id_ : string_ option)
    ?ec2_security_group_name:(ec2_security_group_name_ : string_ option)
    ?cidri_p:(cidri_p_ : string_ option)
    ~cluster_security_group_name:(cluster_security_group_name_ : string_) () =
...
let make_accept_reserved_node_exchange_input_message
