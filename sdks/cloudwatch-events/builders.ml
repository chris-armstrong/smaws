...
let make_update_archive_request ?retention_days:(retention_days_ : retention_days option)
...
let make_redshift_data_parameters ?with_event:(with_event_ : boolean_ option)
    ?statement_name:(statement_name_ : statement_name option) ?db_user:(db_user_ : db_user option)
    ?secret_manager_arn:(secret_manager_arn_ : redshift_secret_manager_arn option) ~sql:(sql_ : sql)
...
let make_target ?retry_policy:(retry_policy_ : retry_policy option)
...
let make_start_replay_request ?description:(description_ : replay_description option)
    ...
    ~event_start_time:(event_start_time_ : timestamp) ~event_source_arn:(event_source_arn_ : arn)
...
let make_list_replays_request ?limit:(limit_ : limit_max100 option)
    ?next_token:(next_token_ : next_token option) ?event_source_arn:(event_source_arn_ : arn option)
    ?state:(state_ : replay_state option) ?name_prefix:(name_prefix_ : replay_name option) () =
  ({
     limit = limit_;
     next_token = next_token_;
     event_source_arn = event_source_arn_;
...
let make_event_bus ?policy:(policy_ : string_ option) ?arn:(arn_ : string_ option)
    ?name:(name_ : string_ option) () =
  ({ policy = policy_; arn = arn_; name = name_ } : event_bus)
...
let make_archive ?creation_time:(creation_time_ : timestamp option)
    ...
    ?state_reason:(state_reason_ : archive_state_reason option)
    ?state:(state_ : archive_state option) ?event_source_arn:(event_source_arn_ : arn option)
...
let make_describe_event_bus_response ?policy:(policy_ : string_ option) ?arn:(arn_ : string_ option)
    ?name:(name_ : string_ option) () =
  ({ policy = policy_; arn = arn_; name = name_ } : describe_event_bus_response)
...
let make_create_event_bus_request ?tags:(tags_ : tag_list option)
  ...
  ({ tags = tags_; event_source_name = event_source_name_; name = name_ }
...
let make_create_archive_request ?retention_days:(retention_days_ : retention_days option)
    ?event_pattern:(event_pattern_ : event_pattern option)
    ?description:(description_ : archive_description option)
    ~event_source_arn:(event_source_arn_ : arn) ~archive_name:(archive_name_ : archive_name) () =
  ({
     retention_days = retention_days_;
     event_pattern = event_pattern_;
     description = description_;
