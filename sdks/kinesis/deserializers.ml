  ...
  let _res : subscribe_to_shard_event =
    {
      child_shards =
        (option_of_yojson
           (value_for_key child_shard_list_of_yojson "ChildShards") _list
           path);
...
let subscribe_to_shard_output_of_yojson tree path =
  ...
  let _res : subscribe_to_shard_output =
    {
      event_stream =
        (value_for_key subscribe_to_shard_event_stream_of_yojson
           "EventStream" _list path)
    } in
  _res
  ...
  let _res : stream_description_summary =
    {
      ...
      encryption_type =
        (option_of_yojson
           ...
           path);
      enhanced_monitoring =
        (value_for_key enhanced_monitoring_list_of_yojson
           "EnhancedMonitoring" _list path);
      stream_creation_timestamp =
        (value_for_key timestamp__of_yojson "StreamCreationTimestamp" _list
           path);
      retention_period_hours =
        (value_for_key retention_period_hours_of_yojson
           "RetentionPeriodHours" _list path);
      stream_mode_details =
        (option_of_yojson
           (value_for_key stream_mode_details_of_yojson "StreamModeDetails")
           _list path);
...
let register_stream_consumer_output_of_yojson tree path =
  ...
  let _res : register_stream_consumer_output =
    { consumer = (value_for_key consumer_of_yojson "Consumer" _list path) } in
  _res
  ...
  let _res : put_records_request_entry =
    {
      ...
      explicit_hash_key =
        (option_of_yojson
           (value_for_key hash_key_of_yojson "ExplicitHashKey") _list path);
      data = (value_for_key data_of_yojson "Data" _list path)
    } in
  _res
  ...
  let _res : put_records_output =
    {
      encryption_type =
        (option_of_yojson
           ...
           path);
      records =
        (value_for_key put_records_result_entry_list_of_yojson "Records"
           _list path);
      failed_record_count =
        (option_of_yojson
           (value_for_key positive_integer_object_of_yojson
  ...
  let _res : list_tags_for_stream_input =
    {
      ...
      exclusive_start_tag_key =
        (option_of_yojson
           (value_for_key tag_key_of_yojson "ExclusiveStartTagKey") _list
           path);
      stream_name =
        (option_of_yojson (value_for_key stream_name_of_yojson "StreamName")
           _list path)
  ...
  let _res : list_shards_input =
    {
      ...
      shard_filter =
        (option_of_yojson
           (value_for_key shard_filter_of_yojson "ShardFilter") _list path);
      stream_creation_timestamp =
        (option_of_yojson
           (value_for_key timestamp__of_yojson "StreamCreationTimestamp")
           _list path);
...
let get_resource_policy_output_of_yojson tree path =
  ...
  let _res : get_resource_policy_output =
    { policy = (value_for_key policy_of_yojson "Policy" _list path) } in
  _res
  ...
  let _res : describe_stream_input =
    {
      ...
      limit =
        (option_of_yojson
           (value_for_key describe_stream_input_limit_of_yojson "Limit")
           _list path);
      stream_name =
        (option_of_yojson (value_for_key stream_name_of_yojson "StreamName")
           _list path)
