...
let make_update_shard_count_output
  ...
  ?current_shard_count:(current_shard_count_ :
                         positive_integer_object option)
...
let make_tag_resource_input ~resource_ar_n:(resource_ar_n_ : resource_ar_n)
  ~tags:(tags_ : tag_map) () =
...
let make_subscribe_to_shard_event
  ?child_shards:(child_shards_ : child_shard_list option)
...
let make_enhanced_metrics
  ?shard_level_metrics:(shard_level_metrics_ : metrics_name_list option) () =
...
let make_shard_filter ?timestamp:(timestamp_ : timestamp option)
  ...
  () =
  ({ timestamp = timestamp_; shard_id = shard_id_; type_ = type__ } : 
  shard_filter)
...
let make_list_tags_for_stream_output
  ~has_more_tags:(has_more_tags_ : boolean_object) ~tags:(tags_ : tag_list)
  () =
...
let make_describe_limits_output
  ~on_demand_stream_count_limit:(on_demand_stream_count_limit_ :
                                  on_demand_stream_count_limit_object)
  ~on_demand_stream_count:(on_demand_stream_count_ :
                            on_demand_stream_count_object)
  ~open_shard_count:(open_shard_count_ : shard_count_object)
