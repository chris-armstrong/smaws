...
let update_shard_count_output_of_yojson tree path =
  ...
  ({
     ...
     current_shard_count =
       option_of_yojson
         (value_for_key positive_integer_object_of_yojson "CurrentShardCount")
         _list path;
...
let update_shard_count_input_of_yojson tree path =
  ...
  ({
     ...
     target_shard_count =
       value_for_key positive_integer_object_of_yojson "TargetShardCount" _list path;
     stream_name = option_of_yojson (value_for_key stream_name_of_yojson "StreamName") _list path;
   }
...
let record_of_yojson tree path =
  ...
  ({
     ...
     approximate_arrival_timestamp =
       option_of_yojson (value_for_key timestamp_of_yojson "ApproximateArrivalTimestamp") _list path;
...
let child_shard_list_of_yojson tree path = list_of_yojson child_shard_of_yojson tree path

let subscribe_to_shard_event_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     child_shards =
       option_of_yojson (value_for_key child_shard_list_of_yojson "ChildShards") _list path;
     millis_behind_latest =
       value_for_key millis_behind_latest_of_yojson "MillisBehindLatest" _list path;
     continuation_sequence_number =
       value_for_key sequence_number_of_yojson "ContinuationSequenceNumber" _list path;
...
let subscribe_to_shard_event_stream_of_yojson (tree : t) path =
  ...
  (match key with
   | "InternalFailureException" ->
       InternalFailureException (internal_failure_exception_of_yojson value_ path)
   ...
   | "KMSAccessDeniedException" ->
       KMSAccessDeniedException (kms_access_denied_exception_of_yojson value_ path)
   | "KMSInvalidStateException" ->
       KMSInvalidStateException (kms_invalid_state_exception_of_yojson value_ path)
   | "KMSDisabledException" -> KMSDisabledException (kms_disabled_exception_of_yojson value_ path)
   | "ResourceInUseException" ->
       ResourceInUseException (resource_in_use_exception_of_yojson value_ path)
   | "ResourceNotFoundException" ->
       ResourceNotFoundException (resource_not_found_exception_of_yojson value_ path)
   | "SubscribeToShardEvent" ->
       SubscribeToShardEvent (subscribe_to_shard_event_of_yojson value_ path)
   | _ as unknown ->
       raise (deserialize_unknown_enum_value_error path "SubscribeToShardEventStream" unknown)
    : subscribe_to_shard_event_stream)
...
let stream_summary_list_of_yojson tree path = list_of_yojson stream_summary_of_yojson tree path
let stream_name_list_of_yojson tree path = list_of_yojson stream_name_of_yojson tree path
...
let metrics_name_list_of_yojson tree path = list_of_yojson metrics_name_of_yojson tree path

let enhanced_metrics_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     shard_level_metrics =
       option_of_yojson (value_for_key metrics_name_list_of_yojson "ShardLevelMetrics") _list path;
   }
    : enhanced_metrics)
...
let stream_description_summary_of_yojson tree path =
  ...
  ({
     ...
     encryption_type =
       option_of_yojson (value_for_key encryption_type_of_yojson "EncryptionType") _list path;
     enhanced_monitoring =
       value_for_key enhanced_monitoring_list_of_yojson "EnhancedMonitoring" _list path;
     stream_creation_timestamp =
       value_for_key timestamp_of_yojson "StreamCreationTimestamp" _list path;
...
let sequence_number_range_of_yojson tree path =
  ...
  ({
     ...
     starting_sequence_number =
       value_for_key sequence_number_of_yojson "StartingSequenceNumber" _list path;
   }
...
let shard_of_yojson tree path =
  ...
  ({
     sequence_number_range =
       value_for_key sequence_number_range_of_yojson "SequenceNumberRange" _list path;
     hash_key_range = value_for_key hash_key_range_of_yojson "HashKeyRange" _list path;
     adjacent_parent_shard_id =
       option_of_yojson (value_for_key shard_id_of_yojson "AdjacentParentShardId") _list path;
     parent_shard_id =
...
let consumer_of_yojson tree path =
  ...
  ({
     consumer_creation_timestamp =
       value_for_key timestamp_of_yojson "ConsumerCreationTimestamp" _list path;
...
let put_records_request_entry_of_yojson tree path =
  ...
  ({
     partition_key = value_for_key partition_key_of_yojson "PartitionKey" _list path;
     explicit_hash_key =
       option_of_yojson (value_for_key hash_key_of_yojson "ExplicitHashKey") _list path;
     data = value_for_key data_of_yojson "Data" _list path;
   }
...
let put_records_output_of_yojson tree path =
  ...
  ({
     ...
     failed_record_count =
       option_of_yojson
         (value_for_key positive_integer_object_of_yojson "FailedRecordCount")
         _list path;
   }
    : put_records_output)
...
let put_record_input_of_yojson tree path =
  ...
  ({
     ...
     sequence_number_for_ordering =
       option_of_yojson
         (value_for_key sequence_number_of_yojson "SequenceNumberForOrdering")
         _list path;
     explicit_hash_key =
       option_of_yojson (value_for_key hash_key_of_yojson "ExplicitHashKey") _list path;
     partition_key = value_for_key partition_key_of_yojson "PartitionKey" _list path;
...
let list_streams_output_of_yojson tree path =
  ...
  ({
     stream_summaries =
       option_of_yojson (value_for_key stream_summary_list_of_yojson "StreamSummaries") _list path;
...
let list_streams_input_of_yojson tree path =
  ...
  ({
     ...
     exclusive_start_stream_name =
       option_of_yojson (value_for_key stream_name_of_yojson "ExclusiveStartStreamName") _list path;
...
let consumer_list_of_yojson tree path = list_of_yojson consumer_of_yojson tree path
...
let list_stream_consumers_input_of_yojson tree path =
  ...
  ({
     ...
     max_results =
       option_of_yojson
         (value_for_key list_stream_consumers_input_limit_of_yojson "MaxResults")
         _list path;
     next_token = option_of_yojson (value_for_key next_token_of_yojson "NextToken") _list path;
     stream_ar_n = value_for_key stream_ar_n_of_yojson "StreamARN" _list path;
   }
...
let list_shards_input_of_yojson tree path =
  ...
  ({
     ...
     stream_creation_timestamp =
       option_of_yojson (value_for_key timestamp_of_yojson "StreamCreationTimestamp") _list path;
     max_results =
       option_of_yojson (value_for_key list_shards_input_limit_of_yojson "MaxResults") _list path;
...
let get_resource_policy_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ policy = value_for_key policy_of_yojson "Policy" _list path } : get_resource_policy_output)
...
let enhanced_monitoring_output_of_yojson tree path =
  ...
  ({
     ...
     desired_shard_level_metrics =
       option_of_yojson
         (value_for_key metrics_name_list_of_yojson "DesiredShardLevelMetrics")
         _list path;
     current_shard_level_metrics =
       option_of_yojson
         (value_for_key metrics_name_list_of_yojson "CurrentShardLevelMetrics")
         _list path;
     stream_name = option_of_yojson (value_for_key stream_name_of_yojson "StreamName") _list path;
   }
...
let describe_stream_summary_output_of_yojson tree path =
  ...
  ({
     stream_description_summary =
       value_for_key stream_description_summary_of_yojson "StreamDescriptionSummary" _list path;
   }
...
let describe_stream_consumer_output_of_yojson tree path =
  ...
  ({
     consumer_description =
       value_for_key consumer_description_of_yojson "ConsumerDescription" _list path;
   }
...
let describe_stream_consumer_input_of_yojson tree path =
  ...
  ({
     ...
     consumer_name =
       option_of_yojson (value_for_key consumer_name_of_yojson "ConsumerName") _list path;
     stream_ar_n = option_of_yojson (value_for_key stream_ar_n_of_yojson "StreamARN") _list path;
   }
...
let describe_stream_input_of_yojson tree path =
  ...
  ({
     ...
     exclusive_start_shard_id =
       option_of_yojson (value_for_key shard_id_of_yojson "ExclusiveStartShardId") _list path;
     limit =
       option_of_yojson (value_for_key describe_stream_input_limit_of_yojson "Limit") _list path;
     stream_name = option_of_yojson (value_for_key stream_name_of_yojson "StreamName") _list path;
   }
...
let describe_limits_output_of_yojson tree path =
  ...
  ({
     on_demand_stream_count_limit =
       value_for_key on_demand_stream_count_limit_object_of_yojson "OnDemandStreamCountLimit" _list
         path;
     on_demand_stream_count =
       value_for_key on_demand_stream_count_object_of_yojson "OnDemandStreamCount" _list path;
     open_shard_count = value_for_key shard_count_object_of_yojson "OpenShardCount" _list path;
...
let create_stream_input_of_yojson tree path =
  ...
  ({
     ...
     stream_mode_details =
       option_of_yojson (value_for_key stream_mode_details_of_yojson "StreamModeDetails") _list path;
     shard_count =
       option_of_yojson (value_for_key positive_integer_object_of_yojson "ShardCount") _list path;
     stream_name = value_for_key stream_name_of_yojson "StreamName" _list path;
   }
    : create_stream_input)
