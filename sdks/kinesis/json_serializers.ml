...
let stream_mode_details_to_yojson (x : stream_mode_details) =
  assoc_to_yojson
    [("StreamMode", (Some (stream_mode_to_yojson x.stream_mode)))]
...
let update_shard_count_output_to_yojson (x : update_shard_count_output) =
  assoc_to_yojson
    ...
    ("TargetShardCount",
      (option_to_yojson positive_integer_object_to_yojson
         x.target_shard_count));
    ("CurrentShardCount",
      (option_to_yojson positive_integer_object_to_yojson
         x.current_shard_count));
...
let encryption_type_to_yojson (x : encryption_type) =
  match x with | KMS -> `String "KMS" | NONE -> `String "NONE"
...
let child_shard_list_to_yojson tree =
  list_to_yojson child_shard_to_yojson tree
...
let subscribe_to_shard_event_stream_to_yojson
  (x : subscribe_to_shard_event_stream) =
  match x with
  | InternalFailureException arg ->
      assoc_to_yojson
        [("InternalFailureException",
           (Some (internal_failure_exception_to_yojson arg)))]
  | KMSThrottlingException arg ->
      assoc_to_yojson
        [("KMSThrottlingException",
           (Some (kms_throttling_exception_to_yojson arg)))]
  | KMSOptInRequired arg ->
      assoc_to_yojson
        [("KMSOptInRequired", (Some (kms_opt_in_required_to_yojson arg)))]
  | KMSNotFoundException arg ->
      assoc_to_yojson
        [("KMSNotFoundException",
           (Some (kms_not_found_exception_to_yojson arg)))]
  | KMSAccessDeniedException arg ->
      assoc_to_yojson
        [("KMSAccessDeniedException",
           (Some (kms_access_denied_exception_to_yojson arg)))]
  | KMSInvalidStateException arg ->
      assoc_to_yojson
        [("KMSInvalidStateException",
           (Some (kms_invalid_state_exception_to_yojson arg)))]
  | KMSDisabledException arg ->
      assoc_to_yojson
        [("KMSDisabledException",
           (Some (kms_disabled_exception_to_yojson arg)))]
  | ResourceInUseException arg ->
      assoc_to_yojson
        [("ResourceInUseException",
           (Some (resource_in_use_exception_to_yojson arg)))]
  | ResourceNotFoundException arg ->
      assoc_to_yojson
        [("ResourceNotFoundException",
           (Some (resource_not_found_exception_to_yojson arg)))]
  | SubscribeToShardEvent arg ->
      assoc_to_yojson
        [("SubscribeToShardEvent",
           (Some (subscribe_to_shard_event_to_yojson arg)))]
...
let stream_summary_to_yojson (x : stream_summary) =
  assoc_to_yojson
    [("StreamCreationTimestamp",
       (option_to_yojson timestamp_to_yojson x.stream_creation_timestamp));
    ("StreamModeDetails",
...
let stream_summary_list_to_yojson tree =
  list_to_yojson stream_summary_to_yojson tree
let stream_name_list_to_yojson tree =
  list_to_yojson stream_name_to_yojson tree
...
let metrics_name_list_to_yojson tree =
  list_to_yojson metrics_name_to_yojson tree
let enhanced_metrics_to_yojson (x : enhanced_metrics) =
  assoc_to_yojson
    [("ShardLevelMetrics",
       (option_to_yojson metrics_name_list_to_yojson x.shard_level_metrics))]
let enhanced_monitoring_list_to_yojson tree =
  list_to_yojson enhanced_metrics_to_yojson tree
...
let stream_description_summary_to_yojson (x : stream_description_summary) =
  assoc_to_yojson
    ...
    ("StreamCreationTimestamp",
      (Some (timestamp_to_yojson x.stream_creation_timestamp)));
...
let start_stream_encryption_input_to_yojson
  (x : start_stream_encryption_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("KeyId", (Some (key_id_to_yojson x.key_id)));
    ("EncryptionType", (Some (encryption_type_to_yojson x.encryption_type)));
...
let consumer_to_yojson (x : consumer) =
  assoc_to_yojson
    [("ConsumerCreationTimestamp",
       (Some (timestamp_to_yojson x.consumer_creation_timestamp)));
...
let register_stream_consumer_output_to_yojson
  (x : register_stream_consumer_output) =
...
let put_record_input_to_yojson (x : put_record_input) =
  assoc_to_yojson
    ...
    ("SequenceNumberForOrdering",
      (option_to_yojson sequence_number_to_yojson
         x.sequence_number_for_ordering));
    ("ExplicitHashKey",
      (option_to_yojson hash_key_to_yojson x.explicit_hash_key));
    ("PartitionKey", (Some (partition_key_to_yojson x.partition_key)));
    ("Data", (Some (data_to_yojson x.data)));
...
let list_stream_consumers_input_to_yojson (x : list_stream_consumers_input) =
  assoc_to_yojson
    [("StreamCreationTimestamp",
       (option_to_yojson timestamp_to_yojson x.stream_creation_timestamp));
    ("MaxResults",
      (option_to_yojson list_stream_consumers_input_limit_to_yojson
         x.max_results));
    ("NextToken", (option_to_yojson next_token_to_yojson x.next_token));
    ("StreamARN", (Some (stream_ar_n_to_yojson x.stream_ar_n)))]
...
let increase_stream_retention_period_input_to_yojson
  (x : increase_stream_retention_period_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("RetentionPeriodHours",
      (Some (retention_period_hours_to_yojson x.retention_period_hours)));
...
let enhanced_monitoring_output_to_yojson (x : enhanced_monitoring_output) =
  assoc_to_yojson
    ...
    ("DesiredShardLevelMetrics",
      (option_to_yojson metrics_name_list_to_yojson
         x.desired_shard_level_metrics));
    ("CurrentShardLevelMetrics",
      (option_to_yojson metrics_name_list_to_yojson
         x.current_shard_level_metrics));
    ("StreamName", (option_to_yojson stream_name_to_yojson x.stream_name))]
let enable_enhanced_monitoring_input_to_yojson
  (x : enable_enhanced_monitoring_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("ShardLevelMetrics",
...
let disable_enhanced_monitoring_input_to_yojson
  (x : disable_enhanced_monitoring_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("ShardLevelMetrics",
      (Some (metrics_name_list_to_yojson x.shard_level_metrics)));
...
let describe_stream_summary_output_to_yojson
  (x : describe_stream_summary_output) =
  assoc_to_yojson
    [("StreamDescriptionSummary",
       (Some
          (stream_description_summary_to_yojson x.stream_description_summary)))]
let describe_stream_summary_input_to_yojson
  (x : describe_stream_summary_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("StreamName", (option_to_yojson stream_name_to_yojson x.stream_name))]
...
let describe_limits_output_to_yojson (x : describe_limits_output) =
  assoc_to_yojson
    [("OnDemandStreamCountLimit",
       (Some
          (on_demand_stream_count_limit_object_to_yojson
             x.on_demand_stream_count_limit)));
    ("OnDemandStreamCount",
      (Some
         (on_demand_stream_count_object_to_yojson x.on_demand_stream_count)));
    ("OpenShardCount",
      (Some (shard_count_object_to_yojson x.open_shard_count)));
...
let decrease_stream_retention_period_input_to_yojson
  (x : decrease_stream_retention_period_input) =
  assoc_to_yojson
    [("StreamARN", (option_to_yojson stream_ar_n_to_yojson x.stream_ar_n));
    ("RetentionPeriodHours",
      (Some (retention_period_hours_to_yojson x.retention_period_hours)));
