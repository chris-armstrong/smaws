...
let make_web_acl_has_out_of_scope_resources_violation
  ...
  ({ out_of_scope_resource_list = out_of_scope_resource_list_; web_acl_arn = web_acl_arn_ }
...
let make_web_acl_has_incompatible_configuration_violation
    ...
    ?web_acl_arn:(web_acl_arn_ : resource_arn option) () =
  ({ description = description_; web_acl_arn = web_acl_arn_ }
...
let make_partial_match
  ...
  ({ target_violation_reasons = target_violation_reasons_; reference = reference_ } : partial_match)

let make_security_group_rule_description ?to_port:(to_port_ : ip_port_number option)
    ?from_port:(from_port_ : ip_port_number option)
    ?protocol:(protocol_ : length_bounded_string option)
    ?prefix_list_id:(prefix_list_id_ : resource_id option) ?ipv6_range:(ipv6_range_ : cid_r option)
...
let make_security_group_remediation_action ?is_default_action:(is_default_action_ : boolean_ option)
...
let make_aws_ec2_network_interface_violation
    ...
    ?violation_target:(violation_target_ : violation_target option) () =
  ({ violating_security_groups = violating_security_groups_; violation_target = violation_target_ }
...
let make_stateless_rule_group ?priority:(priority_ : stateless_rule_group_priority option)
    ?resource_id:(resource_id_ : resource_id option)
    ?rule_group_name:(rule_group_name_ : network_firewall_resource_name option) () =
  ({ priority = priority_; resource_id = resource_id_; rule_group_name = rule_group_name_ }
...
let make_stateful_rule_group
    ...
    ?priority:(priority_ : priority_number option) ?resource_id:(resource_id_ : resource_id option)
    ?rule_group_name:(rule_group_name_ : network_firewall_resource_name option) () =
  ({
     override = override_;
     priority = priority_;
...
let make_stateful_engine_options
  ...
  ({ stream_exception_policy = stream_exception_policy_; rule_order = rule_order_ }
...
let make_expected_route ?route_table_id:(route_table_id_ : resource_id option)
    ...
    ?ip_v6_cidr:(ip_v6_cidr_ : cid_r option) ?prefix_list_id:(prefix_list_id_ : cid_r option)
...
let make_network_firewall_black_hole_route_detected_violation
    ?violating_routes:(violating_routes_ : routes option) ?vpc_id:(vpc_id_ : resource_id option)
    ?route_table_id:(route_table_id_ : resource_id option)
    ?violation_target:(violation_target_ : violation_target option) () =
  ({
     violating_routes = violating_routes_;
...
let make_network_firewall_unexpected_gateway_routes_violation ?vpc_id:(vpc_id_ : resource_id option)
    ?route_table_id:(route_table_id_ : resource_id option)
    ?violating_routes:(violating_routes_ : routes option)
...
let make_network_firewall_missing_expected_routes_violation ?vpc_id:(vpc_id_ : resource_id option)
    ?expected_routes:(expected_routes_ : expected_routes option)
    ?violation_target:(violation_target_ : violation_target option) () =
  ({ vpc_id = vpc_id_; expected_routes = expected_routes_; violation_target = violation_target_ }
...
let make_network_acl_entry
    ...
    ~egress:(egress_ : boolean_object) ~rule_action:(rule_action_ : network_acl_rule_action)
...
let make_action_target ?description:(description_ : length_bounded_string option)
    ?resource_id:(resource_id_ : resource_id option) () =
  ({ description = description_; resource_id = resource_id_ } : action_target)

let make_ec2_create_route_action ?gateway_id:(gateway_id_ : action_target option)
...
let make_ec2_replace_route_action ?gateway_id:(gateway_id_ : action_target option)
    ?destination_ipv6_cidr_block:(destination_ipv6_cidr_block_ : cid_r option)
    ?destination_prefix_list_id:(destination_prefix_list_id_ : resource_id option)
    ?destination_cidr_block:(destination_cidr_block_ : cid_r option)
    ?description:(description_ : length_bounded_string option)
...
let make_ec2_copy_route_table_action ?description:(description_ : length_bounded_string option)
    ~route_table_id:(route_table_id_ : action_target) ~vpc_id:(vpc_id_ : action_target) () =
  ({ route_table_id = route_table_id_; vpc_id = vpc_id_; description = description_ }
...
let make_ec2_associate_route_table_action ?gateway_id:(gateway_id_ : action_target option)
...
let make_fms_policy_update_firewall_creation_config_action
    ...
    ?description:(description_ : length_bounded_string option) () =
  ({ firewall_creation_config = firewall_creation_config_; description = description_ }
    : fms_policy_update_firewall_creation_config_action)

let make_create_network_acl_action ?fms_can_remediate:(fms_can_remediate_ : boolean_ option)
    ?vpc:(vpc_ : action_target option) ?description:(description_ : length_bounded_string option) ()
    =
  ({ fms_can_remediate = fms_can_remediate_; vpc = vpc_; description = description_ }
...
let make_create_network_acl_entries_action ?fms_can_remediate:(fms_can_remediate_ : boolean_ option)
...
let make_delete_network_acl_entries_action ?fms_can_remediate:(fms_can_remediate_ : boolean_ option)
...
let make_possible_remediation_action ?is_default_action:(is_default_action_ : boolean_ option)
    ?description:(description_ : length_bounded_string option)
...
let make_possible_remediation_actions ?actions:(actions_ : possible_remediation_action_list option)
    ?description:(description_ : length_bounded_string option) () =
...
let make_violation_detail
    ...
    ~member_account:(member_account_ : aws_account_id) ~policy_id:(policy_id_ : policy_id) () =
  ({
...
let make_third_party_firewall_firewall_policy
  ...
  ({ firewall_policy_name = firewall_policy_name_; firewall_policy_id = firewall_policy_id_ }
...
let make_network_acl_entry_set ?last_entries:(last_entries_ : network_acl_entries option)
...
let make_security_service_policy_data ?policy_option:(policy_option_ : policy_option option)
    ?managed_service_data:(managed_service_data_ : managed_service_data option)
    ~type_:(type__ : security_service_type) () =
  ({ policy_option = policy_option_; managed_service_data = managed_service_data_; type_ = type__ }
...
let make_resource_set_summary
    ...
    ?description:(description_ : description option) ?name:(name_ : name option)
...
let make_resource_set ?resource_set_status:(resource_set_status_ : resource_set_status option)
    ?last_update_time:(last_update_time_ : time_stamp option)
    ?update_token:(update_token_ : update_token option)
    ?description:(description_ : description option) ?id:(id_ : base62_id option)
    ~resource_type_list:(resource_type_list_ : resource_type_list) ~name:(name_ : name) () =
  ({
     resource_set_status = resource_set_status_;
     last_update_time = last_update_time_;
...
let make_region_scope ?all_regions_enabled:(all_regions_enabled_ : boolean_ option)
    ?regions:(regions_ : aws_region_list option) () =
  ({ all_regions_enabled = all_regions_enabled_; regions = regions_ } : region_scope)

let make_put_resource_set_response ~resource_set_arn:(resource_set_arn_ : resource_arn)
...
let make_protocols_list_data
    ...
    ?list_id:(list_id_ : list_id option) ~protocols_list:(protocols_list_ : protocols_list)
...
let make_put_protocols_list_response ?protocols_list_arn:(protocols_list_arn_ : resource_arn option)
    ?protocols_list:(protocols_list_ : protocols_list_data option) () =
  ({ protocols_list_arn = protocols_list_arn_; protocols_list = protocols_list_ }
...
let make_put_notification_channel_request ~sns_role_name:(sns_role_name_ : resource_arn)
...
let make_apps_list_data ?previous_apps_list:(previous_apps_list_ : previous_apps_list option)
    ?last_update_time:(last_update_time_ : time_stamp option)
    ?create_time:(create_time_ : time_stamp option)
    ?list_update_token:(list_update_token_ : update_token option)
...
let make_put_apps_list_response ?apps_list_arn:(apps_list_arn_ : resource_arn option)
...
let make_account_scope ?exclude_specified_accounts:(exclude_specified_accounts_ : boolean_ option)
...
let make_policy_type_scope ?all_policy_types_enabled:(all_policy_types_enabled_ : boolean_ option)
    ?policy_types:(policy_types_ : security_service_type_list option) () =
  ({ all_policy_types_enabled = all_policy_types_enabled_; policy_types = policy_types_ }
    : policy_type_scope)

let make_admin_scope ?policy_type_scope:(policy_type_scope_ : policy_type_scope option)
...
let make_protocols_list_data_summary ?protocols_list:(protocols_list_ : protocols_list option)
    ?list_name:(list_name_ : resource_name option) ?list_id:(list_id_ : list_id option)
...
let make_policy_summary ?policy_status:(policy_status_ : customer_policy_status option)
    ?delete_unused_fm_managed_resources:(delete_unused_fm_managed_resources_ : boolean_ option)
    ...
    ?policy_name:(policy_name_ : resource_name option) ?policy_id:(policy_id_ : policy_id option)
...
let make_policy_compliance_status ?issue_info_map:(issue_info_map_ : issue_info_map option)
...
let make_compliance_violator ?metadata:(metadata_ : compliance_violator_metadata option)
    ?resource_type:(resource_type_ : resource_type option)
...
let make_policy_compliance_detail ?issue_info_map:(issue_info_map_ : issue_info_map option)
    ...
    ?member_account:(member_account_ : aws_account_id option)
    ?policy_id:(policy_id_ : policy_id option) ?policy_owner:(policy_owner_ : aws_account_id option)
    () =
  ({
     issue_info_map = issue_info_map_;
...
let make_list_resource_sets_response ?next_token:(next_token_ : pagination_token option)
...
let make_list_resource_sets_request ?max_results:(max_results_ : pagination_max_results option)
    ?next_token:(next_token_ : pagination_token option) () =
...
let make_list_resource_set_resources_request ?next_token:(next_token_ : pagination_token option)
    ?max_results:(max_results_ : pagination_max_results option)
    ~identifier:(identifier_ : resource_id) () =
  ({ next_token = next_token_; max_results = max_results_; identifier = identifier_ }
    : list_resource_set_resources_request)

let make_list_protocols_lists_response ?next_token:(next_token_ : pagination_token option)
    ?protocols_lists:(protocols_lists_ : protocols_lists_data option) () =
  ({ next_token = next_token_; protocols_lists = protocols_lists_ } : list_protocols_lists_response)

let make_list_protocols_lists_request ?next_token:(next_token_ : pagination_token option)
    ?default_lists:(default_lists_ : boolean_ option)
    ~max_results:(max_results_ : pagination_max_results) () =
  ({ max_results = max_results_; next_token = next_token_; default_lists = default_lists_ }
    : list_protocols_lists_request)

let make_list_policies_response ?next_token:(next_token_ : pagination_token option)
...
let make_list_policies_request ?max_results:(max_results_ : pagination_max_results option)
    ?next_token:(next_token_ : pagination_token option) () =
  ({ max_results = max_results_; next_token = next_token_ } : list_policies_request)

let make_list_member_accounts_response ?next_token:(next_token_ : pagination_token option)
    ?member_accounts:(member_accounts_ : member_accounts option) () =
  ({ next_token = next_token_; member_accounts = member_accounts_ } : list_member_accounts_response)

let make_list_member_accounts_request ?max_results:(max_results_ : pagination_max_results option)
    ?next_token:(next_token_ : pagination_token option) () =
...
let make_discovered_resource ?name:(name_ : resource_name option)
    ?type_:(type__ : resource_type option) ?account_id:(account_id_ : aws_account_id option)
    ?ur_i:(ur_i_ : identifier option) () =
  ({ name = name_; type_ = type__; account_id = account_id_; ur_i = ur_i_ } : discovered_resource)

let make_list_discovered_resources_response ?next_token:(next_token_ : pagination_token option)
    ?items:(items_ : discovered_resource_list option) () =
  ({ next_token = next_token_; items = items_ } : list_discovered_resources_response)

let make_list_discovered_resources_request ?next_token:(next_token_ : pagination_token option)
    ?max_results:(max_results_ : pagination_max_results option)
    ~resource_type:(resource_type_ : resource_type)
...
let make_list_compliance_status_response ?next_token:(next_token_ : pagination_token option)
    ?policy_compliance_status_list:
      (policy_compliance_status_list_ : policy_compliance_status_list option) () =
  ({ next_token = next_token_; policy_compliance_status_list = policy_compliance_status_list_ }
    : list_compliance_status_response)

let make_list_compliance_status_request ?max_results:(max_results_ : pagination_max_results option)
    ?next_token:(next_token_ : pagination_token option) ~policy_id:(policy_id_ : policy_id) () =
...
let make_apps_list_data_summary ?apps_list:(apps_list_ : apps_list option)
    ?list_name:(list_name_ : resource_name option) ?list_id:(list_id_ : list_id option)
    ?list_arn:(list_arn_ : resource_arn option) () =
  ({ apps_list = apps_list_; list_name = list_name_; list_id = list_id_; list_arn = list_arn_ }
    : apps_list_data_summary)

let make_list_apps_lists_response ?next_token:(next_token_ : pagination_token option)
    ?apps_lists:(apps_lists_ : apps_lists_data option) () =
  ({ next_token = next_token_; apps_lists = apps_lists_ } : list_apps_lists_response)

let make_list_apps_lists_request ?next_token:(next_token_ : pagination_token option)
    ?default_lists:(default_lists_ : boolean_ option)
    ~max_results:(max_results_ : pagination_max_results) () =
  ({ max_results = max_results_; next_token = next_token_; default_lists = default_lists_ }
    : list_apps_lists_request)

let make_list_admins_managing_account_response ?next_token:(next_token_ : pagination_token option)
    ?admin_accounts:(admin_accounts_ : account_id_list option) () =
  ({ next_token = next_token_; admin_accounts = admin_accounts_ }
...
let make_admin_account_summary ?status:(status_ : organization_status option)
  ...
  ({ status = status_; default_admin = default_admin_; admin_account = admin_account_ }
...
let make_get_violation_details_request ~resource_type:(resource_type_ : resource_type)
    ~resource_id:(resource_id_ : resource_id) ~member_account:(member_account_ : aws_account_id)
...
let make_get_resource_set_response ~resource_set_arn:(resource_set_arn_ : resource_arn)
    ~resource_set:(resource_set_ : resource_set) () =
  ({ resource_set_arn = resource_set_arn_; resource_set = resource_set_ }
...
let make_get_protocols_list_response ?protocols_list_arn:(protocols_list_arn_ : resource_arn option)
    ?protocols_list:(protocols_list_ : protocols_list_data option) () =
  ({ protocols_list_arn = protocols_list_arn_; protocols_list = protocols_list_ }
...
let make_get_protection_status_response ?next_token:(next_token_ : pagination_token option)
...
let make_get_protection_status_request ?max_results:(max_results_ : pagination_max_results option)
    ?next_token:(next_token_ : pagination_token option) ?end_time:(end_time_ : time_stamp option)
...
let make_get_notification_channel_response ?sns_role_name:(sns_role_name_ : resource_arn option)
...
let make_get_compliance_detail_request ~member_account:(member_account_ : aws_account_id)
    ~policy_id:(policy_id_ : policy_id) () =
  ({ member_account = member_account_; policy_id = policy_id_ } : get_compliance_detail_request)

let make_get_apps_list_response ?apps_list_arn:(apps_list_arn_ : resource_arn option)
    ?apps_list:(apps_list_ : apps_list_data option) () =
...
let make_get_admin_scope_response ?status:(status_ : organization_status option)
...
let make_get_admin_account_response ?role_status:(role_status_ : account_role_status option)
    ?admin_account:(admin_account_ : aws_account_id option) () =
  ({ role_status = role_status_; admin_account = admin_account_ } : get_admin_account_response)

let make_get_admin_account_request () = (() : unit)

let make_failed_item ?reason:(reason_ : failed_item_reason option) ?ur_i:(ur_i_ : identifier option)
    () =
...
let make_delete_policy_request
    ...
    ~policy_id:(policy_id_ : policy_id) () =
  ({ delete_all_policy_resources = delete_all_policy_resources_; policy_id = policy_id_ }
...
let make_batch_disassociate_resource_response ~failed_items:(failed_items_ : failed_item_list)
    ~resource_set_identifier:(resource_set_identifier_ : identifier) () =
  ({ failed_items = failed_items_; resource_set_identifier = resource_set_identifier_ }
    : batch_disassociate_resource_response)

let make_batch_disassociate_resource_request ~items:(items_ : identifier_list)
    ~resource_set_identifier:(resource_set_identifier_ : identifier) () =
  ({ items = items_; resource_set_identifier = resource_set_identifier_ }
    : batch_disassociate_resource_request)

let make_batch_associate_resource_response ~failed_items:(failed_items_ : failed_item_list)
    ~resource_set_identifier:(resource_set_identifier_ : identifier) () =
  ({ failed_items = failed_items_; resource_set_identifier = resource_set_identifier_ }
