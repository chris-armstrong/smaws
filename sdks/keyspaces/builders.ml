...
let make_column_definition ~type_:(type__ : generic_string) ~name:(name_ : generic_string) () =
...
let make_auto_scaling_settings ?scaling_policy:(scaling_policy_ : auto_scaling_policy option)
...
let make_replica_specification
    ?read_capacity_auto_scaling:(read_capacity_auto_scaling_ : auto_scaling_settings option)
    ?read_capacity_units:(read_capacity_units_ : capacity_units option) ~region:(region_ : region)
    () =
  ({
     read_capacity_auto_scaling = read_capacity_auto_scaling_;
     read_capacity_units = read_capacity_units_;
...
let make_cdc_specification ?propagate_tags:(propagate_tags_ : cdc_propagate_tags option)
...
let make_untag_resource_request ~tags:(tags_ : tag_list) ~resource_arn:(resource_arn_ : ar_n) () =
  ({ tags = tags_; resource_arn = resource_arn_ } : untag_resource_request)

let make_tag_resource_response () = (() : unit)

let make_tag_resource_request ~tags:(tags_ : tag_list) ~resource_arn:(resource_arn_ : ar_n) () =
  ({ tags = tags_; resource_arn = resource_arn_ } : tag_resource_request)

let make_table_summary ~resource_arn:(resource_arn_ : ar_n) ~table_name:(table_name_ : table_name)
    ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({ resource_arn = resource_arn_; table_name = table_name_; keyspace_name = keyspace_name_ }
    : table_summary)
...
let make_clustering_key ~order_by:(order_by_ : sort_order) ~name:(name_ : generic_string) () =
  ({ order_by = order_by_; name = name_ } : clustering_key)

let make_schema_definition ?static_columns:(static_columns_ : static_column_list option)
...
let make_replication_group_status
    ...
    ~keyspace_status:(keyspace_status_ : keyspace_status) ~region:(region_ : region) () =
  ({
...
let make_replica_auto_scaling_specification
  ...
  ({ auto_scaling_specification = auto_scaling_specification_; region = region_ }
...
let make_point_in_time_recovery_summary
  ...
  ({ earliest_restorable_timestamp = earliest_restorable_timestamp_; status = status_ }
...
let make_list_types_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) ~keyspace_name:(keyspace_name_ : keyspace_name) ()
    =
  ({ keyspace_name = keyspace_name_; max_results = max_results_; next_token = next_token_ }
    : list_types_request)
...
let make_list_tags_for_resource_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) ~resource_arn:(resource_arn_ : ar_n) () =
...
let make_keyspace_summary ?replication_regions:(replication_regions_ : region_list option)
    ~replication_strategy:(replication_strategy_ : rs) ~resource_arn:(resource_arn_ : ar_n)
    ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({
...
let make_list_keyspaces_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : next_token option) () =
  ({ max_results = max_results_; next_token = next_token_ } : list_keyspaces_request)

let make_field_definition ~type_:(type__ : generic_string) ~name:(name_ : generic_string) () =
  ({ type_ = type__; name = name_ } : field_definition)

let make_get_type_response ?max_nesting_depth:(max_nesting_depth_ : depth option)
    ...
    ?field_definitions:(field_definitions_ : field_list option) ~keyspace_arn:(keyspace_arn_ : ar_n)
    ~type_name:(type_name_ : type_name) ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({
...
let make_get_table_auto_scaling_settings_request ~table_name:(table_name_ : table_name)
    ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({ table_name = table_name_; keyspace_name = keyspace_name_ }
...
let make_create_type_request ~field_definitions:(field_definitions_ : field_list)
    ~type_name:(type_name_ : type_name) ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({
     field_definitions = field_definitions_;
     type_name = type_name_;
...
let make_create_keyspace_request
    ...
    ?tags:(tags_ : tag_list option) ~keyspace_name:(keyspace_name_ : keyspace_name) () =
  ({
     replication_specification = replication_specification_;
     tags = tags_;
     keyspace_name = keyspace_name_;
