...
let rs_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "rs") : rs) : rs)
...
let view_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "ViewType" value)
    | _ -> raise (deserialize_wrong_type_error path "ViewType") : view_type) : 
  view_type)
...
let client_side_timestamps_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path "ClientSideTimestampsStatus") : 
     client_side_timestamps_status) : client_side_timestamps_status)
...
let cdc_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "CdcStatus" value)
    | _ -> raise (deserialize_wrong_type_error path "CdcStatus") : cdc_status) : 
  cdc_status)
...
let cdc_propagate_tags_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "CdcPropagateTags" value)
    | _ -> raise (deserialize_wrong_type_error path "CdcPropagateTags") : 
     cdc_propagate_tags) : cdc_propagate_tags)
let cdc_specification_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     propagate_tags =
       (option_of_yojson
          (value_for_key cdc_propagate_tags_of_yojson "propagateTags") _list
          path);
     tags =
       (option_of_yojson (value_for_key tag_list_of_yojson "tags") _list path);
     view_type =
       (option_of_yojson (value_for_key view_type_of_yojson "viewType") _list
          path);
...
let update_table_request_of_yojson tree path =
  ...
  ({
     cdc_specification =
       (option_of_yojson
          (value_for_key cdc_specification_of_yojson "cdcSpecification")
          _list path);
...
let type_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "TypeStatus" value)
    | _ -> raise (deserialize_wrong_type_error path "TypeStatus") : type_status) : 
  type_status)
let type_name_of_yojson = string_of_yojson
let type_name_list_of_yojson tree path =
  list_of_yojson type_name_of_yojson tree path
...
let restore_table_request_of_yojson tree path =
  ...
  ({
     ...
     restore_timestamp =
       (option_of_yojson
          (value_for_key timestamp_of_yojson "restoreTimestamp") _list path);
     target_table_name =
...
let keyspace_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "KeyspaceStatus" value)
...
let replication_group_status_of_yojson tree path =
  ...
  ({
     tables_replication_progress =
       (option_of_yojson
          (value_for_key tables_replication_progress_of_yojson
             "tablesReplicationProgress") _list path);
     keyspace_status =
       (value_for_key keyspace_status_of_yojson "keyspaceStatus" _list path);
     region = (value_for_key region_of_yojson "region" _list path)
...
let capacity_specification_summary_of_yojson tree path =
  ...
  ({
     last_update_to_pay_per_request_timestamp =
       (option_of_yojson
          (value_for_key timestamp_of_yojson
...
let point_in_time_recovery_summary_of_yojson tree path =
  ...
  ({
     earliest_restorable_timestamp =
       (option_of_yojson
          (value_for_key timestamp_of_yojson "earliestRestorableTimestamp")
          _list path);
     status =
       (value_for_key point_in_time_recovery_status_of_yojson "status" _list
          path)
...
let field_list_of_yojson tree path =
  list_of_yojson field_definition_of_yojson tree path
...
let get_type_response_of_yojson tree path =
  ...
  ({
     ...
     max_nesting_depth =
       (option_of_yojson (value_for_key depth_of_yojson "maxNestingDepth")
          _list path);
     direct_parent_types =
       (option_of_yojson
          (value_for_key type_name_list_of_yojson "directParentTypes") _list
          path);
     direct_referring_tables =
       (option_of_yojson
          (value_for_key table_name_list_of_yojson "directReferringTables")
          _list path);
     status =
       (option_of_yojson (value_for_key type_status_of_yojson "status") _list
          path);
     last_modified_timestamp =
       (option_of_yojson
          (value_for_key timestamp_of_yojson "lastModifiedTimestamp") _list
          path);
     field_definitions =
       (option_of_yojson
          (value_for_key field_list_of_yojson "fieldDefinitions") _list path);
...
let get_table_response_of_yojson tree path =
  ...
  ({
     cdc_specification =
       (option_of_yojson
          (value_for_key cdc_specification_summary_of_yojson
             "cdcSpecification") _list path);
     latest_stream_arn =
       (option_of_yojson
...
let get_keyspace_response_of_yojson tree path =
  ...
  ({
     replication_group_statuses =
       (option_of_yojson
          (value_for_key replication_group_status_list_of_yojson
             "replicationGroupStatuses") _list path);
     replication_regions =
       (option_of_yojson
          (value_for_key region_list_of_yojson "replicationRegions") _list
          path);
