...
let make_health_check_config
  ...
  ?resource_path:(resource_path_ : resource_path option)
...
let make_soa_change ~tt_l:(tt_l_ : record_tt_l) () =
  ({ tt_l = tt_l_ } : soa_change)
...
let make_http_namespace_change
  ~description:(description_ : resource_description) () =
...
let make_service_summary ?create_date:(create_date_ : timestamp option)
  ...
  ?description:(description_ : resource_description option)
  ?type_:(type__ : service_type option) ?name:(name_ : service_name option)
...
let make_service
  ...
  ?description:(description_ : resource_description option)
  ?namespace_id:(namespace_id_ : resource_id option)
  ?name:(name_ : service_name option) ?arn:(arn_ : arn option)
...
let make_list_operations_request
  ?filters:(filters_ : operation_filters option)
  ?max_results:(max_results_ : max_results option)
  ?next_token:(next_token_ : next_token option) () =
  ({ filters = filters_; max_results = max_results_; next_token = next_token_
...
let make_namespace_summary ?create_date:(create_date_ : timestamp option)
  ...
  ?name:(name_ : namespace_name option) ?arn:(arn_ : arn option)
  ?id:(id_ : resource_id option) () =
  ({
     create_date = create_date_;
...
let make_list_namespaces_request
  ?filters:(filters_ : namespace_filters option)
  ?max_results:(max_results_ : max_results option)
  ?next_token:(next_token_ : next_token option) () =
  ({ filters = filters_; max_results = max_results_; next_token = next_token_
...
let make_get_service_attributes_request
  ~service_id:(service_id_ : resource_id) () =
...
let make_get_service_request ~id:(id_ : resource_id) () =
...
let make_operation ?targets:(targets_ : operation_targets_map option)
  ...
  ?create_date:(create_date_ : timestamp option)
  ?error_code:(error_code_ : code option)
  ?error_message:(error_message_ : message option)
  ?status:(status_ : operation_status option)
  ?type_:(type__ : operation_type option) ?id:(id_ : operation_id option) ()
  =
  ({
     targets = targets_;
...
let make_get_namespace_request ~id:(id_ : resource_id) () =
...
let make_get_instances_health_status_request
  ?next_token:(next_token_ : next_token option)
  ?max_results:(max_results_ : max_results option)
  ?instances:(instances_ : instance_id_list option)
  ~service_id:(service_id_ : resource_id) () =
  ({
     next_token = next_token_;
     max_results = max_results_;
...
let make_delete_service_request ~id:(id_ : resource_id) () =
...
let make_create_private_dns_namespace_request
  ...
  ?description:(description_ : resource_description option)
  ?creator_request_id:(creator_request_id_ : resource_id option)
  ~vpc:(vpc_ : resource_id) ~name:(name_ : namespace_name_private) () =
  ({
     properties = properties_;
     tags = tags_;
     vpc = vpc_;
