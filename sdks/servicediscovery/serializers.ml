...
let dns_record_to_yojson (x : dns_record) =
  assoc_to_yojson
    [ ("TTL", Some (record_tt_l_to_yojson x.tt_l)); ("Type", Some (record_type_to_yojson x.type_)) ]
...
let health_check_type_to_yojson (x : health_check_type) =
  match x with TCP -> `String "TCP" | HTTPS -> `String "HTTPS" | HTTP -> `String "HTTP"
...
let routing_policy_to_yojson (x : routing_policy) =
  match x with WEIGHTED -> `String "WEIGHTED" | MULTIVALUE -> `String "MULTIVALUE"
