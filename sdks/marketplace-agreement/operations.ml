  ...
  struct
    ...
    let request context (request : describe_agreement_input) =
      let open Smaws_Lib.Context in
        let input =
          Json_serializers.describe_agreement_input_to_yojson request in
...
module GetAgreementTerms =
  struct
    ...
    let request context (request : get_agreement_terms_input) =
      let open Smaws_Lib.Context in
        let input =
          Json_serializers.get_agreement_terms_input_to_yojson request in
        Smaws_Lib.Protocols.AwsJson.request
...
module SearchAgreements =
  struct
    ...
    let request context (request : search_agreements_input) =
      let open Smaws_Lib.Context in
        let input =
          Json_serializers.search_agreements_input_to_yojson request in
        Smaws_Lib.Protocols.AwsJson.request
          ~shape_name:"AWSMPCommerceService_v20200301SearchAgreements"
          ~service ~config:context.config ~http:context.http ~input
          ~output_deserializer:Json_deserializers.search_agreements_output_of_yojson
          ~error_deserializer
  end