...
let validity_term_to_yojson (x : validity_term) =
  assoc_to_yojson
    [
      ("agreementEndDate", option_to_yojson timestamp__to_yojson x.agreement_end_date);
      ("agreementStartDate", option_to_yojson timestamp__to_yojson x.agreement_start_date);
...
let agreement_view_summary_to_yojson (x : agreement_view_summary) =
  assoc_to_yojson
    [
      ...
      ("acceptanceTime", option_to_yojson timestamp__to_yojson x.acceptance_time);
...
let resource_type_to_yojson (x : resource_type) = match x with AGREEMENT -> `String "Agreement"
...
let byol_pricing_term_to_yojson (x : byol_pricing_term) =
  assoc_to_yojson [ ("type", option_to_yojson unversioned_term_type_to_yojson x.type_) ]
