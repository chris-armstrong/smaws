...
let make_update_vpc_endpoint_detail
    ...
    ?security_group_ids:(security_group_ids_ : security_group_ids option)
    ?subnet_ids:(subnet_ids_ : subnet_ids option) ?status:(status_ : vpc_endpoint_status option)
    ?name:(name_ : vpc_endpoint_name option) ?id:(id_ : vpc_endpoint_id option) () =
  ({
...
let make_update_vpc_endpoint_request ?client_token:(client_token_ : client_token option)
    ...
    ?add_subnet_ids:(add_subnet_ids_ : subnet_ids option) ~id:(id_ : vpc_endpoint_id) () =
  ({
...
let make_security_policy_detail
    ...
    ?policy_version:(policy_version_ : policy_version option) ?name:(name_ : policy_name option)
...
let make_update_security_policy_request ?client_token:(client_token_ : client_token option)
    ?policy:(policy_ : policy_document option)
    ?description:(description_ : policy_description option)
    ~policy_version:(policy_version_ : policy_version) ~name:(name_ : policy_name)
...
let make_security_config_detail
    ...
    ?type_:(type__ : security_config_type option) ?id:(id_ : security_config_id option) () =
  ({
     last_modified_date = last_modified_date_;
     created_date = created_date_;
...
let make_update_lifecycle_policy_request ?client_token:(client_token_ : client_token option)
    ?policy:(policy_ : policy_document option)
    ?description:(description_ : policy_description option)
    ~policy_version:(policy_version_ : policy_version) ~name:(name_ : policy_name)
...
let make_update_access_policy_request ?client_token:(client_token_ : client_token option)
    ?policy:(policy_ : policy_document option)
    ?description:(description_ : policy_description option)
    ~policy_version:(policy_version_ : policy_version) ~name:(name_ : policy_name)
...
let make_create_security_policy_request ?client_token:(client_token_ : client_token option)
    ?description:(description_ : policy_description option) ~policy:(policy_ : policy_document)
...
let make_create_lifecycle_policy_request ?client_token:(client_token_ : client_token option)
    ?description:(description_ : policy_description option) ~policy:(policy_ : policy_document)
...
let make_lifecycle_policy_error_detail
    ?error_code:(error_code_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?error_message:(error_message_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?name:(name_ : policy_name option) ?type_:(type__ : lifecycle_policy_type option) () =
  ({ error_code = error_code_; error_message = error_message_; name = name_; type_ = type__ }
...
let make_effective_lifecycle_policy_detail
    ...
    ?policy_name:(policy_name_ : policy_name option) ?resource:(resource_ : resource option)
    ?type_:(type__ : lifecycle_policy_type option) () =
  ({
...
let make_effective_lifecycle_policy_error_detail
    ?error_code:(error_code_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?error_message:(error_message_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?resource:(resource_ : resource option) ?type_:(type__ : lifecycle_policy_type option) () =
  ({
...
let make_lifecycle_policy_resource_identifier ~resource:(resource_ : resource_name)
    ~type_:(type__ : lifecycle_policy_type) () =
...
let make_batch_get_collection_request ?names:(names_ : collection_names option)
...
let make_list_vpc_endpoints_response
  ...
  ({ next_token = next_token_; vpc_endpoint_summaries = vpc_endpoint_summaries_ }
...
let make_list_security_policies_response
  ...
  ({ next_token = next_token_; security_policy_summaries = security_policy_summaries_ }
    : list_security_policies_response)

let make_list_security_policies_request
    ?max_results:(max_results_ : Smaws_Lib.Smithy_api.Types.integer option)
    ?next_token:(next_token_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?resource:(resource_ : resource_filter option) ~type_:(type__ : security_policy_type) () =
  ({ max_results = max_results_; next_token = next_token_; resource = resource_; type_ = type__ }
...
let make_list_security_configs_response
  ...
  ({ next_token = next_token_; security_config_summaries = security_config_summaries_ }
...
let make_list_lifecycle_policies_response
    ...
    =
  ({ next_token = next_token_; lifecycle_policy_summaries = lifecycle_policy_summaries_ }
...
let make_list_access_policies_response
  ...
  ({ next_token = next_token_; access_policy_summaries = access_policy_summaries_ }
    : list_access_policies_response)

let make_list_access_policies_request
    ?max_results:(max_results_ : Smaws_Lib.Smithy_api.Types.integer option)
    ?next_token:(next_token_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?resource:(resource_ : resource_filter option) ~type_:(type__ : access_policy_type) () =
  ({ max_results = max_results_; next_token = next_token_; resource = resource_; type_ = type__ }
...
let make_create_vpc_endpoint_detail ?status:(status_ : vpc_endpoint_status option)
    ?name:(name_ : vpc_endpoint_name option) ?id:(id_ : vpc_endpoint_id option) () =
...
let make_create_vpc_endpoint_request ?client_token:(client_token_ : client_token option)
    ?security_group_ids:(security_group_ids_ : security_group_ids option)
...
let make_create_security_config_request ?client_token:(client_token_ : client_token option)
    ...
    ?saml_options:(saml_options_ : saml_config_options option)
    ?description:(description_ : config_description option) ~name:(name_ : config_name)
    ~type_:(type__ : security_config_type) () =
  ({
     client_token = client_token_;
     iam_identity_center_options = iam_identity_center_options_;
...
let make_create_access_policy_request ?client_token:(client_token_ : client_token option)
    ?description:(description_ : policy_description option) ~policy:(policy_ : policy_document)
    ~name:(name_ : policy_name) ~type_:(type__ : access_policy_type) () =
  ({
     client_token = client_token_;
