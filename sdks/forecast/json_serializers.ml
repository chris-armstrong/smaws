...
let filter_condition_string_to_yojson (x : filter_condition_string) =
  match x with IS -> `String "IS" | IS_NOT -> `String "IS_NOT"
...
let time_point_granularity_to_yojson (x : time_point_granularity) =
  match x with ALL -> `String "ALL" | SPECIFIC -> `String "SPECIFIC"
...
let evaluation_type_to_yojson (x : evaluation_type) =
  match x with SUMMARY -> `String "SUMMARY" | COMPUTED -> `String "COMPUTED"
...
let supplementary_feature_to_yojson (x : supplementary_feature) =
  assoc_to_yojson
    [ ("Name", Some (name_to_yojson x.name)); ("Value", Some (value_to_yojson x.value)) ]
...
let categorical_parameter_range_to_yojson (x : categorical_parameter_range) =
  assoc_to_yojson
    [ ("Name", Some (name_to_yojson x.name)); ("Values", Some (values_to_yojson x.values)) ]
