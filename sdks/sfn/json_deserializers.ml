...
let variable_name_list_of_yojson tree path = list_of_yojson variable_name_of_yojson tree path
...
let validate_state_machine_definition_severity_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "ValidateStateMachineDefinitionSeverity" value)
...
let validate_state_machine_definition_result_code_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "ValidateStateMachineDefinitionResultCode"
             value)
...
let validate_state_machine_definition_diagnostic_of_yojson tree path =
  ...
  ({
     location =
       option_of_yojson
         (value_for_key validate_state_machine_definition_location_of_yojson "location")
         _list path;
     message =
       value_for_key validate_state_machine_definition_message_of_yojson "message" _list path;
     code = value_for_key validate_state_machine_definition_code_of_yojson "code" _list path;
     severity =
       value_for_key validate_state_machine_definition_severity_of_yojson "severity" _list path;
   }
    : validate_state_machine_definition_diagnostic)

let validate_state_machine_definition_diagnostic_list_of_yojson tree path =
  list_of_yojson validate_state_machine_definition_diagnostic_of_yojson tree path
...
let validate_state_machine_definition_output_of_yojson tree path =
  ...
  ({
     truncated =
       option_of_yojson
         (value_for_key validate_state_machine_definition_truncated_of_yojson "truncated")
         _list path;
     diagnostics =
...
let validate_state_machine_definition_input_of_yojson tree path =
  ...
  ({
     max_results =
       option_of_yojson
         (value_for_key validate_state_machine_definition_max_result_of_yojson "maxResults")
         _list path;
     severity =
       option_of_yojson
         (value_for_key validate_state_machine_definition_severity_of_yojson "severity")
         _list path;
...
let update_state_machine_output_of_yojson tree path =
  ...
  ({
     state_machine_version_arn =
       option_of_yojson (value_for_key arn_of_yojson "stateMachineVersionArn") _list path;
     revision_id = option_of_yojson (value_for_key revision_id_of_yojson "revisionId") _list path;
     update_date = value_for_key timestamp_of_yojson "updateDate" _list path;
   }
...
let log_destination_of_yojson tree path =
  ...
  ({
     cloud_watch_logs_log_group =
       option_of_yojson
         (value_for_key cloud_watch_logs_log_group_of_yojson "cloudWatchLogsLogGroup")
         _list path;
   }
    : log_destination)

let log_destination_list_of_yojson tree path = list_of_yojson log_destination_of_yojson tree path

let logging_configuration_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     destinations =
       option_of_yojson (value_for_key log_destination_list_of_yojson "destinations") _list path;
     include_execution_data =
       option_of_yojson
         (value_for_key include_execution_data_of_yojson "includeExecutionData")
         _list path;
...
let encryption_configuration_of_yojson tree path =
  ...
  ({
     ...
     kms_data_key_reuse_period_seconds =
       option_of_yojson
         (value_for_key kms_data_key_reuse_period_seconds_of_yojson "kmsDataKeyReusePeriodSeconds")
         _list path;
...
let update_state_machine_input_of_yojson tree path =
  ...
  ({
     ...
     tracing_configuration =
       option_of_yojson
         (value_for_key tracing_configuration_of_yojson "tracingConfiguration")
         _list path;
     logging_configuration =
       option_of_yojson
...
let update_state_machine_alias_input_of_yojson tree path =
  ...
  ({
     routing_configuration =
       option_of_yojson
         (value_for_key routing_configuration_list_of_yojson "routingConfiguration")
         _list path;
     description =
       option_of_yojson (value_for_key alias_description_of_yojson "description") _list path;
...
let update_map_run_input_of_yojson tree path =
  ...
  ({
     tolerated_failure_count =
       option_of_yojson
         (value_for_key tolerated_failure_count_of_yojson "toleratedFailureCount")
         _list path;
     tolerated_failure_percentage =
       option_of_yojson
         (value_for_key tolerated_failure_percentage_of_yojson "toleratedFailurePercentage")
         _list path;
     max_concurrency =
...
let inspection_data_response_of_yojson tree path =
  ...
  ({
     ...
     headers = option_of_yojson (value_for_key http_headers_of_yojson "headers") _list path;
     status_message =
       option_of_yojson (value_for_key http_status_message_of_yojson "statusMessage") _list path;
     status_code =
       option_of_yojson (value_for_key http_status_code_of_yojson "statusCode") _list path;
     protocol = option_of_yojson (value_for_key http_protocol_of_yojson "protocol") _list path;
   }
...
let inspection_data_of_yojson tree path =
  ...
  ({
     ...
     request =
       option_of_yojson (value_for_key inspection_data_request_of_yojson "request") _list path;
     after_result_path =
     ...
     after_input_path =
       option_of_yojson (value_for_key sensitive_data_of_yojson "afterInputPath") _list path;
     after_arguments =
       option_of_yojson (value_for_key sensitive_data_of_yojson "afterArguments") _list path;
...
let test_state_output_of_yojson tree path =
  ...
  ({
     ...
     inspection_data =
       option_of_yojson (value_for_key inspection_data_of_yojson "inspectionData") _list path;
...
let test_state_input_of_yojson tree path =
  ...
  ({
     ...
     inspection_level =
       option_of_yojson (value_for_key inspection_level_of_yojson "inspectionLevel") _list path;
     input = option_of_yojson (value_for_key sensitive_data_of_yojson "input") _list path;
     role_arn = option_of_yojson (value_for_key arn_of_yojson "roleArn") _list path;
     definition = value_for_key definition_of_yojson "definition" _list path;
   }
...
let task_succeeded_event_details_of_yojson tree path =
  ...
  ({
     output_details =
       option_of_yojson
         (value_for_key history_event_execution_data_details_of_yojson "outputDetails")
         _list path;
     output = option_of_yojson (value_for_key sensitive_data_of_yojson "output") _list path;
     resource = value_for_key name_of_yojson "resource" _list path;
     resource_type = value_for_key name_of_yojson "resourceType" _list path;
...
let task_scheduled_event_details_of_yojson tree path =
  ...
  ({
     task_credentials =
       option_of_yojson (value_for_key task_credentials_of_yojson "taskCredentials") _list path;
     heartbeat_in_seconds =
       option_of_yojson (value_for_key timeout_in_seconds_of_yojson "heartbeatInSeconds") _list path;
     timeout_in_seconds =
...
let stop_execution_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ stop_date = value_for_key timestamp_of_yojson "stopDate" _list path } : stop_execution_output)
...
let kms_key_state_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value -> raise (deserialize_unknown_enum_value_error path "KmsKeyState" value)
...
let kms_invalid_state_exception_of_yojson tree path =
  ...
  ({
     message = option_of_yojson (value_for_key error_message_of_yojson "message") _list path;
     kms_key_state =
       option_of_yojson (value_for_key kms_key_state_of_yojson "kmsKeyState") _list path;
   }
...
let state_exited_event_details_of_yojson tree path =
  ...
  ({
     assigned_variables_details =
       option_of_yojson
         (value_for_key assigned_variables_details_of_yojson "assignedVariablesDetails")
         _list path;
     assigned_variables =
       option_of_yojson (value_for_key assigned_variables_of_yojson "assignedVariables") _list path;
     output_details =
       option_of_yojson
         (value_for_key history_event_execution_data_details_of_yojson "outputDetails")
         _list path;
...
let state_entered_event_details_of_yojson tree path =
  ...
  ({
     input_details =
       option_of_yojson
         (value_for_key history_event_execution_data_details_of_yojson "inputDetails")
         _list path;
     input = option_of_yojson (value_for_key sensitive_data_of_yojson "input") _list path;
     name = value_for_key name_of_yojson "name" _list path;
   }
...
let billing_details_of_yojson tree path =
  ...
  ({
     billed_duration_in_milliseconds =
       option_of_yojson
         (value_for_key billed_duration_of_yojson "billedDurationInMilliseconds")
         _list path;
...
let start_sync_execution_output_of_yojson tree path =
  ...
  ({
     billing_details =
       option_of_yojson (value_for_key billing_details_of_yojson "billingDetails") _list path;
     trace_header = option_of_yojson (value_for_key trace_header_of_yojson "traceHeader") _list path;
     output_details =
       option_of_yojson
         (value_for_key cloud_watch_events_execution_data_details_of_yojson "outputDetails")
         _list path;
     output = option_of_yojson (value_for_key sensitive_data_of_yojson "output") _list path;
     input_details =
       option_of_yojson
         (value_for_key cloud_watch_events_execution_data_details_of_yojson "inputDetails")
         _list path;
     input = option_of_yojson (value_for_key sensitive_data_of_yojson "input") _list path;
     cause = option_of_yojson (value_for_key sensitive_cause_of_yojson "cause") _list path;
     error = option_of_yojson (value_for_key sensitive_error_of_yojson "error") _list path;
...
let start_sync_execution_input_of_yojson tree path =
  ...
  ({
     included_data =
       option_of_yojson (value_for_key included_data_of_yojson "includedData") _list path;
     trace_header = option_of_yojson (value_for_key trace_header_of_yojson "traceHeader") _list path;
     input = option_of_yojson (value_for_key sensitive_data_of_yojson "input") _list path;
     name = option_of_yojson (value_for_key name_of_yojson "name") _list path;
     state_machine_arn = value_for_key arn_of_yojson "stateMachineArn" _list path;
...
let redrive_execution_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ redrive_date = value_for_key timestamp_of_yojson "redriveDate" _list path }
...
let publish_state_machine_version_input_of_yojson tree path =
  ...
  ({
     description =
       option_of_yojson (value_for_key version_description_of_yojson "description") _list path;
     revision_id = option_of_yojson (value_for_key revision_id_of_yojson "revisionId") _list path;
     state_machine_arn = value_for_key arn_of_yojson "stateMachineArn" _list path;
   }
...
let map_run_redriven_event_details_of_yojson tree path =
  ...
  ({
     redrive_count =
       option_of_yojson (value_for_key redrive_count_of_yojson "redriveCount") _list path;
     map_run_arn = option_of_yojson (value_for_key long_arn_of_yojson "mapRunArn") _list path;
   }
...
let map_run_list_item_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     stop_date = option_of_yojson (value_for_key timestamp_of_yojson "stopDate") _list path;
     start_date = value_for_key timestamp_of_yojson "startDate" _list path;
     state_machine_arn = value_for_key arn_of_yojson "stateMachineArn" _list path;
     map_run_arn = value_for_key long_arn_of_yojson "mapRunArn" _list path;
     execution_arn = value_for_key arn_of_yojson "executionArn" _list path;
...
let map_run_list_of_yojson tree path = list_of_yojson map_run_list_item_of_yojson tree path
...
let map_run_item_counts_of_yojson tree path =
  ...
  ({
     pending_redrive =
       option_of_yojson (value_for_key long_object_of_yojson "pendingRedrive") _list path;
     failures_not_redrivable =
       option_of_yojson (value_for_key long_object_of_yojson "failuresNotRedrivable") _list path;
...
let list_state_machine_versions_output_of_yojson tree path =
  ...
  ({
     ...
     state_machine_versions =
       value_for_key state_machine_version_list_of_yojson "stateMachineVersions" _list path;
   }
...
let list_state_machine_aliases_output_of_yojson tree path =
  ...
  ({
     ...
     state_machine_aliases =
       value_for_key state_machine_alias_list_of_yojson "stateMachineAliases" _list path;
   }
...
let execution_list_item_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     redrive_date = option_of_yojson (value_for_key timestamp_of_yojson "redriveDate") _list path;
     redrive_count =
       option_of_yojson (value_for_key redrive_count_of_yojson "redriveCount") _list path;
     state_machine_alias_arn =
       option_of_yojson (value_for_key arn_of_yojson "stateMachineAliasArn") _list path;
     state_machine_version_arn =
       option_of_yojson (value_for_key arn_of_yojson "stateMachineVersionArn") _list path;
...
let execution_list_of_yojson tree path = list_of_yojson execution_list_item_of_yojson tree path

let list_executions_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     next_token =
       option_of_yojson (value_for_key list_executions_page_token_of_yojson "nextToken") _list path;
...
let execution_redrive_filter_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "ExecutionRedriveFilter" value)
    | _ -> raise (deserialize_wrong_type_error path "ExecutionRedriveFilter")
     : execution_redrive_filter)
    : execution_redrive_filter)
...
let list_executions_input_of_yojson tree path =
  ...
  ({
     ...
     max_results = option_of_yojson (value_for_key page_size_of_yojson "maxResults") _list path;
     status_filter =
       option_of_yojson (value_for_key execution_status_of_yojson "statusFilter") _list path;
     state_machine_arn = option_of_yojson (value_for_key arn_of_yojson "stateMachineArn") _list path;
   }
...
let activity_list_of_yojson tree path = list_of_yojson activity_list_item_of_yojson tree path
...
let evaluation_failed_event_details_of_yojson tree path =
  ...
  ({
     ...
     location =
       option_of_yojson (value_for_key evaluation_failure_location_of_yojson "location") _list path;
     cause = option_of_yojson (value_for_key sensitive_cause_of_yojson "cause") _list path;
     error = option_of_yojson (value_for_key sensitive_error_of_yojson "error") _list path;
   }
...
let history_event_of_yojson tree path =
  ...
  ({
     evaluation_failed_event_details =
       option_of_yojson
         (value_for_key evaluation_failed_event_details_of_yojson "evaluationFailedEventDetails")
         _list path;
     map_run_redriven_event_details =
       option_of_yojson
         (value_for_key map_run_redriven_event_details_of_yojson "mapRunRedrivenEventDetails")
         _list path;
     map_run_failed_event_details =
       option_of_yojson
         (value_for_key map_run_failed_event_details_of_yojson "mapRunFailedEventDetails")
         _list path;
     map_run_started_event_details =
       option_of_yojson
         (value_for_key map_run_started_event_details_of_yojson "mapRunStartedEventDetails")
         _list path;
     state_exited_event_details =
       option_of_yojson
         (value_for_key state_exited_event_details_of_yojson "stateExitedEventDetails")
         _list path;
     state_entered_event_details =
       option_of_yojson
         (value_for_key state_entered_event_details_of_yojson "stateEnteredEventDetails")
         _list path;
     lambda_function_timed_out_event_details =
       option_of_yojson
     ...
     lambda_function_schedule_failed_event_details =
       option_of_yojson
         (value_for_key lambda_function_schedule_failed_event_details_of_yojson
     ...
     map_iteration_aborted_event_details =
       option_of_yojson
         (value_for_key map_iteration_event_details_of_yojson "mapIterationAbortedEventDetails")
         _list path;
     map_iteration_failed_event_details =
       option_of_yojson
         (value_for_key map_iteration_event_details_of_yojson "mapIterationFailedEventDetails")
         _list path;
     map_iteration_succeeded_event_details =
       option_of_yojson
         (value_for_key map_iteration_event_details_of_yojson "mapIterationSucceededEventDetails")
         _list path;
     map_iteration_started_event_details =
       option_of_yojson
         (value_for_key map_iteration_event_details_of_yojson "mapIterationStartedEventDetails")
         _list path;
     map_state_started_event_details =
       option_of_yojson
         (value_for_key map_state_started_event_details_of_yojson "mapStateStartedEventDetails")
         _list path;
     execution_redriven_event_details =
       option_of_yojson
         (value_for_key execution_redriven_event_details_of_yojson "executionRedrivenEventDetails")
         _list path;
     execution_timed_out_event_details =
       option_of_yojson
         (value_for_key execution_timed_out_event_details_of_yojson "executionTimedOutEventDetails")
         _list path;
     execution_aborted_event_details =
       option_of_yojson
         (value_for_key execution_aborted_event_details_of_yojson "executionAbortedEventDetails")
         _list path;
     execution_succeeded_event_details =
       option_of_yojson
         (value_for_key execution_succeeded_event_details_of_yojson "executionSucceededEventDetails")
         _list path;
     execution_started_event_details =
       option_of_yojson
         (value_for_key execution_started_event_details_of_yojson "executionStartedEventDetails")
         _list path;
     execution_failed_event_details =
       option_of_yojson
         (value_for_key execution_failed_event_details_of_yojson "executionFailedEventDetails")
         _list path;
     task_timed_out_event_details =
       option_of_yojson
         (value_for_key task_timed_out_event_details_of_yojson "taskTimedOutEventDetails")
         _list path;
     task_succeeded_event_details =
       option_of_yojson
         (value_for_key task_succeeded_event_details_of_yojson "taskSucceededEventDetails")
         _list path;
     task_submitted_event_details =
       option_of_yojson
         (value_for_key task_submitted_event_details_of_yojson "taskSubmittedEventDetails")
         _list path;
     task_submit_failed_event_details =
       option_of_yojson
         (value_for_key task_submit_failed_event_details_of_yojson "taskSubmitFailedEventDetails")
         _list path;
     task_started_event_details =
       option_of_yojson
         (value_for_key task_started_event_details_of_yojson "taskStartedEventDetails")
         _list path;
     task_start_failed_event_details =
       option_of_yojson
         (value_for_key task_start_failed_event_details_of_yojson "taskStartFailedEventDetails")
         _list path;
     task_scheduled_event_details =
       option_of_yojson
         (value_for_key task_scheduled_event_details_of_yojson "taskScheduledEventDetails")
         _list path;
     task_failed_event_details =
       option_of_yojson
         (value_for_key task_failed_event_details_of_yojson "taskFailedEventDetails")
         _list path;
     activity_timed_out_event_details =
       option_of_yojson
         (value_for_key activity_timed_out_event_details_of_yojson "activityTimedOutEventDetails")
         _list path;
     activity_succeeded_event_details =
       option_of_yojson
         (value_for_key activity_succeeded_event_details_of_yojson "activitySucceededEventDetails")
         _list path;
     activity_started_event_details =
       option_of_yojson
         (value_for_key activity_started_event_details_of_yojson "activityStartedEventDetails")
         _list path;
     activity_scheduled_event_details =
       option_of_yojson
         (value_for_key activity_scheduled_event_details_of_yojson "activityScheduledEventDetails")
         _list path;
     activity_schedule_failed_event_details =
       option_of_yojson
     ...
     activity_failed_event_details =
       option_of_yojson
         (value_for_key activity_failed_event_details_of_yojson "activityFailedEventDetails")
         _list path;
     previous_event_id =
       option_of_yojson (value_for_key event_id_of_yojson "previousEventId") _list path;
...
let get_execution_history_input_of_yojson tree path =
  ...
  ({
     include_execution_data =
       option_of_yojson
         (value_for_key include_execution_data_get_execution_history_of_yojson
...
let execution_redrive_status_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise (deserialize_unknown_enum_value_error path "ExecutionRedriveStatus" value)
    | _ -> raise (deserialize_wrong_type_error path "ExecutionRedriveStatus")
     : execution_redrive_status)
    : execution_redrive_status)
...
let describe_state_machine_alias_output_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     update_date = option_of_yojson (value_for_key timestamp_of_yojson "updateDate") _list path;
...
let describe_map_run_output_of_yojson tree path =
  ...
  ({
     ...
     redrive_count =
       option_of_yojson (value_for_key redrive_count_of_yojson "redriveCount") _list path;
     execution_counts =
       value_for_key map_run_execution_counts_of_yojson "executionCounts" _list path;
     item_counts = value_for_key map_run_item_counts_of_yojson "itemCounts" _list path;
     tolerated_failure_count =
       value_for_key tolerated_failure_count_of_yojson "toleratedFailureCount" _list path;
     tolerated_failure_percentage =
       value_for_key tolerated_failure_percentage_of_yojson "toleratedFailurePercentage" _list path;
...
let describe_execution_output_of_yojson tree path =
  ...
  ({
     redrive_status_reason =
       option_of_yojson (value_for_key sensitive_data_of_yojson "redriveStatusReason") _list path;
     redrive_status =
       option_of_yojson
...
let delete_activity_input_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({ activity_arn = value_for_key arn_of_yojson "activityArn" _list path } : delete_activity_input)
...
let create_state_machine_alias_input_of_yojson tree path =
  ...
  ({
     routing_configuration =
       value_for_key routing_configuration_list_of_yojson "routingConfiguration" _list path;
