...
[@@ocaml.doc
  "Describes potential issues found during state machine validation. Rather than raise an \
   exception, validation will return a list of {b diagnostic elements} containing diagnostic \
   information. \n\n\
  \  The \
   {{:https://docs.aws.amazon.com/step-functions/latest/apireference/API_ValidateStateMachineDefinition.html}ValidateStateMachineDefinitionlAPI} \
   might add new diagnostics in the future, adjust diagnostic codes, or change the message \
   wording. Your automated processes should only rely on the value of the {b result} field value \
   (OK, FAIL). Do {b not} rely on the exact order, count, or wording of diagnostic messages.\n\
  \  \n\
  \     {b List of warning codes} \n\
  \    \n\
  \      NO_DOLLAR  No [.$] on a field that appears to be a JSONPath or Intrinsic Function.\n\
  \                 \n\
  \                   NO_PATH  Field value looks like a path, but field name does not end with \
   'Path'.\n\
  \                            \n\
  \                              PASS_RESULT_IS_STATIC  Attempt to use a path in the result of a \
   pass state.\n\
  \                                                     \n\
  ...
  \                                                   \n\
  \                                                     INVALID_RESOURCE  The value of a \
   Task-state resource field is invalid.\n\
  \                                                                       \n\
  \                                                                         MISSING_END_STATE  \n\
   The workflow does not have a terminal state.\n\n\
  \  DUPLICATE_STATE_NAME  The same state name appears more than once.\n\
  \                        \n\
  \                          INVALID_STATE_NAME  The state name does not follow the naming \
   convention.\n\
  \                                              \n\
  \                                                STATE_MACHINE_NAME_EMPTY  The state machine \
  ...
  \                               \n\
  \                                 STATE_MACHINE_NAME_ALREADY_EXISTS  The state name already \
   exists.\n\
  \                                                                    \n\
  ...
  \                      \n\
  \                        MISSING_TRANSITION_TARGET  The value of \"Next\" field doesn't match a \
   known state name.\n\
  \                                                   \n\
  \                                                     TOO_DEEPLY_NESTED  The states are too \
   deeply nested.\n\
  \                                                                        \n\
  \                                                                          "]
...
type nonrec invalid_arn = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "The provided Amazon Resource Name (ARN) is not valid.\n"]

type nonrec conflict_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "Updating or deleting a resource can cause an inconsistent state. This error occurs when \
   there're concurrent requests for [DeleteStateMachineVersion], [PublishStateMachineVersion], or \
...
type nonrec missing_required_parameter = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "Request is missing a required parameter. This error occurs if both [definition] and [roleArn] \
   are not specified.\n"]

type nonrec kms_throttling_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "Received when KMS returns [ThrottlingException] for a KMS call that Step Functions makes on \
   behalf of the caller.\n"]

type nonrec kms_access_denied_exception = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "Either your KMS key policy or API caller does not have the required permissions.\n"]

type nonrec invalid_tracing_configuration = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "Your [tracingConfiguration] key does not match, or [enabled] has not been set to [true] or \
   [false].\n"]

type nonrec invalid_logging_configuration = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc "Configuration is not valid.\n"]

type nonrec invalid_encryption_configuration = { message : error_message option [@ocaml.doc ""] }
[@@ocaml.doc
  "Received when [encryptionConfiguration] is specified but various conditions exist which make \
   the configuration invalid. For example, if [type] is set to [CUSTOMER_MANAGED_KMS_KEY], but \
   [kmsKeyId] is null, or [kmsDataKeyReusePeriodSeconds] is not between 60 and 900, or the KMS key \
  ...
  inspection_level : inspection_level option;
      [@ocaml.doc
        ...
        \       {-   [TRACE]: Shows the HTTP request and response for an HTTP Task. This level \
         also shows the final state output along with the input and output data processing result.\n\
        \           \n\
        \            }\n\
        \       }\n\
...
type nonrec cloud_watch_events_execution_data_details = {
  included : included_details option;
      [@ocaml.doc
        "Indicates whether input or output was included in the response. Always [true] for API \
         calls. \n"]
}
...
type nonrec start_execution_input = {
  ...
  state_machine_arn : arn;
      [@ocaml.doc
        ...
        \ \n\
        \  {ul\n\
        \        {-   {b An unqualified state machine ARN} \226\128\147 Refers to a state machine \
         ARN that isn't qualified with a version or alias ARN. The following is an example of an \
        ...
        \              Step Functions doesn't associate state machine executions that you start \
         with an unqualified ARN with a version. This is true even if that version uses the same \
         revision that the execution used.\n\
        \              \n\
        \               }\n\
        \        {-   {b A state machine version ARN} \226\128\147 Refers to a version ARN, which \
         is a combination of state machine ARN and the version number separated by a colon (:). \
        ...
        \              \n\
        \               }\n\
        \        {-   {b A state machine alias ARN} \226\128\147 Refers to an alias ARN, which is \
         a combination of state machine ARN and the alias name separated by a colon (:). The \
         following is an example of the ARN for an alias named [PROD].\n\
        \            \n\
        \              \n\
        \             {[\n\
        \             arn::states:::stateMachine:\n\
  ...
  client_token : client_token option;
      [@ocaml.doc
        "A unique, case-sensitive identifier that you provide to ensure the idempotency of the \
         request. If you don\226\128\153t specify a client token, the Amazon Web Services SDK \
  ...
  revision_id : revision_id option;
      [@ocaml.doc
        ...
        \   To specify an initial revision ID for a state machine with no revision ID assigned, \
         specify the string [INITIAL] for the [revisionId] parameter. For example, you can specify \
         a [revisionID] of [INITIAL] when you create a state machine using the \
  ...
  included_data : included_data option;
      [@ocaml.doc
        ...
        \   When calling a labelled ARN for an encrypted state machine, the [includedData = \
         METADATA_ONLY] parameter will not apply because Step Functions needs to decrypt the \
         entire state machine definition to get the Distributed Map state\226\128\153s definition. \
  ...
  redrive_status_reason : sensitive_data option;
      [@ocaml.doc
        "When [redriveStatus] is [NOT_REDRIVABLE], [redriveStatusReason] specifies the reason why \
         an execution cannot be redriven.\n\n\
        \ {ul\n\
        ...
        \       {-  For a Distributed Map that includes child workflows of type [EXPRESS], \
         [redriveStatusReason] is only returned if the child workflows are not redrivable. This \
...
type nonrec describe_activity_input = {
  activity_arn : arn; [@ocaml.doc "The Amazon Resource Name (ARN) of the activity to describe.\n"]
}
[@@ocaml.doc ""]

type nonrec delete_state_machine_version_output = unit [@@ocaml.doc ""]

type nonrec delete_state_machine_version_input = {
  state_machine_version_arn : long_arn;
      [@ocaml.doc "The Amazon Resource Name (ARN) of the state machine version to delete.\n"]
}
[@@ocaml.doc ""]
...
type nonrec delete_activity_output = unit [@@ocaml.doc ""]

type nonrec delete_activity_input = {
  activity_arn : arn; [@ocaml.doc "The Amazon Resource Name (ARN) of the activity to delete.\n"]
}
[@@ocaml.doc ""]
