...
let state_machine_type_to_yojson (x : state_machine_type) =
  match x with EXPRESS -> `String "EXPRESS" | STANDARD -> `String "STANDARD"
...
let update_state_machine_alias_output_to_yojson (x : update_state_machine_alias_output) =
  assoc_to_yojson [ ("updateDate", Some (timestamp__to_yojson x.update_date)) ]
...
let inspection_level_to_yojson (x : inspection_level) =
  match x with TRACE -> `String "TRACE" | DEBUG -> `String "DEBUG" | INFO -> `String "INFO"
...
let included_data_to_yojson (x : included_data) =
  match x with METADATA_ONLY -> `String "METADATA_ONLY" | ALL_DATA -> `String "ALL_DATA"
...
let redrive_execution_output_to_yojson (x : redrive_execution_output) =
  assoc_to_yojson [ ("redriveDate", Some (timestamp__to_yojson x.redrive_date)) ]
...
let execution_redrive_filter_to_yojson (x : execution_redrive_filter) =
  match x with NOT_REDRIVEN -> `String "NOT_REDRIVEN" | REDRIVEN -> `String "REDRIVEN"
