...
val make_routing_configuration_list_item :
  weight:version_weight -> state_machine_version_arn:arn -> unit -> routing_configuration_list_item
...
val make_send_task_success_input :
  output:sensitive_data -> task_token:task_token -> unit -> send_task_success_input
...
val make_map_run_redriven_event_details :
  ?redrive_count:redrive_count -> ?map_run_arn:long_arn -> unit -> map_run_redriven_event_details
...
val make_map_run_failed_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> map_run_failed_event_details
...
val make_map_iteration_event_details :
  ?index:unsigned_integer -> ?name:name -> unit -> map_iteration_event_details
...
val make_list_state_machines_input :
  ?next_token:page_token -> ?max_results:page_size -> unit -> list_state_machines_input
...
val make_activity_list_item :
  creation_date:timestamp -> name:name -> activity_arn:arn -> unit -> activity_list_item
...
val make_list_activities_input :
  ?next_token:page_token -> ?max_results:page_size -> unit -> list_activities_input
...
val make_lambda_function_failed_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> lambda_function_failed_event_details

val make_activity_failed_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> activity_failed_event_details

val make_activity_schedule_failed_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> activity_schedule_failed_event_details
...
val make_activity_timed_out_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> activity_timed_out_event_details

val make_execution_failed_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> execution_failed_event_details
...
val make_execution_aborted_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> execution_aborted_event_details

val make_execution_timed_out_event_details :
  ?cause:sensitive_cause -> ?error:sensitive_error -> unit -> execution_timed_out_event_details
...
val make_get_activity_task_output :
  ?input:sensitive_data_job_input -> ?task_token:task_token -> unit -> get_activity_task_output
...
val make_create_activity_output :
  creation_date:timestamp -> activity_arn:arn -> unit -> create_activity_output
...
[@@ocaml.doc
  "Creates an activity. An activity is a task that you write in any programming language and host \
   on any machine that has access to Step Functions. Activities must poll Step Functions using the \
   [GetActivityTask] API action and respond using [SendTask*] API actions. This function lets Step \
   Functions know the existence of your activity and returns an identifier for use in a state \
   machine and when polling from the activity.\n\n\
  \  This operation is eventually consistent. The results are best effort and may not reflect very \
   recent updates and changes.\n\
  \  \n\
  \      [CreateActivity] is an idempotent API. Subsequent requests won\226\128\153t create a \
   duplicate resource if it was already created. [CreateActivity]'s idempotency check is based on \
   the activity [name]. If a following request has different [tags] values, Step Functions will \
   ignore these differences and treat it as an idempotent request of the previous. In this case, \
...
[@@ocaml.doc
  ...
  \   For additional control over security, you can encrypt your data using a {b customer-managed \
   ...
   reuse period when creating or updating a {b State Machine}. The execution history and state \
   machine definition will be encrypted with the key applied to the State Machine. \n\
  \  \n\
  \    This operation is eventually consistent. The results are best effort and may not reflect \
   very recent updates and changes.\n\
  \    \n\
  \        [CreateStateMachine] is an idempotent API. Subsequent requests won\226\128\153t create \
   ...
   and [versionDescription] parameters. If a following request has a different [roleArn] or \
   [tags], Step Functions will ignore these differences and treat it as an idempotent request of \
   the previous. In this case, [roleArn] and [tags] will not be updated, even if they are \
   different.\n\
  \       \n\
  \        "]
...
[@@ocaml.doc
  "Creates an \
   ...
   of the same state machine. You can set your application to call [StartExecution] with an alias \
   and update the version the alias uses without changing the client's code.\n\n\
  ...
  \ \n\
  \  To create an alias that points to a single version, specify a single [RoutingConfig] object \
   with a [weight] set to 100.\n\
  \  \n\
...
[@@ocaml.doc
  "Deletes a state machine. This is an asynchronous operation. It sets the state machine's status \
   to [DELETING] and begins the deletion process. A state machine is deleted only when all its \
   executions are completed. On the next state transition, the state machine's executions are \
   terminated.\n\n\
  \ A qualified state machine ARN can either refer to a {i Distributed Map state} defined within a \
   state machine, a version ARN, or an alias ARN.\n\
  \ \n\
  \  The following are some examples of qualified and unqualified state machine ARNs:\n\
  \  \n\
  \   {ul\n\
  \         {-  The following qualified state machine ARN refers to a {i Distributed Map state} \
   with a label [mapStateLabel] in a state machine named [myStateMachine].\n\
  \             \n\
  \               \
   [arn:partition:states:region:account-id:stateMachine:myStateMachine/mapStateLabel] \n\
  \              \n\
  \                If you provide a qualified state machine ARN that refers to a {i Distributed \
  ...
  \   \n\
  \     For [EXPRESS] state machines, the deletion happens eventually (usually in less than a \
   minute). Running executions may emit logs after [DeleteStateMachine] API is called.\n\
  \     \n\
  \      "]
...
[@@ocaml.doc
  "Deletes a state machine \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/concepts-state-machine-alias.html}alias}.\n\n\
  \ After you delete a state machine alias, you can't use it to start executions. When you delete \
   a state machine alias, Step Functions doesn't delete the state machine versions that alias \
   references.\n\
  \ \n\
  \   {b Related operations:} \n\
  \  \n\
...
[@@ocaml.doc
  "Deletes a state machine \
   ...
   After you delete a version, you can't call [StartExecution] using that version's ARN or use the \
  ...
  \     You can't delete a state machine version currently referenced by one or more aliases. \
   Before you delete a version, you must either delete the aliases or update them to point to \
   another state machine version.\n\
  \     \n\
  \        {b Related operations:} \n\
  \       \n\
  \        {ul\n\
...
[@@ocaml.doc
  "Provides information about a state machine execution, such as the state machine associated with \
   the execution, the execution input and output, and relevant execution metadata. If you've \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/redrive-executions.html}redriven} an \
   execution, you can use this API action to return information about the redrives of that \
   execution. In addition, you can use this API action to return the Map Run Amazon Resource Name \
   (ARN) if the execution was dispatched by a Map Run.\n\n\
...
[@@ocaml.doc
  "Provides information about a Map Run's configuration, progress, and results. If you've \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/redrive-map-run.html}redriven} a Map \
   Run, this API action also returns information about the redrives of that Map Run. For more \
   information, see \
...
[@@ocaml.doc
  "Provides information about a state machine's definition, its execution role ARN, and \
   ...
   Resource Name (ARN) in the response. The state machine returned is the state machine associated \
   with the Map Run.\n\n\
  \  This operation is eventually consistent. The results are best effort and may not reflect very \
   recent updates and changes.\n\
  \  \n\
...
[@@ocaml.doc
  "Used by workers to retrieve a task (with the specified activity ARN) which has been scheduled \
   for execution by a running state machine. This initiates a long poll, where the service holds \
   the HTTP connection open and responds as soon as a task becomes available (i.e. an execution of \
   a task of this type is needed.) The maximum time the service holds on to the request before \
   responding is 60 seconds. If no task is available within 60 seconds, the poll returns a \
  ...
  \      Polling with [GetActivityTask] can cause latency in some implementations. See \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/bp-activity-pollers.html}Avoid Latency \
   When Polling for Activity Tasks} in the Step Functions Developer Guide.\n\
  \      \n\
  \       "]
...
[@@ocaml.doc
  "Returns the history of the specified execution as a list of events. By default, the results are \
   returned in ascending order of the [timeStamp] of the events. Use the [reverseOrder] parameter \
...
[@@ocaml.doc
  "Lists all executions of a state machine or a Map Run. You can list all executions related to a \
   state machine by specifying a state machine Amazon Resource Name (ARN), or those related to a \
   Map Run by specifying a Map Run ARN. Using this API action, you can also list all \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/redrive-executions.html}redriven} \
   executions.\n\n\
...
[@@ocaml.doc
  "Lists \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/concepts-state-machine-alias.html}aliases} \
   for a specified state machine ARN. Results are sorted by time, with the most recently created \
   aliases listed first. \n\n\
...
[@@ocaml.doc
  "Creates a \
   ...
   your state machine. You can start executions from versions either directly or with an alias. To \
   create an alias, use [CreateStateMachineAlias].\n\n\
  \ You can publish up to 1000 versions for each state machine. You must manually delete unused \
   versions using the [DeleteStateMachineVersion] API action.\n\
  \ \n\
  \   [PublishStateMachineVersion] is an idempotent API. It doesn't create a duplicate state \
   machine version if it already exists for the current revision. Step Functions bases \
   [PublishStateMachineVersion]'s idempotency check on the [stateMachineArn], [name], and \
   [revisionId] parameters. Requests with the same parameters return a successful idempotent \
   response. If you don't specify a [revisionId], Step Functions checks for a previously published \
...
[@@ocaml.doc
  "Restarts unsuccessful executions of Standard workflows that didn't complete successfully in the \
   last 14 days. These include failed, aborted, or timed out executions. When you \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/redrive-executions.html}redrive} an \
   execution, it continues the failed execution from the unsuccessful step and uses the same \
   input. Step Functions preserves the results and execution history of the successful steps, and \
   doesn't rerun these steps when you redrive an execution. Redriven executions use the same state \
   machine definition and execution ARN as the original execution attempt.\n\n\
  \ For workflows that include an \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-map-state.html}Inline \
   Map} or \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-parallel-state.html}Parallel} \
   state, [RedriveExecution] API action reschedules and redrives only the iterations and branches \
   that failed or aborted.\n\
  \ \n\
  \  To redrive a workflow that includes a Distributed Map state whose Map Run failed, you must \
   redrive the \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/use-dist-map-orchestrate-large-scale-parallel-workloads.html#dist-map-orchestrate-parallel-workloads-key-terms}parent \
   workflow}. The parent workflow redrives all the unsuccessful states, including a failed Map \
   Run. If a Map Run was not started in the original execution attempt, the redriven parent \
   workflow starts the Map Run.\n\
  \  \n\
  \    This API action is not supported by [EXPRESS] state machines.\n\
  \    \n\
  \     However, you can restart the unsuccessful executions of Express child workflows in a \
   Distributed Map by redriving its Map Run. When you redrive a Map Run, the Express child \
   workflows are rerun using the [StartExecution] API action. For more information, see \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/redrive-map-run.html}Redriving Map \
   Runs}.\n\
  \     \n\
  ...
  \              {-  Your workflow execution has not exceeded the redrivable period of 14 days. \
   Redrivable period refers to the time during which you can redrive a given execution. This \
   period starts from the day a state machine completes its execution.\n\
  \                  \n\
  \                   }\n\
  \              {-  The workflow execution has not exceeded the maximum open time of one year. \
   For more information about state machine quotas, see \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/limits-overview.html#service-limits-state-machine-executions}Quotas \
   related to state machine executions}.\n\
  \                  \n\
  \                   }\n\
  \              {-  The execution event history count is less than 24,999. Redriven executions \
   append their event history to the existing event history. Make sure your workflow execution \
   contains less than 24,999 events to accommodate the [ExecutionRedriven] history event and at \
   least one other history event.\n\
  \                  \n\
  \                   }\n\
  \              }\n\
  \  "]
...
[@@ocaml.doc
  "Used by activity workers, Task states using the \
   ...
   pattern, and optionally Task states using the \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-sync}job \
   run} pattern to report that the task identified by the [taskToken] failed.\n\n\
  ...
  \  A caller can mark a task as fail without using any KMS permissions in the execution role if \
   the caller provides a null value for both [error] and [cause] fields because no data needs to \
   be encrypted.\n\
  \  "]
...
[@@ocaml.doc
  "Used by activity workers and Task states using the \
   ...
   pattern, and optionally Task states using the \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-sync}job \
   run} pattern to report to Step Functions that the task represented by the specified [taskToken] \
   is still making progress. This action resets the [Heartbeat] clock. The [Heartbeat] threshold \
   is specified in the state machine's Amazon States Language definition ([HeartbeatSeconds]). \
   This action does not in itself create an event in the execution history. However, if the task \
   times out, the execution history contains an [ActivityTimedOut] entry for activities, or a \
   [TaskTimedOut] entry for tasks using the \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-sync}job \
   run} or \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-resource.html#connect-wait-token}callback} \
   pattern.\n\n\
  \  The [Timeout] of a task, defined in the state machine's Amazon States Language definition, is \
   its maximum allowed duration, regardless of the number of [SendTaskHeartbeat] requests \
   received. Use [HeartbeatSeconds] to configure the timeout interval for heartbeats.\n\
  \  \n\
  \   "]
...
[@@ocaml.doc
  ...
  \    \n\
  \       [StartExecution] is idempotent for [STANDARD] workflows. For a [STANDARD] workflow, if \
   you call [StartExecution] with the same name and input as a running execution, the call \
   succeeds and return the same response as the original request. If the execution is closed or if \
   the input is different, it returns a [400 ExecutionAlreadyExists] error. You can reuse names \
   after 90 days. \n\
  \      \n\
...
[@@ocaml.doc
  ...
  \   [StartSyncExecution] will return a [200 OK] response, even if your execution fails, because \
   ...
   for errors that prevent your execution from running, such as permissions errors, limit errors, \
   or issues with your state machine code and configuration. \n\
  \  \n\
  \     This API action isn't logged in CloudTrail.\n\
  \     \n\
  \      "]
...
[@@ocaml.doc
  ...
  \ An array of key-value pairs. For more information, see \
   ...
   Allocation Tags} in the {i Amazon Web Services Billing and Cost Management User Guide}, and \
   {{:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_iam-tags.html}Controlling Access \
   Using IAM Tags}.\n\
  \ \n\
  \  Tags may only contain Unicode letters, digits, white space, or these symbols: [_ . : / = + - \
   @].\n\
...
[@@ocaml.doc
  "Accepts the definition of a single state and executes it. You can test a state without creating \
   a state machine or updating an existing state machine. Using this API, you can test the \
   following:\n\n\
  \ {ul\n\
  \       {-  A state's \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/test-state-isolation.html#test-state-input-output-dataflow}input \
   and output processing} data flow\n\
  \           \n\
  \            }\n\
  \       {-  An \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/connect-to-services.html}Amazon Web \
  ...
  \    {ul\n\
  \          {-   \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-task-state.html#task-types}All \
   Task types} except \
  ...
  \               }\n\
  \          }\n\
  \   The [TestState] API assumes an IAM role which must contain the required IAM permissions for \
  ...
  \      [TestState] doesn't support \
   ...
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-parallel-state.html}Parallel}, \
   or \
   {{:https://docs.aws.amazon.com/step-functions/latest/dg/amazon-states-language-map-state.html}Map} \
   states.\n\
  \     "]
...
[@@ocaml.doc
  "Updates an existing state machine by modifying its [definition], [roleArn], \
   ...
   the previous [definition] and [roleArn]. You must include at least one of [definition] or \
   [roleArn] or you will receive a [MissingRequiredParameter] error.\n\n\
  \ A qualified state machine ARN refers to a {i Distributed Map state} defined within a state \
   machine. For example, the qualified state machine ARN \
   [arn:partition:states:region:account-id:stateMachine:stateMachineName/mapStateLabel] refers to \
   a {i Distributed Map state} with a label [mapStateLabel] in the state machine named \
   [stateMachineName].\n\
  \ \n\
  \  A qualified state machine ARN can either refer to a {i Distributed Map state} defined within \
...
[@@ocaml.doc
  ...
  \ \n\
  \    [UpdateStateMachineAlias] is an idempotent API. Step Functions bases the idempotency check \
   on the [stateMachineAliasArn], [description], and [routingConfiguration] parameters. Requests \
  ...
  \      This operation is eventually consistent. All [StartExecution] requests made within a few \
   seconds use the latest alias configuration. Executions started immediately after calling \
   [UpdateStateMachineAlias] may use the previous routing configuration.\n\
  \      \n\
  \         {b Related operations:} \n\
  \        \n\
  \         {ul\n\
...
[@@ocaml.doc
  ...
  \  \n\
  \   {ul\n\
  \         {-  Integrate automated checks into your code review or Continuous Integration (CI) \
   process to check state machine definitions before starting deployments.\n\
  \             \n\
  \              }\n\
  ...
  \              }\n\
  \         }\n\
  \   Validation will look for problems in your state machine definition and return a {b result} \
   and a list of {b diagnostic elements}.\n\
  \   \n\
  \    The {b result} value will be [OK] when your workflow definition can be successfully created \
   or updated. Note the result can be [OK] even when diagnostic warnings are present in the \
   response. The {b result} value will be [FAIL] when the workflow definition contains errors that \
   would prevent you from creating or updating your state machine. \n\
  \    \n\
  \     The list of \
   {{:https://docs.aws.amazon.com/step-functions/latest/apireference/API_ValidateStateMachineDefinitionDiagnostic.html}ValidateStateMachineDefinitionDiagnostic} \
   data elements can contain zero or more {b WARNING} and/or {b ERROR} elements.\n\
  \     \n\
  \       The {b ValidateStateMachineDefinition API} might add new diagnostics in the future, \
   adjust diagnostic codes, or change the message wording. Your automated processes should only \
   rely on the value of the {b result} field value (OK, FAIL). Do {b not} rely on the exact order, \
