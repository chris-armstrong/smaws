  ...
  let _res : validate_state_machine_definition_diagnostic =
    {
      location =
        (option_of_yojson
           (value_for_key
              validate_state_machine_definition_location_of_yojson "location")
           _list path);
      message =
        (value_for_key validate_state_machine_definition_message_of_yojson
           "message" _list path);
      code =
        (value_for_key validate_state_machine_definition_code_of_yojson
           "code" _list path);
      severity =
  ...
  let _res : validate_state_machine_definition_output =
    {
      ...
      result =
        (value_for_key
           validate_state_machine_definition_result_code_of_yojson "result"
           _list path)
    } in
  _res
  ...
  let _res : update_state_machine_output =
    {
      ...
      revision_id =
        (option_of_yojson (value_for_key revision_id_of_yojson "revisionId")
           _list path);
      update_date =
        (value_for_key timestamp__of_yojson "updateDate" _list path)
    } in
  _res
  ...
  let _res : logging_configuration =
    {
      destinations =
        (option_of_yojson
           (value_for_key log_destination_list_of_yojson "destinations")
           _list path);
  ...
  let _res : inspection_data_request =
    {
      ...
      headers =
        (option_of_yojson (value_for_key http_headers_of_yojson "headers")
           _list path);
      url =
        (option_of_yojson (value_for_key ur_l_of_yojson "url") _list path);
      method_ =
  ...
  let _res : inspection_data_response =
    {
      ...
      status_message =
        (option_of_yojson
           (value_for_key http_status_message_of_yojson "statusMessage")
           _list path);
      status_code =
        (option_of_yojson
  ...
  let _res : inspection_data =
    {
      response =
        (option_of_yojson
           (value_for_key inspection_data_response_of_yojson "response")
           _list path);
      request =
        (option_of_yojson
  ...
  let _res : test_state_output =
    {
      status =
        (option_of_yojson
           (value_for_key test_execution_status_of_yojson "status") _list
           path);
      next_state =
  ...
  let _res : test_state_input =
    {
      reveal_secrets =
        (option_of_yojson
           (value_for_key reveal_secrets_of_yojson "revealSecrets") _list
           path);
      inspection_level =
        (option_of_yojson
  ...
  let _res : task_timed_out_event_details =
    {
      ...
      resource_type =
        (value_for_key name_of_yojson "resourceType" _list path)
    } in
  _res
  ...
  let _res : start_sync_execution_output =
    {
      ...
      stop_date = (value_for_key timestamp__of_yojson "stopDate" _list path);
      start_date =
        (value_for_key timestamp__of_yojson "startDate" _list path);
      name =
...
let redrive_execution_output_of_yojson tree path =
  ...
  let _res : redrive_execution_output =
    {
      redrive_date =
        (value_for_key timestamp__of_yojson "redriveDate" _list path)
    } in
  _res
...
let map_run_status_of_yojson (tree : t) path =
  (match tree with
   ...
   | `String value ->
       raise (deserialize_unknown_enum_value_error path "MapRunStatus" value)
...
let map_run_started_event_details_of_yojson tree path =
  ...
  let _res : map_run_started_event_details =
    {
      map_run_arn =
        (option_of_yojson (value_for_key long_arn_of_yojson "mapRunArn")
           _list path)
    } in
  _res
...
let map_run_list_item_of_yojson tree path =
  ...
  let _res : map_run_list_item =
    {
      stop_date =
        (option_of_yojson (value_for_key timestamp__of_yojson "stopDate")
           _list path);
      start_date =
        (value_for_key timestamp__of_yojson "startDate" _list path);
      state_machine_arn =
  ...
  let _res : map_run_item_counts =
    {
      ...
      failures_not_redrivable =
        (option_of_yojson
           (value_for_key long_object_of_yojson "failuresNotRedrivable")
           _list path);
      results_written =
  ...
  let _res : list_state_machines_output =
    {
      next_token =
        (option_of_yojson (value_for_key page_token_of_yojson "nextToken")
           _list path);
      state_machines =
        (value_for_key state_machine_list_of_yojson "stateMachines" _list
           path)
    } in
  _res
...
let execution_list_item_of_yojson tree path =
  ...
  let _res : execution_list_item =
    {
      redrive_date =
        (option_of_yojson (value_for_key timestamp__of_yojson "redriveDate")
           _list path);
      redrive_count =
        (option_of_yojson
           (value_for_key redrive_count_of_yojson "redriveCount") _list path);
  ...
  let _res : history_event =
    {
      ...
      lambda_function_start_failed_event_details =
        (option_of_yojson
           (value_for_key
              lambda_function_start_failed_event_details_of_yojson
      ...
      previous_event_id =
        (option_of_yojson
           (value_for_key event_id_of_yojson "previousEventId") _list path);
      id = (value_for_key event_id_of_yojson "id" _list path);
      type_ = (value_for_key history_event_type_of_yojson "type" _list path);
      timestamp_ =
        (value_for_key timestamp__of_yojson "timestamp" _list path)
    } in
  _res
