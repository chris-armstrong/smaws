...
let make_validate_state_machine_definition_diagnostic
  ...
  ({ location = location_; message = message_; code = code_; severity = severity_ }
...
let make_validate_state_machine_definition_input
    ...
    ?type_:(type__ : state_machine_type option) ~definition:(definition_ : definition) () =
  ({ max_results = max_results_; severity = severity_; type_ = type__; definition = definition_ }
...
let make_update_state_machine_output
    ...
    ?revision_id:(revision_id_ : revision_id option) ~update_date:(update_date_ : timestamp) () =
  ({
...
let make_logging_configuration ?destinations:(destinations_ : log_destination_list option)
...
let make_encryption_configuration
    ...
    ?kms_key_id:(kms_key_id_ : kms_key_id option) ~type_:(type__ : encryption_type) () =
  ({
     type_ = type__;
...
let make_update_state_machine_input
    ...
    ?role_arn:(role_arn_ : arn option) ?definition:(definition_ : definition option)
...
let make_inspection_data_request ?body:(body_ : http_body option)
    ...
    ?method_:(method__ : http_method option) ?protocol:(protocol_ : http_protocol option) () =
  ({ body = body_; headers = headers_; url = url_; method_ = method__; protocol = protocol_ }
...
let make_test_state_output ?status:(status_ : test_execution_status option)
    ...
    ?cause:(cause_ : sensitive_cause option) ?error:(error_ : sensitive_error option)
...
let make_task_timed_out_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ~resource:(resource_ : name)
    ~resource_type:(resource_type_ : name) () =
  ({ cause = cause_; error = error_; resource = resource_; resource_type = resource_type_ }
...
let make_task_submit_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ~resource:(resource_ : name)
    ~resource_type:(resource_type_ : name) () =
  ({ cause = cause_; error = error_; resource = resource_; resource_type = resource_type_ }
...
let make_task_start_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ~resource:(resource_ : name)
    ~resource_type:(resource_type_ : name) () =
  ({ cause = cause_; error = error_; resource = resource_; resource_type = resource_type_ }
...
let make_stop_execution_input ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ~execution_arn:(execution_arn_ : arn) () =
  ({ cause = cause_; error = error_; execution_arn = execution_arn_ } : stop_execution_input)

let make_state_machine_version_list_item ~creation_date:(creation_date_ : timestamp)
...
let make_state_machine_alias_list_item ~creation_date:(creation_date_ : timestamp)
    ~state_machine_alias_arn:(state_machine_alias_arn_ : long_arn) () =
  ({ creation_date = creation_date_; state_machine_alias_arn = state_machine_alias_arn_ }
...
let make_state_entered_event_details
  ...
  ({ input_details = input_details_; input = input_; name = name_ } : state_entered_event_details)
...
let make_start_sync_execution_output ?billing_details:(billing_details_ : billing_details option)
    ...
    ?input:(input_ : sensitive_data option) ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ?name:(name_ : name option)
    ?state_machine_arn:(state_machine_arn_ : arn option) ~status:(status_ : sync_execution_status)
...
let make_start_sync_execution_input ?included_data:(included_data_ : included_data option)
    ?trace_header:(trace_header_ : trace_header option) ?input:(input_ : sensitive_data option)
    ?name:(name_ : name option) ~state_machine_arn:(state_machine_arn_ : arn) () =
  ({
...
let make_start_execution_input ?trace_header:(trace_header_ : trace_header option)
...
let make_send_task_failure_input ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) ~task_token:(task_token_ : task_token) () =
...
let make_redrive_execution_input ?client_token:(client_token_ : client_token option)
    ~execution_arn:(execution_arn_ : arn) () =
  ({ client_token = client_token_; execution_arn = execution_arn_ } : redrive_execution_input)
...
let make_publish_state_machine_version_input
    ...
    ?revision_id:(revision_id_ : revision_id option) ~state_machine_arn:(state_machine_arn_ : arn)
    () =
  ({
     description = description_;
     revision_id = revision_id_;
...
let make_map_run_redriven_event_details ?redrive_count:(redrive_count_ : redrive_count option)
...
let make_map_run_list_item ?stop_date:(stop_date_ : timestamp option)
    ~start_date:(start_date_ : timestamp) ~state_machine_arn:(state_machine_arn_ : arn)
    ~map_run_arn:(map_run_arn_ : long_arn) ~execution_arn:(execution_arn_ : arn) () =
  ({
     stop_date = stop_date_;
     start_date = start_date_;
     state_machine_arn = state_machine_arn_;
...
let make_map_run_item_counts ?pending_redrive:(pending_redrive_ : long_object option)
    ?failures_not_redrivable:(failures_not_redrivable_ : long_object option)
    ~results_written:(results_written_ : unsigned_long) ~total:(total_ : unsigned_long)
    ~aborted:(aborted_ : unsigned_long) ~timed_out:(timed_out_ : unsigned_long)
    ~failed:(failed_ : unsigned_long) ~succeeded:(succeeded_ : unsigned_long)
    ~running:(running_ : unsigned_long) ~pending:(pending_ : unsigned_long) () =
  ({
...
let make_map_run_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : map_run_failed_event_details)

let make_map_run_execution_counts ?pending_redrive:(pending_redrive_ : long_object option)
    ?failures_not_redrivable:(failures_not_redrivable_ : long_object option)
    ~results_written:(results_written_ : unsigned_long) ~total:(total_ : unsigned_long)
    ~aborted:(aborted_ : unsigned_long) ~timed_out:(timed_out_ : unsigned_long)
    ~failed:(failed_ : unsigned_long) ~succeeded:(succeeded_ : unsigned_long)
...
let make_map_iteration_event_details ?index:(index_ : unsigned_integer option)
...
let make_list_state_machines_input ?next_token:(next_token_ : page_token option)
...
let make_list_state_machine_versions_output ?next_token:(next_token_ : page_token option)
  ...
  ({ next_token = next_token_; state_machine_versions = state_machine_versions_ }
    : list_state_machine_versions_output)

let make_list_state_machine_versions_input ?max_results:(max_results_ : page_size option)
    ?next_token:(next_token_ : page_token option) ~state_machine_arn:(state_machine_arn_ : arn) () =
  ({ max_results = max_results_; next_token = next_token_; state_machine_arn = state_machine_arn_ }
...
let make_list_state_machine_aliases_input ?max_results:(max_results_ : page_size option)
    ?next_token:(next_token_ : page_token option) ~state_machine_arn:(state_machine_arn_ : arn) () =
  ({ max_results = max_results_; next_token = next_token_; state_machine_arn = state_machine_arn_ }
...
let make_list_map_runs_input ?next_token:(next_token_ : page_token option)
    ?max_results:(max_results_ : page_size option) ~execution_arn:(execution_arn_ : arn) () =
  ({ next_token = next_token_; max_results = max_results_; execution_arn = execution_arn_ }
    : list_map_runs_input)
...
let make_execution_list_item ?redrive_date:(redrive_date_ : timestamp option)
    ...
    ?map_run_arn:(map_run_arn_ : long_arn option) ?stop_date:(stop_date_ : timestamp option)
    ~start_date:(start_date_ : timestamp) ~status:(status_ : execution_status) ~name:(name_ : name)
    ~state_machine_arn:(state_machine_arn_ : arn) ~execution_arn:(execution_arn_ : arn) () =
  ({
...
let make_list_executions_input ?redrive_filter:(redrive_filter_ : execution_redrive_filter option)
    ?map_run_arn:(map_run_arn_ : long_arn option)
...
let make_activity_list_item ~creation_date:(creation_date_ : timestamp) ~name:(name_ : name)
    ~activity_arn:(activity_arn_ : arn) () =
  ({ creation_date = creation_date_; name = name_; activity_arn = activity_arn_ }
    : activity_list_item)
...
let make_lambda_function_timed_out_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
...
let make_lambda_function_start_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
...
let make_lambda_function_schedule_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : lambda_function_schedule_failed_event_details)

let make_lambda_function_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : lambda_function_failed_event_details)

let make_activity_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : activity_failed_event_details)

let make_activity_schedule_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
...
let make_activity_timed_out_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : activity_timed_out_event_details)

let make_execution_failed_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
...
let make_execution_aborted_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
  ({ cause = cause_; error = error_ } : execution_aborted_event_details)

let make_execution_timed_out_event_details ?cause:(cause_ : sensitive_cause option)
    ?error:(error_ : sensitive_error option) () =
...
let make_evaluation_failed_event_details ?location:(location_ : evaluation_failure_location option)
    ?cause:(cause_ : sensitive_cause option) ?error:(error_ : sensitive_error option)
    ~state:(state_ : state_name) () =
...
let make_history_event
    ...
    ?previous_event_id:(previous_event_id_ : event_id option) ~id:(id_ : event_id)
    ~type_:(type__ : history_event_type) ~timestamp:(timestamp_ : timestamp) () =
  ({
...
let make_get_activity_task_output ?input:(input_ : sensitive_data_job_input option)
...
let make_describe_state_machine_output
    ...
    ?encryption_configuration:(encryption_configuration_ : encryption_configuration option)
    ?description:(description_ : version_description option)
    ?revision_id:(revision_id_ : revision_id option) ?label:(label_ : map_run_label option)
    ?tracing_configuration:(tracing_configuration_ : tracing_configuration option)
    ?logging_configuration:(logging_configuration_ : logging_configuration option)
    ?status:(status_ : state_machine_status option) ~creation_date:(creation_date_ : timestamp)
...
let make_describe_state_machine_alias_output ?update_date:(update_date_ : timestamp option)
    ?creation_date:(creation_date_ : timestamp option)
    ?routing_configuration:(routing_configuration_ : routing_configuration_list option)
    ?description:(description_ : alias_description option) ?name:(name_ : name option)
    ?state_machine_alias_arn:(state_machine_alias_arn_ : arn option) () =
  ({
     update_date = update_date_;
     creation_date = creation_date_;
...
let make_describe_map_run_output ?redrive_date:(redrive_date_ : timestamp option)
    ?redrive_count:(redrive_count_ : redrive_count option)
    ...
    ~max_concurrency:(max_concurrency_ : max_concurrency) ~start_date:(start_date_ : timestamp)
    ~status:(status_ : map_run_status) ~execution_arn:(execution_arn_ : arn)
    ~map_run_arn:(map_run_arn_ : long_arn) () =
  ({
     redrive_date = redrive_date_;
     redrive_count = redrive_count_;
...
let make_describe_execution_output
    ...
    ?output:(output_ : sensitive_data option)
    ?input_details:(input_details_ : cloud_watch_events_execution_data_details option)
    ?input:(input_ : sensitive_data option) ?stop_date:(stop_date_ : timestamp option)
    ?name:(name_ : name option) ~start_date:(start_date_ : timestamp)
    ~status:(status_ : execution_status) ~state_machine_arn:(state_machine_arn_ : arn)
    ~execution_arn:(execution_arn_ : arn) () =
  ({
...
let make_describe_execution_input ?included_data:(included_data_ : included_data option)
    ~execution_arn:(execution_arn_ : arn) () =
  ({ included_data = included_data_; execution_arn = execution_arn_ } : describe_execution_input)
...
let make_create_state_machine_output
    ?state_machine_version_arn:(state_machine_version_arn_ : arn option)
    ~creation_date:(creation_date_ : timestamp) ~state_machine_arn:(state_machine_arn_ : arn) () =
  ({
     state_machine_version_arn = state_machine_version_arn_;
     creation_date = creation_date_;
     state_machine_arn = state_machine_arn_;
...
let make_create_state_machine_alias_output ~creation_date:(creation_date_ : timestamp)
    ~state_machine_alias_arn:(state_machine_alias_arn_ : arn) () =
  ({ creation_date = creation_date_; state_machine_alias_arn = state_machine_alias_arn_ }
...
let make_create_state_machine_alias_input ?description:(description_ : alias_description option)
  ...
  ({ routing_configuration = routing_configuration_; name = name_; description = description_ }
    : create_state_machine_alias_input)

let make_create_activity_output ~creation_date:(creation_date_ : timestamp)
    ~activity_arn:(activity_arn_ : arn) () =
  ({ creation_date = creation_date_; activity_arn = activity_arn_ } : create_activity_output)

let make_create_activity_input
    ?encryption_configuration:(encryption_configuration_ : encryption_configuration option)
    ?tags:(tags_ : tag_list option) ~name:(name_ : name) () =
  ({ encryption_configuration = encryption_configuration_; tags = tags_; name = name_ }
