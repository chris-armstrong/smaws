...
let make_analysis_report_summary ?tags:(tags_ : tag_list option)
  ...
  ?end_time:(end_time_ : iso_timestamp option)
...
let make_list_performance_analysis_reports_request
  ?list_tags:(list_tags_ : boolean_ option)
...
let make_list_available_resource_metrics_request
  ?max_results:(max_results_ : max_results option)
  ?next_token:(next_token_ : next_token option)
  ~metric_types:(metric_types_ : metric_type_list)
  ~identifier:(identifier_ : identifier_string)
  ~service_type:(service_type_ : service_type) () =
  ({
     max_results = max_results_;
...
let make_data_point ~value:(value_ : double)
  ~timestamp:(timestamp_ : iso_timestamp) () =
...
let make_dimension_group ?limit:(limit_ : limit option)
  ?dimensions:(dimensions_ : sanitized_string_list option)
  ~group:(group_ : sanitized_string) () =
...
let make_metric_query ?filter:(filter_ : metric_query_filter_map option)
  ?group_by:(group_by_ : dimension_group option)
  ~metric:(metric_ : sanitized_string) () =
...
let make_get_resource_metrics_request
  ...
  ?next_token:(next_token_ : next_token option)
  ?max_results:(max_results_ : max_results option)
  ?period_in_seconds:(period_in_seconds_ : integer option)
  ~end_time:(end_time_ : iso_timestamp)
  ~start_time:(start_time_ : iso_timestamp)
  ~metric_queries:(metric_queries_ : metric_query_list)
  ~identifier:(identifier_ : identifier_string)
  ~service_type:(service_type_ : service_type) () =
  ({
...
let make_insight ?baseline_data:(baseline_data_ : data_list option)
  ...
  ?insight_type:(insight_type_ : string_ option)
  ~insight_id:(insight_id_ : string_) () =
  ({
...
let make_get_dimension_key_details_request
  ?requested_dimensions:(requested_dimensions_ :
                          requested_dimension_list option)
  ~group_identifier:(group_identifier_ : request_string)
  ~group:(group_ : request_string)
  ~identifier:(identifier_ : identifier_string)
  ~service_type:(service_type_ : service_type) () =
  ({
     requested_dimensions = requested_dimensions_;
...
let make_describe_dimension_keys_request
  ...
  ?period_in_seconds:(period_in_seconds_ : integer option)
  ~group_by:(group_by_ : dimension_group) ~metric:(metric_ : request_string)
  ~end_time:(end_time_ : iso_timestamp)
  ~start_time:(start_time_ : iso_timestamp)
  ~identifier:(identifier_ : identifier_string)
  ~service_type:(service_type_ : service_type) () =
...
let make_create_performance_analysis_report_request
  ?tags:(tags_ : tag_list option) ~end_time:(end_time_ : iso_timestamp)
  ~start_time:(start_time_ : iso_timestamp)
  ~identifier:(identifier_ : identifier_string)
  ~service_type:(service_type_ : service_type) () =
  ({
