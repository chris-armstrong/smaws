...
let workflow_type_to_yojson (x : workflow_type) =
  assoc_to_yojson
    [ ("version", Some (version_to_yojson x.version)); ("name", Some (name_to_yojson x.name)) ]
...
let workflow_type_info_to_yojson (x : workflow_type_info) =
  assoc_to_yojson
    [
      ("deprecationDate", option_to_yojson timestamp__to_yojson x.deprecation_date);
...
let workflow_execution_info_to_yojson (x : workflow_execution_info) =
  assoc_to_yojson
    [
      ...
      ("closeTimestamp", option_to_yojson timestamp__to_yojson x.close_timestamp);
