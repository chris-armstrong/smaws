...
let make_workflow_type_info ?deprecation_date:(deprecation_date_ : timestamp option)
    ?description:(description_ : description option) ~creation_date:(creation_date_ : timestamp)
    ~status:(status_ : registration_status) ~workflow_type:(workflow_type_ : workflow_type) () =
  ({
...
let make_workflow_type_infos ?next_page_token:(next_page_token_ : page_token option)
  ...
  ({ next_page_token = next_page_token_; type_infos = type_infos_ } : workflow_type_infos)
...
let make_workflow_type_configuration ?default_lambda_role:(default_lambda_role_ : arn option)
...
let make_workflow_type_detail ~configuration:(configuration_ : workflow_type_configuration)
    ~type_info:(type_info_ : workflow_type_info) () =
  ({ configuration = configuration_; type_info = type_info_ } : workflow_type_detail)

let make_workflow_execution_timed_out_event_attributes ~child_policy:(child_policy_ : child_policy)
    ~timeout_type:(timeout_type_ : workflow_execution_timeout_type) () =
  ({ child_policy = child_policy_; timeout_type = timeout_type_ }
    : workflow_execution_timed_out_event_attributes)

let make_workflow_execution_terminated_event_attributes
    ?cause:(cause_ : workflow_execution_terminated_cause option) ?details:(details_ : data option)
    ?reason:(reason_ : terminate_reason option) ~child_policy:(child_policy_ : child_policy) () =
  ({ cause = cause_; child_policy = child_policy_; details = details_; reason = reason_ }
...
let make_workflow_execution_started_event_attributes ?lambda_role:(lambda_role_ : arn option)
    ...
    ?tag_list:(tag_list_ : tag_list option) ?task_priority:(task_priority_ : task_priority option)
    ...
    ?input:(input_ : data option) ~workflow_type:(workflow_type_ : workflow_type)
...
let make_workflow_execution_info ?cancel_requested:(cancel_requested_ : canceled option)
    ?tag_list:(tag_list_ : tag_list option) ?parent:(parent_ : workflow_execution option)
    ...
    ~start_timestamp:(start_timestamp_ : timestamp) ~workflow_type:(workflow_type_ : workflow_type)
...
let make_workflow_execution_failed_event_attributes ?details:(details_ : data option)
...
let make_workflow_execution_configuration ?lambda_role:(lambda_role_ : arn option)
    ?task_priority:(task_priority_ : task_priority option)
    ~child_policy:(child_policy_ : child_policy) ~task_list:(task_list_ : task_list)
...
let make_workflow_execution_continued_as_new_event_attributes
    ?lambda_role:(lambda_role_ : arn option) ?tag_list:(tag_list_ : tag_list option)
    ?task_start_to_close_timeout:
      (task_start_to_close_timeout_ : duration_in_seconds_optional option)
    ?task_priority:(task_priority_ : task_priority option)
    ?execution_start_to_close_timeout:
...
let make_workflow_execution_canceled_event_attributes ?details:(details_ : data option)
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id) () =
  ({ decision_task_completed_event_id = decision_task_completed_event_id_; details = details_ }
...
let make_undeprecate_workflow_type_input ~workflow_type:(workflow_type_ : workflow_type)
...
let make_undeprecate_activity_type_input ~activity_type:(activity_type_ : activity_type)
    ~domain:(domain_ : domain_name) () =
...
let make_timer_fired_event_attributes ~started_event_id:(started_event_id_ : event_id)
    ~timer_id:(timer_id_ : timer_id) () =
...
let make_timer_canceled_event_attributes
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~started_event_id:(started_event_id_ : event_id) ~timer_id:(timer_id_ : timer_id) () =
  ({
     decision_task_completed_event_id = decision_task_completed_event_id_;
...
let make_terminate_workflow_execution_input ?child_policy:(child_policy_ : child_policy option)
    ?details:(details_ : data option) ?reason:(reason_ : terminate_reason option)
    ?run_id:(run_id_ : workflow_run_id_optional option) ~workflow_id:(workflow_id_ : workflow_id)
    ~domain:(domain_ : domain_name) () =
  ({
     child_policy = child_policy_;
     details = details_;
...
let make_tag_resource_input ~tags:(tags_ : resource_tag_list) ~resource_arn:(resource_arn_ : arn) ()
    =
  ({ tags = tags_; resource_arn = resource_arn_ } : tag_resource_input)

let make_tag_filter ~tag:(tag_ : tag) () = ({ tag = tag_ } : tag_filter)

let make_start_workflow_execution_input ?lambda_role:(lambda_role_ : arn option)
    ...
    ?execution_start_to_close_timeout:
      (execution_start_to_close_timeout_ : duration_in_seconds_optional option)
    ?input:(input_ : data option) ?task_priority:(task_priority_ : task_priority option)
    ?task_list:(task_list_ : task_list option) ~workflow_type:(workflow_type_ : workflow_type)
...
let make_start_timer_decision_attributes ?control:(control_ : data option)
    ~start_to_fire_timeout:(start_to_fire_timeout_ : duration_in_seconds)
    ~timer_id:(timer_id_ : timer_id) () =
  ({ start_to_fire_timeout = start_to_fire_timeout_; control = control_; timer_id = timer_id_ }
    : start_timer_decision_attributes)

let make_start_lambda_function_failed_event_attributes ?message:(message_ : cause_message option)
    ?cause:(cause_ : start_lambda_function_failed_cause option)
    ?scheduled_event_id:(scheduled_event_id_ : event_id option) () =
  ({ message = message_; cause = cause_; scheduled_event_id = scheduled_event_id_ }
...
let make_start_child_workflow_execution_failed_event_attributes ?control:(control_ : data option)
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~initiated_event_id:(initiated_event_id_ : event_id) ~workflow_id:(workflow_id_ : workflow_id)
...
let make_start_child_workflow_execution_decision_attributes ?lambda_role:(lambda_role_ : arn option)
    ?tag_list:(tag_list_ : tag_list option) ?child_policy:(child_policy_ : child_policy option)
    ?task_start_to_close_timeout:
      (task_start_to_close_timeout_ : duration_in_seconds_optional option)
    ?task_priority:(task_priority_ : task_priority option)
...
let make_signal_workflow_execution_input ?input:(input_ : data option)
    ?run_id:(run_id_ : workflow_run_id_optional option) ~signal_name:(signal_name_ : signal_name)
    ~workflow_id:(workflow_id_ : workflow_id) ~domain:(domain_ : domain_name) () =
  ({
     input = input_;
     signal_name = signal_name_;
...
let make_schedule_activity_task_decision_attributes
    ...
    ?input:(input_ : data option) ?control:(control_ : data option)
    ~activity_id:(activity_id_ : activity_id) ~activity_type:(activity_type_ : activity_type) () =
  ({
...
let make_signal_external_workflow_execution_decision_attributes ?control:(control_ : data option)
    ?input:(input_ : data option) ?run_id:(run_id_ : workflow_run_id_optional option)
    ~signal_name:(signal_name_ : signal_name) ~workflow_id:(workflow_id_ : workflow_id) () =
  ({
     control = control_;
     input = input_;
     signal_name = signal_name_;
...
let make_request_cancel_external_workflow_execution_decision_attributes
    ?control:(control_ : data option) ?run_id:(run_id_ : workflow_run_id_optional option)
    ~workflow_id:(workflow_id_ : workflow_id) () =
...
let make_respond_decision_task_completed_input
    ...
    ?task_list:(task_list_ : task_list option) ?execution_context:(execution_context_ : data option)
    ?decisions:(decisions_ : decision_list option) ~task_token:(task_token_ : task_token) () =
  ({
...
let make_respond_activity_task_failed_input ?details:(details_ : data option)
    ?reason:(reason_ : failure_reason option) ~task_token:(task_token_ : task_token) () =
...
let make_register_workflow_type_input ?default_lambda_role:(default_lambda_role_ : arn option)
    ?default_child_policy:(default_child_policy_ : child_policy option)
    ?default_task_priority:(default_task_priority_ : task_priority option)
    ?default_task_list:(default_task_list_ : task_list option)
    ?default_execution_start_to_close_timeout:
    ...
    ?default_task_start_to_close_timeout:
      (default_task_start_to_close_timeout_ : duration_in_seconds_optional option)
    ?description:(description_ : description option) ~version:(version_ : version)
    ~name:(name_ : name) ~domain:(domain_ : domain_name) () =
  ({
     default_lambda_role = default_lambda_role_;
     default_child_policy = default_child_policy_;
     default_task_priority = default_task_priority_;
...
let make_register_domain_input ?tags:(tags_ : resource_tag_list option)
    ...
    ~workflow_execution_retention_period_in_days:
      (workflow_execution_retention_period_in_days_ : duration_in_days) ~name:(name_ : domain_name)
    () =
  ({
     tags = tags_;
...
let make_complete_workflow_execution_failed_event_attributes
  ...
  ({ decision_task_completed_event_id = decision_task_completed_event_id_; cause = cause_ }
...
let make_decision_task_scheduled_event_attributes
    ?schedule_to_start_timeout:(schedule_to_start_timeout_ : duration_in_seconds_optional option)
    ?start_to_close_timeout:(start_to_close_timeout_ : duration_in_seconds_optional option)
    ?task_priority:(task_priority_ : task_priority option) ~task_list:(task_list_ : task_list) () =
  ({
     schedule_to_start_timeout = schedule_to_start_timeout_;
     start_to_close_timeout = start_to_close_timeout_;
     task_priority = task_priority_;
...
let make_decision_task_timed_out_event_attributes ~started_event_id:(started_event_id_ : event_id)
    ~scheduled_event_id:(scheduled_event_id_ : event_id)
...
let make_activity_task_scheduled_event_attributes
    ...
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~task_list:(task_list_ : task_list) ~activity_id:(activity_id_ : activity_id)
...
let make_activity_task_failed_event_attributes ?details:(details_ : data option)
    ?reason:(reason_ : failure_reason option) ~started_event_id:(started_event_id_ : event_id)
    ~scheduled_event_id:(scheduled_event_id_ : event_id) () =
  ({
     started_event_id = started_event_id_;
     scheduled_event_id = scheduled_event_id_;
...
let make_activity_task_canceled_event_attributes
    ...
    ?details:(details_ : data option) ~started_event_id:(started_event_id_ : event_id)
    ~scheduled_event_id:(scheduled_event_id_ : event_id) () =
  ({
...
let make_activity_task_cancel_requested_event_attributes ~activity_id:(activity_id_ : activity_id)
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id) () =
  ({
     activity_id = activity_id_;
     decision_task_completed_event_id = decision_task_completed_event_id_;
...
let make_record_marker_failed_event_attributes
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~cause:(cause_ : record_marker_failed_cause) ~marker_name:(marker_name_ : marker_name) () =
  ({
     decision_task_completed_event_id = decision_task_completed_event_id_;
     cause = cause_;
     marker_name = marker_name_;
...
let make_child_workflow_execution_failed_event_attributes ?details:(details_ : data option)
    ?reason:(reason_ : failure_reason option) ~started_event_id:(started_event_id_ : event_id)
    ~initiated_event_id:(initiated_event_id_ : event_id)
    ~workflow_type:(workflow_type_ : workflow_type)
    ~workflow_execution:(workflow_execution_ : workflow_execution) () =
...
let make_external_workflow_execution_signaled_event_attributes
    ~initiated_event_id:(initiated_event_id_ : event_id)
    ~workflow_execution:(workflow_execution_ : workflow_execution) () =
  ({ initiated_event_id = initiated_event_id_; workflow_execution = workflow_execution_ }
...
let make_schedule_activity_task_failed_event_attributes
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~cause:(cause_ : schedule_activity_task_failed_cause) ~activity_id:(activity_id_ : activity_id)
    ~activity_type:(activity_type_ : activity_type) () =
  ({
     decision_task_completed_event_id = decision_task_completed_event_id_;
     cause = cause_;
...
let make_cancel_timer_failed_event_attributes
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~cause:(cause_ : cancel_timer_failed_cause) ~timer_id:(timer_id_ : timer_id) () =
  ({
     decision_task_completed_event_id = decision_task_completed_event_id_;
     cause = cause_;
     timer_id = timer_id_;
...
let make_lambda_function_failed_event_attributes ?details:(details_ : data option)
    ?reason:(reason_ : failure_reason option) ~started_event_id:(started_event_id_ : event_id)
    ~scheduled_event_id:(scheduled_event_id_ : event_id) () =
  ({
     details = details_;
...
let make_schedule_lambda_function_failed_event_attributes
    ~decision_task_completed_event_id:(decision_task_completed_event_id_ : event_id)
    ~cause:(cause_ : schedule_lambda_function_failed_cause) ~name:(name_ : function_name)
    ~id:(id_ : function_id) () =
  ({
     decision_task_completed_event_id = decision_task_completed_event_id_;
     cause = cause_;
...
let make_history_event
    ...
    ?workflow_execution_started_event_attributes:
      (workflow_execution_started_event_attributes_ :
         workflow_execution_started_event_attributes option) ~event_id:(event_id_ : event_id)
    ~event_type:(event_type_ : event_type) ~event_timestamp:(event_timestamp_ : timestamp) () =
  ({
...
let make_decision_task ?previous_started_event_id:(previous_started_event_id_ : event_id option)
    ?next_page_token:(next_page_token_ : page_token option) ~events:(events_ : history_event_list)
    ~workflow_type:(workflow_type_ : workflow_type)
    ~workflow_execution:(workflow_execution_ : workflow_execution)
    ~started_event_id:(started_event_id_ : event_id) ~task_token:(task_token_ : task_token) () =
  ({
...
let make_poll_for_decision_task_input
    ...
    ?next_page_token:(next_page_token_ : page_token option) ?identity:(identity_ : identity option)
...
let make_activity_task ?input:(input_ : data option) ~activity_type:(activity_type_ : activity_type)
    ~workflow_execution:(workflow_execution_ : workflow_execution)
    ~started_event_id:(started_event_id_ : event_id) ~activity_id:(activity_id_ : activity_id)
    ~task_token:(task_token_ : task_token) () =
  ({
     input = input_;
     activity_type = activity_type_;
...
let make_list_workflow_types_input ?reverse_order:(reverse_order_ : reverse_order option)
    ?maximum_page_size:(maximum_page_size_ : page_size option)
    ?next_page_token:(next_page_token_ : page_token option) ?name:(name_ : name option)
...
let make_list_open_workflow_executions_input
    ...
    ~start_time_filter:(start_time_filter_ : execution_time_filter) ~domain:(domain_ : domain_name)
    () =
  ({
...
let make_domain_info ?arn:(arn_ : arn option) ?description:(description_ : description option)
    ~status:(status_ : registration_status) ~name:(name_ : domain_name) () =
  ({ arn = arn_; description = description_; status = status_; name = name_ } : domain_info)

let make_domain_infos ?next_page_token:(next_page_token_ : page_token option)
    ~domain_infos:(domain_infos_ : domain_info_list) () =
  ({ next_page_token = next_page_token_; domain_infos = domain_infos_ } : domain_infos)

let make_list_domains_input ?reverse_order:(reverse_order_ : reverse_order option)
    ?maximum_page_size:(maximum_page_size_ : page_size option)
    ?next_page_token:(next_page_token_ : page_token option)
    ~registration_status:(registration_status_ : registration_status) () =
  ({
...
let make_activity_type_info ?deprecation_date:(deprecation_date_ : timestamp option)
    ?description:(description_ : description option) ~creation_date:(creation_date_ : timestamp)
    ~status:(status_ : registration_status) ~activity_type:(activity_type_ : activity_type) () =
  ({
     deprecation_date = deprecation_date_;
     creation_date = creation_date_;
     description = description_;
...
let make_activity_type_infos ?next_page_token:(next_page_token_ : page_token option)
  ...
  ({ next_page_token = next_page_token_; type_infos = type_infos_ } : activity_type_infos)

let make_list_activity_types_input ?reverse_order:(reverse_order_ : reverse_order option)
    ?maximum_page_size:(maximum_page_size_ : page_size option)
    ?next_page_token:(next_page_token_ : page_token option) ?name:(name_ : name option)
    ~registration_status:(registration_status_ : registration_status)
    ~domain:(domain_ : domain_name) () =
...
let make_get_workflow_execution_history_input ?reverse_order:(reverse_order_ : reverse_order option)
    ?maximum_page_size:(maximum_page_size_ : page_size option)
    ?next_page_token:(next_page_token_ : page_token option)
    ~execution:(execution_ : workflow_execution) ~domain:(domain_ : domain_name) () =
  ({
     reverse_order = reverse_order_;
     maximum_page_size = maximum_page_size_;
     next_page_token = next_page_token_;
...
let make_describe_workflow_type_input ~workflow_type:(workflow_type_ : workflow_type)
    ~domain:(domain_ : domain_name) () =
  ({ workflow_type = workflow_type_; domain = domain_ } : describe_workflow_type_input)

let make_describe_workflow_execution_input ~execution:(execution_ : workflow_execution)
    ~domain:(domain_ : domain_name) () =
...
let make_activity_type_detail ~configuration:(configuration_ : activity_type_configuration)
    ~type_info:(type_info_ : activity_type_info) () =
  ({ configuration = configuration_; type_info = type_info_ } : activity_type_detail)

let make_describe_activity_type_input ~activity_type:(activity_type_ : activity_type)
    ~domain:(domain_ : domain_name) () =
  ({ activity_type = activity_type_; domain = domain_ } : describe_activity_type_input)

let make_deprecate_workflow_type_input ~workflow_type:(workflow_type_ : workflow_type)
    ~domain:(domain_ : domain_name) () =
...
let make_deprecate_activity_type_input ~activity_type:(activity_type_ : activity_type)
    ~domain:(domain_ : domain_name) () =
  ({ activity_type = activity_type_; domain = domain_ } : deprecate_activity_type_input)

let make_delete_workflow_type_input ~workflow_type:(workflow_type_ : workflow_type)
    ~domain:(domain_ : domain_name) () =
  ({ workflow_type = workflow_type_; domain = domain_ } : delete_workflow_type_input)

let make_delete_activity_type_input ~activity_type:(activity_type_ : activity_type)
    ~domain:(domain_ : domain_name) () =
...
let make_count_pending_decision_tasks_input ~task_list:(task_list_ : task_list)
    ~domain:(domain_ : domain_name) () =
  ({ task_list = task_list_; domain = domain_ } : count_pending_decision_tasks_input)

let make_count_pending_activity_tasks_input ~task_list:(task_list_ : task_list)
    ~domain:(domain_ : domain_name) () =
