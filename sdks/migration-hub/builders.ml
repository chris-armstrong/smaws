...
let make_task ?progress_percent:(progress_percent_ : progress_percent option)
    ?status_detail:(status_detail_ : status_detail option) ~status:(status_ : status) () =
  ({ progress_percent = progress_percent_; status_detail = status_detail_; status = status_ }
    : task)

let make_source_resource ?status_detail:(status_detail_ : status_detail option)
    ?description:(description_ : source_resource_description option)
    ~name:(name_ : source_resource_name) () =
  ({ status_detail = status_detail_; description = description_; name = name_ } : source_resource)
...
let make_notify_migration_task_state_request ?dry_run:(dry_run_ : dry_run option)
    ...
    ~update_date_time:(update_date_time_ : update_date_time) ~task:(task_ : task)
    ~migration_task_name:(migration_task_name_ : migration_task_name)
    ~progress_update_stream:(progress_update_stream_ : progress_update_stream) () =
  ({
     dry_run = dry_run_;
...
let make_notify_application_state_request ?dry_run:(dry_run_ : dry_run option)
    ?update_date_time:(update_date_time_ : update_date_time option)
    ~status:(status_ : application_status) ~application_id:(application_id_ : application_id) () =
  ({
     dry_run = dry_run_;
     update_date_time = update_date_time_;
     status = status_;
...
let make_migration_task_update ?migration_task_state:(migration_task_state_ : task option)
...
let make_migration_task_summary ?update_date_time:(update_date_time_ : update_date_time option)
...
let make_migration_task
    ...
    ?update_date_time:(update_date_time_ : update_date_time option) ?task:(task_ : task option)
    ?migration_task_name:(migration_task_name_ : migration_task_name option)
    ?progress_update_stream:(progress_update_stream_ : progress_update_stream option) () =
  ({
     resource_attribute_list = resource_attribute_list_;
...
let make_list_progress_update_streams_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : token option) () =
  ({ max_results = max_results_; next_token = next_token_ } : list_progress_update_streams_request)

let make_list_migration_tasks_request ?resource_name:(resource_name_ : resource_name option)
    ?max_results:(max_results_ : max_results option) ?next_token:(next_token_ : token option) () =
  ({ resource_name = resource_name_; max_results = max_results_; next_token = next_token_ }
    : list_migration_tasks_request)

let make_list_migration_task_updates_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : token option)
    ~migration_task_name:(migration_task_name_ : migration_task_name)
    ~progress_update_stream:(progress_update_stream_ : progress_update_stream) () =
  ({
...
let make_discovered_resource ?description:(description_ : discovered_resource_description option)
  ...
  ({ description = description_; configuration_id = configuration_id_ } : discovered_resource)
...
let make_application_state ?last_updated_time:(last_updated_time_ : update_date_time option)
...
let make_list_application_states_request ?max_results:(max_results_ : max_results option)
    ?next_token:(next_token_ : token option)
    ?application_ids:(application_ids_ : application_ids option) () =
  ({ max_results = max_results_; next_token = next_token_; application_ids = application_ids_ }
...
let make_describe_migration_task_request
    ~migration_task_name:(migration_task_name_ : migration_task_name)
    ~progress_update_stream:(progress_update_stream_ : progress_update_stream) () =
  ({ migration_task_name = migration_task_name_; progress_update_stream = progress_update_stream_ }
...
let make_delete_progress_update_stream_request ?dry_run:(dry_run_ : dry_run option)
  ...
  ({ dry_run = dry_run_; progress_update_stream_name = progress_update_stream_name_ }
