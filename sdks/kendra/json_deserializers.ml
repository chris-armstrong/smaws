...
let warning_code_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    | `String "QUERY_LANGUAGE_INVALID_SYNTAX" ->
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "UserGroupResolutionMode" value)
    | _ ->
        raise (deserialize_wrong_type_error path "UserGroupResolutionMode") : 
...
let user_context_policy_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "UserContextPolicy"
             value)
...
let update_query_suggestions_config_request_of_yojson tree path =
  ...
  ({
     ...
     query_log_look_back_window_in_days =
       (option_of_yojson
          (value_for_key integer_of_yojson "QueryLogLookBackWindowInDays")
          _list path);
     mode =
...
let document_attribute_value_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path "DocumentAttributeValueType") : 
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     featured_results_set_status) : featured_results_set_status)
...
let share_point_version_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "SharePointVersion"
             value)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     salesforce_standard_object_name) : salesforce_standard_object_name)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path
             "SalesforceKnowledgeArticleState") : salesforce_knowledge_article_state) : 
...
let service_now_build_version_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path "ServiceNowBuildVersionType") : 
     service_now_build_version_type) : service_now_build_version_type)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     service_now_authentication_type) : service_now_authentication_type)
...
let confluence_version_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "ConfluenceVersion"
             value)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     confluence_space_field_name) : confluence_space_field_name)
...
let confluence_page_field_name_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise (deserialize_wrong_type_error path "ConfluencePageFieldName") : 
     confluence_page_field_name) : confluence_page_field_name)
...
let confluence_blog_field_name_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise (deserialize_wrong_type_error path "ConfluenceBlogFieldName") : 
     confluence_blog_field_name) : confluence_blog_field_name)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     confluence_attachment_field_name) : confluence_attachment_field_name)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
     ...
     confluence_authentication_type) : confluence_authentication_type)
...
let fsx_file_system_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "FsxFileSystemType"
             value)
    | _ -> raise (deserialize_wrong_type_error path "FsxFileSystemType") : 
     fsx_file_system_type) : fsx_file_system_type)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "IssueSubEntity") : 
     issue_sub_entity) : issue_sub_entity)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "AlfrescoEntity") : 
     alfresco_entity) : alfresco_entity)
...
let condition_operator_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "ConditionOperator"
             value)
...
let document_attribute_value_of_yojson tree path =
  ...
  ({
     date_value =
       (option_of_yojson (value_for_key timestamp_of_yojson "DateValue")
          _list path);
     long_value =
...
let highlight_of_yojson tree path =
  ...
  ({
     ...
     begin_offset =
       (value_for_key integer_of_yojson "BeginOffset" _list path)
   } : highlight)
...
let table_excerpt_of_yojson tree path =
  ...
  ({
     total_number_of_rows =
       (option_of_yojson
          (value_for_key integer_of_yojson "TotalNumberOfRows") _list path);
     rows =
...
let suggestion_highlight_of_yojson tree path =
  ...
  ({
     end_offset =
       (option_of_yojson (value_for_key integer_of_yojson "EndOffset") _list
          path);
     begin_offset =
       (option_of_yojson (value_for_key integer_of_yojson "BeginOffset")
          _list path)
  ...
  ({
     user_context =
       (option_of_yojson (value_for_key user_context_of_yojson "UserContext")
          _list path);
     page_size =
       (option_of_yojson (value_for_key integer_of_yojson "PageSize") _list
          path);
     page_number =
       (option_of_yojson (value_for_key integer_of_yojson "PageNumber") _list
          path);
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "QuerySuggestionsStatus") : 
     query_suggestions_status) : query_suggestions_status)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ ->
        raise
          (deserialize_wrong_type_error path
             "QuerySuggestionsBlockListStatus") : query_suggestions_block_list_status) : 
  query_suggestions_block_list_status)
let query_suggestions_block_list_summary_of_yojson tree path =
  let _list = assoc_of_yojson tree path in
  ({
     item_count =
       (option_of_yojson (value_for_key integer_of_yojson "ItemCount") _list
          path);
     updated_at =
       (option_of_yojson (value_for_key timestamp_of_yojson "UpdatedAt")
          _list path);
...
let query_result_format_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "QueryResultFormat"
             value)
...
let query_result_of_yojson tree path =
  ...
  ({
     ...
     total_number_of_results =
       (option_of_yojson
          (value_for_key integer_of_yojson "TotalNumberOfResults") _list path);
     facet_results =
       (option_of_yojson
          (value_for_key facet_result_list_of_yojson "FacetResults") _list
          path);
...
let expand_configuration_of_yojson tree path =
  ...
  ({
     max_expanded_results_per_item =
       (option_of_yojson
          (value_for_key integer_of_yojson "MaxExpandedResultsPerItem") _list
          path);
     max_result_items_to_expand =
       (option_of_yojson
          (value_for_key integer_of_yojson "MaxResultItemsToExpand") _list
          path)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "PrincipalMappingStatus") : 
     principal_mapping_status) : principal_mapping_status)
...
let index_edition_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "IndexEdition" value)
    | _ -> raise (deserialize_wrong_type_error path "IndexEdition") : 
     index_edition) : index_edition)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | _ -> raise (deserialize_wrong_type_error path "FaqFileFormat") : 
     faq_file_format) : faq_file_format)
...
let endpoint_type_of_yojson (tree : t) path =
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path "EndpointType" value)
  ...
  (let _list = assoc_of_yojson tree path in
   (match tree with
    ...
    | `String value ->
        raise
          (deserialize_unknown_enum_value_error path
             "DataSourceSyncJobStatus" value)
    | _ ->
        raise (deserialize_wrong_type_error path "DataSourceSyncJobStatus") : 
     data_source_sync_job_status) : data_source_sync_job_status)
  ...
  ({
     ...
     ordering_id =
       (option_of_yojson
          ...
          path);
     received_at =
       (option_of_yojson (value_for_key timestamp_of_yojson "ReceivedAt")
          _list path);
     last_updated_at =
       (option_of_yojson (value_for_key timestamp_of_yojson "LastUpdatedAt")
          _list path);
     status =
       (option_of_yojson
...
let get_snapshots_request_of_yojson tree path =
  ...
  ({
     max_results =
       (option_of_yojson (value_for_key integer_of_yojson "MaxResults") _list
          path);
     next_token =
       (option_of_yojson (value_for_key next_token_of_yojson "NextToken")
          _list path);
...
let get_query_suggestions_request_of_yojson tree path =
  ...
  ({
     ...
     max_suggestions_count =
       (option_of_yojson
          (value_for_key integer_of_yojson "MaxSuggestionsCount") _list path);
     query_text =
...
let describe_query_suggestions_config_response_of_yojson tree path =
  ...
  ({
     ...
     total_suggestions_count =
       (option_of_yojson
          (value_for_key integer_of_yojson "TotalSuggestionsCount") _list
          path);
     last_clear_time =
       (option_of_yojson (value_for_key timestamp_of_yojson "LastClearTime")
          _list path);
     last_suggestions_build_time =
       (option_of_yojson
          (value_for_key timestamp_of_yojson "LastSuggestionsBuildTime")
          _list path);
     minimum_query_count =
       (option_of_yojson
          (value_for_key minimum_query_count_of_yojson "MinimumQueryCount")
