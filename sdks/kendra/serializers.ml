...
let key_location_to_yojson (x : key_location) =
  match x with SECRET_MANAGER -> `String "SECRET_MANAGER" | URL -> `String "URL"
...
let user_context_policy_to_yojson (x : user_context_policy) =
  match x with USER_TOKEN -> `String "USER_TOKEN" | ATTRIBUTE_FILTER -> `String "ATTRIBUTE_FILTER"
...
let mode_to_yojson (x : mode) =
  match x with LEARN_ONLY -> `String "LEARN_ONLY" | ENABLED -> `String "ENABLED"
...
let share_point_online_authentication_type_to_yojson (x : share_point_online_authentication_type) =
  match x with OAUTH2 -> `String "OAUTH2" | HTTP_BASIC -> `String "HTTP_BASIC"
...
let query_identifiers_enclosing_option_to_yojson (x : query_identifiers_enclosing_option) =
  match x with NONE -> `String "NONE" | DOUBLE_QUOTES -> `String "DOUBLE_QUOTES"
...
let salesforce_chatter_feed_include_filter_type_to_yojson
  ...
  match x with STANDARD_USER -> `String "STANDARD_USER" | ACTIVE_USER -> `String "ACTIVE_USER"
...
let highlight_type_to_yojson (x : highlight_type) =
  match x with THESAURUS_SYNONYM -> `String "THESAURUS_SYNONYM" | STANDARD -> `String "STANDARD"
...
let table_excerpt_to_yojson (x : table_excerpt) =
  assoc_to_yojson
    [
      ("TotalNumberOfRows", option_to_yojson integer__to_yojson x.total_number_of_rows);
...
let suggestion_type_to_yojson (x : suggestion_type) =
  match x with DOCUMENT_ATTRIBUTES -> `String "DOCUMENT_ATTRIBUTES" | QUERY -> `String "QUERY"
...
let relevance_type_to_yojson (x : relevance_type) =
  match x with NOT_RELEVANT -> `String "NOT_RELEVANT" | RELEVANT -> `String "RELEVANT"
...
let stop_data_source_sync_job_request_to_yojson (x : stop_data_source_sync_job_request) =
  assoc_to_yojson
    [
      ("IndexId", Some (index_id_to_yojson x.index_id)); ("Id", Some (data_source_id_to_yojson x.id));
    ]
...
and attribute_filter_list_to_yojson tree = list_to_yojson attribute_filter_to_yojson tree
...
let rec document_attribute_value_count_pair_to_yojson (x : document_attribute_value_count_pair) =
  assoc_to_yojson
    [
...
and facet_result_list_to_yojson tree = list_to_yojson facet_result_to_yojson tree
...
let query_result_to_yojson (x : query_result) =
  assoc_to_yojson
    [
      ...
      ("TotalNumberOfResults", option_to_yojson integer__to_yojson x.total_number_of_results);
      ("FacetResults", option_to_yojson facet_result_list_to_yojson x.facet_results);
...
let expand_configuration_to_yojson (x : expand_configuration) =
  assoc_to_yojson
    [
      ...
      ("MaxResultItemsToExpand", option_to_yojson integer__to_yojson x.max_result_items_to_expand);
    ]
...
let group_ordering_id_summary_to_yojson (x : group_ordering_id_summary) =
  assoc_to_yojson
    [
      ...
      ("LastUpdatedAt", option_to_yojson timestamp__to_yojson x.last_updated_at);
...
let get_query_suggestions_request_to_yojson (x : get_query_suggestions_request) =
  assoc_to_yojson
    [
      ...
      ("MaxSuggestionsCount", option_to_yojson integer__to_yojson x.max_suggestions_count);
...
let describe_thesaurus_request_to_yojson (x : describe_thesaurus_request) =
  assoc_to_yojson
    [
      ("IndexId", Some (index_id_to_yojson x.index_id)); ("Id", Some (thesaurus_id_to_yojson x.id));
    ]
    ...
    [
      ( "AttributeSuggestionsConfig",
        option_to_yojson attribute_suggestions_describe_config_to_yojson
          x.attribute_suggestions_config );
      ("TotalSuggestionsCount", option_to_yojson integer__to_yojson x.total_suggestions_count);
      ("LastClearTime", option_to_yojson timestamp__to_yojson x.last_clear_time);
...
let describe_faq_request_to_yojson (x : describe_faq_request) =
  assoc_to_yojson
    [ ("IndexId", Some (index_id_to_yojson x.index_id)); ("Id", Some (faq_id_to_yojson x.id)) ]
...
let describe_experience_request_to_yojson (x : describe_experience_request) =
  assoc_to_yojson
    [
      ("IndexId", Some (index_id_to_yojson x.index_id)); ("Id", Some (experience_id_to_yojson x.id));
    ]
