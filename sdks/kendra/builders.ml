...
let make_seed_url_configuration ?web_crawler_mode:(web_crawler_mode_ : web_crawler_mode option)
  ...
  ({ web_crawler_mode = web_crawler_mode_; seed_urls = seed_urls_ } : seed_url_configuration)

let make_site_maps_configuration ~site_maps:(site_maps_ : site_maps_list) () =
  ({ site_maps = site_maps_ } : site_maps_configuration)

let make_urls ?site_maps_configuration:(site_maps_configuration_ : site_maps_configuration option)
    ?seed_url_configuration:(seed_url_configuration_ : seed_url_configuration option) () =
  ({
...
let make_proxy_configuration ?credentials:(credentials_ : secret_arn option) ~port:(port_ : port)
    ~host:(host_ : host) () =
  ({ credentials = credentials_; port = port_; host = host_ } : proxy_configuration)

let make_basic_authentication_configuration ~credentials:(credentials_ : secret_arn)
...
let make_warning ?code:(code_ : warning_code option) ?message:(message_ : warning_message option) ()
    =
  ({ code = code_; message = message_ } : warning)

let make_jwt_token_type_configuration ?claim_regex:(claim_regex_ : claim_regex option)
    ...
    ?secret_manager_arn:(secret_manager_arn_ : role_arn option) ?ur_l:(ur_l_ : url option)
...
let make_json_token_type_configuration ~group_attribute_field:(group_attribute_field_ : string_)
...
let make_user_context ?data_source_groups:(data_source_groups_ : data_source_groups option)
    ?groups:(groups_ : groups option) ?user_id:(user_id_ : principal_name option)
    ?token:(token_ : token option) () =
  ({
...
let make_s3_path ~key:(key_ : s3_object_key) ~bucket:(bucket_ : s3_bucket_name) () =
  ({ key = key_; bucket = bucket_ } : s3_path)

let make_update_thesaurus_request ?source_s3_path:(source_s3_path_ : s3_path option)
    ?role_arn:(role_arn_ : role_arn option) ?description:(description_ : description option)
    ?name:(name_ : thesaurus_name option) ~index_id:(index_id_ : index_id) ~id:(id_ : thesaurus_id)
    () =
  ({
...
let make_suggestable_config ?suggestable:(suggestable_ : object_boolean option)
    ?attribute_name:(attribute_name_ : document_attribute_key option) () =
  ({ suggestable = suggestable_; attribute_name = attribute_name_ } : suggestable_config)
...
let make_update_query_suggestions_block_list_request ?role_arn:(role_arn_ : role_arn option)
    ...
    ~id:(id_ : query_suggestions_block_list_id) ~index_id:(index_id_ : index_id) () =
  ({
     role_arn = role_arn_;
     source_s3_path = source_s3_path_;
     description = description_;
...
let make_relevance ?value_importance_map:(value_importance_map_ : value_importance_map option)
    ?rank_order:(rank_order_ : order option) ?duration:(duration_ : duration option)
...
let make_search ?sortable:(sortable_ : boolean_ option)
    ?displayable:(displayable_ : boolean_ option) ?searchable:(searchable_ : boolean_ option)
...
let make_document_metadata_configuration ?search:(search_ : search option)
    ?relevance:(relevance_ : relevance option) ~type_:(type__ : document_attribute_value_type)
...
let make_update_index_request
    ...
    ?name:(name_ : index_name option) ~id:(id_ : index_id) () =
  ({
...
let make_featured_results_set ?creation_timestamp:(creation_timestamp_ : long option)
...
let make_conflicting_item ?set_id:(set_id_ : string_ option) ?set_name:(set_name_ : string_ option)
    ?query_text:(query_text_ : query_text option) () =
  ({ set_id = set_id_; set_name = set_name_; query_text = query_text_ } : conflicting_item)

let make_content_source_configuration ?direct_put_content:(direct_put_content_ : boolean_ option)
...
let make_update_experience_request ?description:(description_ : description option)
    ?configuration:(configuration_ : experience_configuration option)
    ?role_arn:(role_arn_ : role_arn option) ?name:(name_ : experience_name option)
...
let make_share_point_configuration
    ...
    ?inclusion_patterns:(inclusion_patterns_ : data_source_inclusions_exclusions_strings option)
    ?use_change_log:(use_change_log_ : boolean_ option)
    ?crawl_attachments:(crawl_attachments_ : boolean_ option) ~secret_arn:(secret_arn_ : secret_arn)
    ~urls:(urls_ : share_point_url_list)
...
let make_connection_configuration ~secret_arn:(secret_arn_ : secret_arn)
    ~table_name:(table_name_ : table_name) ~database_name:(database_name_ : database_name)
    ~database_port:(database_port_ : database_port) ~database_host:(database_host_ : database_host)
    () =
  ({
     secret_arn = secret_arn_;
...
let make_database_configuration ?sql_configuration:(sql_configuration_ : sql_configuration option)
...
let make_salesforce_standard_object_attachment_configuration
    ?field_mappings:(field_mappings_ : data_source_to_index_field_mapping_list option)
    ?document_title_field_name:(document_title_field_name_ : data_source_field_name option) () =
  ({ field_mappings = field_mappings_; document_title_field_name = document_title_field_name_ }
...
let make_one_drive_users ?one_drive_user_s3_path:(one_drive_user_s3_path_ : s3_path option)
    ?one_drive_user_list:(one_drive_user_list_ : one_drive_user_list option) () =
  ({ one_drive_user_s3_path = one_drive_user_s3_path_; one_drive_user_list = one_drive_user_list_ }
    : one_drive_users)

let make_one_drive_configuration ?disable_local_groups:(disable_local_groups_ : boolean_ option)
    ?field_mappings:(field_mappings_ : data_source_to_index_field_mapping_list option)
    ?exclusion_patterns:(exclusion_patterns_ : data_source_inclusions_exclusions_strings option)
    ?inclusion_patterns:(inclusion_patterns_ : data_source_inclusions_exclusions_strings option)
    ~one_drive_users:(one_drive_users_ : one_drive_users) ~secret_arn:(secret_arn_ : secret_arn)
...
let make_service_now_configuration
    ...
    ~secret_arn:(secret_arn_ : secret_arn) ~host_url:(host_url_ : service_now_host_url) () =
  ({
     authentication_type = authentication_type_;
...
let make_confluence_configuration
    ...
    ~version:(version_ : confluence_version) ~secret_arn:(secret_arn_ : secret_arn)
    ~server_url:(server_url_ : url) () =
  ({
     authentication_type = authentication_type_;
     proxy_configuration = proxy_configuration_;
     exclusion_patterns = exclusion_patterns_;
...
let make_box_configuration
    ...
    ?crawl_comments:(crawl_comments_ : boolean_ option)
    ?use_change_log:(use_change_log_ : boolean_ option) ~secret_arn:(secret_arn_ : secret_arn)
...
let make_jira_configuration
    ...
    ?status:(status_ : jira_status option) ?issue_type:(issue_type_ : issue_type option)
    ?project:(project_ : project option) ?use_change_log:(use_change_log_ : boolean_ option)
    ~secret_arn:(secret_arn_ : secret_arn) ~jira_account_url:(jira_account_url_ : jira_account_url)
    () =
  ({
     vpc_configuration = vpc_configuration_;
     exclusion_patterns = exclusion_patterns_;
...
let make_saa_s_configuration ~host_url:(host_url_ : url)
  ...
  ({ host_url = host_url_; organization_name = organization_name_ } : saa_s_configuration)

let make_on_premise_configuration ~ssl_certificate_s3_path:(ssl_certificate_s3_path_ : s3_path)
    ~organization_name:(organization_name_ : organization_name) ~host_url:(host_url_ : url) () =
  ({
     ssl_certificate_s3_path = ssl_certificate_s3_path_;
...
let make_git_hub_configuration
    ...
    ?use_change_log:(use_change_log_ : boolean_ option) ?type_:(type__ : type_ option)
...
let make_document_attribute_value ?date_value:(date_value_ : timestamp option)
...
let make_hook_configuration
    ...
    ~s3_bucket:(s3_bucket_ : s3_bucket_name) ~lambda_arn:(lambda_arn_ : lambda_arn) () =
  ({
...
let make_custom_document_enrichment_configuration ?role_arn:(role_arn_ : role_arn option)
...
let make_update_data_source_request
    ...
    ?language_code:(language_code_ : language_code option) ?role_arn:(role_arn_ : role_arn option)
    ?schedule:(schedule_ : scan_schedule option) ?description:(description_ : description option)
    ?vpc_configuration:(vpc_configuration_ : data_source_vpc_configuration option)
...
let make_principal ?data_source_id:(data_source_id_ : data_source_id option)
  ...
  ({ data_source_id = data_source_id_; access = access_; type_ = type__; name = name_ } : principal)
...
let make_update_access_control_configuration_request
    ...
    ~id:(id_ : access_control_configuration_id) ~index_id:(index_id_ : index_id) () =
  ({
...
let make_thesaurus_summary ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option) ?status:(status_ : thesaurus_status option)
    ?name:(name_ : thesaurus_name option) ?id:(id_ : thesaurus_id option) () =
  ({ updated_at = updated_at_; created_at = created_at_; status = status_; name = name_; id = id_ }
    : thesaurus_summary)

let make_highlight ?type_:(type__ : highlight_type option)
    ?top_answer:(top_answer_ : boolean_ option) ~end_offset:(end_offset_ : integer)
    ~begin_offset:(begin_offset_ : integer) () =
  ({
     type_ = type__;
...
let make_text_with_highlights ?highlights:(highlights_ : highlight_list option)
    ?text:(text_ : string_ option) () =
  ({ highlights = highlights_; text = text_ } : text_with_highlights)

let make_text_document_statistics ~indexed_text_bytes:(indexed_text_bytes_ : indexed_text_bytes)
...
let make_table_cell ?header:(header_ : boolean_ option)
    ?highlighted:(highlighted_ : boolean_ option) ?top_answer:(top_answer_ : boolean_ option)
    ?value:(value_ : string_ option) () =
  ({ header = header_; highlighted = highlighted_; top_answer = top_answer_; value = value_ }
    : table_cell)

let make_table_row ?cells:(cells_ : table_cell_list option) () = ({ cells = cells_ } : table_row)

let make_table_excerpt ?total_number_of_rows:(total_number_of_rows_ : integer option)
...
let make_suggestion_text_with_highlights
    ?highlights:(highlights_ : suggestion_highlight_list option) ?text:(text_ : string_ option) () =
...
let make_suggestion ?source_documents:(source_documents_ : source_documents option)
    ?value:(value_ : suggestion_value option) ?id:(id_ : result_id option) () =
  ({ source_documents = source_documents_; value = value_; id = id_ } : suggestion)

let make_click_feedback ~click_time:(click_time_ : timestamp) ~result_id:(result_id_ : result_id) ()
    =
  ({ click_time = click_time_; result_id = result_id_ } : click_feedback)

let make_relevance_feedback ~relevance_value:(relevance_value_ : relevance_type)
...
let make_spell_corrected_query ?corrections:(corrections_ : correction_list option)
...
let make_sorting_configuration ~sort_order:(sort_order_ : sort_order)
  ...
  ({ sort_order = sort_order_; document_attribute_key = document_attribute_key_ }
...
let make_retrieve_result_item ?score_attributes:(score_attributes_ : score_attributes option)
    ?document_attributes:(document_attributes_ : document_attribute_list option)
    ?document_ur_i:(document_ur_i_ : url option) ?content:(content_ : content option)
    ?document_title:(document_title_ : document_title option)
    ?document_id:(document_id_ : document_id option) ?id:(id_ : result_id option) () =
  ({
...
let make_attribute_filter ?less_than_or_equals:(less_than_or_equals_ : document_attribute option)
...
let make_retrieve_request ?user_context:(user_context_ : user_context option)
    ?page_size:(page_size_ : integer option) ?page_number:(page_number_ : integer option)
...
let make_query_suggestions_block_list_summary ?item_count:(item_count_ : integer option)
...
let make_additional_result_attribute ~value:(value_ : additional_result_attribute_value)
    ~value_type:(value_type_ : additional_result_attribute_value_type) ~key:(key_ : string_) () =
...
let make_collapsed_result_detail ?expanded_results:(expanded_results_ : expanded_result_list option)
  ...
  ({ expanded_results = expanded_results_; document_attribute = document_attribute_ }
...
let make_query_result_item
    ...
    ?format:(format_ : query_result_format option) ?type_:(type__ : query_result_type option)
...
let make_document_attribute_value_count_pair
    ?facet_results:(facet_results_ : facet_result_list option) ?count:(count_ : integer option)
...
let make_featured_results_item ?feedback_token:(feedback_token_ : feedback_token option)
    ?document_attributes:(document_attributes_ : document_attribute_list option)
    ?document_ur_i:(document_ur_i_ : url option)
    ?document_excerpt:(document_excerpt_ : text_with_highlights option)
    ?document_title:(document_title_ : text_with_highlights option)
...
let make_facet ?max_results:(max_results_ : top_document_attribute_value_count_pairs_size option)
    ?facets:(facets_ : facet_list option)
    ?document_attribute_key:(document_attribute_key_ : document_attribute_key option) () =
  ({
...
let make_query_request
    ...
    ?facets:(facets_ : facet_list option)
    ?attribute_filter:(attribute_filter_ : attribute_filter option)
    ?query_text:(query_text_ : query_text option) ~index_id:(index_id_ : index_id) () =
  ({
...
let make_group_members ?s3_pathfor_group_members:(s3_pathfor_group_members_ : s3_path option)
...
let make_put_principal_mapping_request ?role_arn:(role_arn_ : role_arn option)
    ?ordering_id:(ordering_id_ : principal_ordering_id option)
    ?data_source_id:(data_source_id_ : data_source_id option)
    ~group_members:(group_members_ : group_members) ~group_id:(group_id_ : group_id)
    ~index_id:(index_id_ : index_id) () =
  ({
     role_arn = role_arn_;
...
let make_personas_summary ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option) ?persona:(persona_ : persona option)
...
let make_list_thesauri_response
  ...
  ({ thesaurus_summary_items = thesaurus_summary_items_; next_token = next_token_ }
...
let make_list_thesauri_request
    ...
    ?next_token:(next_token_ : next_token option) ~index_id:(index_id_ : index_id) () =
  ({ max_results = max_results_; next_token = next_token_; index_id = index_id_ }
...
let make_list_query_suggestions_block_lists_response ?next_token:(next_token_ : next_token option)
    ?block_list_summary_items:
      (block_list_summary_items_ : query_suggestions_block_list_summary_items option) () =
  ({ next_token = next_token_; block_list_summary_items = block_list_summary_items_ }
...
let make_group_summary ?ordering_id:(ordering_id_ : principal_ordering_id option)
    ?group_id:(group_id_ : group_id option) () =
  ({ ordering_id = ordering_id_; group_id = group_id_ } : group_summary)

let make_index_configuration_summary ?edition:(edition_ : index_edition option)
    ?id:(id_ : index_id option) ?name:(name_ : index_name option) ~status:(status_ : index_status)
...
let make_list_groups_older_than_ordering_id_response ?next_token:(next_token_ : next_token option)
...
let make_list_groups_older_than_ordering_id_request
    ...
    ?next_token:(next_token_ : next_token option)
    ?data_source_id:(data_source_id_ : data_source_id option)
    ~ordering_id:(ordering_id_ : principal_ordering_id) ~index_id:(index_id_ : index_id) () =
  ({
     max_results = max_results_;
     next_token = next_token_;
     ordering_id = ordering_id_;
...
let make_featured_results_set_summary ?creation_timestamp:(creation_timestamp_ : long option)
    ?last_updated_timestamp:(last_updated_timestamp_ : long option)
    ?status:(status_ : featured_results_set_status option)
    ?featured_results_set_name:(featured_results_set_name_ : featured_results_set_name option)
    ?featured_results_set_id:(featured_results_set_id_ : featured_results_set_id option) () =
...
let make_list_featured_results_sets_response ?next_token:(next_token_ : next_token option)
...
let make_faq_summary ?language_code:(language_code_ : language_code option)
    ...
    ?updated_at:(updated_at_ : timestamp option) ?created_at:(created_at_ : timestamp option)
    ?status:(status_ : faq_status option) ?name:(name_ : faq_name option) ?id:(id_ : faq_id option)
    () =
  ({
     language_code = language_code_;
...
let make_list_faqs_response ?faq_summary_items:(faq_summary_items_ : faq_summary_items option)
    ?next_token:(next_token_ : next_token option) () =
  ({ faq_summary_items = faq_summary_items_; next_token = next_token_ } : list_faqs_response)
...
let make_experiences_summary ?endpoints:(endpoints_ : experience_endpoints option)
    ?status:(status_ : experience_status option) ?created_at:(created_at_ : timestamp option)
    ?id:(id_ : experience_id option) ?name:(name_ : experience_name option) () =
  ({ endpoints = endpoints_; status = status_; created_at = created_at_; id = id_; name = name_ }
    : experiences_summary)

let make_list_experiences_response ?next_token:(next_token_ : next_token option)
...
let make_entity_display_data ?last_name:(last_name_ : name_type option)
    ...
    ?group_name:(group_name_ : name_type option) ?user_name:(user_name_ : name_type option) () =
  ({
...
let make_experience_entities_summary ?display_data:(display_data_ : entity_display_data option)
    ?entity_type:(entity_type_ : entity_type option) ?entity_id:(entity_id_ : entity_id option) () =
  ({ display_data = display_data_; entity_type = entity_type_; entity_id = entity_id_ }
    : experience_entities_summary)

let make_list_experience_entities_response ?next_token:(next_token_ : next_token option)
...
let make_list_entity_personas_response ?next_token:(next_token_ : next_token option)
...
let make_list_entity_personas_request
  ...
  ({ max_results = max_results_; next_token = next_token_; index_id = index_id_; id = id_ }
    : list_entity_personas_request)

let make_data_source_summary ?language_code:(language_code_ : language_code option)
    ?status:(status_ : data_source_status option) ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option) ?type_:(type__ : data_source_type option)
    ?id:(id_ : data_source_id option) ?name:(name_ : data_source_name option) () =
  ({
     language_code = language_code_;
     status = status_;
     updated_at = updated_at_;
...
let make_list_data_sources_response ?next_token:(next_token_ : next_token option)
...
let make_data_source_sync_job_metrics ?documents_scanned:(documents_scanned_ : metric_value option)
...
let make_data_source_sync_job ?metrics:(metrics_ : data_source_sync_job_metrics option)
    ...
    ?status:(status_ : data_source_sync_job_status option) ?end_time:(end_time_ : timestamp option)
    ?start_time:(start_time_ : timestamp option) ?execution_id:(execution_id_ : string_ option) () =
  ({
     metrics = metrics_;
...
let make_list_data_source_sync_jobs_response ?next_token:(next_token_ : next_token option)
...
let make_list_access_control_configurations_response ?next_token:(next_token_ : string_ option)
  ...
  ({ access_control_configurations = access_control_configurations_; next_token = next_token_ }
...
let make_index_statistics
  ...
  ({ text_document_statistics = text_document_statistics_; faq_statistics = faq_statistics_ }
    : index_statistics)

let make_group_ordering_id_summary ?failure_reason:(failure_reason_ : failure_reason option)
    ?ordering_id:(ordering_id_ : principal_ordering_id option)
...
let make_get_snapshots_request ?max_results:(max_results_ : integer option)
    ?next_token:(next_token_ : next_token option) ~metric_type:(metric_type_ : metric_type)
    ~interval:(interval_ : interval) ~index_id:(index_id_ : index_id) () =
  ({
     max_results = max_results_;
     next_token = next_token_;
...
let make_get_query_suggestions_response ?suggestions:(suggestions_ : suggestion_list option)
...
let make_attribute_suggestions_get_config ?user_context:(user_context_ : user_context option)
    ?attribute_filter:(attribute_filter_ : attribute_filter option)
...
let make_get_query_suggestions_request
    ...
    ~query_text:(query_text_ : suggestion_query_text) ~index_id:(index_id_ : index_id) () =
  ({
     attribute_suggestions_config = attribute_suggestions_config_;
...
let make_featured_document_with_metadata ?ur_i:(ur_i_ : url option) ?title:(title_ : string_ option)
...
let make_document
    ...
    ?hierarchical_access_control_list:
      (hierarchical_access_control_list_ : hierarchical_principal_list option)
    ?access_control_list:(access_control_list_ : principal_list option)
    ?attributes:(attributes_ : document_attribute_list option) ?s3_path:(s3_path_ : s3_path option)
    ?blob:(blob_ : blob option) ?title:(title_ : title option) ~id:(id_ : document_id) () =
  ({
...
let make_disassociate_personas_from_entities_request ~entity_ids:(entity_ids_ : entity_ids_list)
    ~index_id:(index_id_ : index_id) ~id:(id_ : experience_id) () =
  ({ entity_ids = entity_ids_; index_id = index_id_; id = id_ }
...
let make_disassociate_entities_from_experience_request
    ~entity_list:(entity_list_ : disassociate_entity_list) ~index_id:(index_id_ : index_id)
    ~id:(id_ : experience_id) () =
  ({ entity_list = entity_list_; index_id = index_id_; id = id_ }
    : disassociate_entities_from_experience_request)

let make_describe_thesaurus_response ?synonym_rule_count:(synonym_rule_count_ : long option)
    ?term_count:(term_count_ : long option) ?file_size_bytes:(file_size_bytes_ : long option)
    ?source_s3_path:(source_s3_path_ : s3_path option) ?role_arn:(role_arn_ : role_arn option)
    ?updated_at:(updated_at_ : timestamp option) ?created_at:(created_at_ : timestamp option)
    ?error_message:(error_message_ : error_message option)
    ?status:(status_ : thesaurus_status option) ?description:(description_ : description option)
    ?name:(name_ : thesaurus_name option) ?index_id:(index_id_ : index_id option)
    ?id:(id_ : thesaurus_id option) () =
  ({
...
let make_describe_thesaurus_request ~index_id:(index_id_ : index_id) ~id:(id_ : thesaurus_id) () =
...
let make_describe_query_suggestions_config_response
    ...
    ?include_queries_without_user_information:
      (include_queries_without_user_information_ : object_boolean option)
    ?query_log_look_back_window_in_days:(query_log_look_back_window_in_days_ : integer option)
    ?status:(status_ : query_suggestions_status option) ?mode:(mode_ : mode option) () =
  ({
     attribute_suggestions_config = attribute_suggestions_config_;
...
let make_describe_query_suggestions_block_list_response ?role_arn:(role_arn_ : role_arn option)
    ?file_size_bytes:(file_size_bytes_ : long option) ?item_count:(item_count_ : integer option)
    ?source_s3_path:(source_s3_path_ : s3_path option) ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option)
    ?error_message:(error_message_ : error_message option)
    ?status:(status_ : query_suggestions_block_list_status option)
    ?description:(description_ : description option)
    ?name:(name_ : query_suggestions_block_list_name option)
    ?id:(id_ : query_suggestions_block_list_id option) ?index_id:(index_id_ : index_id option) () =
  ({
     role_arn = role_arn_;
     file_size_bytes = file_size_bytes_;
     item_count = item_count_;
...
let make_describe_query_suggestions_block_list_request ~id:(id_ : query_suggestions_block_list_id)
    ~index_id:(index_id_ : index_id) () =
...
let make_describe_principal_mapping_request
    ?data_source_id:(data_source_id_ : data_source_id option) ~group_id:(group_id_ : group_id)
    ~index_id:(index_id_ : index_id) () =
  ({ group_id = group_id_; data_source_id = data_source_id_; index_id = index_id_ }
...
let make_describe_index_response
    ...
    ?updated_at:(updated_at_ : timestamp option) ?created_at:(created_at_ : timestamp option)
    ?description:(description_ : description option) ?status:(status_ : index_status option)
    ?server_side_encryption_configuration:
      (server_side_encryption_configuration_ : server_side_encryption_configuration option)
    ?role_arn:(role_arn_ : role_arn option) ?edition:(edition_ : index_edition option)
    ?id:(id_ : index_id option) ?name:(name_ : index_name option) () =
  ({
     user_group_resolution_configuration = user_group_resolution_configuration_;
     user_context_policy = user_context_policy_;
     user_token_configurations = user_token_configurations_;
...
let make_describe_faq_response ?language_code:(language_code_ : language_code option)
    ?file_format:(file_format_ : faq_file_format option)
    ?error_message:(error_message_ : error_message option) ?role_arn:(role_arn_ : role_arn option)
    ?status:(status_ : faq_status option) ?s3_path:(s3_path_ : s3_path option)
    ?updated_at:(updated_at_ : timestamp option) ?created_at:(created_at_ : timestamp option)
    ?description:(description_ : description option) ?name:(name_ : faq_name option)
    ?index_id:(index_id_ : index_id option) ?id:(id_ : faq_id option) () =
  ({
     language_code = language_code_;
     file_format = file_format_;
     error_message = error_message_;
...
let make_describe_faq_request ~index_id:(index_id_ : index_id) ~id:(id_ : faq_id) () =
  ({ index_id = index_id_; id = id_ } : describe_faq_request)

let make_describe_experience_response ?error_message:(error_message_ : error_message option)
    ?role_arn:(role_arn_ : role_arn option) ?status:(status_ : experience_status option)
    ?description:(description_ : description option) ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option)
    ?configuration:(configuration_ : experience_configuration option)
    ?endpoints:(endpoints_ : experience_endpoints option) ?name:(name_ : experience_name option)
...
let make_describe_experience_request ~index_id:(index_id_ : index_id) ~id:(id_ : experience_id) () =
...
let make_describe_data_source_response
    ...
    ?error_message:(error_message_ : error_message option) ?role_arn:(role_arn_ : role_arn option)
    ?schedule:(schedule_ : scan_schedule option) ?status:(status_ : data_source_status option)
    ?description:(description_ : description option) ?updated_at:(updated_at_ : timestamp option)
    ?created_at:(created_at_ : timestamp option)
    ?vpc_configuration:(vpc_configuration_ : data_source_vpc_configuration option)
    ?configuration:(configuration_ : data_source_configuration option)
    ?type_:(type__ : data_source_type option) ?name:(name_ : data_source_name option)
...
let make_describe_data_source_request ~index_id:(index_id_ : index_id) ~id:(id_ : data_source_id) ()
    =
...
let make_describe_access_control_configuration_request ~id:(id_ : access_control_configuration_id)
    ~index_id:(index_id_ : index_id) () =
  ({ id = id_; index_id = index_id_ } : describe_access_control_configuration_request)

let make_delete_thesaurus_request ~index_id:(index_id_ : index_id) ~id:(id_ : thesaurus_id) () =
  ({ index_id = index_id_; id = id_ } : delete_thesaurus_request)

let make_delete_query_suggestions_block_list_request ~id:(id_ : query_suggestions_block_list_id)
    ~index_id:(index_id_ : index_id) () =
  ({ id = id_; index_id = index_id_ } : delete_query_suggestions_block_list_request)

let make_delete_principal_mapping_request ?ordering_id:(ordering_id_ : principal_ordering_id option)
    ?data_source_id:(data_source_id_ : data_source_id option) ~group_id:(group_id_ : group_id)
    ~index_id:(index_id_ : index_id) () =
  ({
     ordering_id = ordering_id_;
...
let make_delete_faq_request ~index_id:(index_id_ : index_id) ~id:(id_ : faq_id) () =
  ({ index_id = index_id_; id = id_ } : delete_faq_request)

let make_delete_experience_response () = (() : unit)

let make_delete_experience_request ~index_id:(index_id_ : index_id) ~id:(id_ : experience_id) () =
  ({ index_id = index_id_; id = id_ } : delete_experience_request)

let make_delete_data_source_request ~index_id:(index_id_ : index_id) ~id:(id_ : data_source_id) () =
  ({ index_id = index_id_; id = id_ } : delete_data_source_request)

let make_delete_access_control_configuration_response () = (() : unit)

let make_delete_access_control_configuration_request ~id:(id_ : access_control_configuration_id)
    ~index_id:(index_id_ : index_id) () =
...
let make_data_source_sync_job_metric_target
  ...
  ({ data_source_sync_job_id = data_source_sync_job_id_; data_source_id = data_source_id_ }
    : data_source_sync_job_metric_target)

let make_create_thesaurus_response ?id:(id_ : thesaurus_id option) () =
  ({ id = id_ } : create_thesaurus_response)

let make_create_thesaurus_request ?client_token:(client_token_ : client_token_name option)
    ...
    ~name:(name_ : thesaurus_name) ~index_id:(index_id_ : index_id) () =
  ({
...
let make_create_query_suggestions_block_list_request ?tags:(tags_ : tag_list option)
    ?client_token:(client_token_ : client_token_name option)
    ?description:(description_ : description option) ~role_arn:(role_arn_ : role_arn)
    ~source_s3_path:(source_s3_path_ : s3_path) ~name:(name_ : query_suggestions_block_list_name)
    ~index_id:(index_id_ : index_id) () =
  ({
     tags = tags_;
     role_arn = role_arn_;
...
let make_create_faq_request ?language_code:(language_code_ : language_code option)
    ?client_token:(client_token_ : client_token_name option)
    ?file_format:(file_format_ : faq_file_format option) ?tags:(tags_ : tag_list option)
    ?description:(description_ : description option) ~role_arn:(role_arn_ : role_arn)
    ~s3_path:(s3_path_ : s3_path) ~name:(name_ : faq_name) ~index_id:(index_id_ : index_id) () =
  ({
     language_code = language_code_;
     client_token = client_token_;
     file_format = file_format_;
...
let make_create_experience_request ?client_token:(client_token_ : client_token_name option)
    ?description:(description_ : description option)
    ?configuration:(configuration_ : experience_configuration option)
...
let make_create_data_source_request
    ...
    ?role_arn:(role_arn_ : role_arn option) ?schedule:(schedule_ : scan_schedule option)
    ?description:(description_ : description option)
    ?vpc_configuration:(vpc_configuration_ : data_source_vpc_configuration option)
    ?configuration:(configuration_ : data_source_configuration option)
...
let make_create_access_control_configuration_request
    ...
    ?access_control_list:(access_control_list_ : principal_list option)
    ?description:(description_ : description option)
    ~name:(name_ : access_control_configuration_name) ~index_id:(index_id_ : index_id) () =
  ({
     client_token = client_token_;
     hierarchical_access_control_list = hierarchical_access_control_list_;
     access_control_list = access_control_list_;
...
let make_batch_put_document_response_failed_document
    ?error_message:(error_message_ : error_message option)
    ?error_code:(error_code_ : error_code option)
    ?data_source_id:(data_source_id_ : data_source_id option) ?id:(id_ : document_id option) () =
  ({
     error_message = error_message_;
     error_code = error_code_;
     data_source_id = data_source_id_;
...
let make_batch_put_document_request
    ?custom_document_enrichment_configuration:
      (custom_document_enrichment_configuration_ : custom_document_enrichment_configuration option)
    ?role_arn:(role_arn_ : role_arn option) ~documents:(documents_ : document_list)
    ~index_id:(index_id_ : index_id) () =
  ({
     custom_document_enrichment_configuration = custom_document_enrichment_configuration_;
...
let make_batch_get_document_status_request
    ~document_info_list:(document_info_list_ : document_info_list) ~index_id:(index_id_ : index_id)
    () =
  ({ document_info_list = document_info_list_; index_id = index_id_ }
    : batch_get_document_status_request)

let make_batch_delete_featured_results_set_error ~error_message:(error_message_ : error_message)
...
let make_batch_delete_featured_results_set_request
    ...
    ~index_id:(index_id_ : index_id) () =
  ({ featured_results_set_ids = featured_results_set_ids_; index_id = index_id_ }
...
let make_batch_delete_document_request
    ...
    ~document_id_list:(document_id_list_ : document_id_list) ~index_id:(index_id_ : index_id) () =
  ({
...
let make_associate_personas_to_entities_request
    ~personas:(personas_ : entity_persona_configuration_list) ~index_id:(index_id_ : index_id)
    ~id:(id_ : experience_id) () =
  ({ personas = personas_; index_id = index_id_; id = id_ }
...
let make_associate_entities_to_experience_request
    ~entity_list:(entity_list_ : associate_entity_list) ~index_id:(index_id_ : index_id)
    ~id:(id_ : experience_id) () =
  ({ entity_list = entity_list_; index_id = index_id_; id = id_ }
