...
type nonrec long_term_pricing_id = string [@@ocaml.doc ""]

type nonrec job_id = string [@@ocaml.doc ""]

type nonrec java_boolean = bool [@@ocaml.doc ""]

type nonrec update_long_term_pricing_request = {
  is_long_term_pricing_auto_renew : java_boolean option;
      [@ocaml.doc
        "If set to [true], specifies that the current long-term pricing type for the device should \
...
type nonrec invalid_resource_exception = {
  resource_type : string_ option; [@ocaml.doc "The provided resource value is invalid.\n"]
  message : string_ option; [@ocaml.doc ""]
}
[@@ocaml.doc
  "The specified resource can't be found. Check the information you provided in your last request, \
   and try again.\n"]

type nonrec update_job_shipment_state_result = unit [@@ocaml.doc ""]

type nonrec shipment_state = RETURNED [@ocaml.doc ""] | RECEIVED [@ocaml.doc ""] [@@ocaml.doc ""]
...
type nonrec invalid_job_state_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "The action can't be performed because the job's current state doesn't allow that action to be \
   performed.\n"]

type nonrec update_job_result = unit [@@ocaml.doc ""]

type nonrec role_ar_n = string [@@ocaml.doc ""]

type nonrec sns_topic_ar_n = string [@@ocaml.doc ""]
...
type nonrec job_state_list = job_state list [@@ocaml.doc ""]
...
type nonrec notification = {
  ...
  notify_all : boolean_ option;
      [@ocaml.doc "Any change in job state will trigger a notification for this job.\n"]
  job_states_to_notify : job_state_list option;
      [@ocaml.doc "The list of job states that will trigger a notification for this job.\n"]
  sns_topic_ar_n : sns_topic_ar_n option;
      [@ocaml.doc
        "The new SNS [TopicArn] that you want to associate with this job. You can create Amazon \
         Resource Names (ARNs) for topics by using the \
         {{:https://docs.aws.amazon.com/sns/latest/api/API_CreateTopic.html}CreateTopic} Amazon \
         SNS API action.\n\n\
        \ You can subscribe email addresses to an Amazon SNS topic through the Amazon Web Services \
         ...
         {{:https://docs.aws.amazon.com/sns/latest/api/API_Subscribe.html}Subscribe} Amazon Simple \
...
[@@ocaml.doc
  "The Amazon Simple Notification Service (Amazon SNS) notification settings associated with a \
   specific job. The [Notification] object is returned as a part of the response syntax of the \
   [DescribeJob] action in the [JobMetadata] data type.\n\n\
  \ When the notification settings are defined during job creation, you can choose to notify based \
   on a specific set of job states using the [JobStatesToNotify] array of strings, or you can \
   specify that you want to have Amazon SNS notifications sent out for all job states with \
...
[@@ocaml.doc
  "An object that represents the service or services on the Snow Family device that your \
   transferred data will be exported from or imported into. Amazon Web Services Snow Family \
   supports Amazon S3 and NFS (Network File System).\n"]

type nonrec target_on_device_service_list = target_on_device_service list [@@ocaml.doc ""]
  ...
  target_on_device_services : target_on_device_service_list option;
      [@ocaml.doc
        "Specifies the service or services on the Snow Family device that your transferred data \
         will be exported from or imported into. Amazon Web Services Snow Family supports Amazon \
...
type nonrec s3_resource_list = s3_resource list [@@ocaml.doc ""]
...
[@@ocaml.doc "The container for the [EventTriggerDefinition$EventResourceARN].\n"]

type nonrec event_trigger_definition_list = event_trigger_definition list [@@ocaml.doc ""]
...
type nonrec lambda_resource = {
  ...
  lambda_arn : resource_ar_n option;
      [@ocaml.doc
        "An Amazon Resource Name (ARN) that represents an Lambda function to be triggered by PUT \
         object actions on the associated local Amazon S3 resource.\n"]
}
[@@ocaml.doc "Identifies \n"]

type nonrec lambda_resource_list = lambda_resource list [@@ocaml.doc ""]

type nonrec ami_id = string [@@ocaml.doc ""]

type nonrec ec2_ami_resource = {
  snowball_ami_id : string_ option; [@ocaml.doc "The ID of the AMI on the Snow device.\n"]
  ami_id : ami_id; [@ocaml.doc "The ID of the AMI in Amazon EC2.\n"]
}
[@@ocaml.doc
...
type nonrec ec2_ami_resource_list = ec2_ami_resource list [@@ocaml.doc ""]

type nonrec job_resource = {
  ec2_ami_resources : ec2_ami_resource_list option;
      [@ocaml.doc "The Amazon Machine Images (AMIs) associated with this job.\n"]
  lambda_resources : lambda_resource_list option;
      [@ocaml.doc "The Python-language Lambda functions for this job.\n"]
  s3_resources : s3_resource_list option; [@ocaml.doc "An array of [S3Resource] objects.\n"]
}
[@@ocaml.doc
  "Contains an array of Amazon Web Services resource objects. Each object represents an Amazon S3 \
   bucket, an Lambda function, or an Amazon Machine Image (AMI) based on Amazon EC2 that is \
   associated with a particular job.\n"]

type nonrec storage_limit = int [@@ocaml.doc ""]

type nonrec storage_unit = TB [@ocaml.doc ""] [@@ocaml.doc ""]

type nonrec nfs_on_device_service_configuration = {
  storage_unit : storage_unit option;
      [@ocaml.doc "The scale unit of the NFS storage on the device.\n\n Valid values: TB.\n "]
  storage_limit : storage_limit option;
      [@ocaml.doc "The maximum NFS storage for one Snow Family device.\n"]
}
[@@ocaml.doc
  "An object that represents the metadata and configuration settings for the NFS (Network File \
   System) service on an Amazon Web Services Snow Family device.\n"]
...
type nonrec eks_on_device_service_configuration = {
  eks_anywhere_version : string_ option;
      [@ocaml.doc "The optional version of EKS Anywhere on the Snow Family device.\n"]
  kubernetes_version : string_ option;
      [@ocaml.doc "The Kubernetes version for EKS Anywhere on the Snow Family device.\n"]
}
[@@ocaml.doc
  "An object representing the metadata and configuration settings of EKS Anywhere on the Snow \
   Family device.\n"]

type nonrec s3_storage_limit = float [@@ocaml.doc ""]

type nonrec service_size = int [@@ocaml.doc ""]

type nonrec node_fault_tolerance = int [@@ocaml.doc ""]

type nonrec s3_on_device_service_configuration = {
  fault_tolerance : node_fault_tolerance option;
      [@ocaml.doc
        ">Fault tolerance level of the cluster. This indicates the number of nodes that can go \
         down without degrading the performance of the cluster. This additional input helps when \
         the specified [StorageLimit] matches more than one Amazon S3 compatible storage on Snow \
         family devices service configuration.\n"]
  service_size : service_size option;
      [@ocaml.doc
        "Applicable when creating a cluster. Specifies how many nodes are needed for Amazon S3 \
         compatible storage on Snow family devices. If specified, the other input can be omitted.\n"]
  storage_unit : storage_unit option;
      [@ocaml.doc "Storage unit. Currently the only supported unit is TB.\n"]
  storage_limit : s3_storage_limit option;
      [@ocaml.doc
        "If the specified storage limit value matches storage limit of one of the defined \
         configurations, that configuration will be used. If the specified storage limit value \
         does not match any defined configuration, the request will fail. If more than one \
         configuration has the same storage limit as specified, the other input need to be \
         provided.\n"]
}
[@@ocaml.doc "Amazon S3 compatible storage on Snow family devices configuration items.\n"]

type nonrec on_device_service_configuration = {
  s3_on_device_service : s3_on_device_service_configuration option;
      [@ocaml.doc "Configuration for Amazon S3 compatible storage on Snow family devices.\n"]
  eks_on_device_service : eks_on_device_service_configuration option;
      [@ocaml.doc "The configuration of EKS Anywhere on the Snow Family device.\n"]
  ...
  nfs_on_device_service : nfs_on_device_service_configuration option;
      [@ocaml.doc "Represents the NFS (Network File System) service on a Snow Family device.\n"]
}
[@@ocaml.doc
...
type nonrec address_id = string [@@ocaml.doc ""]
...
type nonrec device_pickup_id = string [@@ocaml.doc ""]
...
type nonrec pickup_details = {
  ...
  identification_number : string_ option;
      [@ocaml.doc "The number on the credential identifying the person picking up the device.\n"]
  email : email option; [@ocaml.doc "The email address of the person picking up the device.\n"]
  phone_number : phone_number option;
      [@ocaml.doc "The phone number of the person picking up the device.\n"]
  name : string_ option; [@ocaml.doc "The name of the person picking up the device.\n"]
}
[@@ocaml.doc "Information identifying the person picking up the device.\n"]
...
type nonrec update_job_request = {
  ...
  description : string_ option;
      [@ocaml.doc "The updated description of this job's [JobMetadata] object.\n"]
  shipping_option : shipping_option option;
      [@ocaml.doc "The updated shipping option value of this job's [ShippingDetails] object.\n"]
  address_id : address_id option; [@ocaml.doc "The ID of the updated [Address] object.\n"]
  on_device_service_configuration : on_device_service_configuration option;
      [@ocaml.doc
        "Specifies the service or services on the Snow Family device that your transferred data \
         will be exported from or imported into. Amazon Web Services Snow Family supports Amazon \
         S3 and NFS (Network File System) and the Amazon Web Services Storage Gateway service Tape \
         Gateway type.\n"]
  resources : job_resource option;
      [@ocaml.doc "The updated [JobResource] object, or the updated [JobResource] object. \n"]
  notification : notification option; [@ocaml.doc "The new or updated [Notification] object.\n"]
...
type nonrec kms_request_failed_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "The provided Key Management Service key lacks the permissions to perform the specified \
   [CreateJob] or [UpdateJob] action.\n"]

type nonrec invalid_input_combination_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
...
type nonrec ec2_request_failed_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "Your user lacks the necessary Amazon EC2 permissions to perform the attempted action.\n"]

type nonrec cluster_limit_exceeded_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "Job creation failed. Currently, clusters support five nodes. If you have fewer than five nodes \
   for your cluster and you have more nodes to create for this cluster, try again and create jobs \
   until your cluster has exactly five nodes.\n"]

type nonrec update_cluster_result = unit [@@ocaml.doc ""]
...
type nonrec update_cluster_request = {
  ...
  notification : notification option; [@ocaml.doc "The new or updated [Notification] object.\n"]
  shipping_option : shipping_option option;
      [@ocaml.doc "The updated shipping option value of this cluster's [ShippingDetails] object.\n"]
  address_id : address_id option; [@ocaml.doc "The ID of the updated [Address] object.\n"]
  on_device_service_configuration : on_device_service_configuration option;
      [@ocaml.doc
        "Specifies the service or services on the Snow Family device that your transferred data \
         will be exported from or imported into. Amazon Web Services Snow Family device clusters \
         support Amazon S3 and NFS (Network File System).\n"]
  resources : job_resource option;
      [@ocaml.doc
        "The updated arrays of [JobResource] objects that can include updated [S3Resource] objects \
         or [LambdaResource] objects.\n"]
  description : string_ option; [@ocaml.doc "The updated description of this cluster.\n"]
  role_ar_n : role_ar_n option;
      [@ocaml.doc
...
type nonrec unsupported_address_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "The address is either outside the serviceable area for your region, or an error occurred. Check \
   the address with your region's carrier and try again. If the issue persists, contact Amazon Web \
   Services Support.\n"]

type nonrec gsti_n = string [@@ocaml.doc ""]
...
[@@ocaml.doc "The tax documents required in Amazon Web Services Region in India.\n"]

type nonrec tax_documents = { in_d : ind_tax_documents option [@ocaml.doc ""] }
[@@ocaml.doc "The tax documents required in your Amazon Web Services Region.\n"]
...
type nonrec shipment = {
  tracking_number : string_ option;
      [@ocaml.doc
        ...
        \ "]
  status : string_ option; [@ocaml.doc "Status information for a shipment.\n"]
}
[@@ocaml.doc "The [Status] and [TrackingNumber] information for an inbound or outbound shipment.\n"]
  ...
  shipping_option : shipping_option option;
      [@ocaml.doc
        "The shipping speed for a particular job. This speed doesn't dictate how soon you'll get \
         the Snow device from the job's creation date. This speed represents how quickly it moves \
...
type nonrec service_version = {
  version : string_ option; [@ocaml.doc "The version number of the requested service.\n"]
}
[@@ocaml.doc "The version of the requested service.\n"]

type nonrec service_version_list = service_version list [@@ocaml.doc ""]

type nonrec service_name = EKS_ANYWHERE [@ocaml.doc ""] | KUBERNETES [@ocaml.doc ""]
[@@ocaml.doc ""]
...
[@@ocaml.doc
  "You get this exception if you call [CreateReturnShippingLabel] and a valid return shipping \
   label already exists. In this case, use [DescribeReturnShippingLabel] to get the URL.\n"]
...
type nonrec long_term_pricing_associated_job_id_list = job_id list [@@ocaml.doc ""]

type nonrec long_term_pricing_list_entry = {
  job_ids : long_term_pricing_associated_job_id_list option;
      [@ocaml.doc "The IDs of the jobs that are associated with a long-term pricing type.\n"]
  snowball_type : snowball_type option;
      [@ocaml.doc "The type of Snow Family devices associated with this long-term pricing job.\n"]
  ...
  is_long_term_pricing_auto_renew : java_boolean option;
      [@ocaml.doc
        "If set to [true], specifies that the current long-term pricing type for the device should \
         be automatically renewed before the long-term pricing contract expires.\n"]
  replacement_job : job_id option;
      [@ocaml.doc "A new device that replaces a device that is ordered with long-term pricing.\n"]
  current_active_job : job_id option;
      [@ocaml.doc "The current active jobs on the device the long-term pricing type.\n"]
  long_term_pricing_type : long_term_pricing_type option;
      [@ocaml.doc "The type of long-term pricing that was selected for the device.\n"]
...
type nonrec long_term_pricing_id_list = long_term_pricing_id list [@@ocaml.doc ""]
...
type nonrec dependent_service = {
  ...
  service_name : service_name option; [@ocaml.doc "The name of the dependent service.\n"]
}
[@@ocaml.doc "The name and version of the service dependant on the requested service.\n"]

type nonrec dependent_service_list = dependent_service list [@@ocaml.doc ""]
...
type nonrec list_service_versions_result = {
  next_token : string_ option;
      [@ocaml.doc
        "Because HTTP requests are stateless, this is the starting point of the next list of \
         returned [ListServiceVersionsResult] results.\n"]
  ...
  service_name : service_name;
      [@ocaml.doc "The name of the service for which the system provided supported versions.\n"]
  service_versions : service_version_list; [@ocaml.doc "A list of supported versions.\n"]
}
[@@ocaml.doc ""]
...
type nonrec list_service_versions_request = {
  next_token : string_ option;
      [@ocaml.doc
        "Because HTTP requests are stateless, this is the starting point for the next list of \
         returned [ListServiceVersionsRequest] versions.\n"]
  max_results : list_limit option;
      [@ocaml.doc "The maximum number of [ListServiceVersions] objects to return.\n"]
  dependent_services : dependent_service_list option;
      [@ocaml.doc "A list of names and versions of dependant services of the requested service.\n"]
  service_name : service_name;
      [@ocaml.doc "The name of the service for which you're requesting supported versions.\n"]
}
[@@ocaml.doc ""]

type nonrec invalid_next_token_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "The [NextToken] string was altered unexpectedly, and the operation has stopped. Run the \
   operation without changing the [NextToken] string, and try again.\n"]

type nonrec address_type = AWS_SHIP [@ocaml.doc ""] | CUST_PICKUP [@ocaml.doc ""] [@@ocaml.doc ""]
...
type nonrec address = {
  ...
  postal_code : string_ option;
      [@ocaml.doc "The postal code in an address that a Snow device is to be delivered to.\n"]
  country : string_ option;
      [@ocaml.doc "The country in an address that a Snow device is to be delivered to.\n"]
  landmark : string_ option; [@ocaml.doc "This field is no longer used and the value is ignored.\n"]
  ...
  state_or_province : string_ option;
      [@ocaml.doc "The state or province in an address that a Snow device is to be delivered to.\n"]
  city : string_ option;
      [@ocaml.doc "The city in an address that a Snow device is to be delivered to.\n"]
  street3 : string_ option;
      [@ocaml.doc "The third line in a street address that a Snow device is to be delivered to.\n"]
  street2 : string_ option;
      [@ocaml.doc "The second line in a street address that a Snow device is to be delivered to.\n"]
  street1 : string_ option;
      [@ocaml.doc "The first line in a street address that a Snow device is to be delivered to.\n"]
  company : string_ option;
      [@ocaml.doc "The name of the company to receive a Snow device at an address.\n"]
  name : string_ option;
      [@ocaml.doc "The name of a person to receive a Snow device at an address.\n"]
  address_id : address_id option; [@ocaml.doc "The unique ID for an address.\n"]
}
[@@ocaml.doc
  "The address that you want the Snow device(s) associated with a specific job to be shipped to. \
   Addresses are validated at the time of creation. The address you provide must be located within \
   the serviceable area of your region. Although no individual elements of the [Address] are \
   required, if the address is invalid or unsupported, then an exception is thrown.\n"]
...
type nonrec list_pickup_locations_request = {
  ...
  max_results : list_limit option; [@ocaml.doc "The maximum number of locations to list per page.\n"]
}
[@@ocaml.doc ""]
...
type nonrec list_long_term_pricing_request = {
  ...
  max_results : list_limit option;
      [@ocaml.doc "The maximum number of [ListLongTermPricing] objects to return.\n"]
}
[@@ocaml.doc ""]

type nonrec job_type = LOCAL_USE [@ocaml.doc ""] | EXPORT [@ocaml.doc ""] | IMPORT [@ocaml.doc ""]
[@@ocaml.doc ""]
...
type nonrec job_list_entry = {
  description : string_ option;
      [@ocaml.doc
        ...
        \        2016-08-11].\n"]
  creation_date : timestamp option; [@ocaml.doc "The creation date for this job.\n"]
  snowball_type : snowball_type option; [@ocaml.doc "The type of device used with this job.\n"]
  ...
  is_master : boolean_ option;
      [@ocaml.doc
        "A value that indicates that this job is a main job. A main job represents a successful \
         ...
         each main job will have at least one job part, and each job part is associated with a \
         Snowball. It might take some time before the job parts associated with a particular main \
...
type nonrec job_list_entry_list = job_list_entry list [@@ocaml.doc ""]
...
type nonrec list_jobs_request = {
  ...
  max_results : list_limit option; [@ocaml.doc "The number of [JobListEntry] objects to return.\n"]
}
[@@ocaml.doc ""]

type nonrec compatible_image = {
  name : string_ option; [@ocaml.doc "The optional name of a compatible image.\n"]
  ami_id : string_ option; [@ocaml.doc "The unique identifier for an individual Snow device AMI.\n"]
}
[@@ocaml.doc
  "A JSON-formatted object that describes a compatible Amazon Machine Image (AMI), including the \
   ID and name for a Snow device AMI. This AMI is compatible with the device's physical hardware \
   requirements, and it should be able to be run in an SBE1 instance on the device.\n"]

type nonrec compatible_image_list = compatible_image list [@@ocaml.doc ""]
...
type nonrec list_compatible_images_request = {
  next_token : string_ option;
      [@ocaml.doc
        "HTTP requests are stateless. To identify what object comes \"next\" in the list of \
         compatible images, you can specify a value for [NextToken] as the starting point for your \
...
[@@ocaml.doc "Contains a cluster's state, a cluster's ID, and other important information.\n"]

type nonrec cluster_list_entry_list = cluster_list_entry list [@@ocaml.doc ""]
...
type nonrec list_clusters_request = {
  next_token : string_ option;
      [@ocaml.doc
        "HTTP requests are stateless. To identify what object comes \"next\" in the list of \
         [ClusterListEntry] objects, you have the option of specifying [NextToken] as the starting \
         point for your returned list.\n"]
  max_results : list_limit option;
...
type nonrec data_transfer = {
  ...
  objects_transferred : long option;
      [@ocaml.doc "The number of objects transferred between a Snow device and Amazon S3.\n"]
  bytes_transferred : long option;
      [@ocaml.doc "The number of bytes transferred between a Snow device and Amazon S3.\n"]
}
[@@ocaml.doc
  "Defines the real-time status of a Snow device's data transfer while the device is at Amazon Web \
   Services. This data is only available while a job has a [JobState] value of [InProgress], for \
   both import and export jobs.\n"]

type nonrec job_logs = {
  job_failure_log_ur_i : string_ option;
      [@ocaml.doc "A link to an Amazon S3 presigned URL where the job failure log is located.\n"]
  job_success_log_ur_i : string_ option;
      [@ocaml.doc "A link to an Amazon S3 presigned URL where the job success log is located.\n"]
...
[@@ocaml.doc
  "Contains job logs. Whenever a Snow device is used to import data into or export data out of \
   ...
   part of the response syntax of the [DescribeJob] action in the [JobMetadata] data type. The job \
   logs can be accessed for up to 60 minutes after this request has been made. To access any of \
   the job logs after 60 minutes have passed, you'll have to make another call to the \
   [DescribeJob] action.\n\n\
  \ For import jobs, the PDF job report becomes available at the end of the import process. For \
   export jobs, your job report typically becomes available while the Snow device for your job \
  ...
  \  \n\
  \   For deeper visibility into the status of your transferred objects, you can look at the two \
   associated logs: a success log and a failure log. The logs are saved in comma-separated value \
   (CSV) format, and the name of each log includes the ID of the job or job part that the log \
   describes.\n\
  \   "]
...
type nonrec job_metadata = {
  snowball_id : string_ option; [@ocaml.doc "Unique ID associated with a device.\n"]
  ...
  remote_management : remote_management option;
      [@ocaml.doc
        "Allows you to securely operate and manage Snowcone devices remotely from outside of your \
         internal network. When set to [INSTALLED_AUTOSTART], remote management will automatically \
         be available when the device arrives at your location. Otherwise, you need to use the \
  ...
  job_log_info : job_logs option;
      [@ocaml.doc
        "Links to Amazon S3 presigned URLs for the job report and logs. For import jobs, the PDF \
         ...
         report typically becomes available while the Snow device for your job part is being \
         delivered to you.\n"]
  ...
  notification : notification option;
      [@ocaml.doc
        "The Amazon Simple Notification Service (Amazon SNS) notification settings associated with \
         a specific job. The [Notification] object is returned as a part of the response syntax of \
  ...
  address_id : address_id option;
      [@ocaml.doc "The ID for the address that you want the Snow device shipped to.\n"]
  role_ar_n : role_ar_n option;
      [@ocaml.doc
  ...
  kms_key_ar_n : kms_key_ar_n option;
      [@ocaml.doc
        "The Amazon Resource Name (ARN) for the Key Management Service (KMS) key associated with \
         this job. This ARN was created using the \
         {{:https://docs.aws.amazon.com/kms/latest/APIReference/API_CreateKey.html}CreateKey} API \
         action in KMS.\n"]
  description : string_ option;
  ...
  snowball_type : snowball_type option; [@ocaml.doc "The type of device used with this job.\n"]
  job_type : job_type option; [@ocaml.doc "The type of job.\n"]
  job_state : job_state option; [@ocaml.doc "The current status of the jobs.\n"]
  job_id : string_ option;
      [@ocaml.doc
        "The automatically generated ID for a job, for example \
         [JID123e4567-e89b-12d3-a456-426655440000].\n"]
...
[@@ocaml.doc
  "Contains information about a specific job including shipping information, job status, and other \
   important metadata. This information is returned as a part of the response syntax of the \
   [DescribeJob] action.\n"]

type nonrec job_metadata_list = job_metadata list [@@ocaml.doc ""]

type nonrec invalid_address_exception = { message : string_ option [@ocaml.doc ""] }
[@@ocaml.doc
  "The address provided was invalid. Check the address with your region's carrier, and try again.\n"]

type nonrec integer = int [@@ocaml.doc ""]

type nonrec initial_cluster_size = int [@@ocaml.doc ""]
...
type nonrec get_snowball_usage_result = {
  snowballs_in_use : integer option;
      [@ocaml.doc "The number of Snow devices that this account is currently using.\n"]
  snowball_limit : integer option;
      [@ocaml.doc
...
type nonrec get_snowball_usage_request = unit [@@ocaml.doc ""]
  ...
  unlock_code : string_ option;
      [@ocaml.doc
        "The [UnlockCode] value for the specified job. The [UnlockCode] value can be accessed for \
         up to 360 days after the job has been created.\n"]
}
[@@ocaml.doc ""]
...
type nonrec describe_return_shipping_label_result = {
  return_shipping_label_ur_i : string_ option;
      [@ocaml.doc "The pre-signed Amazon S3 URI used to download the return shipping label.\n"]
  expiration_date : timestamp option;
      [@ocaml.doc "The expiration date of the current return shipping label.\n"]
  status : shipping_label_status option;
      [@ocaml.doc
        "The status information of the task on a Snow device that is being returned to Amazon Web \
         Services.\n"]
}
[@@ocaml.doc ""]
...
type nonrec conflict_exception = {
  ...
  conflict_resource : string_ option;
      [@ocaml.doc
        "You get this resource when you call [CreateReturnShippingLabel] more than once when other \
         requests are not completed. .\n"]
}
[@@ocaml.doc
  "You get this exception when you call [CreateReturnShippingLabel] more than once when other \
   requests are not completed.\n"]
...
type nonrec cluster_metadata = {
  ...
  job_type : job_type option;
      [@ocaml.doc
        "The type of job for this cluster. Currently, the only job type supported for clusters is \
         [LOCAL_USE].\n"]
  cluster_state : cluster_state option; [@ocaml.doc "The current status of the cluster.\n"]
  role_ar_n : role_ar_n option;
      [@ocaml.doc
  ...
  description : string_ option; [@ocaml.doc "The optional description of the cluster.\n"]
  cluster_id : string_ option; [@ocaml.doc "The automatically generated ID for a cluster.\n"]
}
[@@ocaml.doc "Contains metadata about a specific cluster.\n"]
...
type nonrec describe_cluster_request = {
  cluster_id : cluster_id; [@ocaml.doc "The automatically generated ID for a cluster.\n"]
}
[@@ocaml.doc ""]
  ...
  next_token : string_ option;
      [@ocaml.doc
        "HTTP requests are stateless. If you use the automatically generated [NextToken] value in \
         ...
         point in the array.\n"]
  addresses : address_list option;
      [@ocaml.doc "The Snow device shipping addresses that were created for this account.\n"]
}
[@@ocaml.doc ""]
...
type nonrec describe_addresses_request = {
  ...
  max_results : list_limit option; [@ocaml.doc "The number of [ADDRESS] objects to return.\n"]
}
[@@ocaml.doc ""]
...
type nonrec describe_address_result = {
  address : address option;
      [@ocaml.doc
        "The address that you want the Snow device(s) associated with a specific job to be shipped \
         to.\n"]
}
[@@ocaml.doc ""]

type nonrec describe_address_request = {
  address_id : address_id; [@ocaml.doc "The automatically generated ID for a specific address.\n"]
}
[@@ocaml.doc ""]
...
type nonrec create_long_term_pricing_request = {
  snowball_type : snowball_type;
      [@ocaml.doc "The type of Snow Family devices to use for the long-term pricing job.\n"]
  is_long_term_pricing_auto_renew : java_boolean option;
      [@ocaml.doc
...
type nonrec create_job_request = {
  ...
  remote_management : remote_management option;
      [@ocaml.doc
        "Allows you to securely operate and manage Snowcone devices remotely from outside of your \
         internal network. When set to [INSTALLED_AUTOSTART], remote management will automatically \
         be available when the device arrives at your location. Otherwise, you need to use the \
         Snowball Edge client to manage the device. When set to [NOT_INSTALLED], remote management \
  ...
  snowball_type : snowball_type option;
      [@ocaml.doc
        ...
        \    \n\
        \     For more information, see \
         {{:https://docs.aws.amazon.com/snowball/latest/developer-guide/device-differences.html}Snowball \
         Edge Device Options} in the Snowball Edge Developer Guide.\n\
        \     \n\
        \      For more information, see \
         \"https://docs.aws.amazon.com/snowball/latest/snowcone-guide/snow-device-types.html\" \
         (Snow Family Devices and Capacity) in the {i Snowcone User Guide} or \
  ...
  cluster_id : cluster_id option;
      [@ocaml.doc
        "The ID of a cluster. If you're creating a job for a node in a cluster, you need to \
         provide only this [clusterId] value. The other job attributes are inherited from the \
         cluster.\n"]
  notification : notification option;
      [@ocaml.doc
  ...
  shipping_option : shipping_option option;
      [@ocaml.doc
        "The shipping speed for this job. This speed doesn't dictate how soon you'll get the Snow \
         device, rather it represents how quickly the Snow device moves to its destination while \
  ...
  snowball_capacity_preference : snowball_capacity option;
      [@ocaml.doc
        "If your job is being created in one of the US regions, you have the option of specifying \
         what size Snow device you'd like for this job. In all other regions, Snowballs come with \
  ...
  role_ar_n : role_ar_n option;
      [@ocaml.doc
        "The [RoleARN] that you want to associate with this job. [RoleArn]s are created using the \
         {{:https://docs.aws.amazon.com/IAM/latest/APIReference/API_CreateRole.html}CreateRole} \
         Identity and Access Management (IAM) API action.\n"]
  kms_key_ar_n : kms_key_ar_n option;
      [@ocaml.doc
        "The [KmsKeyARN] that you want to associate with this job. [KmsKeyARN]s are created using \
         the {{:https://docs.aws.amazon.com/kms/latest/APIReference/API_CreateKey.html}CreateKey} \
  ...
  resources : job_resource option;
      [@ocaml.doc
        ...
        \  With [EXPORT] jobs, you specify the bucket or buckets that your transferred data will \
         ...
         sorted.\n\
        \  "]
  job_type : job_type option; [@ocaml.doc "Defines the type of job that you're creating. \n"]
}
[@@ocaml.doc ""]
  ...
  job_list_entries : job_list_entry_list option;
      [@ocaml.doc
        "List of jobs created for this cluster. For syntax, see \
         {{:http://amazonaws.com/snowball/latest/api-reference/API_ListJobs.html#API_ListJobs_ResponseSyntax}ListJobsResult$JobListEntries} \
         in this guide.\n"]
  cluster_id : cluster_id option; [@ocaml.doc "The automatically generated ID for a cluster.\n"]
}
[@@ocaml.doc ""]
...
type nonrec create_cluster_request = {
  ...
  long_term_pricing_ids : long_term_pricing_id_list option;
      [@ocaml.doc
        "Lists long-term pricing id that will be used to associate with jobs automatically created \
         for the new cluster.\n"]
  force_create_jobs : boolean_ option;
      [@ocaml.doc
        "Force to create cluster when user attempts to overprovision or underprovision a cluster. \
         A cluster is overprovisioned or underprovisioned if the initial size of the cluster is \
         more (overprovisioned) or less (underprovisioned) than what needed to meet capacity \
  ...
  remote_management : remote_management option;
      [@ocaml.doc
        "Allows you to securely operate and manage Snow devices in a cluster remotely from outside \
         ...
         automatically be available when the device arrives at your location. Otherwise, you need \
  ...
  role_ar_n : role_ar_n option;
      [@ocaml.doc
        "The [RoleARN] that you want to associate with this cluster. [RoleArn] values are created \
         by using the \
         {{:https://docs.aws.amazon.com/IAM/latest/APIReference/API_CreateRole.html}CreateRole} \
         API action in Identity and Access Management (IAM).\n"]
  kms_key_ar_n : kms_key_ar_n option;
  ...
  address_id : address_id;
      [@ocaml.doc "The ID for the address that you want the cluster shipped to.\n"]
  description : string_ option;
      [@ocaml.doc
...
type nonrec create_address_result = {
  address_id : string_ option;
      [@ocaml.doc
        "The automatically generated ID for a specific address. You'll use this ID when you create \
         a job to specify which address you want the Snow device for that job shipped to.\n"]
}
[@@ocaml.doc ""]

type nonrec create_address_request = {
  address : address; [@ocaml.doc "The address that you want the Snow device shipped to.\n"]
}
[@@ocaml.doc ""]

type nonrec cancel_job_result = unit [@@ocaml.doc ""]
...
type nonrec cancel_cluster_result = unit [@@ocaml.doc ""]
