...
let make_update_job_shipment_state_request ~shipment_state:(shipment_state_ : shipment_state)
...
let make_target_on_device_service ?transfer_option:(transfer_option_ : transfer_option option)
    ?service_name:(service_name_ : device_service_name option) () =
  ({ transfer_option = transfer_option_; service_name = service_name_ } : target_on_device_service)

let make_s3_resource
    ?target_on_device_services:(target_on_device_services_ : target_on_device_service_list option)
    ?key_range:(key_range_ : key_range option) ?bucket_arn:(bucket_arn_ : resource_ar_n option) () =
  ({
...
let make_lambda_resource ?event_triggers:(event_triggers_ : event_trigger_definition_list option)
...
let make_job_resource ?ec2_ami_resources:(ec2_ami_resources_ : ec2_ami_resource_list option)
...
let make_nfs_on_device_service_configuration ?storage_unit:(storage_unit_ : storage_unit option)
...
let make_tgw_on_device_service_configuration ?storage_unit:(storage_unit_ : storage_unit option)
    ?storage_limit:(storage_limit_ : storage_limit option) () =
  ({ storage_unit = storage_unit_; storage_limit = storage_limit_ }
...
let make_eks_on_device_service_configuration
  ...
  ({ eks_anywhere_version = eks_anywhere_version_; kubernetes_version = kubernetes_version_ }
...
let make_pickup_details ?device_pickup_id:(device_pickup_id_ : device_pickup_id option)
    ...
    ?identification_number:(identification_number_ : string_ option) ?email:(email_ : email option)
    ?phone_number:(phone_number_ : phone_number option) ?name:(name_ : string_ option) () =
  ({
...
let make_update_job_request ?pickup_details:(pickup_details_ : pickup_details option)
    ...
    ?notification:(notification_ : notification option) ?role_ar_n:(role_ar_n_ : role_ar_n option)
    ~job_id:(job_id_ : job_id) () =
  ({
...
let make_update_cluster_request ?forwarding_address_id:(forwarding_address_id_ : address_id option)
    ?notification:(notification_ : notification option)
    ?shipping_option:(shipping_option_ : shipping_option option)
    ?address_id:(address_id_ : address_id option)
    ?on_device_service_configuration:
      (on_device_service_configuration_ : on_device_service_configuration option)
    ?resources:(resources_ : job_resource option) ?description:(description_ : string_ option)
    ?role_ar_n:(role_ar_n_ : role_ar_n option) ~cluster_id:(cluster_id_ : cluster_id) () =
  ({
     forwarding_address_id = forwarding_address_id_;
     notification = notification_;
     shipping_option = shipping_option_;
...
let make_shipping_details ?outbound_shipment:(outbound_shipment_ : shipment option)
...
let make_dependent_service ?service_version:(service_version_ : service_version option)
    ?service_name:(service_name_ : service_name option) () =
  ({ service_version = service_version_; service_name = service_name_ } : dependent_service)

let make_list_service_versions_request ?next_token:(next_token_ : string_ option)
...
let make_address ?type_:(type__ : address_type option)
    ?is_restricted:(is_restricted_ : boolean_ option) ?phone_number:(phone_number_ : string_ option)
    ?postal_code:(postal_code_ : string_ option) ?country:(country_ : string_ option)
    ?landmark:(landmark_ : string_ option)
    ?prefecture_or_district:(prefecture_or_district_ : string_ option)
    ?state_or_province:(state_or_province_ : string_ option) ?city:(city_ : string_ option)
    ?street3:(street3_ : string_ option) ?street2:(street2_ : string_ option)
    ?street1:(street1_ : string_ option) ?company:(company_ : string_ option)
    ?name:(name_ : string_ option) ?address_id:(address_id_ : address_id option) () =
  ({
     type_ = type__;
...
let make_list_pickup_locations_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : list_limit option) () =
  ({ next_token = next_token_; max_results = max_results_ } : list_pickup_locations_request)

let make_list_long_term_pricing_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : list_limit option) () =
...
let make_job_list_entry ?description:(description_ : string_ option)
    ...
    ?snowball_type:(snowball_type_ : snowball_type option) ?job_type:(job_type_ : job_type option)
    ?is_master:(is_master_ : boolean_ option) ?job_state:(job_state_ : job_state option)
...
let make_compatible_image ?name:(name_ : string_ option) ?ami_id:(ami_id_ : string_ option) () =
  ({ name = name_; ami_id = ami_id_ } : compatible_image)

let make_list_compatible_images_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : list_limit option) () =
...
let make_list_cluster_jobs_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : list_limit option) ~cluster_id:(cluster_id_ : cluster_id) () =
  ({ next_token = next_token_; max_results = max_results_; cluster_id = cluster_id_ }
...
let make_job_logs ?job_failure_log_ur_i:(job_failure_log_ur_i_ : string_ option)
...
let make_job_metadata ?snowball_id:(snowball_id_ : string_ option)
    ...
    ?forwarding_address_id:(forwarding_address_id_ : address_id option)
    ?cluster_id:(cluster_id_ : string_ option) ?job_log_info:(job_log_info_ : job_logs option)
    ...
    ?address_id:(address_id_ : address_id option) ?role_ar_n:(role_ar_n_ : role_ar_n option)
    ?kms_key_ar_n:(kms_key_ar_n_ : kms_key_ar_n option) ?description:(description_ : string_ option)
    ?resources:(resources_ : job_resource option) ?creation_date:(creation_date_ : timestamp option)
    ?snowball_type:(snowball_type_ : snowball_type option) ?job_type:(job_type_ : job_type option)
    ?job_state:(job_state_ : job_state option) ?job_id:(job_id_ : string_ option) () =
  ({
...
let make_cluster_metadata
    ...
    ?notification:(notification_ : notification option)
    ?shipping_option:(shipping_option_ : shipping_option option)
    ?address_id:(address_id_ : address_id option) ?resources:(resources_ : job_resource option)
    ?creation_date:(creation_date_ : timestamp option)
    ?snowball_type:(snowball_type_ : snowball_type option) ?job_type:(job_type_ : job_type option)
    ?cluster_state:(cluster_state_ : cluster_state option)
    ?role_ar_n:(role_ar_n_ : role_ar_n option) ?kms_key_ar_n:(kms_key_ar_n_ : kms_key_ar_n option)
    ?description:(description_ : string_ option) ?cluster_id:(cluster_id_ : string_ option) () =
  ({
     on_device_service_configuration = on_device_service_configuration_;
     tax_documents = tax_documents_;
     forwarding_address_id = forwarding_address_id_;
...
let make_describe_addresses_request ?next_token:(next_token_ : string_ option)
    ?max_results:(max_results_ : list_limit option) () =
...
let make_create_job_request ?pickup_details:(pickup_details_ : pickup_details option)
    ?impact_level:(impact_level_ : impact_level option)
    ?long_term_pricing_id:(long_term_pricing_id_ : long_term_pricing_id option)
    ?remote_management:(remote_management_ : remote_management option)
    ?device_configuration:(device_configuration_ : device_configuration option)
    ...
    ?snowball_capacity_preference:(snowball_capacity_preference_ : snowball_capacity option)
    ?role_ar_n:(role_ar_n_ : role_ar_n option) ?kms_key_ar_n:(kms_key_ar_n_ : kms_key_ar_n option)
    ?address_id:(address_id_ : address_id option) ?description:(description_ : string_ option)
    ?on_device_service_configuration:
      (on_device_service_configuration_ : on_device_service_configuration option)
    ?resources:(resources_ : job_resource option) ?job_type:(job_type_ : job_type option) () =
  ({
     pickup_details = pickup_details_;
     impact_level = impact_level_;
     long_term_pricing_id = long_term_pricing_id_;
...
let make_create_cluster_request
    ...
    ~snowball_type:(snowball_type_ : snowball_type) ~address_id:(address_id_ : address_id)
    ~job_type:(job_type_ : job_type) () =
  ({
     snowball_capacity_preference = snowball_capacity_preference_;
