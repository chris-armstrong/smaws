...
[@@ocaml.doc
  "Cancels the specified job. You can only cancel a job before its [JobState] value changes to \
   [PreparingAppliance]. Requesting the [ListJobs] or [DescribeJob] action returns a job's \
...
[@@ocaml.doc
  "Creates an address for a Snow device to be shipped to. In most regions, addresses are validated \
   at the time of creation. The address you provide must be located within the serviceable area of \
   your region. If the address is invalid or unsupported, then an exception is thrown. If \
   providing an address as a JSON file through the [cli-input-json] option, include the full file \
...
[@@ocaml.doc
  "Creates a job to import or export data between Amazon S3 and your on-premises data center. Your \
   Amazon Web Services account must have the right trust policies and permissions in place to \
   create a job for a Snow device. If you're creating a job for a node in a cluster, you only need \
   to provide the [clusterId] value; the other job attributes are inherited from the cluster. \n\n\
  ...
  \    Availability of device types differ by Amazon Web Services Region. For more information \
   ...
   {{:https://aws.amazon.com/about-aws/global-infrastructure/regional-product-services/?p=ngi&loc=4}Amazon \
...
[@@ocaml.doc
  "Returns a specified number of [ADDRESS] objects. Calling this API in one of the US regions will \
   return addresses from the list of all addresses associated with this account in all US regions.\n"]
...
[@@ocaml.doc
  "Returns a link to an Amazon S3 presigned URL for the manifest file associated with the \
   ...
   request has been made. To access the manifest file after 60 minutes have passed, you'll have to \
  ...
  \ The manifest is an encrypted file that you can download after your job enters the \
   [WithCustomer] status. This is the only valid status for calling this API as the manifest and \
   [UnlockCode] code value are used for securing your device and should only be used when you have \
   the device. The manifest is decrypted by using the [UnlockCode] code value, when you pass both \
   values to the Snow device through the Snowball client when the client is started for the first \
   time. \n\
  \ \n\
  \  As a best practice, we recommend that you don't save a copy of an [UnlockCode] value in the \
  ...
  \  \n\
  \   The credentials of a given job, including its manifest file and unlock code, expire 360 days \
...
[@@ocaml.doc
  "Returns the [UnlockCode] code value for the specified job. A particular [UnlockCode] value can \
   be accessed for up to 360 days after the associated job has been created.\n\n\
  \ The [UnlockCode] value is a 29-character code with 25 alphanumeric characters and 4 hyphens. \
   This code is used to decrypt the manifest file when it is passed along with the manifest to the \
   Snow device through the Snowball client when the client is started for the first time. The only \
   valid status for calling this API is [WithCustomer] as the manifest and [Unlock] code values \
   are used for securing your device and should only be used when you have the device.\n\
  \ \n\
  \  As a best practice, we recommend that you don't save a copy of the [UnlockCode] in the same \
   location as the manifest file for that job. Saving these separately helps prevent unauthorized \
...
[@@ocaml.doc
  "Returns information about the Snow Family service limit for your account, and also the number \
   of Snow devices your account has in use.\n\n\
  \ The default service limit for the number of Snow devices that you can have at one time is 1. \
   If you want to increase your service limit, contact Amazon Web Services Support.\n\
  \ "]
...
[@@ocaml.doc
  "This action returns a list of the different Amazon EC2-compatible Amazon Machine Images (AMIs) \
   ...
   device. Currently, supported AMIs are based on the Amazon Linux-2, Ubuntu 20.04 LTS - Focal, or \
   Ubuntu 22.04 LTS - Jammy images, available on the Amazon Web Services Marketplace. Ubuntu 16.04 \
   LTS - Xenial (HVM) images are no longer supported in the Market, but still supported for use on \
...
[@@ocaml.doc
  "Returns an array of [JobListEntry] objects of the specified length. Each [JobListEntry] object \
   contains a job's state, a job's ID, and a value that indicates whether the job is a job part, \
   in the case of export jobs. Calling this API action in one of the US regions will return jobs \
   from the list of all jobs associated with this account in all US regions.\n"]
...
[@@ocaml.doc
  "While a cluster's [ClusterState] value is in the [AwaitingQuorum] state, you can update some of \
   the information associated with a cluster. Once the cluster changes to a different job state, \
   usually 60 minutes after the cluster being created, this action is no longer available.\n"]
