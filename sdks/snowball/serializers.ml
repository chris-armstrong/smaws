...
let shipment_state_to_yojson (x : shipment_state) =
  match x with RETURNED -> `String "RETURNED" | RECEIVED -> `String "RECEIVED"
...
let service_name_to_yojson (x : service_name) =
  match x with EKS_ANYWHERE -> `String "EKS_ANYWHERE" | KUBERNETES -> `String "KUBERNETES"
...
let long_term_pricing_list_entry_to_yojson (x : long_term_pricing_list_entry) =
  assoc_to_yojson
    [
      ...
      ("LongTermPricingEndDate", option_to_yojson timestamp__to_yojson x.long_term_pricing_end_date);
...
let address_type_to_yojson (x : address_type) =
  match x with AWS_SHIP -> `String "AWS_SHIP" | CUST_PICKUP -> `String "CUST_PICKUP"
...
let get_snowball_usage_result_to_yojson (x : get_snowball_usage_result) =
  assoc_to_yojson
    [
      ("SnowballsInUse", option_to_yojson integer__to_yojson x.snowballs_in_use);
...
let describe_return_shipping_label_result_to_yojson (x : describe_return_shipping_label_result) =
  assoc_to_yojson
    [
      ...
      ("ExpirationDate", option_to_yojson timestamp__to_yojson x.expiration_date);
