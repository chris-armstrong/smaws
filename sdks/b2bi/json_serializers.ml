...
let element_requirement_to_yojson (x : element_requirement) =
  match x with OPTIONAL -> `String "OPTIONAL" | MANDATORY -> `String "MANDATORY"
...
let x12_split_by_to_yojson (x : x12_split_by) =
  match x with NONE -> `String "NONE" | TRANSACTION -> `String "TRANSACTION"
...
let file_format_to_yojson (x : file_format) =
  match x with XML -> `String "XML" | JSON -> `String "JSON" | NOT_USED -> `String "NOT_USED"
...
let conversion_source_format_to_yojson (x : conversion_source_format) =
  match x with JSON -> `String "JSON" | XML -> `String "XML"
...
let line_terminator_to_yojson (x : line_terminator) =
  match x with CRLF -> `String "CRLF" | LF -> `String "LF" | CR -> `String "CR"
