...
let make_with_http_client (type http_client) ~(http : http_client) ~(config : Config.t) : http_client context =
  { http; config }
...
let extract_http_module (type http_client) 
    ...
    ~(context : http_client context) : 
    (module Http.Client_intf with type t = http_client) =
  http_module
