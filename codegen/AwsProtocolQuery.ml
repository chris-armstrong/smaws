...
let resolve_timestamp_format ?(member_traits : Trait.t list option = None)
  ...
  | Some f -> f
  | None -> (
    match find_fmt shape_traits with
    | Some f -> f
    | None -> Trait.TimestampFormatDateTime)
...
module Serialiser = struct
  ...
  let primitive_serialize_helper ?(member_traits : Trait.t list option = None)
    ...
    | "smithy.api#Long" | "smithy.api#BigInteger" -> Some "int_field"
  ...
  let func_longident ~namespace_resolver target_name =
    let symbol_transformer ~local x =
      if local then [serialiser_func_str x]
      else ["Query_serializers"; serialiser_func_str x]
    in
  ...
  let item_serializer_expr ~namespace_resolver ?(member_traits = None) ?(shape_traits = None)
    ...
    | Some helper ->
      ...
      B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "p"))
        (B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "v"))
           (serialize_call helper
              [(Nolabel, exp_ident "p"); (Nolabel, exp_ident "v")]))
    | None ->
  ...
  let member_value_expr ~namespace_resolver ~shape_resolver ~member_traits ~shape_traits
      ~path_expr value_expr target_name =
    match primitive_serialize_helper ~member_traits:(Some member_traits)
            ~shape_traits:(Some shape_traits) target_name with
    | Some helper ->
      serialize_call helper [(Nolabel, path_expr); (Nolabel, value_expr)]
    | None ->
      (* Look up the shape to handle flattened lists/maps specially *)
      let shape =
        Shape_resolver.find_shape_by_name ~name:target_name shape_resolver
      in
      (match shape with
      | Some (Shape.ListShape ls) when is_flattened (Some member_traits) ->
        ...
        ignore member_tag;  (* flattened lists don't use member_tag *)
      ...
      | Some (Shape.MapShape ms) when is_flattened (Some member_traits) ->
        ...
        serialize_call "flattened_map_to_query"
          [(Nolabel, const_str key_tag); (Nolabel, const_str val_tag);
           (Nolabel, key_f); (Nolabel, val_f);
  ...
  let generate_member_expr ~namespace_resolver ~shape_resolver
      (mem : Shape.member) =
    let open Trait in
    ...
    let field_access = B.pexp_field (exp_ident "x")
        (lident_noloc (SafeNames.safeMemberName mem.name)) in
    ...
    if is_required then
      member_value_expr ~namespace_resolver ~shape_resolver ~member_traits ~shape_traits
        ~path_expr field_access mem.target
    else begin
      ...
      B.pexp_match field_access
        [B.case
           ...
           ~rhs:(B.elist []);
         B.case
           ~lhs:(B.ppat_construct (lident_noloc "Some")
  ...
  let enum_func_body name (s : Shape.enumShapeDetails) =
    ...
    exp_fun_untyped "path"
      (B.pexp_fun Nolabel None
         (B.ppat_constraint (B.ppat_var (Location.mknoloc "x"))
            (B.ptyp_constr (lident_noloc type_name) []))
         (serialize_call "string_field"
            [(Nolabel, path_ident); (Nolabel, match_exp)]))
    ...
    exp_fun_untyped "path"
      (exp_fun_untyped "xs"
         (serialize_call "list_to_query"
            [(Nolabel, const_str member_tag); (Nolabel, elem_f);
             (Nolabel, path_ident); (Nolabel, exp_ident "xs")]))
  ...
  let set_func_body (x : Shape.setShapeDetails) ~namespace_resolver () =
    ...
    exp_fun_untyped "path"
      (exp_fun_untyped "xs"
         (serialize_call "list_to_query"
            [(Nolabel, const_str "member"); (Nolabel, elem_f);
             (Nolabel, path_ident); (Nolabel, exp_ident "xs")]))
    ...
    exp_fun_untyped "path"
      (exp_fun_untyped "pairs"
         (serialize_call "map_to_query"
            [(Nolabel, const_str key_tag); (Nolabel, const_str val_tag);
             (Nolabel, key_f); (Nolabel, val_f);
             (Nolabel, path_ident); (Nolabel, exp_ident "pairs")]))
  ...
  let structure_func_body name (descriptor : Shape.structureShapeDetails)
    ...
    let effective_type_name = if is_exception_type then SafeNames.safeTypeName name else type_name in
    let member_exprs =
      List.map descriptor.members
        ~f:(generate_member_expr ~namespace_resolver ~shape_resolver)
    in
    let body =
      B.pexp_apply (exp_ident "List.concat") [(Nolabel, B.elist member_exprs)]
    in
    exp_fun_untyped "path"
      (B.pexp_fun Nolabel None
         (B.ppat_constraint (B.ppat_var (Location.mknoloc "x"))
  ...
  let generate_func_body (shapeWithTarget : Dependencies.shapeWithTarget)
    ...
    | ListShape x ->
      Some (list_func_body x ~namespace_resolver ())
    | SetShape x ->
      Some (set_func_body x ~namespace_resolver ())
    | MapShape x ->
    ...
    |> List.filter_map ~f:(fun (shapeWithTarget : Dependencies.shapeWithTarget) ->
           ...
           let shape =
             Option.value_map func_body ~default:[] ~f:(fun func_body ->
                 [B.value_binding ~pat:(B.ppat_var (Location.mknoloc serialiser_name))
                    ~expr:func_body])
           in
           let all_shapes =
             ...
             | Some recursWith ->
               let recurs =
                 List.filter_map recursWith
                   ~f:(fun (swt : Dependencies.shapeWithTarget) ->
...
module Deserialiser = struct
  ...
  let read_sequence tag body =
    xml_call xml_read_mod "sequence"
      [(Nolabel, exp_ident "i"); (Nolabel, const_str tag);
       (Nolabel, B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "i"))
  ...
  (* skip_element i *)
  let skip_element_expr =
    xml_call xml_read_mod "skip_element" [(Nolabel, exp_ident "i")]
  ...
  let parse_primitive_from_string target_name str_expr =
    ...
    | "smithy.api#Integer" | "smithy.api#Byte" | "smithy.api#Short"
    | "smithy.api#Long" | "smithy.api#BigInteger" ->
    ...
    | "smithy.api#Timestamp" ->
      ...
      Some (B.pexp_apply
              (B.pexp_ident (Location.mknoloc
                 (make_lident ~names:["Ptime"; "of_rfc3339"])))
              [(Nolabel, str_expr)]
           |> fun e ->
             B.pexp_apply (exp_ident "Result.get_ok") [(Nolabel, e)]
           |> fun e ->
             B.pexp_let Nonrecursive
               [B.value_binding ~pat:(B.ppat_tuple
                  [B.ppat_var (Location.mknoloc "ts"); B.ppat_any; B.ppat_any])
                  ~expr:e]
               (exp_ident "ts"))
    | _ -> None
  ...
  let member_reader_expr ~namespace_resolver ~shape_resolver xml_tag target_name ref_name =
    let assign v_expr =
      B.pexp_apply
        ...
        [(Nolabel, B.pexp_apply (exp_ident "( ! )") [(Nolabel, exp_ident ref_name)]
                   |> fun _ -> B.pexp_ident (Location.mknoloc (Longident.Lident ref_name)));
    ...
    | None ->
          ...
          | Some _ ->
            ...
            | _ ->
              B.pexp_apply
                ...
                [(Nolabel, B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "s"))
                    (Option.value_exn (parse_primitive_from_string ls.target (exp_ident "s"))));
                 (Nolabel, elements)])
          | None ->
            (* List of complex types *)
            let item_func = B.pexp_ident (Location.mknoloc (func_longident ~namespace_resolver ls.target)) in
      ...
      | Some (Shape.MapShape ms) ->
        ...
        let entry_body =
          ...
          let val_expr =
            ...
            | Some e -> e
            | None ->
              let vf = B.pexp_ident (Location.mknoloc (func_longident ~namespace_resolver ms.mapValue.target)) in
      ...
      | _ ->
        ...
        let item_func = B.pexp_ident (Location.mknoloc (func_longident ~namespace_resolver target_name)) in
        assign (read_sequence xml_tag
          (B.pexp_apply item_func [(Nolabel, exp_ident "i")])))
    ...
    if List.is_empty members then
      ...
      let cases =
        List.map members ~f:(fun (mem : Shape.member) ->
          ...
          in
          B.case
            ~lhs:(pat_const_str xml_tag)
            ~guard:None
            ~rhs:reader)
      in
      let wildcard_case =
        B.case ~lhs:B.ppat_any ~guard:None ~rhs:skip_element_expr
      in
      let scan_call =
        xml_call xml_struct_mod "scanSequence"
          [(Nolabel, exp_ident "i");
           (Nolabel, B.elist (List.map xml_tags ~f:const_str));
           (Nolabel, B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "tag"))
      ...
      let typed_record =
        B.pexp_constraint record_expr type_name
      in
      let body =
        List.fold_right ref_bindings
          ~init:(B.pexp_sequence scan_call typed_record)
        ...
        | _ ->
          B.pexp_apply
            ...
            [(Nolabel, B.pexp_fun Nolabel None (B.ppat_var (Location.mknoloc "s"))
             ...
             (Nolabel, elements)])
      | None ->
        let item_func = B.pexp_ident (Location.mknoloc (func_longident ~namespace_resolver x.target)) in
        read_sequences member_tag
          (B.pexp_apply item_func [(Nolabel, exp_ident "i")])
    in
    exp_fun_untyped "i" body
  ...
  let map_func_body (x : Shape.mapShapeDetails) ~namespace_resolver () =
    ...
    let entry_body =
      ...
      let val_expr =
        ...
        | Some e -> e
        | None ->
          let vf = B.pexp_ident (Location.mknoloc (func_longident ~namespace_resolver x.mapValue.target)) in
          B.pexp_apply vf [(Nolabel, exp_ident "i")]
      in
      B.pexp_tuple [key_expr; val_expr]
    in
  ...
  let enum_func_body name (s : Shape.enumShapeDetails) ~namespace_resolver () =
    ...
    let failure_case =
      B.case ~lhs:B.ppat_any ~guard:None
        ~rhs:[%expr failwith "unknown enum value"]
    in
    ...
    exp_fun_untyped "i"
      (B.pexp_let Nonrecursive
         [B.value_binding ~pat:(B.ppat_var (Location.mknoloc "s"))
  ...
  let generate_func_body (shapeWithTarget : Dependencies.shapeWithTarget)
    ...
    match shapeWithTarget.descriptor with
    | StructureShape { members = []; _ } ->
      Some (exp_fun_untyped "i" unit_expr)
    | StructureShape x ->
      Some (structure_func_body shapeWithTarget.name x ~namespace_resolver ~shape_resolver ())
    | ListShape x ->
      Some (list_func_body x ~namespace_resolver ~shape_resolver ())
    | SetShape x ->
      Some (set_func_body x ~namespace_resolver ~shape_resolver ())
    | MapShape x ->
      Some (map_func_body x ~namespace_resolver ())
    | EnumShape s ->
...
module Operations = struct
  ...
  let generate_error_to_string ~(operation_shape : Ast.Shape.operationShapeDetails)
    ...
    let handler_body =
      let default_handler =
        B.pexp_ident
          (Location.mknoloc (make_lident ~names:(awsquery_mod @ ["error_to_string"])))
      in
      match errors with
      | [] -> default_handler
      | errors ->
        ...
        let default_case =
          B.case
            ~lhs:(B.ppat_alias
                    (B.ppat_type
                       (Location.mknoloc
                          (make_lident ~names:(awsquery_mod @ ["error"]))))
                    (Location.mknoloc "e"))
            ~guard:None
    ...
    B.pstr_value Nonrecursive
      [B.value_binding ~pat:(B.ppat_var (Location.mknoloc "error_to_string"))
         ~expr:handler_body]
  ...
  let generate_error_handler ~(operation_shape : Ast.Shape.operationShapeDetails)
    ...
    let default_handler =
      B.pexp_ident
        (Location.mknoloc
           (make_lident ~names:(awsquery_mod @ ["Errors"; "default_handler"])))
    in
    let body =
      if List.is_empty errors then
        ...
        let cases =
          ...
          |> List.map ~f:(fun error ->
               ...
               let expression =
                 B.pexp_apply default_handler [(Nolabel, exp_ident "error")]
               in
  ...
  let generate_request_handler ~name ~operation_name
    ...
    let output_deserializer =
      ...
      |> Option.value_map
           ~default:(B.pexp_ident (Location.mknoloc (make_lident ~names:["Query_deserializers"; "unit_of_xml"])))
           ~f:(fun output_name ->
             let sym_transformer ~local x =
    ...
    let shape_func_body =
      [%expr
        ...
        [%e request_func]
          ~action:[%e const_str action_name]
          ~xmlNamespace:[%e const_str xml_namespace]
          ~service ~context ~fields
          ~output_deserializer:[%e output_deserializer]
          ~error_deserializer]
    in
    let shape_func =
  ...
  let generate_operation_module ~name ~operation_name ~operation_shape ~dependencies
      ~alias_context ~xml_namespace
      ~(namespace_resolver : Namespace_resolver.Namespace_resolver.t) () =
    ...
    let request_handler =
      generate_request_handler ~name ~operation_name ~operation_shape ~alias_context
        ~xml_namespace ~namespace_resolver ()
    in
    ...
    B.pstr_module
      (B.module_binding ~name:(Location.mknoloc (Some module_name)) ~expr:module_expr)
  ...
  let generate_error_type alias_ctx errors ~namespace_resolver () =
    generate_error_constructor_list alias_ctx errors ~namespace_resolver ()
    |> fun cs -> B.ptyp_variant cs Open None

  let generate_error_to_string_type alias_ctx errors ~namespace_resolver () =
    generate_error_constructor_list alias_ctx errors ~namespace_resolver ()
    |> fun cs -> B.ptyp_variant cs Closed None
