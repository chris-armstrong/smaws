...
let load_ini_file_res file_path =
  try Ok (Ini_generic.load_ini file_path)
  with e ->
    Error (AuthError (Fmt.str "Unable to load ~/.aws/credentials: %s" (Printexc.to_string e)))

let load_credentials_profiles =
  let home_dir = Sys.getenv "HOME" in
  let path = Filename.concat (Filename.concat home_dir ".aws") "credentials" in
  load_ini_file_res path

let load_config_profiles =
  let home_dir = Sys.getenv "HOME" in

  let path = Filename.concat (Filename.concat home_dir ".aws") "config" in
  load_ini_file_res path
  |> Result.map (fun kvps ->
         List.filter_map
...
let resolve ?profile_name () =
  try
    ...
    let credentials_file = load_credentials_profiles in
    let config_file = load_config_profiles in
