...
let of_int n =
  if n < 0 then raise (Invalid_argument "UInt64.of_int: negative") else Int64.of_int n
...
let udivmod x y =
  if Int64.equal y 0L then raise Division_by_zero
    ...
    for i = 63 downto 0 do
      ...
      r_lo := Int64.logor (Int64.shift_left !r_lo 1) (Int64.logand (Int64.shift_right_logical x i) 1L);
      r_hi := carry;
      (* [r = r_hi * 2^64 + r_lo]; [r >= y] iff the carry is set, or [r_lo >= y]
         (unsigned) when the carry is clear. *)
      let ge =
        !r_hi > 0
        || Int64.compare (Int64.add !r_lo Int64.min_int) (Int64.add y Int64.min_int) >= 0
      in
      if ge then begin
...
let set_high x h =
  Int64.logor (Int64.shift_left (Int64.logand (Int64.of_int32 h) word32_mask) 32)
