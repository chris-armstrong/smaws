...
module Request = struct
  ...
  let make (type http_t response_t body_t)
      (module Http : Http.Client_intf
        with type t = http_t
         and type Response.t = response_t
         and type Body.t = body_t) ~(service : Service.descriptor) ~config ~action
      ~(fields : field option list) ~http =
    ...
    in
    Http.request ~method_:`POST ~headers ~body:(`String body) ~uri http
end

let request (type http_t) (module Http : Http.Client_intf with type t = http_t) ~service ~config
    ~http ~action ~xmlNamespace ~fields ~output_deserializer ~error_deserializer =
