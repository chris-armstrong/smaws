...
let parse_shapes_from_json json_content =
  try
    ...
    | Ok parsed_shapes ->
        ...
        let shapes_map =
          List.fold parsed_shapes
            ~init:(Map.empty (module String))
            ~f:(fun acc shape -> Map.set acc ~key:shape.Shape.name ~data:shape)
        in
        Ok shapes_map
...
let is_error_status_code code = code >= 400 && code <= 599
...
let rec generate_value_constructor shapes shape_descriptor json_value ~is_required =
  ...
  | Shape.BlobShape _, `String s when is_required ->
      Ok (Printf.sprintf "%S" s) (* Blob as base64 string *)
  | Shape.BlobShape _, `String s -> Ok (Printf.sprintf "Some %S" s) (* Blob as base64 string *)
  | Shape.TimestampShape _, `String s when is_required ->
      Ok (Printf.sprintf "%S" s) (* Timestamp as ISO string *)
  | Shape.TimestampShape _, `String s ->
      Ok (Printf.sprintf "Some %S" s) (* Timestamp as ISO string *)
  | Shape.TimestampShape _, `Int i when is_required ->
      Ok (Printf.sprintf "(Int.to_string %d)" i) (* Unix timestamp *)
  | Shape.TimestampShape _, `Int i ->
      Ok (Printf.sprintf "Some (Int.to_string %d)" i) (* Unix timestamp *)
  ...
  | Shape.ListShape { target; _ }, `List values -> (
      ...
      | Ok member_descriptor -> (
          let constructor_results =
            List.map values
              ~f:(generate_value_constructor shapes member_descriptor ~is_required:true)
          in
          match
            List.fold_result constructor_results ~init:[] ~f:(fun acc result ->
                match result with Ok value -> Ok (value :: acc) | Error e -> Error e)
          with
          | Ok constructors when is_required ->
              Ok (Printf.sprintf "[%s]" (String.concat ~sep:"; " (List.rev constructors)))
          | Ok constructors ->
              Ok (Printf.sprintf "Some [%s]" (String.concat ~sep:"; " (List.rev constructors)))
          | Error e -> Error e)
      | Error e -> Error e)
  | Shape.MapShape { mapValue = { target; _ }; _ }, `Assoc assoc -> (
      ...
      | Ok value_descriptor -> (
          let constructor_results =
            List.map assoc ~f:(fun (key, value) ->
                match
                  generate_value_constructor shapes value_descriptor value ~is_required:true
                with
          ...
          match
            List.fold_result constructor_results ~init:[] ~f:(fun acc result ->
                match result with Ok pair -> Ok (pair :: acc) | Error e -> Error e)
          with
          | Ok pairs when is_required ->
              Ok (Printf.sprintf "[%s]" (String.concat ~sep:"; " (List.rev pairs)))
  ...
  | Shape.EnumShape { members; _ }, `String enum_value ->
      ...
      if List.mem enum_names enum_value ~equal:String.equal then
        if is_required then Ok enum_value else Ok (Printf.sprintf "Some %s" enum_value)
      else Error (InvalidParams ("Invalid enum value: " ^ enum_value))
  | _, _ when is_required -> Error (InvalidParams "Missing required field value")
  ...
  let field_constructors =
    List.map members ~f:(fun member ->
        ...
        | Some json_value -> (
            ...
            | Ok member_descriptor -> (
                match
                  generate_value_constructor shapes member_descriptor json_value
                    ~is_required:member_is_required
                with
        ...
        | None -> Ok (Printf.sprintf "%s = None" ocaml_field_name))
  in
  match
    List.fold_result field_constructors ~init:[] ~f:(fun acc result ->
        match result with Ok field -> Ok (field :: acc) | Error e -> Error e)
  with
  | Ok fields -> Ok (Printf.sprintf "{ %s }" (String.concat ~sep:"; " (List.rev fields)))
  | Error e -> Error e

(* Generate input constructor function for a test case *)
let generate_input_constructor shapes operation_name input_shape_target
          ...
          | Shape.StructureShape { members; _ } -> (
              ...
              | `Assoc assoc -> (
                  ...
                  | Ok field_constructor ->
                      Ok
                        (Printf.sprintf "let make_%s_input () = %s.%s" test_case.id simple_name
                           field_constructor)
                  | Error e -> Error e)
              | _ -> Error (InvalidParams "Expected object for structure parameters"))
          | _ -> (
              match
                generate_value_constructor shapes shape_descriptor params_json ~is_required:true
              with
              | Ok constructor_expr ->
...
let generate_response_constructor shapes operation_name output_shape_target
      ...
      | Some params_json -> (
          ...
          | Shape.StructureShape { members; _ } -> (
              ...
              | `Assoc assoc -> (
                  ...
                  | Ok field_constructor ->
                      Ok
                        (Printf.sprintf "let make_%s_expected () = %s.%s" test_case.id simple_name
                           field_constructor)
                  | Error e -> Error e)
              | _ -> Error (InvalidParams "Expected object for structure parameters"))
          | _ -> (
...
let generate_error_response_constructor shapes operation_name error_shapes
    (test_case : Parselib.Ast.Trait.httpResponseTest) =
        ...
        let try_error_shape error_shape_name =
          ...
          | Ok shape_descriptor -> (
              ...
              | Shape.StructureShape { members; _ } -> (
                  ...
                  | `Assoc assoc -> (
                      match
                        generate_structure_constructor shapes members assoc ~is_required:false
                      with
                      | Ok field_constructor ->
                          Ok
                            (Printf.sprintf "let make_%s_error () = %s.%s" test_case.id simple_name
                               field_constructor)
                      | Error e -> Error e)
                  | _ -> Error (InvalidParams "Expected object for structure parameters"))
              | _ -> (
        ...
        | error_shape :: _ -> (
            ...
            | Ok shape_descriptor -> (
                ...
                | Shape.StructureShape _ ->
                    ...
                    Ok
                      (Printf.sprintf "let make_%s_error () = %s.default ()" test_case.id
                         simple_name)
  ...
  | Some operation_shape -> (
      ...
      | Shape.OperationShape { input = Some input_target; _ } ->
          ...
          List.fold_result request_tests ~init:[] ~f:(fun acc test_case ->
              match
                generate_input_constructor parsed_shapes operation_name input_target test_case
              with
...
let generate_operation_constructors shapes operation_json operation_name =
  ...
  | Some input_target ->
      ...
      in
      let test_cases =
        List.filter_map request_tests ~f:(fun test_json ->
            try
              Some
                {
                  Parselib.Ast.Trait.id = test_json |> member "id" |> to_string;
                  ...
                  headers =
                    (match test_json |> member "headers" with
                  ...
                  params = (match test_json |> member "params" with `Null -> None | p -> Some p);
                  vendorParams = None;
            ...
            | Some output_target ->
                List.fold_result response_tests ~init:[] ~f:(fun acc test_case ->
                    if is_error_status_code test_case.code then (
                      ...
                      | Some error_list -> (
                          match
                            generate_error_response_constructor parsed_shapes operation_name
                              error_list test_case
                          with
                          | Ok constructor -> Ok (constructor :: acc)
                          | Error e -> Error e)
                      | None ->
                          Error
                            (InvalidParams
                               ("Error status code but no error shapes defined for: "
                              ^ operation_name)))
                    else (
                      (* Handle normal response constructors *)
                      match
                        generate_response_constructor parsed_shapes operation_name output_target
                          test_case
                      with
                      | Ok constructor -> Ok (constructor :: acc)
                      | Error e -> Error e))
