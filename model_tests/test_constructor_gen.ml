...
let test_constructor_generation () =
  ...
  | Error err -> Stdio.printf "Failed to parse shapes: %s\n" (string_of_constructor_error err)
  | Ok shapes -> (
      ...
      match
        shapes_json |> Yojson.Basic.Util.member "aws.protocoltests.json10#SimpleScalarProperties"
      with
      | `Null -> Stdio.printf "Operation not found\n"
      | operation_json -> (
          ...
          | Ok constructors ->
              ...
              List.iter constructors ~f:(fun constructor -> Stdio.printf "- %s\n" constructor)
          | Error err ->
...
let test_response_constructor_generation () =
  ...
  | Ok shapes -> (
      ...
      match
        generate_complete_operation_constructors shapes
      ...
      | Error err -> (
          ...
          let test_response =
            {
              Parselib.Ast.Trait.id = "test_response";
              protocol = "awsJson1_0";
              code = 200;
              headers = None;
              body = None;
              bodyMediaType = None;
              params =
                Some (`Assoc [ ("stringValue", `String "test"); ("booleanValue", `Bool true) ]);
              vendorParams = None;
          ...
          match
            generate_response_constructor shapes "aws.protocoltests.json10#SimpleScalarProperties"
              "aws.protocoltests.json10#SimpleScalarPropertiesOutput" test_response
          with
          | Ok constructor ->
