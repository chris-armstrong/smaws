...
let extract_traits_from_operation op_json =
  try
    ...
    let request_tests = 
      ...
      | `Null -> []
      | tests -> 
        (match Parselib.Parse.Smithy.parseTestHttpRequestTests (Ok { tree = tests; path = "smithy.test#httpRequestTests" }) with
    ...
    let response_tests =
      ...
      | `Null -> []
      | tests ->
        (match Parselib.Parse.Smithy.parseTestHttpResponseTests (Ok { tree = tests; path = "smithy.test#httpResponseTests" }) with
...
let validate_protocol_compatibility expected_protocols test_protocol =
  if List.mem expected_protocols test_protocol ~equal:String.equal
  then Ok ()
...
let validate_test_completeness 
    (request_tests : Parselib.Ast.Trait.httpRequestTest list) 
    (response_tests : Parselib.Ast.Trait.httpResponseTest list) =
  let request_ids = List.map request_tests ~f:(fun t -> t.id) in
  let response_ids = List.map response_tests ~f:(fun t -> t.id) in
  let missing_responses = List.filter request_ids ~f:(fun id ->
    not (List.mem response_ids id ~equal:String.equal)) in
  let missing_requests = List.filter response_ids ~f:(fun id ->
    not (List.mem request_ids id ~equal:String.equal)) in
  ...
  | mrs, mqs ->
    Error (ValidationError ("Missing tests - responses: " ^ String.concat ~sep:", " mrs ^ 
                           ", requests: " ^ String.concat ~sep:", " mqs))
...
let extract_test_cases_from_json json_content =
  try
    ...
    let operations = shapes |> to_assoc |> List.filter ~f:(fun (_, shape) ->
        ...
        (match validate_all_request_tests, validate_all_response_tests with
         | Ok (), Ok () ->
           ...
           let all_protocols = 
             ...
             List.map response_tests ~f:(fun t -> t.protocol) in
           let unique_protocols = List.dedup_and_sort ~compare:String.compare all_protocols in
           
           (match unique_protocols with
            | [] -> Ok None
            | [protocol] when String.equal protocol "awsJson1_0" || String.equal protocol "awsJson1_1" ||
                              String.equal protocol "aws.protocols#awsJson1_0" || String.equal protocol "aws.protocols#awsJson1_1" ->
            ...
            | protocols -> Error (ValidationError ("Mixed protocols not supported: " ^ String.concat ~sep:", " protocols)))
