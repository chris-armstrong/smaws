...
let aws_query_date_time_with_negative_offset () =
  (Eio.Switch.run ~name:"AwsQueryDateTimeWithNegativeOffset") @@
    (fun sw ->
...
let aws_query_date_time_with_positive_offset () =
  (Eio.Switch.run ~name:"AwsQueryDateTimeWithPositiveOffset") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let datetime_offsets_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#DatetimeOffsets",
    [("AwsQueryDateTimeWithNegativeOffset", `Quick,
       aws_query_date_time_with_negative_offset);
    ("AwsQueryDateTimeWithPositiveOffset", `Quick,
      aws_query_date_time_with_positive_offset)])
...
let query_query_flattened_xml_map () =
  (Eio.Switch.run ~name:"QueryQueryFlattenedXmlMap") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         (let response = FlattenedXmlMap.request ctx () in
          ...
          | Ok result ->
              let expected =
                ({ my_map = (Some [("foo", FOO); ("baz", BAZ)]) } : Types.flattened_xml_map_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_flattened_xml_map_output
                   Types.equal_flattened_xml_map_output) "expected output"
                expected result
...
let query_query_flattened_xml_map_with_xml_name () =
  (Eio.Switch.run ~name:"QueryQueryFlattenedXmlMapWithXmlName") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         (let response = FlattenedXmlMapWithXmlName.request ctx () in
          ...
          | Ok result ->
              let expected =
                ({ my_map = (Some [("a", "A"); ("b", "B")]) } : Types.flattened_xml_map_with_xml_name_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_flattened_xml_map_with_xml_name_output
...
let flattened_xml_map_with_xml_name_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#FlattenedXmlMapWithXmlName",
    [("QueryQueryFlattenedXmlMapWithXmlName", `Quick,
       query_query_flattened_xml_map_with_xml_name)])
...
let flattened_xml_map_with_xml_namespace_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#FlattenedXmlMapWithXmlNamespace",
    [("QueryQueryFlattenedXmlMapWithXmlNamespace", `Quick,
       query_query_flattened_xml_map_with_xml_namespace)])
let aws_query_date_time_with_fractional_seconds () =
  (Eio.Switch.run ~name:"AwsQueryDateTimeWithFractionalSeconds") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let fractional_seconds_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#FractionalSeconds",
    [("AwsQueryDateTimeWithFractionalSeconds", `Quick,
       aws_query_date_time_with_fractional_seconds)])
let query_greeting_with_errors () =
  (Eio.Switch.run ~name:"QueryGreetingWithErrors") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_complex_error () =
  (Eio.Switch.run ~name:"QueryComplexError") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_customized_error () =
  (Eio.Switch.run ~name:"QueryCustomizedError") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         Mock.mock_response
           ...
           ~status:402 ~headers:[("Content-Type", "text/xml")] ();
         (let response = GreetingWithErrors.request ctx () in
          match response with
          | Error (`CustomCodeError e) ->
              let expected =
                ({ message = (Some "Hi") } : Types.custom_code_error) in
              check
                (Alcotest_http.testable_nan_aware Types.pp_custom_code_error
                   Types.equal_custom_code_error) "expected error" expected e
          | Error other ->
              failwith (GreetingWithErrors.error_to_string other)
          | Ok _ -> failwith "expected an error response, got Ok"))
let query_invalid_greeting_error () =
  (Eio.Switch.run ~name:"QueryInvalidGreetingError") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_ignores_wrapping_xml_name () =
  (Eio.Switch.run ~name:"QueryIgnoresWrappingXmlName") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         (let response = IgnoresWrappingXmlName.request ctx () in
          ...
          | Ok result ->
              let expected =
                ({ foo = (Some "bar") } : Types.ignores_wrapping_xml_name_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_ignores_wrapping_xml_name_output
                   Types.equal_ignores_wrapping_xml_name_output)
                "expected output" expected result
          | Error error ->
              failwith (IgnoresWrappingXmlName.error_to_string error)))
...
let query_no_input_and_no_output_with_response_metadata () =
  (Eio.Switch.run ~name:"QueryNoInputAndNoOutputWithResponseMetadata") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let no_input_and_no_output_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#NoInputAndNoOutput",
    ...
    ("QueryNoInputAndNoOutput", `Quick, query_no_input_and_no_output);
    ("QueryNoInputAndNoOutputWithResponseMetadata", `Quick,
          ...
          | Ok result ->
              let expected =
                ({
                 ...
                 } : Types.recursive_xml_shapes_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_recursive_xml_shapes_output
                   Types.equal_recursive_xml_shapes_output) "expected output"
                expected result
          | Error error ->
              failwith (RecursiveXmlShapes.error_to_string error)))
...
let query_simple_scalar_properties () =
  (Eio.Switch.run ~name:"QuerySimpleScalarProperties") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({
                 ...
                 } : Types.simple_scalar_xml_properties_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_simple_scalar_xml_properties_output
                   Types.equal_simple_scalar_xml_properties_output)
                "expected output" expected result
          | Error error ->
              failwith (SimpleScalarXmlProperties.error_to_string error)))
let aws_query_supports_na_n_float_outputs () =
  (Eio.Switch.run ~name:"AwsQuerySupportsNaNFloatOutputs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let aws_query_supports_infinity_float_outputs () =
  (Eio.Switch.run ~name:"AwsQuerySupportsInfinityFloatOutputs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let aws_query_supports_negative_infinity_float_outputs () =
  (Eio.Switch.run ~name:"AwsQuerySupportsNegativeInfinityFloatOutputs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let simple_scalar_xml_properties_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#SimpleScalarXmlProperties",
    ...
    ("AwsQuerySupportsNaNFloatOutputs", `Quick,
      aws_query_supports_na_n_float_outputs);
    ("AwsQuerySupportsInfinityFloatOutputs", `Quick,
      aws_query_supports_infinity_float_outputs);
    ("AwsQuerySupportsNegativeInfinityFloatOutputs", `Quick,
      aws_query_supports_negative_infinity_float_outputs)])
let query_xml_blobs () =
  (Eio.Switch.run ~name:"QueryXmlBlobs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({ data = (Some (Smaws_Lib.CoreTypes.Blob.of_string "value"))
                 } : Types.xml_blobs_output) in
              check
                (Alcotest_http.testable_nan_aware Types.pp_xml_blobs_output
                   Types.equal_xml_blobs_output) "expected output" expected
                result
...
let xml_blobs_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlBlobs",
    [("QueryXmlBlobs", `Quick, query_xml_blobs)])
let query_xml_empty_blobs () =
  (Eio.Switch.run ~name:"QueryXmlEmptyBlobs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         (let response = XmlEmptyBlobs.request ctx () in
          ...
          | Ok result ->
              let expected =
                ({ data = (Some (Smaws_Lib.CoreTypes.Blob.of_string "")) } : 
                Types.xml_blobs_output) in
              check
                (Alcotest_http.testable_nan_aware Types.pp_xml_blobs_output
                   Types.equal_xml_blobs_output) "expected output" expected
...
let query_xml_empty_self_closed_blobs () =
  (Eio.Switch.run ~name:"QueryXmlEmptySelfClosedBlobs") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let xml_empty_blobs_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlEmptyBlobs",
    ...
    ("QueryXmlEmptySelfClosedBlobs", `Quick,
      query_xml_empty_self_closed_blobs)])
let query_xml_empty_lists () =
  (Eio.Switch.run ~name:"QueryXmlEmptyLists") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({
                   ...
                   string_set = (Some []);
                   string_list = (Some [])
                 } : Types.xml_lists_output) in
              check
                (Alcotest_http.testable_nan_aware Types.pp_xml_lists_output
                   Types.equal_xml_lists_output) "expected output" expected
                result
...
let query_xml_empty_maps () =
  (Eio.Switch.run ~name:"QueryXmlEmptyMaps") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
         ...
         (let response = XmlEmptyMaps.request ctx () in
          ...
          | Ok result ->
              ...
              check
                (Alcotest_http.testable_nan_aware Types.pp_xml_maps_output
                   Types.equal_xml_maps_output) "expected output" expected
                result
          | Error error -> failwith (XmlEmptyMaps.error_to_string error)))
let query_xml_empty_self_closed_maps () =
  (Eio.Switch.run ~name:"QueryXmlEmptySelfClosedMaps") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_xml_enums () =
  (Eio.Switch.run ~name:"QueryXmlEnums") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({
                   ...
                   foo_enum2 = (Some ZERO);
                   foo_enum1 = (Some FOO)
                 } : Types.xml_enums_output) in
              check
                (Alcotest_http.testable_nan_aware Types.pp_xml_enums_output
                   Types.equal_xml_enums_output) "expected output" expected
                result
...
let xml_enums_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlEnums",
    [("QueryXmlEnums", `Quick, query_xml_enums)])
let query_xml_int_enums () =
  (Eio.Switch.run ~name:"QueryXmlIntEnums") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({
                   ...
                   int_enum2 = (Some B);
                   int_enum1 = (Some A)
                 } : Types.xml_int_enums_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_xml_int_enums_output
                   Types.equal_xml_int_enums_output) "expected output"
                expected result
...
let xml_int_enums_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlIntEnums",
    [("QueryXmlIntEnums", `Quick, query_xml_int_enums)])
let query_xml_lists () =
  (Eio.Switch.run ~name:"QueryXmlLists") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let xml_lists_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlLists",
    [("QueryXmlLists", `Quick, query_xml_lists)])
let query_xml_maps () =
  (Eio.Switch.run ~name:"QueryXmlMaps") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let xml_maps_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlMaps",
    [("QueryXmlMaps", `Quick, query_xml_maps)])
let query_query_xml_maps_xml_name () =
  (Eio.Switch.run ~name:"QueryQueryXmlMapsXmlName") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
                ...
                ({
                   my_map =
                     (Some
                        [("foo", { hi = (Some "there") });
                        ("baz", { hi = (Some "bye") })])
                 } : Types.xml_maps_xml_name_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_xml_maps_xml_name_output
                   Types.equal_xml_maps_xml_name_output) "expected output"
                expected result
...
let query_xml_namespaces () =
  (Eio.Switch.run ~name:"QueryXmlNamespaces") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
          ...
          | Ok result ->
              let expected =
                ({
                   nested =
                     (Some
                        { values = (Some ["Bar"; "Baz"]); foo = (Some "Foo")
                        })
                 } : Types.xml_namespaces_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_xml_namespaces_output
                   Types.equal_xml_namespaces_output) "expected output"
                expected result
...
let xml_namespaces_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlNamespaces",
    [("QueryXmlNamespaces", `Quick, query_xml_namespaces)])
let query_xml_timestamps () =
  (Eio.Switch.run ~name:"QueryXmlTimestamps") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
                   ...
                   normal =
                     (Some
                        (Option.get
                           (Smaws_Lib.CoreTypes.Timestamp.of_float_s
                              1398796238.)))
                 } : Types.xml_timestamps_output) in
              check
                (Alcotest_http.testable_nan_aware
                   Types.pp_xml_timestamps_output
                   Types.equal_xml_timestamps_output) "expected output"
                expected result
...
let query_xml_timestamps_with_date_time_on_target_format () =
  (Eio.Switch.run ~name:"QueryXmlTimestampsWithDateTimeOnTargetFormat") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_xml_timestamps_with_epoch_seconds_format () =
  (Eio.Switch.run ~name:"QueryXmlTimestampsWithEpochSecondsFormat") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_xml_timestamps_with_http_date_format () =
  (Eio.Switch.run ~name:"QueryXmlTimestampsWithHttpDateFormat") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let query_xml_timestamps_with_http_date_on_target_format () =
  (Eio.Switch.run ~name:"QueryXmlTimestampsWithHttpDateOnTargetFormat") @@
    (fun sw ->
       let module Mock = (val Http_mock.create_http_mock ()) in
         let http_type = ((module
           Mock) : (module Smaws_Lib.Http.Client with type t = Mock.t)) in
...
let xml_timestamps_test_suite : unit Alcotest.test =
  ("aws.protocoltests.query#XmlTimestamps",
    ...
    ("QueryXmlTimestampsWithDateTimeFormat", `Quick,
      query_xml_timestamps_with_date_time_format);
    ("QueryXmlTimestampsWithDateTimeOnTargetFormat", `Quick,
      query_xml_timestamps_with_date_time_on_target_format);
    ("QueryXmlTimestampsWithEpochSecondsFormat", `Quick,
      query_xml_timestamps_with_epoch_seconds_format);
    ("QueryXmlTimestampsWithEpochSecondsOnTargetFormat", `Quick,
      query_xml_timestamps_with_epoch_seconds_on_target_format);
    ("QueryXmlTimestampsWithHttpDateFormat", `Quick,
      query_xml_timestamps_with_http_date_format);
    ("QueryXmlTimestampsWithHttpDateOnTargetFormat", `Quick,
