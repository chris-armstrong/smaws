...
let make_xml_timestamps_output ?normal:(normal_ : Smaws_Lib.Smithy_api.Types.timestamp option)
...
let make_xml_namespaces_output ?nested:(nested_ : xml_namespace_nested option) () =
  ({ nested = nested_ } : xml_namespaces_output)

let make_xml_maps_xml_name_output ?my_map:(my_map_ : xml_maps_xml_name_output_map option) () =
...
let make_xml_lists_output ?string_list:(string_list_ : Shared.Types.string_list option)
...
let make_xml_int_enums_output ?int_enum1:(int_enum1_ : Shared.Types.integer_enum option)
...
let make_xml_enums_output ?foo_enum1:(foo_enum1_ : Shared.Types.foo_enum option)
...
let make_xml_blobs_output ?data:(data_ : Smaws_Lib.Smithy_api.Types.blob option) () =
...
let make_simple_scalar_xml_properties_output
    ...
    ?empty_string_value:(empty_string_value_ : Smaws_Lib.Smithy_api.Types.string_ option)
...
let make_simple_input_params_input ?foo:(foo_ : Smaws_Lib.Smithy_api.Types.string_ option)
...
let make_recursive_xml_shapes_output_nested2 ?bar:(bar_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?recursive_member:(recursive_member_ : recursive_xml_shapes_output_nested1 option) () =
  ({ bar = bar_; recursive_member = recursive_member_ } : recursive_xml_shapes_output_nested2)

let make_recursive_xml_shapes_output_nested1 ?foo:(foo_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?nested:(nested_ : recursive_xml_shapes_output_nested2 option) () =
  ({ foo = foo_; nested = nested_ } : recursive_xml_shapes_output_nested1)

let make_recursive_xml_shapes_output ?nested:(nested_ : recursive_xml_shapes_output_nested1 option)
    () =
  ({ nested = nested_ } : recursive_xml_shapes_output)

let make_query_timestamps_input
    ?normal_format:(normal_format_ : Smaws_Lib.Smithy_api.Types.timestamp option)
  ...
  ({ normal_format = normal_format_; epoch_member = epoch_member_; epoch_target = epoch_target_ }
    : query_timestamps_input)

let make_nested_struct_with_map ?map_arg:(map_arg_ : Shared.Types.string_map option) () =
  ({ map_arg = map_arg_ } : nested_struct_with_map)

let make_query_maps_input ?map_arg:(map_arg_ : Shared.Types.string_map option)
    ?renamed_map_arg:(renamed_map_arg_ : Shared.Types.string_map option)
    ?complex_map_arg:(complex_map_arg_ : complex_map option)
    ?map_with_xml_member_name:(map_with_xml_member_name_ : map_with_xml_name option)
    ?flattened_map:(flattened_map_ : Shared.Types.string_map option)
    ?flattened_map_with_xml_name:(flattened_map_with_xml_name_ : map_with_xml_name option)
    ?map_of_lists:(map_of_lists_ : map_of_lists option)
    ?nested_struct_with_map:(nested_struct_with_map_ : nested_struct_with_map option) () =
  ({
     map_arg = map_arg_;
...
let make_query_lists_input ?list_arg:(list_arg_ : Shared.Types.string_list option)
    ...
    ?list_arg_with_xml_name_member:(list_arg_with_xml_name_member_ : list_with_xml_name option)
    ?flattened_list_arg_with_xml_name:
      (flattened_list_arg_with_xml_name_ : list_with_xml_name option)
...
let make_struct_arg ?string_arg:(string_arg_ : Smaws_Lib.Smithy_api.Types.string_ option)
    ?other_arg:(other_arg_ : Smaws_Lib.Smithy_api.Types.boolean_ option)
    ?recursive_arg:(recursive_arg_ : struct_arg option) () =
  ({ string_arg = string_arg_; other_arg = other_arg_; recursive_arg = recursive_arg_ }
    : struct_arg)

let make_nested_structures_input ?nested:(nested_ : struct_arg option) () =
  ({ nested = nested_ } : nested_structures_input)

let make_ignores_wrapping_xml_name_output ?foo:(foo_ : Smaws_Lib.Smithy_api.Types.string_ option) ()
    =
...
let make_flattened_xml_map_output ?my_map:(my_map_ : Shared.Types.foo_enum_map option) () =
