    ...
    [
      ...
      | "httpDate" ->
          r_http_date :=
            Some
              (let ts, _, _ =
                 Result.get_ok (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "httpDate" ()))
               in
               ts)
      | "epochSecondsOnTarget" ->
      ...
      | "epochSeconds" ->
          r_epoch_seconds :=
            Some
              (let ts, _, _ =
                 Result.get_ok
                   (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "epochSeconds" ()))
               in
               ts)
      | "dateTimeOnTarget" ->
      ...
      | "dateTime" ->
          r_date_time :=
            Some
              (let ts, _, _ =
                 Result.get_ok (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "dateTime" ()))
               in
               ts)
      | "normal" ->
          r_normal :=
            Some
              (let ts, _, _ =
                 Result.get_ok (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "normal" ()))
               in
               ts)
    ...
    [
      ...
      | "flattenedListWithNamespace" ->
          r_flattened_list_with_namespace :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "flattenedListWithNamespace"
                 (fun i _ -> Smaws_Lib.Xml.Parse.Read.elements i "member" ())
                 ())
      | "flattenedListWithMemberNamespace" ->
          r_flattened_list_with_member_namespace :=
            Some
              (Smaws_Lib.Xml.Parse.Read.sequence i "flattenedListWithMemberNamespace"
                 (fun i _ -> Smaws_Lib.Xml.Parse.Read.elements i "member" ())
                 ())
      | "customName" ->
          r_flattened_list2 :=
...
let rec struct_arg_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "RecursiveArg"; "OtherArg"; "StringArg" ]
    (fun tag _ ->
      match tag with
      | "RecursiveArg" ->
          r_recursive_arg :=
...
let simple_input_params_input_of_xml i =
  ...
  let r_foo = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i
    [ "IntegerEnum"; "FooEnum"; "Qux"; "Boo"; "FloatValue"; "Bam"; "Baz"; "Bar"; "Foo" ]
    (fun tag _ ->
      match tag with
      | "IntegerEnum" ->
          r_integer_enum :=
...
let query_timestamps_input_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "epochTarget"; "epochMember"; "normalFormat" ]
    (fun tag _ ->
      match tag with
      | "epochTarget" ->
          r_epoch_target :=
      ...
      | "epochMember" ->
          r_epoch_member :=
            Some
              (let ts, _, _ =
                 Result.get_ok
                   (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "epochMember" ()))
               in
               ts)
      | "normalFormat" ->
          r_normal_format :=
            Some
              (let ts, _, _ =
                 Result.get_ok
                   (Ptime.of_rfc3339 (Smaws_Lib.Xml.Parse.Read.element i "normalFormat" ()))
               in
               ts)
      | _ -> Smaws_Lib.Xml.Parse.Read.skip_element i);
  ({
...
let map_with_xml_name_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "entry"
    (fun i _ ->
      (Smaws_Lib.Xml.Parse.Read.element i "K" (), Smaws_Lib.Xml.Parse.Read.element i "V" ()))
    ()

let map_of_lists_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "entry"
    (fun i _ ->
      (Smaws_Lib.Xml.Parse.Read.element i "key" (), Shared.Query_deserializers.string_list_of_xml i))
    ()
...
let complex_error_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i [ "Nested"; "TopLevel" ] (fun tag _ ->
      match tag with
      | "Nested" ->
          r_nested :=
            Some
