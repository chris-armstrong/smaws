...
let my_union_of_yojson (tree : t) path =
  ...
  (match key with
   ...
   | "timestampValue" ->
       TimestampValue (Smaws_Lib.Smithy_api.Json_deserializers.timestamp_of_yojson value_ path)
...
let rec kitchen_sink_of_yojson tree path =
  ...
  ({
     unix_timestamp =
       option_of_yojson
         (value_for_key Smaws_Lib.Smithy_api.Json_deserializers.timestamp_of_yojson "UnixTimestamp")
         _list path;
     timestamp =
       option_of_yojson
         (value_for_key Smaws_Lib.Smithy_api.Json_deserializers.timestamp_of_yojson "Timestamp")
         _list path;
     struct_with_json_name =
       option_of_yojson
