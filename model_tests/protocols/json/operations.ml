...
module KitchenSinkOperation = struct
  let error_to_string = function
    | `ErrorWithMembers _ -> "aws.protocoltests.json#ErrorWithMembers"
    | `ErrorWithoutMembers _ -> "aws.protocoltests.json#ErrorWithoutMembers"
    | #Smaws_Lib.Protocols.AwsJson.error as e -> Smaws_Lib.Protocols.AwsJson.error_to_string e

  let error_deserializer tree path =
    let handler handler tree path = function
      | _, "ErrorWithMembers" ->
          `ErrorWithMembers (Json_deserializers.error_with_members_of_yojson tree path)
      | _, "ErrorWithoutMembers" ->
          `ErrorWithoutMembers (Json_deserializers.error_without_members_of_yojson tree path)
      | _type -> handler tree path _type
    in
    Smaws_Lib.Protocols.AwsJson.(
      error_deserializer (handler Smaws_Lib.Protocols.AwsJson.Errors.default_handler) tree path)
...
module OperationWithOptionalInputOutput = struct
  ...
  let request context (request : operation_with_optional_input_output_input) =
...
module PutAndGetInlineDocuments = struct
  let error_to_string = Smaws_Lib.Protocols.AwsJson.error_to_string
  ...
  let request context (request : put_and_get_inline_documents_input_output) =
    ...
    Smaws_Lib.Protocols.AwsJson.request ~shape_name:"JsonProtocol.PutAndGetInlineDocuments" ~service
      ~context ~input
...
module SimpleScalarProperties = struct
  ...
  let request context (request : simple_scalar_properties_input_output) =
    ...
    Smaws_Lib.Protocols.AwsJson.request ~shape_name:"JsonProtocol.SimpleScalarProperties" ~service
      ~context ~input
