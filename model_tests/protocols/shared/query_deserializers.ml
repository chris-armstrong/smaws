open Types
let unit_of_xml _ = ()
let foo_union_of_xml _i =
  failwith "union xml deserialization not implemented"
...
let foo_enum_map_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "entry"
    (fun i _ ->
       ((Smaws_Lib.Xml.Parse.Read.element i "key" ()), (foo_enum_of_xml i)))
    ()
