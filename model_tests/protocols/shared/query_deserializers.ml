...
let foo_union_of_xml i =
  ...
  Smaws_Lib.Xml.Parse.Structure.scanSequence i ["integer"; "string"]
    (fun tag _ ->
       match tag with
       | "integer" ->
           r_integer :=
...
let union_set_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "member"
    (fun i _ -> foo_union_of_xml i) ()
let timestamp_set_of_xml i =
  List.map
    (fun s -> let (ts, _, _) = Result.get_ok (Ptime.of_rfc3339 s) in ts)
...
let text_plain_blob_of_xml i =
  Smaws_Lib.Protocols.AwsQuery.Deserialize.blob_of_string
    (Smaws_Lib.Xml.Parse.Read.data i)
let greeting_struct_of_xml i =
  let r_hi = ref None in
  Smaws_Lib.Xml.Parse.Structure.scanSequence i ["hi"]
    (fun tag _ ->
       match tag with
...
let structure_set_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "member"
    (fun i _ -> greeting_struct_of_xml i) ()
    ...
    (fun i _ ->
       ...
       let v =
         Smaws_Lib.Xml.Parse.Read.sequence i "value"
           (fun i _ -> string_list_of_xml i) () in
       (k, v)) ()
let sparse_string_map_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "entry"
    (fun i _ ->
       let k = Smaws_Lib.Xml.Parse.Read.element i "key" () in
       let v = Smaws_Lib.Xml.Parse.Read.element i "value" () in (k, v)) ()
let sparse_string_list_of_xml i =
  Smaws_Lib.Xml.Parse.Read.elements i "member" ()
...
let nested_string_list_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "member"
    (fun i _ -> string_list_of_xml i) ()
let long_set_of_xml i =
  List.map (fun s -> Smaws_Lib.CoreTypes.Int64.of_string s)
    (Smaws_Lib.Xml.Parse.Read.elements i "member" ())
...
let integer_enum_map_of_xml i =
  Smaws_Lib.Xml.Parse.Read.sequences i "entry"
    (fun i _ ->
       ...
       let v =
         Smaws_Lib.Xml.Parse.Read.sequence i "value"
           (fun i _ -> integer_enum_of_xml i) () in
       (k, v)) ()
...
let blob_set_of_xml i =
  List.map (fun s -> Bytes.of_string (Base64.decode_exn s))
    (Smaws_Lib.Xml.Parse.Read.elements i "member" ())
