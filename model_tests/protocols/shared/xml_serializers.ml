...
let union_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> foo_union_to_xml w item)) xs
...
let text_plain_blob_to_xml w v =
  text w (Base64.encode_exn (Bytes.to_string v))
let greeting_struct_to_xml w (x : greeting_struct) =
  ignore
    [(match x.hi with
      | None -> null w
      | Some v -> element w "hi" (fun w -> text w v))]
let structure_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> greeting_struct_to_xml w item))
    xs
let string_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> (fun w v -> text w v) w item))
    xs
    ...
    (fun (k, v) ->
       element w "entry"
         (fun w ->
            ...
            element w "value" (fun w -> (fun w v -> text w v) w v))) pairs
  ...
  List.iter
    (fun (k, v) ->
       element w "entry"
         (fun w ->
            element w "key" (fun w -> (fun w v -> text w v) w k);
            element w "value" (fun w -> string_list_to_xml w v))) pairs
...
let sparse_short_list_to_xml w xs =
  List.iter
    (fun item ->
       element w "member"
         (fun w -> (fun w v -> text w (string_of_int v)) w item)) xs
...
let nested_string_list_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> string_list_to_xml w item)) xs
let long_set_to_xml w xs =
  List.iter
    (fun item ->
       element w "member"
         (fun w ->
            (fun w v -> text w (Smaws_Lib.CoreTypes.Int64.to_string v)) w
              item)) xs
...
let integer_enum_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> integer_enum_to_xml w item)) xs
  ...
  List.iter
    (fun (k, v) ->
       element w "entry"
         (fun w ->
            element w "key" (fun w -> (fun w v -> text w v) w k);
            element w "value" (fun w -> integer_enum_to_xml w v))) pairs
...
let http_date_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> http_date_to_xml w item)) xs
...
let foo_enum_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> foo_enum_to_xml w item)) xs
  ...
  List.iter
    (fun (k, v) ->
       element w "entry"
         (fun w ->
            element w "key" (fun w -> (fun w v -> text w v) w k);
            element w "value" (fun w -> foo_enum_to_xml w v))) pairs
...
let date_time_set_to_xml w xs =
  List.iter
    (fun item -> element w "member" (fun w -> date_time_to_xml w item)) xs
...
let boolean_set_to_xml w xs =
  List.iter
    (fun item ->
       element w "member"
         (fun w -> (fun w v -> text w (string_of_bool v)) w item)) xs
...
let blob_set_to_xml w xs =
  List.iter
    (fun item ->
       element w "member"
         (fun w ->
            (fun w v -> text w (Base64.encode_exn (Bytes.to_string v))) w
              item)) xs
