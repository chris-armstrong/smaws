...
let make_service_trait sdk_id endpoint_prefix =
  ...
  ServiceTrait { sdkId = sdk_id; arnNamespace = None; cloudFormationName = None; 
                cloudTrailEventSource = None; endpointPrefix = Some endpoint_prefix }

(* Helper function to create a test shape *)
let make_test_shape name shape_type =
  Ast.Dependencies.{
    name = name;
    descriptor = shape_type;
    targets = [];
    recursWith = None;
  }
...
let test_partition_by_namespace () =
  let shapes = [
    (* Amplify namespace *)
    make_test_shape "com.amazonaws.amplify#AmplifyService" (ServiceShape { version = "2017-07-25"; operations = Some ["com.amazonaws.amplify#CreateApp"]; traits = Some [make_service_trait "Amplify" "amplify"] });
    make_test_shape "com.amazonaws.amplify#CreateApp" (OperationShape { input = None; output = None; errors = None; documentation = None; traits = None });
...
let test_namespace_resolver () =
  let resolver = Codegen.Namespace_resolver.Namespace_resolver.create 
  ...
  let local_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
  ...
  let cross_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
...
let test_builtin_types_not_partitioned () =
  ...
  let shapes = [
    make_test_shape "com.amazonaws.amplify#AmplifyService" (ServiceShape { version = "2017-07-25"; operations = Some []; traits = Some [make_service_trait "Amplify" "amplify"] });
...
let test_builtin_types_in_namespace_resolver () =
  ...
  let builtin_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "smithy.api#String" in
  check string "builtin reference" "string" builtin_ref;
  
  let builtin_bool = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "smithy.api#Boolean" in
  check string "builtin boolean" "bool" builtin_bool;

  let builtin_int = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "smithy.api#Integer" in
...
let test_optional_reference_resolution () =
  ...
  (* Test None case *)
  let none_result = Codegen.Namespace_resolver.Namespace_resolver.resolve_optional_reference 
    resolver None in
  check (option string) "none reference" None none_result;
  
  (* Test Some local case *)
  let local_result = Codegen.Namespace_resolver.Namespace_resolver.resolve_optional_reference 
    resolver (Some "com.amazonaws.amplify#CreateApp") in
  check (option string) "some local reference" (Some "CreateApp") local_result;
  
  (* Test Some cross-namespace case *)
  let cross_result = Codegen.Namespace_resolver.Namespace_resolver.resolve_optional_reference 
    resolver (Some "com.amazonaws.s3#Bucket") in
...
let test_unknown_namespace_error () =
  ...
  check_raises "unknown namespace error" 
    (Failure "Unknown namespace: com.amazonaws.unknown")
    (fun () -> 
      ignore (Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
        resolver "com.amazonaws.unknown#SomeType"))
...
let test_cross_namespace_structure_references () =
  ...
  let shapes = [
    ...
    make_test_shape "com.amazonaws.amplify#AmplifyService" (ServiceShape { version = "2017-07-25"; operations = Some ["com.amazonaws.amplify#CreateApp"]; traits = Some [make_service_trait "Amplify" "amplify"] });
    make_test_shape "com.amazonaws.amplify#CreateApp" (OperationShape { 
    ...
    make_test_shape "com.amazonaws.amplify#CreateAppRequest" (StructureShape { 
      members = [
    ...
    make_test_shape "com.amazonaws.amplify#CreateAppResponse" (StructureShape { 
      members = [{ name = "app"; target = "com.amazonaws.amplify#App"; traits = None }]; 
      traits = None 
    });
    make_test_shape "com.amazonaws.amplify#App" (StructureShape { 
      members = [
...
let test_suite = [
  ...
  test_case "cross_namespace_structure_references" `Quick test_cross_namespace_structure_references;
]
...
let () =
  run "Namespace Support Tests" [
    ("partition_by_namespace", [
      ...
      test_case "cross-namespace structure references" `Quick test_cross_namespace_structure_references;
    ]);
    ("namespace_resolver", [
