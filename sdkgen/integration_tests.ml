...
let test_synthetic_multi_namespace_generation () =
  ...
  if Stdlib.Sys.file_exists model_path then (
    ...
    | Ok context ->
        ...
        (match Sdkgen.create_from_model_file_with_namespaces ~namespace_module_mapping:namespace_mapping model_path with
...
let test_realistic_cross_namespace_resolution () =
  ...
  let s3_bucket_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "com.amazonaws.s3#BucketName" in
  check string "s3 bucket reference" "S3.BucketName" s3_bucket_ref;
  
  let s3_object_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "com.amazonaws.s3#ObjectKey" in
  ...
  let local_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
    resolver "com.amazonaws.amplify#CreateAppRequest" in
  check string "local reference" "CreateAppRequest" local_ref;
  
  (* Test builtin references resolve correctly *)
  let string_ref = Codegen.Namespace_resolver.Namespace_resolver.resolve_reference 
...
let test_existing_single_namespace_models () =
  ...
  if Stdlib.Sys.file_exists model_path then (
    ...
    check bool "backward compatibility test skipped (file not found)" true true
  )
...
let test_complete_generation_pipeline () =
  ...
  if Stdlib.Sys.file_exists model_path then (
    ...
    | Ok namespace_contexts ->
        ...
        let has_amplify = List.exists namespace_contexts ~f:(fun (namespace, _) ->
          String.equal namespace "com.amazonaws.amplify") in
        ...
        List.iter namespace_contexts ~f:(fun (namespace, context) ->
          ...
          check bool ("context has operations for " ^ namespace) true (List.length context.operation_shapes > 0);
          check bool ("context has service details for " ^ namespace) true (String.length context.service_details.sdkId > 0)
        )
    ...
    check bool "complete pipeline test skipped (file not found)" true true
  )
...
let test_real_aws_model () =
  ...
  let test_model (model_path, expected_namespace) =
    if Stdlib.Sys.file_exists model_path then (
      ...
      match Sdkgen.create_from_model_file model_path with
      | Ok context ->
          check bool ("real model " ^ model_path ^ " works with original function") true (String.length context.name > 0);
...
let test_error_handling () =
  ...
  if Stdlib.Sys.file_exists model_path then (
    ...
    (match Sdkgen.create_from_model_file_with_namespaces ~namespace_module_mapping:empty_mapping model_path with
    | Ok namespace_contexts ->
    ...
    let partial_mapping = Map.Poly.of_alist_exn [
      ("com.amazonaws.amplify", "Amplify");
      (* Deliberately omit smithy.api namespace *)
    ] in
    (match Sdkgen.create_from_model_file_with_namespaces ~namespace_module_mapping:partial_mapping model_path with
    | Ok namespace_contexts ->
        (* Should return only the mapped namespace *)
        check bool "partial mapping works" true (List.length namespace_contexts >= 0);
        let has_only_mapped = List.for_all namespace_contexts ~f:(fun (namespace, _) ->
          Map.mem partial_mapping namespace) in
    ...
    check bool "error handling test skipped (file not found)" true true
  )

let test_suite = [
  test_case "synthetic_multi_namespace_generation" `Quick test_synthetic_multi_namespace_generation;
  test_case "realistic_cross_namespace_resolution" `Quick test_realistic_cross_namespace_resolution;
...
let () =
  run "Integration Tests" [
    ("multi_namespace_pipeline", [
      test_case "synthetic multi-namespace generation" `Quick test_synthetic_multi_namespace_generation;
    ...
    ("cross_namespace_references", [
      test_case "realistic cross-namespace resolution" `Quick test_realistic_cross_namespace_resolution;
    ]);
    ("compatibility_tests", [
