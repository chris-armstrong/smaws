...
let generate_mli ~name ~service ~namespace_resolver ~operation_shapes ~structure_shapes
  ...
  Gen_types.generate_mli ~name ~service ~namespace_resolver ~structure_shapes ~alias_context
    ~no_open:true output_fmt;
  Fmt.pf output_fmt "(** {1:builders Builders} *)@\n@\n";
  Gen_builders.generate_mli ~name ~service ~namespace_resolver ~operation_shapes ~structure_shapes
