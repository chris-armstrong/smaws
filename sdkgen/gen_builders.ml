...
let generate ~name ~(service : Ast.Shape.serviceShapeDetails) ~operation_shapes ~structure_shapes
    ~alias_context fmt =
  let structure_shapes = Codegen.Builders.stri_builders ~alias_context ~structure_shapes in
...
let generate_mli ~name ~service ~operation_shapes ~structure_shapes ~alias_context
    ?(no_open = false) fmt =
  let structure_shapes = Codegen.Builders.sigi_builders ~alias_context ~structure_shapes in
