...
let generate_ml ~name ~(service : Ast.Shape.serviceShapeDetails) ~structure_shapes ~alias_context
  ...
  let sm = [] in
  let types =
    stri_structure_shapes ~with_derivings ~namespace_resolver alias_context structure_shapes fmt
  in
...
let generate_mli ~name ~(service : Ast.Shape.serviceShapeDetails) ~structure_shapes ~alias_context
  ...
  let sm = [] in
  let types =
    sigi_structure_shapes ~with_derivings ~namespace_resolver alias_context structure_shapes fmt
  in
